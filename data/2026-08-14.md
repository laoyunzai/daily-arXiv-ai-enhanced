...
- [physics.comp-ph](#physics.comp-ph) [Total: 15]
...
TL;DR: 本文证明任意[[n,k,d]]稳定子码可以等距嵌入到单个D维系统中并保持完整的纠错结构，同时推导出精确泄漏纠正的充要条件，建立多能级量子系统中量子纠错的统一框架。


<details>
  <summary>Details</summary>
Motivation: 在多能级量子硬件中，泄漏到计算子空间之外的错误是主要误差来源，需要理解如何在多能级系统中进行量子纠错。

Method: 通过等距嵌入方法将任意稳定子码嵌入单一D维系统，并推导泄漏可纠正的充要条件。

Result: 证明了任意[[n,k,d]]稳定子码可等距嵌入单一D维系统，并得出泄漏可纠正当且仅当泄漏不区分逻辑态。

Conclusion: 建立了多能级量子系统中量子纠错的统一理论框架，为多能级量子硬件的容错计算提供理论基础。
...
TL;DR: 本文提出一种基于纳米机械谐振器与应变敏感固态自旋的玻色子量子信息处理架构，利用自旋作为非线性元件实现机械猫态量子位，并论证该架构可实现片上的最近邻连接、通用量子计算、高空间密度逻辑量子位以及可扩展的量子信息处理路径。


<details>
  <summary>Details</summary>
Motivation: 纳米机械结构具有高品质因数，可用于射频波段的长期量子激发和玻色子编码，但机械模式本身缺乏非线性，无法实现玻色子信道间的相互作用，难以扩展到多量子位规模。因此需要引入额外机制来产生非线性相互作用，以实现玻色子量子计算的可扩展性。

Method: 该工作采用应变敏感固态自旋作为非线性元件，通过与纳米机械谐振器的耦合产生非经典机械态（猫态）。论文描述了实现机械猫态量子位片上最近邻连接所需的架构、实现通用量子计算所需的控制与读取架构，并利用玻色子纠错方案的高效性和纳米谐振器与自旋量子位的小尺寸实现高空间密度逻辑量子位。

Result: 研究表明该架构可以实现纳米机械猫态量子位的片上最近邻连通性及通用量子计算的完整控制和读取方案，并能实现高空间密度逻辑量子位。同时，论文识别了实现高量子位密度下纠错阈值所需的性能指标。

Conclusion: 作者论证了该架构为可扩展量子信息处理提供了可行路径，明确了达到错误纠正阈值所需的关键性能指标，为后续的实验实现和工程优化指明了方向。
...
TL;DR: 本文通过两种互补方法（左-右Cayley复形描述和"提升"视角）显式构造了具有良好码率和距离的量子Tanner（QT）码，并在中等码长区间（n∈[500,1000]）通过对GAP SmallGrp库中非阿贝尔群的广泛搜索，发现了多个距离上界超过20的新码实例，同时提供了Tesseract译码器下的噪声阈值估计和开源的Julia库QuantumExpanders.jl。


<details>
  <summary>Details</summary>
Motivation: 量子Tanner码作为有前途的量子LDPC码候选，需要在高码率和好距离之间取得平衡。已有构造在中等码长区间的具体码实例有限，且缺乏系统性的群搜索来发现最优参数。本文旨在通过大规模群搜索和两种互补构造视角，填补这一空白。

Method: 采用两种互补方法构造QT码：(1) 左-右Cayley复形（LRCC）描述；(2) "提升"视角，即通过有限群G的交换左-右正则作用提升一个种子CSS码。利用GAP SmallGrp库对非阿贝尔群进行广泛搜索，使用sQetch随机距离估计器（最多3.5亿次试验）估计距离上界，并用Tesseract译码器评估噪声阈值。

Result: 发现多个新码实例，包括[[480,8,(≤21,≤21)]]、[[504,4,(≤36,≤27)]]、[[672,4,(≤48,≤28)]]、[[720,6,(≤30,≤30)]]和[[864,8,(≤39,≤31)]]，检查权重范围为9-20。Tesseract译码器估计的伪阈值为：现象噪声下3.6%-4.6%，电路级噪声下0.14%-0.27%，与较短码长的先前结果相当。

Conclusion: 系统性群搜索证实了中等码长QT码可实现超过20的距离上界，且性能与更短码长相当。提供的开源Julia库QuantumExpanders.jl可用于构建QT码和显式Ramanujan图，为后续研究者提供实用工具。
...
TL;DR: 本文研究手性连续时间量子行走中规范不变的磁通如何影响单顶点顺序测量的干扰效应。作者证明完整的两时间统计量（含测量反作用和Leggett-Garg关联）完全由返回振幅决定，并发现了饱和Lüders界3/2的图无关充分机制——即磁通可将根植动力学约化为具有等谱权重的二维Krylov子空间。该机制在双通量金刚石图上精确实现，并通过环上的精确卷绕数展开揭示了宇称依赖的磁通对比起始阶数。


<details>
  <summary>Details</summary>
Motivation: 理解手性连续时间量子行走中规范不变的磁通（Peierls相位）如何作为资源来工程化局部测量反作用和时序量子关联是核心动机。作者希望建立时序相关性与局部谱测度及规范不变闭合行走干扰之间的精确联系，并寻找饱和Lüders界的不依赖具体图结构的充分条件。

Method: 作者聚焦于单一顶点的顺序测量，使用区分返回该顶点与占据其补集的两分观测值。通过初始局域化在测量顶点的步行者，他们用返回振幅精确确定完整的两时间统计量（含测量反作用和Leggett-Garg关联子）。采用短时间扰动展开、Krylov子空间约化分析，以及对磁通贯穿环路的精确卷绕数展开来揭示宇称依赖的磁通效应。

Result: 主要结果包括：（1）短时间主导扰动独立于Peierls相位，磁通敏感性在更高阶通过闭合行走干扰进入；（2）识别出图无关的饱和Lüders界机制：磁通将根植动力学约化为等谱权重的平衡二维Krylov子空间，在有限时间达到3/2界，并在双通量金刚石图上精确实现；（3）磁通环路给出宇称依赖起始阶：偶环为t^N阶、奇环为t^{2N}阶的磁通对比；（4）磁通可增强最大Leggett-Garg违背或将强违背移至更早测量时间，半通量驱动四站点环至Lüders界。

Conclusion: 该工作确立了规范不变的磁通作为工程化局部测量反作用与时序量子关联的可靠资源，提供了磁通工程设计的建设性判据，并为量子行走中的时序关联研究建立了基于返回振幅的统一分析框架。
...
TL;DR: 本文演示了用量子脉冲门（QPG）对多模参量下转换（PDC）单光子源进行相干时间滤波，实现了纯度高于0.90的标记光子，而传统光谱强度滤波只能产生混合态光子。


<details>
  <summary>Details</summary>
Motivation: 实际单光子源通常发射光谱多模态，导致纯度降低。传统光谱强度滤波虽能部分改善纯度，但无法选择明确定义的时间模式（TM），这限制了量子网络应用中高可见度干涉等量子信息协议的性能。

Method: 利用量子脉冲门（QPG）对具有强光谱关联的多模PDC源进行相干时间滤波，并与传统光谱强度滤波进行基准对比；采用chronocyclic Q函数层析技术测量光子纯度，并进一步演示了包括皮秒时间仓叠加态在内的结构化TM提取。

Result: QPG滤波无论滤波形状如何均能一致地产生纯度高于0.90的标记光子，而光谱强度滤波产生混合光子；QPG还能提取结构化时间模式，包括皮秒时间仓的叠加态。

Conclusion: QPG被确立为一种实用且普适的相干滤波工具，适用于量子网络应用。
...
TL;DR: 本文通过脉冲工程技术构建一个有效哈密顿量，使其在混沌的带杂质海森堡XXX链与ZX哈密顿量之间连续插值，发现多个具有不同纠缠和非稳定化性质的解析可解非典型本征态，并揭示量子混沌与量子资源生成之间的部分解耦。


<details>
  <summary>Details</summary>
Motivation: 非平衡量子物理中，理解并可控地调控相互作用量子多体系统的性质是核心挑战。虽然已有大量多体哈密顿量用于研究量子混沌、非典型本征态和量子资源，但在单一微观模型中系统地工程化并连续调谐这些性质仍未被充分探索。

Method: 采用脉冲工程方案构造有效哈密顿量，在混沌的Heisenberg (XXX)链加局域杂质与ZX哈密顿量之间实现连续插值。沿插值路径识别多族解析可解的非典型本征态，分析其纠缠与非稳定化性质，并考察脉冲工程对混沌与非混沌区域的保持及量子资源生成的影响。

Result: 在XXX极限发现纠缠和稳定子Rényi熵的精确平台，对应长程价键固体(VBS)态的相干叠加；随脉冲强度增加出现低能谱中的近似纠缠平台层级；在完全脉冲工程化的ZX极限发现一对长程纠缠的稳定子本征态（彩虹疤痕态）。脉冲工程保持原模型的混沌与非混沌区域，但两种区域中动态生成的纠缠和非稳定化资源几乎相同，揭示量子混沌与量子资源生成的解耦。

Conclusion: 脉冲工程是生成具有结构化本征态和可调谐量子资源的多体哈密顿量的通用框架，为研究量子混沌与量子资源之间的深层关系提供了新平台。
...
TL;DR: 本文构建了一个包含两个相互作用原子耦合到单模腔的Dicke最小模型，研究原子间相互作用对超辐射临界现象的影响，发现相互作用可改变相边界、完全抑制原子-光子耦合阈值并改变相变的普适性类别，同时研究了耗散通道下的耗散相变和量子动力学。


<details>
  <summary>Details</summary>
Motivation: 发射体之间的相互作用是控制集体光-物质现象的有力手段，但它们在超辐射临界性中的作用尚未被深入研究。本文旨在通过一个可解析求解的最小模型来系统探究原子间相互作用对超辐射相变的影响。

Method: 构建一个含两个相互作用原子耦合到单模腔的Dicke模型，通过解析方法研究相互作用对相边界、超辐射耦合阈值和普适性类别的影响，并进一步在光子损耗和自旋弛豫等耗散通道存在的情况下研究耗散相变与量子动力学。

Result: 研究发现原子间相互作用会改变相边界，甚至可能完全抑制超辐射所需的原子-光子耦合阈值，并改变相变的普适性类别。在耗散通道下也观察到相互作用对耗散相变和量子动力学的显著影响。

Conclusion: 原子间相互作用在超辐射临界现象中扮演重要角色，可通过工程设计原子间相互作用来控制原子-光子耦合行为，为调控集体光-物质现象提供了新的途径。
...
TL;DR: 本文研究大规模自由空间量子网络的端到端性能，将用户建模为Waxman随机图，推导了单路径和多路径路由下的平均网络容量、边消耗和连通性相变；同时分析了以路由器为中心的星型网络的端到端容量分布。进一步提出用少量光纤骨干节点增强自由空间网络，证明最优骨干配置等价于容量最大化的Voronoi剖分，可通过Lloyd算法和Delaunay三角剖分高效近似求解。


<details>
  <summary>Details</summary>
Motivation: 自由空间量子通信是对光纤量子网络的重要补充，但其点对点容量受衍射、大气消光和湍流引起的波束漂移等物理因素根本性限制。现有研究缺乏对大规模自由空间量子网络端到端性能的系统分析，作者旨在填补这一空白，并探索如何通过少量光纤骨干节点有效增强网络性能。

Method: 将随机分布的固定或移动用户建模为Waxman随机图，分别推导单路径和多路径（洪泛）路由下的平均网络容量、边消耗及连通性相变；推导路由器中心星型网络的端到端容量完整分布。证明最优骨干节点配置对应于容量最大化的Voronoi剖分，利用Lloyd算法通过质心Voronoi剖分近似求解，骨干节点间以Delaunay三角剖分连接。

Result: 数值结果表明，即使是数量适度的光纤骨干节点也能显著提升固定和移动用户的端到端容量，并降低边消耗。研究同时给出了网络容量、边消耗和连通性相变的解析结果，以及星型网络的容量分布特征。

Conclusion: 以光纤骨干节点增强自由空间量子网络是一种高效且可扩展的策略；最优骨干部署可通过Voronoi剖分与Lloyd算法在多项式时间内近似实现，为大规模混合量子网络设计提供了理论指导。
...
TL;DR: 本文提出一种基于拓扑数据分析（TDA）的参考拓扑后选择预言机，用于克服TF-QKD在低轨星座动态非IID轨道环境中因严格独立性条件违反或二阶惩罚导致的密钥率崩溃问题。仿真显示TDA在2000km处提高条件候选速率，但未扩展正速率范围，且完整可组合安全证明尚未完成，认证可组合密钥率仍为零。


<details>
  <summary>Details</summary>
Motivation: 星际链路是全球量子网络的关键骨干，TF-QKD虽突破线性率损界限，但其对相位极度敏感，难以应对动态非IID轨道环境。在广义熵累积定理（GEAT）框架下，传统自适应后选择启发式方法要么违反严格独立条件，要么承受巨大二阶惩罚导致秘密密钥率崩溃。因此需要一种既能满足GEAT独立性要求又不会引入过多二阶代价的后选择机制。

Method: 将星座建模为细胞层（Cellular Sheaf），应用拓扑数据分析（TDA）从经典信标遥测数据中仅导出公开接受事件Ω，构建参考拓扑后选择预言机。同时开发了一个模块化仿真框架，配备随机噪声注入和显式GEAT安全账本。在2000-5000km星际分离条件下，对比同一Hodge-Koopman门在TDA禁用与启用下的性能。

Result: 在2000km距离处，TDA禁用和启用时中位条件候选速率分别为2.14×10⁻⁶和5.87×10⁻⁷ bit/发射脉冲；两配置在3000-5000km均为零。TDA在全部4788个评估窗口中均被激活，但未扩展正候选速率范围。导出的速率为条件数值候选值，完整协议级可组合安全证明尚不完整，认证可组合速率全程为零。

Conclusion: 虽然TDA拓扑后选择预言机作为概念验证在短距离上表现出候选速率改进，但该方法在此场景下并未真正扩展正速率覆盖范围，且缺乏完整可组合安全证明。当前结果仅为数值候选，协议尚未达到可部署的认证安全水平。
...
TL;DR: 本文提出一种基于量子比特预算约束的序列分布式QAOA框架，用于解决大规模电力系统的受控孤立（controlled islanding）问题。该框架将问题分解为边界条件约束的区域QUBO子问题，在固定量子比特预算下序贯求解，电路宽度与网络规模无关，聚合量子计算负载在有限度网络上呈线性增长。在11个IEEE测试系统（9到300母线）上使用IBM量子计算资源验证，恢复出可行的Gurobi最优分区，并相比单体QAOA大幅降低了量子资源需求和电路复杂度。


<details>
  <summary>Details</summary>
Motivation: 分布式能源资源的大规模接入加剧了电力系统的波动性和不确定性，在扰动期间可能加剧发电-负荷失衡及级联故障。受控孤立策略可通过将受损电网分割为连通且电气可持续的孤岛来限制故障传播。然而经典方法随网络规模和孤岛数量增长面临快速上升的计算成本；量子优化虽为组合分区空间探索提供新途径，但单体量子公式将所有分配决策编码进单一电路，导致量子比特需求和电路复杂度随网络规模增长，超出当前硬件限制。

Method: 提出一种量子比特有界的序列分布式量子近似优化算法（QAOA）框架。其核心做法是将受控孤立优化问题表述为边界条件化的区域QUBO子问题，在固定的量子比特预算内序贯求解各子问题。该设计使得电路宽度（qubit数）不依赖网络规模，且在有限度数网络上聚合量子工作量呈线性扩展。评估在IEEE 9-300母线共11个系统上使用IBM量子计算资源进行，以Gurobi和单体QAOA作为参照基准。

Result: 在所有11个IEEE测试系统上，该框架在噪声条件下均能恢复可行的Gurobi最优分区，验证了其解质量的鲁棒性。与单体QAOA相比，该方法大幅降低了量子资源需求和电路复杂度，使得大规模孤立问题可在当前硬件限制内得到求解。

Conclusion: 该框架为大规模电力系统中的量子优化提供了一条可行且可扩展的路径，证明了在受限量子资源条件下实现大规模受控孤立问题的可行性，展示了量子优化在电力系统规模化应用中的潜力。
...
TL;DR: 本文提出CoupledPhaseTexture基准测试，用于分离量子扩散去噪器中固定量子特征映射的三种结构性限制（奇偶性、扇区内近似、样本复杂度）。证明了RY+CNOT+Pauli-Z深度1特征族存在不可约的奇偶性下限，该下限源于噪声条件下的去噪目标本身而非静态可表示性，且纠缠无法消除、重上传无法可靠闭合该下限。


<details>
  <summary>Details</summary>
Motivation: 固定的量子特征映射被越来越多地插入扩散去噪器中，但标准图像基准无法揭示究竟是哪种结构约束限制了它们的表现。需要一种能够解耦不同限制来源的分析基准，以准确诊断量子特征映射的性能瓶颈。

Method: 引入CoupledPhaseTexture，一个环形扩散基准，使用解析热核噪声，可分离奇偶性、扇区内近似和样本复杂度三种限制。对深度1的RY+CNOT+Pauli-Z特征族，通过数学证明建立包含自由的奇偶性下限，并将超额风险精确分解为不可达的奇部与扇区内残差两部分。

Result: 证明奇偶性下限对每个偶特征类均成立且不可约，不依赖包含性、线性或封闭性假设。实测超额风险由奇偶性代理主导；高阶Z读出可改善偶数扇区，但纠缠降低不了下限，重上传无法可靠闭合间隙。经典对照实验确认该缺陷源于奇偶性而非量子性：仅余弦库同样受限，加入正弦扇区后达到参考水平。

Conclusion: 阻碍源于噪声调节去噪目标的奇偶性结构，而非静态可表示性，且该下限在每个噪声尺度上重新推导。结果提示需要非经典数据访问或无高效经典替代的特征类，但并未证明这些足以实现量子优势。
...
TL;DR: 该论文展示了利用磁性Paul阱在室温下稳定悬浮一个小型钇铁石榴石（YIG）球体，实现了约25的Q因子和15.8 Hz、17.2 Hz的质点频率，并通过数值估算表明质点运动与激发磁子模式之间可实现协同度大于1的增强耦合，具有量子信息处理的潜在应用。


<details>
  <summary>Details</summary>
Motivation: 磁悬浮可用于先进惯性传感器、生成非经典宏观运动态以及桌面级低能引力与量子力学测试。磁子作为自旋波量子，在室温下也能与电磁场强耦合，因此将磁悬浮与磁子系统结合具有重要的物理意义和应用前景。

Method: 采用磁性Paul阱对软磁材料YIG小球进行稳定俘获；进行了经典稳定性分析并给出所有机械自由度的稳定性图；分析了摆动（librational）模式与平动模式之间的耦合对稳定区域的影响；在室温下实验悬浮YIG球，测量其Q因子和质点频率；通过数值模拟估算质点运动与磁子模式的耦合能力，计算协同度（cooperativity）。

Result: 成功在室温下稳定悬浮YIG球，测得Q因子约为25，质点频率为15.8 Hz和17.2 Hz。稳定性分析表明摆动与平动模式的耦合改变了稳定区域。数值估算显示尽管存在强机械阻尼，质点运动与磁子模式之间的协同度仍可超过1。

Conclusion: 球形磁悬浮系统与磁子模式的有效耦合可突破机械阻尼限制实现协同度大于1，表明该系统在量子信息处理、量子互连和量子存储中具有潜在应用价值。
...
TL;DR: 本文构建了一个基数为14的三-qutrit广义不可扩展乘积基（GUPB），解决了GUPB是否存在这一开放问题，并通过填充程序将其扩展到所有局部维度至少为3的三方系统。该GUPB的正交补投影算子在每个二分下都是正部分转置且有界纠缠的，同时展现出无纠缠的强量子非局域性。


<details>
  <summary>Details</summary>
Motivation: 是否存在真正意义上的不可扩展乘积基（GUPB）一直是一个开放性问题，该问题的解决对于理解量子纠缠、量子非局域性以及量子信息处理中的资源理论具有重要意义。

Method: 作者在最小的三方希尔伯特空间（三-qutrit系统）中显式构造了一个基数为14的GUPB，并通过填充程序（padding procedure）将构造推广到所有局部维度至少为3的三方系统。

Result: 证明了三维三方系统中GUPB的最小基数为14，其正交补空间的归一化投影算子在每个二分下均为正部分转置且有界纠缠态，且该GUPB展现出无纠缠的强量子非局域性。

Conclusion: 该工作解决了GUPB存在性的开放问题，建立了最小基数14的严格下界，并将构造扩展到所有局部维度≥3的三方系统，同时揭示了GUPB在纠缠检测和非局域性研究中的重要应用价值。
...
TL;DR: 本文提出有限窗口可恢复性（finite-window recoverability）这一信道基准，用于区分量子信息从局部量子比特消失后是转移到邻近自由度还是仅对浅层局部控制不可达。作者在无序 kicked-Ising Floquet 链中验证了该框架，并展示了有界深度译码器在有限窗口内可认证并恢复离位量子记忆。


<details>
  <summary>Details</summary>
Motivation: 局域量子比特中的量子信息消失后去向不明：可能转移到邻近自由度，也可能只是对浅层局域控制不可达。现有方法（如目标位持续性和重建相干信息增量）无法区分这两种可能，需要一种能够分离它们的操作性信道基准。

Method: 引入有限窗口可恢复性作为信道基准，包含三个比较项：目标位的最优恢复、有界深度译码器在有限窗口内的恢复、以及该窗口的无约束最优恢复。其操作组件为局部变分恢复（local variational recovery），使用局域态制备、窗内局域控制和目标量子比特 Pauli 读出，来认证目标之外的可恢复记忆并量化同一窗口优势中浅层控制可达的比例。所有译码器通过变分优化训练得到。

Result: 在无序 kicked-Ising Floquet 链中，深度-6 译码器在五站点窗口上实现 Q_0^opt < Q_2^shallow < Q_2^opt，在交叉区间内大多数无序实现给出正认证增益，且浅层可达占有可观比例。在更长开链中嵌入任务时，正半径-2 增益依然保持。在载体删除（carrier-deletion）任务中，深度-8 译码器从半径-3 环境重构输入，held-out 中位 F_avg=0.758，超过单比特经典基准 2/3，并优于最优的一、二、三站点子窗口反事实。

Conclusion: 有限窗口可恢复性确立为离位量子记忆的局域控制基准，能够诊断局域量子信息残留位置，并判定有界深度控制能否将其重新聚焦回复。该框架提供了超越目标位持久性和相干信息增量的更精细操作性判据。
...
TL;DR: 本文发现在耗散拓扑超导体中，非布洛赫体边对应关系在局域观测量层面失效。通过键耗散二聚化Kitaev链的第三量子化快度矩阵表述，作者发现Majorana快度矩阵在零化学势下分解为两个独立非厄米扇区，各自的密度是跨扇区协变量，因此即使一个扇区处于无间隙拓扑态，局域观测量仍对其"失明"。而在平衡的增益与损耗条件下，纯周期边界Lindblad演化下纠缠谱的有限时间零事件可以逐扇区恢复隐藏的边缘内容，成为一种无需物理开链即可返回开边界边缘快度的动力学不变量。


<details>
  <summary>Details</summary>
Motivation: 耗散非厄米拓扑的核心特征是弛豫动力学追踪非布洛赫体边对应，局域观测量可直接诊断拓扑。作者希望探究这一对应关系在耗散拓扑超导体中是否依然成立，并寻找能够恢复隐藏拓扑信息的替代探测手段。

Method: 采用第三量子化的快度矩阵表述，研究键耗散二聚化Kitaev链。在零化学势下将Majorana快度矩阵分解为两个独立非厄米扇区，分别定义广义布里渊区与非布洛赫绕数。通过解析局域密度的弛豫行为（其为跨扇区协变量，弛豫速率是两个扇区速率之和），并在平衡增益/损耗条件下分析周期边界Lindblad演化纠缠谱的有限时间零事件，以逐扇区恢复边缘信息。

Result: 发现零化学势下Majorana快度矩阵分解为两个独立非厄米扇区，各自拥有独立的广义布里渊区和非布洛赫绕数，揭示扇区分辨的非布洛赫体边对应。然而局域密度作为跨扇区协变量，即使某扇区无间隙且拓扑，该观测量仍对拓扑"失明"。但对平衡增益/损耗系统，周期边界下纠缠谱的有限时间零事件能逐扇区恢复隐藏边缘快度，构成无需开链即可提取开边界信息的动力学不变量。

Conclusion: 局域观测量在耗散拓扑超导体中对拓扑的失明性表明，非布洛赫体边对应在此类系统中并非总能通过局域密度等简单观测量体现。然而，纠缠谱的有限时间零事件提供了一种新的动力学不变量，能够在不物理打开链的情况下逐扇区恢复边缘拓扑信息，为探测隐藏的拓扑内容提供了可行途径。
...
TL;DR: 本文研究非正交量子特征求解器（NOQE）中投影矩阵元素误差对特征值估计的影响，证明了一个有限采样扰动界，表明在重叠阈值化后特征值灵敏度由保留重叠矩阵的条件数控制。通过可扩展的阈值化方案，达到目标精度所需的每个矩阵元素采样数上界从O(M³)改进为O(M)。数值实验表明结构化NOQE实例的测量成本可低于线性界。


<details>
  <summary>Details</summary>
Motivation: 量子子空间对角化方法有望在近期量子计算机上实现量子化学计算，但由于矩阵元素估计误差会被病态重叠矩阵放大为更大的特征值误差，这一瓶颈阻碍了方法的实际应用。理解并缓解这一误差放大效应是推动该类算法走向实用化的关键。

Method: 通过理论推导证明有限采样扰动界：在重叠矩阵阈值化后，特征值灵敏度由保留重叠矩阵的条件数控制（而非最坏情况下的维度因子）。提出了可扩展的重叠阈值化方案，将每矩阵元素所需的采样数上界从O(M³)改进为O(M)。通过氢链和氢环的数值实验验证了理论结果。

Result: 理论证明显示采样复杂度上界可从O(M³)降至O(M)；数值实验表明结构化NOQE实例的实际测量成本增长甚至比线性界更慢。

Conclusion: NOQE的测量成本瓶颈可以通过重叠阈值化策略得到显著缓解，采样复杂度从立方降到线性，使量子子空间对角化方法在近端量子计算上更具可行性。
...
TL;DR: 本文提出了一个用于量化中性原子量子优化协议真实浓度效应的度量指标STS(r)，并通过激发密度匹配的随机基线对比实验，在多达125个Rydberg原子阵列上证明量子退火比随机基线表现出更强的低能解结构浓度。研究揭示了两种目标依赖的机制：近精确目标下所需射击数随系统尺寸指数增长但量子退火在同一指数级别上降低该成本；而宽松目标下射击成本近似恒定，量子增强效应减弱，经典后处理启发式即可在常数次尝试内达到目标。


<details>
  <summary>Details</summary>
Motivation: 中性原子量子优化协议是否真正展现出向低能解结构浓度收敛的特性仍是一个悬而未决的问题。现有研究缺乏一个操作性的度量方法来区分量子数据中的真实浓度效应与仅由激发密度等平凡因素导致的表观改进。

Method: 引入STS(r)度量（r为近似比），采用退化权重壳层分布建模后处理输出，由单一有效参数β量化向近最优独立集的浓度。核心方法是对实验比特串和激发密度匹配的随机比特串施加相同的后处理，构建激发匹配随机基线以提取量子数据中的真实浓度效应。实验基于可编程Rydberg原子阵列，系统尺寸达125个格点。

Result: 量子退火在所有情况下都一致地超过随机基线，显示出超越激发密度可解释范围的增强浓度效应。研究发现两种目标依赖机制：近精确目标（r≈1）下射击数随系统尺寸指数增长，量子退火在同一指数水平上降低所需射击数；宽松目标下射击成本接近常数，量子增强效应减弱，经典后处理启发式可在常数次尝试内达到目标。

Conclusion: 该工作建立了一种操作性的量子优化性能量化方法，明确了量子方案在实际中能够产生收益的适用区间——即近精确解目标下量子退火能带来指数级别的射击数节省，而宽松目标下量子优势有限。
...
TL;DR: 本文提出了一种混合DFT-量子嵌入（QDFT）框架，将经典HPC上的DFT与变分量子特征求解器（VQE）结合，通过将大体系划分为化学相关的活性空间用量子计算处理、其余自由度用DFT描述，在无噪声量子模拟条件下系统评估了精度、收敛性、活性空间依赖性、计算成本与HPC可扩展性，并开发了QPU运行时估算方法。


<details>
  <summary>Details</summary>
Motivation: DFT在HPC上支持大规模电子结构模拟，但受近似限制，难以准确处理强关联体系和带隙预测；量子计算虽有望解决该问题，但NISQ硬件受限于量子比特资源、噪声和执行成本，需要一种融合两者优势的混合方案。

Method: 提出QDFT框架，将大体系划分为化学相关的活性空间（由VQE处理）与其余自由度（由DFT描述），整合了活性空间选择、嵌入哈密顿量构建、对称性保持、算子映射、自洽密度更新和模块化经典-量子耦合等环节；在无噪声量子模拟下进行系统评估，并开发QPU运行时估算方法。

Result: 结果表明量子嵌入能改善选定的电子结构性质，同时保持经典HPC的可扩展性；详细性能剖析识别了计算瓶颈，揭示了CPU量子模拟的局限性，并提供了实际量子硬件上的执行需求估算。

Conclusion: QDFT框架展示了量子嵌入提升电子结构精度与HPC可扩展性兼容的潜力；带噪声的量子模拟和真实QPU执行是未来关键方向，为硬件成熟后实用化、可扩展的HPC-量子混合模拟提供了路径。
...
TL;DR: 本文提出AutoQuREO，一个自动化全栈量子资源估计与优化框架，通过灵活的用户定义栈抽象、模块化组件库、神经符号代理建模和集成多目标优化四大创新，作为量子计算栈的数字孪生，支持复杂设计空间的系统探索。


<details>
  <summary>Details</summary>
Motivation: 量子计算从原理验证迈向实用化过程中，需要跨异构软硬件栈的系统级优化。现有量子资源估计(QRE)方法要么依赖编译过程，要么依赖领域知识引导的符号注释，且紧密耦合于大规模容错的长期假设，限制了其应用的针对性。缺乏一个通用、灵活且可扩展到不同量子计算场景的资源估计与优化平台。

Method: AutoQuREO包含四大核心创新：(i) 灵活的用户定义量子计算栈抽象；(ii) 可复用栈组件的模块化库，支持快速全栈原型设计；(iii) 通过算法剖析和神经符号学习对逐层资源进行代理建模；(iv) 将QRE直接嵌入部署管线的集成多目标优化。这些设计使其作为量子计算栈的数字孪生。

Result: 通过代表性协同设计案例验证了AutoQuREO的能力，包括早期容错量子算法、小尺寸纠错码、门分解和参数化量子电路的变分训练。这些案例展示了AutoQuREO能系统性地发现现有QRE工具计算上难以处理或难以理解的资源权衡。

Conclusion: AutoQuREO定位为通用平台，用于推动量子技术成熟度的提升，弥补现有QRE工具在灵活性、可扩展性和设计空间探索方面的不足。
...
TL;DR: 本文提出CAMERA方法，一种在Rydberg原子阵列上高效编码Circuit-SAT问题的方案。通过将每个逻辑门编码为紧凑的加权gadget，并借助VLSI设计启发的布局布线编译器，CAMERA在king子图几何上组装gadget，相比传统CNF编码路线平均降低原子开销22.4倍。作者在随机多门基准上验证了该方法的优势，并编译了全加器和乘法器，通过精确基态计算验证其正确性，最后通过30原子实例的端到端模拟展示了完整工作流程。


<details>
  <summary>Details</summary>
Motivation: 传统的Circuit-SAT到Rydberg原子阵列的编码需要通过CNF转换，导致大量原子开销，降低了硬件利用率和可扩展性。因此需要一种更高效的直接编码方法，以降低原子成本并使其能够解决更大规模的组合优化问题。

Method: CAMERA方法将每个逻辑门表示为一个紧凑的加权gadget（使用加权原子编码），并通过受VLSI设计启发的布局布线编译器在阵列的king子图几何上组装这些gadget。编码过程直接从门级电路到MWIS，绕过了CNF中间表示。验证部分使用精确经典基态计算检查全加器和乘法器的完整真值表，并使用闭系统张量网络模拟硬件兼容的退火协议进行端到端求解。

Result: 在随机多门基准上，直接编码路线的原子成本相比CNF路线平均降低22.4±1.8倍。编译并验证了全加器和乘法器，经精确经典基态计算确认正确。端到端演示了一个30原子的Circuit-SAT实例，从门级编译经张量网络模拟退火协议到读出满足赋值，验证了完整工作流程的可行性。

Conclusion: CAMERA建立了从Circuit-SAT到Rydberg原子阵列的完整编码与模拟工作流程，证明了原理可行性，并为在Rydberg原子阵列上解决更广泛的组合问题提供了具体路径。该方法显著降低了原子开销，是向实用化量子/量子启发计算迈出的重要一步。
...
### [21] [Geometry versus excitation sector in the decoherence of asymmetric $N$-qubit $W$ states](https://arxiv.org/abs/2608.12965)
*Sougata Bhattacharyya,Sovik Roy,Fatih Ozaydin*

Main category: quant-ph

TL;DR: 本文研究了不对称多体W态中网络几何结构与激发扇区如何分别控制成对纠缠的衰减。通过引入N-qubit不对称Lohmayer几何构型及其互补激发伙伴，发现顶点-基底（VB）对的纠缠噪声依赖性与对称W态参考完全相同，而基底-基底（BB）对则表现出真正的同扇区结构脆弱性。


<details>
  <summary>Details</summary>
Motivation: 理解量子网络中纠缠退相干的控制因素至关重要，但网络几何与激发扇区的影响在以往研究中相互混淆。作者希望将这两个效应解耦，以精确区分它们对纠缠动力学各自的作用。

Method: 提出了N-qubit不对称Lohmayer几何构型的解析可处理推广形式及其互补激发伙伴，得到不等价的VB和BB两类成对构型，并与对称W态参考进行直接比较。在代表性单侧噪声模型下推导出闭式concurrence动力学。

Result: 在任一激发扇区内，VB对的concurrence噪声依赖性与对应对称参考完全相同，保持与噪声无关的比例优势。三量子比特Lohmayer态的幅值阻尼重排序被证明是跨扇区效应而非VB几何的内在脆弱性。BB对表现出真正的同扇区结构脆弱性，在去极化噪声下以及(N-1)激发扇区的幅值阻尼下，其纠缠突然死亡阈值低于VB对。

Conclusion: 网络几何、激发扇区和噪声对称性是控制不对称量子网络中成对纠缠鲁棒性的三个独立要素，本文为量子网络设计中的纠缠保护策略提供了理论指导。
...
### [22] [Always-on, highly efficient microwave photon detector based on a superconducting artificial molecule](https://arxiv.org/abs/2608.12972)
*Vyom Kulkarni,Mohammed Ali Aamir,Simon Sundelin,Simone Gasparinetti*

Main category: quant-ph

TL;DR: 本文展示了一种基于超导人工分子的连续工作微波单光子探测器，通过将光子从亮态转移到长寿命暗态并监测量子跳变，实现了0.73的循环探测效率和0.47的连续探测效率（5 MHz瞬时带宽，1 μs时间分辨率，15 μs死时间），克服了传统探测器在效率与占空比之间的权衡。


<details>
  <summary>Details</summary>
Motivation: 微波单光子探测是新兴量子技术的核心能力，但远落后于光学域探测器。现有探测器只能循环工作，在探测效率与占空比之间存在根本性权衡，难以同时实现高效率、低暗计数和连续运行，亟需突破这一瓶颈。

Method: 采用超导人工分子方案：入射光子被分子的亮态捕获，随后通过驱动-耗散过程转移到长寿命的暗态；通过连续监测暗态的量子跳变来揭示光子"咔哒"信号，从而实现连续而非循环的探测工作模式。

Result: 实现了0.73的循环探测效率，以及0.47的连续探测效率，覆盖5 MHz瞬时带宽，具备1 μs时间分辨率和15 μs死时间。

Conclusion: 该方案通过克服效率与占空比之间的权衡，为量子传感、量子热力学和基础物理建立了连续微波光子探测能力。
...
### [23] [Completeness for flow-preserving rewrite rules](https://arxiv.org/abs/2608.13035)
*Miriam Backens,Simon Perdrix*

Main category: quant-ph

TL;DR: 本文扩展了ZX-演算中保流（flow-preserving）的图形重写规则集，并证明该规则集对具有合适形式的ZX图之间所有保流变换是完备的。证明使用了显式的保流电路提取等价方法，将具有gflow的ZX图仅通过保流重写规则转换为具有因果流（causal flow）的图。


<details>
  <summary>Details</summary>
Motivation: ZX-演算中的图形重写规则对于全图形化推理量子计算至关重要。许多近期应用利用ZX图与单向量子计算模型（measurement-based quantum computation）之间的对应关系，其中流（flow）性质确保确定性可实施性和向量子电路的转换效率。因此，保留流性质的ZX-演算重写规则具有重要价值，但现有文献中的保流规则集尚不完整。

Method: 作者扩展了文献中已有的保流重写规则集，新增了若干规则并对现有规则进行了推广。然后证明了扩展后的规则集对于所有合适形式的ZX图之间的保流翻译是完备的。证明的核心是引入一种显式保流的电路提取等价方法，利用该方法可将具有gflow的ZX图通过纯保流重写规则转化为具有因果流的图。

Result: 得到了一个扩展的保流ZX-演算重写规则集，并证明了该规则集对ZX图之间所有保流翻译的完备性。这为流保持的图形推理提供了完整的公理化基础。

Conclusion: 该工作填补了保流ZX-演算重写规则的完备性空白，使得涉及流性质保持的图形重写推理在理论上是完整的，为ZX-演算在测量基量子计算领域的应用提供了更坚实的理论基础。
...
### [24] [Time evolution of nonlinear dynamics on a quantum processor](https://arxiv.org/abs/2608.13041)
*José Diogo da Costa Jesus,Abhishek Setty,Tommaso Calarco,Dieter Jaksch,Francisco Cárdenas López,Felix Motzoi*

Main category: quant-ph

TL;DR: 本文在量子处理器上首次实验实现了非线性流体动力学的时间演化，使用混合变分框架求解粘性和无粘Burgers方程，避免了Carleman线性化方法所需的线性嵌入和截断开销，并通过无额外电路折叠开销的零噪声外推法成功执行了深错误电路。


<details>
  <summary>Details</summary>
Motivation: 非线性偏微分方程的数值模拟是现代科学计算的基础，但将其扩展到量子计算机上一直是长期挑战，因为非线性与非厄米演化与传统的基于哈密顿量的量子模拟本质上不兼容。现有方法（如Carleman线性化）需要增大线性嵌入维度和截断开销，效率受限。

Method: 采用混合变分框架求解二维Burgers方程：将非线性动力学直接编码为变分优化过程，将控制演化编码进参数化量子电路，通过量子-经典联合优化迭代重建时变场。同时引入一种无额外电路折叠开销的零噪声外推（ZNE）方法，以在硬件噪声下准确执行超过Hadamard测试电路典型深度的纠缠门电路。

Result: 在量子处理器上成功实验实现了粘性和无粘Burgers方程的时间演化，展示了雷诺数约10^2的对流主导动力学；尽管存在硬件噪声和有限设备相干时间，仍能在多个时间步上准确重建时间演化，实现了超越Hadamard测试电路深度的深错误电路执行。

Conclusion: 这是量子处理器上非线性时间传播的首次实验实现，将量子模拟从以线性为主的场景扩展到非线性连续介质动力学，为量子计算在非线性连续动力学中的应用开辟了道路。
...
### [25] [Quantifying nonclassicality in qubit systems via positive operator-valued measures](https://arxiv.org/abs/2608.13088)
*Abdul Sattar Khan,Mehdi Abdi*

Main category: quant-ph

TL;DR: 本文提出了一种基于顺序测量中Kolmogorov一致性条件违反的qubit系统非经典性操作度量，该度量通过POVM不锐度参数a_z显式量化非经典性。投影测量在初始对角态下无法揭示非经典性，但非投影POVM测量即使在初始对角态下也能揭示非经典性，且该度量的最大值为1/4。


<details>
  <summary>Details</summary>
Motivation: 以往的经典性刻画工作（如Milz等人的NCGD映射方法和Sakuldee等人的测量扰动方法）未能以操作性的方式显式量化非经典性。作者旨在提供一个更直接、可实验验证的非经典性度量，该度量基于Kolmogorov一致性条件的违反，并与Leggett-Garg不等式和POVM建立联系。

Method: 作者基于顺序测量中Kolmogorov一致性条件的违反定义非经典性度量，该度量以POVM不锐度参数a_z明确表示。通过比较投影测量与POVM测量的情形，推导了一般初始态（包括相干项）下度量的显式表达式，并证明其最大值1/4在无偏POVM、最大去相干和等初始布居条件下取得。

Result: 度量在投影测量且初始态为对角态时恒为零；推广到POVM后，即使初始态为对角态也能检测到非经典性。度量最大值达到1/4，这是一个新的结果。该度量连接了Kolmogorov一致性、Leggett-Garg不等式与POVM框架。

Conclusion: 该工作提供了实验上可访问的qubit系统非经典性检测工具，可与现有经典性刻画方法互补，在量子技术认证中具有潜在应用价值。
...
### [26] [Robust controlled-Z gate for Rydberg atoms based on level-crossing-free echoing rapid adiabatic passage](https://arxiv.org/abs/2608.13090)
*Yichi Zhang,Zhenqi Bai,Xu Zhao,Hongyan Fan,Ximo Wang,Tiecheng Wang*

Main category: quant-ph

TL;DR: 本文提出一种基于无交叉回波快速绝热布居转移的里德堡原子受控Z门方案，通过反对称拉比频率脉冲和对称失谐脉冲避免能级交叉，利用时间反演对称性消除动力学相位。数值模拟表明两量子比特CZ门保真度达0.9999，三量子比特CCZ门保真度达0.999，且对参数波动具有良好鲁棒性，无需激光相位跳变或快速开关，适用于大规模中性原子阵列的并行门操作。


<details>
  <summary>Details</summary>
Motivation: 传统里德堡原子CZ门的实现通常需要激光相位跳变或快速开关操作，且容易受到能级交叉和参数波动的影响，限制了门操作的保真度和可扩展性。作者旨在设计一种无需快速切换、对噪声鲁棒且适用于大规模中性原子阵列并行操作的受控Z门方案。

Method: 基于无交叉回波快速绝热布居转移（Echoing Rapid Adiabatic Population Transfer）设计CZ门方案。具体地，设计反对称拉比频率脉冲和对称失谐脉冲，使系统在整个演化过程中完全避免能级交叉点；利用双脉冲序列的时间反演对称性自然消除动力学相位；通过Lindblad主方程纳入耗散效应进行数值模拟。协议无需激光相位跳变或快速开关操作，零面积脉冲结构可抑制一阶强度噪声。

Result: 数值模拟结果表明：两量子比特CZ门保真度为0.9999；在拉比频率波动±2%和失谐偏移±1%范围内，保真度仍高于0.999；相同耗散模型下三量子比特CCZ门保真度为0.999；单参数波动不超过±3%时，保真度始终高于0.997。

Conclusion: 该方案在保持高保真度的同时，避免了激光相位跳变和快速开关操作的需求，显著降低了实验实现难度。零面积脉冲结构抑制一阶强度噪声，对称双脉冲序列避免空间分辨激光开关，使其特别适合大规模中性原子阵列中的并行门操作，为可扩展量子计算提供了实用方案。
...
### [27] [Time-resolved correlation engineering in DLCZ Raman photon sources](https://arxiv.org/abs/2608.13091)
*Jiun-Shiuan Shiu,Chang-Wei Lin,Chi-Ming Yang,Ite A. Yu,Yong-Fan Chen*

Main category: quant-ph

TL;DR: 本文提出了一种包含传播效应的开放量子系统理论，用于预测DLCZ协议中存储器兼容光子源的关联特性，并通过实验验证了其在可控时域整形与关联工程方面的预测能力。


<details>
  <summary>Details</summary>
Motivation: DLCZ协议基于原子系综中的自发拉曼散射为量子网络提供了平台，但缺乏将控制参数与真实传播和噪声条件下的关联联系起来的统一预测理论。

Method: 构建传播包含的开放系统量子理论，保留写入诱导的粒子数重分布，将海森堡-朗之万动力学与麦克斯韦-薛定谔传播方程相结合，并进行实验验证。

Result: 理论成功预测了时间依赖的Stokes产生、自旋波演化、反Stokes波包检索及时间分辨互关联；实验证实了检索调谐下的稳健关联以及更短写入脉冲增强的关联；经典控制的检索实现了反Stokes波包的时间门控与切片。

Conclusion: 该工作为存储器兼容DLCZ光子源的关联工程建立了定量框架，弥补了理论预测与实验控制之间的空白。
...
### [28] [Formal Verification of Quantum Ancilla Safety](https://arxiv.org/abs/2608.13099)
*Jiqi Li,Jingyi Mei,Wang Fang,Ji Guan*

Main category: quant-ph

TL;DR: 本文提出了一种端到端的量子编译辅助量子比特（ancilla）安全性验证与修复框架，通过两步归约策略将脏辅助量子比特的验证问题分解为独立的清洁辅助比特安全性检查，并进一步归约为代数交换性检查，实现了可扩展到数千量子比特的高效验证与轻量级修复。


<details>
  <summary>Details</summary>
Motivation: 辅助量子比特（ancilla）是量子编译中常用的资源，用于以更少的门数和更浅的深度实现复杂操作。然而，确保ancilla安全性（即使用后恢复原状）是正确性验证的关键要求，形式化验证这一问题由于量子比特数量增加导致的状态空间爆炸而计算困难，尤其是脏ancilla（携带未知初始态）的验证更具挑战性。

Method: 提出两步归约策略：(1) 将m量子比特脏ancilla寄存器的验证分解为2m个独立的清洁ancilla安全性检查；(2) 将每个清洁ancilla安全性实例归约为针对Pauli-Z和Pauli-X算子的代数交换性检查。在此基础上，将违规分类为逻辑错误和相位错误，并设计轻量级修复例程（追加局部单量子比特旋转）来消除局部ancilla故障。实现采用双后端架构，结合决策图和加权模型计数。

Result: 在从算术基准到Grover算法的多种电路上验证了该框架，实验表明可扩展到数千个量子比特，且所提出的修复方法能有效改善ancilla安全性同时保持电路功能不变。

Conclusion: 该工作通过数学归约将原本高复杂度的ancilla安全性验证问题简化为可并行的高效检查，并提供可操作的错误诊断与修复机制，为量子编译中的ancilla安全性保障提供了实用的端到端解决方案。
...
### [29] [Controlling quantum transport by measurement-rate modulation](https://arxiv.org/abs/2608.13109)
*Jesús Casado-Pascual,Luis Octavio Castaños-Cervantes*

Main category: quant-ph

TL;DR: 本文研究通过时间调制测量率来控制开放系统动力学，在最小可精确求解的连续监测量子粒子切换晶格模型中，推导了周期和随机切换下长时间电流的精确解析表达式，发现了测量诱导的量子Zeno与anti-Zeno输运机制交叉，该交叉控制电流方向。


<details>
  <summary>Details</summary>
Motivation: 测量反作用（measurement backaction）是开放量子系统动力学的核心机制之一，而时间调制测量率可能提供一种有力的控制手段。作者希望在一个可精确求解的最小模型中，从理论上揭示测量率时间调制如何影响量子粒子的输运行为。

Method: 构建一个连续监测的量子粒子在切换晶格（switching lattice）上的最小可精确求解模型。分别在周期切换和随机切换两种时间调制方案下，推导长时间电流的精确解析表达式，并比较两种方案的共同慢切换极限。

Result: 获得了周期与随机切换下长时间电流的精确解析解，识别出共同的慢切换极限行为，并发现了测量诱导的Zeno效应与anti-Zeno效应主导的输运区间的交叉，该交叉决定了电流方向。

Conclusion: 时间调制测量率可作为控制开放量子系统输运方向的有效手段，通过测量导致Zeno与anti-Zeno行为之间的交叉可以实现电流方向的可控切换，为基于测量反作用的量子输运调控提供了理论框架。
...
### [30] [Classical Simulation and Design Frontiers for IBM's Doped Clifford Sampling Experiment](https://arxiv.org/abs/2608.13110)
*Hidetaka Manabe,Hanfeng Gu,Feng Pan*

Main category: quant-ph

TL;DR: 本文对IBM 70量子比特、70层纠缠、468个T门的掺杂Clifford随机电路采样实验进行了经典模拟。作者设计了面向开放边界一维砖墙电路和张量缩并的方法，通过时间边界张量网络收缩，在32个节点（每节点8块NVIDIA H100 GPU）上37.3分钟内完成了所有2051个振幅批次的计算，得到log-XEB估计0.35034，与IBM的保真度下界数值兼容。


<details>
  <summary>Details</summary>
Motivation: 验证和挑战IBM声称的量子优势掺杂Clifford随机电路采样实验，证明此类实验可以被经典计算高效模拟，并为未来掺杂Clifford采样实验的设计提供定量诊断工具。

Method: 采用确定性的时间边界张量网络收缩方法，专门针对具有算子Schmidt秩为2纠缠门的开放边界一维砖墙电路。该方法吸收单量子比特门而不改变网络拓扑，使收缩宽度和调度成本与单比特门取值及T门数量和位置无关；对于n量子比特深度d的电路，未切片路径的精确振幅收缩宽度为⌈d/2⌉，并由Ratcatcher计算验证其最优性。

Result: 对于IBM实例，最大中间张量含2^35个complex64条目（比IBM估计小256倍），即256 GiB，分布在单节点8块GPU上。使用32个节点共256块H100 GPU，在37.3分钟内完成全部2051个振幅批次，得到log-XEB估计0.35034（95%置信区间[0.29763,0.40305]）。在Porter-Thomas和杂乱噪声假设下与IBM保真度下界数值兼容；保真度加权资源估算预测583次收缩负载，32节点上10.6分钟即可完成。

Conclusion: 该方法成功以远低于IBM预期计算成本的方式经典模拟了70比特掺杂Clifford采样实验，为实验输出提供了实用诊断工具，并为设计未来掺杂Clifford采样实验提供了定量设计指南。
...
### [31] [Controlled dynamics of a multi-component discrete-time quantum walker](https://arxiv.org/abs/2608.13161)
*Vikash Mittal,Tomasz Sowiński*

Main category: quant-ph

TL;DR: 本文研究了在一维晶格上运行的三分量离散时间量子行走，采用由Gell-Mann矩阵生成的参数化旋转作为硬币算子，系统探索了参数空间中不同内部耦合对动力学的影响，发现了从近对称快速扩散到强各向异性部分局域化的多种输运机制。


<details>
  <summary>Details</summary>
Motivation: 离散时间量子行走是多分量量子粒子动力学的重要平台，但此前对三分量系统内部耦合如何影响和控制动力学的研究尚不充分，缺乏系统性的参数空间探索。

Method: 采用Gell-Mann矩阵生成参数化旋转作为硬币算子来调节内部分量间的耦合，通过系统扫描宽泛参数空间分析位置空间概率分布，并利用每半晶格中平均位置与方差的比值来量化不同分量间耦合的影响。

Result: 研究发现该系统支持丰富的输运机制，从近乎对称且快速扩散的行走到强各向异性伴随部分局域化的动力学不等。

Conclusion: 该框架为在多分量离散时间量子行走中工程化定向扩展和捕获行为提供了新途径。
...
### [32] [Witnessing the architecture of quantum circuits](https://arxiv.org/abs/2608.13169)
*Raphaël Mothe,Otfried Gühne*

Main category: quant-ph

TL;DR: 本文提出了一种通用的量子电路架构见证（quantum circuit architecture witness）框架，通过将见证构造转化为半定规划（SDP）问题（最大化目标酉算子Choi态与测试电路Choi态之间的保真度），严格证明某个酉变换无法在给定的量子电路架构中实现。对于Clifford酉算子，利用稳定子形式化将问题简化为线性规划，可高效处理约七个双量子比特门的电路，并为任意数目门的架构族提供解析见证。


<details>
  <summary>Details</summary>
Motivation: 现有的综合和编译方法主要是构造性的，一般无法提供严格的证据证明某个酉算子无法在给定的实现资源下实现。本文旨在填补这一空白，为量子电路架构不兼容性提供严格的认证框架。

Method: 通过最大化目标酉算子的Choi态与待测电路Choi态之间的保真度，将见证构造形式化为半定规划（SDP）问题。所得见证提供实用且定量的不兼容性证明，暗示门数或电路深度等实现资源的下界。对于Clifford酉算子，利用稳定子形式化将构造简化为线性规划（LP），提高数值认证效率，并能构造解析见证。

Result: 该框架能提供实用且定量的不兼容性证书，给出实现资源（如门数、电路深度）的下界；可用于实验上基准测试量子设备，认证实际实现的酉通道超出给定电路架构的能力。对于Clifford酉算子，线性规划方法可高效认证包含约七个双量子比特门的电路，并能得到任意门数架构族的解析见证。

Conclusion: 量子电路架构见证为量子电路编译与优化中的不可实现性问题提供了严格的数学认证工具，兼具理论普适性（SDP框架）与实际可扩展性（Clifford情形下的LP简化），在量子硬件基准测试和高效量子计算验证中具有重要应用价值。
...
### [33] [Entanglement distribution and quantum storage of more than 8000 modes over a metropolitan network](https://arxiv.org/abs/2608.13177)
...
TL;DR: 本研究构建了一个基于171Yb3+:Y2SiO5多模量子存储器的量子中继节点，实现了电信波段光子与物质之间的纠缠生成。该平台具备250 MHz带宽和76.6 μs存储寿命，利用Schmidt分解引入并验证了有效时间模式容量的定量度量。研究展示了电信光子经过25.3 km光纤传输后与存储在164万个时间模式上的979 nm光子之间的纠缠，并在日内瓦都市光纤网络中进行了5.66 km的现场部署演示。


<details>
  <summary>Details</summary>
Motivation: 实现基于光纤的量子中继器需要电信光子与物质系统之间的纠缠生成，而实际通信速率需要多路复用（multiplexing）能力。稀土离子掺杂晶体凭借其吸收光谱中的大量光谱通道，能够提供大规模的时间模式多路复用存储能力，是实现高速率量子通信的关键平台。

Method: 采用171Yb3+:Y2SiO5稀土离子多模量子存储器（带宽250 MHz、寿命76.6 μs），结合带宽匹配的纠缠光子对源。引入Schmidt分解作为定量度量有效时间模式容量的工具，并验证了该方法的有效性。实验通过电信光子与979 nm存储光子之间的纠缠来评估系统性能。

Result: 实现了电信光子经25.3 km光纤传输后与存储在16340个时间模式上（125 μs）的979 nm光子之间的纠缠。在日内瓦都市光纤网络中完成5.66 km的现场部署，成功存储8235个模式，存储时间63 μs。

Conclusion: 该工作证明了基于稀土离子多模量子存储器的量子中继节点在实际光纤网络环境中的可行性，大规模时间模式多路复用能力显著提升了量子通信速率潜力，为未来建设高速率、长距离量子中继网络奠定了重要实验基础。
...
### [34] [Mid-circuit ground-state cooling and ancilla readout in the $\textit{omg}$ architecture](https://arxiv.org/abs/2608.13181)
*Sean Brudney,Connor Burns,Gabriel J. Gregory,Evan Ritchie,David J. Wineland,David T. C. Allcock,Jameson O'Reilly*

Main category: quant-ph

TL;DR: 本文展示了被困离子OMG架构中，通过在地态qubit上进行耗散操作即可将混合亚稳态-基态库仑晶体的全局运动模式冷却至基态，且不扰动亚稳态qubit的相干性，实现利用基态qubit荧光检测对亚稳态qubit进行非破坏性量子逻辑谱读取，为容错量子计算的关键原语铺平道路。


<details>
  <summary>Details</summary>
Motivation: 传统双物种离子阱实验虽能提供同情冷却和非破坏性辅助读取功能，但硬件开销过大。OMG架构旨在仅用单一离子物种的不同电子态（基态和亚稳态）实现同样的全功能，从而消除硬件冗余。本文核心动机是验证该架构的关键冷却与读取可行性。

Method: 采用混合基态-亚稳态库仑晶体，对混合离子体系的全局运动模式施加基于耗散的激光冷却策略，仅作用于基态qubit，同时监测亚稳态qubit的相干性是否保持；随后利用量子逻辑谱技术，通过基态qubit的荧光检测对亚稳态qubit进行非破坏性状态读取。

Result: 成功将混合基态-亚稳态的全局运动模式冷却至运动基态，且在冷却过程中亚稳态qubit的相干性未受扰动；实现了基于基态qubit荧光检测的亚稳态qubit非破坏性读取。

Conclusion: 该演示为OMG架构在更大系统中的扩展奠定基础，有望缓解离子穿梭后的运动加热问题并支持量子纠错的症候提取，这是未来基于被困离子的容错量子计算机的关键原语。
...
### [35] [Counter-examples for Tensorization Property of Strong Data Processing Inequality for Quantum Divergences](https://arxiv.org/abs/2608.13204)
*Yu Cao*

Main category: quant-ph

TL;DR: 本文报道了两个关于量子散度强数据处理不等式（SDPI）张量化的负面结果：对于某些量子卡方散度，张量化性质不成立；对于量子相对熵，张量化性质同样不成立。


<details>
  <summary>Details</summary>
Motivation: 在经典信息论中，强数据处理不等式（SDPI）的张量化性质对于整个f-散度族都成立，但这一性质在量子信息论中的对应情况尚不清楚。此前仅在一些特殊情形下证明了SDPI的张量化，量子散度SDPI张量化性质的普适理解仍然是一个开放性难题。

Method: 本文通过构造具体的反例来证明两个负面结果：针对一类量子卡方散度，以及量子相对熵，分别证伪了SDPI张量化性质的普适成立性。

Result: 获得了两个负面结果：（1）张量化性质对某些量子卡方散度不成立；（2）张量化性质对量子相对熵不成立。

Conclusion: 量子散度的SDPI张量化性质并不是普适成立的，这与经典信息论形成鲜明对比。这揭示了量子散度与经典f-散度在张量化性质上的根本差异，为量子信息论中SDPI的适用边界提供了重要的反面证据。
...
### [36] [Ion trap on borosilicate substrate with integrated femtosecond-laser-written waveguide](https://arxiv.org/abs/2608.13207)
...
TL;DR: 本文提出一种基于硼硅酸盐玻璃的离子阱平台，集成飞秒激光刻写波导用于片上光传输。光学层与电极基板物理分离并键合在阱上方，在729 nm和405 nm波长下实现单模低损耗传输（弯曲半径可低至6 mm），并在低温捕获离子系统中用^40Ca^+验证了离子捕获、穿梭以及通过波导传输729 nm光驱动的相干操作。


<details>
  <summary>Details</summary>
Motivation: 传统离子阱系统依赖于自由空间光学或光纤传输来输送激光到离子位置，光路对准复杂、体积大、扩展性差。为了实现大规模可扩展的量子计算/模拟，需要一种稳健、可规模化、与工业制造兼容的集成化光传输方案，将光信号高效地输送到阱内离子附近。

Method: 在硼硅酸盐玻璃基底上用飞秒激光直写技术制备单模波导，并将其作为独立光学层与电极基板物理分离后键合在表面电极阱上方。通过优化波导设计实现729 nm及405 nm波长下的单模低损耗传输，实现弯曲半径低至6 mm的弯曲波导，并与单金属层表面电极阱的工业制造工艺（包括光纤主动对准和键合）兼容。

Result: 在低温捕获离子系统中，用^40Ca^+离子验证了平台功能：成功实现离子捕获、将离子穿梭至波导前方区域，并通过集成波导传输的729 nm光驱动离子相干操作。表征了暴露介电材料对离子的影响，测得杂散电场在小时量级时间尺度上存在缓慢漂移。

Conclusion: 该架构与混合微光学（如pick-and-place透镜）兼容，可实现单离子寻址，为捕获离子器件提供了一条稳健、可扩展的集成光传输路线，且与硅基集成兼容，具有良好的工业可制造性前景。
...
### [37] [Heterogeneously Integrated Squeezed-Light Generation and Detection on a Single Photonic Chip](https://arxiv.org/abs/2608.13218)
...
TL;DR: 通过异构集成技术，在单个光子芯片上实现了压缩光生成、路由和平衡零差检测的完整集成，测量到包含34个量子模式的二模压缩量子微梳，压缩度约3 dB，为全集成量子光子芯片建立了可扩展架构。


<details>
  <summary>Details</summary>
Motivation: 压缩光是量子增强传感和连续变量量子信息处理的基础，集成光子学是规模化产生压缩光的重要途径。然而，压缩光生成和光电检测需要在同一芯片上集成，二者对材料的要求本质矛盾：压缩光生成需要低光学损耗以保持量子关联，而光电检测需要高效的光子吸收。此前在单一芯片上实现两者的集成一直极具挑战性。

Method: 采用异构集成（heterogeneous integration）技术，将压缩光生成、路由和平衡零差检测集成在单个光子芯片上。利用量子微梳产生二模压缩光，包含34个量子模式，并通过平衡零差检测进行测量。

Result: 成功在单个光子芯片上演示了压缩光生成、路由和平衡零差检测的完整集成，实现了包含34个量子模式的二模压缩量子微梳，压缩度约为3 dB，验证了全集成方案的可行性。

Conclusion: 该工作建立了全集成压缩光量子光子系统的可扩展架构，首次将量子态生成、处理和检测统一到单个芯片上，解决了材料相容性这一根本难题，为大规模量子光子芯片的应用铺平了道路。
...
### [38] [Homomorphic Aggregation of Continuous-Variable GKP States](https://arxiv.org/abs/2608.13227)
*Nilesh Vyas*

Main category: quant-ph

TL;DR: 本文提出了一种基于主动测量和补偿的框架，用于对多节点连续变量GKP量子比特进行同态聚合，通过GKP贝尔态和零差前馈构建完全正迹保护映射，在有限压缩约束下计算分布式量子态的逻辑和，并证明其作为近似量子非破坏测量、界定密码泄漏和逻辑保真度极限。


<details>
  <summary>Details</summary>
Motivation: 分布式量子计算需要聚合连续变量相空间中编码的逻辑量子信息，但无源线性光学对非高斯GKP码因辛格点阵压缩和纠缠诱导退相干而失效，因此需要一种能在多节点间实现GKP态同态聚合的主动方案。

Method: 利用GKP贝尔态和零差前馈补偿构建完全正迹保护(CPTP)映射，实现分布GKP态的逻辑和聚合计算，同时保持逻辑码空间几何使其仅产生可修正的有限压缩变形。通过量子非破坏测量框架分析协议性质，并对连续一次性密码本的密码学泄漏与有限压缩下的逻辑保真度进行解析推导。

Result: 证明了该协议可作为近似量子非破坏测量运作，给出了连续一次性密码本的密码泄漏上界，并推导了有限压缩约束下逻辑保真度的解析极限。

Conclusion: 该主动测量框架突破了无源线性光学对非高斯GKP态聚合的限制，为分布式连续变量量子计算中的逻辑信息同态聚合提供了具有可证明保真度和安全性边界的可行方案。
...
### [39] [Heat transport in driven quantum systems: Comparison between the Floquet-Redfield equation and the master equation in the instantaneous eigenbasis](https://arxiv.org/abs/2608.13308)
*Luca Magazzù,Christoforus Dimas Satrya,Aleksandr S. Strelnikov,Bayan Karimi,Jukka P. Pekola*

Main category: quant-ph

TL;DR: 本文利用主方程与Floquet方法相结合，对周期性驱动的量子系统中的热输运进行了全面研究。在弱耦合Redfield理论框架下（不做Markovian或secular近似）给出精确结果，并与采用进一步近似的其他方法进行比较，对驱动自旋-玻色子模型在相应驱动区间内提供了数值和解析结果，并讨论了与热机相关的绝热区间。


<details>
  <summary>Details</summary>
Motivation: 周期性驱动的量子系统在量子热机、量子制冷机等热力学应用中具有重要意义，但目前缺乏在弱耦合Redfield理论框架下不做额外近似的精确热输运结果，需要系统性地研究驱动机制下热流的精确描述。

Method: 结合主方程（master equation）和Floquet理论，在弱耦合Redfield框架内（不做Markovian或secular近似）导出精确热输运表达式，并在相应驱动区间内对驱动自旋-玻色子模型进行数值与解析求解，同时与引入进一步近似的方法进行对比。

Result: 给出了弱耦合Redfield理论下精确的热输运结果，获得了驱动自旋-玻色子模型在相应驱动区间内的数值与解析解，并明确了不同近似方法在各驱动区间内的适用性与偏差。

Conclusion: 在Redfield理论框架下可精确描述周期性驱动系统的热输运，无需Markovian或secular近似即可获得可靠结果；绝热区间（对热机应用尤为重要）得到了充分的讨论与分析。
...
### [40] [Phase information transfer by post-selection in Spin--Mechanical assisted magnetometry](https://arxiv.org/abs/2608.13325)
*Raúl Coto,Hugo Molinares,Vitalie Eremeev*

Main category: quant-ph

TL;DR: 本文研究了自旋-机械振子混合量子系统中，通过后选择实现对自旋相位信息到机械振子的条件性量子信息传递。利用测量诱导的反作用（measurement-induced backaction），通过对自旋系统进行后选择，将自旋相位信息重新聚焦到机械振子。作者识别了控制条件相位传递的物理资源和工作区间（如振子量子相干性、自旋数量、机械初始态、耦合强度、振子振幅和弛豫），并使用方差作为性能指标，通过半经典方差估计器和Pegg-Barnett量子估计器两种互补方法进行评估，同时计算Cramér-Rao下界进行比较。


<details>
  <summary>Details</summary>
Motivation: 早期量子信息传递的演示主要依赖相干控制，但测量诱导的反作用作为量子控制的强有力替代方案尚未被充分探索。在自旋-机械振子这类高维混合量子系统中，理解条件相位传递的物理机制和资源，对于量子磁力测量等应用具有重要意义。

Method: 采用后选择机制作为选择性反作用，将自旋相位信息传递到机械振子系统。通过两个互补的估计方法（半经典方差估计器和Pegg-Barnett量子估计器）对相位传递性能进行量化，并与Cramér-Rao下界进行比较，系统扫描影响传递效率的多个物理参数。

Result: 识别出控制条件相位传递的关键物理资源和工作区间，包括振子量子相干性、自旋数量、机械初始态、耦合强度、振子振幅与弛豫等因素。两种互补的估计方法为相位传递性能提供了完整评估，并与Cramér-Rao界建立了对比基准。

Conclusion: 该工作为理解高维混合量子系统中的相位传递以及测量诱导反作用在量子磁力测量中的应用提供了统一的理论框架。
...
### [41] [Robust Genuine Multipartite Entanglement in Two Walker Quantum Walks](https://arxiv.org/abs/2608.13338)
*Sandipan Hazra,Tamoghna Das,Sougato Bose,Sonjoy Majumder*

Main category: quant-ph

TL;DR: 该论文研究了二维离散时间量子行走中两个行走者之间的二分纠缠和真正多体纠缠动力学，揭示了晶格拓扑（开放边界vs闭合边界）对纠缠演化的关键影响。开放边界条件下，广义几何度量（GGM）迅速趋近理论最大值1/2，且对初始贝尔态选择和局域硬币算符的变化高度鲁棒，表明开放边界双行走者离散时间量子行走是生成最大真正多体纠缠的通用平台。


<details>
  <summary>Details</summary>
Motivation: 离散时间量子行走是研究复合量子系统中量子关联的生成、再分配和输运的通用框架。理解和调控量子行走中多体纠缠的动力学特性，对于量子信息处理和量子模拟中的应用至关重要，特别是在不同晶格拓扑条件下纠缠行为的系统性表征尚不充分。

Method: 采用对数负度（logarithmic negativity）和广义几何度量（GGM）两种纠缠度量，系统研究了在一维晶格上双行走者离散时间量子行走中不同子系分区的二分纠缠再分配以及涉及两个硬币和两个位置自由度的真正多体纠缠的产生。分别考察了开放边界和闭合边界两种边界条件下的纠缠动力学。

Result: 研究发现纠缠动力学强烈依赖晶格拓扑：开放边界条件下量子关联呈单调再分配，GGM迅速接近理论最大值1/2，且对初始贝尔态选择及局域硬币算符的大范围连续变化不敏感（仅在Pauli-X硬币附近例外）；闭合边界条件下则因边界诱导干涉和波包周期性重叠产生显著的振荡行为。

Conclusion: 最大真正多体纠缠的产生是开放边界双行走者离散时间量子行走的鲁棒且普通的特征，确立了该系统作为工程化多体量子关联的可行平台，在量子信息处理和量子模拟中具有广阔应用前景。
...
### [42] [Quantum-Inspired Phase Bicoherence Spectroscopy: A Framework for Detecting Universal Textural Angular Order Across Multi-Modal Complex Datasets](https://arxiv.org/abs/2608.13342)
*Zheng Xing,Chan-Tong Lam,Xiaochen Yuan*

Main category: quant-ph

TL;DR: 本文提出量子相位双相干（QPBC）光谱学，一种基于九量子比特纠缠态和辅助量子比特的量子干涉测量框架，用于提取图像中规范不变的角向序特征，无需显式相位重构。该框架在三个公共多模态成像数据集（荧光、明场、组织病理）上验证，能高统计显著性区分生物表型，并超越传统傅里叶相位统计。


<details>
  <summary>Details</summary>
Motivation: 经典图像分析通常丢弃傅里叶相位中编码的结构性取向信息，而这些信息容易被局部细胞旋转破坏。虽然量子启发的数据处理为复杂信号表征提供了新途径，但缺乏无需显式相位重构即可直接提取规范不变角向关联的实际工具。

Method: 将图像角向扇区嵌入九量子比特纠缠态，通过辅助量子比特探测三体双相干性（bicoherence），输出16个可解释的读出通道。训练/分析流程包括主轴对齐以及频率调谐；通过负对照实验验证频率调谐作为开关机制。

Result: 在BBBC021（荧光）、BBBC041（明场）和PathMNIST（组织病理）三个公共数据集上，QPBC持续解析角相位序并高统计显著性区分不同生物表型。主轴对齐后最优探测频率普遍收敛到由傅里叶方向敏感性驱动的值；负对照实验完全消除判别能力；跨数据集基准显示QPBC优于传统傅里叶相位统计，且内置的反演对称性作为流程自检机制。

Conclusion: QPBC提供了一种通用、经典不可实现的定量纹理观测量，建立了可解释的量子形态学（quantum morphometry），扩展了适用于多种多模态显微测量的量子启发性分析工具箱。
...
### [43] [Universal magic state concentration](https://arxiv.org/abs/2608.13376)
*Jacopo Rizzo,Lorenzo Leone*

Main category: quant-ph

TL;DR: 本文提出"通用魔法态浓缩"：一个固定的稳定子协议，可将少量未知纯非稳定子量子比特态转化为精确的目标CCZ魔法态。六拷贝输入是精确蒸馏一个CCZ态的充要条件，最优成功概率由线性化三阶稳定子Rényi熵M_3^lin决定，且该熵量在最多九拷贝输入下支配所有协议的最优态依赖。块重复该协议可获得渐进蒸馏速率，达到对数因子内的最优标度。作为推论，任何未知纯量子比特魔法态都足以通过精确CCZ注入实现通用量子计算。


<details>
  <summary>Details</summary>
Motivation: 魔法态在量子计算中扮演双重角色：一方面使稳定子动力学从经典可模拟变为通用计算，另一方面却对容错构成挑战，因为非稳定子操作更难抵抗噪声。魔法态蒸馏是解决此问题的关键途径，但现有协议通常假设输入具有先验结构（如接近目标态或特定噪声模型）。作者希望设计不依赖输入先验信息的普适协议，并探究不同魔法态家族（如T态与CCZ态）在精确浓缩方面的根本差异。

Method: 作者引入了"通用魔法态浓缩"框架：一个固定的稳定子协议，将未知纯非稳定子量子比特态转化为精确目标魔法态。他们分析了T态精确浓缩的障碍，并证明CCZ态在本质上是不同的。通过线性化三阶稳定子Rényi熵M_3^lin来刻画最优成功概率，并推导出六拷贝充要条件。此外还构建了一个八拷贝协议提升成功概率，并通过块重复实现渐进蒸馏速率的优化。

Result: 证明了六输入拷贝是蒸馏一个精确CCZ态的充要条件，最优成功概率由线性化三阶稳定子Rényi熵M_3^lin确定；M_3^lin在最多九拷贝输入下支配任何协议的最优态依赖；提出了八拷贝协议以改进成功概率；块重复协议达到对数因子内的最优渐进蒸馏速率；任何未知纯量子比特魔法态都可通过精确CCZ注入实现通用量子计算。

Conclusion: 稳定子Rényi熵被确立为魔法态蒸馏中的一个基本操作量。CCZ态相对于T态在精确浓缩问题上展现出根本不同的行为，这为理解魔法态蒸馏的结构提供了新视角，并为不依赖先验噪声模型的通用容错量子计算方案铺平了道路。
...
### [44] [Field-Widened Multimode Interferometer with Long Time-Bin Delay Using a Multi-Pass Herriott Cell](https://arxiv.org/abs/2608.13399)
*Ramy Tannous,Stéphane Vinet,Kaylee Sherk,Kimia Mohammadi,Thomas Jennewein*

Main category: quant-ph

TL;DR: 本文展示了一种场加宽多模干涉仪设计，利用多程Herriott腔实现大路径差和小型化，在12ns路径差下对空间多模光束保持高干涉可见度，视场角达0.4°。


<details>
  <summary>Details</summary>
Motivation: 自由空间信道中大气湍流导致光学信号需要接收多个空间模式，传统自适应光学系统复杂且昂贵。场加宽干涉仪作为一种无源替代方案具有吸引力，尤其适用于延迟约1纳秒的时间仓编码信号，但需要解决多模干涉和紧凑设计之间的权衡问题。

Method: 采用多程Herriott腔设计，在紧凑外形下实现非常长的路径分离，使时间仓编码信号的空间多模光束能够发生干涉。通过数值光线追踪模拟和原理验证实验进行测试，并制作了路径差为12ns的原型干涉仪。

Result: 原型干涉仪实现了12ns的路径长度差，在0.4°的大视场范围内保持高干涉可见度，同时兼顾了小型化的外形尺寸。

Conclusion: 该场加宽多模干涉仪为自由空间量子或经典光通信提供了一种紧凑、无源的多模干涉方案，能够满足长时间仓间隔的时间仓编码信号需求，避免了对自适应光学系统的依赖。
...
### [45] [Spectral Localization Principle for Entanglement Harvesting](https://arxiv.org/abs/2608.13449)
*Hao Xu*

Main category: quant-ph

TL;DR: 本文提出一个统一的物理原理：两个局域探测器从量子场中提取纠缠的能力完全由场的有效频谱密度的局域化程度决定。作者在分析可解的"两个量子比特耦合到泄漏单模腔、腔再耦合连续电磁浴"模型中，推导出可提取纠缠（concurrence）的闭式最大值公式，并揭示了高Q极限下纠缠对腔损耗的鲁棒性、低Q极限下指数衰减等现象，同时将该框架与Reeh-Schlieder定理的算符化及Anderson局域化中的电导-参与比关系建立了对应。


<details>
  <summary>Details</summary>
Motivation: 纠缠提取（entanglement harvesting）是量子信息与量子场论交叉的核心问题，但以往缺乏统一的物理原理来解释不同模型中提取纠缠的行为。作者希望找到决定纠缠提取能力的单一控制参数，并提供一个解析可解的模型来在闭式层面上阐明局域化程度与可提取纠缠之间的定量关系。

Method: 作者构建了一个分析可解的物理模型：两个量子比特与一个泄漏单模腔耦合，该腔再与连续电磁浴耦合。利用该模型推导出最大可提取纠缠（concurrence）的闭式表达式 C_max(Q)=2e^{-π/(2Q)}(1+e^{-π/(2Q)})/(1+3e^{-π/Q})，其中Q是量子比特-腔失谐与腔线宽之比。通过高Q与低Q极限分析揭示纠缠行为的渐近特征，并将Q解释为有效频谱密度的逆参与比（IPR），从而建立统一的物理原理。

Result: 推导出最大并发度的精确闭式公式；高Q极限下 C_max≈1-π²/(16Q²)，表明在小腔线宽条件下纠缠对腔损耗高度鲁棒；低Q极限下并发度指数衰减至零，体现连续场的不可逆电阻（reservoir）特性。揭示了Q作为唯一无量纲参数，控制着从确定性门操作纠缠（Q→∞）到真空纠缠提取（Q→0）的过渡。并发度与IPR的对应关系类似于Anderson局域化中电导与参与比的关系。该预测曲线原则上可在超导电路QED实验中直接观测。

Conclusion: 本文提供了一个统一的物理原理：纠缠提取完全由有效频谱密度的局域化程度（即Q参数）决定。该框架不仅将Reeh-Schlieder定理在可操作层面量化（即局域探测器可获取的真空关联分数），还揭示了与Anderson局域化的深层数学类比。由于预测的C_max(Q)曲线在超导电路QED中可观测，该工作为实验验证纠缠提取的统一原理提供了清晰路径。
...
### [46] [Strong unitary designs in optimal depth and space](https://arxiv.org/abs/2608.13491)
*Teodor Parella-Dilmé,Júlia Barberà-Rodríguez,Salvatore F. E. Oliviero,Antonio A. Mele*

Main category: quant-ph

TL;DR: 本文构造了仅使用n个系统量子比特、最优Θ(log n)全连接电路深度下的强近似酉k-设计，解决了Schuster等人留下的开放问题。


<details>
  <summary>Details</summary>
Motivation: 强酉设计要求即使在算法可访问酉算子及其逆、转置和复共轭的情况下也能与Haar随机不可区分，是更严格的操作性近似概念。物理上要求混沌（scrambling）发生在系统自身内部，因此Schuster等人留出了开放问题：能否仅使用系统量子比特在对数深度内生成强酉设计。

Method: 新引入的关键工具是对完美匹配系综（perfect-matching ensemble）的对数深度Pauli混合界。该系综的每一层将量子比特均匀随机配对并施加独立的随机两比特门。这个界控制了前向-反向混合双查询情形，再与已有的设计构造和拼接（gluing）结果相结合，获得任意固定阶数的强酉设计。

Result: 对于任意固定设计阶数k和可测误差容限，成功构造了最优Θ(log n)深度下的强近似酉k-设计，且仅使用n个原始系统量子比特。

Conclusion: 该结果证明了在无辅助量子比特的对数深度电路中即可实现强酉设计，达到深度下界最优，解决了此前文献中遗留的开放问题。
...
### [47] [Quantum correlations and Basis-Independent Coherence Distribution in Two Gravitational Cat States](https://arxiv.org/abs/2608.13493)
*Mostafa Mansour,Mansoura Oumennana*

Main category: quant-ph

TL;DR: 本文研究双阱势阱中通过牛顿引力相互作用耦合的一对大质量粒子系统中量子关联与基无关相干性的分布，揭示了温度、引力耦合强度和单粒子能量尺度如何调控集体相干性与局域相干性的再分配。


<details>
  <summary>Details</summary>
Motivation: 探索引力系统中的量子关联和非经典相干性，特别是在大质量粒子体系中引力相互作用如何影响量子信息的分布，为量子引力界面和量子信息理论的交叉研究提供理论基础。

Method: 利用Bures距离纠缠度和量子失谐表征非经典关联，采用量子Jensen-Shannon散度的平方根作为基无关的相干性度量，将总相干性分解为集体相干性和局域相干性两个分量，并系统分析温度T、引力耦合Δ和单粒子能量尺度w对相干性再分配的影响。

Result: 局域相干性CL对热涨落比集体相干性CC更鲁棒；增大引力耦合Δ会通过增强引力诱导的粒子间关联而优先增强集体相干性。

Conclusion: 引力相互作用能够在双粒子体系中有效调节量子相干性的集体与局域分配，为该类系统的量子信息处理应用提供了理论指导。
...
### [48] [Exponential quantum advantage for learning signals with a single qubit](https://arxiv.org/abs/2608.13521)
...
TL;DR: 本文提出量子特征感知（Quantum Feature Sensing）算法，证明将单个可控量子比特耦合到常规传感器可将学习经典信号所需的测量次数指数级减少，并在超导腔-量子比特平台上实验实现了10^7倍的测量次数缩减，同时建立了统一的量子相空间推断（QΨ）理论框架。


<details>
  <summary>Details</summary>
Motivation: 量子优势通常需要远超现有实验平台的处理能力，难以在近期的量子技术上实现。作者希望探索是否能用单个可控量子比特对常规传感任务带来指数级增强，从而在实用的经典信号学习中建立可实验验证的严格量子优势。

Method: 提出量子特征感知算法，将单个可控量子比特耦合到常规传感器；并建立量子相空间推断（QΨ）统一理论框架，该框架可同时将实验目标和约束转化为紧下界与最优的量子增强学习算法，同时生成量子优势证书。基于超导腔-量子比特架构进行实验验证，包括傅里叶系数学习、时变信号时间关联提取和物理可观测量变换估计等基本感知任务。

Result: 超导腔-量子比特实验中实现了傅里叶振幅和时变信号学习中10^7倍的测量次数缩减；在弱信号暗物质探测模拟和无线通信应用中实现数量级的性能提升。QΨ理论框架超越了量子Fisher信息的适用范围，为实际实验任务中严格量子优势的系统性识别提供了新方法。

Conclusion: 近期的量子技术能够指数级增强我们从经典信号中学习的能力，单个可控量子比特耦合至常规传感器即可在多种基础感知任务中获得严格且可实验验证的量子优势，为实用化量子增强感知铺平了道路。
...
### [49] [Ambient unitaries don't enable shallow group designs](https://arxiv.org/abs/2608.13528)
*Maxwell West,M. Cerezo,Martin Larocca*

Main category: quant-ph

TL;DR: 本文证明在matchgate、正交、辛和Clifford群的局部最近邻次线性深度电路中，即使允许使用来自母群之外的环境酉算子（包括作用于辅助量子比特的算子），也无法形成近似2-design（Clifford情况下为4-design）。这推翻了此前认为该指数级分离可能仅局限于子群内部酉算子的可能性，并表明相关断层扫描和基准测试方案相比全酉群采样方案需要巨大的电路深度开销，同时证明了已知的线性深度设计构造在常数因子意义上是最优的。


<details>
  <summary>Details</summary>
Motivation: 此前已知次线性深度的局部最近邻电路（属于matchgate、正交、辛群）无法构成其母群的近似2-design，Clifford次线性深度电路也无法构成Clifford 4-design，存在令人瞩目的指数级分离现象。关键开放问题是：这种分离是否仅仅源于将酉算子限制在子群内部，还是即使允许使用环境酉算子这一更一般的设定下仍然成立。本文旨在填补这一空白，揭示问题的本质。

Method: 本文采用理论证明的方法，将先前关于子群内部酉算子设计的否定性结果推广到更一般的情形，即允许电路中的酉算子来自母群之外（可能涉及辅助量子比特）。通过系统性分析各类局部最近邻次线性深度电路的结构约束，证明在这些放宽条件下近似设计仍然无法实现。

Result: 证明了不存在任何局部最近邻次线性深度的酉算子集合能在上述情形下实现近似设计，即使允许使用来自子群之外的环境酉算子（含辅助量子比特）。这意味着涉及的量子断层扫描和基准测试协议需要承受巨大的电路深度代价，且已知的线性深度构造在常数因子内已达到最优。

Conclusion: 本文验证了次线性深度与线性深度之间在设计实现上的指数级分离是一个根本性的现象，而非局限于子群内部结构的伪影。这为量子信息处理中设计复杂性的理解提供了更完整的图景，并明确了相关协议在实际实现中的深度瓶颈。
...
### [50] [Quantum simulation of non-Markovian dynamical systems](https://arxiv.org/abs/2608.13533)
*Abtin Ameri,Arkopal Dutt,Hari Krovi*

Main category: quant-ph

TL;DR: 本文提出了针对非马尔可夫动力学系统（具体为带有卷积记忆核的线性 Volterra 积分微分方程，VIDEs）的高效量子算法。在一般核且M<1条件下算法实现了相对经典算法的指数级加速；当M≥1时给出了下界证明一般核问题不可解，但通过将VIDE转化为更大规模ODE集合的"马尔可夫化"（Markovianization）过程，对具有指数分解结构的内核仍可实现高效量子模拟。该框架可应用于Mori-Zwanzig形式体系下的开放量子系统和流体动力学。


<details>
  <summary>Details</summary>
Motivation: 现有量子模拟算法主要针对马尔可夫动力学系统（系统的未来演化只依赖当前状态），而对于未来演化依赖历史状态（即具有记忆性）的非马尔可夫动力学系统缺乏高效的量子算法。本文旨在填补这一空白，开发针对非马尔可夫动力学系统的量子模拟算法。

Method: 1) 针对一般卷积记忆核的VIDEs，在M<1条件下设计高效量子算法，输出编码时间区间或特定时刻状态描述的量子态，实现相对经典算法的指数级加速；2) 证明M≥1时一般核VIDEs对一类系统不可解（给出下界）；3) 针对可分解为指数函数简洁组合的结构化核，提出"马尔可夫化"（Markovianization）方法，将VIDE转化为更大幅度的ODE集合，即便M≥1也能实现高效量子算法。

Result: 在M<1条件下实现了对一般核VIDEs的高效量子模拟和指数级加速；证明了M≥1时一般核VIDEs的下界不可解性；但对结构化指数可分解核，通过Markovianization方法在M≥1情况下仍能实现高效量子算法。该方法框架可推广至Mori-Zwanzig形式体系下的开放量子系统和流体动力学应用。

Conclusion: 本文显著扩展了量子计算机可高效模拟的动力学系统范围，从马尔可夫动力学扩展到了非马尔可夫（带记忆）动力学。通过区分一般核与结构化核的处理策略（含困难度下界与马尔可夫化方法），为量子模拟非马尔可夫系统提供了理论基础和算法工具。
...
### [51] [Clifford Circuit Synthesis for Distributed Quantum Architectures with Arbitrary Network Topology](https://arxiv.org/abs/2608.13543)
*Tuomas Laakkonen*

Main category: quant-ph

TL;DR: 本文提出一种基于块矩阵高斯消元法的渐近最优综合方法，用于在局部和非局部连接均受限的分布式量子计算场景下合成CNOT和Clifford电路，并扩展至所有Clifford+RZ电路。作为应用，展示了如何利用O(nk)个块间横贯CNOT和块内Pauli测量，在CSS码中实现编码n个逻辑量子比特于k个块的CNOT电路。


<details>
  <summary>Details</summary>
Motivation: 大规模容错量子计算的实现可能比构建单个大型量子比特阵列更容易通过组合多个小型量子比特集合来完成（如块码量子纠错或分布式量子处理器）。在这些场景中，整体计算的时间和错误预算可能由非局部操作主导，因此最小化这些操作的数目具有重要意义。

Method: 基于块矩阵高斯消元法，提出渐近最优的分布式CNOT和Clifford电路综合方法。通过推广Pauli指数电路表示，将方法扩展至所有Clifford+RZ电路，并与现有的T计数优化方法自然集成。

Result: 给出了渐近最优的合成方法，并展示了在CSS码中，编码n个逻辑量子比特于k个块时，仅需O(nk)个块间横贯CNOT和块内Pauli测量即可实现CNOT电路。

Conclusion: 该方法为受限连接下的分布式量子电路合成提供了渐近最优的理论保证，并为块码量子纠错和分布式量子计算中的非局部操作优化提供了实用途径。
...
TL;DR: 本文证明每个PPT线性映射都具有有限的纠缠破坏指数，从而完整确立了PPT信道的最终纠缠破坏性质。此外，利用低纠缠维数的完全正映射，证明了一类严格包含2-超正映射的PPT映射族具有统一以3为上界的纠缠破坏指数（与维数无关），为PPT-立方猜想在完全一般性下成立提供了有力证据。


<details>
  <summary>Details</summary>
Motivation: 纠缠破坏信道和PPT信道是量子信息理论中的核心概念。理解PPT信道的纠缠破坏性质（即反复应用信道后是否最终破坏所有纠缠）是一个重要的开放问题。已有的结果仅在部分情形下成立，本文旨在完全一般性下证明这一性质，并探索PPT-立方猜想的可能性。

Method: 文章采用两种技术手段：（1）对一般PPT线性映射，证明其具有有限的纠缠破坏指数；（2）引入低纠缠维数的完全正映射，构造一个严格包含2-超正映射的PPT映射族，并证明该族中所有映射的纠缠破坏指数至多为3。

Result: 证明了所有PPT线性映射具有有限的纠缠破坏指数；构造了一个大规模PPT映射族（严格包含2-超正映射类），其纠缠破坏指数在任意维数下统一以3为上界。这些结果为PPT-立方猜想的完全成立提供了强有力的证据。

Conclusion: 本文在完全一般性下确立了PPT映射的最终纠缠破坏性质，并通过精致的构造将一大类PPT映射的纠缠破坏指数统一限制在3以内，显著推进了PPT-立方猜想的研究进展。
...
### [53] [Experimental Quantum Key Distribution in an Indefinite Causal Order](https://arxiv.org/abs/2608.13561)
...
TL;DR: 本文报告了利用光量子SWITCH实验实现BB84-like量子密码协议的方案，将Alice和Bob的测量与制备操作置于不定因果序叠加中，通过测量控制量子比特来检测窃听，无需公开和丢弃密钥比特，达到每共享量子比特0.15±0.02的平均窃听者检测概率。


<details>
  <summary>Details</summary>
Motivation: 传统BB84协议检测窃听需要公开并丢弃一部分原始密钥，浪费密钥资源。作者希望利用量子力学中的不定因果序（indefinite causal order）作为一种新的量子资源，实现在不牺牲密钥比特的情况下检测窃听者的方案。

Method: 将Alice和Bob的测量与制备操作嵌入到光子的量子SWITCH中，使操作的因果顺序处于叠加态。通过测量控制量子比特来检测窃听，而非对比密钥。实验中开发了一种新技术，允许在量子SWITCH内部测量光子偏振而不破坏路径相干性。注意，当前实现依赖后选择，尚不构成安全的QKD协议。

Result: 实验实现了平均窃听者检测概率为0.15±0.02（每个共享量子比特），且所有保留的量子比特原则上均可进行窃听检测而同时用于密钥生成，无需披露任何密钥材料。

Conclusion: 该工作为利用不定因果序检测窃听而不牺牲密钥比特提供了原理验证。虽然当前实现由于量子SWITCH内测量的后选择要求尚不能构成安全的量子密钥分发协议，但展示了不定因果序作为量子资源在密码学中的新应用潜力。
...
### [54] [Quantum-Geometric Bound on Dynamical Instability in Bosonic Systems](https://arxiv.org/abs/2608.12406)
*A. M. Tishin*

Main category: quant-ph

TL;DR: 本文研究了多模二次玻色子系统的量子几何速度与动力学不稳定性之间的关系，证明Fubini-Study速度v_FS等于flows对称拉伸部分的Frobenius范数，并导出一个由共振纯压缩器饱和的尖锐上界。


<details>
  <summary>Details</summary>
Motivation: 量子几何速度（Fubini-Study速度）和动力学不稳定性是驱动量子系统的两个自然速率，但即使在可精确求解的动力学中，二者之间的关系也尚未明确。作者希望建立这两个关键速率之间的严格数学联系。

Method: 对任意相对于裸模真空的多模二次玻色子系统，将Fubini-Study速度v_FS表示为流（flow）对称拉伸部分的Frobenius范数。利用这一表示推导出上界lambda_max <= sqrt(2) v_FS，并通过共振纯压缩器（resonant pure squeezer）证明该界的紧致性。通过失谐参量放大器（detuned parametric amplifier）的数值/解析分析展示该界不可逆，即可以固定一个速率同时变化另一个。

Result: 得到尖锐上界lambda_max <= sqrt(2) v_FS，并由共振纯压缩器饱和；同时证明该界不具有可逆性，在失谐参量放大器中可独立调控两个速率中的任意一个。

Conclusion: 量子几何速度与动力学不稳定性之间存在严格的数学关系，但该关系不是双向约束。这为理解驱动量子系统中几何与动力学速率的本质差异提供了理论基础。
...
### [55] [Is the Aharonov-Casher phase geometrical or dynamical?](https://arxiv.org/abs/2608.12427)
*Igor Kuzmenko,Y. B. Band,Yshai Avishai*

Main category: quant-ph

TL;DR: 本文研究了二维电子系统和单层石墨烯在Rashba自旋轨道耦合下的Aharonov-Casher(AC)相位。结果表明，SU(2)Rashba矢量势无法通过规范变换从二维薛定谔方程或二维狄拉克方程中消除，但在平面波解中可通过AC相位因子从一维薛定谔方程中消除。关键发现是：薛定谔方案中的AC相位是几何相位，而狄拉克方案中的AC相位是含时的动力学相位，且垂直于波矢的Rashba分量会产生无法消除的有效电子质量。


<details>
  <summary>Details</summary>
Motivation: 探究Rashba自旋轨道耦合产生的SU(2)矢量势能否通过规范变换或幺正变换消除，并系统比较薛定谔方程与狄拉克方程两种框架下的Aharonov-Casher相位性质差异。

Method: 分别用2D薛定谔方程描述非相互作用的二维导体电子系统和2D狄拉克方程描述单层石墨烯无质量费米子，将RSOI表示为SU(2)Rashba矢量势，分析平面波解下是否存在幺正矩阵消除该矢量势，并对比两种方案中AC相位的数学形式与物理性质。

Result: 发现Rashba矢量势不能通过规范变换从2D SE或2D DE中消除；但平面波解中，一个AC相位因子幺正矩阵可从1D SE中消除该势。对于DE，Rashba势的两个分量中垂直于波矢的分量产生不可消除的有效质量，平行于波矢的分量产生的AC相位可通过含时幺正变换消除，因此狄拉克AC相位是动力学相位，而薛定谔AC相位是几何相位。

Conclusion: 薛定谔方案与狄拉克方案中的AC相位具有本质不同的物理性质：前者为几何相位，后者为动力学相位，揭示了不同电子系统框架下Rashba自旋轨道耦合与几何/动力学相位关系的深刻差异。
...
### [58] [Stabilizer complexity and the Python's lunch](https://arxiv.org/abs/2608.12472)
*Abhirup Bhattacharya,Jatin Narde,Onkar Parrikar,Suprakash Paul*

Main category: quant-ph

TL;DR: 本文研究了全息CFT中部分纠缠热（PET）态约化密度矩阵的稳定子复杂度，发现其Wigner负性由外极值面面积与最小极值面面积之差决定，表现为指数形式exp[(A_out - A_min)/8G_N]，从而稳定子复杂度在没有"蟒蛇午餐"时为O(1)，存在"蟒蛇午餐"时呈指数增强。


<details>
  <summary>Details</summary>
Motivation: 作者希望从全息对偶的角度理解量子态的稳定子复杂度（stabilizer complexity）与魔幻（magic）资源。Wigner负性作为可操作的魔幻单调量，可解释为量子电路制备该态所需的经典模拟复杂度。通过研究PET态在给定能量下的约化密度矩阵，作者旨在建立体几何结构（特别是极值面的存在性，即"蟒蛇午餐"）与边界态魔幻复杂度之间的定量联系。

Method: 作者在固定能量边界条件的全息CFT中构造PET态，利用CFT能谱及重算子插入的伪随机性假设，计算约化密度矩阵相对于微正则密度矩阵的Wigner负性。推导过程中关联了体几何中的外极值面（outer extremal surface）和最小极值面（minimal extremal surface）的面积，从而将Wigner负性表达为这两者面积差的指数函数。

Result: 推导出Wigner负性为exp[(A_out - A_min)/8G_N]。当体几何中没有"蟒蛇午餐"时（即无额外的极值面结构），稳定子复杂度为O(1)；当存在"蟒蛇午餐"时，稳定子复杂度获得指数增强。这建立了量子信息论中的魔幻资源与全息几何中的拓扑结构（极值面）之间的定量对应。

Conclusion: 该工作揭示了一个重要全息对应关系：体几何中"蟒蛇午餐"结构的存在会导致边界约化密度矩阵的稳定子复杂度呈指数增长。这为理解量子引力的computational complexity与量子信息论中的magic resource之间的关系提供了新的视角，也进一步支持了"复杂度=体分（complexity=volume-like）"猜想类方向的研究。
...
### [59] [Dual Gauge Theory for Two Dimensional Superfluid Turbulence](https://arxiv.org/abs/2608.12485)
*Tobias Helbig,Sayak Bhattacharjee,Srinivas Raghu*

Main category: quant-ph

TL;DR: 本文通过二维超流体中点状涡旋与涌现的2+1维U(1)规范场的耦合动力学，描述了超流体湍流。超流体动能级联等价于对偶电场能量的级联，并发现与二维流体湍流的Kolmogorov标度律一致的湍流级联，同时观察到同号涡旋聚集以及逆能量级联现象。


<details>
  <summary>Details</summary>
Motivation: 二维超流体湍流是一个基础但复杂的问题，理解其湍流级联机制（正/逆级联）具有重要的物理意义。作者通过将对偶规范理论框架引入超流体湍流研究，旨在为超流体湍流提供一个新的、等价的描述框架，从而更深入地理解二维超流体中的能量级联与涡旋动力学行为。

Method: 作者将二维超流体中点状涡旋的动力学与涌现的2+1维U(1)规范场耦合起来，利用对偶规范理论的运动方程在存在驱动和耗散的情况下研究超流体湍流。在点状涡旋极限下，对偶运动方程直接还原为超流体的流体动力学方程，并通过计算动能通量来分析湍流级联的性质。

Result: 获得了与二维流体湍流Kolmogorov标度律一致的湍流级联；观察到了同号涡旋的聚集现象；通过计算动能通量证明了湍流区域表现出逆能量级联（inverse energy cascade）。

Conclusion: 对偶规范理论框架可以等价并且有效地描述二维超流体湍流，其中超流体动能级联可等价为对偶电场能量的级联。该框架重现了Kolmogorov标度律，并揭示了二维超流体湍流中同号涡旋聚集与逆能量级联等关键物理特征。
...
### [60] [Type III von Neumann Algebras are Magical](https://arxiv.org/abs/2608.12512)
*Mudassir Moosa*

Main category: quant-ph

TL;DR: 本文论证了Type III von Neumann代数本质上需要无限量的magic（非Clifford门资源）。作者考虑有限维量子系统（如格点系统）的热力学极限，证明如果热力学极限下的态仅具有有界数量的magic，则相应的局部von Neumann代数不可能是Type III的。这对量子场论的量子模拟有直接影响，因为局部子区域的代数已知是Type III的。


<details>
  <summary>Details</summary>
Motivation: 探索Type III von Neumann代数（描述无穷维量子系统如量子场论和量子统计力学的数学结构）所需的magic资源量。由于Type III代数中密度矩阵和迹等标准概念失效，其是否本质上需要无限量magic是一个根本性的开放问题，对于理解容错量子计算中非Clifford门的资源需求至关重要。

Method: 作者从有限维量子系统（如格点系统）的热力学极限出发进行分析。通过论证：若热力学极限下的态只含有有界数量的magic（即非Clifford门资源有限），则相应的局部von Neumann代数不能是Type III类型。这是一个基于magic资源有界性与代数类型之间关系的理论论证。

Result: 证明了Type III von Neumann代数本质上要求无限量的magic资源。具体而言，热力学极限中若态只含bounded magic，则其局部von Neumann代数必然不是Type III的。由于量子场论局部子区域代数已知为Type III，这意味着量子场论的模拟本质上需要无限多的非Clifford门资源。

Conclusion: Type III von Neumann代数与无限magic资源之间存在根本性联系。这为量子场论量子模拟的资源复杂度提供了新的理论下界，表明模拟量子场论不可能仅依靠有限量的magic资源来完成，从而对容错量子计算的资源规划具有重要指导意义。
...
### [61] [Ising-machine-like Driving Condition Restrictions in SSBM and Additional Pseudo-annealing- Driving Method for SSBM that Achieves Best Cut Value Search -](https://arxiv.org/abs/2608.12545)
*Toshiya Sato*

Main category: quant-ph

TL;DR: 本文在前作提出的自发对称破缺机（SSBM）基础上，探讨其能够表现出类似连续变量伊辛机行为的条件，并发现通过在这些条件下加入类似物理退火的效应，SSBM 能够在K_{2000}基准问题上搜索到已知最优切割。


<details>
  <summary>Details</summary>
Motivation: 前作已验证SSBM在K_{2000}大规模基准问题上可收敛至已知最优的99.7%，本文旨在进一步探索SSBM具备连续变量伊辛机类行为的条件，并通过物理退火效应提升其求解能力至已知最优水平。

Method: 分析SSBM展现连续变量型伊辛机行为的条件，并在满足这些条件的基础上引入类似物理退火的效应，对K_{2000}基准问题进行求解实验。

Result: 通过添加类似物理退火的效应，SSBM在K_{2000}问题上能够搜索到已知最佳切割值。

Conclusion: SSBM在特定条件融合物理退火机制后，可达到连续变量伊辛机的搜索能力，甚至获得已知最优解，进一步验证了其作为组合优化求解器的潜力。
...
### [62] [One-sided stripe supersolidity from engineered non-axisymmetric dipolar interactions](https://arxiv.org/abs/2608.12867)
*Chao Zhang*

Main category: quant-ph

TL;DR: 该论文研究软核Bose-Hubbard模型中条纹超固体的掺杂行为。通过工程化非轴对称偶极相互作用锁定(q,0)条纹通道，发现半填充母体对空穴和粒子掺杂的响应不对称：空穴侧形成锁定共度条纹固体（超流刚度为零），粒子侧则形成具有有限压缩率、有限超流刚度和增强双占据的条纹超固体。增大U/t至硬核极限后粒子侧超固体消失，说明位点软性是相位相干缺陷扇区的关键。


<details>
  <summary>Details</summary>
Motivation: 超固体结合了密度序与相位相干，而掺杂的晶格固体提出了一个基本问题：加入的缺陷能否在不熔化有序背景的情况下变得相干。作者希望通过研究掺杂的条纹固体，理解缺陷相干性与背景有序之间的相互作用机制。

Method: 采用软核Bose-Hubbard模型，含各向同性跳跃和工程化的非轴对称偶极相互作用V_ij=V_2(x_ij^2-y_ij^2)/r_ij^5+W_6/r_ij^6。符号变化的d_{x^2-y^2}分量选择固定的(q,0)条纹通道，W_6/r^6核心项稳定短距离吸引分支。使用无符号问题的量子蒙特卡洛方法配合虫算法求解。

Result: 半填充条纹母体对掺杂具有不对称响应：空穴侧形成锁定的共度条纹固体，超流刚度消失；粒子侧形成条纹超固体，具有有限压缩率κ>0、有限超流刚度ρ_s>0和增强的双占据D。增大U/t趋向硬核极限时，粒子侧超固体消失，表明doublon类缺陷被投影掉后相位相干消失。

Conclusion: 工程化偶极核选择固定的(q,0)条纹通道，而位点软性（onsite softness）选择相位相干的缺陷扇区。这两个因素共同决定了掺杂条纹系统的超固体行为。
...
### [63] [Critical Microwave Mach-Zehnder-Type Interferometry with Dual-LO Rydberg Atoms](https://arxiv.org/abs/2608.13222)
...
TL;DR: 本文提出并演示了一种基于里德伯原子和双本地振荡器（dual-LO）结构的微波马赫-曾德尔干涉仪，实现了优于0.1°的相位解析精度、完整360°相位覆盖、超过25 dB的相位-幅度转换灵敏度增强，并拓展至微波传播距离（<20 μm @ 5.7 GHz）和偏振角（>0.1°）计量，无需复杂光学配置和锁相检测。


<details>
  <summary>Details</summary>
Motivation: 高精度微波相位测量广泛应用于无线通信、分布式雷达、等离子体诊断和天线计量等领域，但现有基于里德伯原子的方法在相位分辨率、测量范围和系统复杂度之间存在权衡，亟需一种同时兼顾高分辨率、全范围覆盖和低复杂度的方案。

Method: 采用双本地振荡器配置构建里德伯原子微波马赫-曾德尔型干涉仪：两个本地振荡器在里德伯介质中建立两条相干干涉路径，与信号场相干混频产生中频输出；利用相位到强度的传递特性实现临界点增强效应，通过可重构的dual-LO架构实现直接相位提取和全360°无模糊覆盖；同时利用干涉传递机制进行传播距离和偏振计量。

Result: 相位解析精度优于0.1°，覆盖完整360°相位范围；在临界干涉点附近获得超过25 dB的相位-幅度转换灵敏度增强；在5.7 GHz下实现低于20 μm的传播距离测量精度，偏振角分辨率优于0.1°。

Conclusion: 该方法消除了对复杂光学配置和锁相检测的需求，提供了一种简单、可扩展、可重构的马赫-曾德尔型量子微波干涉测量框架，适用于多功能高精度微波计量。
...
### [64] [Inductively-protected Andreev (IPA) spin qubit](https://arxiv.org/abs/2608.13530)
*J. L. del Olmo N.,F. J. Matute-Cañadas,A. Levy Yeyati,R. Seoane Souto,R. Aguado*

Main category: quant-ph

TL;DR: 本文提出了一种电感保护的安德烈夫自旋量子比特（IPA），通过将ASQ并联线性电感，将自旋量子比特态分离到相位空间的不同势阱中，几乎消除了波函数重叠，从而显著延长弛豫时间。该量子比特等效于两个重磁通型量子比特（重极限中的两个fluxonium），兼具受保护超导量子比特的长相干时间、低频基态流形和大非谐性，以及自旋自由度的操作优势。


<details>
  <summary>Details</summary>
Motivation: 安德烈夫自旋量子比特（ASQ）利用量子点约瑟夫森结中准粒子自旋与超导性之间的相互作用形成自旋分辨的约瑟夫森势，但其弛豫时间受自旋态间波函数重叠限制。作者希望通过引入电感分流来保护该自旋量子比特，延长其相干时间。

Method: 将ASQ与线性电感并联分流，使自旋量子比特态在相位空间中分别落入不同的势阱中，从而几乎完全消除两个自旋态波函数之间的重叠，实现电感保护的安德烈夫自旋量子比特（IPA）。该方案从理论上等价于两个重磁通极限下的fluxonium量子比特（每个自旋对应一个）。

Result: 电感分流显著延长了ASQ的弛豫时间；IPA量子比特等效于两个重极限fluxonium的叠加，结合了受保护超导量子比特的优势（长相干时间、低频基态流形、大非谐性）与自旋自由度的操作便捷性。

Conclusion: IPA量子比特通过电感保护策略实现了自旋量子比特与受保护超导量子比特優势的融合，为半导体-超导体混合量子计算平台提供了一种具有长弛豫时间和优越操作特性的新型量子比特方案。
...
### [65] [TCAD Cryogenic Model Parameters Calibration based on 65 nm NMOS Experimental Data](https://arxiv.org/abs/2608.12692)
*Wesley Xi,Vikram Magendar,Alhaji A Sharka,Hiu Yung Wong*

Main category: physics.comp-ph

TL;DR: 本文对65 nm n型MOSFET在292 K至4.5 K的低温范围内进行了流片和测量，并通过TCAD仿真校准了模型参数，使其与实验电学特性（CV、IDVG、IDVD）精确匹配。校准的模型包括Lucent迁移率模型（扩展PhuMob模型、Lombardi表面迁移率模型和Hänsch高场迁移率模型）及带尾模型。研究还发现除饱和速度外无需引入新的温度依赖模型，且该校准方法可推广至其他工艺和器件类型。


<details>
  <summary>Details</summary>
Motivation: 低温（深低温）工作条件下，MOSFET的传输特性与室温下显著不同，但现有TCAD模型在低温范围内对65 nm及以下先进节点的适用性缺乏验证。作者希望通过实验与仿真结合，校准出适用于宽温域（292 K至4.5 K）的TCAD模型参数，并为其他工艺节点和器件类型的低温建模提供通用校准策略。

Method: 首先在65 nm工艺上流片并制作nMOSFET器件，在292 K至4.5 K温区内进行电学测量；利用TEM表征器件实际尺寸以确保几何建模准确；随后使用TCAD仿真工具，通过校准模型参数使仿真CV、IDVG、IDVD曲线与实验数据吻合。主要校准的物理模型包括：Lucent模型（由扩展PhuMob模型、Lombardi表面迁移率模型和Hänsch高场迁移率模型组成）以及带尾模型（调整带宽以拟合各温度下的亚阈值摆幅）。

Result: 在292 K至4.5 K全温区内，使用校准后的TCAD模型参数，仿真CV、IDVG和IDVD曲线与实验曲线拟合良好。校准过程中发现，除饱和速度外，其他模型均无需额外引入新的温度依赖关系。此外，由于TCAD模型本身不具有工艺特定性，校准后的参数具有跨工艺节点移植的潜力。

Conclusion: 本文证明了通过系统化的校准流程，现有TCAD物理模型可以很好地描述65 nm nMOSFET在4.5 K至292 K宽温域内的电学特性。所提出的校准流程和拟合策略具有通用性，可推广至其他晶体管类型和工艺技术，为低温集成电路（如量子计算控制电路）的器件建模提供了可靠基础。
...
### [66] [A discrete duality finite volume method with harmonic average for semiconductor drift-diffusion equations](https://arxiv.org/abs/2608.12808)
*Shuya Liu,Zhicheng Liu,Bo Lin,Chijie Zhuang,Qingyuan Shi,Weizhu Bao,Rong Zeng*

Main category: physics.comp-ph

TL;DR: 本文提出将调和平均稳定化技术集成到离散对偶有限体积法（DDFV-HA）中，用于求解半导体器件中的稳态漂移-扩散模型。相比经典的有界体积Scharfetter-Gummel（FVSG）方法，DDFV-HA在低质量或不规则网格上表现更可靠和准确，同时在高品质网格上与FVSG相当。通过在真实晶闸管上的应用验证了该方法在复杂不规则域的适用性。


<details>
  <summary>Details</summary>
Motivation: 经典FVSG方法依赖于Voronoi图和Delaunay网格，在高品质Delaunay网格上表现良好，但在不规则或畸变网格上效果不佳。这限制了其在复杂、不规则半导体器件仿真中的应用，因为此类域中难以生成高品质网格。

Method: 将调和平均稳定化（harmonic average stabilization）技术融入离散对偶有限体积法（discrete duality finite volume method，DDFV），提出DDFV-HA新格式。通过在高、低品质网格上进行DDFV-HA与FVSG的对比实验，并在真实晶闸管器件上应用验证。

Result: 实验表明，DDFV-HA在高品质网格上能匹配FVSG的精度；在低品质网格上比FVSG更可靠、更准确。在真实晶闸管上的应用进一步确认了该方法在复杂、不规则域中半导体仿真的适用性与鲁棒性。

Conclusion: DDFV-HA是一种克服网格质量限制、适用于复杂不规则几何域的半导体漂移-扩散仿真数值方案，在高品质网格下可与经典FVSG相当，在低品质网格下更具优势，适合作为复杂器件仿真的可靠选择。
...
### [67] [Using Diffusion Models to Estimate Uncertainties in Analytic Continuation](https://arxiv.org/abs/2608.13123)
*Sagi Meir,Daniel Freedman,Barak Hirshberg*

Main category: physics.comp-ph

TL;DR: 本文提出一种基于扩散模型的生成式框架用于解析延拓问题，通过学习与给定虚时关联函数一致的光谱分布，实现了不确定性量化，并通过新的"不确定性伪体积"指标评估每个反演问题的内在难度。应用于液态仲氢的路径积分分子动力学模拟数据，成功给出了自扩散系数的误差棒并识别出可能的虚假高频峰。


<details>
  <summary>Details</summary>
Motivation: 解析延拓（虚时关联函数到实频域的逆拉普拉斯变换）是典型的病态反问题，大量谱函数对应同一iTCF。现有基于回归的神经网络方法仅输出单一解（近似于真实解空间的平均），无法捕捉合理功率谱的完整分布，缺乏基于严格概率论的置信度量。

Method: 引入扩散模型的生成式框架，学习与给定iTCF一致的光谱分布；通过分析该分布的展宽和结构来量化每个反演问题的内在硬度，提出新的度量指标——不确定性伪体积（uncertainty pseudo-volume）；应用框架于液态仲氢的路径积分分子动力学模拟所得iTCF。

Result: 在液态仲氢的iTCF应用中，成功获得带误差棒的自扩散系数，并标记了一个可能的次级高频伪影峰；相比之下，以往的不确定性量化尝试缺乏严格概率基础，而该生成式方法具有具体概率依据，为重建功率谱提供了更理论基础的置信度量。

Conclusion: 该扩散生成式框架不仅能够从病态反问题中提取完整的光谱解分布以实现严谨的不确定性量化，还能用不确定性伪体积客观评估每个反演问题的固有难度，比传统和现有NN方法提供更可靠、更理论自洽的置信度度量。
...
### [68] [Modeling Bond-Dependent Kitaev-like interaction in 2D Edge-Sharing Tetrahedral Magnets: FeX (X=Te, Se)](https://arxiv.org/abs/2608.13427)
*Mengdong Li,Can Huangb,Bingjie Liu,Zhixin Liu,Yanfei Pan,Jiyu Fan,Chunlan Ma,Daning Shi,Yan Zhua*

Main category: physics.comp-ph

TL;DR: 该论文在单层FeTe和FeSe中构建了Kitaev类模型，揭示了由硫族介导的自旋轨道耦合和四面体晶体场几何共同诱导的键依赖Ising型相互作用，将Kitaev物理从八面体配位拓展到共享边四面体环境。


<details>
  <summary>Details</summary>
Motivation: 以往键依赖磁性相互作用（如Kitaev模型）主要被认为存在于共享边八面体系统中，而共享边四面体环境（与铁基超导体母体化合物相关）中是否存在类似相互作用是一个悬而未决的问题。

Method: 构建单层FeTe和FeSe的Kitaev类模型，通过第一性原理计算推导磁各向异性能映射，在代表性线性磁序上分离键依赖贡献与单离子各向异性，从而解析出键依赖的Ising型相互作用。

Result: 发现在FeTe中Kitaev类相互作用主导磁各向异性，而在FeSe中它与符号相反的单离子各向异性激烈竞争。由此产生的非共线局域各向异性轴引发本征单自旋阻挫，为超越各向同性交换模型的磁无序提供了微观机制。

Conclusion: 该研究确立了共享边四面体磁体作为键依赖相互作用的新场景，将Kitaev物理的适用范围从八面体配位拓展到更广泛的晶体环境。
...
### [69] [Characteristic Mode Analysis of Plasmonic Nanostructures Using Hydrodynamic Volume Integral Equation](https://arxiv.org/abs/2608.13435)
*Meruyert Khamitova,Ran Zhao,Doolos Aibek Uulu,Sebastian Celis Sierra,Hakan Bagci*

Main category: physics.comp-ph

TL;DR: 本文提出了一种将特征模式分析扩展到等离子体纳米结构的框架，通过将流体动力学模型与体积积分方程结合，将问题转化为广义特征值问题，从而实现与激励无关的模态分析。该方法在一维纳米球、纳米棒和纳米二聚体上验证，能够识别局部模型无法捕捉的非局部响应相关的额外共振。


<details>
  <summary>Details</summary>
Motivation: 金属纳米结构在亚波长尺度上限制电磁场，可作为等离子体纳米天线。在此尺度下，金属的响应变得非局部，流体动力学模型被广泛用于捕捉该响应。然而，现有求解器只能提供对给定激励的响应，无法直接揭示结构的内在共振特性，因此需要一种与激励无关的模式分析方法。

Method: 针对简单金属，将耦合的流体动力学方程与体积积分方程简化为单一的水动力学体积积分方程（以感应电流为未知量）。将该方程离散化并构建为特征模式分析框架下的广义特征值问题，求解得到特征模式电流和模态显著性曲线。

Result: 通过纳米球、纳米棒和纳米二聚体三种金属纳米结构进行了验证。结果表明，该方法能识别每种结构的内在共振，包括给定激励未能激发的共振，以及由非局部响应引起的、在局部模型中不存在的额外共振。
...
### [70] [Mechanisms of Nanoscroll Formation and Particle Encapsulation in Janus MXenes](https://arxiv.org/abs/2608.12439)
*Sasan Rezaee,Fatemeh Mohammad Dezashibi,Ould el Moctar,Hossein Darban*

Main category: physics.comp-ph

TL;DR: 本文利用大规模反应性分子动力学模拟（经DFT和实验验证），研究了三种Janus MXenes在纳米卷（nanoscroll）形成过程中的原子级机制，发现1-7%的晶格诱导应变产生弯曲力矩，决定卷曲、弯曲或纳米管形成。MXene在锚定纳米颗粒存在下可自发卷曲形成核壳复合材料，从而获得局部可调、增大的层间距，为MXene基储能电极提供设计路线，但模拟揭示封装过程中释放H2气体会形成纳米气泡，可能缩短电池寿命。


<details>
  <summary>Details</summary>
Motivation: 虽然2D Janus MXenes向纳米卷的形态转移已通过实验验证了可扩展合成路线，但纳米卷形成的原子级机制仍不清楚。理解该机制对于调控MXene纳米卷的结构和性能、进而开发高性能储能材料至关重要。

Method: 使用大规模反应性分子动力学（reactive MD）模拟，以密度泛函理论（DFT）和实验的结构及弹性数据为验证基准；模拟了三种Janus MXenes (Tx)Ti2C(Ty)（Tx/Ty分别为-b、-O、-OH），包括方形和无限宽薄片，长度从10nm到超过120nm，并模拟了锚定纳米颗粒下MXene的自发卷曲封装过程。

Result: 发现1-7%的晶格诱导应变产生弯曲力矩，导致片材卷曲、弯曲或形成纳米管，取决于曲率和初始片材尺寸。120nm长的MXene可形成多壁纳米卷：(O)Ti2C(OH)和(b)Ti2C(OH)层间距约0.7nm、内径约7nm，而(b)Ti2C(O)层间距约1.7nm、内径超过20nm。锚定纳米颗粒可诱导自发卷曲形成核心@壳复合材料，颗粒局部变形纳米卷并扩大层间通道；但模拟显示封装过程中释放H2气体，促进纳米气泡形成，可能降低电池寿命。

Conclusion: 自发卷曲的Janus MXene包裹锚定纳米颗粒可形成核壳复合材料，实现局部可调且增大的层间距，为MXene基储能电极提供了有前景的设计路线。但H2释放导致的纳米气泡问题需在器件设计中加以考虑，以保障电池寿命。
...
TL;DR: 本文提出结构保持认知神经网络(S-PENNs)，这是一种用于具有硬约束的科学机器学习模型的不确定性量化通用框架。通过在预训练模型的受约束组件上附加轻量级epinets，确保每个采样实现都保持物理可容许性，并结合分裂共形预测提供有限样本边际覆盖保证。在GENERIC动力学上验证，相比深度集成将计算成本降低约1-3个数量级。


<details>
  <summary>Details</summary>
Motivation: 标准的不确定性量化方法在硬约束(硬编码物理结构)模型中可能违反所编码的容许性条件，或需要修改网络架构，或带来高昂的计算成本。亟需一种既能保持物理结构约束又不牺牲计算效率的UQ框架。

Method: S-PENNs通过在预训练模型的受约束组件上附加轻量级epinets来引入认知不确定性，确保所有采样实现满足物理容许性；对于GENERIC动力学，该框架保证热力学一致性(第一、第二定律)；进一步与分裂共形预测(split conformal prediction)结合作为事后校准方法，提供有限样本边际覆盖保证的预测区间。

Result: 在三个数值例子(谐波振荡器耦合热浴、理想化化学马达(ODE)、一维粘塑性模型(PDE))上验证，S-PENNs产生热力学一致的随机实现和良好校准的预测区间，计算成本比深度集成降低约1-3个数量级。

Conclusion: S-PENNs为具有硬或软约束的科学机器学习模型提供了通用且计算高效的UQ框架，目前聚焦于GENERIC动力学，但可广泛扩展到计算力学中的各类模型。
...
### [72] [Integrated Alchemical and Conformational Enhanced Sampling for Solvation Free Energy Calculations](https://arxiv.org/abs/2608.12691)
*Gabriela B. Correa,Charlles R. A. Abreu,Nisanth N. Nair,Mark E. Tuckerman*

Main category: physics.comp-ph

TL;DR: 本文开发了一个统一的λ-动力学增强采样框架，结合质量缩放、OPES和d-AFED方法，用于解决溶剂化自由能计算中的慢变量采样问题。该框架对刚性有机溶质和柔性肽类溶质均表现出优异的性能，将柔性溶质的实验辛醇-水分配系数预测误差从0.75 log单位降低到0.30 log单位。


<details>
  <summary>Details</summary>
Motivation: 分子动力学模拟中准确计算溶剂化自由能面临多重采样挑战：溶剂坐标、溶质构象模式以及alchemical坐标λ都是耦合的慢变量。传统方法（如预先定义λ窗口的副本交换）需要大量并行模拟和预定义参数，效率受限；而对柔性溶质，构象采样与alchemical采样的耦合进一步加剧了困难。

Method: 提出一个统一的增强采样协议，包含三个关键组件：(1) 对刚性溶质，先使用HREM（哈密顿副本交换）结合质量缩放（将辛醇原子质量降低10倍）来量化溶剂弛豫效应；(2) 开发λ-OPES双偏置λ-动力学策略，融合"standard"和"explore"两种OPES变体，无需预定义λ窗口即可沿alchemical坐标促进转换；(3) 对柔性溶质，将λ-OPES与d-AFED（驱动绝热自由能动力学）耦合，对选定的骨架和二面角进行同时的alchemical和构象增强采样。

Result: 质量缩放将辛醇溶剂弛豫的收敛速度提高5倍以上，同时保持平衡溶剂化自由能不变。λ-OPES在无需预定义λ窗口或多个并行模拟的情况下，达到了与副本交换相当的时间尺度收敛。对于N-乙酰氨基酸酰胺类柔性溶质，λ-OPES-d-AFED组合策略将辛醇-水分配系数的平均绝对误差从单独使用λ-OPES的0.75 log单位降低到0.30 log单位。

Conclusion: 该工作建立了一个集成的增强采样协议，可同时适用于刚性有机溶质和柔性肽类溶质，为将alchemical自由能方法推广到更大、更复杂构象体系奠定了基础。
...
### [73] [Finite-Temperature Thermodynamics of Cu(100) Oxidation: Missing-Row Reconstruction, Defect States, and Order-Disorder Transition from Nested Sampling](https://arxiv.org/abs/2608.12787)
*Felix Riccius,Karsten Reuter,Hendrik H. Heenen,Jutta Rogal*

Main category: physics.comp-ph

TL;DR: 本文展示了一种结合嵌套采样、机器学习势函数和GPU加速的模拟方法，可在第一性原理预测精度下探索金属表面构型空间，并成功预测了Cu(100)早期氧化中实验观测到的复杂(2√2×√2)R45°-O缺行重构，同时提供了温度依赖的表面演化细节。


<details>
  <summary>Details</summary>
Motivation: 金属表面会随化学环境发生结构、组成和形态变化，理解表面演化与外部条件的耦合对调控表面功能和稳定性至关重要，但传统方法难以在无先验条件下获得完整的配分函数和任意温度下的系综平均值。

Method: 采用嵌套采样（nested sampling）方法探索完整构型空间以估计配分函数，结合机器学习训练的原子间势（MLIPs）保证第一性原理预测精度，并利用高效的GPU实现和定制化采样移动策略提升计算可行性。

Result: 成功预测了Cu(100)早期氧化中实验观测到的复杂(2√2×√2)R45°-O缺行重构；通过完整配分函数详细刻画了温度依赖的表面演化，映射了缺陷态的出现以及重构表面的有序-无序转变过程。

Conclusion: 嵌套采样结合机器学习势函数与GPU加速是可行的第一性原理级表面演化预测工具，无需先验知识即可获得完整热力学信息，可用于复杂表面重构和相转变的系统性研究。
...
### [74] [Parametric Matrix Models for Emulation in Nuclear and Many-Body Physics](https://arxiv.org/abs/2608.12837)
*Patrick Cook*

Main category: physics.comp-ph

TL;DR: 本论文系统地介绍了核物理与多体物理中仿真器（emulator）的现状，并提出了参数化矩阵模型（PMMs）这一融合传统降基方法与参数化机器学习的仿真与通用机器学习框架，同时开发了开源软件包pyPMM以供研究者使用。


<details>
  <summary>Details</summary>
Motivation: 核物理与多体物理中，随着理论模型日益复杂，计算代价急剧上升，同时在拟合实验数据和量化预测不确定性时需要反复评估这些昂贵模型。因此需要开发高效的替代模型（仿真器）来解决这一计算瓶颈。

Method: 论文的核心方法是参数化矩阵模型（PMMs），它将传统降基方法（reduced basis method）与现代参数化机器学习相结合，允许根据需求保留任意程度的底层系统物理信息。论文建立了PMMs的形式化数学框架，并提供了逐步实施流程，同时开发了开源的pyPMM包，采用模块化、可扩展且GPU优化的代码实现。

Result: 论文展示了PMMs在性能、适应性、可解释性和可信度方面均表现出色，同时指出许多流行的仿真方法在数学上密切相关，并可通过权衡取舍来优化特定属性或应用。所有PMM示例均通过开发的pyPMM包实现。

Conclusion: PMMs作为一种仿真与通用机器学习框架，在保留物理可解释性的同时提供优异的仿真性能，结合开源工具pyPMM，使得任何研究者都能便捷地构建、训练和部署基于PMM的仿真器，有望推动核物理及多体物理中的不确定性量化和模型拟合研究。
...
### [75] [Time-ordered free energy in correlated quantum systems: An agentic approach](https://arxiv.org/abs/2608.12942)
*Ruo Cheng Huang,Isha Singh Le Xue,Yuxuan Qu,Paul M. Riechers,Varun Narasimhachar,Mile Gu*

Main category: physics.comp-ph

TL;DR: 本文研究智能体从时序量子态序列中提取功的最优策略问题，该序列可能非马尔可夫且由不可观测的隐马尔可夫机器生成。作者结合动态规划与计算力学方法，提出了识别可证明最优智能体策略的方法，时间复杂度随序列长度线性增长，并引入了"时间有序自由能"(TOFE)作为因果约束下时域相关量子系统中可用自由能的基本度量。


<details>
  <summary>Details</summary>
Motivation: 智能体在因果约束下只能在线操作——基于过去观测信息决定当前的能量提取策略。如何在这种受限条件下从（可能非马尔可夫的）量子态序列中提取最大功是一个开放问题，需要一个既能识别最优策略又能量化可提取自由能的理论框架。

Method: 利用动态规划与计算力学的技术，处理由隐马尔可夫机器生成的非马尔可夫量子态序列。智能体在线决策，依据历史观测选择能量提取方式，算法的时间复杂度与序列长度呈线性关系。

Result: 提出了一种可证明最优的智能体策略识别方法，时间复杂度为线性；并在此基础上引入时间有序自由能(TOFE)作为该问题场景下的功提取上限，即因果约束下时域相关量子系统可提取自由能的基本度量。

Conclusion: TOFE为因果受限条件下从时间相关的量子系统中提取自由能提供了理论基础，将计算力学、动态规划与量子热力学相结合，为理解"量子记忆"在能量提取中的价值提供了新的角度。
...
### [76] [Emergence of moiré magnetic chaos in twisted bilayer CrI3](https://arxiv.org/abs/2608.13062)
*Gyuyoung Park,OukJae Lee,Kyoung-Min Kim*

Main category: physics.comp-ph

TL;DR: 本文在扭曲双层CrI3中发现了一种无需外部驱动的全新磁性混沌类型——莫尔磁性混沌（moiré magnetic chaos）。该混沌源于层间交换阻挫稳定化的介观磁性畴，通过微磁模拟展示了弛豫过程对初始态微小扰动的极端敏感性，并揭示了最终态的高度随机性与无关联性。


<details>
  <summary>Details</summary>
Motivation: 传统磁性混沌研究局限于外加驱动力下的宏观变量，而自然界中是否存在无需外部驱动、源于材料内部本征机制的微观磁性混沌尚属空白。作者旨在探索并确立一种自主的、无驱动的磁性混沌新范式，以拓展非线性磁学的研究边界。

Method: 采用微磁模拟方法，针对扭曲双层CrI3体系进行研究。通过引入层间交换阻挫来稳定介观磁性畴并赋予系统多动力学自由度，进而考察系统在弛豫过程中对初始态微小扰动（跨越五个数量级振幅）的敏感性，计算有限时间李雅普诺夫指数，并进行统计相关分析来刻画最终畴结构的随机性。

Result: 揭示了弛豫动力学对初始态的极小扰动具有极端敏感性，有限时间李雅普诺夫指数显著，最终态敏感性在五个数量级扰动振幅范围内持续存在；统计结果表明所得畴构型是随机的且两两不相关。

Conclusion: 研究首次证实了扭曲磁性材料中存在一种微观、无驱动的自主混沌，将非线性磁学从传统的受驱动范畴扩展至本征自主混沌的新领域，为磁性混沌研究提供了全新的物理平台。
...
### [77] [Giant exciton effects and magneto-excitonic coupling in V4S9X4 2D magnetic semiconductors](https://arxiv.org/abs/2608.13093)
*Yingjie Wei,Fan Zhang,Ying Zhao,Lixin Zhou,Yan Su,Yu Guo,Jijun Zhao*

Main category: physics.comp-ph

TL;DR: 该论文提出了一种通过团簇组装设计V4S9X4（X=F, Cl, Br, I）单层材料的方法，成功在同一材料中同时实现室温铁磁性（居里温度高达507.6 K）和巨大的激子束缚能（如V4S9Br4为1.85 eV），突破了传统半导体中铁磁性与激子束缚能相互排斥的瓶颈。该材料的最低声激子为暗态（寿命1.20 ns），第一亮激子具有超快辐射衰减（86.87 ps），且可通过铁磁-反铁磁转变大幅调控激子寿命而几乎不影响巨大束缚能，为室温自旋光子学和量子信息器件提供了新范式。


<details>
  <summary>Details</summary>
Motivation: 室温自旋光电子器件需要同时具备稳健的铁磁性和巨大的激子束缚能，但两者在传统半导体中相互排斥（磁局域化会屏蔽激子）。这一根本性矛盾限制了该类器件的实现，因此亟需寻找能打破这一瓶颈的新型材料设计策略。

Method: 采用层级团簇组装设计策略：团簇内局域态同时承载局域磁矩和强电子-空穴相互作用，团簇间耦合媒介长程铁磁有序。通过第一性原理计算等方法系统研究了V4S9X4（X=F, Cl, Br, I）单层材料的铁磁性、激子性质及磁序对激子寿命的调控效应。

Result: V4S9X4单层为本征铁磁半导体，居里温度最高达507.6 K；V4S9Br4的激子束缚能为1.85 eV，暗激子寿命1.20 ns，亮激子辐射寿命仅86.87 ps；铁磁-反铁磁转变可在大范围内调控激子寿命，同时巨大束缚能几乎保持不变。

Conclusion: 团簇组装被确立为设计下一代室温自旋光子与量子信息器件的强大新范式，能够同时实现在传统材料中不可兼得的室温铁磁性、巨大激子束缚能以及可调控的激子动力学性质。
...
### [78] [Thermal transport in crystals: from the quantum Dyson equation to mesoscopic phonon hydrodynamics](https://arxiv.org/abs/2608.13339)
*Enrico Di Lucente,Michele Simoncelli,Nicola Marzari*

Main category: physics.comp-ph

TL;DR: 本文系统评述了介电非磁性晶体中热输运的多尺度理论框架，从量子声子Dyson和Kadanoff-Baym方程出发，经半经典玻尔兹曼输运形式，粗粒化到介观粘性热方程，揭示了Guyer-Krumhansl、双相滞后方程作为其特殊极限的涌现机制，并展示了这些方程对声子流体中Poiseuille流、第二声、负热阻、稳态热回流及涡旋等非扩散热输运现象的预测能力，最后给出了将这些工具推广到其他准粒子的路线图。


<details>
  <summary>Details</summary>
Motivation: 尽管声子热输运可在量子、半经典和介观连续介质等多个理论层面描述，但这些层面之间的严格理论联系与近似假设尚未被系统梳理，且非扩散热输运（如声子流体动力学）中的奇异现象（负热阻、热回流、涡旋）需要统一的框架进行预测与解释，以便将微观声子物理与宏观可观测现象相连接。

Method: 本文采用理论评述方法：(1) 从量子声子Dyson方程和Kadanoff-Baym方程出发，通过严格的近似步骤推导半经典玻尔兹曼输运方程；(2) 通过粗粒化过程建立介观粘性热方程；(3) 证明Guyer-Krumhansl方程（线性各向同性带极限）和双相滞后方程（无粘极限）是粘性热方程的特殊情形；(4) 将粘性热方程重写为Helmholtz方程和双调和方程以获得解析解；(5) 结合第一性原理模拟将微观声子物理与宏观非扩散热输运现象关联；(6) 与经典流体和电子流体的水动力学行为进行类比分析。

Result: 该评述严格建立了量子、半经典和介观多尺度理论之间的桥梁，证明粘性热方程在特定极限下可退化为Guyer-Krumhansl和双相滞后方程；系统展示了这些方程能预测Poiseuille流、第二声、负热阻、稳态热回流和涡旋等非扩散现象；并通过对Helmholtz/双调和方程的解析求解，揭示了声子流体与经典流体及电子流体在可压缩性、涡量等方面的相似性和差异性。

Conclusion: 统一的多尺度理论框架不仅能描述和预测声子水动力学中的非扩散热输运现象，还能指导其在器件中的检测、放大与控制；未来可将此方法论推广至其他准粒子系统，推动固体中集体量子输运现象的研究发展。
...
### [79] [Evaluating Electrostatic Embedding MLIP/MM for Relative Binding Free Energy Calculations](https://arxiv.org/abs/2608.13355)
*Stephen E. Farr,Gianni De Fabritiis*

Main category: physics.comp-ph

TL;DR: 本文评估了一种将静电嵌入方案集成到生产级炼金术相对结合自由能（RBFE）工作流中的方法，使用机器学习原子间势/分子力学（MLIP/MM）杂化方案，其中TensorNet2模型（AceFF-2-RESP-1）联合预测能量、力和RESP电荷。在Wang等人的基准集五个靶标上的测试表明，静电嵌入仅在TYK2上显著改善了所有精度和相关性指标（ΔΔG RMSE从0.86降至0.45 kcal/mol），而在CDK2、凝血酶、p38和JNK1上表现与经典和机械嵌入基线相当。标准单分子能量和电荷基准无法预测这种靶标依赖性的结果。


<details>
  <summary>Details</summary>
Motivation: 经典力场的固定电荷近似限制了炼金术相对结合自由能（RBFE）计算的精度。虽然MLIP/MM杂化方案通过机械嵌入可以纠正配体应变，但仍以静态点电荷描述配体-环境静电相互作用。已有的静电嵌入方案虽在简单系统中验证过，却未在工业级炼金术工作流中测试。本研究旨在填补这一空白，评估静电嵌入方案在蛋白-配体RBFE中的实际表现。

Method: 采用Semelak等人的静电嵌入方案，在AMBER家族力场（GAFF2）框架下，以均匀矩阵乘法（PME）求和的短程直接空间部分引入机器学习预测的RESP电荷，并用Thole阻尼防止炼金术变换中的极化灾难。训练了一个TensorNet2模型AceFF-2-RESP-1，基于AceFF数据集的10^6个构象，联合预测能量、力和RESP电荷（相较MBIS选择RESP以与AMBER力场兼容）。实验在Wang等人基准集中预先固定的五个靶标（TYK2、CDK2、凝血酶、p38、JNK1）上进行，每个边三个重复，采用匹配的协议。

Result: 静电嵌入在TYK2上显著改善了所有精度和相关性指标（ΔΔG RMSE从0.86降至0.45 kcal/mol，相对GAFF2），但在CDK2、凝血酶、p38和JNK1上表现与经典和机械嵌入基线相当。标准单分子能量和电荷基准无法作为靶标依赖性结果的可靠预测因子。TYK2兼顾了良好的ΔΔG精度和最低的Schrödinger基准力误差，但该模式未在其他靶标上复现。

Conclusion: 静电嵌入方案在炼金术RBFE中的收益是靶标依赖性的，虽然对TYK2效果显著，但对其他靶标未见改进。这表明当前的静电嵌入方案尚不能作为通用改进手段，其应用效果取决于具体蛋白-配体体系。标准单分子基准无法预测工作流级别的性能，提示需要更接近生产条件的评估策略。未来需要进一步研究为何靶标之间存在这种差异，并探索更稳健的嵌入策略。
...
### [80] [Intermediate scattering function of Brownian particles in a tilted cosine potential](https://arxiv.org/abs/2608.12421)
*Regina Rusch,Thomas Franosch*

Main category: cond-mat.stat-mech

TL;DR: 本文求解了倾斜余弦势中布朗粒子的Fokker-Planck方程，推导出包含两个波矢的广义中间散射函数（ISF），利用Bloch定理在谱理论框架下数值计算本征函数与本征值，并通过时间依赖微扰理论展开ISF，得到均方位移、时间依赖扩散率、偏度及非高斯参数等低阶矩。解析结果经布朗动力学模拟验证，并与谐振近似及确定性极限进行了比较。


<details>
  <summary>Details</summary>
Motivation: 理解布朗粒子在周期性势场中（如倾斜余弦势）的完整时空动力学，特别是中间散射函数能全面刻画系统关联结构，但此前缺乏系统的解析推导，需要对不同倾斜力区域的动力学行为进行深入分析。

Method: 采用谱理论结合Bloch定理求解一维过阻尼布朗粒子在倾斜余弦势中的Fokker-Planck算符，数值计算其本征函数与本征值以获得ISF和概率密度；利用时间依赖微扰理论展开ISF以推导低阶矩。

Result: 成功获得广义两波矢ISF及概率密度的解析与数值结果，推导出均方位移、时间依赖扩散率、偏度与非高斯参数的表达式，并通过布朗动力学模拟验证了理论结果在不同倾斜力区域的有效性。

Conclusion: 该谱理论框架能够全面刻画倾斜周期性势中布朗粒子的时空动力学，ISF的推导为后续研究提供了理论基础，并与谐振近似和确定性极限在不同情形下表现出各自适用范围。
...
### [81] [Thermodynamics of Learning: A Typed Four-Component Accounting of Memory, Fit, and Value](https://arxiv.org/abs/2608.12791)
*Akihito Sudo*

Main category: cond-mat.stat-mech

TL;DR: 本文为有限状态学习设备建立了类型化会计框架，将"设备已记录的内容"与"对未来任务有价值的内容"分离为四个独立分量：训练侧拟合泛函、记录相关库存、更新侧搜索账本和运营资本价值。文章在三块内容中证明了分离定理、资本化账本恒等式与价值保留对齐域，揭示了记录相关性与世界相关性均可增长但资本增益为零的现象。


<details>
  <summary>Details</summary>
Motivation: 传统学习理论将"记录"与"价值"混为一谈，但有限学习设备已经记录的信息量与其未来任务中的效用并非同一量。作者希望建立更精确的会计核算体系，区分设备在训练侧、记录侧、更新侧与运营侧的不同角色，从而解释为何记录相关性与世界相关性增长时资本价值可能不变，并为设备价值评估提供严格框架。

Method: 作者提出了一个四元分解：(1) 训练侧拟合泛函 Φ_fit；(2) 记录相关库存 J_D = I(M;D)；(3) 更新侧搜索账本 σ_M；(4) 运营资本价值 V(M;T;b)，定义为知情协议类与删除记忆读取端口后重新优化的盲协议类之间的工作间隙。在 flat* 机制下建立精确提取恒等式和普适账本恒等式，在 (F5') 稳定 M-局部更新与无丢弃记录相关条件 (f) 下推导资本化效率上界 η_cap ≤ 1，并通过两层对齐域刻画价值保留的性质，包括精确汇率和基于联合侧信息中性条件的原始记录库存汇率，标出一次性密码本证明确认边界。

Result: (I) 分离定理：对每个 n 都存在设备族使得记录相关性与世界相关性增长 n ln 2 但资本增益恰为零；在 flat* 机制下数据无关更新不增加 V。(II) 资本化账本：在特定稳定性与条件 (f) 下，资本化效率 η_cap ≤ 1，并给出了等号成立的充要条件。(III) 价值保留：给出两层对齐域；第一层在无记录侧信息独立假设下建立 V 与侧信息调整后记录拟合 I(M';D|Y) 之间的精确汇率，第二层在联合侧信息中性条件 (M,D)⫫Y 下建立原始记录库存汇率，其边界由显式一次性密码本实例标记。

Conclusion: 本文刻画了有限状态学习设备中"记录"与"价值"的严格分离，展示了资本化效率上界及等号条件，并在任务分布漂移下提供了价值保留的两层精确对齐域。作者明确强调这些都是关于有限设备价值保留的形式化陈述，而非统计泛化理论。
...
### [82] [Local and quasilocal conservation laws of three-state IRF cellular automata and their quantum deformations](https://arxiv.org/abs/2608.13080)
*Tomaž Prosen*

Main category: cond-mat.stat-mech

TL;DR: 本文利用patch-matrix-product方法，研究了Klobas和Prosen引入的两种可逆三态相互作用绕面元胞自动机（物种保持与物种翻转规则）及其量子相干变形。两类经典规则共享一参数族拟局域守恒律，其最大欧拉速度为√(3/23)，为物种翻转规则提供了严格的传播速度下界，与文献报告的外推值0.361一致。物种保持规则还存在交错生成族产生无限塔严格局域荷。量子变形中揭示了低键维不变态（疤痕候选）、指数长寿命拟局域拟模以及非零Mazur界。


<details>
  <summary>Details</summary>
Motivation: 旨在严格刻画可逆三态相互作用绕面元胞自动机的守恒律结构、最大欧拉速度（GHD框架中的传播速度）以及量子变形下的动力学性质，验证并支持Klobas-Prosen关于sound-active荷完整性的猜想。

Method: 采用patch-matrix-product（补丁矩阵积）方法，在平移不变精确ansatz下构造辅助维数为三的拟局域守恒律族；通过解析辅助维数为二的生成族提取中心对数导数；在量子变形中利用相同代数结构构造不变态、拟模和Mazur界。

Result: 两类经典规则共享一参数族拟局域守恒律，非零欧拉速度唯一为±√(3/23)，由此给出严格的最大欧拉速度下界。物种翻转规则的√(3/23)≈0.3612与外推值0.361一致。物种保持规则额外有交错生成族产生严格局域荷塔。量子变形中构造了疤痕候选态、指数长寿命拟模和非零Mazur界。

Conclusion: 结果强有力地支持了物种翻转规则中已知sound-active荷的完整性，同时为物种保持规则提供了新的严格局域守恒荷结构；量子变形的代数框架为构造精确动力学不变量和长寿命拟模提供了统一途径。
...
### [83] [Equivariant learning of a transferable three-dimensional classical density functional](https://arxiv.org/abs/2608.13506)
*Bingqing Cheng*

Main category: cond-mat.stat-mech

TL;DR: 本文展示了可以从三维平衡密度场直接学习经典密度泛函理论（cDFT）中的超额自由能泛函，无需自由能或化学势标签，同时保持空间对称性和变分一致性。单个学习到的泛函可跨温度、体系尺寸和统计系综迁移，能恢复结构因子、状态方程、液-汽共存和界面展宽等未作为训练目标的性质，并成功预测复杂三维几何中的溶剂桥力和孔内吸附行为。


<details>
  <summary>Details</summary>
Motivation: 液体的集体行为对热力学条件、界面和限域敏感，但预测每一种新状态通常需要单独进行原子模拟，计算成本高昂。经典密度泛函理论提供可复用的变分描述，但其核心的超额自由能泛函形式通常是未知的，而现有的学习近似大多局限于平面或低维设置，缺乏处理真实三维复杂几何的能力。

Method: 直接从全三维平衡密度场中学习超额自由能泛函，在训练过程中保持空间对称性和变分一致性，无需任何自由能或化学势标签。该方法利用平衡密度数据本身作为监督信息，通过变分原理约束来构造可迁移的热力学泛函。

Result: 单个学习到的泛函可跨温度、体系尺寸和统计系综迁移，能够恢复结构因子、状态方程、液-汽共存曲线和界面展宽等未作为训练目标的性质。在复杂三维几何中，成功预测了胶体间溶剂耗尽桥形成与破裂过程中的非单调力，以及互连螺旋二十四面体（gyroid）孔内的吸附行为。

Conclusion: 该工作证明平衡密度数据可被转化为一个可迁移的热力学生成器，将微观液体结构与响应、相行为及集体现象联系起来，为密度泛函理论的学习提供了三维、无标签条件下的可行路径，有望替代大量重复的原子模拟。
...
### [84] [Biorthogonal-only Floquet Dynamical Quantum Phase Transitions](https://arxiv.org/abs/2608.07882)
*Jiangrong Wen,Qidong Yuan,Zi-Xiang Hu,Jian-Jun Dong*

Main category: cond-mat.stat-mech

TL;DR: 本文在精确可解的周期驱动非厄米SSH链中，发现了一个有限的仅双正交Floquet DQPT区域——双正交Loschmidt速率出现非解析性而自正规Loschmidt速率保持光滑，从而确立了双正交与自正规DQPT之间的根本区别。


<details>
  <summary>Details</summary>
Motivation: 非厄米动力学量子相变(DQPT)对内积选择固有敏感，但此前研究表明双正交和自正规DQPT发生在相同参数区间，暗示两者并存。本文旨在证明这一认知是错误的，揭示两种形式的动力学临界性并非总是共存的。

Method: 采用精确可解的周期驱动非厄米Su-Schrieffer-Heeger(SSH)链作为模型，通过解析推导得到临界条件，分别计算双正交与自正规Loschmidt率，并分析其非解析性及其与有效Floquet哈密顿量奇异线(exceptional lines)之间的关系。

Result: 发现了一个有限的双正交Floquet DQPT区域，其中双正交Loschmidt率非解析而自正规Loschmidt率光滑。双正交Floquet DQPT的起始被锁定在有效Floquet哈密顿量的奇异线上，而自正规临界性没有对应的谱边界。此外，每个临界动量下，双正交DQPT在每个驱动周期内出现一对临界时间，而自正规DQPT仅出现一个。

Conclusion: 该研究确立了双正交与自正规DQPT之间的根本区别，打破了以往认为两者始终共存的认知，为非厄米系统中新的非平衡量子现象开辟了道路。
...
### [85] [Stochastic Nonlinear Waves in all the Right Phases](https://arxiv.org/abs/2608.12369)
*C. H. S. Hamster*

Main category: cond-mat.stat-mech

TL;DR: 本文综述了反应-扩散方程中行波相位的不同定义方式，并在一维随机设定下评估这些相位描述的适用性，其中首次显式计算了Nagumo方程的自守等时相位。


<details>
  <summary>Details</summary>
Motivation: 在反应-扩散方程中，行波的"相位"有不同的数学描述方式（如等时相位、等变相位等），但在随机环境（噪声扰动）下，这些不同相位定义的适用性和有效性尚未被系统地比较和评估。理解不同相位描述在随机设定下的表现，对于研究噪声对行波传播的影响具有重要意义。

Method: 本文采用综述与分析相结合的方法：（1）系统梳理文献中用于描述反应-扩散方程行波相位的各种定义和框架；（2）在一维随机设定下分析这些相位描述的有效性；（3）针对Nagumo方程，给出了自守等时相位（autonomous isochronal phase）的显式解析表达式，这是一个新的数学结果。

Result: 提供了一维随机设定下不同行波相位描述方法的系统比较，并给出了Nagumo方程的自守等时相位的显式计算，这是此前文献中尚未见到的结果。

Conclusion: 不同的相位定义在随机环境下具有不同的适用性，需要根据具体问题选择合适的相位描述；同时，Nagumo方程等时相位的显式公式为后续随机分析提供了新的工具。
...
### [86] [Diffusion and Solvation Dynamics of Ions in Water: Beyond the Brownian Approximation](https://arxiv.org/abs/2608.12562)
*Ian C. Bourg,Minh Thê Hoang Ngoc,Thomas R. Underwood,Carlos Vega*

Main category: cond-mat.stat-mech

TL;DR: 本文研究了碱金属（Li、Na、K、Rb、Cs）、碱土金属（Mg、Ca、Sr、Ba）及氯离子在水合壳层中的离子-水分子耦合动力学，通过分析其对离子同位素质量的依赖性展开研究，发现这些离子在水中的振动功率谱呈现两个或三个峰（即多个rattling频率），且不同频率对同位素质量的灵敏度不同，可作为离子溶剂化过程的特征标志，并可能与Hofmeister效应（包括不同金属对RNA的相对亲和力）相关。


<details>
  <summary>Details</summary>
Motivation: 离子与其第一水合壳层中水分子之间的耦合动力学影响多种关键过程，包括离子扩散、纳米孔中的选择性离子传输、离子配对、离子吸附以及金属-配体结合反应的动力学。研究这些耦合动力学有助于深入理解离子溶剂化机制及相关生物和化学过程。

Method: 通过分析离子振动功率谱对同位素质量（isotopic mass）的依赖性来研究离子-水耦合动力学。研究对象涵盖五种碱金属（Li、Na、K、Rb、Cs）、四种碱土金属（Mg、Ca、Sr、Ba）和氯离子。结果与已有的离子在水中的扩散系数同位素质量依赖性测量以及从头算（ab initio）计算的离子高频动力学进行了验证对比。

Result: 研究发现离子在水中的振动功率谱一致地呈现两个或三个峰，即离子在其溶剂化壳层内具有多个rattling频率（此前的子集物种已有报道）。这些频率对同位素质量的灵敏度各不相同，可能作为离子溶剂化过程（如离子定向第一壳层水分子的倾向）的特征标志。这些发现还可能涉及Hofmeister样效应，包括不同金属对核糖核酸（RNA）的相对亲和力。

Conclusion: 离子在溶剂化壳层内的多个rattling频率及其对同位素质量的不同敏感性，可以作为离子溶剂化过程的具有物理意义的特征标志，并可能与Hofmeister效应等"特异性离子效应"相关联，为理解离子在生物和化学体系中的行为提供新的视角。
...
### [87] [Beyond persuasion: Mobile electoral interfaces in polarized societies](https://arxiv.org/abs/2608.13184)
*Nuno Crokidakis,Celia Anteneodo*

Main category: cond-mat.stat-mech

TL;DR: 本文提出一个最小动力学模型，将选民划分为两个稳定的党派集团和一个移动选民界面，通过Fermi-like概率规则分配移动选民。分析表明选举对外部冲击的敏感性正比于移动界面的稳定规模，并预测从冻结极化到高响应性选举的连续移动性谱系，强调极化选举中的整体变化主要由移动界面波动主导，而非大规模意识形态转变。


<details>
  <summary>Details</summary>
Motivation: 传统的选举动力学模型强调意见变化、社会影响和说服机制，但这些模型难以解释当代极化社会中党派阵营相对稳定却同时存在选举相关移动人群的现象，因此需要一种不依赖直接说服、而是由移动选举界面动力学主导的新模型。

Method: 构建一个将选民划分为两个稳定党派集团和移动人群的极小动力学模型，压制极化阵营间的直接转换，通过基于拒绝不对称性的Fermi-like概率规则实现移动选民在竞争阵营间的分配，并进行解析计算推导选举敏感性与移动界面规模的关系。

Result: 解析计算表明政治冲击下的选举敏感性正比于移动选举界面的稳态规模；模型自然预测出从冻结极化到广泛移动界面的高响应性选举之间的连续移动性谱系。

Conclusion: 在强极化选举中，总体选举变化可能主要由移动选举界面上的波动所控制，而不是大规模意识形态转化所致；选举流动性是控制极化系统对外部扰动宏观响应的关键量。
...
### [88] [Capillary self-folding chains](https://arxiv.org/abs/2608.13349)
*Megan Delens,Axel Franckart,Martin Poty,Nicolas Vandewalle*

Main category: cond-mat.stat-mech

TL;DR: 本文构建了一种由浮动单体组成的柔性毛细链，单体弯曲分支使液体界面产生上凸或下凹变形，对应有效正或负毛细电荷，从而产生沿链的局部吸引或排斥作用。通过调节毛细序列可获得直线、锯齿和环等多折叠构型。短链折叠主要由近邻相互作用主导，结构明确；随着链长增加，非近邻段相互靠近并相互作用，折叠景观变得复杂，亚稳态数量随链长呈指数增长。数值映射折叠景观并通过机械搅拌实验展示亚稳态间的转变。毛细序列不仅编码目标几何构型，还控制折叠景观复杂度、简并性和突变鲁棒性，为研究局部相互作用规则如何产生集体折叠和复杂序列-结构关系提供了可控的中尺度平台。


<details>
  <summary>Details</summary>
Motivation: 探索中尺度自组装中如何通过局部相互作用规则实现可编程的折叠行为，并深入研究从简单序列编码到复杂折叠景观的转变，类比生物分子系统中序列-结构关系。

Method: 构建浮动单体的柔性毛细链，利用单体弯曲分支诱发液体界面上凸或下凹变形来编码有效正/负毛细电荷；通过调节毛细序列改变链内局部吸引/排斥作用；采用数值方法映射不同链长下的折叠能量景观，并通过机械搅拌进行实验验证。

Result: 成功获得直线、锯齿、环等不同折叠构型；短链折叠由近邻相互作用主导并形成明确定义的结构；长链折叠景观复杂度增加，亚稳态数量随链长指数增长；实验证明机械搅拌能使链在亚稳态之间转变。毛细序列控制目标几何编码、折叠景观复杂度、构型简并性及突变鲁棒性。

Conclusion: 毛细链可作为可控的中尺度平台，用于研究局部相互作用规则如何驱动集体折叠行为，揭示与生物分子系统类似的序列-结构复杂关系，为可编程微系统设计提供新途径。
...
### [89] [Aperiodicity is sufficient for macroscopic thermalization](https://arxiv.org/abs/2608.13462)
*Amit Vikram*

Main category: cond-mat.stat-mech

TL;DR: 本文提出了一种宏观可观测量的有限时间热化的一般机制，基于初始态量子动力学的两个基本要素：(1) 非周期性，提供可计算的热化度量；(2) 希尔伯特空间的动力学部分遍历探索。该机制仅凭初始态系综的返回概率在有限时间内较小这一可观测无关信息，即可预测几乎所有集中的宏观可观测量在几乎所有初始态和几乎所有有限时间内的平衡，并将宏观热化纳入可操作量子统计力学的范畴。


<details>
  <summary>Details</summary>
Motivation: 传统的量子热化研究（如本征态热化假设ETH）主要关注无限时间极限下的平衡描述，难以对有限时间内的宏观热化行为提供可计算的、可操作的判据。作者希望将宏观热化纳入"可操作量子统计力学"的框架，该框架此前已被发展用于微观热化的有限可计算判据，从而弥合宏观热化理论与有限观测时间实验之间的鸿沟。

Method: 作者通过识别两个核心要素建立热化机制：(1) 非周期性（aperiodicity），作为初始态量子动力学的一种可计算度量；(2) 动力学部分遍历探索（dynamical partially ergodic exploration of Hilbert space），刻画系统对希尔伯特空间的探索程度。核心条件是该机制仅需观测无关的信息——初始态系综在有限时间范围内的返回概率足够小，即可推导出所有集中的宏观可观测量在几乎所有初始态和几乎所有有限时间内达到平衡的结论。作为特例，该方法还能恢复无限长时间平衡的标准结果（基于有效维度等更强的条件）。

Result: 该机制预测：给定初始态系综的返回概率在有限时间范围内较小，则几乎所有（集中的）宏观可观测量在几乎所有初始态和几乎所有有限及更长的时间区间内都能实现平衡。这作为特例包含了无限长时间热化的标准结果（由有效维度的更强版本导出），并且相较于传统本征态描述，在热力学极限下的有限观测时间内提供了更强且更可获得的预测。

Conclusion: 本文建立了三个层次的联系：(1) 可（理论或实验）计算的探针（返回概率）的衰减反映了系统的"无记忆性"；(2) 其背后是希尔伯特空间中可观测量或态对齐（alignment）这一基本不变机制；(3) 这预测了经典与量子热化的不同自然形式，其中大多数严格恢复了传统本征态描述的无限时间热化作为特例，但在热力学极限下提供了更强的有限观测时间可访问预测。整体上将宏观热化纳入了可操作量子统计力学的理论框架。
...
### [90] [Hidden Dyson Universality in Inverse-Spectral Geometry](https://arxiv.org/abs/2608.13475)
*Momo Hayashi,Kazumitsu Sakai*

Main category: cond-mat.stat-mech

TL;DR: 本文通过Dyson变换将谱数据重构为算子的几何结构，发现Dyson普适性在重构后仍存留于矩阵几何中。通过距离壳矩的诊断，区分GOE/GUE/GSE，并将Riemann zeta函数的非平凡零点定位于GUE扇区。


<details>
  <summary>Details</summary>
Motivation: Dyson普适性通常体现在局部本征值统计中，作者希望探索其能否在非线性逆谱重构后存活，并在重构算子的矩阵几何中以新的形式重新显现，从而为谱分类提供独立于直接输入统计的诊断工具。

Method: 利用dressing变换将每个展开谱映射为固定谐振子的变形f(x)，在共同基下表示为矩阵元F_mn。按固定距离d=|m-n|（对应参考谐振子的能量转移通道）将矩阵元权重分解为壳层，并定义距离壳矩来描述分布。先在Gaussian β系综（GOE/GUE/GSE）上独立校准这些矩随β的变化，再将该诊断应用于Riemann zeta函数的非平凡零点。

Result: 距离壳矩随β平滑变化，能够区分GOE、GUE和GSE三个系综。将该校准好的诊断应用于Riemann zeta函数零点时，重构算子被定位在GUE扇区，表明零点具有GUE特性。

Conclusion: Riemann zeta函数零点的GUE特性不是通过输入能级的直接统计获得，而是从重构算子的距离分辨几何中恢复的。这证明了Dyson普适性的特征在逆谱重构后的矩阵几何中依然存留，提供了一种新的谱系综分类途径。
...
### [91] [Symmetry Emergence in Self-Organized Criticality](https://arxiv.org/abs/2608.13500)
*Ernesto Lupercio,Mikhail Shkolnikov*

Main category: cond-mat.stat-mech

TL;DR: 本文研究自组织临界性原型模型（砂堆模型）在最大密度状态下的仿射对称涌现机制。当底层网格间距平方的倒数远大于随机扰动点数量时，坍缩函数（odometer）的标度极限是满足Dirichlet边界条件的Monge-Ampère方程的唯一凹解，其势由给定概率测度决定，从而可在任意宏观窗口准确估计密度偏差。


<details>
  <summary>Details</summary>
Motivation: 理解自组织临界性模型中经典砂堆模型在最大密度区域的对称性涌现机制，并建立坍缩函数标度极限与最优传输和微分几何中经典非线性偏微分方程（Monge-Ampère方程）之间的联系，以便在宏观尺度上精确刻画密度偏差。

Method: 本文采用分析与组合论证相结合的方法：一方面利用归纳论证（近期已被升级为严格证明）建立关键经验事实；另一方面证明在最大密度状态下，当晶格网格的平方倒数远大于扰动点数时，坍缩函数的标度极限唯一收敛到带Dirichlet边界的Monge-Ampère方程的解，其中势函数即所给概率测度。

Result: 证明了仿射对称涌现的机制：在该参数区域中，坍缩函数的标度极限是Monge-Ampère方程在凸域上满足Dirichlet边界条件的唯一凹解，从而能够精确估计任意宏观窗口内密度偏离最大值的误差界。

Conclusion: 该工作将自组织临界性模型与最优输运理论及微分几何联系起来，为最大密度区域中仿射对称性的涌现提供了严格的数学基础，并为进一步研究砂堆模型中相关临界现象提供了可计算的分析框架。
...
TL;DR: 本文提出了一种将预训练黑盒预测函数的知识迁移到异构输入空间的方法。该方法将目标回归函数分解为可迁移部分（由黑盒提供信息）和不可迁移部分（仅依赖新空间信息），并通过两步神经网络过程估计这两部分，理论分析与实验均表明该方法优于非迁移基线。


<details>
  <summary>Details</summary>
Motivation: 预训练黑盒预测函数蕴含大量数据和计算带来的知识，但当可用的输入特征与黑盒期望的特征不一致时，无法直接使用这些黑盒。因此需要一种方法，能够将黑盒中的预测知识迁移到异构输入空间中，即使输入空间不同也能利用黑盒的预测能力。

Method: 作者将目标回归函数分解为可迁移组件（由黑盒作为信息源）和不可迁移组件（捕捉新空间独有的信息）。提出两步神经网络方法：第一步利用大量无标签的特征对（跨越两个输入空间的桥梁）估计可迁移组件；第二步利用有限标签数据估计不可迁移组件。框架可扩展至多个黑盒（各自在不同输入空间），通过聚合降低预测误差。

Result: 推得了预测风险界，当不可迁移组件较小或平滑时，优于非迁移备选方案，且方法能自适应于两种情况。在额外条件下，估计量的最坏情况风险在多项式阶上严格小于仅利用标签数据的最小极大风险。模拟与真实数据实验验证了方法的实用性。

Conclusion: 该工作为跨异构输入空间的黑盒知识迁移提供了一种理论基础扎实且实践有效的框架，并证明了多黑盒聚合可进一步降低预测误差。
...
TL;DR: 本文提出 SSPO（结构感知相似度加权偏好优化）方法，通过不相似度加权的留一基线联合打分所有 B 个采样解，同时解决神经组合优化训练中梯度信号极化和基线冗余两个问题。在 TSP、EFL、JSP 基准上一致优于现有最佳锚点与均匀权重基线，且已部署于京东生产级设施选址系统。


<details>
  <summary>Details</summary>
Motivation: NCO 训练依赖并行解采样，但现有方法未能充分利用共采样解组中的丰富信息。偏好优化锚定单一最优解并丢弃其他解的细粒度质量与结构信号（梯度信号极化）；均值基线又对同伴均匀加权，导致结构近似的解涌入基线产生冗余并使梯度方差偏高（基线冗余）。

Method: 提出 SSPO，对所有 B 个采样解联合打分，采用不相似度加权留一基线：结构上不同的解获得更高权重，通过单一机制同时解决上述两个失败。基线利用从编码器现有节点表征构建的零参数、问题自适应的解嵌入。

Result: 在 TSP、EFL、JSP 基准上一致优于既有最佳锚点与均匀权重基线；在 TSP 与 EFL 上直接对比均匀 RLOO，证实结构感知加权是改进的主要驱动力。SSPO 训练的 EFL 策略已部署于京东物流生产级设施选址系统，证实了规模化实际可行性。

Conclusion: 结构感知的相似度加权基线能够有效利用共采样解组的结构信息，克服梯度极化和基线冗余两大缺陷，显著提升 NCO 训练性能，并具备实际部署价值。
...
TL;DR: 本文研究了分布强化学习中分位数时序差分学习（QTD）的统计推断问题。在生成模型假设下，建立了同步与异步QTD的函数中心极限定理，并提出了基于随机缩放的在线推断方法，在无需存储完整轨迹的情况下构造渐近枢轴统计量，大幅降低内存需求。


<details>
  <summary>Details</summary>
Motivation: 分布强化学习中QTD算法的统计推断问题尚未得到充分研究，现有的推断方法通常需要存储完整的迭代轨迹，内存开销大，限制了其在实际应用中的效率。

Method: 首先在生成模型假设下，为同步和异步QTD建立函数中心极限定理（FCLT），证明QTD的平均迭代弱收敛于重缩放布朗运动；随后基于随机缩放（random scaling）方法构造渐近枢轴统计量进行在线推断，该统计量利用整个QTD路径的信息，但可在线计算而无需存储完整轨迹。

Result: 建立了同步与异步QTD的函数中心极限定理，证明其平均迭代弱收敛于重缩放布朗运动；提出的随机缩放推断方法构造的统计量具有渐近枢轴性质，且实现了在线计算，显著降低了内存需求。

Conclusion: 本文为QTD算法提供了理论基础上的统计推断框架，通过随机缩放方法实现了高效、低内存的在线推断，为分布强化学习中的不确定性量化提供了实用工具。
...
TL;DR: 本文研究分布偏移下DRO（分布鲁棒优化）和RS（鲁棒满足）两种鲁棒学习方法的有限样本泛化误差，推导目标环境下的泛化误差界，提出信息导向的超参数校准方法，并在网络批量问题中验证两者在部分信息下的互补行为。


<details>
  <summary>Details</summary>
Motivation: 现有鲁棒学习分析（如DRO和RS）的有限样本保证通常局限于源环境或对抗性最坏情况性能，缺乏对目标偏移环境下泛化误差的系统研究，且两种方法在偏移场景下的对比尚属空白。

Method: 首先推导DRO和RS在偏移目标环境下的有限样本泛化误差界，显式刻画鲁棒性超参数带来的偏移敏感性降低与正则化惩罚之间的权衡，并避免Wasserstein经验集中带来的维度灾难；其次在部分偏移信息（如偏移幅度或方向）可用时，提出信息导向的超参数校准方法并对比两方法；最后在网络批量问题中进行应用验证。

Result: 得到了DRO和RS在目标偏移环境下的有限样本误差界，明确了鲁棒超参数的权衡结构；在部分信息机制下，两者展现出互补的理论与经验行为；网络批量问题中对需求和分布正偏移下鲁棒策略的响应提供了解释。

Conclusion: 这些结果弥补了鲁棒学习方法在分布偏移下统计性质理解的空白，并为DRO与RS的系统比较提供了原则性依据。
...
TL;DR: 在高维网络分析中，当参数数量远超观测值时，最小描述长度（MDL）方法能够提供保守（低假阳性率）的邻域估计，而Lasso、AIC、BIC等传统方法则会产生虚假边（高假阳性率）。


<details>
  <summary>Details</summary>
Motivation: 随着越来越多变量被纳入网络分析，高维场景下（参数远多于观测）如何避免遗漏重要变量及其连接，同时保持低假阳性率成为关键挑战。

Method: 从线性模型估计邻域出发，对比线性与非线性两种真实模型场景，讨论岭参数对均方误差的影响，并结合机器学习中的双下降现象，提出在高维设置下需将模型空间体积纳入惩罚项，采用最小描述长度（MDL）进行邻域选择。

Result: 证明在高维场景下，无论真实模型是线性还是非线性（线性假设），MDL方法都能实现正确的邻域选择或产生更小的假阳性率，而传统方法（Lasso、AIC、BIC）会导出大量虚假边。

Conclusion: 高维网络邻域选择应将模型空间体积纳入惩罚，MDL是兼顾正确性与保守性的有效方法，其洞察与机器学习的双下降现象相呼应。
...
TL;DR: 本文建立了特征参数化代价下逆最优运输(IOT)的统计与算法理论，核心贡献是Sinkhorn线性化（隐函数灵敏度分析）及其谱代理公式，并基于受限Hessian的谱夹心界推导出四个定理和一个观测（可辨识性、稀疏恢复、良置性、收敛性和误设定分析）。


<details>
  <summary>Details</summary>
Motivation: 逆最优运输问题是给定观测到的运输计划反推代价函数，在经济学、心理学和机器学习中广泛应用。然而，特征参数化代价下的IOT缺乏系统的统计与算法理论，特别是如何从熵正则化OT计划的灵敏度出发建立可辨识性、估计一致性和收敛性保证，是亟待解决的开放问题。

Method: 作者提出Sinkhorn线性化——对熵OT计划关于代价参数的隐函数灵敏度分析，并构造一个谱精确但几何透明的谱代理公式。在切空间上推导受限Hessian满足谱夹心不等式(pi_min/epsilon)I <= H_T^{-1} <= (pi_max/epsilon)I，由此得到核心界sigma_min >= (pi_min/(a_max epsilon)) * sqrt(lambda_min(Sigma))，并在此基础上一一证明四个定理和一个观测。

Result: T1：theta在gauge核的商空间上全局单射，维度界为F <= (K-1)^2；T2：l1惩罚估计器在不可表示性和得分集中条件下以指数失败概率恢复正确支撑；T3：特征矩映射M(theta)强单调，逆映射Lipschitz常数为L <= epsilon ||Phi^T S_a||_op / (pi_min lambda_min(Sigma))；T4：局部强凸性保证梯度下降单调收敛，mu >= pi_min^2 lambda_min(Sigma)/epsilon^2；O5：估计器收敛到OT模型对真实值的投影，投影映射的Holder连续性经验指数alpha_eff在(0,1)内。

Conclusion: 该工作为特征参数化的逆最优运输问题提供了统一的理论框架，通过Sinkhorn线性化和谱夹心界将可辨识性、稀疏性、良置性、收敛性与误设定分析整合为一个完整的统计与算法理论体系，为后续IOT方法在实际数据中的应用奠定了坚实基础。
...
TL;DR: 本文提出Wasserstein Filtering (WF)框架，通过最大化子集经验分布与总体污染经验分布之间的Wasserstein距离来筛选并剔除异常样本，从而在部分样本被污染的数据集中恢复真实的干净分布。该方法提供了三个可计算算法（SinkMarg、SinkWF、SlicedWF），并提出FELP污染模型，在该模型下证明了WF估计器在有界协方差分布族上达到极小极大最优性。实验表明WF在异常检测和生成建模（如扩散模型）中表现优异。


<details>
  <summary>Details</summary>
Motivation: 现实数据集常含有被污染的样本，如何从这类数据中可靠地恢复潜在的干净分布是一个核心问题。传统方法面对几何上具有影响力的异常值时可能失效，且缺乏在统一理论框架下保证估计质量的样本筛选方法。

Method: 提出Wasserstein Filtering (WF)框架：选择一组样本子集，使其经验分布与完全污染的经验分布之间的Wasserstein距离最大化，从而优先隔离并移除几何上突出的异常值。为计算可行性，开发三种算法：(1) SinkMarg——基于边际筛选的方案；(2) SinkWF——利用熵正则化最优传输的联合优化算法；(3) SlicedWF——基于切片Wasserstein近似的联合优化算法。理论上提出FELP (Far Exclusion and Local Projection)污染模型，刻画由良好分离的异常值和局部不可区分的扰动构成的污染形式，并在该模型下证明WF估计器的极小极大最优性。

Result: 在合成数据集、基准异常检测套件以及带扩散模型的鲁棒生成学习任务上的大量数值实验表明，WF作为模型无关的预处理工具具有高度实用性，在异常检测中取得了有竞争力的性能，并在重度污染条件下为生成建模带来了显著的下游收益。

Conclusion: WF是一种实用的、模型无关的数据预处理工具，能够有效从受污染数据中恢复干净分布，兼具理论最优性保证和实际算法可计算性，在异常检测和鲁棒生成建模中展现出广泛的应用前景。
...
TL;DR: 本文重新研究了测试时对抗鲁棒学习问题，证明VC类可以被对抗鲁棒学习，采样复杂度与VC维d呈线性关系，相比Montasser等人(2019)的先前上界实现了指数级改进。该结果通过结合Breiman(1996)的bagging启发式方法与鲁棒经验风险最小化(RERM)的简单不当学习算法实现。算法在O(d*)个独立bootstrap样本上计算RERM并多数投票输出。论文还给出了下界，证明任何学习器在此oracle模型下至少需要Ω(d*)次RERM oracle调用。


<details>
  <summary>Details</summary>
Motivation: 先前Montasser等人(2019)对对抗鲁棒可学习性的上界不够紧，本文旨在改进对抗鲁棒学习的样本复杂度界限，将其从指数级降低到与VC维线性相关。

Method: 提出一个简单的"不当学习"(improper learning)算法，将经典bagging（bootstrap聚合）与RERM结合：在O(d*)个独立bootstrap采样上分别计算RERM，然后取其多数投票结果作为最终预测器，其中d*为对偶VC维。

Result: 证明了VC类的对抗鲁棒可学习性样本复杂度为O(d)（线性于VC维），相对先前上界实现指数级改进；同时给出匹配的下界Ω(d*)，证明在该oracle模型中RERM调用的次数不可避免。

Conclusion: 该工作不仅大幅改进了对抗鲁棒学习的样本复杂度理论上界，还表明简单的bagging+RERM组合就能达到近似最优性能，同时下界证明该方法的oracle调用次数在一般情形下是紧的。
...
### [100] [Unifying Generative Models with Path Integrals](https://arxiv.org/abs/2608.12438)
*Ramon Winterhalder*

Main category: stat.ML

TL;DR: 本文提出了一个统一的生成建模路径积分框架（主作用量），将流模型、扩散模型、变分模型和对抗模型统一为同一主作用量的不同评估原则。通过MSRJD形式将自由概率流与相互作用概率流分离，并引入费曼图微扰论：单圈修正可在无随机采样成本下改善确定性采样器（将53%的树级误差降至1.6%）；不完美学习得分作为插入项产生响应加权的得分匹配目标；对称等变漂移设计被重构为具有EFT功率计数的算子展开。


<details>
  <summary>Details</summary>
Motivation: 当前生成模型领域存在多种看似独立的方法论（流模型、扩散模型、变分和对抗模型），缺乏统一的数学框架来理解其内在联系与差异。作者旨在通过路径积分这一理论工具构建涵盖所有主要生成建模范式的统一形式体系，并挖掘该框架带来的实际计算收益（如无需额外采样成本的微扰修正）。

Method: 将生成建模表述为路径积分，采用MSRJD形式体系区分自由与相互作用概率流，使系统可应用费曼图微扰论。具体方法包括：(1) 推导单圈修正公式用于改进确定性采样器；(2) 将不完美学习得分作为外线插入处理，导出响应加权得分匹配目标函数；(3) 利用有效场论（EFT）功率计数对对称等变漂移设计进行算子展开分析。

Result: 在可解析和非线性漂移系统上验证了单圈修正的有效性，将树级误差从53%显著降低至1.6%，且不产生额外随机采样成本。此外，推导出的响应加权得分匹配目标以及对称等变漂移的EFT算子展开为设计更优的生成模型提供了新的理论指导。

Conclusion: 该工作建立了生成建模与统计场论之间的深层联系，证明微扰论技术在生成模型中具有实用价值。统一框架不仅提供了理论上的整合视角，还带来了实质性的数值改进，为后续在生成建模中引入更丰富的场论工具（如重整化、对称性分析）开辟了道路。
...
### [101] [Don't Cut Corners: How Training Outside the Prior Makes Simulation-Based Inference More Robust](https://arxiv.org/abs/2608.12470)
*Chaipat Tirapongprasert,Matthew Ho*

Main category: stat.ML

TL;DR: 本文提出Tailed-Uniform，一种混合提议分布族，用于在仿真驱动（simulation-based inference）训练中解决均匀先验盒边界处神经后验估计器学习精度差的问题。通过在硬截断训练盒外添加衰减尾，提升网络在边界附近及之外的推理精度，并在玩具问题与物质功率谱宇宙学参数推断上验证了效果，且维度越高优势越明显。


<details>
  <summary>Details</summary>
Motivation: 大型天体物理仿真通常从均匀先验盒子中采样参数生成训练数据，但由于提议分布的尖锐边缘，神经后验估计器在边界附近难以学习到准确的近似，导致边界处的推断精度下降。

Method: 提出Tailed-Uniform分布族：在原始硬截断均匀训练盒的基础上，向边界外填充衰减尾（decaying tails），形成混合提议分布用于采样训练仿真数据。以加宽的均匀盒子作为对照，研究不同尾形状的影响。

Result: Tailed-Uniform训练的神经网络在边界附近及边界之外产生更准确的后验分布。在玩具问题和基于物质功率谱的宇宙学参数推断上均验证了改进，且这些优势在高维情形下更显著，因为高维空间中边界占据更大比例的体积。

Conclusion: 在训练盒边界附近增加额外的仿真数据能更好地约束网络接近边界时的行为，即便最终推断使用的是均匀先验，Tailed-Uniform提议策略仍带来显著的精度提升，为仿真推理中的训练数据采样提供了更优方案。
...
TL;DR: 本文对离线策略评估中的确定性top-k策略进行了系统基准测试。研究了六种估计器在五个数据集上的表现，重点考察了三大威胁：弱重叠（由logger-target动作对齐而非logging锐度决定）、优化者诅咒（交叉拟合无法消除复用偏差，需要诚实的策略级拆分）、倾向性估计误差（对IPS伤害最大，可逆转重叠诊断）。


<details>
  <summary>Details</summary>
Motivation: 组织需要在预算约束下决定如何分配资源，并希望在实际部署之前知道目标分配规则能带来多少收益。离线策略评估（OPE）可以从日志数据中估计这一点，但可部署的规则是确定性top-k策略——它消除了所有动作平均，使得弱重叠问题直接冲击估计值。现有工作缺乏对这一特定场景下OPE方法的系统性评估。

Method: 基准测试六种估计器，跨五个数据集和两组已知效应扫描，并使用非模拟的配对参考验证机制。倾向性分数下限设为0.02以确保权重有界。通过logger-target动作对齐程度来构造不同的重叠条件，系统施加交叉拟合、倾向性估计等压力测试。

Result: 三大发现：(1) 弱重叠由logger-target动作对齐主导而非logging锐度；有效样本量能跨环境排序风险，但无法在单一日志内排序候选估计器，其截点不可迁移。(2) 交叉拟合倾向性因子无法修复优化者诅咒，甚至会加重复用偏差；需要诚实的策略级拆分，这改变了估计目标而非对全样本策略去偏。(3) 倾向性估计误差是最大的退化来源：out-of-fold估计对IPS伤害最大，几乎不影响双重稳健估计，甚至可逆转重叠诊断。倾向性下限将两个调参混合方法退化为未调参的父方法。

Conclusion: 研究揭示了确定性top-k策略OPE的三个关键失败机制，为实践者提供了重叠风险诊断的建议（应关注动作对齐而非logging锐度），并强调了策略级诚实拆分和倾向性估计质量在离线评估中的核心重要性。基准已开源，仅使用公共数据。
...
### [103] [Supervised Mixed-Frequency Learning for Macro-Financial Forecasting When Factors are Weak](https://arxiv.org/abs/2608.12589)
*Ulrich Hounyo,Zhendong Li*

Main category: stat.ML

TL;DR: 本文提出SsPCA-MIDAS方法，将监督式缩放主成分分析（SsPCA）集成到混合数据采样（MIDAS）回归框架中，用于从高维高频预测变量中提取弱因子以预测低频宏观金融目标，解决了传统PCA-MIDAS在弱因子假设下失效的问题。


<details>
  <summary>Details</summary>
Motivation: 传统Factor-MIDAS回归依赖PCA提取共同因子来缓解维度灾难，但PCA假设因子具有普及性（pervasiveness），这一假设在宏观金融预测中常见的弱因子情形下经常被违背，导致预测失效。

Method: 将监督式缩放PCA（SsPCA）嵌入MIDAS框架中，在PCA前结合目标变量信息对预测变量进行缩放，从而在弱因子条件下仍能有效提取因子；并建立了弱因子下因子估计的相合性与渐近正态性理论。

Result: 模拟实验表明SsPCA-MIDAS在弱因子普遍存在时优于其他基于PCA和监督式的对比方法；将机器学习技术（如boosting）应用于提取的更干净因子可进一步改善预测；美国宏观金融实证表明该方法能选择有经济意义的预测变量并改善GDP、通胀、失业率、资产价格和波动率的预测。

Conclusion: SsPCA-MIDAS为弱因子场景下的混合频率预测提供了理论保证和实证优势，是一种稳健且有效的宏观金融预测方法。
...
### [104] [When Explanations Betray Backdoors: Black-Box Auditing for Language Model Classifiers](https://arxiv.org/abs/2608.12623)
*Yang Liu,Ran Zou*

Main category: stat.ML

TL;DR: 本文研究语言模型分类器在附带解释（rationale/引证证据）场景下的黑盒审计问题。提出Groundedness Drift指标，衡量答案摘要是否仍锚定于输入文本，在7B规模的两个骨干模型、五个数据集和四种非自适应OpenBackdoor攻击下，以5%干净误报率为预算时，AUROC和残余目标ASR均优于所有对比检测器。进一步提出Unsupported Groundedness多探针升级方案应对解释伪装压力测试，虽能改善信号但无法完全消除自适应攻击差距。


<details>
  <summary>Details</summary>
Motivation: 现有的带解释的分类器被广泛用于内容审核、路由、主题分类和低资源标注等场景，但攻击者可利用后门注入并生成伪装性解释来逃避检测。防御方仅有干净校准数据、缺少触发信息，因此需要一种仅利用分类器输出（标签+简短解释/引证）即可高效识别被投毒模型的黑盒审计方法。

Method: 提出Groundedness Drift分数，一种轻量级度量，检测模型输出摘要是否仍与输入保持"锚定性"；并设计Unsupported Groundedness作为多探针升级方案，针对解释伪装（explanation-camouflage）的高压场景，通过多重探测信号增强检测能力。

Result: 在两个7B骨干模型、五个数据集、四种常见非自适应OpenBackdoor攻击家族上，Groundedness Drift在5%干净FPR预算下，所有实验场景的AUROC和残余目标ASR均优于所有对比检测器。Unsupported Groundedness进一步改善检测信号，但未能完全弥合自适应攻击下的性能差距。

Conclusion: Groundedness Drift是有效的轻量级黑盒后门审计指标，尤其适用于带解释的LM分类器场景；但在面对自适应（解释伪装式）攻击时仍存在检测鲁棒性的瓶颈，值得未来在更强对抗条件下进一步探索。
...
TL;DR: 本文提出了一种条件因果发现方法，将因果发现问题（如因果效应约束）视为贝叶斯推理问题，并采用罕见事件估计技术（粒子群体逐步逼近约束区域）来高效估计条件后验分布，在合成图和Sachs蛋白质数据上验证了方法的准确性。


<details>
  <summary>Details</summary>
Motivation: 传统因果发现的目标不仅是预测因果边，还要能解释和说明观测或假设的现象（如大的因果效应）。现有贝叶斯因果发现方法在事件后验质量很小（罕见事件）时计算困难，因此需要一个有效的方法来处理小后验质量条件下的因果推理问题。

Method: 将条件因果发现建模为贝叶斯推理问题，目标是获得给定因果效应约束等事件条件下的因果图和参数的后验分布。采用罕见事件估计技术（粒子滤波/粒子群体方法），逐步驱动粒子群体朝向约束区域移动，同时保持样本能够近似条件后验分布。

Result: 在合成图上从小规模到大规模均验证了方法的准确性；在Sachs蛋白质数据集上的案例研究展示了该方法可通过通路级摘要辅助科学探索。

Conclusion: 该方法能够有效处理小后验质量事件下的条件因果发现问题，为科学探索提供了可解释的通路级因果摘要工具。
...
TL;DR: 本文提出"Training Under Challenge"（挑战训练）框架，一种可执行证书机制，通过预先声明、架构有效的程序构造同类别中的完整替代方案并重新评估同一目标函数，以量化神经网络检查点的全局最优性差距。该框架定义了资源索引的挑战功率模量，证明了无覆盖保证时一阶ReLU训练器可无限趋近精确条件头部最优而非全局点。在通道门控ResNet-18蒸馏问题中，八个内部挑战覆盖全部240个审计输出方向，实现残差界在真实差距的1.74–3.02倍范围内。


<details>
  <summary>Details</summary>
Motivation: 平坦的训练曲线无法区分神经网络是达到全局最优、局部陷入、表征受限还是与训练器不匹配，现有方法缺乏可执行的证书机制来量化全局最优性差距，因此需要一种能构造替代方案并重新评估目标函数的框架。

Method: 提出Training Under Challenge框架：(1) 预先声明架构有效的程序构造同类别完整替代方案；(2) 任何更低值的候选成为可重放见证者，下界检查点的经验全局最优性差距；(3) 定义资源索引的挑战功率模量；(4) 对平方损失利用块下降算子使覆盖可检查，得到一致性和实现残差界；(5) 通过配对预测证书分离解码器未充分利用与表征不足，并在量化去噪研究中展示诊断、修复与重认证。

Result: (1) 证明确认无覆盖时，一阶ReLU训练器可达无限多个精确条件头部最优却收敛到非全局点；(2) 在通道门控ResNet-18蒸馏问题中8个内部挑战覆盖全部240个审计输出方向；(3) 实现残差界在真实差距的1.74–3.02倍范围内；(4) 配对预测证书有效区分解码器未充分利用与表征不足；(5) 量化去噪研究验证了诊断、修复和重认证流程。

Conclusion: 挑战训练是一个可执行证书框架，能给出可重放的全局最优性差距下界。通过有限测试套件只能获得套件相对结论，全局差距结论需要单独论证的覆盖机制。该框架提供了覆盖可检查性、理论边界（含无覆盖时的不可能性结果）以及实际蒸馏和去噪任务的完整验证，为神经网络训练终点评估提供了可审计的量化手段。
...
TL;DR: 本文通过纳入协变量和中介变量中编码的因果信息，推导出多值（multivalued）设置下因果关系概率（PoCs）的更紧边界，相较现有非二元边界具有更紧的性质。


<details>
  <summary>Details</summary>
Motivation: Tian和Pearl推导了二元PoCs的尖锐边界，Mueller等人通过纳入协变量和中介变量的因果信息进一步收紧了二元PNS边界，Li和Pearl以及Shu等人将PoCs扩展到多值设置并推导了理论边界。这些进展自然引出一个问题：在多值设置下，能否通过额外的因果知识进一步收紧边界？本文正是针对这一问题展开研究。

Method: 本文在多值设置下，将协变量和中介变量中蕴含的因果信息纳入到PoCs的边界推导过程中，从而导出比现有非二元边界更紧的理论界限。通过玩具示例说明理论结果，并通过仿真研究验证边界收紧的效果。

Result: 理论推导表明所提出的多值PoCs边界比现有的非二元边界更紧，玩具示例展示了具体效果，仿真研究进一步验证了边界收紧的结论。

Conclusion: 在协变量和中介变量的辅助下，多值设置下的PoCs边界可以得到进一步收紧，从而更精确地刻画个体的因果响应。
...
### [108] [Evaluating AlphaEarth Foundations Embeddings for Wildfire Susceptibility Mapping](https://arxiv.org/abs/2608.12663)
*Yuan Zhuang,Sanaa Hobeichi,Peng Shi,Fei Huang*

Main category: stat.ML

TL;DR: 本研究以澳大利亚维多利亚州（2017-2025）为案例，系统评估了AlphaEarth Foundations（AEF）地理空间嵌入在野火易发性制图中的应用价值。研究发现AEF嵌入能高精度重建传统野火易发性分析所需的物理变量，基于嵌入的易发性模型ROC-AUC超过0.92，并在澳大利亚东部识别出高野火易发区域。AEF嵌入在气候相似区域表现出强迁移能力，跨区域迁移时性能远优于传统物理变量模型（维州模型迁移至堪培拉时AUC提升约4%，至悉尼蓝山仅下降约2%，而物理变量模型平均下降约25%）。


<details>
  <summary>Details</summary>
Motivation: 传统野火易发性制图依赖多种遥感、气候和地理空间产品的人工整合与特征工程，过程繁琐且成本高昂。虽然AEF提供的分析就绪地理空间嵌入可能减少这种依赖，但其在野火易发性制图中的价值尚未被系统评估，因此急需验证其有效性并探索其在可扩展工作流中的潜力。

Method: 以澳大利亚维多利亚州（2017-2025）为案例，首先验证AEF嵌入能否高精度重建野火易发性分析中常用的物理变量；然后基于卫星衍生的火灾发生数据训练下游易发性模型，评估嵌入模型的ROC-AUC性能；最后通过将维州训练的嵌入模型迁移到堪培拉和西悉尼-蓝山地区，对比物理变量模型的迁移性能，检验嵌入在气候相似区域的迁移能力。

Result: AEF嵌入可高精度重建常用野火易发性变量；嵌入模型ROC-AUC超过0.92，一致识别出维多利亚东部（特别是吉普斯兰和东北高地）高野火易发区域，并在维州中部和西北部发现局部热点。迁移测试中，嵌入模型在堪培拉AUC提升约4%、西悉尼-蓝山下降约2%，而物理变量模型平均下降约25%，显示嵌入模型的显著迁移优势。

Conclusion: AEF嵌入能够有效支持野火易发性制图，减少对传统多源数据整合和特征工程的依赖，并在气候相似区域内具备出色的迁移能力。该发现为政府机构和（再）保险公司等下游用户提供了可扩展的野火易发性制图工作流的实用指导。
...
### [109] [A Local-Linearly Convergent Algorithm for Nonconvex Equality-Constrained Optimization](https://arxiv.org/abs/2608.12665)
*Frank E. Curtis,Lingjun Guo,Daniel P. Robinson*

Main category: stat.ML

TL;DR: 本文扩展了Gradient-Eigenstep算法的分析，证明其在强二阶驻点附近以足够小的步长和足够大的罚参数启动时可获得局部线性收敛速率；并将其作为渐进采样策略的子问题求解器，用于求解由大样本均值定义的目标和约束函数，相比直接求解全样本问题具有更优的最坏情况样本复杂度。


<details>
  <summary>Details</summary>
Motivation: 非凸等式约束优化问题普遍存在于机器学习和大规模数据分析中。Goyens等人提出的Gradient-Eigenstep算法虽然迭代高效，但其收敛速率分析和在大规模问题中的应用潜力尚未被充分探索，特别是如何将其与采样策略结合以降低样本复杂度仍是一个开放问题。

Method: 作者从两方面展开分析：(1) 在强二阶驻点附近，通过选择合适的步长参数和罚参数，证明Gradient-Eigenstep算法退化为对Fletcher增广拉格朗日函数执行梯度下降，从而获得局部线性收敛速率；(2) 将上述结果应用于渐进采样策略框架，将Gradient-Eigenstep作为子问题求解器，在目标与约束函数为大样本均值定义的等式约束优化问题中，实现改进的样本复杂度。

Result: 证明了Gradient-Eigenstep算法在强二阶驻点附近以适当的参数设置可取得局部线性收敛；在渐进采样策略中作为子问题求解器使用时，相比直接求解全样本问题，获得了更优的最坏情况样本复杂度。

Conclusion: 该工作不仅深化了对Gradient-Eigenstep算法理论性质的理解，还展示了其在采样策略下的实际价值，为非凸等式约束优化在大规模数据场景中的高效求解提供了新的理论保证。
...
### [110] [High-resolution Calibrated Probabilistic Hourly Precipitation from a Deterministic Forecast](https://arxiv.org/abs/2608.12685)
*Thomas M Hamill*

Main category: stat.ML

TL;DR: 本文提出一种基于Attention Residual U-Net的深度学习概率定量降水预报（PQPF）方法，通过混合两个Gamma分布来预测逐小时无降水概率及正降水量分布。模型使用GRAF和GFS的NWP预报数据及地形信息作为输入，以MRMS雷达数据为目标，采用负对数似然作为训练损失函数，并在CONUS整个区域上单次前向推理生成高分辨率、高可靠性的概率降水预报，特别在地形起伏大的区域表现出色。


<details>
  <summary>Details</summary>
Motivation: 传统定量降水预报通常只提供确定性预报，无法充分表达降水的不确定性。概率降水预报（PQPF）能更好地表征降水的不确定性和极端事件风险，但现有方法在空间细节、可靠性以及地形复杂区域的表现上仍有不足，需要更先进的深度学习方法来改善。

Method: 采用Attention Residual U-Net神经网络架构，将预报输出建模为无降水概率与两个Gamma分布的加权混合。输入包括The Weather Company的GRAF对流解析NWP逐小时降水、地形信息和NOAA GFS的柱平均相对湿度；目标数据为与GRAF同网格采样的MRMS雷达质量控制数据。网络输出每个网格点的分布参数，训练采用负对数似然作为严格评分规则（proper scoring rule），并使用气候初始化保证收敛稳定性。推理时在CONUS域上进行单次前向传播，通过边缘复制填充满足网络空间可整除性要求。

Result: 生成的预报在空间上具有高分辨率细节，可靠性高，相对于气候预报和更简单的参考预报方法展现出显著的技巧优势，尤其是在地形变化较大的区域，概率估计效果尤为突出。

Conclusion: Attention Residual U-Net结合混合Gamma分布输出的框架能够有效生成空间细致且可靠的概率降水预报，是一种适用于业务化PQPF的强有力方法，尤其在地形复杂区域具有明显优势。
...
TL;DR: 本文提出了一个基于切片最优传输（sliced optimal transport, SOT）的有限时域控制框架，用于驱动态系统的状态分布从指定初始分布到达终端分布。在每个采样时刻，利用一维投影最优传输映射定义方向性终端条件，通过最小能量实现得到随机单方向控制器，再对投影方向取平均得到确定性切片反馈。文章证明了对单积分器动力学，该反馈使切片Wasserstein距离非增；对高斯端点分布，反馈是仿射的、保持高斯性，并能将均值和协方差精确驱动到目标值。此外还给出了线性衰减的增益特性、控制能量刻画，以及随机控制器在采样周期趋零时收敛到平均切片流的结论，并将构造推广到线性控制系统。


<details>
  <summary>Details</summary>
Motivation: 最优传输为分布转向提供了自然的几何方法，但其实现通常需要在全状态空间构造传输映射或耦合，维度高、计算复杂。切片最优传输虽通过一维投影避免了全维构造，但投影映射只给出方向性位移，并未直接提供可实现的反馈律。因此需要搭建一个桥接切片最优传输与反馈控制的框架，使得投影结果能够转化为实际可执行的控制器。

Method: 提出一个基于切片最优传输的有限时域控制框架。具体方法为：(1) 在每个采样时刻，由投影最优传输映射定义方向性终端条件；(2) 通过最小能量实现得到一个随机化的单方向控制器；(3) 对所有投影方向取平均，得到确定性的切片反馈控制器。对于单积分器系统给出收敛与单调性分析；对高斯端点情形分析仿射性质与矩演化；并利用可达性归一化坐标和局部可控性Gramian将框架推广到线性系统。

Result: 对单积分器动力学，平均反馈使切片Wasserstein距离非增；对高斯端点分布，反馈为仿射、保持高斯性，并精确驱动均值协方差到目标值；识别出与分布相关的增益，造成切片Wasserstein距离线性衰减并给出控制能量的显式刻画；证明了随机控制器在采样周期趋零时收敛到平均切片流；对均匀完全驱动系统可达性归一化坐标可即时实现切片速度，对一般可控系统可用局部可控性Gramian实现有限步精确实现。数值算例验证了分布流行为。

Conclusion: 该工作成功将切片最优传输从纯几何工具转化为可实际实现的分布转向反馈控制律，为有限时域分布控制提供了一种低维、可计算且可扩展的方法，并推广至一般线性可控系统，为后续更广泛的非线性与随机系统分布控制奠定了基础。
...
### [112] [Distinguishing case-mix from context heterogeneity in prognostic regression model synthesis settings](https://arxiv.org/abs/2608.12885)
...
TL;DR: 本文提出一种诊断方法，用于区分预后回归模型中的系数异质性来源：是病例组合（case-mix）还是情境效应（contextual effects）。该方法通过在降维空间中拟合局部回归，用自编码器将平滑系数曲面划分为跨站点参考和站点特异性偏差，并在结局尺度上生成观测级和站点级摘要。在COPD双站点试验中验证，发现系数表面变异以情境性为主，而观测级变异以案例组合为主。


<details>
  <summary>Details</summary>
Motivation: 在多站点预后回归建模中，分析者需要决定是使用统一的联合模型还是站点特异性模型。现有度量如系数级别的tau^2虽能量化异质性，却无法区分异质性的来源。区分案例组合异质性（由非线性函数近似和不同协变量分布导致）与情境性异质性（由不同站点对可比患者需要不同回归关系导致）对于模型选择至关重要，但此前缺乏系统性诊断工具。

Method: 方法包括四个核心步骤：(1) 拟合站点特异性局部回归于维度缩减空间（autoencoder结构化的潜在空间）；(2) 将平滑的系数曲面分解为跨站点参考部分和站点特异性偏差部分；(3) 将此分解投影到结局尺度，生成观测级和站点级的方差摘要；(4) 使用置换站点标签的阴性对照来检验情境性摘要是否可能仅由偶然产生。

Result: 在包含两个站点的COPD试验中，前三个主要潜在斜率坐标上的系数表面变异以情境性为主。观测级结局尺度方差分解以案例组合为主导，而站点间聚合则集中在情境性差异而非案例组合偏移。置换站点标签的阴性对照用于评估情境性摘要的显著性。

Conclusion: 该诊断框架能够区分预后模型系数异质性的来源（案例组合vs情境效应），为分析者决定采用联合模型还是站点特异性模型提供依据。研究强调了在多站点数据分析中，异质性诊断不应仅停留在系数层面，还需通过潜在空间学习区分其结构性来源。
...
TL;DR: 本文提出CardioState-JEPA，一个跨ECG、PPG、PCG三种心脏感知模态的共享心脏基础模型，采用生理感知的联合嵌入预测架构，将异构波形映射到统一token空间，通过掩码潜在心脏状态预测进行预训练，并利用延迟对齐器处理电-机械-血液动力学事件间的时间偏移。在25个下游任务上，冻结编码器在PPG、PCG和ECG分类上分别比最优秀的自监督信号基线提升8.2、18.8和15.5个AUROC点。


<details>
  <summary>Details</summary>
Motivation: ECG、PPG和PCG从不同视角描述同一个心动周期，但现有心脏基础模型仅针对单一传感模态训练，未能利用跨传感器的共享生理学信息，这浪费了异构信号间互补性带来的潜在表征提升。

Method: 构建CardioState-JEPA基础模型：(1) 将异构波形映射到统一的token空间；(2) 使用单一共享Transformer编码器处理；(3) 通过预测掩码的潜在心脏状态进行预训练，预训练目标聚焦于共享生理状态而非传感器特定的波形外观；(4) 引入学习式延迟对齐器（learned delay aligner）以匹配电、机械和血液动力学事件在对应心动时刻的时间偏移；(5) 采用两阶段训练：先从丰富的单模态数据学习模态内结构，再用配对数据在潜在心脏时间维度上对齐模态。

Result: 在25个覆盖ECG、PPG、PCG的下游任务上作为冻结编码器评估，相比最佳自监督信号基线，PPG分类平均提升8.2 AUROC点，PCG杂音检测提升18.8 AUROC点，ECG分类提升15.5 AUROC点；在多个ECG基准上达到或超过使用特权临床文本或监督标签训练的模型。

Conclusion: 异构心脏信号可以相互监督训练一个统一的心脏生理基础模型，验证了跨模态共享表征的可行性，表明共享生理学比单模态外观更能有效驱动心脏基础模型的学习。
...
TL;DR: 本文提出VALG，一个将机器学习理论研究过程组织为自主智能体工作流的系统。VALG结合多层级验证、自适应问题表述和图形化证明开发，在九个COLT 2026开放问题的子问题上进行评估，其中两个运行产生了与源问题范围匹配的最终定理候选。


<details>
  <summary>Details</summary>
Motivation: 机器学习理论研究通常需要研究者同时协调问题表述、定理目标和证明机制。传统上这一过程由人类研究者手工完成，本文旨在探究这一过程能否被组织为自主智能体工作流，从而实现ML理论研究的自动化探索。

Method: VALG系统包含三个核心组件：(1) 多层级验证（Verification），(2) 自适应学习理论问题表述（Adaptive formulation），(3) 图形结构的证明开发（Graph-structured proof development）。系统在源相关定理分支中维护固定的数学规范，检查类型化证明依赖图的定理级组合，并按依赖顺序构建和审查局部证明。当证明失败时，系统识别障碍来源于推导、证明结构还是定理表述，并相应调整策略。

Result: 在五个COLT 2026开放问题的九个子问题上的评估显示：两次运行产生了与源任务范围匹配的内部最终定理候选，其余七次产生了受限方法结果、特例或条件定理。案例分析表明VALG能够保持源范围匹配、放宽、条件结果和受阻尝试在数学上的区分。

Conclusion: VALG展示了自主智能体系统可以在ML理论研究中发挥作用，能够系统地执行假设形成、证明尝试、失败归因和问题重构的循环，并以开源形式发布以供进一步研究。
...
### [115] [Huber-Wasserstein barycenters for robust distribution-valued data](https://arxiv.org/abs/2608.13131)
*Carlos Cardoso-Perelló,Alberto González-Sanz*

Main category: stat.ML

TL;DR: 本文提出了一种稳健的Huber-Wasserstein重心（barycenter），通过将Huber损失直接引入最优传输代价（作用于个体传输位移而非传输距离）来构建分布型数据的稳健重心估计，实现了Wasserstein均值与L1型Wasserstein中位数之间的自然插值，并建立了完整的解析与统计理论基础。


<details>
  <summary>Details</summary>
Motivation: 分布型数据（distribution-valued data）的均值估计（如Wasserstein重心）对异常值敏感，而现有方法要么仅在优化后对Wasserstein距离施加Huber损失（度量空间Huber均值），要么缺乏对个体传输位移的细粒度鲁棒控制。作者希望构建一种既保留二次最优传输的局部行为、又能限制大位移影响的稳健重心估计，并为其提供严格的理论保证。

Method: 1) 将Huber损失直接嵌入最优传输代价函数，对个体传输位移（individual transport displacements）施加Huber损失，而非对整体Wasserstein距离施加。 2) 对带Huber损失的最优传输问题证明了对偶势的正则性与唯一性、最优传输映射的存在性，以及关于Huber参数的稳定性。 3) 对相应的重心问题证明了存在性、刻画、经验plug-in估计量的一致性，以及有限样本崩溃点（breakdown point）本质上等于1/2。 4) 在一维情形推导了逐点影响函数（influence function）和渐近分布，量化了鲁棒性-效率权衡，并比较了位移级Huber化与距离级Huber化在局部形状污染下的信息保留差异。 5) 通过数值实验验证稳健性及均值-中位数插值行为。

Result: 证明了Huber损失下的最优传输具有对偶势正则性、唯一性和传输映射存在性，以及Huber参数变化时的稳定性；重心问题具有存在性、一致性估计和近似1/2的有限样本崩溃点；一维情形下得到了影响函数和渐近分布，揭示了鲁棒性-效率权衡，并表明位移级Huber化在局部形状污染下能保留距离级Huber化所丢失的一阶信息。数值实验验证了所提重心的稳健性及其介于均值与中位数之间的行为。

Conclusion: 位移级Huber化的Huber-Wasserstein重心是分布型数据稳健估计的一种自然且理论完备的构造，它在Wasserstein均值和L1型Wasserstein中位数之间提供了可控的连续插值，同时兼具强鲁棒性（崩溃点约1/2）和良好的统计性质，特别在一维情形下具有完整的影响函数表征。
...
### [116] [Chance-constrained selection of sequential intervention strategies from counterfactual estimates](https://arxiv.org/abs/2608.13209)
*Minkyoung Kim,Beakcheol Jang*

Main category: stat.ML

TL;DR: 提出一个predict-then-optimize两阶段框架：预测阶段用任意可互换的估计器输出结果值和成本分布，优化阶段在有限候选集上进行机会约束选择，以界定累计成本超预算的概率，从而提供安全-效用前沿并给出无分布有限样本界。


<details>
  <summary>Details</summary>
Motivation: 许多运营决策是受累积资源限制的干预序列（如维护排程受工时预算约束）。传统方法只约束均值成本而非尾部，无法控制超预算过冲的频率；且现有两阶段架构只在单一决策点分配，能约束成本尾部的分布方法则依赖指定模型而非从数据识别。

Method: 提出predict-then-optimize框架：(1) 预测步骤，任何能返回结果值和成本分布的估计器都可被决策规则消费，预测器可互换；(2) 优化步骤，在有限候选集上进行机会约束选择，界定累计成本超过预算的概率。由于尾部不跨阶段分解，每个策略被整体评分。扫描可容忍违规概率生成安全-效用前沿，并提供无分布有限样本边界。

Result: 在五个环境中验证，其中四个提供精确反事实真值（涵盖临床治疗和设备维护），第五个采用数字健康微随机试验的真实结果。对比表明该方法在点估计规则超预算的环境中能守住预算，且前沿明确展示了对应的结果代价。

Conclusion: 所提框架能有效约束累计成本的尾部而非仅均值，在预算保持与结果效用之间提供了可量化的权衡，且预测器可互换增强了方法的通用性和可扩展性。代码已开源。
...
### [117] [Foundations of Independent Component Analysis](https://arxiv.org/abs/2608.13229)
*Patrick Forré*

Main category: stat.ML

TL;DR: 本文以自包含的形式阐述线性独立成分分析（ICA）模型的数学基础，基于测度论概率论背景，系统发展特征函数理论并给出不同假设强度下ICA模型的可识别性结果，同时介绍在线等变梯度下降ICA算法。


<details>
  <summary>Details</summary>
Motivation: ICA模型是信号处理和盲源分离中的重要工具，但其数学基础分散于多篇文献中，缺乏自包含且严谨的整理，尤其是面向具有测度论概率论背景读者的系统性阐述。作者希望填补这一空白。

Method: 首先系统构建R^d上概率测度特征函数的理论，包括其解析性质及对分布的确定与刻画作用；然后依次在源向量从非常数、非高斯到无高斯分量的渐强假设下，给出ICA模型的可识别性结果；最后在标准完备无噪声非高斯ICA设定下，给出在线等变梯度下降算法来恢复独立源。

Result: 在最严格假设（高斯无关的独立源）下，即使存在加性高斯噪声，独立源仍可在平移、置换、尺度和符号意义下被识别；算法部分提供了在线等变梯度下降方法用于数据中恢复独立源。

Conclusion: 本文为线性ICA模型提供了严谨且自包含的数学基础，明确了不同假设强度下的可识别性边界，并给出了可行的在线恢复算法，可作为相关研究者进入ICA理论领域的系统性参考。
...
### [118] [The data geometry of masking diffusion: Certified-optimal schedules via unmasking growth complexity](https://arxiv.org/abs/2608.13520)
*Martin J. Wainwright*

Main category: stat.ML

TL;DR: 本文研究离散扩散的掩码采样问题，提出一种称为"去掩码增长复杂度"(UGC)的路径解析数据几何度量。UGC的局部增量直接控制KL离散化误差，统一了Bernoulli子集和固定基数去掩码方案的分析。在log-reveal-odds坐标系中，该结构产生优化的单块和多块调度策略，并量化自适应计算精力带来的增益。作者展示了如何通过耦合揭示轨迹上的KL增量从样本中估计UGC增量，从而得到"认证最优"采样器，这些采样器能以高概率达到规定的KL误差，且迭代复杂度在Oracle过程的常数因子范围内。在精细划分极限下，平方根UGC密度的平方积分决定尖锐的首阶最优Euler离散化误差。


<details>
  <summary>Details</summary>
Motivation: 离散扩散模型中的掩码(masking)采样方法在生成建模中广泛使用，但对其离散化误差的理解缺乏统一的理论框架。现有分析通常针对特定调度策略或特定数据结构，无法量化数据几何对采样效率的影响，也难以实现自适应最优采样。

Method: 引入UGC度量刻画去掩码过程中的数据几何，利用其局部增量建立KL离散化误差的统一上界；在log-reveal-odds坐标下推导单块与多块的优化调度；通过耦合揭示轨迹的KL增量估计UGC，构建具有高概率KL误差保证和常数因子最优迭代复杂度的认证采样器。

Result: 在精细划分极限下，平方根UGC密度的平方积分精确决定首阶最优Euler离散化误差；实例显示自适应放置常数个块即可实现相对于粗调度的~Ω(√d)维度相关提升，表现出显著的维度依赖增益。

Conclusion: UGC为离散扩散采样提供了一种统一且可估计的几何框架，不仅实现理论最优的采样策略，还揭示了数据几何与离散化误差之间的定量联系，为自适应采样调度提供了坚实的理论基础。
...
### [119] [Exponential Convex Calibration Dimension for the Multi-Label Jaccard Measure](https://arxiv.org/abs/2608.13549)
*Mingyuan Zhang*

Main category: stat.ML

TL;DR: 本文研究多标签分类中逐实例Jaccard分数（IoU）的校准与风险转移问题。证明其损失矩阵非奇异且列仿射维度为2^s-1，给出精确校准的CC维界[2^{s-1}, 2^s-1]，意味着零遗憾校准需要指数维预测坐标；同时提出两种多项式维近似保证，包括F1到Jaccard的风险转移和MinHash平方损失的遗憾下界保证。


<details>
  <summary>Details</summary>
Motivation: Jaccard分数（IoU）是多标签分类和二分分割的标准评价指标，但其损失的校准性质和预测维度需求尚未被充分理解。核心动机是确定：要实现零遗憾校准需要多少预测维度？能否在多项式维度内实现固定遗憾容差的近似校准？

Method: 本文采用Boolean Möbius逆变换与有限MinHash Gram表示相结合的方法证明损失矩阵的非奇异性。校准维度下界通过构造因子加权分布和贝叶斯最优报告实现；近似保证方面，将现有(s^2+1)维F1代理转化为Jaccard风险转移，并提出基于MinHash的平方损失代理及其有符号变体以实现多项式维遗憾控制。

Result: 证明了Jaccard、平移损失和普通损失矩阵非奇异，损失列仿射维度为2^s-1；精确校准CC维满足2^{s-1} ≤ CCdim ≤ 2^s-1。F1到Jaccard的转移给出渐近Jaccard遗憾至多3-2√2；MinHash平方损失代理在任意条件标签分布上均匀实现α遗憾下限，直接构造维度O((s^2+s·log(1/ρ))/α^2)，有符号变体维度O((s+log(1/ρ))/α^2)。

Conclusion: 零遗憾校准本质上需要指数级预测维度，而固定的加性遗憾容差只需多项式预测维度即可实现，揭示了Jaccard指标校准确认中的一个基本维度分隔现象。
...
### [120] [Defensive Boosting for Online Probabilistic Forecasting](https://arxiv.org/abs/2608.13554)
*Georgy Noarov,Aaron Roth*

Main category: stat.ML

TL;DR: 本文提出了一种名为Defensive Booster的防御性在线预测算法，通过操作化boosting的"对偶视角"，同时获得两种先前只能由不同在线boosting技术分别提供的保证：(1) 在Brier得分上与H的跨度诱导的最佳预测器竞争；(2) 在平滑弱学习条件下，分类误差率与在线分类boosting一致。该算法仅需一个弱学习器，效率极高，并通过实验验证了其优越的预测性能和大幅提升的运行速度。


<details>
  <summary>Details</summary>
Motivation: 现有的在线boosting方法只能单独提供两种不兼容的保证：在线梯度boosting在Brier得分上具有竞争力，但当地类外无准确预测器时没有保证；在线弱到强boosting在弱学习条件下能驱动分类误差到零，但条件不满足时保证不足。如何在一个算法中同时获得这两种保证是核心挑战。

Method: 提出了Defensive Booster算法，是一种防御性预测算法。其核心创新在于操作化boosting的"对偶视角"：当算法的随机分类误差持续较高时，其错误权重会形成一种平滑的重加权，使得所有弱假设的边(edge)都很低，从而提供事后硬核(hard-core)证据表明弱学习条件不成立。这实现了两种保证的自动切换。同时提出了强自适应变体，在任意时间区间上同时满足两种保证。算法只访问一个弱类学习器，而之前的在线boosting方法需要维护大型弱学习器集成。

Result: Defensive Booster在自适应序列上同时获得两种保证：Brier得分与在线梯度boosting同速率的竞争力，以及在平滑弱学习条件下与在线分类boosting同速率的Brier得分和随机分类误差保证。强自适应变体在任意时间区间上保持两种保证。该算法仅需一个弱学习器，而先前的在线boosting方法需要大型集成。在合成和真实数据流上的实验显示其预测性能强大（有时大幅超过所有先前基线），运行速度提升数个数量级。

Conclusion: Defensive Booster通过一个简单而高效的算法解决了在线boosting中两种重要保证不可兼得的困境。其理论贡献在于建立了防御性预测与在线boosting之间的联系，并通过对偶视角提供了事后可验证的弱学习条件失败证据。实际应用价值体现在其单学习器的高效性、快速的运行时间以及优越的预测性能，使其成为在线概率预测和分类的有力工具。
...
### [121] [Graph-theoretic design of lasing networks for physical vision](https://arxiv.org/abs/2608.13097)
...
TL;DR: 本文提出一种在抽象图空间而非物理空间中优化物理神经网络拓扑的高效方法。以随机激光网络为例，建立"图度量→非线性激光物理→视觉性能"的定量三层联系，并利用图度量驱动进化算法生成网络拓扑，相比物理仿真实现3000倍加速，显著提升图像分类精度。


<details>
  <summary>Details</summary>
Motivation: 物理神经网络的优化极具挑战性：复杂多体物理虽能提供强大计算能力，但仿真成本高昂且大规模实验优化不可行，导致高维网络拓扑空间无法被有效直接搜索。

Method: 1) 以耦合纳米波导构成的随机激光网络为原型物理视觉系统；2) 建立图论指标→非线性激光物理→视觉性能的三层定量映射关系；3) 用物理仿真验证该关系后，利用图度量驱动的进化算法搜索网络拓扑，替代昂贵的物理仿真优化。

Result: 图度量优化的网络拓扑在图像分类任务上显著优于随机设计，同时相比物理仿真实现3000倍计算加速。

Conclusion: 该框架基于网络拓扑而非特定基质的物理特性运作，可迁移至其他基于网络的物理学习系统，为复杂强相互作用物理神经网络的定向设计与优化提供高效路径。
...
### [122] [Superconductivity in the $t$-$t'$ Hubbard Model from Symmetry-Preserving Neural-Network Quantum States](https://arxiv.org/abs/2608.12465)
*Riccardo Rende,Luciano Loris Viteritti,Antoine Georges*

Main category: cond-mat.dis-nn

TL;DR: 本文提出对称保持回流配对（SBP）神经网络波函数，通过构造性地保持平移对称性，避免了变分方法中因对称性破缺最小值导致的偏差，在高达24×24、504电子的t-t' Hubbard模型上达到了最先进的变分能量，并通过热力学极限外推证明1/8掺杂、t'/t=-0.2、U/t=8.0体系存在d波超导序，解决了长期争议。


<details>
  <summary>Details</summary>
Motivation: 二维掺杂Hubbard模型基态性质是强关联电子理论中的根本性问题，但变分方法的结果高度依赖波函数参数化、平均场初始化、固定场和边界条件，容易偏向某种对称性破缺，难以区分真实的竞争序与参数化引入的偏差。

Method: 引入对称保持回流配对（SBP）变分基，一种神经网络波函数，通过构造保证平移对称性，避免落入破缺对称性的局部极小值。在高达24×24（504电子）的t-t' Hubbard模型上进行变分优化，并外推到热力学极限。

Result: SBP变分基在24×24晶格上达到优于纯条纹态的变分能量，处于当前最优水平。热力学极限外推提供了1/8掺杂、t'/t=-0.2、U/t=8.0体系存在d波超导序的稳健证据。

Conclusion: 基于对称性和局域性的一般原理，SBP波函数为具有挑战性的相互作用费米子系统提供了广泛适用的变分表示，解决了关于该掺杂体系是否存在d波超导序的长期争议。
...
### [123] [Exceptional activated mode theory for generalized real-complex transitions](https://arxiv.org/abs/2608.12475)
*Mengjie Yang,Alexander N. Poddubny,Ching Hua Lee*

Main category: cond-mat.dis-nn

TL;DR: 本文提出一个通用的、非微扰的"激活模式"原理，用于描述非厄米系统中实谱到复谱转变（即放大起始）的阈值。核心洞见是转变起始仅涉及一个可利用谱失谐与模式级非厄米耦合竞争来变分确定的小希尔伯特子空间，从而得到适用于任意大小扰动的闭式激发条件，并应用于三类不同问题的阈值分析。


<details>
  <summary>Details</summary>
Motivation: 非厄米系统中实谱到复谱的转变标志着放大的起始（Exceptional Point），但现有阈值分析通常是模型特异的或微扰性的，缺乏一个统一的、非微扰的理论框架来预测这一转变阈值。

Method: 提出"激活模式"原理：在转变起始时仅有一个小希尔伯特子空间被激活，可通过谱失谐与模式级投影非厄米耦合之间的竞争进行变分求解。由此推导出闭式"奇异激活条件"（exceptional-activation condition），适用于任意大小的扰动量，而非仅限微扰范围。

Result: 将该框架应用于三个对照性问题：(i) 给出临界非厄米趋肤效应放大在所有系统尺寸下的闭式阈值；(ii) 建立杂质隧穿阈值与奇异点切换之间的新联系；(iii) 展示无底层拓扑相变情况下的激活通道切换现象。

Conclusion: 实复谱转变可被重新理解为通用的模式选择问题，与具体对称性无关，为非厄米系统中的放大阈值提供统一的理论描述。
...
### [124] [The nature of the "pseudogap" in the insulating phase of highly disordered superconductors](https://arxiv.org/abs/2608.12508)
*Mark Nikolaevsky,Abhisek Samanta,Nandini Trivedi,Aviad Frydman*

Main category: cond-mat.dis-nn

TL;DR: 本文将超高电阻平面隧穿谱技术应用于非晶氧化铟薄膜，并结合吸引Hubbard模型的量子蒙特卡洛模拟，在超导-绝缘体转变的深绝缘区探测单粒子激发谱。实验与理论惊人一致：单粒子能隙在转变后不仅存活而且随无序度显著增大（超过超导侧两倍），升温时能隙"填充"而非"闭合"。该工作首次定量证实了局域库珀对绝缘态的存在，统一了Tc以上赝能隙与绝缘体能隙这一配对但无全局相位相干的物理图像。


<details>
  <summary>Details</summary>
Motivation: 理论预测局域库珀对可在超导-绝缘体转变的深绝缘相中存在，但由于高绝缘薄膜需要超高电阻结和皮安级电流灵敏度，一直缺乏直接的谱学实验验证。作者旨在填补这一实验空白，并为赝能隙物理提供无序体系中无竞争电子序干扰的受控研究平台。

Method: 实验上采用超高电阻平面隧穿谱技术测量非晶氧化铟薄膜的单粒子激发谱；理论上运用吸引Hubbard模型的量子蒙特卡洛模拟进行直接对比，两者在深绝缘区同时探测单粒子能隙随无序度的演化规律及其温度依赖行为。

Result: 实验与理论定量吻合：单粒子能隙跨越超导-绝缘体转变后继续存活且随无序度大幅增加，峰值约为超导侧两倍以上；升温过程中能隙呈"填充"而非"闭合"行为，相干峰被抑制，谱权重向远超能隙标度的能量再分布。

Conclusion: 该工作为理论预言的局域库珀对绝缘态（Cooper-pair insulator）提供了直接的定量实验证实，并建立了一个统一框架：Tc以上赝能隙与绝缘体能隙本质上是同一种"配对但缺乏全局相位相干"现象的不同表现。
...
### [125] [Magnetism in antiperovskite (Li$_2$\textit{M})\textit{Ch}O (\textit{M} = Fe, Mn, Co; \textit{Ch} = S, Se) diluted magnets with fixed 1/3 filling: the key role of magnetic anisotropy](https://arxiv.org/abs/2608.13111)
...
TL;DR: 本文报道了锂富集反钙钛矿材料(Li₂M)ChO (M = Fe, Co, Mn; Ch = Se, S)的磁性研究。该材料中过渡金属和锂离子在X位随机分布，形成强稀释磁性亚晶格。研究发现，在1/3填充（接近但略高于渗流阈值）的体系中存在长程反铁磁有序，Néel温度从Mn体系的~30 K升高到Fe体系的~50 K和Co体系的70-90 K。除Co外，硫族元素对T_N影响不大。磁性各向异性是决定T_N的关键参数。


<details>
  <summary>Details</summary>
Motivation: 研究者旨在探索在固定1/3填充（接近渗流阈值）条件下，具有强稀释磁性亚晶格的体系中磁有序的演化规律，特别是自旋大小、磁各向异性和轨道构型对磁性行为的影响。

Method: 制备了一系列(Li₂M)ChO反钙钛矿材料，通过改变过渡金属（M = Fe, Co, Mn）和硫族元素（Ch = Se, S），系统研究了磁性随自旋尺寸、磁各向异性和轨道构型的变化。采用了磁化率测量和电子自旋共振（ESR）等手段。

Result: 观察到体系在350 K以上不存在明显的Curie-Weiss行为，而是呈现较大且弱温度依赖的磁化率。存在明确的长程反铁磁有序：T_N从Mn体系的~30 K到Fe体系的~50 K，再到Co体系的70-90 K。除Co外，硫族元素对T_N影响不大。过渡金属的磁各向异性是决定T_N的关键因素。

Conclusion: 强磁耦合和短程磁关联在远高于T_N的温度下仍存在（与室温下宽ESR信号一致），实际有序温度受磁稀释作用大幅降低。结构参数（如容忍因子和键角）对T_N影响有限，磁各向异性才是控制T_N的核心参数。
...
### [126] [Neural Quadratic Forms: A Unified Minimal Model for Sudden Learning and Scaling Laws](https://arxiv.org/abs/2608.13335)
*Liu Ziyin,Yizhou Xu,Tomaso Poggio,Isaac Chuang*

Main category: cond-mat.dis-nn

TL;DR: 该论文揭示了神经网络训练中“阶梯式”损失曲线与“幂律式”损失曲线统一于同一理论框架：层的置换对称性迫使近零权重的展开具有普适二次形式 Tr[WW^T A(x)]，所有架构差异都浓缩在唯一的“结构矩阵”A(x)中。训练动力学在序参量 M=WW^T 上封闭，并在数据矩阵共享特征基时约化为 Lotka–Volterra 方程，模式依次开启。初始权重越小，开启时间间隔越大，平台区表现为光滑流的奇异极限；当众多模式未分辨时，同一事件合并为训练时间的幂律，其指数可由理论预测。


<details>
  <summary>Details</summary>
Motivation: 梯度下降训练的网络尽管代价函数光滑，却常呈现“平台-突降”的阶梯式学习行为；与此同时训练损失又遵循光滑的幂律。这两种看似矛盾的行为在不同微观结构的架构中均出现，暗示存在少数相关集体变量。作者希望找出决定这些变量的机制，并统一解释阶梯式与幂律式学习现象。

Method: 作者利用神经层对可互换单元置换的不变性这一对称性，结合光滑性以及单元梯度在原点的消失条件，推导出近零权重展开的普适前导形式 Tr[WW^T A(x)]。这将感知机、注意力层、专家混合和卷积等不同架构统一为一个模型，仅通过“结构矩阵”A(x) 区分。训练动力学在 M=WW^T 上封闭；当数据矩阵共享特征基时，约化为 Lotka–Volterra 方程，模式依次被激活。理论预测平台区是光滑流的奇异极限，而未分辨模式合并时表现为训练时间幂律，指数可由理论算出。

Result: 作者从理论上证明了该普适形框架的正确性，并首次在数值上跨训练方法与多种架构（感知机、注意力、专家混合、卷积）验证了两种看似不同的行为——阶梯式平台与幂律下降——均能被统一理论所刻画。还定量预测了幂律的指数。

Conclusion: 神经网络训练的阶梯性和幂律性并非源于不同的机制，而是同一对称性约束下的普适动力学在不同分辨尺度上的表现。置换对称性决定了重要的集体变量（序参量 M），从而为理解深度学习的涌现式阶段学习提供了统一的理论基础。
...
### [127] [In-situ Adjoint Wave Control in Reconfigurable Non-Hermitian Nonlinear Systems](https://arxiv.org/abs/2608.13503)
*John Guillamon,William Tuxbury,Cheng-Zhen Wang,Owen Miller,Zin Lin,Tsampikos Kottos*

Main category: cond-mat.dis-nn

TL;DR: 本文提出并实验验证了一种在非线性多径波动系统中利用系统自身进行前向与伴随传播的位点伴随优化协议，将多径复杂性与非线性从障碍转化为物理优化的资源，在由同轴电缆与二极管腔构成的最小非线性平台上实现了无需数字孪生或数值反向传播的梯度提取。


<details>
  <summary>Details</summary>
Motivation: 传统波动信息处理通常回避复杂多径环境（因重复散射产生干扰路径、极端敏感性）以及非线性机制（破坏叠加原理），两者被视为波控障碍。作者试图证明二者并非天然缺陷，而可被转化为物理优化的关键资源，从而在复杂、部分未知环境中实现无需传统建模的自适应控制。

Method: 采用在位伴随优化（in-situ adjoint optimization）协议：在波浪混沌平台中引入单一局域非线性缺陷（二极管加载腔），系统自身同时执行梯度评估所需的前向与伴随传播；利用多径反复返回使波多次暴露于缺陷，从最小硬件生成丰富的非线性输入-输出映射与路径介导自由度；通过适当的伴随激励直接从测量中提取灵敏度，不依赖数字孪生或数值反向传播。实验平台由非可公度同轴电缆经T型接头互连构建。

Result: 在由非可公度同轴电缆与T型接头（其一接二极管腔）构成的最小非线性多径平台上实验验证了该协议，成功实现了直接基于测量的灵敏度提取与梯度评估，证明了多径与非线性作为优化资源的可行性。

Conclusion: 该工作开辟了在复杂且部分未知环境中实现自适应无线通信、成像与模拟智能的新途径，尤其是在传统建模不切实际的场景下，多径复杂性与非线性可被协同利用以实现物理层面的原位优化。
...
### [128] [The "Moiré Capacitor Effect" and Stabilization of Fractional Chern Insulators in Rhombohedral Graphene Superlattices](https://arxiv.org/abs/2608.12452)
*Nicolas Regnault,Heqiu Li,Yves H. Kwan,B. Andrei Bernevig,Jonah Herzog-Arbeitman*

Main category: cond-mat.str-el

TL;DR: 本文提出"莫尔电容器效应"机制，通过静电将价带电荷密度印迹到导带，增强菱方石墨烯-hBN超晶格中的莫尔势，从而稳定Chern数C=1的母态（填充ν=1），并进一步在ν=2/3处确认分数陈绝缘体（FCI）的出现。这是首个针对该体系中FCI态一致性的理论，解释了其在对齐样品中出现而在未对齐样品中缺失的原因。


<details>
  <summary>Details</summary>
Motivation: 尽管莫尔势对于菱方石墨烯-hBN超晶格中分数陈绝缘体（FCI）的必要性已被实验证实（最初在Ref. 1预测），但其来源和重要性的理论仍然缺失，需要建立一个统一的机制来解释该体系中FCI的稳定化条件。

Method: 1) 提出莫尔电容器效应机制，推导其解析形式；2) 提出母态理论，认为陈绝缘体的稳定性和空穴激发的平带性是掺杂获得FCI的必要条件；3) 采用多带精确对角化计算，验证在莫尔电容器效应存在时ν=2/3处FCI的出现，并对比无莫尔势情况下的带混合坍缩行为。

Result: 莫尔电容器效应显著增强莫尔势，稳定C=1的母态；多带精确对角化证实了在ν=2/3处存在FCI，且该FCI态由带间涨落稳定，而无莫尔势时带混合会导致FCI坍缩。

Conclusion: 提供了针对菱方石墨烯-hBN体系中同位对齐样品FCI态的第一个自洽理论，同时解释了未对齐样品中FCI缺失的原因，确立了莫尔电容器效应作为该体系中莫尔势增强与FCI稳定的关键机制。
...
### [129] [Boundary phases and thermodynamics of the Kondo spin-$s$ chain: from overscreened Kondo to boundary-bound states](https://arxiv.org/abs/2608.12453)
*Abay Zhakenov,Pradip Kattel,Andreas Gleis,Natan Andrei*

Main category: cond-mat.str-el

TL;DR: 本文研究自旋-1/2杂质耦合到自旋-s Takhtajan–Babujian链边界的强关联可积模型，结合边界共形场论、精确Bethe Ansatz、广义热力学Bethe Ansatz与张量网络模拟，统一描述了杂质屏蔽、边界束缚态形成和多激发塔重构的边界量子相变机制。研究发现弱反铁磁耦合实现过屏蔽2s-通道Kondo普适类，强耦合则产生局域边界模式及新的屏蔽机制，并揭示杂质熵的非单调温度依赖与谱函数的阈值特征。


<details>
  <summary>Details</summary>
Motivation: 传统Kondo框架无法充分描述强关联自旋链边界杂质在高耦合强度下出现的丰富物理现象，尤其是边界束缚态的形成和Hilbert空间的多塔重构机制。作者旨在突破常规Kondo范式的局限，建立统一的描述框架来理解强耦合区域中杂质屏蔽的新机制。

Method: 采用多方法互补的研究策略：(1) 边界共形场论（BCFT）确定弱耦合区域的低能普适类；(2) 精确Bethe Ansatz揭示边界量子相变序列及多激发塔结构；(3) 发展广义热力学Bethe Ansatz（gTBA）框架以捕捉跨所有区域的多塔贡献；(4) 大规模有限温度矩阵乘积算子（MPO）模拟进行交叉验证；(5) 动力学计算（杂质谱函数）补充阈值特征分析。

Result: 弱反铁磁耦合再现2s-通道过屏蔽Kondo普适类，杂质熵与BCFT预言定量吻合。强耦合下边界束缚态出现后，杂质熵呈现显著的非单调温度依赖，与MPO模拟定量一致。杂质谱函数出现反映塔结构的尖锐阈值特征，广义TBA框架成功刻画全参数区域的完整热力学。

Conclusion: 边界共形场论、精确Bethe Ansatz、广义热力学Bethe Ansatz和张量网络模拟共同构成了对相关自旋-s链杂质问题的统一描述，揭示了超越传统Kondo区域的丰富边界相图，为理解强关联体系中杂质屏蔽、边界束缚态形成与激发塔重构提供了完整的理论框架。
...
### [130] [Fermionic Anomalies of Finite Symmetries on Lattices](https://arxiv.org/abs/2608.12455)
*Ameya Chavda,Ryohei Kobayashi*

Main category: cond-mat.str-el

TL;DR: 本文提出了(1+1)D和(2+1)D有限内部对称性下费米子't Hooft反常的格点表征，通过对称短程纠缠(SRE)态的障碍来刻画。作者建立了费米子反常指标的层次结构，并揭示了格点对称性与连续QFT反常之间的不匹配：格点精确对称性无法实现连续QFT中的某些反常层。


<details>
  <summary>Details</summary>
Motivation: 理解费米子系统的对称性反常通常通过连续QFT框架进行，但量子场论中的反常分类并不总能由微观格点模型精确实现。本文旨在建立格点意义上的费米子't Hooft反常表征，澄清"哪些连续反常可以拥有精确的微观格点实现"这一基本问题。

Method: 作者采用格点系统框架，考虑由局域费米子和玻色子希尔伯特空间张量积构成的系统，对称性为中心扩张Z2^F→Gf→Gb的有限内部对称性。他们提取了对称算符的费米子反常指标层次，在(1+1)D由上同调数据(n2,ν3)刻画，在(2+1)D由三层上同调数据(n2,n3,ν4)刻画，并系统分析各层的SRE障碍性质。

Result: (1+1)D：证明了平凡反常指标的对称性可以onsite化并允许对称SRE态，指标忠实检测格点反常；格点对称不实现连续QFT中额外的H^1(BGb,Z2)反常层，且Gb=Z2时仅实现连续Z8分类中的偶Z4子群。(2+1)D：三个反常层(n2,n3,ν4)中任一层非平凡都阻碍SRE；对Gf=Gb×Z2^F还禁止对称可逆态；当Gb被费米子奇偶性非平凡扩张时，格点SRE障碍与连续反常不一致，并构造了Z4^F格点对称性作为反例。

Conclusion: 格点反常与连续反常存在系统性差异：格点精确对称性只能实现连续反常分类的一部分，且当玻色子群被费米子奇偶性非平凡扩张时，两者甚至不一致。这凸显了需要系统研究哪些连续反常允许精确微观格点实现，并为格点反常理论提供了严格框架。
...
### [131] [Revisiting magnon bound states: ferro-antiferromagnetic $J_1\!-\!J_2$ square-lattice model](https://arxiv.org/abs/2608.12459)
*Cesar A. Gallegos,A. L. Chernyshev*

Main category: cond-mat.str-el

TL;DR: 本文对海森堡 S=1/2 方格 J1-J2 铁磁-反铁磁模型中磁振子的配对与束缚态进行了系统分析，提出了晶格分波命名法，并揭示了高对称对动量下束缚态稳定性与有效维度降维/增强之间的定量关系。


<details>
  <summary>Details</summary>
Motivation: 理解磁振子束缚态不仅有助于阐明晶格上自旋翻转束缚态与连续介质中粒子束缚态之间的异同，还能为更一般的磁振子配对现象提供统一的理论框架。

Method: 在整个布里渊区内对有限对动量下的磁振子束缚态进行研究，引入晶格分波命名法，并通过有效维度降维或增强的机制来定量识别和描述高对称对动量下束缚态稳定性增强或脆弱性增加的机制，同时提出确定束缚态相边界的方法。

Result: 识别并量化了高对称对动量下束缚态稳定性增强或减弱的具体机制，证明这些机制与有效维度降维或增强直接相关，并揭示了束缚态随模型参数演化的规律。

Conclusion: 提供了理解磁振子配对问题的透明框架，并给出了一套确定束缚态相边界的实用方法，为更一般背景下磁振子束缚态的研究奠定了基础。
...
### [132] [Defect-induced optical magnons and local magnetic correlations in MnSb2Te4](https://arxiv.org/abs/2608.12505)
*Dhurba R. Jaishi,Bing Li,Tianxiong Han,S. X. M. Riberolles,D. M. Pajerowski,Jiaqiang Yan,R. J. McQueeney*

Main category: cond-mat.str-el

TL;DR: 本文利用非弹性中子散射技术直接探测了MnSb2Te4单晶中由缺陷诱导的磁相互作用，发现反位混合产生了不等价且无序的磁子晶格，强缺陷诱导的反铁磁耦合产生了具有大能隙的光学磁振子。半经典自旋动力学模拟准确捕获了有序态和顺磁态的磁激发，并确认线性Mn-Te-Mn键介导了与反位磁缺陷的耦合。


<details>
  <summary>Details</summary>
Motivation: 磁缺陷工程为操纵量子材料中的磁性和电子态提供了一种途径。在Mn(Bi,Sb)2Te4拓扑磁性绝缘体家族中，Sb取代促进了Mn和Sb原子之间的位点混合，这种位点混合会影响磁序和能带拓扑。然而，此前缺乏对这些缺陷诱导磁相互作用的直接探测，因此需要直接的实验手段来揭示这些相互作用的本质。

Method: 研究采用非弹性中子散射（INS）技术，对MnSb2Te4单晶进行直接探测。同时使用半经典自旋动力学模拟来重现和解释实验观测到的磁激发谱，包括有序态和顺磁态下的激发行为。

Result: 研究发现反位混合生成了不等价且无序的磁子晶格，其中强缺陷诱导的反铁磁耦合产生了具有大能隙（gap）的光学磁振子。半经典自旋动力学模拟准确捕获了有序态和顺磁态的磁激发特征，并识别出线性Mn-Te-Mn键是介导与反位磁缺陷耦合的关键结构单元。

Conclusion: 本研究通过直接的中子散射测量揭示了MnSb2Te4中缺陷诱导磁相互作用的微观机制，阐明了反位缺陷如何通过线性Mn-Te-Mn超交换路径产生强反铁磁耦合和大能隙光学磁振子，为理解该拓扑磁性绝缘体家族的磁性和电子结构提供重要实验依据。
...
### [133] [Dipolar-driven mean-field criticality in the ferrimagnet Eu$_2$MnSi$_2$O$_7$](https://arxiv.org/abs/2608.12639)
*Masahiro Kawamata,Maxim Avdeev,Yusuke Nambu*

Main category: cond-mat.str-el

TL;DR: 本文报道了Eu₂MnSi₂O₇这一褐帘石型铁氧磁体中的平均场临界行为。通过磁化测量和中子粉末衍射实验，发现其临界指数接近平均场值，表明长程偶极相互作用主导了该绝缘铁氧磁体的渐近临界行为。磁性结构由磁空间群P2₁2₁′2′描述，呈现由Dzyaloshinskii-Moriya相互作用驱动的倾斜铁氧磁构型。这是首个由偶极相互作用驱动平均场临界性的绝缘铁氧磁体。


<details>
  <summary>Details</summary>
Motivation: 探索平均场理论在复杂绝缘磁体中的适用性，并寻找新的铁氧磁体平台以研究长程相互作用驱动的临界行为。

Method: 结合磁化测量与中子粉末衍射技术，提取临界指数并精修磁性结构，确定磁空间群与磁构型。

Result: 获得的临界指数接近平均场值，证明长程偶极相互作用主导临界行为；磁性结构为P2₁2₁′2′空间群下的倾斜铁氧磁构型，由DMI相互作用驱动。

Conclusion: 该工作将平均场理论的适用范围扩展至复杂绝缘磁体，并确立Eu₂MnSi₂O₇作为探索铁氧磁性与长程相互作用的平台，这是首个偶极相互作用驱动平均场临界性的绝缘铁氧磁体。
...
### [134] [Correlation versus Causation in Quantum Criticality](https://arxiv.org/abs/2608.12770)
*Conrad Wichmann,Ryan Thorngren,Ruben Verresen*

Main category: cond-mat.str-el

TL;DR: 论文提出"causation"（静态磁化率）作为研究CFT中算符的新工具。与关联函数按|x|^{−2Δ}衰减不同，causation按|x|^{−2Δ+1}衰减，但研究发现其可额外衰减至多15个数量级。机制在于时间导数场无法贡献静态响应。该机制用于识别(2+1)D临界Ising模型中的新角主场（Δ≈8.8）和重磁线缺陷主场（Δ≈4.6），并解释了(1+1)D无间隙SPT相中边缘模式分裂异常小的现象，构造出分裂尺度小至1/L^18和1/L^25的自旋链。


<details>
  <summary>Details</summary>
Motivation: 传统关联函数通过空间衰减揭示标度维数，但作者提出研究静态磁化率（causation）作为替代/补充工具。关键动机在于发现causation可展现远超维度分析预期的额外衰减（最多15个数量级），这一现象源于时间导数场无法贡献静态响应的普遍机制，并具有广泛的理论与应用价值。

Method: 从CFT维度分析出发建立causation的衰减标度| x |^{−2Δ+1}，揭示时间导数场在静态响应中消失的机制，即确保主导项来自主场（primaries）而非descendants。利用这一原理结合DMRG数值方法识别(2+1)D临界Ising模型的角主场和磁线缺陷主场；同时将该机制映射到(1+1)D无间隙SPT相的边缘模式局域化，指导自旋链构造以极小化边缘模式分裂。

Result: 发现causation可产生额外15个数量级的衰减；鉴定出(2+1)D临界Ising模型中此前未分辨的角主场（Δ≈8.8）和重磁线缺陷主场（Δ≈4.6）；解释了边缘模式分裂异常小的观测现象，并构造出分裂小至1/L^18和1/L^25的自旋链。

Conclusion: causation是揭示CFT中主场信息的有力工具，其普遍机制（时间导数场不贡献静态响应）不仅深化了对静态响应的理解，还在临界系统和无间隙拓扑相中带来重要应用，具有广泛的适用性。
...
### [135] [Topology and Quantum-Spin-Classical-Spin Crossover of the Gapped Kondo Effect](https://arxiv.org/abs/2608.13065)
*David Krüger,Michael Potthoff*

Main category: cond-mat.str-el

TL;DR: 本文通过朗扎斯变换与自洽组态相互作用方法，系统研究了有能隙近藤效应的局域相图，涵盖量子自旋（S=1/2）、欠屏蔽（S>1/2）与过屏蔽（双通道）等多种情形。通过引入B空间陈数（有限磁场极限）和S空间陈数（经典自旋极限），将量子与经典自旋哈密顿量连续关联，并用拓扑不变量和散射理论刻画各相。研究发现双通道过屏蔽近藤效应伴随自发粒子-空穴对称性破缺，形成非平凡的量子-经典相图。


<details>
  <summary>Details</summary>
Motivation: 有能隙近藤效应标志着量子杂质物理与拓扑绝缘体物理的交叉，此前对不同自旋大小和屏蔽通道情形下的相图缺乏系统研究，尤其是量子自旋极限与经典自旋极限之间的拓扑对应关系仍未阐明。

Method: 采用朗扎斯变换结合自洽组态相互作用（SCI）方案进行数值计算，定义三类拓扑不变量：传统动量空间陈数、耦合杂质自旋到虚构局域磁场的B空间陈数（B→0极限），以及经典杂质自旋的S空间陈数（B→∞或S→∞极限）。通过调节磁场强度在量子与经典极限间连续插值，辅以散射理论解释各相。

Result: 获得了S=1/2、S>1/2（欠屏蔽）及双通道过屏蔽情形的完整局域相图。量子与经典自旋极限间的相图可通过B空间/ S空间陈数建立连续对应；双通道过屏蔽情形展现出自发粒子-空穴对称性破缺，产生高度非平凡的量子-经典相图。

Conclusion: 有能隙近藤效应可由多种拓扑不变量完备刻画，量子与经典自旋极限之间存在可连续变形的拓扑联系，而多通道过屏蔽情形引入的对称性破缺使相图结构更加丰富，为理解量子-经典过渡中的拓扑相变提供了统一框架。
...
### [136] [Accelerating a Strong-Coupling Non-Equilibrium Steady-State Impurity Solver using (Quantics) Tensor Trains](https://arxiv.org/abs/2608.13146)
*Bastian Schindler,Martin Eckstein*

Main category: cond-mat.str-el

TL;DR: 本文提出并比较了四种不同的积分元参数化方法，通过张量交叉插值获得低秩（quantics）张量-train表示，用于高效计算强耦合展开中的高维、时间有序积分。基于高斯基准验证了精度和计算标度，并在平衡和非平衡DMFT计算中验证至强耦合展开三阶，同时扩展到含延迟密度-密度相互作用的杂质模型及非平衡扩展DMFT。


<details>
  <summary>Details</summary>
Motivation: 强耦合展开中高阶图解修正的计算主要受限于高维时间有序积分的评估，传统方法计算成本极高，亟需一种更高效、可控且可系统改进的方法来处理这类积分。

Method: 提出四种不同的积分元参数化方案以获得低秩quantics张量-train表示，特别强调了一种quantics时间差分离散化方案，其中所需的延迟卷积直接在quantics张量-train形式中执行；使用受控高斯基准分析精度、键维数和计算标度。

Result: 高斯基准验证了各方法的精度与标度；在自洽平衡和非平衡DMFT计算中成功验证了最有效的公式，实现了强耦合展开至三阶的计算；进一步将该求解器扩展到含延迟密度-密度相互作用的杂质模型，并应用于非平衡扩展DMFT。

Conclusion: 张量交叉插值显著降低了高阶图解评估的计算成本，为远离平衡的量子杂质计算提供了一个可控且可系统改进的框架。
...
### [137] [Emergent Symmetry-Protected Topological Phases via Polyakov Confinement in Quantum Spin Systems](https://arxiv.org/abs/2608.13238)
*Li-Wei He,Shun-Li Yu,Jian-Xin Li*

Main category: cond-mat.str-el

TL;DR: 本文证明Polyakov禁闭机制可以将Dirac自旋液体转变为对称性保护拓扑（SPT）态，通过单体占据约束触发磁单极凝聚，禁闭体自旋子同时保留无能隙边缘态，实现了自旋子版的量子自旋霍尔效应，并在三角反铁磁体（含Dzyaloshinskii-Moriya相互作用）上通过大尺度变分蒙特卡洛模拟提供了微观证据。


<details>
  <summary>Details</summary>
Motivation: 凝聚态物理的核心问题之一是理解量子自旋系统中分数化激发与规范结构如何重新禁闭形成新的物相，探索从量子自旋液体到传统物相转变的机制与途径。

Method: 利用Polyakov禁闭机制（紧致规范场论的基石）作为动力学引擎，结合物理希尔伯特空间固有的单体占据约束触发磁单极凝聚；采用大尺度变分蒙特卡洛模拟于含Dzyaloshinskii-Moriya相互作用的三角反铁磁体上，通过Wilson环面积律、拓扑纠缠熵及自旋泵浦响应等观测量表征该SPT相。

Result: 实现了Dirac自旋液体到禁闭SPT态的转变：体自旋子被动力学禁闭而边缘模式保持无能隙；Wilson环满足特征面积律、拓扑纠缠熵为零；识别出在磁场下可测量的自旋泵浦响应，直接编码母Dirac锥的Berry曲率。

Conclusion: 揭示了从量子自旋体系到SPT物理的全新路径，将规范场论、量子磁性与拓扑物质三个领域联结起来，为研究分数化激发重禁闭形成新物相提供了理论框架与微观证据。
...
### [138] [Imaginary-time correlations in time-sliced stochastic series expansion](https://arxiv.org/abs/2608.13477)
*Ryan Flynn,Adam Iaizzi,Sibin Yang,Ying-Jer Kao,Anders W. Sandvik*

Main category: cond-mat.str-el

TL;DR: 本文在随机级数展开（SSE）框架内提出了一种高效且通用的算法，用于计算对角和非对角算符的虚时关联函数。该算法通过对SSE算符串进行离散虚时切片，在无离散误差的虚时网格点上提供关联函数；对于非对角算符，推导出可直接集成到现有directed-loop或cluster更新方案中的估计量。通过一维横向场Ising模型和XXZ自旋链进行基准测试，结果与精确对角化高度一致。


<details>
  <summary>Details</summary>
Motivation: 量子蒙特卡洛（QMC）方法结合数值解析延拓技术可以从虚时关联函数中提取实频动力学性质，但高效计算虚时关联函数在处理非对角算符（相对于计算基）时往往具有挑战性，因此需要一种通用的高效算法来同时处理对角和非对角算符的虚时关联函数计算。

Method: 在SSE框架内，通过对算符串进行离散虚时切片，在定义的虚时网格点上计算关联函数且无离散误差；针对非对角算符，推导了可集成到现有SSE directed-loop或cluster更新方案中的估计量，仅引入极小的额外计算开销。

Result: 在一维横向场Ising模型（使用cluster更新）和XXZ自旋链（使用directed-loop采样）上进行基准测试，结果与小型系统的精确对角化仅有统计误差级别的一致性；同时研究了更大系统以展示算法的效率。

Conclusion: 该算法为SSE框架内计算对角和非对角算符的虚时关联函数提供了一种通用、高效且无系统离散误差的解决方案，具有良好的可扩展性和实用性。
...
### [139] [Resonant Far-Infrared Spectroscopy of Flat-Band Fermions in Magic Angle Graphene](https://arxiv.org/abs/2608.12553)
...
TL;DR: 本文利用新开发的毫开尔文远红外（FIR）光谱平台，在魔角扭曲双层石墨烯（MATBG）中首次观测到平带电子的特征远红外共振，并通过拓扑重费米子模型揭示了巡游拓扑电子作为"天线"与局域重电子杂化耦合的物理机制，确立了光-重费米子杂化的光学选择定则。


<details>
  <summary>Details</summary>
Motivation: 虽然莫尔工程在扭曲二维材料中可根本改变低能带、相互作用和拓扑量子态，但受极端实验挑战限制，特征远红外波段下相互作用莫尔带的光谱研究长期处于空白，亟需在该波段实现对MATBG平带电子的直接光学探测与理解。

Method: 开发毫开尔文温度的远红外（FIR）光学光谱测量平台；在MATBG中测量平带电子的FIR特征共振；结合拓扑重费米子模型（THF）进行理论建模，分析巡游拓扑电子与局域重电子的杂化耦合对光学响应的贡献，并推导光学选择定则。

Result: 观测到高度可调谐的轻/重费米子相互作用光谱特征；证明巡游拓扑电子充当"天线"强烈耦合光场，共振频率受与局域重电子杂化重整化；确立了揭示光-重费米子杂化关键对称性的光学选择定则；在电荷中性点观察到低于局域库仑能的显著共振，表明新的多体模式涌现。

Conclusion: 实验与理论建模提供了对MATBG中光-物质相互作用的基础理解，实现了毫开尔文温度下莫尔带的共振光学光谱学，为研究莫尔平带中的多体物理开辟了新途径。
...
### [140] [Thermal Hall tomography of chiral superconductivity in rhombohedral graphene](https://arxiv.org/abs/2608.12586)
*Kumar Ghosh*

Main category: cond-mat.str-el

TL;DR: 本文提出在菱面体石墨烯（rhombohedral graphene）等手性超导体中，低温热霍尔电导能直接读出Bogoliubov–de Gennes (BdG) Chern数这一拓扑整数，无需重建复杂的正常态费米面。通过对称/反对称哈密顿量分解论证了反对称部分拓扑惰性，并用525个参数点的直接Chern计算验证不变量被保持，指出了Bogoliubov费米面破坏量子化的条件判据。


<details>
  <summary>Details</summary>
Motivation: 手性超导体的边缘携带手性Majorana模式，其数量由单一的整数量子数——BdG Chern数——决定。但三十年来候选材料均未能测量该整数，因为通常引用的磁学特征并不受拓扑保护。菱面体石墨烯使该问题既紧迫又可回答：磁成像虽能分辨超导相内可重写的破缺时间反演对称的畴，但量子振荡揭示的正常态过于复杂，无法逐pocket重构。因此需要一种能直接返回该整数的可测量。

Method: 作者展示了低温热霍尔电导κ_xy/T=(π²k_B²/6h)C_BdG可直接返回Chern数整数。对带投影配对情形，该值等于被占据动量区域所包围的配对涡旋缠绕数。算法上，将带内(valley内)哈密顿量分解为对称与反对称两部分，以隔离真实材料中的三角翘曲（trigonal warping）和有限Cooper对动量；证明反对称部分拓扑惰性，并通过525个参数点的直接Chern数计算验证不变量被保持；确定一个不等式标记Bogoliubov费米面何时移除量子化。

Result: 热霍尔电导平台直接读出Chern数整数，其符号随成像畴反转；写入的畴壁应携带2|C_BdG|条Majorana通道。所需的热测量技术在毫开尔文温度下已能分辨封装的石墨烯中的单热量子，使该方案在实验上可行。

Conclusion: 在菱面体石墨烯这一具体材料平台上，热霍尔电导为测量BdG Chern数这一拓扑整数提供了直接、无需重构正常态费米面的实验途径，弥补了三十年来手性超导体拓扑量子数无法被测量这一根本性空白，并对实验实现给出了明确可操作的预测。
...
### [141] [Observation of Time-Domain Braiding of Non-Abelian Anyons at $ν= 5/2$ State](https://arxiv.org/abs/2608.12897)
...
TL;DR: 该论文报告了在ν=5/2分数量子霍尔态中首次观测到非阿贝尔任意子的"时域编织"签名。通过在弱分割的高稀释一维边缘模式上独立测量下游带电模式和上游中性模式的电流涨落（散粒噪声），实验结果与"粒子-空穴Pfaffian"拓扑序中非阿贝尔任意子的理论预测一致，为非阿贝尔任意子的存在提供了实验证据。


<details>
  <summary>Details</summary>
Motivation: 非阿贝尔编织统计是拓扑量子计算的基础，但此前一直缺乏实验证据。与阿贝尔任意子仅改变波函数的统计相位不同，非阿贝尔任意子的编织可实现简并子空间内的酉变换，其验证对未来拓扑量子计算具有重要意义。

Method: 采用"时域编织"方法，将此前在ν=1/3态中验证阿贝尔任意子的方法推广至ν=5/2态。通过对高度稀释的一维边缘模式进行弱分割，独立测量下游带电模式的散粒噪声和上游中性模式的散粒噪声，分别探测阿贝尔和非阿贝尔任意子的编织过程。

Result: 下游带电模式与上游中性模式的独立噪声测量结果均与"粒子-空穴Pfaffian"拓扑序中关于下游阿贝尔任意子和上游非阿贝尔任意子"时域编织"的理论预测定量吻合。

Conclusion: 独立测量的实验结果与理论预测的一致性，为ν=5/2分数量子霍尔态中存在非阿贝尔任意子提供了证据，这是该领域长期追求的里程碑式结果。
