...
TL;DR: 本文提出一种修改后的哥本哈根解释，通过假设普适的观察者无关量子态、将波函数坍缩视为量子力学时间演化的一部分、以及明确区分量子系统状态与观察者认知，来解决Wigner之友和扩展Wigner之友悖论，并给出可证伪的概率差异预测。


<details>
  <summary>Details</summary>
Motivation: Wigner之友悖论及扩展版本揭示了量子力学测量问题中观察者角色的内在矛盾，传统哥本哈根解释无法令人满意地解决这些悖论，需要修改的解释方案。

Method: 提出三个核心假设：(i)存在普适且观察者无关的量子态；(ii)测量过程中的波函数坍缩是量子力学时间演化（包括孤立系统内部）的组成部分；(iii)严格区分量子系统状态与意识观察者的知识。在此基础上构建修改后的哥本哈根解释。

Result: 该解释解决了Wigner之友和扩展Wigner之友悖论，在某些结果概率上与先前分析产生差异，这些差异原则上可用来证伪该修改或先前分析（或两者）。该论证同样适用于基于坍缩模型的机制性描述（随机薛定谔方程扩展或幺正演化和不可逆性的统计力学方法）。

Conclusion: 修改后的哥本哈根解释为悖论提供了解决路径并给出了可检验的预测差异，但未解决测量机制缺失这一根本问题；其论证框架也兼容未来可能的机械论坍缩模型。
...
TL;DR: 本文研究了量子信息理论中长期未解的问题——负偏转置（NPT）纠缠是否可能保持不可蒸馏性。研究聚焦于第一个未解决的三拷贝Werner端点情形，通过引入互补偏迹的锋利无维不等式和正交三部分向量的最优外平方不等式，证明了在某些条件下端点非负性，并建立了精确的交叉Gram判据。


<details>
  <summary>Details</summary>
Motivation: 负偏转置纠缠是否可保持不可蒸馏是量子信息理论中的长期开放问题。理解Werner态端点（特别是三拷贝情形）的非负性/蒸馏性边界对于阐明纠缠可蒸馏性理论至关重要。

Method: 本文采用两种核心数学工具：（1）互补偏迹的锋利无维不等式（sharp dimension-free inequality）；（2）正交三部分向量的最优外平方不等式。后者将局部SWAP奇偶统计量与可分解Plucker双向量的度量数据联系起来。此外，针对非正常算子引入了精确的交叉Gram判据。

Result: 证明了每个正半定秩二系数算子的端点非负性以及任意有限局域维度下完全正常秩二扇区的非负性。对于真正的非正常算子，当局域输入输出支撑重叠至多为二时（包括任何含qubit因子的系统），以及当任一支撑平面包含乘积射线时，交叉Gram判据证明非负性。显式反态和秩边界族确立了常数和秩限制的最优性。

Conclusion: 通过多种不等式和判据的联合作用，本文在NPT纠缠可蒸馏性这一长期问题上取得了实质性进展，严格划定了三拷贝Werner端点非负性成立的边界条件，并证明了所导出常数和秩限制的紧致性。
...
TL;DR: 本文建立了一个曲率-规范框架，将嵌入流形的几何视为可调控制场，通过推导螺旋面上两体系统的精确约化哈密顿量，证明曲率会修改有效动力学结构而投影规范场重构守恒动量景观，从而实现有界相空间、零能量局域化、对称性破缺分岔和临界软模行为等可控制转变；量化几何依赖有效势后展现出从谐振束缚演化到四阶临界态的谱重构。


<details>
  <summary>Details</summary>
Motivation: 几何正日益被视为一种能够超越常规外部控制机制、主动修改量子系统行为的物理资源。作者希望建立一种框架，使嵌入流形的几何能够作为可调控制场来操控经典轨迹和量子态，从而为控制局域化和谱组织提供一条通用途径。

Method: 作者建立了曲率-规范框架：首先推导约束在螺旋面上的两体系统的精确约化哈密顿量；然后分析曲率如何修改有效动力学结构以及投影规范场如何重构守恒动量景观；再通过量化几何依赖的有效势来研究谱重构，观察谐波束缚向四阶临界态的演化。

Result: 该框架产生了可控制的动力学转变，包括有界相空间结构、零能量局域化、对称性破缺分岔和临界软模行为。几何重正化在量化后导致相应的谱重构，在局域化阈值处谐波束缚演化为四阶临界态，从而实现了对局域化和谱组织的几何控制。

Conclusion: 工程化的几何被确立为控制弯曲量子、电子、光子及合成平台上局域化和谱组织的一般性路线，其中形变本身成为功能性自由度而非被动约束。
...
TL;DR: 本文首次研究了混合态张量网络（LPDO）的基本定理，证明在可逆性或循环条件下，两个sLPDO表示生成相同密度矩阵当且仅当它们通过矩阵乘积等距变换相联系，并给出了周期性边界条件下一般LPDO基本定理可能受阻的反例。


<details>
  <summary>Details</summary>
Motivation: 矩阵乘积态（MPS）的数学结构已通过MPS基本定理得到充分理解，但混合态张量网络的类似理解仍然缺失。具体而言，如果两个纯化张量生成相同的密度矩阵，它们之间存在何种关系？这一基本问题尚未被系统研究。

Method: 研究顺序生成的LPDO（sLPDO）这一广泛子类，它可通过在初始态上连续应用量子信道来解释。在适当的可逆性或循环条件下，证明两个sLPDO表示生成相同密度矩阵的充要条件是它们通过作用在纯化键上的矩阵乘积等距变换相联系。同时构造反例说明周期性边界条件下一般LPDO基本定理的障碍。

Result: 在可逆性或循环条件下，两个sLPDO表示生成相同密度矩阵（对任意系统大小成立）当且仅当它们由作用于纯化键的矩阵乘积等距关联。但超出sLPDO范围时，给出了一个反例，表明周期性边界条件下的LPDO可能不存在一般的基本定理。

Conclusion: 该工作建立了混合态张量网络（sLPDO）的类比于MPS基本定理的数学框架，并讨论了其对混合态对称性保护拓扑相的影响，包括仅通过弱对称条件保护的非平凡相的可能性。
...
TL;DR: 本文证明无权重（全单位顶点权重）的团复形同调问题在g_2={X,CX,CCX,H⊗H}门集下仍是QMA_1^g2-困难的且包含于QMA_1^g2中，将King和Kohler（FOCS 2024）的加权结果推广到无权重情形，表明间隙承诺而非顶点权重是门控团复形同调复杂性的根源。


<details>
  <summary>Details</summary>
Motivation: 先前King和Kohler的结果表明在顶点权重和谱间隙承诺下，判定团复形的非平凡同调是QMA_1-困难的，但证明严重依赖顶点权重提供的尺度分离。自然的问题是：如果所有顶点权重都为1（无权重情形），问题是否仍然困难？本文旨在回答这一问题，以厘清复杂性的真正来源。

Method: 将带权复形的每个顶点放大为一个团，团的大小编码原始权重，实现"用扩张替代权重"的构造。通过选取合适的块尺寸使对称平均复现加权Hodge度量，从而对称扇区携带加权Laplacian（差一个公共标量因子），局部平均论证给出正交补上的一致下界，将加权间隙分析转移至无权重团复形。包含性通过Rudolph的稀疏整数团Laplacian的精确LCU（线性组合酉算子）模拟实现，该模拟在g_2门集下有效。

Result: 证明无权重团复形同调问题（在逆多项式谱间隙承诺下）是QMA_1^g2-困难的，且包含于QMA_1^g2中。构造过程中不引入额外低能态，也不产生虚假同调。

Conclusion: 问题的复杂性根源在于间隙承诺本身而非顶点加权结构，无权重情形保持了与加权情形相同的复杂类，只是通用门集从标准QMA_1的门集缩减为g_2={X,CX,CCX,H⊗H}。
...
TL;DR: 本文研究了量子信号处理中哈密顿量模拟的渐进最优技术，通过精细处理雅可比-安格展开中的贝塞尔函数尾部，利用Kapteyn和Watson不等式将多项式度数的乘性常数因子逼近理论最优值≈1，将整体开销估计减少了约e/2倍。


<details>
  <summary>Details</summary>
Motivation: 量子信号处理(QSP)是哈密顿量模拟的渐进最优技术，虽然已知多项式度数d∈Õ(τ)是渐进最优的，但现有的界与最优常数乘性因子（约等于1）之间仍存在差距，缩小这一差距对实际量子计算具有重要意义。

Method: 利用雅可比-安格展开将哈密顿量模拟问题转化为多项式逼近问题，通过仔细处理雅可比-安格级数中的贝塞尔函数尾部，借助Kapteyn和Watson不等式来获得更紧的界，从而改进多项式度数的常数因子。

Result: 将多项式度数的上界几乎完全闭合到最优常数因子，使得未来可能的改进不再具有实际意义；所有哈密顿量模拟任务的开销估计减少了约e/2倍。

Conclusion: 通过精细的数学分析（Kapteyn和Watson不等式处理贝塞尔尾部），成功将近乎闭合了常数乘积因子差距，为量子计算机上的哈密顿量模拟提供了更紧的复杂度上界。
...
TL;DR: 本文基于Walsh-Hadamard表示，发展了Schmidt-gauge非局域magic（非稳定化子资源）的数学理论，将该理论置于离散调和分析的框架下，证明其精确性、推导解析性质并建立与Walsh纠缠谱的联系。


<details>
  <summary>Details</summary>
Motivation: 量子态的完整非稳定化子（magic）资源包含局域的、依赖基的贡献，而基于局域酉最小化的非局域表述可以分离出与真正非局域关联相关的成分。Schmidt-gauge框架建立了非局域非稳定化子关联与量子多体态纠缠谱之间的直接联系，但其数学理论尚未充分发展。

Method: 基于随附Letter中引入的精确Walsh-Hadamard表示，将Schmidt-gauge非局域magic的表述扩展到任意二分划分；证明任意1×N二分划分下Schmidt gauge的精确性；推导Schmidt-gauge非局域magic的一般解析性质，包括纠缠界、选择定则以及与归一化Walsh谱矩的精确关系。

Result: 证明了Schmidt gauge对任意1×N二分划分的精确性，导出了非局域magic的纠缠界、选择定则以及与Walsh谱矩的精确解析关系。进一步将非局域magic解释为Walsh纠缠谱的逆参与比（IPR），将其与Walsh空间中的浓度度量联系起来。

Conclusion: Walsh-Hadamard表示揭示了原始谱表述中隐藏的离散调和结构，不仅仅是Schmidt-gauge非局域magic的等价表达式，而是其解析研究之自然数学框架，将整个理论置于离散调和分析的更广泛语境中。
...
TL;DR: 本文提出一种量子模拟格点规范理论（LGT）的新范式：利用高斯定律（Gauss's law）作为动态生成器，从更简单的哈密顿量出发生成规范理论本身，而非仅用于能量惩罚或探测丢弃违规态。作者在156量子比特IBM量子处理器上用101个量子比特实现了该方法，将每个Trotter步的纠缠门深度减少5倍，且实时动力学与目标LGT定量一致。


<details>
  <summary>Details</summary>
Motivation: 现有模拟LGT的两种范式均将高斯定律视为"约束"：模拟平台上通过能量惩罚抑制规范违背，数字平台上通过检测和丢弃违背结果。这两种方式都消耗额外资源。作者希望将高斯定律从"对称性保护"转变为"哈密顿量工程资源"，以更高效的路径模拟越来越复杂的规范理论。

Method: 从可直接编程的三体XXX模型出发，利用实验上高效的单量子比特U(1)规范对称生成器项来诱导U(1)格点规范理论的动力学。高斯定律不再被用作惩罚或后选择手段，而是作为生成U(1)LGT动力学的结构性资源，从而简化初始哈密顿量并降低电路复杂度。

Result: 在156量子比特IBM量子处理器上使用101个量子比特进行实验验证，实时动力学与目标LGT定量吻合；与直接实现相比，每个Trotter步的纠缠门深度降低5倍。

Conclusion: 该工作确立了规范保护（gauge protection）可作为哈密顿量工程的资源而不仅仅是对称性保持的手段，为在更高空间维度上进行更复杂规范理论的可扩展、资源高效的数字量子模拟开辟了新途径。
...
TL;DR: 本文提出Cornucopia码，一种硬件高效的量子LDPC码家族，在标准电路级噪声模型下实现超过1/2的超高编码率且伪阈值超过0.4%。通过将代码布局、原子重排和综合征提取调度协同设计，利用可重构中性原子阵列的长程连接性，所有X型和Z型校验可在12个纠缠层内并行测量（与代码尺寸无关）。单个代码块[[2844,1426,18]]可编码1426个距离为18的逻辑量子比特，在0.1%（0.01%）物理错误率下，每个逻辑量子比特每循环的外推逻辑错误率分别达到2.6×10^-16（1.9×10^-31），显著优于双变量自行车码所需的68000+物理量子比特。


<details>
  <summary>Details</summary>
Motivation: 大规模量子计算的核心挑战是抑制错误，量子纠错被认为是可行方案，但现有编码通常在编码效率、错误阈值和硬件可行性之间存在权衡取舍。本文旨在设计一种同时具备高编码效率、可接受错误阈值和硬件可行性的实用量子LDPC码。

Method: 受仿射置换码构造和可重构中性原子阵列的长程连接能力启发，采用结构化码几何设计，将代码布局、原子重排和综合征提取调度协同设计。通过简单并行的原子重排实现非局域综合征测量，完整的综合征提取循环仅需12个纠缠层（与代码尺寸无关）即可并行测量所有X型和Z型校验。

Result: Cornucopia码实现超过1/2的编码率和超过0.4%的伪阈值，阈值与表面码和双变量自行车码相当。代码块[[2844,1426,18]]编码1426个距离18的逻辑量子比特，在0.1%/0.01%物理错误率下外推逻辑错误率为2.6×10^-16/1.9×10^-31。相比之下，双变量自行车码需超过68000个物理量子比特才能达到等效性能，展示了超低开销量子纠错的实用前景。

Conclusion: Cornucopia码证明高编码率、合理错误阈值和硬件可行性可以同时实现，使超低开销量子纠错的演示在近期量子处理器上触手可及，为实用化大规模量子计算提供了关键进展。
...
TL;DR: QuScope 是一个开源 Python 框架，将透射电子显微镜（TEM）的完整成像管线表达为量子电路。电子波函数被振幅编码到量子比特中，所有光学元件（相位光栅透射、菲涅耳传播、像差物镜）均以实现为受量子傅里叶变换共轭的对角酉算子。在经典硬件上进行无噪声态矢量模拟，所有量子结果与经典孪生实现完全一致（单位保真度），并提供了量子资源估计与完整的测试文档。


<details>
  <summary>Details</summary>
Motivation: TEM 成像的物理过程（电子波函数在样品和物镜中的相干演化）天然映射到量子计算的门模型上，但此前缺乏一个完整的、可验证的框架来将 TEM 成像管线表达为量子电路，也不清楚近端量子硬件执行这些算法的资源瓶颈。

Method: 将 N×N 电子波函数振幅编码到 2log2N 个量子比特中；每个光学元件（相位光栅透射、切片间菲涅耳传播、像差物镜）实现为由量子傅里叶变换共轭的对角酉算子；在 QuScope v0.2.0 中实现相位物体近似下的常规 TEM、厚样品的多层切片 CTEM 和 STEM 管线；通过经典孪生实现进行交叉验证，并给出转译后的量子资源估计和对角合成瓶颈分析。

Result: 所有量子结果来自经典硬件上的精确、无噪声态矢量模拟；量子与经典多层切片出射波达到单位保真度一致；所有物理常数均对照标准参考文献验证；提供了每个算法的转译量子资源估计、对角合成瓶颈分析，以及经过全面测试、文档完善且可 pip 安装的软件包。

Conclusion: QuScope v0.2.0 成功验证了将 TEM 成像完整表达为量子电路的可行性，量子与经典结果完全吻合，并揭示了近端硬件执行的主要瓶颈在于对角合成环节，为未来量子计算辅助电子显微成像研究提供了可靠的开源基础。
...
TL;DR: 本文研究简并χ(2)哈密顿量在较大总激发数下的能谱，通过将光谱问题映射到具有双阱势的一维薛定谔方程，推导出势垒顶部附近能级的隐式量子化条件及两个互补光谱区域的显式渐近公式，在常规JWKB近似失效的光谱中心区域提供了精确的解析描述。


<details>
  <summary>Details</summary>
Motivation: 常规JWKB近似在光谱中心区域（势垒顶部附近）失去精度，需要一种新的解析方法来准确描述简并χ(2)哈密顿量在大激发数下的能级结构。

Method: 利用总激发数守恒将谱问题化为有限维三对角矩阵的对角化；遵循Alvarez和Alvarez-Estrada的方法，将问题映射为具有双阱势的有效一维薛定谔方程；推导势垒顶部附近能级的隐式量子化条件，并得到两个适用于互补光谱区域的显式渐近公式；通过与精确矩阵对角化和常规JWKB近似比较验证适用范围和精度。

Result: 建立了所导出的渐近公式的适用范围，并与精确矩阵对角化及常规JWKB近似进行了精度比较。所提出的方法在光谱中心区域提供了精确的能级解析描述，而该区域正是常规JWKB近似失去精度的地方。

Conclusion: 所提出的方法成功填补了常规JWKB近似在光谱中心区域的精度空缺，为简并χ(2)哈密顿量在大激发数下的能谱提供了准确且互补的解析描述。
...
TL;DR: 本文提出了一种量子比特高效的混合量子公式，用于解决电网受控解列这一NP难问题。通过物理信息驱动的紧凑编码和量子比特高效的拉格朗日策略，将QAOA的相位分离器和每层门复杂度从二次降至线性，并在8个IEEE系统（9至89母线）上验证了可行且高质量的解，具有抗噪性和更平滑的成本面。


<details>
  <summary>Details</summary>
Motivation: 受控解列是NP难问题，随电网规模增长求解难度急剧上升。传统QAOA因量子比特容量限制难以扩展到大规模电网解列问题，亟需一种量子比特高效的量子优化方法来突破这一瓶颈，使近期的量子硬件能够处理更实际的电力系统规模。

Method: 提出物理信息引导的紧凑编码方案，利用电网结构在保证可行解空间和优化目标形式不变的前提下压缩状态表示；同时设计量子比特高效的拉格朗日策略，将量子优化与经典精炼相结合以强制执行运行约束；通过复杂度分析证明在稀疏图上固定解列数量时，相位分离器和每层门复杂度从二次降至线性。

Result: 在8个IEEE系统（9至89母线）上、多种量子后端下，在实用电路深度和采样预算内产生可行且高质量的解。因子消融实验表明资源与运行时间收益来自紧凑编码和量子比特高效拉格朗日约束处理的互补效应。噪声分析显示解质量在设备噪声下保持稳定，景观诊断显示QAOA成本面更平滑、尺度更一致。

Conclusion: 该方法为在近端硬件上将受约束量子优化扩展到更大规模的实际应用提供了可迁移的路径，证明量子比特高效的公式化设计是克服量子资源瓶颈、推动量子优化实用化的关键技术方向。
...
TL;DR: 本文提出了PaQit框架，一个面向中性原子量子计算系统的保真度感知量子比特封装方案，通过将设备级Rydberg相互作用物理与系统级调度相结合，联合优化能量消耗、运行时间和计算保真度，并在QuEra的Aquila和Gemini系统上验证了模型准确性。


<details>
  <summary>Details</summary>
Motivation: 中性原子量子计算机虽具有可扩展性优势，但系统级能耗、运行时间与计算保真度之间的关系尚不明确，限制了实际调度决策的制定。

Method: 开发了硬件接地解析模型，刻画能耗和运行时间随量子比特利用率的缩放关系；提出PaQit框架，将保真度目标转化为封装决策，在最大化并行度的同时满足相互作用驱动的串扰约束。

Result: 通过QuEra模拟Aquila、数字Gemini系统及真实硬件执行验证，预测趋势与观测系统行为高度一致。

Conclusion: PaQit能够有效识别最优运行区间，在能耗、运行时间与保真度之间取得平衡，为中性原子系统提供了实用的调度决策工具。
...
TL;DR: 本文研究强化学习中马尔可夫决策过程（MDP）的策略计算问题，提出针对有限时域和无限时域折扣两种MDP类型的新量子算法，用于计算近似最优策略。


<details>
  <summary>Details</summary>
Motivation: 强化学习研究智能体与环境交互以最大化奖励，标准方法是通过MDP选择最优策略。作者旨在改进先前量子算法的查询复杂度，使其更接近已知的量子下界。

Method: 将标准值迭代算法与量子子程序（如量子均值估计和量子最大值查找）相结合，并融入样本最优经典算法的技术。

Result: 所提出的查询复杂度优于先前工作，逼近已建立的量子下界。

Conclusion: 新的量子算法在计算MDP近似最优策略方面比现有方法更高效，接近理论最优。
...
TL;DR: 本文发展了一个预测钻石中量子退相干的理论框架，结合第一性原理电子结构计算、量子多体自旋浴模拟和实验验证，揭示了缺陷种类和自旋浴组分（而非仅缺陷密度）对相干时间的决定性影响，并识别出空位相关缺陷是除传统P1自旋浴外的关键退相干源。


<details>
  <summary>Details</summary>
Motivation: 固态量子技术中缺陷诱导的量子退相干是主要限制因素，但预测真实材料中的退相干在计算上非常困难。传统方法将复杂缺陷群体近似为均匀自旋浴，忽略了缺陷特异性的电子结构和自旋动力学对退相干的贡献。

Method: 提供（1）第一性原理电子结构计算以获取缺陷分辨的自旋哈密顿量；（2）量子多体自旋浴模拟以处理含多种顺磁缺陷物种的异质自旋浴；（3）磁场依赖的Hahn回波实验验证。以金刚石氮-空位色心作为模型平台，研究混合氮、空位和氢相关缺陷环境。

Result: 研究发现退相干不仅取决于缺陷密度，还取决于缺陷身份和浴组成。异质缺陷群体可抑制或增强退相干，产生无法用均匀浴模型解释的趋势。计算结果在广泛的磁场范围内复现了实验观测的相干时间和拉伸指数衰减行为，并识别出空位相关缺陷是传统P1自旋浴之外的关键退相干贡献者。

Conclusion: 该框架将原子尺度缺陷性质与量子相干性联系在一起，为识别隐藏缺陷环境和优化基于缺陷的量子材料的退相干提供了预测性路径。
...
TL;DR: 本文研究经典集体噪声下自旋压缩态的量子增强频率估计问题。作者证明，在信号轴与噪声轴非平行且已知的情况下，通过适当的一轴扭转（one-axis-twisted）输入态，可以将信号编码在与噪声扩散方向度量正交的 quadrature 上，从而在无论噪声时间相关性如何的情况下实现海森堡标度的估计精度。若噪声轴信息不完全，则性能在中等探测数下仍保持海森堡标度，但渐近地退化回集体退相干的已知界限。


<details>
  <summary>Details</summary>
Motivation: 自旋压缩态虽能实现超越标准量子极限的纠缠增强频率估计，但实际应用中集体退相干严重限制了其性能。现有研究大多集中于特定噪声模型（如平行退相干或马尔可夫噪声），缺乏对非平行噪声轴、非马尔可夫噪声等更一般情形下的统一分析框架。作者旨在填补这一理论空白，探讨在经典集体噪声下如何通过态制备策略恢复海森堡标度。

Method: 作者将分析限制在高斯噪声统计，但不假设噪声时间相关性的任何具体形式，从而同时涵盖平行（退相）和单轴横向噪声，以及马尔可夫与非马尔可夫计量学区域。在自旋充分压缩的极限下（Holstein-Primakoff 描述成立），作者利用度量几何方法，证明对已知的非平行信号轴与噪声轴，一轴扭转输入态可将信号编码在与噪声扩散度量正交的方向上，从而实现噪声免疫。测量方案采用单个集体自旋分量。

Result: 主要结果：在信号轴与噪声轴非平行且已知的假设下，无论噪声的时间相关性如何，估计精度均呈现海森堡标度（随探测数N按1/N²或更快衰减），且可通过测量单一集体自旋分量达到最优。对噪声轴的不完全认知会导致从中间探测数的海森堡标度渐近回退到集体退相极限的交叉行为。

Conclusion: 该工作揭示了经典集体噪声下的几何噪声免疫机制，为实验上利用自旋压缩态进行抗噪量子计量提供了理论指导：关键在于使信号与噪声方向形成非平凡的几何分离，而非仅依赖压缩本身。同时指出了噪声轴不确定性的局限性，界定了实际可行的参数估计性能边界。
...
TL;DR: 本文利用量子多参数估计框架，研究了在未知无源线性变换下分离独立热光学源的盲源分离问题，提出了一种基于传感辅助的迭代估计与抑制光学关联的方法，该方法的集体量子测量可渐近达到Holevo Cramér-Rao界，并在弱光区域相较于传统外差层析方法具有显著优势。


<details>
  <summary>Details</summary>
Motivation: 盲源分离（BSS）是一个重要的信号处理问题，即将由未知无源线性混合的独立热光学源分离出来。传统方法通常依赖外差探测重构完整的协方差矩阵，再在经典计算机上进行对角化，但这在弱光区域效率有限。作者希望利用量子多参数估计框架，开发一种直接利用可编程干涉仪进行在传感测量的方法，以避免经典层析的开销并提升弱光下的估计精度。

Method: 作者提出一种传感辅助方法，在可编程干涉仪的单位单元内迭代地估计并抑制光学关联。具体地，（1）将盲源分离转化为量子多参数估计问题； （2）在干涉仪的一个单位单元内构造集体量子测量，渐近实现该子问题的Holevo Cramér-Rao界； （3）通过在光子网状结构中系统地排列多个这样的单元，实现输入多模相关矩阵的原位Jacobi对角化； （4）将该方法与传统异质外差探测（重构完整协方差矩阵后在经典计算机上对角化）进行对比。

Result: 数值或理论分析表明，所构造的集体量子测量能够渐近达到单位单元子问题的Holevo Cramér-Rao界，展现出优于常规探测方法的实质性优势。与传统外差层析（协方差重构+经典对角化）方法相比，该传感辅助方法在弱光区域（低光子数）具有显著优势。

Conclusion: 该工作表明，量子传感辅助的迭代方案能够高效地在可编程光子网络中实现盲源分离，利用集体量子测量在弱光场景下超越经典外差层析方案，并在量子多参数估计框架下提供了理论保障（达到Holevo Cramér-Rao界）。此方法为量子增强的光学信号处理和模式分离开辟了新途径。
...
TL;DR: 本文研究了将稀疏错误检测编码（Iceberg码和Hypercube码）应用于量子计算机上的格点规范理论（特别是格点Schwinger模型）时间演化的可行性，发现大数据块、物理感知后选择以及额外轮次的错误检测能系统性地提高可观测量估计的准确性，表明近端量子模拟中融入最小化容错实现能增强核物理和高能物理模拟性能。


<details>
  <summary>Details</summary>
Motivation: 纠错码在量子模拟中具有潜在应用价值，作者希望探索稀疏错误检测能否提升量子计算机模拟格点规范理论的精度，以应对近端量子设备的噪声限制。

Method: 将轴向规范下的电子-正电子格点嵌入Iceberg码族[[N+2,N,2]]和Hypercube码族[[2^N,N,2]]中（单码块或多码块），使用带噪经典模拟、低频综合征测量和物理感知后选择来评估错误检测对可观测量估计的影响。

Result: 在无连接约束下，大码块更有优势；在现实近端噪声率下，稀疏错误检测能提高可观测量估计精度；额外轮次错误检测能系统性地将可观测量误差驱动到码的噪声下限。

Conclusion: 在近端量子计算中融入最小化容错实现（稀疏错误检测）能增强核物理和高能物理领域的量子模拟性能。
...
TL;DR: 本文研究了利用线性光学网络将可分离光子输入演化为数路径纠缠态并配合光子数分辨探测的干涉相位估计方案。作者推导了任意N光子态在2N个光学模式（分为相位编码块和参考块）下零相位处经典Fisher信息的解析表达式，证明了平衡构型在所有光子分布中对每个光子数N都能最大化相位灵敏度且具有独特的相位无关响应，灵敏度随分布不对称性增加而单调退化，并在光子损耗和分布式多接收器架构下验证了方法的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 量子增强计量学需要在多模分布式架构中实现高精度的相位估计，但现有方案在光子数分辨探测和非局域测量下的Fisher信息缺乏统一解析描述，且光子损耗对灵敏度的影响尚不清楚。作者旨在建立可扩展的分布式多模计量框架，并阐明光子分布对称性、损耗鲁棒性与测量灵敏度之间的基本关系。

Method: 采用线性光学网络将可分离光子输入转化为数路径纠缠态，结合光子数分辨探测。推导了零相位处经典Fisher信息的通用解析表达式，适用于任意N光子分布在2N个模式（分为相位编码和参考两个区块）上的情形。通过比较不同光子分布构型来寻找最优灵敏度配置，并分别在低光子通量下分析真空涨落对局域正交测量（quadrature measurements）的限制，以及非局域光子数分辨测量利用多光子干涉抑制损耗退化的机制。

Result: 平衡光子分布（相位编码块与参考块对称）对所有光子数N均能最大化相位灵敏度，且是唯一具有相位无关响应的构型；灵敏度随光子分布不对称性增加而单调下降。在光子损耗存在时方案仍具鲁棒性，但在低光子通量区，局部正交测量受真空涨落制约，而非局域光子数分辨测量可通过多光子干涉缓解损耗引起的灵敏度退化。分布式多接收器架构下结果同样成立。

Conclusion: 该工作建立了可扩展的量子增强分布式多模计量框架，核心结论是平衡光子分布是最优且唯一相位无关的构型，非局域光子数分辨测量在多接收器与损耗场景下具有显著优势，为大规模量子传感网络的设计提供了理论指导。
...
TL;DR: 本文综述了基于里德伯原子的量子光学领域，重点介绍了如何利用里德伯原子的强长程相互作用实现单光子层面的有效光子-光子相互作用，涵盖单光子工程、光子量子门、无接触非线性光学和量子纠缠等关键进展。


<details>
  <summary>Details</summary>
Motivation: 传统光学非线性效应本质上非常微弱，难以在单光子层面实现有效的光与物质相互作用。里德伯原子凭借其强长程相互作用提供了一种克服这一局限的新途径，因此对该领域进行系统综述具有重要意义。

Method: 本文首先介绍里德伯介导量子光学的基本物理原理，即将里德伯原子间的相互作用映射到光子上以实现有效的光子-光子相互作用，然后系统讨论该领域的关键进展方向。

Result: 该综述涵盖了单光子工程、光子量子门、无接触非线性光学和量子纠缠等多项关键进展，为该快速发展领域提供了全面的现状概述。

Conclusion: 里德伯原子介导的量子光学为在单光子层面实现强非线性提供了可行平台，该领域正快速发展并展现出广阔前景，有望推动量子光学和量子信息处理的发展。
...
TL;DR: 本文提出了一种利用量子发射器产生的单光子实验观测量来计算光子量子纠错中Pauli错误率的方案，并建立了从不完美光子波函数到Pauli错误概率的解析映射关系，为容错融合光子量子计算中光子质量的评估提供了方法。


<details>
  <summary>Details</summary>
Motivation: 在基于光子的量子纠错和融合量子计算中，光子之间的不可区分性、激光噪声和光子损耗等不完美因素会导致Pauli错误，从而影响容错计算的可行性。然而，如何利用实验可测量的观测量来确定这些错误率，并评估量子发射器产生光子的质量是否满足容错要求，仍然是一个关键挑战。

Method: 该方法使用一阶相干测量和一阶交叉关联测量（可通过光子计数实现）来提取存在不可区分性、激光噪声和光子损耗等不完美条件下的单光子及纠缠单光子波函数。在此基础上，将来自单一量子发射器的任意噪声光子纠缠态波函数表示为矩阵乘积态形式，并利用该形式解析地计算相应纠缠态稳定子生成元的期望值，从而进一步获得Pauli错误概率关于光子噪声参数的解析表达式。此外，还计算了纠缠光子经过Bell态测量后的Pauli错误映射。

Result: 获得了Pauli错误概率与光子噪声参数之间的解析表达式，并得到了纠缠光子经Bell态测量后的Pauli错误映射，从而建立了从实验测量数据到量子纠错性能评估的完整桥梁。

Conclusion: 该工作提供了一种利用实验测量来确定量子发射器产生光子所需质量的方法，以支持容错融合光子量子计算。
...
TL;DR: 本文提出一种资源高效的高维编码方法，用单光子操作替代多光子态制备，实现多量子比特图态的生成与Grover搜索算法演示。


<details>
  <summary>Details</summary>
Motivation: 多光子纠缠是光学量子信息处理（尤其通用量子计算）的关键，但多光子源发射效率低，限制了实际应用。

Method: 通过单光子高维扩展、路由和多层量子测量来构建多量子比特态；每个光子以分布式方式编码多个量子比特，并用可编程光子集成线路实现演示。

Result: 在可编程光子集成线路上成功生成并表征了多量子比特图态（GHZ态和团簇态），并利用单光子团簇态演示了Grover搜索算法。

Conclusion: 该工作为光子多种纠缠态生成开辟了新途径，可推动大规模通用光子量子信息处理的发展。
...
TL;DR: 本文提出一种物理信息量子机器学习（PIQML）框架，通过硬约束嵌入和参数位移规则，使参数化量子电路能够在NISQ时代高效求解微分方程，并在多种微分方程上验证了与高精度经典数值基准的高度一致性。


<details>
  <summary>Details</summary>
Motivation: 基于线性系统方法的量子算法求解微分方程需要超出近期量子设备能力的量子比特和精度资源，因此需要在NISQ时代提出更具资源效率的量子求解方案。

Method: 采用参数化量子电路作为机器学习模型，通过傅里叶特征映射将输入变量编码到高维特征空间；设计严格的函数映射器，将初始条件作为硬约束解析地嵌入解构造中以消除关键物理条件的近似误差；利用参数位移规则（量子原生梯度评估技术）计算对输入变量的导数，避免经典离散化；损失函数基于微分方程残差和参考数据，确保模型既逼近数据又内在满足微分方程所表达的物理约束。

Result: 在多种微分方程（包括高度振荡型方程）上进行了验证，实验结果表明该量子模型成功学习了方程解，并与高精度经典数值基准结果高度吻合，展示了其在处理具有挑战性的非线性动力学问题上的能力。

Conclusion: 该PIQML框架通过硬约束嵌入和量子原生梯度计算，为NISQ时代求解微分方程提供了一种资源高效且精确的替代方案，弥补了线性系统量子算法的不足之处。
...
TL;DR: 本文展示自由电子可克服有限亚波长原子链中最亚辐射本征态的光学制备难题：电子速度决定印刻波矢，衍射波包轨迹塑造激发包络，实现近单位条件保真度（F>99.5%）的宣告式制备；此外，路径叠加的自由电子可在两条紧密平行链中激发反对称态，产生比相同原子数的单链更强的亚辐射。


<details>
  <summary>Details</summary>
Motivation: 亚波长原子链中最亚辐射本征态受抑制辐射衰变保护，是光子存储、量子传感和多体量子光学的强大资源，但其光学制备受限于需同时匹配光锥外的波矢和非均匀包络，这两者在传播的自由空间光子中难以同时实现。

Method: 利用自由电子作为量子写入工具：电子的速度设定印刻波矢（突破光锥限制），衍射波包的轨迹塑造激发包络（实现非均匀包络匹配）。通过同步的动量与包络匹配实现宣告式制备；并通过路径叠加的电子激发双平行链中的反对称态以增强亚辐射。

Result: 在深亚波长区域（传播自由空间光子难以进入）实现条件保真度超过99.5%的宣告式制备；在两条平行链中激发反对称态，利用链间相消干涉获得比相同总原子数的单链更强的亚辐射。

Conclusion: 自由电子被确立为集体激发的量子写入器，能够访问传播光学场难以触及的集体激发态，为量子光学和量子信息处理开辟新途径。
...
TL;DR: 本文研究了碳掺杂六方氮化硼（hBN）中可光学寻址的自旋缺陷，发现零场分裂参数D与发射体的零声子线直接相关，而ODMR对比度无此相关性。研究者可控合成了高密度自旋复合物缺陷，并分析了一个具有68% ODMR对比度的窄带缺陷，通过固态浸没透镜将光子收集效率提高了约40%。


<details>
  <summary>Details</summary>
Motivation: hBN中的自旋缺陷适合近表面量子传感，具有明亮的、波长可调的室温单光子发射和高ODMR对比度，但对其微观起源的理解以及确定性制备ODMR活性量子发射体仍然不足，需要推进自旋复合物的实际合成和机理理解。

Method: 在碳掺杂的hBN薄片中可控合成高密度的自旋复合物缺陷；通过测量零场分裂参数D与零声子线的关系进行分析相关性；使用固态浸没透镜（SIL）增强单个窄带缺陷的光子收集效率。

Result: 发现零场分裂参数D与发射体的零声子线直接相关，而ODMR对比度无此相关性；分析了一个窄带缺陷表现出68%的ODMR对比度，并通过固态浸没透镜将光子收集提升约40%。

Conclusion: 该研究同时推进了自旋复合物的实际合成方法和对微观起源的理解，为确定性制备ODMR活性的量子发射体迈出了关键一步。
...
### [26] [Characterizing pairwise swapping capabilities of dense coding channels](https://arxiv.org/abs/2608.03184)
*Abhishek Muhuri,Nirman Ganguly,Aditi Sen De,Indranil Chakrabarty*

Main category: quant-ph

TL;DR: 本文提出了一种新的多体纠缠辅助经典通信任务——稠密编码交换（dense coding swapping），合法方通过联合酉操作将一个通信信道的稠密编码能力交换到另一个信道上，并给出了支持该任务的三量子比特纯态与混合态的判定条件，同时分析了最优酉算子、噪声鲁棒性及纠缠需求。


<details>
  <summary>Details</summary>
Motivation: 稠密编码是量子通信中的重要协议，但多用户网络中的稠密编码能力分配受到DC排除原理的约束。如何在多体网络中灵活地迁移或交换稠密编码能力，以及当某一接收方被攻破时如何防护，是尚未充分探索的问题。作者旨在引入一种可交换稠密编码能力的任务，并建立完整的理论框架。

Method: （1）定义多体纠缠辅助的稠密编码交换任务，明确合法方通过联合酉操作在不同通信信道间交换稠密编码能力；（2）利用Schmidt系数推导三量子比特纯态支持DC交换的充要条件；（3）利用Bloch关联参数给出混合态支持DC交换的充分条件；（4）识别实现稠密编码能力交换的最优两量子比特酉算子；（5）分别在有色噪声和白噪声下分析候选态的容忍度；（6）分析支持DC交换的多体态所需的最小真多体纠缠量及其随系统规模的缩放关系。

Result: （1）严格推导了三量子比特纯态支持DC交换的充要条件及其Schmidt系数表达式；（2）给出了混合态的充分判据；（3）找到了可实现交换的最优两量子比特酉算符；（4）验证了该方案在有色噪声和白噪声下均具有鲁棒性；（5）揭示了支持DC交换的多体态仅需少量真多体纠缠，且该需求随系统规模增大而降低。

Conclusion: 稠密编码交换作为一种新任务，不仅有别于传统的资源交换（可被视为一种过程交换），还可在接收方被攻破时提供预防措施。该工作建立了完整的理论基础，展示了多体纠缠在量子网络通信中的新应用前景。
...
### [27] [Factorization of Exclusive-Sum-Of-Products Expressions with Rectangle Covering to Reduce Quantum Circuit Cost](https://arxiv.org/abs/2608.03188)
*Audrey Hou,Lucia Zhang,Ali Al-Bayaty,Marek Perkowski*

Main category: quant-ph

TL;DR: 该论文提出两种ESOP表达式因子化算法（Disjoint和Even-Odd矩形覆盖法），通过将每个乘积项转换为2D矩阵中的单元来优化量子电路成本，实验表明可降低20%-95%的字面量数和Maslov成本。


<details>
  <summary>Details</summary>
Motivation: 量子电路的实现成本高昂，尤其是大型Toffoli门的使用。随着表达式复杂度增加，需要通过因子化来优化电路成本，以降低量子电路的总开销。

Method: 将ESOP表达式中的每个乘积项转换为2D矩阵中的单元，并利用Disjoint和Even-Odd矩形覆盖两种算法来寻找最优的因子化AND/EXOR解决方案。使用两个Python程序实现算法，并在公开基准上进行测试评估。

Result: 两种算法在经典逻辑电路的字面量数以及量子电路的Maslov成本上均实现了20%-95%的降低，具体改善幅度取决于表达式的大小。

Conclusion: 提出的ESOP表达式因子化方法能够有效降低逻辑电路和量子电路的实现成本，是一种有前景的量子电路优化手段。
...
### [28] [Analysis of Superradiance-Based Quantum Metrology under Independent Markovian Pure Dephasing](https://arxiv.org/abs/2608.03262)
*Yasunari Sakuma,Yuichiro Matsuzaki,Hoi-Kwan Lau,Junko Ishi-Hayase*

Main category: quant-ph

TL;DR: 本文研究了基于N自旋系综超辐射的直流磁强计协议在独立马尔可夫纯退相干下的鲁棒性。研究发现，在大N极限下，估计误差仅增加一个常数因子，与GHZ态磁力计误差增加√N因子形成鲜明对比，表明超辐射磁力计对纯退相干具有高度鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 量子计量学通常容易受到独立马尔可夫纯退相干的影响，例如GHZ态磁力计的精度标度会从O(1/N)恶化到O(1/√N)。虽然超辐射磁力计很可能也会受到纯退相干的影响，但其定量影响尚不清楚，需要系统研究。

Method: 通过数值模拟和平均场分析相结合的方法，定量研究独立马尔可夫纯退相干对基于N自旋系综超辐射直流磁力计协议的影响。

Result: 证明在大N极限下，纯退相干导致的估计误差增加仅限制在一个常数因子内，与GHZ态传感中误差增加√N因子形成鲜明对比，并通过解析解定性阐释了这一鲁棒性的物理起源。

Conclusion: 该研究确立了对独立马尔可夫纯退相干的高鲁棒性是超辐射直流磁力计的重要优势，为实际量子磁力计应用提供了理论依据。
...
### [29] [Single-molecule strong optomechanical regimes in SERS via hybrid plasmonic cavities](https://arxiv.org/abs/2608.03307)
*Miguel Á. Martínez-García,Johannes Feist,Diego Martín-Cano*

Main category: quant-ph

TL;DR: 该论文发现混合金属-介电腔体与共振荧光分子结合可增强强相干表面增强拉曼散射（SERS）相互作用。通过窄带混合等离激元模式照射近饱和的失谐电子跃迁，在远低于不可逆SERS损伤阈值的强度下观察到斯托克斯和反斯托克斯光谱中的增强劈裂，揭示了强光机械耦合和非线性振动修正。通过计算二阶光子相关性识别非经典特征，并利用SERS构型光学表征振动模式的非谐性。


<details>
  <summary>Details</summary>
Motivation: 探索在低强度下实现强相干SERS相互作用，避免SERS损伤阈值限制，并理解强光机械耦合与非线性振动行为及其非经典量子特征。

Method: 采用混合金属-介电腔体（hybridized metallo-dielectric cavities）与共振荧光分子结合，利用窄带混合等离激元模式照射失谐电子跃迁（近饱和状态），测量斯托克斯/反斯托克斯光谱劈裂，并计算二阶光子相关性（g(2)）以表征非经典性质和振动模式非谐性。

Result: 观察到增强的斯托克斯和反斯托克斯光谱劈裂，表明强光机械耦合和非线性振动修正可在远低于SERS损伤阈值的强度下实现；二阶光子相关计算揭示了强光机械相互作用中的非经典特征，并验证了可用SERS构型光学表征振动模式非谐性。

Conclusion: 混合金属-介电腔体与荧光分子的强耦合体系为实现低强度下的强相干SERS提供了新途径，同时为量子光学表征振动非谐性及强光机械耦合提供了可行的实验方案。
...
### [30] [Harvest: Resource-Aware Quantum Compilation for Magic State Protocols](https://arxiv.org/abs/2608.03315)
*Jannik Pflieger,Aleksandra Świerkowska,Emmanouil Giortamis,Pramod Bhatotia*

Main category: quant-ph

TL;DR: Harvest是一个针对晶格手术（lattice surgery）的资源感知编译方法，在协议无关的资源模型下协同优化魔法态消耗、电路感知的布局和拥塞感知的路由，并在调度后回收未使用的布局空间，在多个基准测试上实现平均4.83倍加速。


<details>
  <summary>Details</summary>
Motivation: 容错量子处理器的编译优化中，指令调度、布局、路由和魔法态供应无法孤立优化，因为无电路级依赖的操作在放置后仍可能争用相同的端口、路由或魔法态终端。现有编译器要么将调度与魔法态生成解耦，要么硬编码单一生成协议，导致在执行时间和布局占用之间被迫权衡，无法跨协议协同优化。

Method: Harvest采用资源感知的编译方法，在协议无关的资源模型下协同优化魔法态消耗、电路感知的布局（circuit-aware placement）和拥塞感知的路由（congestion-aware routing），并在调度完成后回收未使用的布局空间，包括未使用的魔法态补丁和路由补丁。

Result: 在QAOA、QFT、QASMBench等标准基准套件上，相对于顺序执行，Harvest实现了平均4.83倍（最高17.8倍）的加速；通过电路感知布局将调度长度提升最多1.35倍；回收了最多72.0%的未使用魔法态补丁和33.9%的未使用路由补丁。

Conclusion: Harvest证明将魔法态生成、布局和路由放在统一的资源模型下协同优化，可以在不牺牲额外布局面积的情况下显著提升晶格手术编译的执行效率，同时通过空间回收机制减少资源浪费。
...
### [31] [Protected measurements for protected superconducting qubits](https://arxiv.org/abs/2608.03325)
*Xanda C Kolesnikow,Thomas B Smith,Andrew C Doherty*

Main category: quant-ph

TL;DR: 本文提出了两种正交基下对$0$-$\pi$量子比特进行受保护测量的具体方案，该方案利用量子非破坏测量的特性来容忍辅助测量量子比特的故障，为容错通用控制提供了关键技术。


<details>
  <summary>Details</summary>
Motivation: 受保护超导量子比特（如$0$-$\pi$量子比特）能大幅抑制错误率，但因其受保护特性导致测量困难，此前尚无在不破坏其保护性的前提下进行测量的具体方案。

Method: 作者展示了如何在两个正交基下对$0$-$\pi$量子比特执行受保护的测量。方案的核心在于利用量子非破坏（QND）测量性质，使得辅助测量量子比特上的故障能够被容忍，从而在测量过程中维持主量子比特的保护性。

Result: 成功实现了$0$-$\pi$量子比特在两个正交基下的受保护测量方案，且测量的保护性由量子非破坏性质保障，辅助量子比特的故障可被容忍。

Conclusion: 随着实验将受保护量子比特推向低错误率区域，该测量技术将对容错通用控制至关重要，为受保护量子比特的实用化测量奠定了基础。
...
### [32] [The geometry of absolute separability and other convex matrix properties from spectrum](https://arxiv.org/abs/2608.03390)
*Jennifer Ahiable,Naga Bhavya Teja Kothakonda,Andreas Winter*

Main category: quant-ph

TL;DR: 本文研究了双部分绝对可分态（ASEP）和绝对正部分转置态（APPT）的谱集合几何结构，证明了APPT为谱面体（spectrahedron）且所有面都是暴露的，而ASEP为半代数集；给出了APPT面的完整特征描述，并通过内接多胞体对最大纯度、最小熵以及相对谱体积提供了严格的上下界。


<details>
  <summary>Details</summary>
Motivation: 理解哪些双部分量子态在任意全局酉变换下仍保持可分性（绝对可分）或正部分转置性（APPT）是量子信息论中的基本问题。这些集合的几何结构，特别是其谱集合的边界、面和极值点，对于量子态的纯度极限、纠缠资源以及量子信息处理具有重要意义。然而，ASEP和APPT集合的精确几何结构此前尚未被系统刻画。

Method: 论文首先建立了绝对凸矩阵集合的一般几何性质理论；然后将绝对PPT判据重构为置换对称形式，利用该重构证明APPT是谱面体。通过分析谱面体的面和极值点，给出了APPT面的完整特征，并指出面的维数由某矩阵的秩决定。在定量方面，利用内接多胞体P_{m,n}对最大纯度给出下界，对最小von Neumann熵给出上界，并通过数值验证和渐近分析建立了相对谱体积的指数衰减律。

Result: 主要结果包括：(1) APPT_{m,n}对所有m≤n都是谱面体，且所有面均为暴露面，最大真面的维数为(mn-m-1)；(2) ASEP_{2,n}是谱面体，但一般情形下ASEP_{m,n}是半代数集；(3) 完整刻画了APPT_{m,n}的面与极值点；(4) 给出了APPT最大纯度的严格下界和最小von Neumann熵的严格上界，并提出纯度与内接多胞体重合的猜想；(5) 证明了APPT_{m,n}的相对谱体积随n呈指数衰减，衰减常数为内接多胞体相对体积的常数倍。

Conclusion: 该工作系统揭示了APPT与ASEP谱集合的几何本质差异（谱面体vs半代数集），为绝对可分态与APPT态的量子资源理论提供了严格的几何框架和定量边界，并对最大纯度、最小熵和谱体积等核心量子信息量给出了可靠估计。
...
### [33] [On-chip Quantum Measurement of Squeezing Generated from a Silicon Nitride Micro-ring Resonator](https://arxiv.org/abs/2608.03402)
*Yuhang Lei,Chenfei Cui,Yue Li,Hon Ki Tsang,Z. Y. Ou*

Main category: quant-ph

TL;DR: 本文提出了一种片上量子测量的方法，通过在硅氮化物（SiN）微环谐振器芯片上集成高增益参量放大器来测量压缩光，克服了芯片到光纤耦合损耗对量子关联破坏的问题。实验直接观察到4.6 dB的量子噪声抑制，并首次实现了单片SU(1,1)干涉仪，相比传统线性干涉仪信噪比提升约5 dB。


<details>
  <summary>Details</summary>
Motivation: 片上集成量子光学技术是大规模量子应用的关键，但芯片生成的压缩态光在耦合到片外探测器时会产生显著的耦合损耗，破坏微妙的量子关联，限制了可观测的压缩量。实际应用中需要解决片上生成与片外测量之间的损耗问题。

Method: 提出片上量子测量的方案：在硅氮化物芯片上依次构建两个匹配的SiN微环谐振器。第一个微环生成压缩态光，第二个微环作为高增益参量放大器（PA），在光经历显著片外损耗之前对压缩态进行测量。该架构具有天然的损耗容忍性——放大器将量子噪声提升到远高于真空噪声的水平，使测量对下游损耗不敏感。

Result: 在芯片到光纤耦合损耗超过5 dB的条件下，直接观察到来自第一个微环的4.6 dB量子噪声抑制。同时演示了首个单片集成的SU(1,1)干涉仪，相比传统线性干涉仪实现约5 dB的信噪比增强。

Conclusion: 该工作通过片上参量放大测量方案解决了压缩态的耦合损耗问题，建立了片上集成量子传感的实用化路径，为芯片级量子传感器和量子计算的大规模应用提供了可行的技术方案。
...
### [34] [Mesoscopic Quantum Communication via Photon-Number Moments](https://arxiv.org/abs/2608.03418)
*Gabriele Cenedese,Alex Pozzoli,Luca Razzoli,Alessia Allevi*

Main category: quant-ph

TL;DR: 本文提出一种基于介观光学态的量子通信协议，将信息编码在经典光态光子数分布的一阶和二阶矩中，由光子数分辨探测器解码。安全性源于与信号同时在量子信道传输的孪生光束（twin-beam）的非经典光子数关联，其为针对截断重发和分束攻击提供了可实验检测的安全见证。数值模拟表明在实验可达参数区域内协议可行且安全，可产生非零密钥率。


<details>
  <summary>Details</summary>
Motivation: 介观光学态兼具抗损耗鲁棒性和真实量子特征保留，是量子通信的有前景资源。作者希望利用这一优势设计一种能在实际信道条件下保持安全性的量子通信协议。

Method: 将信息编码于经典光态光子数分布的第一阶矩（均值）和第二阶矩（方差），通过光子数分辨探测器进行解码；利用与信号同步传输的孪生光束的非经典光子数关联作为安全见证，检测截断重发和分束两类窃听攻击。

Result: 数值模拟在实验可达参数范围内验证了协议的安全性和可行性，在所考虑的两种窃听攻击下均能获得非零密钥生成率。

Conclusion: 该协议有望在未来实验中得到验证和实施，为基于介观光学态的实用化量子通信提供了新的方案。
...
### [35] [Suppressing Cavity Frequency Noise Using a Kerr Nonlinearity](https://arxiv.org/abs/2608.03440)
*J. P. van Soest,S. Meilof,G. L. Bhai,M. Villiers,C. A. Potts,G. A. Steele*

Main category: quant-ph

TL;DR: 本文演示了一种通过强泵浦音锁定非线性超导微波腔来抑制共振频率波动的方法。利用系统的Kerr非线性产生内在反馈机制，无需主动外部反馈即可实现被动稳定，将频率波动降低近两个数量级至1/f噪声底。


<details>
  <summary>Details</summary>
Motivation: 超导微波腔的共振频率波动会限制其在量子比特读取、光机械位移传感和磁通检测等应用中的灵敏度与稳定性，亟需一种有效的频率稳定方案。

Method: 将非线性的噪声超导微波腔锁定到强泵浦音上，利用Kerr非线性（共振频率依赖于腔内场幅度）形成固有反馈机制，实现无源自稳定。通过双音谱学表征该非线性反馈机制，并使用Allan方差分析研究其时间稳定性。

Result: 锁定后的腔模频率波动减少近两个数量级，达到1/f噪声底，且无需连续频率跟踪或主动控制。Kerr锁定在抑制低频腔噪声的同时保持了锁定带宽外信号的灵敏度。

Conclusion: Kerr锁定为非线性微波谐振器的自稳定提供了一种通用方法，可广泛应用于基于SQUID的谐振器、光机械器件和参量放大器等系统。
...
### [36] [Matrix Product Evolution: A Method for Simulating Quantum Circuits Using Tensor Networks](https://arxiv.org/abs/2608.03472)
*Haruyuki Kawabe,Minoru Nagai,Tsuyoshi Okubo,Synge Todo*

Main category: quant-ph

TL;DR: 本文提出Matrix Product Evolution (MPE)，一种沿电路深度而非量子比特索引构建的张量网络表示方法，用于量子电路的经典模拟。通过zip-up过程实现高效张量收缩，数值实验表征了时间维键维数的增长、后选择对收缩成本与精度的影响，并识别了其作为标准MPS模拟补充的适用场景。


<details>
  <summary>Details</summary>
Motivation: 量子电路的经典模拟受限于希尔伯特空间的指数增长和量子态的纠缠结构。标准MPS方法沿量子比特维度进行张量分解，但在某些场景（如高纠缠、深电路）下效率受限。需要探索沿电路深度方向构建张量网络表示的替代模拟框架。

Method: 1. 引入MPE，一种沿电路深度而非量子比特索引构造的张量-训练（tensor-train）表示。2. 将量子电路模拟建模为多个MPE张量的收缩。3. 基于zip-up过程开发高效收缩策略。4. 通过随机量子电路模拟和量子多体态时间演化进行数值验证，表征时间维键维数增长、后选择效应以及收缩成本与精度。

Result: 表征了时间维键维数的增长规律；阐明了后选择如何影响收缩成本和近似精度；识别了深度导向张量网络收缩相对于标准MPS模拟具有补充优势的参数区域。

Conclusion: MPE提供了一种沿时间（电路深度）方向的张量网络模拟范式，在特定场景下能有效补充标准MPS方法，为量子电路的经典模拟提供了新的视角和实用工具。
...
### [37] [Gravitational redshift as a quantum channel: modeling the effects of gravitational redshift in quantum optics](https://arxiv.org/abs/2608.03549)
*Thomas Mieling,Andreas Wolfgang Schell,David Edward Bruschi*

Main category: quant-ph

TL;DR: 本文解决了量子理论中引力红移的多模混频器(MMM)模型不一致性的根本原因，并提供了两种互补的修正方法：一种从场论视角出发的"自然"方法，另一种适用于有限维系统量子力学语言的方法。后者可在限制输入为高斯光态的情况下，将多模传输设置中的红移建模为量子信道，并使用标准量子信息论技术进行表征。


<details>
  <summary>Details</summary>
Motivation: 光的引力频率红移在经典电磁理论中已被充分理解，但其量子理论描述尚未完全发展。先前工作指出了现有红移多模混频器模型的矛盾之处，但未获得完整解决方案，因此需要识别问题根源并提供一致的修正方法。

Method: 作者首先识别了MMM模型不一致性的根本原因，然后提出两种互补修正方法：一是从场论视角的"自然"方法，二是适配有限维系统量子力学语言的另一方法。后者将红移建模为多模传输设置中的量子信道，并利用标准量子信息论技术进行表征，尤其针对高斯光态输入进行限制。

Result: 作者成功找到MMM模型不一致的根源，提供了两种互补的修正途径，并证明第二种方法能够将引力红移建模为量子信道，在输入为高斯光态时可用标准量子信息论技术进行表征。

Conclusion: 该工作完善了量子理论中引力红移的描述，通过量子信道框架为多模传输中的红移效应提供了可操作的表征方法，填补了先前模型存在的不一致性问题。
...
### [38] [Observation of quantum nonclassicality without freedom of choice in a minimal causal network](https://arxiv.org/abs/2608.03552)
*Ya Xiao,Yan-Xin Rong,Ran He,Yu Meng,Yang Zhang,Xiao-Ye Xu,Yong-Jian Han,Zheng-Hao Liu,Yong-Jian Gu*

Main category: quant-ph

TL;DR: 本文在光学实验中实现了最小三节点不相关混杂因子(UC)量子因果网络，利用两个独立的偏振纠缠光子源和中央节点的高保真纠缠测量，结合观察性与干预性数据的因果融合协议，以超过三个标准差违反了相应的混合因果不等式，验证了无需自由选择和类空间隔假设的量子非经典性。


<details>
  <summary>Details</summary>
Motivation: 传统贝尔非局域性检验依赖于一些物理上不太合理的假设（如自由选择和类空间隔）。作者希望利用不相关混杂因子因果网络这一更简单、更稳健的路径来认证量子非经典性，同时放宽这些不必要的假设，为设备无关量子协议提供更易实现的实验平台。

Method: 在光学平台上实现最小三节点UC网络：使用两个独立的偏振纠缠光子源构建网络结构，在中央节点执行高保真纠缠测量以实现干预；提出因果数据融合协议，将观察性数据与干预性数据相结合，提升噪声容限；通过违反混合因果不等式来检验量子非经典性。

Result: 实验成功实现了UC量子因果网络，因果数据融合协议显著提高了协议的噪声容限，并观测到混合因果不等式被违反超过三个标准差。

Conclusion: 实验结果深化了对网络中量子非局域性的理解，同时凸显了UC网络作为一种紧凑、实验上易获取的平台，可用于不需要主动选择测量设置的设备无关量子协议。
...
### [39] [Generation and Enhancement of Bipartite and Tripartite Entanglement in an Electro-Optomechanical Ring Cavity](https://arxiv.org/abs/2608.03578)
*Fouad Essaadi,Yassine Oussarhan,Mohamed Ouhammou,Said Mouslih,Mohamed Jakha,Bouzid Manaut,Souad Taj*

Main category: quant-ph

TL;DR: 本文研究了电光机械环形腔系统中量子纠缠的产生与增强机制，利用两个库仑耦合的机械谐振器产生二分与三分纠缠，并通过光参量放大器（OPA）注入可控非线性来显著增强纠缠。研究基于协方差矩阵形式量化纠缠度，发现纠缠强度对多个参数高度敏感，存在最大化纠缠与稳定运行区间之间的权衡，并揭示了低温运行的必要性。


<details>
  <summary>Details</summary>
Motivation: 探索如何在混合量子系统中有效地产生并增强多体量子纠缠，为量子技术提供可控的多体纠缠平台。特别是通过非线性参量驱动来克服传统方案中纠缠强度有限的问题。

Method: 构建电光机械环形腔系统，集成两个库仑耦合的机械谐振器。推导系统哈密顿量与量子朗之万方程，在稳态解附近线性化以分析高斯量子涨落。采用协方差矩阵形式，用对数负度量化二分纠缠，用最小残余共纠缠度量化三分纠缠。

Result: 结果表明：库仑相互作用是产生纠缠不可或缺的条件，而OPA作为强控制工具能大幅放大所有子系统的量子关联。纠缠强度对OPA增益与相位、激光失谐和输入功率等参数高度敏感，可通过策略性参数选择优化。存在关键权衡：最大化纠缠的参数同时会约束系统的稳定运行区域。热噪声会逐步退化所有量子关联，需低温运行。

Conclusion: 该研究为参数优化提供了全面指导，描绘了从纠缠产生到增强的清晰路径，展示了此类混合系统作为量子技术中可控多体纠缠通用平台的潜力。
...
### [40] [Sample-half-inserted quantum interferometer](https://arxiv.org/abs/2608.03622)
...
TL;DR: 本文提出并验证了一种半样本插入的Hong-Ou-Mandel（SHOM）干涉仪，通过引入不对称光子-样本相互作用，在单次干涉事件中将Fisher信息提升了五个数量级，实现了4.09 nm（13.63 as）的平均测量精度和1.22 nm（4.07 as）的平均准确度，建立了高效的相位不敏感干涉测量方法。


<details>
  <summary>Details</summary>
Motivation: 传统HOM干涉仪虽然能实现阿秒级时间分辨率，但其每次试验的Fisher信息较低，通常需要数万次重复才能达到如此精度，严重限制了实际测量的效率。

Method: 提出样本半插入HOM（SHOM）干涉仪结构，通过光子与样本的不对称相互作用产生独特的"凹陷-凸起-凹陷"（dip-bump-dip）干涉结构，将传统上被视为伪影的特征转化为有用的计量资源，从而在单次干涉事件中显著增强Fisher信息。

Result: 实验上使用O(10^7)个光子实现了4.09 nm（13.63 as）的平均精度和1.22 nm（4.07 as）的平均准确度的光学路径差测量，Fisher信息提升了五个数量级。

Conclusion: SHOM干涉法是一种高效的相位不敏感测量方法，不仅为透明材料的实用化量子增强厚度测量铺平了道路，还为提升各类量子器件的性能提供了优雅的策略。
...
### [41] [Entangling Topological Invariants](https://arxiv.org/abs/2608.03634)
*Kazuki Ikeda,Yaron Oz*

Main category: quant-ph

TL;DR: 本文研究了孤立占据多重态在无全局子系统结构下是否存在局部张量积描述的问题，通过比较占据多重态的过渡函数与两个候选子系统中独立基变换生成的过渡函数来刻画阻碍。在指定闭曲面上的秩一扇区分解时，商结构去除行/列可加Chern数据并得到混合Chern类；扇区标签的动量依赖混合引入运动线的高斯-科达齐曲率，而在标签守恒极限下混合类由交叉Thouless泵浦度量。当仅指定因子维数p和q时，通过S4上秩pq丛的clutching映射进行比较，全局分解可行当且仅当C2可被gcd(p,q)整除；特别地，奇数C2阻碍2×2分解。文中用八能级哈密顿量展示两种设置，并给出泵浦和占据投影层析协议进行读出。


<details>
  <summary>Details</summary>
Motivation: 在量子多体物理和拓扑能带理论中，占据多重态（occupied multiplet）是否能在局部区域分解为张量积形式（即子系统结构），以及这种分解能否全局一致地成立，是一个基础而重要的问题。然而目前缺乏对该问题的系统性刻画，特别是当系统不存在全局的子系统结构时，如何定量描述分解受阻的拓扑障碍。

Method: 本文采用两种互补的数学框架：(1) 在指定闭曲面上的秩一扇区分解设置中，通过比较过渡函数与独立基变换生成的过渡函数构造商结构，提取混合Chern类；同时引入Gauss-Codazzi曲率刻画动量依赖的扇区标签混合，并在标签守恒极限下用交叉Thouless泵浦度量混合类。(2) 在仅指定因子维数p和q的设置中，通过S4上秩pq丛的clutching映射计算product frame的缠绕数，建立C2可整除性与全局因子分解可行性的等价条件。

Result: 核心结果为：在扇区分解设置下，商结构消除行/列可加Chern数据后得到混合Chern类；在仅指定维数的设置下，全局因子分解可行的充要条件为第二Chern数C2被gcd(p,q)整除，且奇数C2必然阻碍2×2因子分解。文中通过有限八能级哈密顿量对两种场景进行了具体演示。

Conclusion: 本文建立了占据多重态张量积分解的完整拓扑障碍理论，将全局因子分解的可行性精确定位到Chern数的可整除性条件，并通过明确的哈密顿量模型和可操作的实验协议验证了理论预测。
...
### [42] [Iterative linear quadratic regulator on SU(N) for multi-qubit gate synthesis](https://arxiv.org/abs/2608.03656)
*Dirk Heimann,Felix Wiebe,Elie Mounzer,Shivesh Kumar*

Main category: quant-ph

TL;DR: 本文将迭代线性二次调节器(iLQR)推广到特殊酉群SU(N)的Lie群结构上，用于量子门合成，并与标准欧几里得iLQR在2到5量子比特门上进行系统比较。在无约束理想条件下，Lie群公式收敛更快；在受2-局域哈密顿量约束时，Lie群变体在早期迭代中收敛更快，但对初始化和局部极小值更敏感。


<details>
  <summary>Details</summary>
Motivation: 量子最优控制中的梯度下降轨迹优化方法在设计多量子比特量子门时已被证明是灵活有效的，但融入底层Lie群结构可以进一步加速优化过程。作者希望通过将iLQR适应到SU(N)群结构上，系统地评估Lie群几何建模相对于标准欧几里得公式的收敛优势。

Method: 将Lie群公式iLQR扩展到特殊酉群SU(N)，应用于量子门合成任务；在多个2至5量子比特门上进行系统比较，分别考察无约束（所有Lie代数基元可作为驱动哈密顿量项）和约束（仅2-局域哈密顿量项）两种设置下的收敛性能。

Result: 无约束理想化设置下，Lie群公式比欧几里得iLQR收敛更快。在约束为2-局域哈密顿量项时，Lie群变体在早期优化迭代中收敛更快，但对初始化表现出更大敏感性，且更倾向于陷入局部极小值。

Conclusion: 将Lie群几何融入iLQR能显著改善量子门合成的收敛性能，但在受约束控制设置下仍存在改进空间，这对后续研究指明了方向。
...
### [43] [Bimodal non-Gaussian photonic states from a single quantum emitter in a waveguide](https://arxiv.org/abs/2608.03658)
*Thomas Copie,Ofer Firstenberg,G. P. Teja,Radim Filip,Hanna Le Jeannic*

Main category: quant-ph

TL;DR: 本文研究了利用手性波导中单个二能级系统确定性生成和宣告式生成光的非高斯态。相干脉冲输入可确定性生成Wigner负态但秩有限；压缩真空输入可产生双模非高斯态，从中能以较高成功率提取高阶恒星秩态（包括大压缩猫态）。文中还分析了非理想耦合、对称波导50%收集极限的影响，并提出一种恢复有效手性相互作用的干涉方案。


<details>
  <summary>Details</summary>
Motivation: 非高斯光态是连续变量量子信息处理的关键资源，但现有生成方法通常效率低或概率性。利用手性波导中单量子发射体的确定性相互作用，探索高效生成高阶非高斯态（特别是Wigner负态和高恒星秩态）的可行途径。

Method: 理论分析单个二能级系统在手性波导中分别受相干脉冲和压缩真空驱动的过程。对比两种输入下的输出态结构：相干输入产生确定性Wigner负态（秩受限）；压缩真空输入产生双模非高斯态，可通过条件测量提取高阶恒星秩态。同时分析非理想耦合、对称波导的50%收集损耗，并提出干涉方案将双向波导转换为有效手性相互作用。

Result: 相干脉冲输入可确定性产生Wigner负态但状态秩有限；压缩真空输入能产生双模非高斯态，从中可提取包括大压缩猫态在内的高恒星秩态，且成功率可观。手性耦合优于对称波导，但可通过干涉方案克服双向波导的50%收集限制。

Conclusion: 该方案为实验上生成高阶非高斯光态提供了可行路径，手性波导平台结合压缩输入和条件测量可高效产生压缩猫态等高阶量子资源，干涉方案降低了对手性波导的硬件要求。
...
### [44] [DAMPyF: a Python implementation of the DAMPF method for the simulation of open-system dynamics](https://arxiv.org/abs/2608.03668)
*Nicola Lorenzoni,Susana F. Huelga,Martin B. Plenio*

Main category: quant-ph

TL;DR: 本文介绍了DAMPyF，一个开源的Python实现，用于耗散辅助矩阵乘积分解（DAMPF）方法，该方法基于张量网络实现对耦合到玻色子环境的有限维量子系统的数值精确模拟，提供激发能量转移动力学和分子光谱计算两种工作流程。


<details>
  <summary>Details</summary>
Motivation: 精确模拟有限维量子系统与玻色子环境的耦合是量子动力学中的重要问题，需要开发高效的数值方法及其开源软件实现，以支持量子物理和分子光谱等领域的研究。

Method: 该方法利用结构化储库的赝模表示和扩展系统（系统加赝模）密度矩阵的矩阵乘积态表示。DAMPyF提供两种工作流程：一是在单系统激发子空间内传播系统激发以实现激发能量转移动力学；二是面向分子光谱学的高层工作流程，其中系统能级代表电子态，传播光学相干性以计算线性光谱（包括吸收光谱和圆二色谱）。

Result: DAMPyF作为开源Python包提供DAMPF算法的实现，包含物理模型、算法、用户代码结构、安装与执行指南、输入输出格式及最小示例。

Conclusion: DAMPyF为量子开放系统动力学和分子光谱模拟提供了一个可访问、开源的数值精确工具，扩展了DAMPF方法在量子动力学和光谱计算中的应用范围。
...
### [45] [Fidelity-Based Robustness Margins for Finite-Time Quantum Control](https://arxiv.org/abs/2608.03698)
*S. P. O'Neil,F. C. Langbein,C. A. Weidner,E. A. Jonckheere,S. Schirmer*

Main category: quant-ph

TL;DR: 本文针对有限维封闭量子系统在分段常数相干控制下，提出了一种结构特定的保真度阈值鲁棒性余量。通过微分灵敏度界得到阈值依赖的Lipschitz常数，进而获得可认证的有限扰动半径，并以此为迭代步长开发了一维搜索方法。三量子比特门控示例表明该有限余量在不同控制器间可相差三倍，包含名义微分灵敏度无法捕捉的结构依赖信息。


<details>
  <summary>Details</summary>
Motivation: 量子控制在实际应用中不可避免地受到物理参数扰动的干扰，现有的鲁棒性分析通常仅基于名义微分灵敏度（局域信息），无法为保真度阈值提供可认证的有限扰动范围。需要一个结构特定的、可认证的鲁棒性余量来量化控制器在实际扰动下的可靠性。

Method: 针对分段常数相干控制的有限维封闭量子系统，基于trace-amplitude守真度建立微分灵敏度界，推导出阈值依赖的Lipschitz常数，定义连通安全参数分量上的可认证有限扰动半径；通过最近化该证书，构建迭代一维方法，在朝向保真度阈值边界的方向上以认证安全步长前进。

Result: 三量子比特门控示例表明，结构特定的有限扰动余量在名义保真度相近的不同控制器之间可相差高达三倍，且这些余量包含名义微分灵敏度无法捕获的结构依赖信息，验证了该方法的区分能力与实用性。

Conclusion: 该方法为量子控制器的鲁棒性提供了一种超越局域灵敏度的、可认证的有限扰动余量，并能支持向阈值边界的迭代搜索，为量子门设计的鲁棒性评估和优化提供了新的结构感知工具。
...
### [46] [Inverse Design of Quantum Control Sequences with Fourier Neural Operators](https://arxiv.org/abs/2608.03702)
*Anastasia Pipi,Valentin Duruisseaux,Emily Been,Xuecheng Tao,Taylor L. Patti,Anima Anandkumar,Prineha Narang*

Main category: quant-ph

TL;DR: 本文提出基于傅里叶神经算子（FNO）的框架，用于学习高维分子量子动力学并加速控制协议的逆设计。FNO预测分子运动种群动力学的速度比GPU加速的数值传播快10^7倍，并基于此开发了FNO随机脉冲测量规划器（FNO-SPMP），在水合氢离子的888维子空间中实现目标态布居0.98，序列成功率达86.2%，性能优于强化学习基线。


<details>
  <summary>Details</summary>
Motivation: 量子最优控制是操纵量子动力学的关键工具，但其计算成本随希尔伯特空间维度的增长而急剧增加。对于高维量子系统，传统直接优化方法的计算代价过高，亟需一种加速逆设计方案的方法。

Method: 提出FNO-based框架：1）构建傅里叶神经算子作为可微代理模型，以初始分子种群分布、激光频率和偏振为输入，预测分子运动种群动力学；2）基于该快速可微代理，开发FNO随机脉冲测量规划器（FNO-SPMP），构造脉冲序列将初始混合Boltzmann分布纯化到目标态。

Result: FNO预测速度比GPU加速的CUDA-Q Dynamics数值传播快10^7倍。在水合氢离子888维子空间、20 K条件下，目标态布居达到0.98，序列成功率达86.2%。在共享离散控制空间中，FNO-SPMP的成功率约为强化学习基线的两倍，脉冲序列生成时间从约10小时缩短至10-20分钟，且使用的量子控制脉冲数量约为其一半。

Conclusion: 算子学习代理模型能够实现对希尔伯特空间过大而无法进行传统直接优化的量子系统的逆设计，为高维量子控制提供了可行的加速路径。
...
### [47] [Entanglement Distillation of some Rank-Five Symmetric NPT States in Two-Qutrit Systems](https://arxiv.org/abs/2608.03710)
*Yuwei Lei,Zihua Song,Lin Chen,Mingju Liu*

Main category: quant-ph

TL;DR: 本文研究了一类秩为五的两-qutrit对称NPT态的蒸馏性质，解决了该类的1-可蒸馏问题（证明此前开放的参数区间是1-不可蒸馏的），并揭示了2-可蒸馏的结构性障碍（不存在在相关子空间中具有负期望的Schmidt秩为二的向量），同时通过数值研究探索了该障碍之外的2-可蒸馏性。


<details>
  <summary>Details</summary>
Motivation: 纠缠蒸馏是量子信息处理中的基本任务，理解哪些纠缠态可以被蒸馏对于量子通信和量子计算至关重要。此前一类两-qutrit对称NPT态（秩为五）的蒸馏性质存在开放问题，特别是1-可蒸馏性在某个特征值参数区间内尚未解决，2-可蒸馏性也不清楚，需要给出完整的刻画。

Method: 通过数学证明解决1-蒸馏性问题（证明开放参数区间为1-不可蒸馏）；通过揭示结构性障碍研究2-蒸馏性（即证明不存在Schmidt秩为二的向量在该相关子空间中具有负期望值）；并辅以数值计算探索结构性障碍之外的2-可蒸馏行为。

Result: 1) 完全解决了该类态的1-可蒸馏性问题，证明此前开放的参数区间是1-不可蒸馏的；2) 发现了2-可蒸馏的结构性障碍：没有任何Schmidt秩为二的向量能在相关子空间中产生负期望值；3) 通过数值研究进一步探索了超越此障碍的2-可蒸馏性质。

Conclusion: 该工作完整刻画了这类两-qutrit对称NPT态的可蒸馏性边界，为理解高维系统中的纠缠蒸馏提供了新的理论洞察，特别是揭示了结构限制在2-蒸馏中的作用。
...
### [48] [Preserving Symmetry: Spontaneous Symmetry Breaking through Decoherence](https://arxiv.org/abs/2608.03736)
*Sam Kuypers*

Main category: quant-ph

TL;DR: 本文通过量子化环境，证明有限量子系统中固体的定域化发生在退相干分支内，而复合系统的态矢量仍保持平移对称，其方法类比Page-Wootters构造与量子参考系，并可在远离半经典极限时产生实验可分辨的修正。


<details>
  <summary>Details</summary>
Motivation: 固体表现出局域化的质心，但多体量子理论用平移对称模型描述固体，无法直接产生局域化。传统解决方案通过自发对称破缺引入半经典环境，且仅在热力学极限下可移除环境交互。这引发了有限量子系统（非热力学极限）中局域化如何产生的问题。

Method: 将环境量子化，使其与固体系统形成复合系统。系统的定域化在退相干分支内出现，而复合系统的整体态矢量保持平移对称。方法类比Page-Wootters构造和量子参考框架，半经典描述作为极限情况被恢复。

Result: 有限量子系统中，固体的局域化可以在量子化环境的退相干分支内自然产生，同时保持复合系统态矢量的平移对称性。远离半经典极限时，该框架预测了实验可区分的修正效应。

Conclusion: 通过类比量子参考系和Page-Wootters构造，该工作为有限量子系统中国体局域化问题提供了新的解决途径，将半经典自发对称破缺描述作为一般量子框架的极限情形，并给出可实验检验的偏离预言。
...
### [49] [Uncovering Non-Gaussianity through Multi-Copy Symmetries](https://arxiv.org/abs/2608.03755)
*Hao Dai,Yue Zhang*

Main category: quant-ph

TL;DR: 本文提出一种基于群论和多拷贝框架的玻色子系统中非高斯性检测方法，通过研究混合量子态拷贝的被动线性光学变换与高斯幺正变换的交换性质，构造出所有单模高斯态均满足的见证子族，并可通过纯度固定热参考参数来认证非高斯性。


<details>
  <summary>Details</summary>
Motivation: 高斯态是连续变量量子信息中的基础态，但由于高斯集合的非凸性，表征非高斯性一直具有挑战性。现有见证子通常依赖Wigner负性或其他信息论量，缺乏统一且系统的检测框架。

Method: 采用群论的多拷贝方法：研究混合量子态多个拷贝的被动线性光学变换，分析其与作用于每个拷贝的相同高斯幺正变换的交换关系。正交拷贝混合变换与高斯作用的辛部分对易，而位移部分将对称性限制为集体模式的稳定化子。基于此结构推导出所有单模高斯态满足的见证子族，并通过纯度固定热参考参数实现非高斯性认证。

Result: 通过多个单模实例验证了该方法的有效性，并提出了基于被动干涉测量和光子数分辨探测的实验方案，证明相关多拷贝期望值可从有界相位可观测量估计。此外将构造扩展到多模系统，并讨论了利用相同对称性框架导出非高斯性定量度量的可能性。

Conclusion: 该工作提供了一种系统的、基于对称性的非高斯性检测新框架，不依赖Wigner负性等传统量，且具备实验可行性，并为非高斯性的定量刻画开辟了新方向。
...
### [50] [Optimal and Deterministic Quantum Search on the Simplex of Complete Graphs](https://arxiv.org/abs/2608.03777)
*Kiyoji Huang Fujiwara,Yujia Shi,Thomas G. Wong*

Main category: quant-ph

TL;DR: 本文首次解析证明了在加权完全图单纯形（first-order truncated simplex lattice）上进行量子搜索的最优性：当桥权重等于M时，算法以O(√N)的运行时达到确定性成功率1；当权重在√M到M之间时，算法既渐近最优又确定性；并给出了非确定性情况下通过检查邻居顶点来找到标记顶点的方法，同时展示了连通性在该图族内部也不能可靠预测量子搜索速度。


<details>
  <summary>Details</summary>
Motivation: 此前关于在完全图单纯形上进行连续时间量子行走搜索的研究存在一个缺口：要么通过数值方法证明最优运行时间O(√N)，要么解析证明确定性成功概率为1，但从未同时实现两者——即使对桥进行加权也是如此。作者旨在填补这一空白，首次解析证明该图上的最优量子搜索。

Method: 作者采用连续时间量子行走框架对加权完全图单纯形进行搜索分析。通过解析方法证明当桥权重等于M时算法最优且确定性；通过数值方法展示权重大于等于√M时即可达到最优运行时间；对于非确定性权重情形，提出了一种基于检查邻居顶点的恢复策略。

Result: （1）当桥权重等于M时，解析证明量子搜索最优（O(√N)）且确定性成功概率为1；（2）数值结果表明权重大于等于√M时即可实现最优运行时间；（3）当权重在√M和M之间时算法既渐近最优又确定性，这是该图族上首次同时具备这两个性质；（4）非确定性权重下可通过检查邻居顶点找到标记顶点；（5）证明了连通性在该图族内部也无法可靠预测量子搜索速度。

Conclusion: 该工作首次在完全图单纯形上同时实现了解析证明的最优运行时间和确定性成功概率，填补了此前研究中的关键空白。同时揭示了连通性即使在同一个图族内部也不能作为量子搜索效率的可靠指标，深化了对图结构、连通性与量子搜索性能之间关系的理解。
...
### [51] [Impossibility of Perfectly Complete Many-Round Key Agreement in the QROM](https://arxiv.org/abs/2608.03824)
*Longcheng Li,Qian Li,Xingjian Li,Qipeng Liu*

Main category: quant-ph

TL;DR: 本文证明在量子安全单向函数(OWF)的黑盒构造下，无法实现完美完整的量子密钥协商(QKA)协议。具体地，对于只交换经典消息、各自对布尔值随机预言机进行有限量子查询的Alice和Bob协议，存在窃听者仅通过监听经典消息并做O((qA+qB)^5)次经典预言机查询即可确定性地恢复共享密钥。该界限与轮数、转录长度、密钥长度和预言机域大小无关，优于此前仅适用于两轮协议或依赖未证猜想的结果。


<details>
  <summary>Details</summary>
Motivation: 此前的量子密钥协商(QKA)不可能性结果存在局限：Li等人的结果(CRYPTO 26)仅适用于两轮密钥协商协议，而Austrin等人的结果(CRYPTO 22)依赖于未经验证的猜想。需要一个通用的、不依赖于具体轮数限制和未证猜想的不可能性证明，以回答QKA协议是否可以从量子安全OWF黑盒构造这一根本性问题。

Method: 论文考虑任何只交换经典消息、Alice和Bob分别对布尔值随机预言机进行qA和qB次量子查询的QKA协议。通过构造窃听者，使其仅利用截获的经典消息作为线索，对随机预言机进行O((qA+qB)^5)次经典查询，即可确定性地恢复双方共享的密钥。

Result: 证明了无法从量子安全OWF以黑盒方式构造完美完整的QKA协议。窃听者恢复密钥所需的经典预言机查询次数为O((qA+qB)^5)，该上界独立于轮数、转录长度、密钥长度和预言机域大小，显著推广了此前的结果。

Conclusion: 黑盒量子安全OWF不足以构造完美完整的QKA协议，即使允许经典消息交换和多轮交互。这一不可能性结果填补了此前研究的空白，并可能引导研究者探索无需黑盒归约或更强密码学假设的QKA构造方案。
...
### [52] [CNOT-Distance is NP-complete under all-to-all connectivity](https://arxiv.org/abs/2608.03825)
*Antonio Acuaviva,Arturo Acuaviva,Pablo Acuaviva*

Main category: quant-ph

TL;DR: 本文证明了在固定标记线且全连接拓扑下，判断一个GL(N,2)矩阵能否用至多K个CNOT门实现的问题是NP完全的。作者从有限简单图G构造上三角幺模矩阵A_G，使得其CNOT门数恰好为2|V|+2|E|+τ(G)（τ(G)为最小顶点覆盖大小），并进一步证明该CNOT电路优化问题的近似问题是APX困难的。


<details>
  <summary>Details</summary>
Motivation: CNOT电路综合是量子编译中的核心问题，理解最小化CNOT门数的计算复杂度对于量子电路优化具有重要意义。此前对该问题复杂性的认识不完整，需要通过严格证明来确定其计算难度的边界。

Method: 作者采用归约方法：从有限简单图G构造特定的上三角幺模矩阵A_G∈GL(2|V|+|E|+1,2)，该矩阵具有O(N)个非零元素且每行Hamming权重最多为4。下界证明通过将任意CNOT电路展开为XOR有向无环图，并应用投影-收缩（projection-contraction）操作，允许取消和自由重用中间奇偶校验值。此外，通过多项式时间解码器获得固定加性常数内的近似NP困难性，并通过从三次图上的最小顶点覆盖进行L-归约建立APX困难性。

Result: 作者证明CNOT门数最小化问题（给定矩阵A和整数K，判断是否可用至多K个CNOT门实现）是NP完全的。构造的矩阵族中，最优CNOT门数与图的最小顶点覆盖大小精确对应，且该最优值不受有限数量的干净或借用辅助线的添加而改变。由于三次图上最小顶点覆盖问题为APX困难，通过L-归约得出CNOT电路优化问题为APX困难。

Conclusion: 该工作确定了CNOT电路综合问题在精确求解下为NP完全，在近似层面为APX困难，为量子电路编译中CNOT优化算法的设计与复杂度边界分析提供了坚实的理论基础。
...
### [53] [Separable States Violate the Complementary-Quantum Correlation Conjecture](https://arxiv.org/abs/2608.03828)
*Jinbo Wang,Qihang Wang,Kun Chen*

Main category: quant-ph

TL;DR: 本文否证了互补量子相关猜想（complementary-quantum correlation conjecture），该猜想认为在互补局部基下测量的两个经典互信息之和不会超过量子互信息。作者在所有维度d≥3中构造了显式的秩二可分离态作为反例，在qutrit情形下超出量精确为(1/3)log₂(3456/3125)≈0.0484 bits。该效应不需要纠缠或量子失协，而是源于对同一经典潜变量的两种互补读出。


<details>
  <summary>Details</summary>
Motivation: 互补基测量提供了一种实验上可访问的双粒子量子态总关联探针。此前学界普遍认为两个互补基下的经典互信息之和不会超过预测量的量子互信息，这一直觉性的猜想若成立将为量子关联的测量提供简洁的上界。作者旨在检验并挑战这一猜想。

Method: 作者通过构造一个显式的秩二可分离量子态作为反例。该态的设计使得其中一个互补测量能完全恢复单比特分支标签，而另一个互补测量保留额外的经典关联。在qutrit情形下精确计算出超出量为(1/3)log₂(3456/3125)比特。利用连续性论证，作者进一步将反例推广到满秩可分离态。

Result: 在任意局部维度d≥3下，均找到了违反该猜想的显式可分离态。qutrit情形下超出量为(1/3)log₂(3456/3125)=0.0484156759比特。通过连续性论证，满秩可分离态也能实现违反。关键发现是：该效应既不依赖纠缠也不需要量子失协，仅源于对同一经典潜变量的两个互补读出。

Conclusion: 互补量子相关猜想在所有维度d≥3下被证伪。关联的超额部分完全来自经典来源——同一经典潜变量在不同互补基下的读出差异，而非任何量子关联资源。这一结果修正了人们对互补测量与量子互信息关系的理解。
...
### [54] [Geometric-Symmetry Logical Gate and Local-Probe Selectivity in the Three-Leg AKLT Ladder](https://arxiv.org/abs/2608.03861)
*Jingnuo Han,Youning Li*

Main category: quant-ph

TL;DR: 本文研究了三腿AKLT梯子这一精确可解的对称性保护拓扑（SPT）系统，利用矩阵乘积态构造刻画了边界编码空间的对称性作用与可访问性，建立了由Wigner-Eckart定理控制的对称性分辨的局部可访问性衰减定律，建立了SPT对称性、晶格几何与边界量子信息保护之间的定量联系。


<details>
  <summary>Details</summary>
Motivation: SPT相为在受保护边界自由度中编码量子信息提供了平台，但对称性如何具体作用于边界编码空间、局部算子如何访问这些空间以及这种访问能力如何随对称性分辨而衰减，这些问题尚未得到精确的定量理解。

Method: 使用精确矩阵乘积态（MPS）构造解析三腿AKLT梯子（具有SO(3)×Z₂on-site对称性）；利用Knill-Laflamme条件启发定义可区分度度量；结合Wigner-Eckart定理推导出选择定则，将秩为ℓ的局部算子耦合到L=ℓ的转移矩阵扇区；识别出与探测算子位置无关的有限尺寸通道。

Result: 发现连续SO(3)对称性诱导边界旋转，而腿交换对称性产生依赖几何的逻辑置换。推导出对称性分辨的局部可访问性衰减定律，其衰减由选择定则控制（秩ℓ算子仅耦合L=ℓ转移矩阵扇区），衰减长度由相应关联长度决定，并存在与算子位置无关的有限尺寸通道。

Conclusion: 上述结果建立了SPT对称性、晶格几何与边界编码量子信息保护之间的定量联系，为理解SPT系统中量子信息保护的机制提供了精确的理论框架。
...
### [55] [Interplay between teleportation fidelity and basis-independent coherence in maximally sliced states under decoherence](https://arxiv.org/abs/2608.03865)
*Anushree Pandey,Sovik Roy*

Main category: quant-ph

TL;DR: 本文研究了环境退相干对量子隐形传态的影响，以三量子比特Maximally Sliced（MS）态为共享纠缠资源，通过Kraus算符形式推导了振幅阻尼和相位阻尼通道下的传态保真度，并建立了保真度与量子相干性及CKW三并发（three-tangle）之间的解析关系，揭示了两种噪声通道对量子传态性能的不同影响。


<details>
  <summary>Details</summary>
Motivation: 量子隐形传态在实际物理系统中的实现不可避免地受到环境退相干的影响，因此需要深入理解噪声如何影响传态性能。然而，多体纠缠、量子相干性与传态保真度之间的内在关联尚未被统一地解析刻画。

Method: 采用Kraus算符形式化方法，对三量子比特MS态分别施加振幅阻尼和相位阻尼两个退相干通道，导出传态保真度的解析表达式；同时计算与基无关的（basis-independent）相干性，并将结果进一步用CKW三-tangle表示，建立纠缠-相干-保真度三者之间的解析关系。

Result: 振幅阻尼通道引入了依赖于量子态参数的传态阈值，低于该阈值无法实现量子传态优势；相位阻尼通道则在完全退相干之前始终保留量子传态优势，两种噪声通道对传态性能的影响存在本质区别。

Conclusion: 该工作提供了一个统一的解析框架，用于理解噪声三量子比特MS态中多体纠缠、量子相干性与量子隐形传态之间的相互作用，为量子信息处理中资源评估提供了理论依据。
...
TL;DR: 本文研究双音驱动约瑟夫森结Kerr振荡器中的参量过程。通过四波混频激发简并参量下转换，发现系统响应保留半经典Kerr参量振荡器结构（包括失稳瓣和双稳态相空间拓扑），但常规单模约化在定量上失效（AC Stark位移被严重低估）。全三音量子描述能精确复现实验观测，三个交互音均处于深少光子极限，量子涨落动态主导耗散，使半经典宏观瓣发生根本性重整化。该结果确立了双音驱动Kerr振荡器作为参量放大器的潜力，为驱动-耗散电路中量子到经典交叉的研究开辟新方向。


<details>
  <summary>Details</summary>
Motivation: 研究多音外部驱动在少光子（few-photon）区域参量响应的有效性。此前，多音驱动虽为参量物理提供了一条无需直接调制器件的途径，但参量响应在极端少光子区域的正确性尚未被充分探索。作者希望理解双音驱动下Kerr振荡器的量子行为，尤其是单模约化近似何时失效，以及如何在极弱驱动下正确描述系统。

Method: 实验上对约瑟夫森结Kerr振荡器施加两个相干微波音，通过四波混频过程刺激简并参量下转换。利用透射光谱学观测系统的参量响应。理论上分别采用常规单模约化描述和全三音量子模型（包含三个交互音的完整量子涨落），将二者与实验观测的透射谱、AC Stark位移及光子分布进行比较。

Result: 实验观测显示响应保留半经典Kerr参量振荡器的定性结构（失稳瓣和双稳态相空间拓扑）。但定量上，单模约化严重低估AC Stark位移；当单光子Kerr位移K超过腔线宽κ时，单模约化给出的分布可能不具物理意义。相比之下，全三音量子描述精确复现实验可观测量。在深少光子极限下，驱动音的量子涨落动态上主导耗散，三个交互音的半经典宏观瓣因与量子方差深刻混合而发生根本性重整化。

Conclusion: 研究表明双音驱动的Kerr振荡器可作为参量放大器，并揭示了在驱动-耗散电路中量子到经典转变的新机制：即驱动音自身的量子涨落可成为动态主导因素。这打破了仅耗散主导量子行为的常规认识，为少光子区域的参量物理和量子-经典交叉研究提供了新的实验平台和理论框架。
...
### [57] [High-level quantum structured programs as quantum registers compositions](https://arxiv.org/abs/2608.03873)
*David Chamizo,Jose Garcia-Alonso,Juan M. Murillo*

Main category: quant-ph

TL;DR: 本文提出一种量子计算的高层抽象框架，将量子计算视为由量子寄存器组成的整体结构，通过语义明确的变换（作用于或纠缠量子寄存器）推进计算。作者用代数形式化描述了高层语法与低层语义的桥接，并基于此设计了量子可满足性模理论（SMT）求解器，为量子结构化编程奠定方法论基础。


<details>
  <summary>Details</summary>
Motivation: 当前量子程序主要在量子门级别设计，大规模复杂问题上程序员认知负荷高、易出错。需要提供更高层的抽象机制，使量子编程更易于管理且对设计错误更鲁棒。

Method: 将整个量子计算视为由量子寄存器（每个视为不可分割的实体）组成的结构；计算通过语义明确定义的变换作用于或纠缠量子寄存器以修改全局状态，遵循特定组合策略达到目标状态。提出基于代数形式化的高层语法以桥接低层语义，语法基于量子算法中常见的操作（如条件满足时的相位偏移、并行评估）。基于形式化操作设计量子SMT求解器。

Result: 建立了一个高通量子计算的概念框架，包括高层语法及其代数形式化描述，并基于形式化操作成功设计了量子SMT求解器的架构。

Conclusion: 本工作为实现高层量子结构化编程确立了方法论原则，有助于降低量子程序设计的认知负担和错误率。
...
### [58] [Exact Tradeoff Between Quantum Error Correction and Quantum Darwinism: An Information-Theoretic No-Go Theorem](https://arxiv.org/abs/2608.03944)
*Arghya Maity,Kelvin Onggadinata,Teck Seng Koh*

Main category: quant-ph

TL;DR: 本文通过一个基于Shor [[9,1,3]]量子纠错码的逻辑GHZ块严格可解模型，首次建立了量子纠错（QEC）与量子达尔文主义之间的定量信息论联系。研究表明，冗余经典记录的出现以逻辑量子信息的丧失为代价，并证明了一个模型无关的no-go定理：逻辑保真度超过临界阈值将阻碍达尔文式冗余的出现。


<details>
  <summary>Details</summary>
Motivation: 量子纠错和量子达尔文主义都源于系统-环境相互作用，但分别描述了其对立后果——前者保护逻辑量子信息，后者解释客观经典信息的涌现。尽管两者物理起源相同，此前从未建立直接的定量联系，填补这一理论空白是本文的核心动机。

Method: 构造一个基于Shor [[9,1,3]]码的逻辑GHZ块的集体耦合块-环境模型，系统解析地计算任意环境尺寸和任意恢复效率下的逻辑保真度、Holevo信息和达尔文式冗余度；通过消去共同退相干参数导出精确的权衡关系；并证明一个与具体哈密顿量和环境结构无关的no-go定理。

Result: 获得了环境尺寸和恢复效率任意条件下三大量（逻辑保真度、Holevo信息、达尔文冗余度）的精确解析表达式；建立了冗余度与恢复后逻辑保真度之间的精确权衡关系；证明了逻辑保真度超过临界阈值即排除达尔文式冗余出现的no-go定理；可解模型恰好饱和该通用界。

Conclusion: 该工作确立了逻辑量子信息保护与冗余经典记录涌现之间首个定量的信息论联系，揭示了量子纠错与量子达尔文主义之间的根本性权衡，为理解量子-经典过渡提供新的理论框架。
...
### [59] [Real-time decoding of quantum error correction codes using high-performance computing](https://arxiv.org/abs/2608.03948)
...
TL;DR: 本文提出THQLink架构，利用高性能计算（HPC）资源实现量子纠错码的实时解码，通过TH-Express网络连接QPU控制系统与HPC，实现平均2.944μs的往返延迟，并采用并行窗口策略在CPU上完成距离19的表面码实时解码（每轮QEC 1μs）。


<details>
  <summary>Details</summary>
Motivation: 量子纠错（QEC）是可扩展容错量子计算的关键，但实时解码器必须在微秒级时间内处理综合征测量结果并确定纠错方案，以避免数据积压。随着逻辑量子比特数量增加，解码所需的计算资源急剧增长，传统方案难以满足实时性要求，因此需要利用HPC资源构建可扩展的实时解码框架。

Method: 提出THQLink架构，基于TH-Express网络将HPC与QPU控制系统集成，可适配不同量子技术和控制栈。采用并行窗口策略，在CPU上使用基于匹配的解码器（matching-based decoder）对表面码进行实时解码，通过多跳网络实现低延迟数据传输和分布式计算。

Result: 往返延迟平均为2.944μs，每增加一跳的增量开销为130ns；采用并行窗口策略，可在CPU上实现对距离19的表面码每轮QEC 1μs的实时解码。

Conclusion: THQLink提供了可扩展的实时解码框架，可直接应用于量子中心超级计算机（QPU与HPC紧密集成），为混合量子-经典算法和从QPU卸载的计算密集型工作负载提供高效支持。
...
### [60] [Separating quantum circuits from classical LLMs](https://arxiv.org/abs/2608.03962)
*Srinivasan Arunachalam,Arkopal Dutt,Hari Krovi,Rik Sengupta*

Main category: quant-ph

TL;DR: 本文证明了低深度量子计算与有界资源经典语言模型架构（Transformer和扩散语言模型）之间不可条件分离，分别给出分布级分离和函数级分离的结果，开创了大语言模型时代量子优势的研究。


<details>
  <summary>Details</summary>
Motivation: 现代大语言模型基于两个核心算法任务：预测和生成。作者旨在从计算复杂度理论的角度，探究低深度量子电路是否能在这些任务上超越受资源限制的经典语言模型架构，从而开启大语言模型时代量子计算优势的正式研究。

Method: 通过构造具体的分布和函数来实现分离：(1) 构造一个可由QNC^0电路采样的分布，证明任何具有浅层调度和去噪的常数轮扩散语言模型（DLM），即使允许次线性思维链和输出token修订/重掩码，也无法在常数距离内采样该分布；(2) 构造一个可在log log n深度QNC^0电路后跟单个经典AND门中计算的函数，证明任何计算该函数的常数深度decoder-only Transformer必须具有n^{Ω(1)}的宽度。

Result: (1) 分布级分离：存在QNC^0可采样的分布，任何常数轮且浅层调度去噪的扩散语言模型无法在常数距离内采样；(2) 函数级分离：存在可由∧∘QNC^0[log log n]计算的函数，任何计算它的常数深度decoder-only Transformer需要多项式级别的宽度。

Conclusion: 该工作在严格的计算复杂度框架下建立了低深度量子电路相对于经典语言模型架构（无论Transformer还是扩散模型）的分离结果，开启了大语言模型时代量子优势的系统性研究。
...
### [61] [A quantum game of telephone](https://arxiv.org/abs/2608.03963)
*Arefur Rahman,Matthew L. Stevens,Cory M. Nunn,Daniel E. Jones,Brian T. Kirby,Joseph M. Lukens*

Main category: quant-ph

TL;DR: 本文提出"量子电话"（quantum telephone）协议，一种迭代式的辅助量子过程层析成像方法，利用中间探测事件将先前探测的链路作为辅助量子比特来表征多节点量子网络，并在部署的真实光纤网络上验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 在缺乏普遍可用的局部纠缠源的情况下，表征多节点量子网络面临显著的实验挑战；标准辅助辅助过程层析成像对辅助态有严格的全秩要求，而实际噪声中间信道会导致参数简并并放大顺序估计中的推理误差。

Method: 提出"量子电话"协议，采用迭代式辅助辅助过程层析成像方法：将先前已探测的链路作为后续链路的辅助量子比特，利用中间探测事件传递量子信息；同时通过全局推理结合下游近酉信道的边界约束来解析序列中早期信道的模糊性，补偿局部信息损失。

Result: 在真实部署的多节点光纤网络上实现，观察到噪声中间信道造成的参数简并使顺序估计中的推理误差加剧；但下游近酉信道的边界约束与全局推理相结合可解决早期信道歧义；即使在中间态完全去极化的情况下也能工作，避免了对标准辅助辅助过程层析成像严格全秩要求的依赖。

Conclusion: 量子电话协议提供了一种硬件高效且信息最大化的路径，能够在资源有限的情况下表征复杂的量子网络，弥补了标准方法在全秩要求和局部信息损失方面的不足。
...
### [62] [A lower bound on the classical simulation cost of star-network correlations](https://arxiv.org/abs/2608.03986)
*Martin J. Renner*

Main category: quant-ph

TL;DR: 该论文研究星型网络中的量子通信复杂度，提出了一种排他性任务，该任务在各方发送量子d维系统时可以完美解决，但若发送经典消息则需要大量符号（少于n^(d-1)个符号则无法达到确定性解决）。这展示了量子相对于经典通信的优势，其规模同时依赖于量子系统维度和同时测量的系统数量。


<details>
  <summary>Details</summary>
Motivation: 量子策略在通信任务中优于经典策略是众所周知的事实，但作者希望在星型网络这一特定场景下，量化量子通信相对于经典通信的优势规模，并探索这种优势是否随量子系统维度和参与方数量同时扩展。

Method: 论文引入了一种排他性（exclusion）任务，其中多个参与方各向中心节点发送量子系统，中心节点对接收到的系统进行联合测量。作者通过对比量子d维系统通信与经典消息通信在该任务中的可行性，推导出经典通信所需消息规模的下界。

Result: 该任务在各方发送量子d维系统时可完美求解，但若n方各发送经典消息且每条消息少于n^(d-1)个符号，则任务无法以确定性方式解决。这表明量子优势同时随量子系统维度d和参与方数量n扩展。作为推论，任何有限大小的经典描述都不足以在足够多的量子比特联合测量中复现其统计特性。

Conclusion: 该研究证明了星型网络中量子通信在排他性任务上具有显著优势，且这种优势随系统维度和参与方数量同时增长，并揭示了量子系统统计特性无法用有限经典描述逼近的本质。
...
### [63] [Realified tensor networks: quantum circuit simulation on real-valued matrix accelerators](https://arxiv.org/abs/2608.03987)
*Yusheng Zhao,Xiwei Pan,Enji Xiong,Chengkai Zhu,Jinguo Liu*

Main category: quant-ph

TL;DR: 本文提出一种"实化重写"(realification rewrite)方法，将复数张量网络收缩映射为实数运算，使量子电路模拟能在仅支持实数GEMM的现代矩阵加速器（NPU/TPU）上高效运行。通过高斯三次乘法(3M)公式将复数收缩重构为实数网络，并在Ascend 910 NPU上验证了其优越性。


<details>
  <summary>Details</summary>
Motivation: 量子电路模拟依赖复数张量网络收缩，但现代矩阵加速器（如NPU、TPU）仅暴露实数GEMM流水线。复数网络需要在软件层重构为实数运算，而传统的四实数GEMM基线方法效率低下，存在性能瓶颈。本文旨在解决复数张量网络与实数硬件加速器之间的根本性失配问题。

Method: 提出实化重写(realification)：将任意复数张量网络映射为实数网络。具体地，合并两个复数张量时引入秩-3结构张量实现高斯三次乘法(3M)公式；对含一个或零个复数操作数的收缩只需2个或1个实数乘法。同时推导出紧致的成本定律：实数乘法开销为1+2m+r（m和r分别为双复数与单复数操作数收缩的体积分数），最坏不超过实数收缩的3倍，且每个中间张量大小至多翻倍。

Result: 在67个电路（随机、Clifford+T、QAOA、VQE）上验证了成本定律，且发现成本由复数门的位置决定而非数量。收缩顺序从复数网络迁移时，66/67个电路的相对算术成本差低于5×10^-4（例外可通过低温模拟退火消除）。在Ascend 910 NPU上，重写方法全面优于四实数GEMM基线和逐GEMM高斯降维方案：全部12个随机电路和52/55个结构化电路更快（3个慢至多12%）；四GEMM基线中位慢1.7×（随机）和1.4×（结构化）。

Conclusion: 实化重写使复数张量网络收缩成为实数矩阵引擎的原生操作，以不超过3×的乘法开销和至多2倍的中间张量代价，实现了从复数网络向实数硬件的无缝映射，为量子电路模拟在实数加速器上的高效运行提供了理论保证和实际验证。
...
### [64] [Non-Relativistic Quantum Electrodynamics of Atoms in a Rotating Ring Cavity](https://arxiv.org/abs/2608.03997)
*Jarrod T. Reilly,Murray J. Holland*

Main category: quant-ph

TL;DR: 本文从第一性原理出发，在旋转环形腔中推导了一个量子光学模型。从弯曲时空中的Dirac方程出发，加入电磁场的最小耦合，通过Foldy-Wouthuysen变换取非相对论极限，并投影到原子的费米Fock空间。对氢原子采用Power-Zienau-Woolley变换从最小耦合规范转向多极展开规范，发现旋转诱导的超精细频移，最终得到包含旋转效应的Jaynes-Cummings类哈密顿量。


<details>
  <summary>Details</summary>
Motivation: 旋转（非惯性）参考系中的量子光学系统具有重要的基础物理意义和应用前景，但此前缺乏从第一性原理出发的严格推导。作者旨在建立一个自洽的量子光学模型，系统地涵盖旋转对原子-腔场相互作用的影响，包括Sagnac效应和旋转诱导的超精细频移。

Method: 从弯曲时空中的Dirac方程出发，对广义Born度规加入电磁场最小耦合，得到Dirac哈密顿量；通过Foldy-Wouthuysen变换严格取非相对论极限；投影到电子、质子、中子的费米Fock空间；对氕原子在偶极和长波长近似下进行Power-Zienau-Woolley变换进入多极展开规范；最后在电偶极、二能级和旋波近似下得到Jaynes-Cummings类哈密顿量。

Result: 推导出了一个旋转环形腔中原子-腔场相互作用的完整量子光学哈密顿量。发现旋转系统引入了额外项，包括旋转诱导的原子跃迁超精细频移（即使对很小的旋转速率也可能在实验上观察到），以及腔对向传播模式的Sagnac频移。

Conclusion: 该工作从第一性原理建立了非惯性旋转腔中的量子光学模型，揭示旋转不仅通过Sagnac效应影响腔模，还通过旋转诱导的超精细频移影响原子内部能级结构，为旋转系统中的量子光学实验和理论提供了严格的物理基础。
...
### [65] [Surface Variables Description of Axion Topological Materials](https://arxiv.org/abs/2607.10509)
*Francisco J. Solis,Hugo García-Compeán*

Main category: quant-ph

TL;DR: 本文研究了拓扑绝缘体（轴向拓扑材料）在外部静态电场和磁场下的宏观准静态磁电响应。采用源自软凝聚态问题的表面变量技术，将整个系统用有效表面电荷和表面电流密度描述，推导出积分方程组，给出了轴对称情形的精确解析解，并发展了边界有限元数值方法应用于球体、空心球和环面等几何形状。


<details>
  <summary>Details</summary>
Motivation: 拓扑绝缘体在外加静电场和磁场下的磁电响应是一个重要的研究课题，但先前缺乏一种统一且完整的宏观描述框架。本文旨在利用表面变量的方法，为异质系统（由多种均匀材料组成）提供一种完整的解析与数值求解途径。

Method: 采用软凝聚态物理中处理异质系统的表面变量技术，将整个系统约化为表面自由度（有效表面电荷密度和表面电流密度），推导满足的积分方程；对轴对称情形给出精确解析解；发展基于边界有限元方法的数值求解方案，并应用于球体、空心球和环面等不同几何构型的拓扑绝缘体。此外，还证明了可通过变分原理恢复表面变量方程。

Result: 成功获得了表面电荷和表面电流密度满足的积分方程组，给出了轴对称情形的精确解析解，并通过边界有限元数值方法对球体、空心球和环面等不同几何构型的拓扑绝缘体进行了求解，验证了数值方法的有效性。

Conclusion: 表面变量的积分方程方法为拓扑绝缘体在外部静态电磁场下的磁电响应提供了一套完整且通用的描述框架，既包含精确解析解（轴对称情形），又具有可推广到任意几何的数值方法（边界有限元），同时通过变分原理建立了理论上的统一基础。
...
### [66] [Collider Spin Tomography with Missing Neutrinos](https://arxiv.org/abs/2607.28346)
*Tengyu Ai,Qi Bi,Yuxin He,Zekun Li,Jia Liu,Xiao-Ping Wang*

Main category: quant-ph

TL;DR: 本文提出在存在不可见粒子导致运动学模糊时，仍可通过将可见测量表述为生产自旋密度矩阵上的粗粒化POVM来进行自旋层析。信息损失由可见数据映射的零空间决定，而非运动学解的数量。在e+e-→τ+τ-→π+π-+νν̄过程中，二重模糊仅使反对称自旋关联组合C_nr-C_rn不可识别，其余十四项自旋系数均可识别。作者开发了仅依赖可见数据的自洽定点展开（unfolding）方法，在标准模型和反常tau偶极矩基准下均能重构真实微分产生率和所有可识别的自旋系数，优于常规的平面平均方法。


<details>
  <summary>Details</summary>
Motivation: 中微子等不可见粒子的存在会导致运动学模糊，可能威胁基于末态粒子动量的自旋层析测量。此前普遍认为运动学多重解会严重破坏自旋信息恢复，因此需要澄清信息损失的真实机制并发展鲁棒的实用重构方法。

Method: 将可见测量表达为生产自旋密度矩阵上的粗粒化POVM；通过研究可见数据映射的零空间分析信息可识别性；提出自洽定点展开方法，仅使用可见数据且无需假设理论产生模板，实现对微分产生率和自旋系数的重构。

Result: 运动学二重模糊仅导致C_nr-C_rn一个组合无法识别，其余十四个自旋系数可识别。定点展开方法在SM和反常tau偶极矩基准下均准确重构所有可识别量，而平面平均方法产生显著偏差。存在非平凡零空间时，可识别子空间结合正定性约束可给出纠缠度（concurrence）和CHSH参数的控制区间。

Conclusion: 缺失的中微子不会摧毁对撞机自旋层析；信息损失由映射零空间而非运动学解数目决定，且定点展开方法可在实践中可靠恢复可识别自旋信息。
...
### [67] [de Sitter holography from a Lorentzian torus](https://arxiv.org/abs/2608.01729)
*Kosei Fujiki,Michitaka Kohara,Javier Moreno,Kotaro Shinmyo,Tadashi Takayanagi*

Main category: quant-ph

TL;DR: 本文提出一种新的dS/CFT对应实现方式：将de Sitter时空未来与过去边界上的CFT之间的量子纠缠，通过Wick旋转到具有两个类时方向的几何来描述。量子引力在该双时间几何上对偶于定义在Lorentzian环面上的CFT，该对偶能再现dS全息的关键特性（dS熵、关联函数和伪熵），并通过路径积分优化框架解释dS时空如何从Lorentzian环面上的CFT涌现。


<details>
  <summary>Details</summary>
Motivation: de Sitter时空的全息描述是全息原理在宇宙学背景中的重要难题。传统dS/CFT对应中，未来和过去边界上的两个CFT之间的量子纠缠缺乏清晰而严格的描述框架，需要一种新的几何构造来理解和量化这种纠缠关系。

Method: (1) 通过Wick旋转将dS/CFT对应的双边界CFT纠缠问题映射到具有两个类时方向的几何上；(2) 提出该双时间几何上的量子引力与定义在Lorentzian环面上的CFT之间的新全息对偶；(3) 利用该对偶计算dS熵、关联函数和伪熵；(4) 扩展路径积分优化框架，从Lorentzian环面上的CFT出发构造dS时空的涌现。

Result: 新提出的对偶关系成功再现了dS全息的关键特征，包括dS熵、两点关联函数以及伪熵，表明双时间几何+Lorentzian环面CFT的框架是dS/CFT对应的一致性和可计算性俱佳的实现方式。路径积分优化方法还解释了dS时空如何从CFT数据中涌现出来。

Conclusion: 该工作为dS全息提供了一种新的、可计算的实现方案，将未来与过去边界CFT之间的纠缠通过双时间几何自然编码，并从CFT出发展示了dS时空的涌现机制，为理解宇宙学全息原理提供了新的视角和工具。
...
### [68] [Intent-Level Quantum Programming with Assertion-Guided Execution and Inspectable Intermediate Representation](https://arxiv.org/abs/2608.02648)
*Ilesh Vora,Srikanth Thudumu,John Carlson,Harshil Kamdar,Rajesh Vasa*

Main category: quant-ph

TL;DR: 本文提出一种量子领域特定语言（QDSL），通过意图-执行分离、一等IR内省与断言引导的模式推断三个机制，解决量子程序验证与跨后端可移植性问题。在基准电路上验证了故障注入检测（TPR=1.0）、跨框架差分测试一致性及亚毫秒级IR生成开销。


<details>
  <summary>Details</summary>
Motivation: 量子程序的验证面临三大挑战：电路通常以命令式门序列表达、难以检查；执行模式需人工选择；输出本质上是概率性的。此外，跨不同后端框架（具有不同的内部约定）的可移植性进一步加剧了这些困难。

Method: 提出QDSL，包含三个机制：(i) 意图-执行分离：将制备、叠加、纠缠、测量等算法构造表示为可检查对象，仅在预执行IR验证后才编译为后端特定电路；(ii) 一等IR内省：在执行前向开发者暴露电路宽度、线映射、操作顺序和测量意图；(iii) 断言引导的模式推断：引擎根据声明的验证属性自动选择采样、statevector或双执行模式。结果以结构化形式记录以支持可复现的回归测试。

Result: 在纠缠、基于预言机、结构化变换和变分等基准电路上评估原型。在Bell和3-qubit GHZ电路的故障注入实验中，基于全变分距离的检测器在所有评估的shot预算下识别出注入故障（TPR=1.0），仅在128 shots时观察到假阳性（FPR=2.7-3.8%）。差分测试表明在字节序规范化后，PennyLane和Qiskit编译目标在评估构造上数值一致。IR生成成本为亚毫秒级。

Conclusion: 所提出的QDSL通过可检查的IR表示、程序化内省和自动执行模式推断，有效解决了量子程序验证与跨框架移植问题，在故障检测、跨后端一致性方面均达到优异性能，且编译开销极低。
...
### [69] [High-Frequency Gravitational Wave Detection with Superconducting Qubits](https://arxiv.org/abs/2608.02733)
*Heechan Yi,Seong Chan Park,Kyoungchul Kong,Myeonghun Park*

Main category: quant-ph

TL;DR: 本文提出利用超导transmon量子比特嵌入共振微波腔的新型量子感测范式，通过逆Gertsenshtein效应探测高频引力波（HFGWs），利用Dicke纠缠态实现信号概率的二次方增益，使应变灵敏度标度达到h_min ∝ n_q^{-3/4}，800个量子比特的全局寄存器可将灵敏度提升五个数量级，突破宏观腔功率极限和Planck尺度抑制。


<details>
  <summary>Details</summary>
Motivation: 高频引力波（HFGWs）是探索高能和早期宇宙物理的独特窗口，但传统宏观干涉测量法无法探测此频段，存在显著的探测空白，需要新的探测范式来弥补这一差距。

Method: 将超导transmon量子比特嵌入共振微波腔，利用逆Gertsenshtein效应（HFGWs在静态磁场中传播时共振激发腔模），利用自旋-2四极电磁场模式在TE212模电场热点处定位量子比特，并将量子比特阵列构造成对称Dicke态纠缠量子寄存器以实现集体量子增强。

Result: 信号概率随量子比特数二次方增长，应变灵敏度标度为h_min ∝ n_q^{-3/4}；理想化800量子比特全局寄存器的应变灵敏度超过标准宏观腔功率极限五个数量级，有效缓解引力相互作用的Planck尺度抑制。

Conclusion: 该集体量子增强机制确立了GHz频段下一代高频引力波搜索的变革性框架，展示了量子纠缠资源在基础物理探测中的巨大潜力。
...
### [70] [Ramp, Plateau, and Wormholes without Averaging, and Hyper-non-perturbative Structures in Gravity](https://arxiv.org/abs/2608.02743)
*Hong Liu*

Main category: quant-ph

TL;DR: 该论文提出了一种替代方法：量子混沌的标志性现象（如谱型因子中的ramp和plateau）并非源于系综或谱平均，而是微观不规则数据中隐藏的宏观光滑结构，可通过光滑滤波投影来分离。作者以半经典Gutzwiller迹公式为例，展示了无需平均即可推导出随机矩阵模型的特征；并推测全息系统大N极限中存在最小Gutzwiller-like结构，预测了态的密度中普遍的快速宏观振荡和超瞬子（双重指数效应），同时在引力侧论证了时空虫洞如何构建这些超非微扰对象。


<details>
  <summary>Details</summary>
Motivation: 量子混沌的通用特征（如谱型因子中的ramp与plateau）及其引力对偶（虫洞）通常被解释为谱平均或系综平均的结果。作者希望提出一种替代框架：这些现象本质上是隐藏在随机微观数据中的宏观光滑结构，可以通过滤波投影提取，而不需要引入任何平均过程。这在概念上可能更贴合AdS/CFT等全息框架中单一量子系统的谱。

Method: 论文以半经典Gutzwiller迹公式为范例，发展光滑滤波投影方法，从微观无规谱数据中分离出宏观光滑结构，进而推导出ramp、plateau、谱曲线和单本征值瞬子等随机矩阵特征。作者进一步假设全息系统大N极限中存在最小Gutzwiller-like结构，并从引力侧通过虫洞构建婴儿宇宙和虫洞凝聚体，产生双重指数效应，从而与边界侧的推测形成对偶。

Result: 在半经典极限下，无需系综或谱平均即可推导出随机矩阵理论的典型特征（ramp、plateau、谱曲线、单本征值瞬子）。Gutzwiller-like结构进一步预测了态密度中的快速宏观振荡和可能的超瞬子，两者都涉及1/N^2中的双重指数效应。在引力侧，时空虫洞构建的婴儿宇宙和虫洞凝聚体产生G_N中的双重指数效应，与边界预测一致。

Conclusion: 论文建立了量子混沌通用特征的一种非平均化理论框架，表明ramp和plateau等可作为半经典极限下宏观光滑滤波的产物。全息对偶表明，边界侧的Gutzwiller-like结构与引力侧的虫洞构建的超非微扰对象（如虫洞凝聚体）在双重指数层面上相互呼应，为态密度快速宏观振荡和谱plateau提供了统一的引力推导。
...
### [71] [Magneto-oscillations, nonlinearity, and nonreciprocity of Coulomb drag in quantum circuits](https://arxiv.org/abs/2608.02812)
*Alex Levchenko,Mingyang Zheng,Dominique Laroche*

Main category: quant-ph

TL;DR: 该论文研究了由绝热收缩结构（量子点接触和短量子线通道）组成的相互作用耦合量子电路中的库仑拖拽问题，推导出了线性拖拽电导的闭式表达式，并探讨了非线性区域、耗散诱导的非互易性以及磁场、温度等多种因素对拖拽振荡的影响。


<details>
  <summary>Details</summary>
Motivation: 理解在空间限制和磁场共同作用下，量子电路中的库仑拖拽现象及其丰富的振荡响应特性，这对于基础量子输运研究和潜在器件应用具有重要意义。

Method: 通过解析方法推导线性拖拽电导的闭式表达式，在热长度远大于线间耦合范围的实验相关极限下简化相互作用核；分析非线性区域的拖拽电流（可测量驱动通道的跨导）；并纳入塞曼劈裂、相互作用重正化、高场背散射和电路耦合频率结构等物理机制进行系统性研究。

Result: 发现拖拽峰跟踪磁电子子带的去居组过程，在逆磁场中呈渐近周期性；拖拽振荡的可见性由温度与场加宽的收缩隧穿宽度之间的竞争控制；在特定极限下相互作用核可显著简化；拖拽电流在非线性区域可测量驱动通道的跨导。

Conclusion: 论文系统描绘了各物理机制（塞曼劈裂、相互作用重正化、高场背散射、耦合频率结构）如何影响拖拽振荡的温度依赖性和线形，为量子电路中的库仑拖拽现象提供了完整的理论图景。
...
### [72] [Topological Recursion and Quantum Path Signatures](https://arxiv.org/abs/2608.02861)
*Inês Aniceto,Thomas Cass,Samuel Crew*

Main category: quant-ph

TL;DR: 本文提出了一种路径泛函的非交换拉普拉斯变换，将路径空间上的卷积转化为张量代数中形式级数的代数乘积，并将其应用于随机酉矩阵值路径发展（量子路径签名），得到平面回路方程的非交换谱形式，进而扩展到1/N属展开，由拓扑递归组织，获得路径空间上修正项的积分方程层次。


<details>
  <summary>Details</summary>
Motivation: 传统拉普拉斯变换将卷积变为乘积，但在路径空间上缺乏对应的非交换框架。作者希望建立一种非交换拉普拉斯变换，使路径泛函的卷积与张量代数中的形式级数相乘建立对应关系，从而为量子路径签名及其回路方程提供新的代数工具。

Method: （1）定义非交换拉普拉斯变换，将路径泛函映射到张量代数中的形式级数；（2）证明该变换将路径泛函的自然卷积变为级数乘法（代数乘积）；（3）将此变换应用于随机酉矩阵值路径发展（量子路径签名）；（4）推导出平面回路方程的非交换谱形式；（5）进一步将回路方程扩展到1/N属展开，采用拓扑递归进行组织；（6）得到路径空间上各阶修正项的积分方程层次。

Result: 成功建立了非交换拉普拉斯变换与张量代数形式级数之间的对应关系；获得了量子路径签名控制的平面回路方程的非交换谱形式；并将其推广到1/N属展开，通过拓扑递归获得路径空间上修正项的积分方程层级结构。

Conclusion: 非交换拉普拉斯变换为路径泛函分析提供了新的工具，量子路径签名的回路方程可被系统地表述为非交换谱形式，并可通过属展开和拓扑递归得到完整修正层次，建立了随机矩阵理论与路径空间微积分之间的深层联系。
...
### [73] [TNASS: Tensor Network Active Space Selection with the Entanglement Feature](https://arxiv.org/abs/2608.03645)
*Angus Mingare,Isabelle Heuzé,Peter V. Coveney*

Main category: quant-ph

TL;DR: 本文提出了一种基于纠缠特征和张量网络的自动活性空间选择方法（TENSES），通过将轨道划分的纯度表示为矩阵乘积态来隔离强关联电子区域，无需人工预选目标原子或计算高阶密度矩阵，在基态能量和偶极矩方面优于其他全自动选择方案。


<details>
  <summary>Details</summary>
Motivation: 多尺度分子电子结构计算（如嵌入法和子空间法）的质量依赖于所选的活性空间，而活性空间选择的自动化对确保计算精度、可复现性和可扩展性至关重要。现有方法要么需要人工预选目标原子，要么需要昂贵的计算资源。

Method: 该方法通过将所有可能轨道划分的纯度表示为矩阵乘积态（MPS），从而隔离强电子关联区域。它利用纠缠特征来自动选择活性空间，避免了人工预选目标原子和计算高阶密度矩阵的高昂代价，为嵌入方法提供了可扩展的基础。

Result: 实验结果表明，该方法相比仅基于单轨道熵或围绕HOMO/LUMO能隙选择空间轨道的全自动选择方案，能够获得更低的基态能量和更精确的偶极矩。

Conclusion: TENSES方法为多尺度建模中的嵌入方法提供了一个可扩展且全自动的活性空间选择方案，兼顾了精度与计算效率，有望提升此类计算的准确性和可复现性。
...
### [74] [Quantum Impurities as Probes of Finite-Temperature Fluctuations in Two-Dimensional Bose Gases](https://arxiv.org/abs/2608.03665)
*Victor Velasco,Gabriele Spada,Giovanni Midei,Andrea Perali,Luis A. Peña Ardila*

Main category: quant-ph

TL;DR: 本文研究了沉浸在有界弱相互作用二维玻色气体中的单个吸引杂质（玻色极化子），发现红外动量尺度引入的声子激活温度使杂质能量在低温下几乎与温度无关，而当声子模式热激发后，其贡献与凝聚体耗损竞争导致极化子能量的非单调温度依赖关系。


<details>
  <summary>Details</summary>
Motivation: 二维量子气体中增强的热涨落、有限尺寸效应和两体束缚态形成内在交织，为理解玻色极化子的准粒子性质提供了独特平台。作者希望阐明有限尺寸效应对杂质准粒子能量的影响，并探索吸引玻色极化子作为探测低维玻色气体中有限尺寸热涨落、声子缀饰和束缚态物理的灵敏探针。

Method: 采用混合方法：将有限温度多体散射理论与路径积分蒙特卡洛输入相结合，利用Bogoliubov描述有限体系中由红外动量尺度稳定化的非零凝聚分数，系统分析杂质准粒子能量在凝聚态和正常态两个区域中的行为。

Result: 发现红外尺度产生一个声子激活温度：低于该温度时杂质能量几乎与温度无关；一旦解析的声子模式被热激发，其贡献与凝聚体耗损相互竞争，导致极化子能量随温度呈现非单调变化。

Conclusion: 吸引玻色极化子可作为低维玻色气体中有限尺寸热涨落、声子缀饰及束缚态物理的灵敏探测工具，为实验上观测这些现象提供了理论依据。
...
### [75] [Controllable interaction between photons and distant spins via vacuum Rabi oscillations](https://arxiv.org/abs/2608.03809)
...
TL;DR: 本文展示了一种由两个空间分离的双量子点自旋量子比特与超导腔组成的器件，观察到每个自旋量子比特与腔之间的多次真空拉比振荡，并通过拼接这些振荡实现能量从一个量子比特到另一个量子比特的传递，制备了Fock态腔。


<details>
  <summary>Details</summary>
Motivation: 在栅定义量子点中观察到强自旋-光子耦合之后，在时域中探测这种相互作用一直是一个重要目标。在单个能量量子级别揭示光-物质相互作用的潜力，需要观察到可控的真空拉比振荡及量子比特间的信息传递。

Method: 精心设计并制造了一个由两个空间分离的双量子点（各容纳一个单电子自旋量子比特）和容纳微波光子的超导腔组成的器件，通过拼接涉及两个自旋的真空拉比振荡，实现能量激励在量子比特间的光子介导传递。

Result: 观察到每个自旋量子比特与腔之间的多次真空拉比振荡。当单光子发射时，腔被制备在Fock态，表现出加速的真空拉比频率；能量激励可从一个自旋量子比特发射为光子并传递到另一个量子比特。

Conclusion: 这些结果为探索光-物质相互作用提供了基础模块，也为将半导体自旋量子比特与光子链路接口连接打下了基础。
...
### [76] [Joint spectral characterization of SPDC photon pairs near 2 $μ$m in (Al)GaAs-on-insulator waveguides](https://arxiv.org/abs/2608.03950)
...
TL;DR: 本文在GaAs和AlGaAs-on-insulator直波导中实现了自发参量下转换（SPDC），产生了中心波长约2 μm的宽带光子对，并通过联合光谱强度和谐振二阶相关函数测量表征了其强光谱反关联特性。这是该波段集成SPDC源的首次直接联合光谱表征。


<details>
  <summary>Details</summary>
Motivation: 集成光子对源是芯片级量子计算、通信和计量的核心组件。虽然传统电信波段的此类源已广泛实现，但2 μm波段因其在自由空间量子通信、新兴光纤网络中的低损耗传输以及与硅光子平台集成的优势而值得研究，但该波段目前探索较少。

Method: 在直GaAs和AlGaAs-on-insulator波导中实现自发参量下转换（SPDC）。该平台具有强二阶非线性和几何可调色散，有利于片上高效光子对产生。通过联合光谱强度和谐振二阶相关函数测量进行表征。

Result: 测量结果显示在2 μm附近有宽带发射，且光子对具有强光谱反关联特性。这是该波长范围内集成SPDC源的首次直接联合光谱表征。

Conclusion: GaAs/AlGaAs-on-insulator平台成功展示了2 μm波段的集成SPDC光子对源，为该波段在自由空间量子通信、低损耗光纤传输和硅光子集成等应用中的量子光源发展奠定了基础。
...
### [77] [Universal crossovers in weakly-monitored quantum critical states](https://arxiv.org/abs/2608.02716)
*Abhishek Kumar,Rushikesh A. Patil,Andreas W. W. Ludwig,Romain Vasseur*

Main category: cond-mat.stat-mech

TL;DR: 本文研究了三临界和临界一维量子Ising基态在弱能量测量和弱自旋测量（无后选择）下的测量后系综，揭示了测量作为相关扰动驱动系统流向量子临界点附近的"测量主导"或"测量改变"不动点。临界Ising模型直接流向投影测量不动点（面积律纠缠），而三临界Ising模型则流向具有对数纠缠的弱测量不动点。


<details>
  <summary>Details</summary>
Motivation: 理解测量如何作为相关扰动改变量子临界基态的普适性质，特别是弱测量对多临界Ising基态引入的Born规则随机性如何产生丰富的长程标度行为。

Method: 采用有限尺寸重整化群（RG）交叉分析，利用纠缠有效中心荷、有效Affleck-Ludwig边界熵以及测量平均关联函数矩的多分形特征来刻画普适性质。

Result: 两个模型中均发现由Born规则随机性主导的测量不动点；临界Ising直接流到投影测量不动点（面积律纠缠），三临界Ising则存在弱测量不动点（对数纠缠）。

Conclusion: 该工作澄清了弱测量多临界Ising基态的RG流结构，展示了测量诱导的固有随机性能够在后测量系综中产生复杂而丰富的普适长程标度行为，且可通过可控的解析RG和数值有限尺寸RG交叉分析获得。
...
### [78] [Fractal deconfinement and confinement in Sierpinski ice](https://arxiv.org/abs/2608.02741)
*James Walkling,Roderich Moessner*

Main category: cond-mat.stat-mech

TL;DR: 本文研究了Sierpinski垫片（一种豪斯多夫维度为log_2 3的四配位层级分形）上的六顶点模型，通过递归计算配分函数和相关函数，获得了包含多种不同状态的丰富相图。冰模型中表现出熵驱动的电荷禁闭，而特定的四顶点极限呈现分形解除禁闭现象，连接退禁闭电荷的弦本身是一个统计分形，分形维度约为1.3。文章最后提出了将Sierpinski冰作为人造自旋冰的实验方案。


<details>
  <summary>Details</summary>
Motivation: 六顶点模型是统计力学中的经典模型，其长波行为强烈依赖于系统维度。Sierpinski垫片作为介于二维和三维之间的分形结构（维度d_f=log_2 3≈1.58），为研究维度对模型行为的影响提供了独特平台，但目前对该模型在此类层级分形上的研究尚属空白。

Method: 利用Sierpinski垫片的层级自相似结构，递归计算六顶点模型的配分函数和相关函数，系统地扫描顶点权重参数空间以构建相图。针对冰模型（等权重点）和四顶点极限分别分析电荷禁闭与退禁闭行为，并计算连接电荷的弦的分形维度。

Result: 获得了包含多种不同状态的丰富相图。冰模型表现出熵驱动的电荷禁闭行为；而特定的四顶点极限则呈现分形解除禁闭，退禁闭电荷之间由统计弦连接，该弦的分形维度为d_l=log_2(5/2)≈1.3。

Conclusion: Sierpinski垫片上的六顶点模型展现了由分形几何导致的独特现象，与传统二维或三维系统中的行为截然不同。该工作不仅在理论上揭示了层级分形上的冰模型丰富的物理内涵，还通过人造自旋冰的设计为实验验证提供了可行路径，并可推广到更一般的Sierpinski类分形系统。
...
### [79] [Two dimensional inhomogeneous classical systems at criticality](https://arxiv.org/abs/2608.02903)
*Jean-Marie Stéphan*

Main category: cond-mat.stat-mech

TL;DR: 本文研究二维经典格点模型中缓慢变化的不均匀形变，发现其在全局上可用弯曲时空中的CFT描述，并识别了底层空间度量。以Ising模型和六顶点模型（含畴壁边界条件）为例，后者通过流体动力学求解自由情形，在存在相互作用时确定了分隔临界涨落区域与有序（冻结）相的北极曲线。


<details>
  <summary>Details</summary>
Motivation: 理解不均匀形变下临界格点系统的有效描述，探讨此类系统在长距离下如何由CFT刻画，并识别其在弯曲空间中的几何结构。

Method: 选取缓慢变化（在格点尺度上）的不均匀形变以保持临界行为；利用CFT在弯曲空间中的框架识别空间度量；借助流体动力学方法求解自由六顶点模型；对于相互作用情形，确定北极曲线以区分临界区与冻结相。

Result: 发现系统全局由弯曲空间中的CFT描述，识别出底层空间度量；在自由六顶点模型中成功求解；在相互作用情形下确定了北极曲线，将临界涨落区域与有序（冻结）相分离。

Conclusion: 不均匀形变的临界格点模型等价于弯曲时空中的CFT，这一框架为理解复杂边界条件下（如畴壁边界条件）的临界现象提供统一几何描述，并通过北极曲线刻画相分离结构。
...
### [80] [Exact Resonances Are Not Sufficient for Phonon Energy Diffusion](https://arxiv.org/abs/2608.03180)
*Wei Lin,Yong Zhang,Hong Zhao*

Main category: cond-mat.stat-mech

TL;DR: 本文揭示了多声子共振条件（精确共振匹配、非零相互作用系数和网络连通性）并不足以保证声子能量扩散的持续进行。对称性强制平衡关系使精确共振碰撞电流趋于非热零通量态，导致从单个共振集到连通网络的动力学停滞；真正的能量完全传播依赖于准共振。热力学极限与弱非线性极限不可交换，且精确共振的存在性与连通性只是运动学判据而非充分动力学判据。


<details>
  <summary>Details</summary>
Motivation: 声子输运和晶格热化的动力学理论以多声子共振条件为基础，但此前对精确共振是否足以驱动持续能量扩散缺乏严格检验。作者旨在探究精确共振匹配、非零相互作用系数和网络连通性这三要素是否构成声子能量扩散的充分条件。

Method: 通过理论分析与对称性论证，研究精确共振碰撞电流的平衡关系；对比精确共振与准共振在能量扩散中的作用；考察热力学极限与弱非线性极限的可交换性，并分析有限尺寸下非线性趋于零时的热化时间发散与高阶交叉行为。

Result: 发现对称性强制平衡关系使精确共振碰撞电流流向非热零通量态，导致动力学停滞从个体共振集扩展到连通网络；仅准共振能维持完整的能量传播；热力学极限和弱非线性极限不可交换，前者恢复主导动力学行为，后者在固定有限尺寸下热化时间通过高阶交叉发散。

Conclusion: 精确共振的存在性与网络连通性仅为运动学判据，不足以作为声子能量扩散的充分动力学条件；准共振是能量传播的关键，且极限顺序对热化动力学行为具有决定性影响。
...
### [81] [Discrepancy between the H-Function and Entropy: Insights into the rigorously established Boltzmann equation for hard-sphere gases](https://arxiv.org/abs/2608.03355)
*Li-Xiang Cen*

Main category: cond-mat.stat-mech

TL;DR: 本文通过研究H函数与熵之间的演化差异，阐明了在玻尔兹曼-格勒极限下，对于硬球气体模型严格推导出的玻尔兹曼方程中，H函数不能作为有效的时间箭头判据。


<details>
  <summary>Details</summary>
Motivation: 经典玻尔兹曼H定理长期以来被视为时间箭头的重要判据，但H函数与热力学熵之间在严格推导的玻尔兹曼方程框架下存在演化不一致性，需要澄清H函数作为时间箭头判据的有效性。

Method: 对比分析H函数与熵的演化行为，在玻尔兹曼-格勒极限下对硬球气体模型严格推导玻尔兹曼方程，并从数学上考察H函数是否满足时间箭头判据的要求。

Result: 在严格推导的硬球气体玻尔兹曼方程中，H函数与熵的演化存在本质差异，H函数不满足作为有效时间箭头判据的条件。

Conclusion: H函数不能作为严格玻尔兹曼方程框架下有效的时间箭头判据，这对于重新审视统计力学的宏观不可逆性与时间方向性基础具有重要意义。
...
### [82] [The intermediate scattering function of an interacting adlayer as a characteristic function: a closed-form theory of Ising lattice-gas surface diffusion](https://arxiv.org/abs/2608.03398)
*S. Miret-Artés*

Main category: cond-mat.stat-mech

TL;DR: 本文利用氦自旋回波测量的中间散射函数（ISF）作为特征函数的双重角色，将伊辛晶格气体型横向相互作用引入表面扩散。通过在动量转移、时间、覆盖度和温度下给出ISF的闭式解，推导出完整的线性响应层级。该闭式解不是假设的，而是通过对主方程投影得到的精确运动方程简化而来。一个相关参数同时决定平衡结构和相互作用重整化的跳跃率，在二维表面上通过最近邻相关的对近似（pair approximation）和集体的随机相位处理推广。KMC验证显示跳跃率和初始集体速率误差分别小于1%和1-2%。文中给出四个不相关、非临界的相互作用指纹，并应用于Na/Cu(111)和H/Pt(111)体系。


<details>
  <summary>Details</summary>
Motivation: 传统表面扩散研究通常忽略吸附质间的横向相互作用，或只能采用数值模拟方式处理。作者希望将伊辛晶格气体型横向相互作用以解析闭式形式纳入表面扩散理论，从而能够直接由氦自旋回波实验数据反演相互作用参数和动力学参数，弥合理论闭式解与实验测量之间的鸿沟。

Method: 利用ISF作为特征函数的双重角色（零时刻值为静态结构因子），将伊辛晶格气体相互作用纳入表面扩散。通过投影主方程到密度得到精确的运动方程，其中振幅满足恒等式、初始速率满足求和规则，从而保证准弹性权重就是吸附层自身的结构因子，de Gennes窄化成为定理。在舍弃存储核后得到闭式解。在一维通道中精确成立，二维表面通过最近邻相关的对近似引入，结构因子的动量依赖性由集体随机相位处理给出。用KMC验证闭式解准确性。

Result: 得到了ISF在动量传递、时间、覆盖度和温度下的闭式表达式。KMC验证表明：重整化跳跃率的误差优于1%，初始集体速率的误差在1-2%。给出四个相互关联且非临界的相互作用指纹：准弹性振幅、线宽、Arrhenius斜率以及拟合线宽对拟合时间窗口的依赖性，它们可闭式反演得到尝试频率、静态势垒和相互作用参数。并将理论应用于Na/Cu(111)和H/Pt(111)体系进行分析。

Conclusion: 该工作建立了从氦自旋回波实验数据中解析提取横向相互作用和动力学参数的完整理论框架。闭式解不是人为假设的拟合形式，而是由主方程投影导出的精确运动方程的近似结果（丢弃存储核），具有坚实的理论根基。KMC验证表明其准确性高，四个相互作用指纹为实验分析提供了可操作的解析工具。
...
### [83] [Lower bounds on entropy production from dynamical correlation functions](https://arxiv.org/abs/2608.03619)
*Paul Raux,Alexander M. Maier,Udo Seifert*

Main category: cond-mat.stat-mech

TL;DR: 本文推导了基于粗粒化态可观测量两时间相关函数非对称性的熵产生下界，涵盖非平衡稳态（任意相关时间滞后均有效）和时间依赖过程（需滞后趋近于零）两种情形，适用于底层马尔可夫动力学或过阻尼朗之万方程系统。


<details>
  <summary>Details</summary>
Motivation: 在随机热力学中，熵产生是关键属性，但对于部分观测和粗粒化系统，其推断具有挑战性，通常依赖于已被证明的下界。本文旨在提出新的基于实验可获取的两时间相关函数非对称性的熵产生下界。

Method: 作者从粗粒化状态可观测量中提取两时间相关函数的非对称性，推导出两种版本的熵产生下界。对于非平衡稳态，下界对任意相关滞后时间均有效；对于时间依赖过程，下界在滞后趋近于零的极限下成立。证明适用于底层马尔可夫动力学或耦合的过阻尼朗之万方程系统。

Result: 得到了两种基于两时间相关函数非对称性的熵产生下界，分别在非平衡稳态（任意滞后）和时间依赖过程（零滞后极限）条件下成立，并通过两类动力学系统验证了这些下界的有效性。

Conclusion: 这些下界为部分观测和粗粒化系统的熵产生推断提供了新的可行途径，作者进一步讨论了这些下界的优化策略和潜在紧致性。
...
### [84] [Kappa distributions as asymptotic marginals of exponential family ensembles](https://arxiv.org/abs/2608.03960)
*Sergio Davis*

Main category: cond-mat.stat-mech

TL;DR: 本文基于先前关于驱动经典系统中超统计行为的研究结果，证明了当N→∞时，总动能服从逆伽马分布的N粒子系统，其单粒子速度必然服从kappa分布。该工作为kappa分布提供了非广延统计力学之外的理论视角，并提出了一种通过逆伽马系综中蒙特卡洛Metropolis模拟生成kappa速度的实用方法。


<details>
  <summary>Details</summary>
Motivation: 近年来超统计行为在驱动经典系统中的涌现已被证实（用于具有恒定微正则热容量的系统）。本文旨在将该理论结果应用于具体物理场景，揭示kappa分布的深层本质，并发展一种不依赖非广延统计力学框架的实用生成方法。

Method: 利用先前关于超统计行为涌现的理论结果（Physica A 660 130370, 2025），将逆伽马分布的总动能分布与超统计框架联系起来，在N→∞极限下推导单粒子速度的kappa分布。同时提出在逆伽马系综中采用蒙特卡洛Metropolis模拟来生成kappa速度分布。

Result: 证明了在N→∞极限下，总动能服从逆伽马分布的N粒子系统中，单粒子速度必然呈现kappa分布特征。

Conclusion: 该工作为kappa分布在非广延统计力学之外的存在提供了理论依据，为理解kappa分布的本质提供了新的视角，并提出了一种实用的蒙特卡洛Metropolis方法用于生成kappa速度分布。
...
### [85] [Typical Output States of Monitored Random Clifford Circuits: A Graph-Theoretic Approach](https://arxiv.org/abs/2608.03102)
*Yu-Xuan Zhang,Yu-Xiang Zhang*

Main category: cond-mat.stat-mech

TL;DR: 本文开发了基于图态的框架来研究受监测随机Clifford电路，在热力学极限下证明随机稳定子图的图表示收敛于Erdős–Rényi随机图系综G(N,1/2)，据此解决了随机稳定子态中GHZ纠缠量的开放问题，揭示了体积律相中输出态存在G(N_sub,1/2)形式的稠密子图（可解释量子纠错能力），并在图网络上通过平均场论证精确定位了测量诱导相变临界点p_c=0.1608。


<details>
  <summary>Details</summary>
Motivation: 受监测随机Clifford电路是研究非平衡量子多体动力学的重要平台，其展示的测量诱导相变（MIPT）备受关注。然而，对典型输出态的图结构缺乏直接访问方法，随机稳定子态中GHZ纠缠量问题尚未解决，且体积律相中量子纠错能力的微观起源不清楚。

Method: 提出基于图态的框架：证明大N极限下随机稳定子态的图表示收敛到Erdős–Rényi随机图系综G(N,1/2)；据此解析推导平均GHZ含量；对一维受监测电路在体积律相输出态图中发现稠密子图结构G(N_sub,1/2)；识别稠密子图沿一维链的空间聚类效应并用感染-恢复玩具模型重现；最终通过图上的平均场论证定位MIPT临界点。

Result: 解析得到平均GHZ含量<g₃>=1.204（偶数N）和1.325（奇数N）；发现输出态等价于N_sub量子比特上幺正电路输出加少量纠缠的微扰；用感染-恢复模型重现空间聚类效应（表现为测量诱导吸收态相变）；平均场给出临界点p_c=0.1608，与数值结果p_c≈0.16高度一致。

Conclusion: 图态框架为理解受监测Clifford电路的典型输出态提供了统一图论视角，不仅解决了GHZ纠缠量的开放问题，还从图结构上解释了体积律相的量子纠错能力，并通过图上的平均场论证精确预测了MIPT临界点，展示了图表示作为分析量子多体非平衡动力学的强大工具。
...
### [86] [Hydrodynamic description of proliferating active matter](https://arxiv.org/abs/2608.03317)
*Nathan Silvano,Emilio Hernandez-Garcia,Cristóbal López*

Main category: cond-mat.stat-mech

TL;DR: 本文从微观随机模型出发，为增殖型活性物质建立了连续介质理论。通过主方程推导出粒子密度和极化场的平均场演化方程，并用von Mises假设闭合层次，得到耦合流体动力学描述。以Active Brownian Bug模型为例，线性稳定性分析预测了Turing和Hopf不稳定性，其理论阈值与数值模拟一致。该连续模型重现了均匀态、静止周期性团簇和相干传播的成群态等主要动力学行为。


<details>
  <summary>Details</summary>
Motivation: 增殖型活性物质系统（如生物群体中的细胞和组织）广泛存在，但缺乏从微观模型出发的系统性连续介质理论框架。已有的活性物质理论大多关注守恒系统，未充分考虑出生、死亡和非局部竞争等人口动力学因素的影响，因此需要建立一个能够统一描述增殖与活性相互作用的连续体理论。

Method: 从自驱动粒子经历出生、死亡和非局部竞争的微观随机模型出发，基于主方程推导粒子密度和极化场的平均场演化方程；通过von Mises分布假设闭合BBGKY层次，得到耦合流体动力学方程组；随后将此框架应用于Active Brownian Bug模型，进行线性稳定性分析（识别Turing与Hopf不稳定性）并与直接数值模拟进行对比验证。

Result: 理论预测的Turing和Hopf不稳定性阈值均与数值模拟定量吻合；连续模型成功重现了粒子系统的主要动力学状态，包括均匀态、静止周期团簇和相干传播的成群态。这些结果验证了所提出连续体框架的有效性和普适性。

Conclusion: 该工作建立了增殖型活性物质的一般性连续介质理论框架，揭示了在缺乏显式对齐相互作用的情况下，活性与种群动态的相互作用可以自发产生集体运动（成群行为），为理解此类系统的物理机制提供了理论工具和物理解释。
...
TL;DR: 本文研究了一维Maxwell-Cattaneo-Vernotte（MCV）热传导模型的数值初始化问题，系统评估了三种初始化策略（零导数、空间均匀非零导数、精确空间相关导数）。结果表明，在交错有限差分格式下，只有将精确的空间相关初始时间导数映射到交错网格，才能保持初始状态的局部热力学结构，产生与解析解（Galerkin方法）一致的瞬态响应，而零导数或均匀导数假设会导致非物理振荡和热流预测误差。


<details>
  <summary>Details</summary>
Motivation: 在非Fourier热传导模型（如MCV模型）中，如何数学上定义非平衡状态的初始时间导数是一个关键挑战。传统时间积分方案和商业有限元求解器在处理非Fourier模型时经常引入严重的数值伪影，因此需要一种热力学一致的初始化技术。

Method: 采用显式交错有限差分格式，对指数分布的初始温度场在绝热边界条件下进行瞬态模拟，比较三种初始化策略（零导数、空间均匀非零导数、精确空间相关导数），并以Galerkin方法的解析解作为基准。

Result: 零导数或空间均匀初始导数假设会引入显著的非物理振荡偏差，且随着弛豫时间增加，热流预测误差增大；精确空间相关导数映射到交错网格后，瞬态响应与解析基准吻合良好，且无显著额外计算开销。

Conclusion: 建立了一种热力学一致的初始化技术，可有效消除非Fourier热传导模拟中的数值伪影，且该方法可推广到非局域模型。
...
TL;DR: 该论文将多个互不交叠的有向聚合物在淬火随机介质中的有限密度问题重述为填充单聚合物传递矩阵乘积的对数特征值，从而获得淬火自由能、累积量及与replica预言一致的无序诱导线性谱边缘。


<details>
  <summary>Details</summary>
Motivation: 互不交叠的有向聚合物在淬火随机介质中是玻璃态线状物质的最小模型。复本Bethe ansatz给出了稀释理论的解析预言，但直接数值验证因多聚合物传递矩阵的组合爆炸而受阻，亟需新的计算方法验证理论预言。

Method: 将问题重述为填充单聚合物传递矩阵乘积的对数特征值，通过这种方式避免构建巨大组合规模的多聚合物传递矩阵，从而高效计算淬火自由能及其累积量，并提取谱边缘信息。

Result: 获得了淬火自由能及其累积量，并发现了一种无序诱导的线性谱边缘，该结果与replica的预言一致。

Conclusion: 所提出的特征值填充方法成功绕过了多聚合物传递矩阵的组合爆炸难题，为验证关于相互作用自由能与无序累积量的解析理论预言提供了一条可行的数值途径。
...
TL;DR: 本文针对具有柯西分布对角无序的无限连通Bethelattice，提出了典型介质理论（TMT）的数值与解析相结合的实现方案，推导出典型态密度（TDOS）的解析表达式和简化的TMT自洽方案，并建立了ω–W相图。


<details>
  <summary>Details</summary>
Motivation: Anderson局域化在无限连通Bethe格上的研究具有重要的理论意义，但传统TMT方法通常依赖复杂数值计算。利用柯西分布和Bethe格的特殊性质，可以简化计算并获得解析结果，从而更清晰地揭示无序强度对局域化转变的影响。

Method: 作者利用柯西分布的特殊性质（稳定分布、无高阶矩）和Bethe格的无限连通结构，推导出典型态密度（TDOS）的闭式解析表达式，并将TMT自洽方程简化为一个更易处理的方案。通过中心带处TDOS对无序强度的依赖关系分析临界行为。

Result: TDOS在带中心随无序强度线性递减，并在临界无序强度W_c=D/2处消失为零。ω–W相图展示了迁移率边随无序增强的连续坍缩以及扩展态的最终消失。

Conclusion: 该工作为Bethe格上含柯西无序的Anderson局域化提供了完全解析的TMT框架，明确了扩展态消失的临界条件，加深了对无限连通体系中局域化转变本质的理解。
...
### [90] [Two-Parameter Ansatz for the Violation of Eigenstate Thermalization](https://arxiv.org/abs/2608.02744)
*Kohei Ohgane,Lev Vidmar*

Main category: cond-mat.dis-nn

TL;DR: 本文提出一个统一的二参数框架来描述ETH破缺的连续演化，区分了"褪色遍历性"（fading ergodicity）和"受困遍历性"（trapped ergodicity）两种机制，并在自旋-1/2 J1-J2链的局域无序和线性势两种情形下进行了验证，发现观测到的ETH破缺与受困遍历性一致。


<details>
  <summary>Details</summary>
Motivation: 本征态热化假说（ETH）是理解孤立多体系统量子热化和遍历性的主流框架，但目前缺乏一个通用的理论来描述ETH破缺从常规ETH到完全破缺之间的连续演变过程。

Method: 引入一个二参数ansatz来刻画ETH破缺，将破缺机制统一并区分为两种：褪色遍历性（fading ergodicity）和受困遍历性（trapped ergodicity）。前者描述既有的遍历性破缺路径，后者描述有限系统中ETH破缺但在热力学极限下恢复的新场景。随后在自旋-1/2 J1-J2链中分别引入局域无序和线性势进行测试。

Result: 在J1-J2链的局域无序和线性势两种情形下，观测到的ETH破缺均与受困遍历性机制一致，而非褪色遍历性。

Conclusion: 该二参数ansatz为ETH破缺的连续转变提供了统一描述框架，揭示了受困遍历性作为一种区别于传统遍历性破缺的新机制，在有限系统中ETH可以被破坏但在热力学极限下得到恢复，拓展了对量子热化和遍历性破缺的理解。
...
### [91] [Nonlocality-induced critical-length hierarchy from non-Hermitian competition](https://arxiv.org/abs/2608.02746)
*Mengjie Yang,Alexander N. Poddubny,Ching Hua Lee*

Main category: cond-mat.dis-nn

TL;DR: 本文研究非厄米晶格中的谱跃迁，发现长程跳跃会从根本上重构两链耦合系统中皮肤累积与杂化竞争所导致的临界行为，产生一系列不同的标度律：混合耦合幂律衰减时临界长度呈代数增长N_c~D^(α/3)；链内跳跃也为幂律时，对α<2进入尺度协变区间（临界阈值仅依赖宽高比N_c/D），α=2及以后分别进入边际对数修正和代数修正区间。作者识别了两种新的非局域机制并表明非局域性系统地消除了常规临界行为所依赖的物理长度尺度。


<details>
  <summary>Details</summary>
Motivation: 理解非厄米晶格中短程系统（双链耦合）中皮肤累积与杂化竞争如何导致对数临界长度定律，并探究长程跳跃如何重构这一临界行为，揭示标度律的全新层级结构。

Method: 作者通过理论分析研究双链非厄米系统，分别考虑两种情形：(1)仅杂化耦合为幂律衰减（指数α），导致代数临界律N_c~D^(α/3)；(2)链内跳跃与杂化耦合均为幂律衰减，对于α<2进入尺度协变区，α=2及以后分别出现对数修正和代数修正的标度行为。同时识别了两种新机制：长程链内跳跃引起的非解析带边色散，以及非互易性诱导的奇偶混合杂化。

Result: 发现长程跳跃产生层级化的临界标度律：代数律N_c~D^(α/3)、尺度协变律（依赖N_c/D）、边际对数修正律和代数修正律，取代了短程系统的对数定律N_c~ln D。识别了两种非局域机制，并证明非局域性消除了传统皮层深度这一物理长度尺度。

Conclusion: 长程性系统地消除了非厄米皮肤行为的常规物理长度尺度（皮肤深度），提供了平台无关的理论框架，可在可编程拓扑电路、光子晶格和数字量子模拟器中验证。
...
### [92] [Chaos and Diffusion in Twisted Bilayer Graphene](https://arxiv.org/abs/2608.02916)
*Olga Arroyo-Gascón,Manuel Pino*

Main category: cond-mat.dis-nn

TL;DR: 本文数值分析了扭转双层石墨烯中的混沌与扩散行为，发现无论旋转角度是否相称，有限系统中的电子均表现出混沌特性，并揭示了边界诱导散射是比单层石墨烯强得多的单粒子散射机制。


<details>
  <summary>Details</summary>
Motivation: 理解扭转双层石墨烯中非相互作用电子的散射机制，探究边界效应在系统混沌和扩散过程中所起的作用，并比较其与单层石墨烯的差异。

Method: 采用数值方法对相称和非相称旋转角度的扭转双层石墨烯有限系统进行分析，通过计算Thouless能量来研究中间扭转角度下的扩散过程，并分析平均自由程与系统尺寸的标度关系。

Result: 相称和非相称旋转角度的有限系统均表现出混沌行为；平均自由程随系统尺寸线性标度，表明存在由边界诱导散射引起的扩散过程；边界效应作为单粒子散射机制远强于单层石墨烯。

Conclusion: 边界在扭转双层石墨烯中扮演了实验上重要的单粒子散射机制，可诱导混沌和扩散，且该效应独立于旋转角度和晶格相称性。
...
### [93] [Fibonacci number systems and the localization criterion in the many-body Aubry-André model](https://arxiv.org/abs/2608.03458)
*Balázs Hetényi*

Main category: cond-mat.dis-nn

TL;DR: 本文引入了一种分数阶斐波那契数制，它介于用于自然数的传统斐波那契数制和用于实数的不规则基数-φ数制之间。该数制可用于解释多体非相互作用Aubry-André模型的局域化相图，在有限尺寸下粒子密度用该数制表达可得到局域化判据，在热力学极限下判据仍成立但粒子密度改用基数-φ表达。


<details>
  <summary>Details</summary>
Motivation: 需要一种统一的数制框架来同时处理自然数（斐波那契数制）和实数（基数-φ数制）的表示，并以此解释Aubry-André模型中的局域化-退局域化相变。

Method: 提出分数阶斐波那契数制作为两种已有数制之间的插值框架，将其应用于多体非相互作用Aubry-André模型的粒子密度表示，从而在有限尺寸和热力学极限下推导局域化判据，并讨论热力学极限的性质。

Result: 在有限系统尺寸下，通过将粒子密度写成分数阶斐波那契数制可获得局域化判据；在热力学极限下该判据仍然成立，但此时粒子密度需以基数-φ数制表示。此外对热力学极限的本质进行了讨论。

Conclusion: 分数阶斐波那契数制成功连接了两种已知数制，并为Aubry-André模型的局域化相图提供了统一的数学解释框架，展示了数论与凝聚态物理之间深刻的联系。
...
### [94] [A Hyperfinite Framework for Score-Based Generative Modeling](https://arxiv.org/abs/2608.02799)
*Sunder Ram Krishnan*

Main category: stat.ML

TL;DR: 本文在非标准分析框架下提出了一种基于超有限网格的分数基生成建模（score-based diffusion）的统一形式化方法。作者从超有限网格上的内扩散过程出发，推导出相应的无穷小生成元，并建立了其与经典Fokker-Planck方程之间的对应关系；进一步获得了超有限反向均值恒等式，从而构造性推导出反向时间SDE；通过内评分匹配目标最小化恢复分数函数，并推导了超有限Girsanov公式以建立似然优化与Fisher散度目标之间的关联。最后分析了二阶一致性，指出修正项显式依赖于增量分布的四阶矩，且高斯值κ=3可消除主导色散贡献。


<details>
  <summary>Details</summary>
Motivation: 现有基于分数的扩散模型通常采用连续时间SDE和测度论随机微积分来表述，这种方法在数学框架上较为抽象且与离散网格实现之间存在概念鸿沟。作者希望通过非标准分析（Nonstandard Analysis）中的超有限（hyperfinite）框架，将离散网格动力学、反向扩散、分数匹配和似然表述统一在一个共同的数学设定中，从而为扩散生成模型提供一个更统一且可扩展的理论基础。

Method: 作者在非标准分析框架下进行推导：(1) 在超有限网格上构造内扩散过程，并推导其无穷小生成元，建立其与经典Fokker-Planck方程的对应；(2) 获得超有限反向均值恒等式，构造性地推导反向时间SDE的漂移项；(3) 提出内部score-matching目标，通过最小化该目标恢复分数函数；(4) 推导超有限Girsanov公式，建立似然优化与Fisher散度目标的联系；(5) 分析超有限动力学的二阶一致性，揭示前导修正项与增量分布四阶矩的关系。

Result: 作者在超有限水平上完成了分数估计与生成采样之间的直接连接（无需经过连续极限），推导出超有限反向SDE和Girsanov公式，并证明了似然目标与Fisher散度在超有限框架下的等价关系。二阶一致性分析表明，前导修正项由增量分布的四阶矩决定，当增量分布为高斯分布（κ=3）时该主导色散贡献可被消除，这为选择特定噪声分布提供了理论依据。

Conclusion: 本文提供了一个统一的超有限框架，将离散网格动力学、反向时间扩散、分数匹配和似然式表述纳入同一非标准设定之中，为扩散生成建模奠定了理论基础，并为后续扩展（如处理更一般分布或更复杂动力学）铺平了道路。
...
### [95] [Particle-based Generalised Stochastic Optimisation](https://arxiv.org/abs/2608.02844)
*Jiechen Jackie Zhang,O. Deniz Akyildiz*

Main category: stat.ML

TL;DR: 本文提出了一类基于扩散的随机粒子优化方法，用于处理损失函数梯度不可解析计算的问题（梯度是参数依赖分布的积分）。引入平均场动力学及其交互粒子近似，该框架包含多种现有算法作为特例，并支持构建新方法。在适定性和联合收缩性假设下，证明了指数收敛性和连续时间粒子系统的非渐近误差界，并开发了动量和高阶Langevin变体。


<details>
  <summary>Details</summary>
Motivation: 许多机器学习问题（如生成模型训练、微调、潜变量模型学习）中的损失函数梯度难以解析求解，因为梯度是参数依赖分布的积分形式。现有的粒子优化方法缺乏统一的理论框架，需要一种通用方法及其收敛性保证。

Method: 提出了基于扩散的随机粒子优化方法类。核心包括：(1) 引入平均场动力学描述粒子系统的极限行为；(2) 构造交互粒子近似算法，统一了多个现有算法并为新方法提供构建路径；(3) 推导了连续时间粒子系统的指数收敛性定理与非渐近误差界；(4) 基于该框架开发了动量Langevin和高阶Langevin变体算法。

Result: 在适定性与联合收缩性假设下证明了指数收敛性，并给出了连续时间粒子系统的非渐近误差界。通过最大边际似然估计和基于能量的模型训练两项任务，验证了所提出的动量及高阶Langevin变体的有效性。

Conclusion: 该工作提供了一个统一的粒子优化理论框架，不仅统一了现有算法，还为设计新的梯度不可解析问题的优化方法提供了系统化途径，并辅以严格的收敛性理论保证。
...
### [96] [Causal Inference with Unstructured Outcomes](https://arxiv.org/abs/2608.03085)
*Kevin Christian Wibisono,Yixin Wang*

Main category: stat.ML

TL;DR: 该论文提出了一种针对非结构化结局（如文本、图像）的因果推断查询方法——最大化对比特征（MCF），通过学习特征打分函数将结果映射为标量，从而捕捉处理组与对照组之间最显著的特征差异。


<details>
  <summary>Details</summary>
Motivation: 传统因果推断聚焦于标量结局（如恢复与否、收入多少），但现代研究越来越多地关注非结构化结局（如临床笔记、开放式调查回复、图像）。对于此类结局，平均处理效应（ATE）无法定义，因为不能对文本或图像进行有意义的减法运算。

Method: 提出最大化对比特征（MCF）作为因果查询；学习一个特征打分函数，将每个非结构化结局映射为标量，以最尖锐地暴露处理组与对照组潜在结局之间的对比。通过允许打分函数依赖观测协变量来扩展至异质性效应，并支持处理和结局均为非结构化的情况。同时开发了识别条件和估计算法。

Result: 在文本和图像上的实证研究表明，算法能够恢复由处理改变的结果中最突出的方面。

Conclusion: 该工作为因果关系推理在非结构化结局上的应用提供了新的查询与估计框架，弥补了传统平均处理效应在非结构化数据上的不足。
...
### [97] [Minimax-Optimal Semiparametric Contextual Dynamic Pricing with Multimodal Revenue](https://arxiv.org/abs/2608.03142)
*Xueping Gong,Zhuoluo Zhang,Zhaowei Miao,Jiheng Zhang*

Main category: stat.ML

TL;DR: 本文研究具有任意协变量序列和有界（可能非二元）购买数量的语境动态定价问题。需求遵循半参数盈余指数模型，包含未知线性估值参数和未知Hölder光滑响应函数。作者提出了一种"试点校正分层决策划分"策略，结合方向性试点估计、局部多项式学习、可预测数据分配和全局动作消除，在无需凹性或强单峰性假设（允许非唯一最优价格）的情况下，达到光滑度依赖的极小极大最优时限率（对数因子内）。


<details>
  <summary>Details</summary>
Motivation: 现有语境动态定价研究通常对收益函数施加凹性或强单峰性假设，或限制购买数量为二元，且未能在一般光滑度条件下获得匹配上下界的策略。本文旨在放宽这些限制，在任意协变量序列和半参数需求模型下建立平滑度依赖的极小极大最优定价策略。

Method: 提出"试点校正分层决策划分"策略，包含四个关键组件：(1) 方向性试点估计用于初步估计估值参数；(2) 局部多项式学习用于估计光滑响应函数；(3) 可预测数据分配以确保在自适应采样下的浓度性质；(4) 全局动作消除用于排除次优价格。其中，试点校正消除了估值参数误差的一阶效应，永久标签机制保障自适应采样下的浓度界。

Result: 该策略在任意协变量序列、非凸非强单峰收益及非唯一最优价格情形下，实现了光滑度依赖的极小极大时限率（达对数因子）。同时，对常数语境二元需求子类给出了匹配的下界，证明上界是紧的。

Conclusion: 本文证明了在半参数盈余指数模型下，无需凹性/强单峰性假设即可实现平滑度依赖的极小极大最优动态定价，并给出了匹配的常数语境二元需求下界，确认了结果的紧性。
...
### [98] [Conformal risk control for model-form uncertainty in parametric non-intrusive reduced-order models](https://arxiv.org/abs/2608.03360)
*Edgar Jaber,Rémy Vallot,Thibault Dairay,Mathilde Mougeot*

Main category: stat.ML

TL;DR: 本文提出了一种用于非侵入式降阶模型（NIROM）的模型形式不确定性量化框架，通过结合Stiefel流形上的基扰动随机表示与无分布假设的共形风险控制方法，实现了对降阶模型预测可靠性的量化评估。


<details>
  <summary>Details</summary>
Motivation: NIROM在逼近参数化偏微分方程时计算成本低，但其预测的可靠性评估仍具挑战，尤其是在外推区域或训练数据有限的情况下，现有的不确定性量化方法难以有效度量模型形式（model-form）不确定性。

Method: 从确定性降阶基出发，在Stiefel流形上沿被丢弃的模态方向施加随机扰动，构建随机降阶逼近；利用传输近似（transport approximation）获得闭式后验方差，将基诱导与回归诱导的不确定性分离（无需重新训练高斯过程）；将该后验方差纳入共形风险控制（conformal risk control）校准框架，生成具有坐标误覆盖保证的预测集。

Result: 在参数化PDE基准问题和工业轮胎制造压延工艺上进行了数值验证，结果表明该方法能够提供可靠且局部信息量丰富的不确定性量化，超越了高斯预测方差的表达能力。

Conclusion: 所提框架为NIROM提供了一种可解释的、无分布假设的不确定性量化工具，其校准因子本身可作为不确定度估计质量的标量诊断指标。
...
### [99] [Should the Boundary Term Be Learned in Reflected Diffusion? Conormal Trace and Reflection Masking](https://arxiv.org/abs/2608.03469)
*Ziyue Wang,Takafumi Kanamori*

Main category: stat.ML

TL;DR: 本文研究了有界域上反射扩散的分数学习问题，发现反射虽保证轨迹可行，但无法确保学习到的分数满足前向过程隐含的边界行为。通过隐式分数匹配，分部积分会遗留一个边界项，其依赖于每个边界点上的一个标量：扩散加权的分数法向分量（共法向迹）。无通量条件固定了该值，但其余边界分量不受约束；在各项异性扩散下，该值通常与普通分数法向分量不同。在超矩形上，所提参数化无需额外可训练参数或随机边界估计器即可强制执行所需迹，且在正则性假设下能表示真实分数；而固定错误值会产生数据无法消除的误差。构造可扩展至单纯形和多边形域，并识别了反射掩蔽现象：即使学习迹错误，硬反射仍可使样本保持可行，因此反射后指标可能隐藏误差。实验表明，在反射频率较低、各向异性扩散及约束交集附近质量聚集时分离最明显；在完全反射下，最终样本放置改进不一致，说明硬修复可能掩盖边界分数误差并使分数准确性与下游生成质量脱钩。


<details>
  <summary>Details</summary>
Motivation: 反射扩散在有界域上保持轨迹可行，但现有分数学习方法未考虑反射前向过程所隐含的边界行为。作者希望理解边界项的作用，并设计能正确捕获共法向迹的分数参数化，以避免系统性、无法通过增加数据消除的误差。

Method: 作者从隐式分数匹配出发，推导出分部积分遗留的边界项，证明其由每个边界点上的一个标量（共法向迹）决定。在超矩形上，提出一种无需额外参数的参数化方法强制执行所需迹；将该构造扩展至单纯形和多边形域，并分析"反射掩蔽"现象——即硬反射可能掩盖错误的边界迹学习。

Result: 实验显示，在反射频率较低、各向异性扩散以及质量集中在约束交集附近的条件下，正确与错误的共法向迹处理方法之间差异最为显著。在完全反射场景下，最终样本放置的改进不一致，证实硬修复会掩盖边界分数误差，导致分数准确性与生成质量脱钩。

Conclusion: 反射扩散中的分数学习必须显式处理共法向迹这一边界条件，否则会产生无法由更多数据消除的结构性误差。所提参数化在超矩形等域上可无额外开销地强制执行正确迹，但硬反射的掩蔽效应要求在实际生成评估中谨慎解读反射后指标。
...
### [100] [Divide-and-Conquer: Towards Generalizable Amortized Bayesian Inference for the Drift Diffusion Model](https://arxiv.org/abs/2608.03566)
*Yufei Wu,Shanqing Gao,Andreas Voss,Francis Tuerlinckx*

Main category: stat.ML

TL;DR: 本文提出一种分治框架，通过将漂移扩散模型（DDM）的数据集分解为共享共同结构的成对分片，利用单个神经网络进行摊销贝叶斯推断（ABI），并通过共识MCMC合并各分片的后验分布，在保持与MCMC相当精度和不确定性的同时，将计算成本降低数个数量级。


<details>
  <summary>Details</summary>
Motivation: 研究者需要既快速又灵活的DDM推断方法。摊销贝叶斯推断（ABI）虽然能为复杂随机模型提供近乎即时的推断，但针对一种研究设计训练的神经网络无法泛化到其他设计，这严重限制了ABI的实用性和可扩展性。

Method: 提出分治（divide-and-conquer）框架：利用DDM的独立性假设，将完整数据集分解为成对分片（pairwise shards），各分片共享共同结构，可由单一神经网络学习。对每个分片分别进行推断，再通过共识MCMC（consensus MCMC）合并各分片的后验分布，以近似完整后验。

Result: 在模拟数据集上的评估表明，该方法在精度和不确定性方面与MCMC相当，同时计算成本降低了数个数量级。

Conclusion: 该工作不仅推进了DDM估计方法，还展示了一种提升ABI方法跨应用可扩展性和泛化能力的通用策略。
...
### [101] [Robust Low-Tubal-Rank Tensor Completion under Cross-Concentrated Sampling](https://arxiv.org/abs/2608.03928)
*Hanqin Cai,Longxiu Huang,Jing Qin,Chengyue Wu*

Main category: stat.ML

TL;DR: 本文提出R-ItCUR算法，用于在部分张量十字交叉采样(t-CCS)观测下，对受稀疏任意大异常值污染的三阶低tubal秩张量进行鲁棒恢复。该算法直接在采样十字上操作，通过自适应块状Welsch校正抑制异常值，并用投影块状梯度下降更新低秩成分，避免了全张量重建，节省内存和计算资源。


<details>
  <summary>Details</summary>
Motivation: 现有的t-CCS张量补全方法假设观测值不含严重损坏（即无异常值），但在实际应用中观测数据常受稀疏、任意大的粗差污染，因此需要研究在t-CCS部分观测下对低tubal秩张量的鲁棒恢复方法。

Method: 提出张量原生算法R-ItCUR，将采样的张量十字划分为两个外部块和一个交叉块，应用自适应块状Welsch校正来抑制异常值，并通过投影块状梯度下降更新低秩分量。该算法直接在采样的张量十字上操作，避免迭代中重建完整张量。

Result: 在合成张量、心脏MRI数据和三维地震数据上的实验表明，该方法能实现准确恢复，对稀疏严重损坏具有强鲁棒性，结果强调了在鲁棒张量补全中显式利用十字集中采样结构的重要性。

Conclusion: R-ItCUR在t-CCS采样结构下实现了对异常值污染的低tubal秩张量的高效鲁棒恢复，兼具内存与计算效率，验证了该方法的有效性和实际应用价值。
...
### [102] [Information-Geometric Forward Policy Training in GFlowNets](https://arxiv.org/abs/2608.03967)
*Yordan Raykov,Rodrigo Veiga*

Main category: stat.ML

TL;DR: 本文提出了一种基于信息几何的GFlowNets前向策略训练框架，将前向策略视为诱导轨迹采样器，利用Fisher-Rao度量和自然梯度进行优化，并推导出轨迹Fisher信息的逐条件二阶矩分解，区分了三种计算场景（精确Fisher、蒙特卡洛估计、结构可利用近似），最后通过图模型工具实现了结构感知的训练。


<details>
  <summary>Details</summary>
Motivation: GFlowNets作为离散及混合离散-连续对象上的摊销推断框架虽已广泛应用，但其前向策略训练缺乏基于信息几何的严格理论视角。现有方法未能充分利用目标结构（如局部性、因子分解）来指导优化，导致训练效率和学习行为受限。

Method: 1) 将前向策略视为诱导轨迹采样器，证明其本征一阶几何由轨迹族的Fisher-Rao度量给出；2) 推导轨迹Fisher信息分解为逐条件二阶矩的精确表达式，分析时序分数交互何时消失、何时因共享参数化而保留稠密耦合；3) 提出三种计算场景：精确Fisher、蒙特卡洛估计、以及利用目标局部性或因子分解的结构化近似；4) 在结构化场景中引入精确边缘化、separator方法和信念传播作为自然梯度更新的替代工具。

Result: 通过实证比较Riemannian和Euclidean优化下的收敛性与探索行为，验证了所提框架的有效性，表明将目标结构转化为优化几何可以显著改善前向策略训练的收敛速度和探索质量。

Conclusion: 该框架将目标结构转译为优化几何，为GFlowNets前向策略训练提供了一条可计算的结构感知路径，丰富了GFlowNets在信息几何层面的理论理解与算法设计空间。
...
### [103] [Neural network realization of binary refinement iterates via a two-chart atlas selector](https://arxiv.org/abs/2608.02624)
*Tsogtgerel Gantumur*

Main category: stat.ML

TL;DR: 本文为已知定理（有限标量二元细化算子的迭代可由固定宽度、线性深度增长的ReLU网络精确实现）提供了一种新的构造方法。通过在多边形圆环模型上表示残差动力学，利用两套重叠坐标系（普通和半位移）在精确有效点处切换，避免了变量选择器的乘法，从而实现对任意紧支撑连续分段线性种子的精确ReLU实现。


<details>
  <summary>Details</summary>
Motivation: 细化算子产生的函数在深度学习、小波构造和几何建模中均有应用，其迭代会产生大量线性片段，是测试深度神经网络表达能力的自然场景。此前已有定理证明此类迭代可由ReLU网络精确实现，但本文旨在提供一种新的构造，以更优雅的方式处理细化级联中不连续二进制数字选择与ReLU网络连续性之间的矛盾。

Method: 给定定理的证明采用新构造：1）将残差动力学表示在圆的多边形模型上；2）为每个残差位置引入两套重叠坐标系统（普通坐标和半位移坐标），它们的不连续点位置不同；3）网络仅在两套描述均有效且对应的固定线性级联更新一致的位置进行切换，使切换精确无需乘以变量选择器；4）对局部种子采用两遍（two-pass）网络处理；5）利用平移协变性、有限分解和粘合（gluing）技术将结果推广到任意紧支撑连续分段线性种子。

Result: 新构造给出了对任意连续分段线性圆函数（满足自然端点相容性条件）的精确读出；通过两遍网络处理局部种子，并借助平移协变、有限分解和粘合技术，结果可扩展到任意紧支撑连续分段线性种子，且在保留的支撑窗口内保持精确。

Conclusion: 本文提供了一种全新的、更简洁的ReLU网络实现细化迭代精确计算的构造方法，消除了对变量选择器乘法的依赖，为理解深度神经网络对细化过程的表达提供了新视角。
...
TL;DR: 本文提出GeoID-PINN，一种用于SIRD流行病动力学的物理信息神经网络，通过行随机源组成矩阵建模区域间空间依赖，并利用距离、邻接、通勤或领先-滞后信息构建先验进行正则化。模拟实验表明，轨迹拟合良好并不能保证区域依赖结构恢复正确；在路易斯安那州64县的COVID-19回顾性评估中，模型相比自回归负二项基线显著提升了点预测精度（MSE从32,957降至11,468）。


<details>
  <summary>Details</summary>
Motivation: 区域监测数据混杂了本地传播、报告偏差、外部输入种子和感染压力等因素，难以分别识别。现有方法无法有效区分这些成分，需要一种能够建模空间依赖结构并识别区域间传播关系的模型。

Method: GeoID-PINN将SIRD动力学嵌入物理信息神经网络，用行随机源组成矩阵表示空间依赖（每行权重非负且和为1），并通过距离、邻接、通勤或领先-滞后信息构造空间先验进行正则化。在四区域模拟中验证真相已知情况下的识别能力，并在64个路易斯安那县COVID-19数据上做回顾性评估，同时设置15县受控比较检验邻接先验的增益。

Result: 模拟实验：兼容距离先验下源组成误差0.099，无正则化时0.159，强错误指定先验下升至0.577，但轨迹拟合和传播尺度估计保持相似。COVID-19数据：相对于自回归负二项基线，MSE从32,957降至11,468，MAE从70.60降至57.73；基线NLL更低（5.158 vs 5.346），分布拟合更好但点精度更差。15县比较中，县邻接先验使MSE降低6.85%、MAE降低3.1%。

Conclusion: 准确的轨迹拟合并不保证区域依赖结构的正确恢复；结构化正则化对模型有益，但多个合理先验产生相似性能意味着无法唯一恢复边结构。在解释结果之前必须进行先验敏感性和观测模型检查。
...
### [105] [Can Training Logs Make Model Comparisons More Precise?](https://arxiv.org/abs/2608.02705)
*Wei-Jung Huang*

Main category: stat.ML

TL;DR: 本文研究是否可以利用训练日志中的协变量来提高随机训练模型比较的精度。通过臂特定（arm-specific）协变量调整方法，利用各模型自身训练日志中的早期统计量来减小性能比较的置信区间。在跨3个架构和3个数据集的视觉实验中，简单的早期训练日志调整通常能降低比较的不确定性，但广泛的协变量搜索会引入过大的选择噪声，反而降低精度。


<details>
  <summary>Details</summary>
Motivation: 比较随机训练模型时，需要从重复运行中估计性能差异及其不确定性。现有方法仅使用最终性能指标，忽略了训练过程中产生的大量日志信息。作者希望探索这些训练日志是否能为模型比较提供额外的精度，从而减少为获得足够统计功效所需的重复运行次数。

Method: 提出臂特定协变量调整方法：每个模型仅使用自身运行产生的训练日志统计量进行调整（而非跨运行共享协变量），并保持原始均值差异作为报告效应。该方法的关键在于只用每个模型自己的运行数据计算协变量，然后通过协变量调整减小方差。研究还对比了简单调整（选择少数早期训练统计量）与广泛搜索（从日志池中寻找最相关的统计量）两种策略。

Result: 在涵盖三种架构和三个数据集的视觉任务研究中，基于早期训练日志的简单调整通常能有效减少模型比较中的不确定性。但广泛搜索日志池中相关性最高的统计量往往引入过大噪声，即使事后知道存在有用的统计量，也会导致精度下降而非提升。

Conclusion: 训练日志在模型比较中具有一定的实用价值，能够提高比较精度，但前提是调整策略需避免过大的选择噪声。简单、受限的协变量选择优于广泛搜索。这一发现强调了在利用训练日志信息时，协变量选择策略的重要性。
...
### [106] [DAIF: A Data-Driven Intermediate Fusion Framework for Multimodal Supervised Learning via Approximate Message Passing](https://arxiv.org/abs/2608.02769)
*Sagnik Nandy,Samriddha Lahiry,Pragya Sur,Subhabrata Sen*

Main category: stat.ML

TL;DR: 本文提出DAIF（数据自适应中间融合框架），通过随机矩阵理论和非参数依赖度量从数据中学习多模态融合结构，利用贝叶斯多模态因子模型下的聚类先验估计和近似消息传递（AMP）去噪框架，实现跨模态信息增强与模态特有信号保留，在多模态监督学习任务中优于或匹敌现有最先进方法。


<details>
  <summary>Details</summary>
Motivation: 多模态监督学习中，融合粒度的选择（过度融合会放大噪声，融合不足则无法利用跨模态依赖）是一个核心挑战。现有方法依赖预先指定的融合架构（从早期融合到晚期融合），无法自适应地匹配模态间的依赖结构。

Method: DAIF框架包括：(1) 使用贝叶斯多模态因子模型，其中潜在因子的先验决定跨模态依赖；(2) 基于非参数依赖度量估计模态间依赖并对模态进行聚类；(3) 对每个聚类进行经验贝叶斯先验估计；(4) 在近似消息传递（AMP）框架下利用估计先验构建去噪器，生成降维特征。这些嵌入用于下游监督预测。

Result: 通过在不同依赖结构和信号强度下的模拟实验以及与多种基准方法的对比，DAIF在TEA-seq三模态数据集（T细胞分化标志蛋白表达预测）和TCGA-BRCA多模态数据集（患者生存预测）上均匹敌或优于现有最先进技术。

Conclusion: DAIF通过数据驱动的融合结构学习，在多样化的多模态监督学习任务中展现出色的通用性和竞争力，有效解决了融合粒度自适应问题。
...
### [107] [Tight Information Complexity of the Coin Problem in the Broadcast Model](https://arxiv.org/abs/2608.02776)
*Hadi Kazemi,Varun Jog*

Main category: stat.ML

TL;DR: 本文研究了广播（共享黑板）模型下伯努利分布 Ber(α) 与 Ber(β) 的分布式假设检验问题，完整刻画了常数优势协议在两种假设下的信息复杂度（差一个常数因子），识别出三个参数区间并构造了基于干净样本、二元对称信道和不对称 Z 信道的三种最优协议；还推广到任意离散分布的检验，并通过优化问题加以刻画。


<details>
  <summary>Details</summary>
Motivation: 分布式假设检验是分布式信息理论的核心问题，而广播模型（共享黑板模型）是通信复杂性中的重要模型。理解在不同分布假设下进行 Bernoulli 分布检验所需的信息量，对于设计高效协议以及建立通信复杂性下界至关重要。此前缺乏对信息复杂度的完整刻画，特别是在常数优势协议下、两个假设之间的信息成本可能存在显著差异。

Method: 作者通过以下方法实现刻画：(1) 针对 Ber(α) 与 Ber(β) 的检验问题，利用信息论框架定义协议在两种假设下的信息复杂度，并证明其与常数因子下的精确表征；(2) 根据参数 β<α 的不同关系识别出三个参数区间，分别对应三种最优协议：干净样本协议、二元对称信道（BSC）噪声协议和不对称 Z 信道协议；(3) 提出一种全新的混合 Hellinger–Jensen–Shannon 不等式作为下界证明的核心工具；(4) 将结果推广到任意离散分布，将信息复杂度表征为关于信道的优化问题，并证明二元输出信道即可达到最优；(5) 针对有界似然比分布给出界，并利用 χ² 散度给出一般性上界。

Result: (1) 对任意 β<α，给出了常数优势协议在两种假设下信息复杂度的完整刻画（差常数因子），并发现两种信息成本可以相差很大；(2) 识别出三个参数区间，分别对应干净样本、BSC 和 Z 信道三种最优协议；(3) 证明了二元输出信道足以达到任意离散分布检验的最优信息复杂度；(4) 恢复并强化了广播模型中的 set-disjointness 下界，并推导了多趟流式（multi-pass streaming）设置下若干问题的更强下界。

Conclusion: 本文对广播模型下 Bernoulli 分布检验的信息复杂度给出了近乎精确的刻画，揭示了不同参数区域对应不同结构的通信机制，并通过新颖的信息论不等式工具建立了紧的下界。该结果不仅深化了对分布式假设检验的理解，还为通信复杂性中的下界推导提供了有力工具，具有独立的理论价值。
...
### [108] [When Predictions Become Regressors: A Split-Sample Correction for Biases in Downstream Inference](https://arxiv.org/abs/2608.02909)
*Nathan Canen,Ted Enamorado*

Main category: stat.ML

TL;DR: 本文提出一种简单的工具变量方法解决预测生成测量（如LLM产生的变量）在回归中因测量误差导致的偏误问题：在原始数据独立分割上生成多个测量指标作为工具变量。该方法理论上有效、易于实现且无需新数据，模拟研究表明即使在较小样本中也能接近真实估计值。


<details>
  <summary>Details</summary>
Motivation: 基于预测的方法（包括LLM和机器学习技术）常被用于构建难以直接量化的政治现象测量指标，如竞选纲领中的政策立场或社交媒体上的情绪。但在许多应用中，这些预测生成的测量被用作回归模型中的解释变量，而它们本身包含测量误差，导致估计结果产生偏误。

Method: 提出使用在原始数据独立分割上创建的多个测量指标构造工具变量的方法。具体而言，将数据随机分为多个子集，在每个子集上分别训练模型生成预测测量，然后将这些多重测量作为回归中该解释变量的工具变量，以纠正测量误差带来的偏差。该方法无需收集新数据，实现简单。

Result: 通过模拟研究，该方法即使在相对较小的样本中也能恢复接近真实值的估计，而标准方法在实践中可能产生显著偏差。作者还通过两个实际应用案例验证了该方法：德国议会中性别化语言对立法结果的影响，以及中国政治风险对扶贫项目的影响。

Conclusion: 基于数据独立分割构造多测量指标作为工具变量的方法，为预测生成测量在回归分析中的测量误差偏误问题提供了简单有效的解决方案，具有较强的理论有效性和实际可操作性。
...
### [109] [Calibrated Bayesian Inference for Stochastic Intervention Effects](https://arxiv.org/abs/2608.02924)
*Tyler M. Schmidt,Nathan B. Wikle*

Main category: stat.ML

TL;DR: 本文提出一种后处理校正方法，应用于非参数贝叶斯模型的样本后验，使随机干预效应的推断达到渐近有效且置信区间具有有效频率覆盖，并满足半参数 Bernstein-von Mises 定理。同时为 SoftBART 提供了新理论，证明其在随机干预效应下支持校准贝叶斯推断。


<details>
  <summary>Details</summary>
Motivation: 因果推断正从确定性治疗分配的经典因果效应扩展到随机干预效应，后者能放宽正性假设并更具政策相关性。非参数贝叶斯模型因其灵活性和内在不确定性传播而具吸引力，但后验不确定性对目标因果效应未必校准良好，需要一种不改变先验或拟合算法的校正方法。

Method: 开发了一种简单的后处理校正方法，可直接应用于现有后验样本而无需更改先验或拟合算法。通过理论证明，对一类广泛的随机干预（包括与观测治疗过程独立指定的干预以及修改其过程的干预，如增量倾向得分干预和新的幂倾斜干预），校正后的后验满足半参数 Bernstein-von Mises 定理，从而实现渐近有效推断和有效渐近频率覆盖。此外，提出了 SoftBART 的新理论，给出了该灵活基于树的贝叶斯模型支持随机干预效应校准贝叶斯推断的条件。

Result: 模拟实验表明，校正方法相对未校正的贝叶斯分析减少了偏差并改善了覆盖率，同时与频率派替代方法保持竞争力。实际应用中，该方法被用于估计在假设增加或减少接受他汀类药物治疗几率条件下，预期 LDL 胆固醇的变化。

Conclusion: 该后处理校正方法为非参数贝叶斯模型下的随机干预效应推断提供了频率意义上的可靠性保障，拓展了贝叶斯因果推断的适用性，兼顾了贝叶斯灵活性与频率派置信保证。
...
### [110] [Temporal Leakage in LLM Backtesting: Measurement, Validation, and Adjusted Scores](https://arxiv.org/abs/2608.02985)
*Zeyu Zhang,Bradly C. Stadie*

Main category: stat.ML

TL;DR: 本文揭示LLM回测中的标准污染检测方法（比较训练截止前后得分）是无效的，因为模型对临近截止时间的信息有结构性优势，导致"近因效应"伪装成数据泄漏。作者证明被动回测无法将二者区分，提出两种外部信息源（已知截止时间和匹配干净的对照组）来度量泄漏，并推导了泄漏的分布特征。通过在孪生模型中植入泄漏验证了估计器，并在前沿模型上检测到一个截止点附近的泄漏信号，同时以审计功效下限为多个模型辩护。


<details>
  <summary>Details</summary>
Motivation: 现有LLM回测污染检测方法（比较截止前后得分）根本无效——四个旗舰模型在截止后无法记忆的问题上均"失败"了该检查。由于模型对临近截止时间的信息了解更多是合法的结构性现象，近因效应会伪装成泄漏，且任何被动回测理论上都无法将二者与真实能力区分。因此需要借助回测之外的信息来从根本上解决污染度量问题。

Method: 作者首先在理论上证明被动回测无法区分"近因性知识增长"与"数据泄漏"与"真实能力"。然后引入两种外部信息源：(1) 已知的模型截止时间，用于识别截止点边界处的泄漏；(2) 匹配的干净对照数据，用于全局识别泄漏并计算泄漏调整后得分。作者还推导了泄漏的集中位置——那些让大众意外且训练中覆盖充分的结果——并指出部分记忆（partial memorization）会获得不成比例的奖励。验证方式是在孪生模型中人为植入泄漏，检验估计器能否恢复注入剂量并对干净问题返回零结果。

Result: 在孪生模型（twin models）验证中，估计器成功恢复了注入的泄漏剂量，并在干净问题上返回空值（null）。部署在前沿模型上时，检测到一个截止点局部化的泄漏签名；在审计的功效下限（power floor）下，五个模型被证明其表面优势纯属近因效应导致的假象，从而被洗清。

Conclusion: 回测并非需要被废弃，而是需要一个可辩护的参照（reference）——即来自回测之外的信息（如已知截止时间或匹配对照）。这为LLM能力评估中的污染检测提供了更加严谨、可验证的度量框架。
...
### [111] [Stochastic Saddle Avoidance Beyond Unit Excitation and Smoothness: A Pathwise Lyapunov-Perron Framework](https://arxiv.org/abs/2608.03001)
*Junwen Qiu,Bohao Ma,Andre Milzarek,Junyu Zhang*

Main category: stat.ML

TL;DR: 本文提出了一个无需单位激励（UE）假设的抽象几乎必然鞍点回避定理。通过可验证的路径条件替代UE要求，并结合路径相关的变量变换与Lyapunov-Perron证明策略，该定理适用于随机镜像下降（含SGD）、随机重排（random reshuffling）以及近端型随机梯度方法，证明了它们几乎必然收敛到局部极小值。


<details>
  <summary>Details</summary>
Motivation: 单位激励（UE）假设是随机鞍点回避中的常见前提，要求随机误差在每个方向上均具有一致正的分量。然而该假设过于简化实际噪声结构，在过参数化/插值模型（噪声在平稳点附近消失）和有限和问题（噪声位于低维数据相关子空间）中并不可满足。因此需要一个不依赖UE的通用鞍点回避理论框架。

Method: 本文提出抽象几乎必然回避定理，将UE型要求替换为可验证的路径条件。核心策略包括：（1）路径相关的变量变换（path-dependent change of variables）；（2）基于Lyapunov-Perron方法的路径证明策略。该方法回避了确定性分析中经典的center-stable manifold论证，因为随机采样映射通常不共享不动点。

Result: 作为应用，本文获得了以下严格鞍点回避结果：随机镜像下降（含SGD）的严格鞍点回避；随机重排（random reshuffling）的严格鞍点回避；非光滑复合目标下近端型随机梯度方法的回避结果。结合适当的迭代收敛保证，可建立收敛到原目标函数局部极小值的结果。

Conclusion: 本文建立的抽象几乎必然鞍点回避定理弥补了UE假设在过参数化、插值模型和有限和问题等常见场景中的不足，为无UE条件下随机优化的鞍点回避提供了统一的路径级分析框架，拓展了随机优化方法收敛到局部极小值的理论适用范围。
...
### [112] [A Direct Route to Markov Chain Convergence via Asymptotic Equivalence with the Target](https://arxiv.org/abs/2608.03353)
*Patrick Forré*

Main category: stat.ML

TL;DR: 本文给出一个自包含的Markov链收敛定理证明，通过一个称为"与目标渐近等价"的准则。该准则基于Lebesgue分解，要求两类奇异质量均趋于0，在可数生成可测空间上既是充分也是必要条件。文中不依赖不可约、非周期、回归、耦合、分裂构造或小集等常用假设。


<details>
  <summary>Details</summary>
Motivation: 现有Markov链收敛定理通常依赖不可约性、非周期性、回归性、耦合、分裂构造或小集等复杂且限制性的假设。作者希望提供一个简洁、通用且适用范围广泛的收敛准则，在一般可测空间上工作，同时避免上述经典假设，以简化收敛性证明并为常见算法（如Metropolis-Hastings、Gibbs采样、并行回火）提供统一的分析框架。

Method: 作者提出"与目标渐近等价"准则，基于每一起点x下T^n_x与π的Lebesgue分解，要求两个方向的奇异质量均趋于0：(1) 渐近绝对连续性（singular mass sing(T^n_x|π)→0）；(2) 渐近支配目标（singular mass sing(π|T^n_x)→0）。在可数生成可测空间上，该准则被证明是收敛的充分必要条件。此外，作者在一般可测空间上验证了该准则的密度版本，覆盖三种情形：(i) T关于π有正转移密度；(ii) T由绝对连续部分加起点原子组成（覆盖Metropolis-Hastings）；(iii) 转移密度在有限步后为正（可能依赖于起点）。最后表明在这些设置下Birkhoff遍历定理适用以获取强大数定律。

Result: 论文证明了渐近等价准则在可数生成可测空间上是Markov链收敛的充要条件。在一般可测空间上，该准则的密度版本在三种具体情形下成立：正转移密度、带起点的原子（Metropolis-Hastings情形）、以及有限步后正密度。作者进一步将准则应用于随机扫描Gibbs采样器和并行回火算法的收敛性分析，并证明了Birkhoff遍历定理在上述所有设置下适用，从而获得强大数定律。全文不使用不可约性、非周期性、回归性、耦合、分裂构造或小集概念。

Conclusion: 论文提供了一个新的、简洁的统一Markov链收敛准则，兼具充分性和必要性，可广泛应用于多种经典MCMC算法。该方法的显著优点是避免了传统证明中的复杂假设和构造，使收敛性分析更为直接。作者明确指出文中所有定理并非新结果，其贡献在于提供了一条通往通用收敛准则的短路径，且该准则在比传统条件更弱的一般可测空间假设下成立。
...
### [113] [Beyond the Gegenbauer Paradigm: q-Orthogonal Kernels for Machine Learning](https://arxiv.org/abs/2608.03482)
*Álvaro Sánchez-Paniagua Ríos,Juan P. Llerena,Alberto Lastra,Nuria Torrado,Edmundo J. Huertas*

Main category: stat.ML

TL;DR: 本文提出基于离散q-Hermite I多项式的正交多项式核函数，用于支持向量机（SVM）。该核函数通过形变参数q推广经典Hermite多项式，在Mercer定理下被证明有效，其天然有界性避免了数值上的湮灭和爆炸问题。在20个基准数据集上的实验表明，该核在数值稳定性和计算简单性上具有优势，性能与经典核及其他正交多项式核相当。


<details>
  <summary>Details</summary>
Motivation: SVM的性能在很大程度上依赖于核函数的选择，经典核（如RBF）虽然流行但可解释性有限。正交多项式核是可解释的替代方案，能够融入结构化的先验知识，本文在此基础上进一步拓展该范式，引入q-正交多项式（特别是离散q-Hermite I多项式）来构建新型核函数。

Method: 形式化定义了q-Hermite核函数，并通过Mercer定理验证其有效性；利用q-Hermite多项式的固有有界性避免核值湮灭和爆炸，无需显式缩放机制；在20个基准数据集上与经典核及其他正交多项式核进行对比实验。

Result: 所提出的q-Hermite核在20个基准数据集上取得了与经典核和其他正交多项式核相当的性能，同时在数值稳定性和计算简单性方面具有优势。

Conclusion: q-正交多项式是核函数设计的有前景方向，兼具数学优雅性与实际机器学习应用价值，可为未来的量子计算范式提供概念性和算法性资源。完整实现和实验流程已在GitHub开源。
...
### [114] [A fully nonlinear structural vector autoregressive model identified via independent innovation analysis](https://arxiv.org/abs/2608.03486)
*Savi Virolainen*

Main category: stat.ML

TL;DR: 本文提出了一种完全非线性的结构向量自回归（SVAR）框架，其中同期结构映射可以是非线性和非加性的。通过利用由观测外生变量引起的相互独立结构冲击的条件分布变化，结合对比学习和指数族结构来实现识别，并使用前馈神经网络估计模型。实证研究分析了美国工业产出对实际油价冲击的不对称响应，发现存在适度的符号和经济状态不对称性，并附有R包iiasvar。


<details>
  <summary>Details</summary>
Motivation: 现有独立创新分析（Independent Innovation Analysis）结果只能将结构冲击识别到任意分量可逆变换，这对于结构计量经济分析通常是不够的。作者旨在解决这一识别不足问题，构建一个能充分利用外生变量信息、实现更强识别性的完全非线性SVAR框架。

Method: （1）提出完全非线性、非加性的SVAR框架；（2）利用观测外生变量引起的独立结构冲击条件分布变化，借助对比学习框架配合指数族结构恢复冲击；（3）通过结构化指数族规范将识别模糊性降至每个冲击的一参数变换尺度映射；（4）在自然参数采用logistic规范下，进一步将识别加强至置换和分量符号变化；（5）冲击恢复后使用前馈神经网络（基于通用逼近能力）估计完全非线性SVAR。

Result: 实证分析发现美国工业产出对实际油价冲击的响应存在适度的不对称性，主要体现在冲击的符号和经济状态两个维度上。论文还提供了实现该方法的R包iiasvar。

Conclusion: 该框架通过结构化指数族假设和对比学习有效解决了完全非线性SVAR中结构冲击的识别问题，将其从任意分量可逆变换收缩到置换和符号变化级别，并通过实证验证了油价冲击对工业产出存在适度不对称影响，为非线性结构计量分析提供了可操作的工具。
...
### [115] [When Many Answers Are Valid, Voting Fails: Symbolic Verification for Best-of-K Causal Reasoning in LLMs](https://arxiv.org/abs/2608.03506)
*Omatharv Bharat Vaidya,Connor Thomas Jerzak,Zayne Rea Sprague,Fangcong Yin,Nhat Ho*

Main category: stat.ML

TL;DR: 提出CALVER，一种无训练的符号验证器，基于Pearl因果准则（d-分离、后门调整、干预）对结构化推理痕迹打分，无需参考答案即可选出最可靠的因果推理候选，显著优于多数投票、奖励模型和LLM裁判。


<details>
  <summary>Details</summary>
Motivation: 自一致性假设采样推理痕迹中最频繁的答案是可靠的，但在因果推理中这一假设常常失效：样本会重复相同的混杂错误，且投票在不同有效答案间分散，导致无效答案胜出而有效少数派被忽略。需要一种不依赖频率、基于因果准则的验证方法。

Method: CALVER通过将结构化推理痕迹与Pearl因果准则（d-分离、后门调整、干预）进行符号化匹配打分，在无需训练和参考答案的情况下，从采样候选中选择得分最高的答案。它只需要因果结构（可直接给定或从文本构建），支持跨多个候选的因果有效性聚合。

Result: 在CLEAR find-one-valid查询中达到42.1%准确率，而多数投票、奖励模型、LLM裁判和模型置信度仅约30%。扩展到72B规模的裁判仍无法缩小差距。在审计的干净核心子集中，21个图有效选择中有11个与基准所列答案不同但仍满足谓词要求。优势随采样预算扩大而增加，并在十个贝叶斯网络、第二个模型家族以及需从文本构建图表的场景中复现。同时提升了阈值化ATE决策性能，在真值表检查器下推广到逻辑推理，CPU上每个候选仅需毫秒级评分。

Conclusion: CALVER证明了因果验证可以替代频率聚合来自我一致性，在因果推理中显著优于现有选择方法；只要具备因果结构（直接提供或从文本构建），通过因果有效性聚合候选选择就是可行且高效的。
...
### [116] [Confidence Horizons](https://arxiv.org/abs/2608.03889)
*Chase Mathis,Ian Waudby-Smith*

Main category: stat.ML

TL;DR: 本文提出了"置信水平线"（confidence horizons）这一族统计对象，用于在有限时间范围内获得更尖锐的大样本任意时刻有效推断（anytime-valid inference），并给出了精确的闭式分布函数来替代传统组序贯方法中的重复积分。


<details>
  <summary>Details</summary>
Motivation: 现有的任意时刻有效推断方法为了在无限时间范围内保持有效性而引入了额外的保守性。然而在实际应用中，实验常常受到预算、实践或伦理约束，时间范围实际上是有限的。作者希望探究：如果放弃无限时间范围内的有效性，是否可以在有限时间范围内获得更尖锐（更不保守）的大样本推断结果。

Method: 作者提出"置信水平线"这一族统计对象，它可被视为有界时间范围内的置信序列，或具有最大中途窥视次数的组序贯重复置信区间。作者建立了与Pocock、O'Brien-Fleming和Wang-Tsiatis组序贯边界的联系，并推导了某些统计量的闭式分布函数，从而精确计算置信水平线的渐近分位数，避免了传统组序贯方法中常用的重复积分。该方法还应用于自适应Neyman分配下的序贯随机实验中的处理效应估计。

Result: 作者给出了正面回答：通过放弃无限时间范围的有效性，确实可以在有限时间范围内获得更尖锐的任意时刻有效推断。所提出的置信水平线具有闭式分布函数，可精确计算渐近分位数，并成功应用于自适应Neyman分配下的序贯随机实验中的处理效应估计。

Conclusion: 置信水平线提供了一种在有限时间范围内进行更高效（更不保守）的任意时刻有效推断的新途径，架起了无限时间置信序列与经典组序贯方法之间的桥梁，同时通过闭式解的推导避免了重复积分的计算负担。
...
### [117] [Trajectory inference via Acceleration Matching](https://arxiv.org/abs/2608.03916)
*Bartolo Dazzini,Giovanni Conforti,Alain Durmus,Aram-Alexandre Pooladian*

Main category: stat.ML

TL;DR: 本文提出了一种名为Acceleration Matching (AM)的新算法，用于解决轨迹推断问题。该方法将原始的插值问题提升到相空间（phase space），通过回归显式的条件加速度场来生成与给定边际分布一致的随机平滑轨迹，无需昂贵的预处理或基于仿真的训练目标。


<details>
  <summary>Details</summary>
Motivation: 现有轨迹推断算法面临计算挑战：有的依赖预处理子程序来强制平滑性，有的依赖基于仿真的训练目标，这些方法都可能计算开销高昂。为了克服这些局限性，需要一种更高效且不需要额外预处理的算法。

Method: 提出Acceleration Matching (AM)算法，核心思想是将原始插值问题提升到相空间，然后回归到显式的条件加速度场上，该场能够诱导出与给定边际分布一致的随机平滑轨迹。训练过程仅需位置数据，避免了轨迹仿真和昂贵的预处理步骤。

Result: 大量数值实验表明，AM算法在多个现有文献中的基准问题上与现有算法具有竞争力或表现更优。

Conclusion: Acceleration Matching是一种高效、无需昂贵预处理和仿真训练的轨迹推断新方法，在多个基准问题上展现出优于或匹敌现有算法的性能。
...
### [118] [1+1d Lattice Dirac Fermions from Non-Onsite Vector and Axial Symmetries](https://arxiv.org/abs/2608.02722)
*Tabin Dharanikota,Lukasz Fidkowski*

Main category: cond-mat.str-el

TL;DR: 本文构造了一个精确可解的1+1维Dirac费米子哈密顿晶格模型，具有精确的微观矢量U(1)和轴矢量U(1)对称性，混合反常通过对称性的非局域作用来容纳。Hilbert空间是局域Z2-分次Hilbert空间的Z2-分次张量积，包含无限维转子自由度。通过保局域性的幺正映射到等价的费米子Villain Hilbert空间后，哈密顿量变得显式精确可解。该构造还可实现相互作用的费米子Luttinger液体的精确可解实现。在自由Dirac不动点处，哈密顿量包含无关相互作用，作者计算了其领头阶形式。


<details>
  <summary>Details</summary>
Motivation: 在格点模型中精确实现具有连续对称性的Dirac费米子并且同时容纳混合反常是一个基本挑战。此前的工作通常要么牺牲精确的晶格对称性，要么在对称性和可解性之间做出妥协。作者的目标是构建一个既满足精确微观对称性（矢量与轴矢量U(1)），又能精确容纳量子反常的哈密顿量晶格模型，同时保持精确可解性。

Method: 作者将Hilbert空间构造为局域Z2-分次Hilbert空间的Z2-分次张量积，其中包含了无限维转子自由度（这会自然产生Villain类型的紧致变量）。对称性的非局域（not-on-site）作用方式容纳了矢量与轴矢量U(1)对称性之间的混合反常。然后通过一个保局域性的幺正变换将模型映射到等价的费米子Villain Hilbert空间，使得哈密顿量变成精确可解的形式。该框架还自然地推广到相互作用情形，实现了可解的Luttinger液体。

Result: 成功构造了具有精确矢量/轴矢量U(1)对称性的可解1+1d Dirac费米子格点模型，实现了混合反常的格点化容纳；推广到可解的相互作用费米子Luttinger液体；在自由Dirac不动点上计算了由晶格正则化引入的无关相互作用的领头阶修正。

Conclusion: 该工作展示了在格点上通过非局域对称性作用精确实现Dirac费米子及其反常的有效方案，为格点量子场论和强关联费米子系统提供了精确可解的新工具，特别是在研究Luttinger液体和手征反常现象方面具有潜在应用。
...
### [119] [Spin-charge separation in the triangular-lattice Hofstadter-Hubbard model](https://arxiv.org/abs/2608.02727)
*Yuntian Gu,Hui Yang,Zhehao Dai,Yantao Wu*

Main category: cond-mat.str-el

TL;DR: 本文利用变分蒙特卡洛结合神经量子态（NQS）和投影纠缠对态（PEPS）两种数值方法，系统研究了三角晶格Hofstadter-Hubbard模型中的手性自旋液体相，在实空间中观测到自旋-电荷分离现象，并探讨了任意子超导的可行性。


<details>
  <summary>Details</summary>
Motivation: 莫尔材料实验推动了多种奇异量子相的实现，Hofstadter-Hubbard模型被认为可能承载手性自旋液体。同时，二维强关联费米子系统的计算方法取得重大进展，使得对该模型在真实二维几何中进行数值研究成为可能。作者希望利用这些新工具验证该模型中手性自旋液体相的存在。

Method: 采用变分蒙特卡洛（VMC）方法，结合两种先进的变分波函数——神经量子态（NQS）和投影纠缠对态（PEPS），对三角晶格Hofstadter-Hubbard模型进行数值研究。通过数值自旋泵浦模拟以及实时的自旋和电荷运动观测，直接在实空间中探测自旋-电荷分离；通过计算两电子束缚能研究任意子超导性。

Result: 在实空间中直接观测到了自旋-电荷分离现象；在小体系中发现了正的二电子束缚能（支持任意子超导的迹象），但随着体系尺寸增大，束缚能降至数值分辨率以下，暗示在大体系极限下可能不存在稳定的两电子束缚态。

Conclusion: 研究表明NQS和PEPS是诊断强关联电子系统中拓扑序和分数化激发的重要工具，且两种方法可以相互交叉验证。这项工作为理解Hofstadter-Hubbard模型中的手性自旋液体和相关量子相提供了坚实的数值证据。
...
### [120] [Particle-Vortex Duality of Hydrodynamics](https://arxiv.org/abs/2608.02732)
*Cenke Xu,Matthew P. A. Fisher*

Main category: cond-mat.str-el

TL;DR: 本文基于混合态物质中新近认识的对称性结构以及"强弱自发对称性破缺"(SW-SSB)，发展了二维玻色子/转子系统涌现的model-F和model-A流体动力学的量子粒子-涡旋对偶关系。该对偶表明2d玻色子的经典流体动力学既可由电荷对称性U(1)_c描述，也可等价地由对偶的(涌现)1形式对称性U(1)_e^{ (1)}以及涡旋守恒对应的U(1)_v来描述，为物质流体动力学与磁流体动力学之间建立了桥梁。


<details>
  <summary>Details</summary>
Motivation: 利用最近在混合态物质中发现的对称性结构和"强弱自发对称性破缺"(SW-SSB)这一新概念，为二维玻色子系统建立统一的量子粒子-涡旋对偶框架，从而将物质流体动力学与磁流体动力学联系起来。

Method: 运用混合态对称性(尤其是SW-SSB)理论框架，对二维玻色子/转子系统的涌现model-F和model-A流体动力学进行量子粒子-涡旋对偶分析，揭示其与U(1)_c电荷对称性、U(1)_e^{(1)}对偶1形式对称性以及U(1)_v涡旋数守恒对称性之间的关系。

Result: 建立了2d玻色子/转子系统流体动力学的量子粒子-涡旋对偶关系，证明经典流体动力学可以在电荷对称性与对偶1形式对称性及涡旋对称性的等价描述下成立，并实现了物质流体动力学与磁流体动力学之间的对偶桥梁。

Conclusion: 该研究通过SW-SSB框架揭示了二维玻色子系统流体动力学中的深层对称性结构，确立了粒子-涡旋对偶的量子版本，为理解物质流体动力学与磁流体动力学之间的统一提供了新的理论视角。
...
### [121] [Large scale neural quantum states reveal the interplay between superconductivity and quantum criticality in the Hofstadter-Hubbard model](https://arxiv.org/abs/2608.02753)
*Christopher Roth,Andrew Millis,Tomohiro Soejima*

Main category: cond-mat.str-el

TL;DR: 本文利用神经网络量子态方法研究三角晶格Hofstadter-Hubbard模型，发现半填充时波矢通量退化的两个母态（整数量子霍尔绝缘体和手性自旋液体）之间的相变为连续相变，掺杂后两侧均出现d+id波配对拓扑超导体，且配对序参量几乎不随母态变化，而超流刚度在临界点附近显著增强，表明超导能量尺度由母态间转变的邻近性决定，而非具体母态本身。


<details>
  <summary>Details</summary>
Motivation: 理解母态绝缘体如何塑造掺杂后涌现的超导电性是一个长期未解的问题，可追溯至Anderson的共振价键理论。三角晶格Hofstadter-Hubbard模型在半填充下提供两个截然不同的母态（弱耦合整数量子霍尔绝缘体与中等耦合手性自旋液体），为研究母态性质与超导性的关系提供了理想平台。

Method: 采用神经网络量子态（neural quantum states）方法，在最多432个格点的环面（torus）几何上求解三角晶格Hofstadter-Hubbard模型（每plaquette含π/2通量）。通过计算电荷能隙、电荷涨落、配对序参量和超流刚度等物理量，系统分析半填充母态转变的连续性及掺杂后超导性质的演化。

Result: (1) 半填充下的母态转变是连续的，2e电荷能隙消失且电荷涨落具有临界性；(2) 掺杂后在转变两侧均出现具有离域长程序的d+id波配对拓扑超导体；(3) 配对形成与相位相干两个超导要素对母态响应截然不同：配对序参量跨转变几乎不变，而超流刚度在临界点附近强烈增强；(4) 超导能量尺度由与临界点的邻近性决定，而非具体掺杂的母态。

Conclusion: 该工作揭示母态间拓扑相变的临界涨落是增强超导相位相干的关键机制，确定了超导能量尺度由临界点邻近性而非母态具体性质决定这一普适物理图像，并展示了神经网络量子态在处理长程电子关联与超导性精细相互作用方面的强大能力。
...
### [122] [Tunable Skyrmions in a Topological Wigner Crystal](https://arxiv.org/abs/2608.02832)
*David A. Dahlbom,Daniel Kaplan,Premala Chandra,Cristian D. Batista*

Main category: cond-mat.str-el

TL;DR: 本文研究拓扑维格纳晶体中赝自旋相互作用引起的零场斯格明子晶体，发现铁磁海森堡交换作用J与手性相互作用γ之间的竞争在三角格点上稳定了密度可通过γ/J连续调节的斯格明子晶体。在大手性耦合极限下，揭示了一个携带四个拓扑电荷的24位点四斯格明子晶体，并绘制了铁磁拓扑维格纳晶体的完整相图。


<details>
  <summary>Details</summary>
Motivation: 在具有强相互作用的低密度系统中，电子预期会结晶形成维格纳固体。近期在携带贝里曲率的二维系统中的进展为维格纳结晶引入了拓扑维度。本文旨在理解拓扑维格纳晶体中手性相互作用如何稳定拓扑磁结构（如斯格明子晶体），并建立其完整相图。

Method: 采用三角格点上赝自旋相互作用的模型，研究铁磁海森堡交换作用J与手性相互作用γ之间的竞争。在连续极限下，将手性相互作用表示为斯格明子的化学势，利用非线性sigma模型之外的高阶梯度项来确定斯格明子密度，并分析宏观自旋构型及其磁振子结构。

Result: 1) 竞争J与γ可稳定密度由γ/J连续调节的零场斯格明子晶体；2) 在手性耦合较大时发现24位点四斯格明子晶体，每个磁原胞携带四个单位拓扑电荷；3) 当J→0时该相退化为四亚晶格四面体序；4) 表征了这些相的磁振子结构，并讨论了从可调斯格明子晶体到大γ/J区域这些态的转变。

Conclusion: 该工作建立了铁磁拓扑维格纳晶体的完整相图，揭示了贝里曲率诱导的手性相互作用如何在拓扑维格纳晶体中稳定可调密度的斯格明子晶体，以及在高手性耦合下的新型四斯格明子晶体相，为拓扑磁结构与维格纳结晶之间的交叉领域提供了理论基础。
...
### [123] [Scale-covariant liquid in nonlocal high Tc strange metals](https://arxiv.org/abs/2608.03013)
*Jian Xian Sim*

Main category: cond-mat.str-el

TL;DR: 本文提出一种基于非局域有效排斥相互作用的高温超导体标度协变自能机制，通过掺杂依赖的指数α（1≤α≤3）天然插值于Mott绝缘体与费米液体极限之间，解释了实验中观测到的随掺杂连续变化的奇怪金属行为，并指出量子临界性并非标度协变现象的必要条件。


<details>
  <summary>Details</summary>
Motivation: 近年来高温超导实验发现puzzling的节点标度协变自能，其指数随掺杂连续变化，现有理论框架难以自洽解释这一现象。

Method: 提出由屏蔽不良的有效排斥V_α(r)~1/r^α产生的非局域机制，发展流体动力学屏蔽现象学，推导出标度协变的准粒子衰减率Γ(ω,T)∝T^γ Φ(ω/T)，其中γ=2-1/α（对应1<α<2的非局域区域）。

Result: 该理论自然捕获了最优掺杂到过掺杂区域的行为，欠掺杂区域则呈现定性不同特征。光谱拟合的指数直接反映带电流体的有效空间非局域性。

Conclusion: 标度协变现象可以由非局域性解释，无需诉诸量子临界性。该工作为理解高温超导体中掺杂依赖的奇怪金属行为提供了新视角。
...
### [124] [Exotic superconductivity in the doped Kitaev quantum spin liquid](https://arxiv.org/abs/2608.03024)
*Takahiro Misawa,Kota Ido*

Main category: cond-mat.str-el

TL;DR: 本文利用多变量变分蒙特卡洛方法研究掺杂Kitaev量子自旋液体中的超导性，发现铁磁Kitaev相互作用下出现与铁磁共存的稳健三重态p波超导，反铁磁Kitaev相互作用下超导配对对称性从低掺杂的三重态p波转变为中掺杂的单重态d+id。


<details>
  <summary>Details</summary>
Motivation: 探索载流子掺杂的Kitaev候选材料中可能存在的非常规超导，特别是三重态超导与铁磁共存的机制，为实验搜索提供理论基础。

Method: 采用多变量变分蒙特卡洛方法，对空穴掺杂的t-J型Kitaev模型使用可精确表示Kitaev量子自旋液体的投影对乘积波函数，在各向同性二维团簇上考察超导态的稳定性。

Result: 铁磁Kitaev相互作用下，低至中等掺杂区间出现稳健的三重态p波超导与铁磁共存，但当系统接近全极化铁磁相时超导被抑制；反铁磁Kitaev相互作用下，低掺杂为自旋依赖三重态p波，中等掺杂转变为单重态d+id；固定掺杂时增强铁磁Kitaev相互作用使三重态超导随铁磁矩增大而增强，在全极化附近最强。

Conclusion: 研究为载流子掺杂Kitaev候选材料中寻找与铁磁共存的三重态等非常规超导提供了坚实的理论基础。
...
### [125] [Contrasting anisotropic electron-phonon-spin coupling in Fe$_{3}$GeTe$_{2}$ and Fe$_{5}$GeTe$_{2}$: A helicity-resolved Raman study](https://arxiv.org/abs/2608.03193)
*Smrutiranjan Mekap,Jyoti Saini,Andrzej Ptok,Pawan Kumar Srivastava,Changgu Lee,Subhasis Ghosh,Anushree Roy*

Main category: cond-mat.str-el

TL;DR: 本文通过波长与温度依赖的手性分辨拉曼光谱，对比研究了二维范德华铁磁体Fe3GeTe2和Fe5GeTe2中光学共振诱导的各向异性电子-声子耦合及其与磁序的关联。研究发现手性声子劈裂的温度演化不追踪磁化行为，表明其响应源于自旋轨道耦合电子相互作用而非单纯的磁序时间反演对称性破缺；F3GT中面外磁化间接调控面内各向异性电子-声子耦合，而F5GT呈现静态各向异性相互作用，并展现出模式与温度依赖的Fano耦合强度。


<details>
  <summary>Details</summary>
Motivation: 二维范德华铁磁材料具有显著磁光响应，为研究晶格、电子与磁自由度之间的耦合提供了理想平台，但在这些系统中光学共振诱导的电子-声子耦合及其与磁有序的内在关联尚未被系统理解。

Method: 采用波长与温度依赖的手性分辨（左/右圆偏振激发）拉曼光谱，对F3GT和F5GT中双重简并E声子模式的手性劈裂进行系统解析，并结合Fano不对称参数来量化声子与电子连续态之间的耦合强度及其角度各向异性。

Result: （1）手性声子劈裂的温度演化不随磁化行为变化，证明该响应不仅源于磁序导致的时间反演对称性破缺，还涉及自旋轨道耦合电子相互作用；（2）F3GT中面外磁化通过光学共振间接调控面内各向异性电子-声子耦合，F5GT则表现出静态各向异性；（3）Fano参数1/q显示F3GT具有显著角度各向异性耦合而F5GT呈各向同性，后者源于其多Fe位点与增强的层间杂化。

Conclusion: 该研究揭示了晶体结构（不同Fe位点数目）与磁各向异性在塑造层状金属铁磁体中各向异性电子-声子-自旋耦合动力学中的关键作用，确立了FexGeTe2作为研究层状金属铁磁体中手性光-物质相互作用的通用微观平台。
...
### [126] [Fluctuations of topological charges in two-dimensional classical Heisenberg model through high-temperature and low-temperature expansions](https://arxiv.org/abs/2608.03195)
*Shan-Chang Tang*

Main category: cond-mat.str-el

TL;DR: 本文通过扩展KT相变中拓扑电荷涨落的概念到二维海森堡模型，利用高温和低温展开分别计算拓扑电荷涨落，发现高温下涨落正比于区域面积（面积律），低温下服从周长律（边界律）。


<details>
  <summary>Details</summary>
Motivation: KT相变由拓扑缺陷的束缚-解束缚机制驱动，可通过区域内拓扑电荷的涨落来表征。作者希望将这一思想推广到二维海森堡模型，通过拓扑电荷涨落的标度行为来区分高温和低温阶段的特征。

Method: 将KT相变中拓扑电荷涨落的表征方法扩展到二维海森堡模型。分别采用高温展开和低温展开两种解析方法，计算区域内拓扑电荷的涨落。

Result: 高温下，拓扑电荷涨落与区域面积成正比（面积律）；低温下，拓扑电荷涨落服从周长律，即与区域边界长度成正比。

Conclusion: 拓扑电荷涨落的高温面积律与低温周长律的差异，可以作为区分二维海森堡模型不同温度区域（高温无序相与低温有序相）的标度特征，体现了与KT相变中拓扑缺陷行为类似的物理机制。
...
### [127] [Magnetoresistance in Magnetic Weyl semimetal Mn$_{3}$ZnC](https://arxiv.org/abs/2608.03414)
*Sunil Gangwar,C. S. Yadav*

Main category: cond-mat.str-el

TL;DR: 本文研究了反钙钛矿磁性节线半金属Mn₃ZnC的磁电阻(MR)特性，发现其MR与磁化强度密切相关，在铁磁(FM)和亚铁磁(FIM)态表现出不同的行为，并在FM态极低磁场下出现不寻常的正MR效应。


<details>
  <summary>Details</summary>
Motivation: 磁电阻揭示自旋相关的电子散射信息，在拓扑磁性材料中有限贝里曲率还会引入内禀散射通道。研究Mn₃ZnC这类反钙钛矿磁性节线半金属的MR行为，有助于理解自旋极化与拓扑性质对电子输运的联合影响。

Method: 对反钙钛矿Mn₃ZnC进行MR测量，该材料在约420 K发生铁磁(FM)转变，约195 K发生亚铁磁(FIM)转变。分析MR与磁化强度M(H)曲线之间的关系，系统考察不同磁场和温度下MR曲线的变化特征。

Result: MR与磁化强度在低磁场下存在相关性，FM和FIM态表现出不同行为。MR曲线类似等温磁化曲线，先急剧上升后在高场呈线性变化。尖峰状异常随磁场变化更显著，在磁相变处消失，随温度升高又重新出现。FIM态中MR符号变化归因于载流子迁移率的剧烈变化。FM态极低磁场下呈现正MR，这一现象很不寻常。

Conclusion: Mn₃ZnC的MR与磁化强度密切相关，其FIM态中的MR符号翻转源于载流子迁移率的剧烈变化，而FM态极低场下的正MR则是一个值得深入研究的异常现象。
...
### [128] [Manipulation of localized excitons in CrPS$_4$ by temperature and magnetic field](https://arxiv.org/abs/2608.03543)
...
TL;DR: 本文通过多体微扰理论、动力学平均场理论和光致发光实验相结合的方法，系统研究了块体CrPS₄的电子和激子特性，建立了其直接带隙半导体特性（带隙2.48 eV），预测了多种亚带隙激子跃迁（包括自旋允许激子和自旋翻转激子），并通过温度和磁场依赖的光学测量揭示了磁性相变的光学特征。


<details>
  <summary>Details</summary>
Motivation: 层状范德华磁性半导体为探究激子与自旋、晶格自由度之间的相互作用提供了理想平台，激子可作为磁有序的灵敏探针。CrPS₄作为层状反铁磁半导体具有丰富的激子特征，但其微观起源及与磁有序的关联尚未被充分理解，因此需要深入研究其电子和激子性质。

Method: 采用多体微扰理论（many-body perturbation theory）、动力学平均场理论（dynamical mean-field theory）和基于光致发光（photoluminescence）的实验手段相结合的方法，对块体CrPS₄的电子结构和激子特性进行理论与实验的综合研究。

Result: 计算表明CrPS₄在反铁磁相中为直接带隙半导体，带隙为2.48 eV。理论预测了多个亚带隙激子跃迁，包括多个自旋允许的激子以及一个额外的自旋翻转激子，这些激子主要局域在Cr³⁺离子上。温度依赖和磁场依赖的光学测量揭示了热驱动的激子在局域态之间的重新分布，并识别出能够作为CrPS₄磁性相变清晰光学信号的特性能量位移。

Conclusion: 这些结果为了解反铁磁体的激子跃迁提供了新的见解，并提示了实现磁有序全光学传感和光驱动调控的潜在途径。
...
### [129] [Emergence and Detection of Surface altermagnetism in KV$_2$Se$_2$O](https://arxiv.org/abs/2608.03551)
*Rodrigo Jaeschke-Ubiergo,Xanthe H. Verbeek,Colin Lange,Sergio Rodriguez,Atasi Chakraborty,Alexander Mook,Jairo Sinov*

Main category: cond-mat.str-el

TL;DR: 本文在KVSO材料中展示了表面涌现交变磁性（emergent surface altermagnetism）的概念。研究表明，块体反铁磁有序的KVSO材料，(001)表面表现出d波交变磁性。该结果解释了此前看似矛盾的两个实验证据（中子衍射显示反铁磁块体，而光电子能谱显示d波自旋劈裂）。作者预测了非线性Edelstein响应作为关键实验特征，该响应局域于表面且遵循d波交变磁对称性。工作还推广到其他Lieb晶格体系，拓展了在反铁磁表面检测交变磁性的技术手段。


<details>
  <summary>Details</summary>
Motivation: 解决KVSO材料中块体反铁磁序与表面d波自旋劈裂这两个看似矛盾的实验证据之间的不一致，并验证'表面涌现交变磁性'这一新概念。

Method: 通过理论分析，在反铁磁有序的KVSO块体材料的(001)表面上展示d波交变磁性的涌现，并预测表面局域的非线性Edelstein响应作为关键实验表征手段，该响应遵循d波交变磁对称性。

Result: 成功解释了KVSO中块体反铁磁序与表面d波自旋劈裂共存的实验现象；预测了与d波交变磁对称性一致、表面局域的非线性Edelstein响应作为可验证的关键实验信号；结论同样适用于其他Lieb晶格体系。

Conclusion: 这项工作证实了表面涌现交变磁性的概念，为在反铁磁材料表面检测交变磁性提供了新的实验技术途径，并扩展了该现象在金属、室温磁性材料及其他Lieb晶格体系中的适用性。
...
### [130] [Microscopic Origin of Random Singlet Behavior in B-site Disordered Spin-1/2 Perovskite BaCu_1/3Nb_2/3O_3 Revealed by EXAFS and Thermodynamics](https://arxiv.org/abs/2608.03671)
*Sagar Mahapatra,Francesco De Angelis,Martin Etter,Edmund Welter,M. P. Saravanan,Rajeev Rawat,Carlo Meneghini,Surjeet Singh*

Main category: cond-mat.str-el

TL;DR: 本文通过联合结构（同步辐射XRD）与热力学（磁化率、比热）研究，揭示了ABO3型无序钙钛矿BaCu1/3Nb2/3O3（BCNO）中B位Cu/Nb的局域化学有序性——即Cu与Nb优先形成异质配对、抑制Cu-Cu直接连接，从而解释了该材料在具有显著反铁磁相互作用（ΘCW≈-50 K）时仍不出现自旋玻璃冻结或长程磁序的原因，并观测到与随机单态（random-singlet）现象学一致的幂律行为和T/H标度行为。


<details>
  <summary>Details</summary>
Motivation: 理解在接近渗流阈值的磁性离子浓度下，局域化学有序如何调控磁交换网络，从而决定无序磁体的基态性质（如随机单态vs自旋玻璃）。

Method: 结合同步辐射粉末X射线衍射（XRD）和X射线吸收精细结构谱（XAFS）表征微观结构，辅以磁化率χ(T)和比热cp(T)的低温热力学测量，并进行T/H单参数标度分析。

Result: XRD显示无长程Cu/Nb有序且平均占位符合化学计量比；XAFS揭示局域化学有序——Cu与Nb择优异质关联，显著抑制Cu-Cu连接。磁化率和比热呈现幂律行为及T/H标度（γ≈0.6），符合随机单态现象学。极低温下比热从零场的T^{1-γ}行为转变为高场下的T线性依赖，表明存在向另一低能态的交叉。

Conclusion: BCNO中Cu/Nb的局域异质化学有序有效抑制了Cu-Cu交换路径的渗流，使系统在较强反铁磁相互作用下仍偏离传统磁玻璃行为，呈现随机单态特征；高场下比热的T线性行为暗示存在新的低能激发机制，其微观起源有待进一步研究。
...
### [131] [Cavity control of quantum phase transitions in a two-dimensional kondo lattice](https://arxiv.org/abs/2608.03714)
*Jun Mochida,Atac Imamoglu,Yuto Ashida*

Main category: cond-mat.str-el

TL;DR: 本文研究了腔量子电动力学（cQED）如何通过真空涨落调控二维Kondo晶格体系的量子相变。研究发现，腔诱导的相互作用通过导带的动量依赖自能，可以移动重费米子相与反铁磁相之间的量子相变边界；纵向投影场（h-BN双曲声子极化激元激发）扩展重费米子区域，横向场有利于磁有序相，圆偏振场介于两者之间。


<details>
  <summary>Details</summary>
Motivation: 摩尔斯材料已成为研究二维重费米子系统及其量子相变的核心平台，而Kondo晶格是研究量子相边界（Kondo屏蔽与局域磁矩有序竞争）的原型模型。腔量子电动力学提供了一种通过电磁场真空涨落控制量子相的新途径，因此探索腔诱导相互作用能否有效调控二维Kondo晶格中的量子相变具有重要意义。

Method: 基于二维Kondo晶格模型，引入腔诱导的相互作用，通过导带的动量依赖自能来分析其对量子相变的影响。分别考察三种场结构：由h-BN双曲声子极化激元激发的纵向投影场、横向场以及圆偏振面内场，比较它们对Kondo杂化与磁有序竞争的不同作用。

Result: 腔诱导自能显著移动了重费米子相与反铁磁相之间的量子相变边界。纵向投影场有利于Kondo杂化，扩大重费米子相区域；横向场相对有利于磁有序相；圆偏振场的效果介于纵向和横向之间。

Conclusion: 电磁真空涨落可以有效地修改强关联二维Kondo材料的控制参数，为通过腔QED实现对量子相变的非侵入式调控提供了理论依据和可行路径。
...
### [132] [Antisymmetric Dynamical Spin Correlations: Spin-Space-Group Constraints and Frequency-Moment Sum Rules](https://arxiv.org/abs/2608.03786)
*Tsutomu Momoi*

Main category: cond-mat.str-el

TL;DR: 本文研究了极化非弹性中子散射中动力学自旋结构因子张量的反对称（自旋分量反对称）部分的对称性约束与求和规则。作者推导了磁空间群和自旋空间群的幺正/反幺正剩余操作下的对称性约束，确定了允许的张量分量、动量反转宇称及对称性强制的节点，并引入反对称对易子谱函数及其零温求和规则。对自旋波模型（altermagnet和螺旋磁体）的线性自旋波计算验证了这些约束。


<details>
  <summary>Details</summary>
Motivation: 极化非弹性中子散射能够通过动力学自旋结构因子张量的反对称部分探测磁激发的手性（handedness），但该反对称部分在磁空间群和自旋空间群下的完整对称性约束此前尚未被系统推导。建立这些对称性约束与相应的求和规则，对于理解和解释实验中观测到的磁激发手性信号（特别是altermagnet和螺旋磁体中的信号）至关重要。

Method: 作者：(1) 在磁空间群与自旋空间群的幺正和反幺正剩余操作下推导反对称动力学结构因子张量的对称性约束，确定允许的张量分量、动量反转宇称和对称性强制的节点；(2) 定义反对称对易子谱函数并推导其频率矩的精确零温求和规则——零阶矩由均匀磁化确定，一阶矩由动量加权的静态矢量手性确定（针对Heisenberg或XXZ交换哈密顿量）；(3) 利用线性自旋波计算对代表性的altermagnetic和螺旋模型进行验证。

Result: 关键结果表明：对于补偿的共线反铁磁体，若反演或平移操作联系了相反自旋的子晶格，则反对称动力学结构因子被禁止；在旋转相关的altermagnet中，动量反转下偶的xy分量在一般波矢下被允许，因此有限的反对称谱权重可以与对易子谱函数消失的零阶和一阶矩共存；而在平面螺旋中，残留的反幺正自旋空间群对称性使得允许的xy分量在动量反转下为奇，其一阶矩一般非零并由一阶矩求和规则固定。线性自旋波计算显式验证了这些对称性和求和规则约束。

Conclusion: 本文建立了极化中子散射探测磁激发手性所需的系统理论框架：反对称动力学结构因子的对称性选择定则与求和规则为实验提供了清晰的判定依据。特别地，altermagnet和螺旋磁体分别展现出动量反转偶/奇的反对称xy分量，这一差异可用于实验区分和表征不同的磁序类型，且"有限谱权重与零阶+一阶矩同时为零可共存"的结论揭示了对称性、求和规则与谱结构之间的深层联系。
...
### [133] [Local magnetic order in vacancy-disrupted spin ice Ho2TiO5](https://arxiv.org/abs/2608.03850)
*Raju Baral,Haidong Zhou,Qiang Zhang,Benjamin A. Frandsen,Stuart Calder*

Main category: cond-mat.str-el

TL;DR: 本文研究了经典烧绿石自旋冰Ho2Ti2O7转化为部分无序的填充化合物Ho2TiO5时，局域磁关联的演化过程。通过多种中子散射分析方法，发现两种化合物均保持长程Fd-3m对称性，但Ho2TiO5中Ho/Ti部分占据导致局域结构畸变。尽管存在无序，Ho矩仍保持<111>局域Ising各向异性并形成自旋进出构型。Ho2Ti2O7在0.3K呈现近理想的自旋冰态（95%四面体满足冰规则），而Ho2TiO5因非磁性Ti占据四面体顶点导致磁性网络拓扑不完整，表现为空位扰动的自旋冰，局域冰规则趋向在拓扑不完整网络上得以保留。


<details>
  <summary>Details</summary>
Motivation: 理解结构无序如何影响局域磁关联和集体冰排序行为，探究经典烧绿石自旋冰与其填充/部分无序变体之间磁性行为的联系与差异。

Method: 结合三种表征方法：实空间磁对分布函数（mPDF）、倒空间反向蒙特卡洛（RMC）、半极化中子粉末衍射，系统分析平均晶体结构与局域磁关联之间的关联。

Result: 1) 两化合物均保持长程Fd-3m对称性，但Ho2TiO5中存在Ho-O、Ti-O和O-O键长的局域无序；2) Ho矩在无序存在下仍保留<111>Ising各向异性；3) Ho2Ti2O7在0.3K时95%四面体满足两进两出的冰规则，升温后逐步热无序化；4) Ho2TiO5中大多数四面体磁学上不完整（因部分顶点被非磁性Ti占据），主要的不完整构型为2进1出和1进2出，与恢复缺失Ho自旋后的冰规则局域相容。

Conclusion: 单离子<111>Ising各向异性在结构无序中得以保留，但集体冰排序被强烈破坏。Ho2TiO5是空位扰动的自旋冰，其局域冰规则倾向在拓扑不完整的磁性网络上持续存在。
...
### [134] [Accelerated quantum Monte Carlo simulations of the attractive Hubbard model on the kagome lattice](https://arxiv.org/abs/2608.03894)
*Jie Zhang,Xiang Li,Yu Wang*

Main category: cond-mat.str-el

TL;DR: 本文提出了一种基于快速傅里叶变换（FFT）的传播子乘法加速方案，结合延迟更新算法，使行列式量子蒙特卡洛（DQMC）模拟的体系尺寸达到此前研究的两倍，从而实现对吸引型kagome晶格Hubbard模型可靠的有限尺寸标度分析，揭示了狄拉克填充下相互作用驱动的零温超流量子临界性，并排除了三角规则电荷密度波序在热力学极限下的存续。


<details>
  <summary>Details</summary>
Motivation: kagome材料家族及光学kagome格子实验的发展催生了对kagome格子相互作用驱动关联态的大量数值研究，但现有DQMC模拟可访问的体系尺寸有限，无法进行可靠的有限尺寸标度分析。

Method: 开发了一种基于快速傅里叶变换（FFT）的通用加速方案，用于复合格子上的传播子乘法，并将其与延迟更新（delay-update）算法相结合，使体系尺寸扩大为原先的两倍。

Result: 大规模模拟揭示了狄拉克填充下相互作用驱动的零温超流量子临界性，并给出了关联临界指数的可靠估计；未发现三角规则电荷密度波序在热力学极限下存续的证据，表明其很可能是有限尺寸效应；此外，在当前二维光学晶格实验可及的体系尺寸下，FFT与延迟更新组合方案的有效计算成本标度约为N^2.49，明显低于传统DQMC的O(N^3)。

Conclusion: FFT加速与延迟更新算法的结合显著提升了DQMC可模拟的kagome晶格体系尺寸，不仅揭示了超流量子临界性，还表明此前提出的电荷密度波序为有限尺寸假象，同时该方案的计算效率远优于传统方法，为未来kagome格子的强关联研究提供了有力的数值工具。
...
### [135] [Size Operator and Spectral Clustering in the Two Coupled SYK Model](https://arxiv.org/abs/2608.02696)
*Juan Santos-Suárez,Martí Berenguer,Javier Mas,Alfonso V. Ramallo*

Main category: cond-mat.str-el

TL;DR: 该论文揭示了在大N极限下两个耦合Sachdev-Ye-Kitaev模型实现的可穿越虫洞，其有限N谱按operator size（算符大小）组织成聚簇结构，"小尺寸"聚簇演化为共形塔并构成弱遍历破缺子空间，驱动长寿命的虫洞复兴动力学；尺寸能量与尺寸熵的竞争为虫洞-黑洞相变提供了微观解释。


<details>
  <summary>Details</summary>
Motivation: 理解全息虫洞的微观量子力学起源，特别是大N极限下的引力物理如何从有限N的量子谱中涌现，以及渗透性虫洞（eternal traversable wormhole）的离散谱结构的微观机制。

Method: 通过分析两个耦合SYK模型在有限N下的能谱，将其聚类为以operator size为标签的簇，研究低尺寸簇到共形塔的演化，并从尺寸能量与尺寸熵之间的竞争关系分析虫洞-黑洞转变。

Result: 有限N谱按operator size聚类；低尺寸簇构成弱遍历破缺子空间并解释虫洞复兴动力学；尺寸能量与尺寸熵的竞争提供了虫洞-黑洞转变的微观图景；operator size成为连接有限N谱与大N涌现引力物理的桥梁。

Conclusion: operator size是理解SYK模型中从量子谱到经典引力物理的关键微观自由度，为虫洞的量子-经典对应提供了新的理论框架。
...
### [136] [Thermal Order in the Biconical Model](https://arxiv.org/abs/2608.02720)
*Michael Smolkin,Lev Yung*

Main category: cond-mat.str-el

TL;DR: 该论文利用大N方法，在包括2+1维在内的时空维度中解析研究了双锥矢量模型，在1/N展开的前导阶和次前导阶上确定了RG流、不动点结构和红外理论的CFT数据，并证明了其Z2对称性在任意高温下都会自发破缺，从而将此前看似不同的理论统一在一个共同的分析框架中。


<details>
  <summary>Details</summary>
Motivation: 虽然已有多种方法（ε展开、FRG、大N技术）显示出2+1维中的一族标量理论在任意高温下都保持Z2对称性破缺，且这些理论之间具有相似性，暗示它们属于同一普适类，但这种联系尚未被明确建立。本文旨在填补这一空白。

Method: 采用大N方法，在包括2+1维在内的时空维度范围内，对双锥矢量模型进行解析研究。计算在1/N展开的前导阶（LO）和次前导阶（NLO）进行，确定RG流、不动点结构及红外理论的CFT数据；推导有效势，在零温和有限温度下建立其稳定极小值。

Result: 再现并扩展了此前的结果，将之前看似不同的理论构造统一在共同的分析框架中。推导出有效势并确立其在零温和有限温度下的稳定极小值，证明了对于大的有限N，Z2对称性在任意高温下都会自发破缺。

Conclusion: 通过大N解析方法，明确了双锥矢量模型与之前发现的高温Z2破缺标量理论族之间的普适类联系，为理解热涨落不破坏对称性的反常现象提供了统一的理论框架。
...
### [137] [Fermionic Villain model with exact lattice chiral symmetries](https://arxiv.org/abs/2608.02728)
*Zhiyao Lu,Shu-Heng Shao*

Main category: cond-mat.str-el

TL;DR: 本文提出了一种精确实现1+1维无质量Dirac费米子的U(1)_L×U(1)_R全局对称性及其手征反常的费米子晶格哈密顿量。该构造将Villain玻色子与Kitaev链Majorana费米子耦合，玻色子场对于规避Nielsen-Ninomiya型no-go定理至关重要。对于模型的两个拷贝，可实现无反常的U(1)_3450全局对称性并构建对称边界条件；通过费米子T-对偶将对称性保持的六费米子相互作用映射为费米子双线性项，解析地证明了对称质量生成。通过规范一般的无反常全局对称性，可获得一类广泛的晶格手征规范理论，并在Schwinger模型和3450规范理论中验证了与连续谱结果的一致性。


<details>
  <summary>Details</summary>
Motivation: 在格点规范理论中实现手征费米子一直是一个长期挑战，因为Nielsen-Ninomiya定理限制了在晶格上无法直接获得只具有单一手征分量的无质量费米子。作者旨在构建一个能够在晶格上精确保持手征对称性及其反常结构的费米子模型，从而为晶格手征规范理论提供新的实现途径。

Method: 作者将Villain玻色子与Kitaev链的Majorana费米子耦合构造哈密顿量，利用玻色子自由度绕过Nielsen-Ninomiya no-go定理的限制。通过取两个模型拷贝实现U(1)_3450无反常对称性，并利用费米子T-对偶（Fermionic T-duality）将对称性保持的六费米子相互作用映射为费米子双线性项，从而解析地展示对称质量生成机制。最后通过规范一般无反常的全局对称性，得到广泛的晶格手征规范理论类别。

Result: 作者成功构造了具有精确U(1)_L×U(1)_R手征对称性的晶格哈密顿量；解析地证明了六费米子相互作用通过费米子T-对偶等价于费米子双线性质量项，实现了对称质量生成；通过规范全局对称性获得了包括Schwinger模型和3450规范理论在内的晶格手征规范理论。并计算了这些理论的质量谱，与连续极限结果一致。

Conclusion: 该工作为晶格上的手征费米子模拟提供了新的可行框架，克服了Nielsen-Ninomiya no-go定理的障碍，并首次在晶格上实现了具有精确手征对称性和反常的费米子模型。通过对称质量生成机制和费米子T-对偶的运用，为研究手征规范理论（包括施温格模型和3450规范理论）提供了新的非微扰工具。
...
### [138] [Helicity in monoaxial chiral magnets](https://arxiv.org/abs/2608.03389)
*Victor Laliena,Diego Gironés-Magaña,Javier Campo*

Main category: cond-mat.str-el

TL;DR: 本文揭示了单轴手性磁体与两个单轴铁磁体接触时涌现的亚稳态螺旋态（具有不同波数/螺旋度）的拓扑起源——即"动力学拓扑保护"机制，并研究了这些态的静态和动态性质，包括弱磁场与极化电流下的拓扑不变量保持、极化电流下的非均匀绕数密度分布，以及强场/电流下的态切换机制，展示了其在自旋电子学和磁子学中的应用前景。


<details>
  <summary>Details</summary>
Motivation: 传统认知认为，被非磁介质包围的单轴手性磁体只能存在单一波矢的平衡螺旋结构，且不存在亚稳态。然而近期实验和理论证明（Phys. Rev. B 109, 214424），当手性磁体与两个单轴铁磁体紧密接触时，会出现大量区别于平衡态的亚稳态螺旋态。本文旨在阐明这些亚稳态得以稳定的拓扑起源，并系统研究其静态与动态特性，为后续应用奠定理论基础。

Method: 本文通过理论分析阐明了亚稳态螺旋态稳定化的拓扑机制，命名为"动力学拓扑保护"（dynamical topological protection）。研究者分析了这些螺旋态的绕数（winding number）在弱磁场和极化电流下的守恒性质，研究了施加极化电流时系统的静态演化和绕数密度的空间分布，并考察了强磁场和大电流动力学响应下不同螺旋态之间的切换机制。

Result: 研究发现：（1）由于动力学拓扑保护，在足够弱的磁场和极化电流作用下，螺旋态的绕数保持不变；（2）对亚稳态螺旋态施加极化电流后，系统达到静态构型，保留原始绕数，但绕数密度变为非均匀分布，集中于与某一铁磁体的界面附近；（3）足够大的磁场和电流能够驱动不同螺旋态之间的切换；由于磁性依赖于螺旋度，这些态在自旋电子学和磁子学中具有很高的应用潜力。

Conclusion: 本文阐明了手性磁体-铁磁体复合结构中亚稳态螺旋态稳定化的拓扑机制（动力学拓扑保护），揭示了绕数在弱外场下的拓扑守恒以及在强外场下的可切换性。这种螺旋度依赖的磁性特性使得这些亚稳态螺旋态成为自旋电子学和磁子学中有前景的功能性体系。
...
### [139] [Vector-field control and emergent basal-plane anisotropy of magnetic textures in noncentrosymmetric (Fe$_{0.63}$Ni$_{0.3}$Pd$_{0.07}$)$_3$P](https://arxiv.org/abs/2608.03513)
...
TL;DR: 本文通过结合共振小角X射线散射和叠层衍射成像技术，系统研究了(Fe₀.₆₃Ni₀.₃Pd₀.₀₇)₃P在矢量磁场和宽温度范围内的拓扑自旋织构行为。发现面内磁场可确定性控制磁条纹畴的取向，通过手性条纹到非手性扇形构型的转变实现连续旋转；50K以下条纹取向被亚稳态钉扎。此外，揭示了冷却过程中各向异性相互作用的显著温度演化，特别是有效DMI方向随温度旋转的非平凡各向异性轴。


<details>
  <summary>Details</summary>
Motivation: 非中心对称磁性材料中的拓扑自旋织构（如斯格明子等）在自旋电子学应用中有重要前景，但对其在矢量磁场和温度调控下的行为理解仍不充分。(Fe₀.₆₃Ni₀.₃Pd₀.₀₇)₃P作为一种具有S₄对称性的室温磁性材料，蕴含丰富的拓扑自旋织构，是研究这些问题的理想体系。此前缺乏对其中磁畴结构随磁场方向和温度演化的系统性研究。

Method: 采用共振小角X射线散射（resonant small-angle x-ray scattering）与叠层衍射成像（ptychography）相结合的实验方法，在矢量磁场（可同时控制面内和面外磁场分量）和宽温度范围内对(Fe₀.₆₃Ni₀.₃Pd₀.₀₇)₃P的磁畴结构进行系统表征。通过分析散射图案和实空间成像，提取磁条纹的取向、波矢大小及各向异性相互作用的演化规律。

Result: 研究发现：(1) 面内磁场可实现磁条纹畴的确定性取向控制，通过手性条纹到非手性扇形构型的转变实现连续旋转；(2) 在50K及以下，条纹取向被亚稳态钉扎并保持磁场训练方向；(3) 室温下波矢大小在基面内近似各向同性，但冷却过程中各向异性相互作用显著演化；(4) 20-50K出现非平凡的各向异性轴，反映磁晶各向异性、各向异性交换和DMI的综合效应，且有效DMI方向随温度旋转。

Conclusion: 这项工作确立了(Fe₀.₆₃Ni₀.₃Pd₀.₀₇)₃P作为矢量磁场控制手性自旋织构的模型系统，并揭示了一种此前未被认识的非中心对称磁体中有效DMI构型随温度驱动的演化机制，为磁性拓扑自旋织构的调控提供了新的物理见解。
...
### [140] [The New High-entropy Compound RhMnFeCoGe4 with Cubic Non-centrosymmetric B20 Structure](https://arxiv.org/abs/2608.03523)
...
TL;DR: 本文报道了一种新型高熵化合物RhMnFeCoGe₄，具有立方非中心对称B20结构，在高温高压条件下合成。该材料表现出铁磁性，居里温度TC=146 K，每公式单元自发磁矩2.5 μB。通过磁化率临界区数据、NMR谱和从头计算，系统表征了其临界指数、各组分磁矩以及电子结构，并发现晶格压缩可提高TC。


<details>
  <summary>Details</summary>
Motivation: 探索新型高熵化合物在磁性材料领域的应用潜力，尤其是具有非中心对称B20结构的体系，这类结构因其独特的拓扑磁性和自旋电子学性质而备受关注。通过高压合成扩展高熵材料的组成空间，并深入研究其磁性与电输运性质。

Method: 在高温高压条件下合成RhMnFeCoGe₄样品；在常压和高压下测量电输运和磁性；在4.2 K下获取核磁共振(NMR)谱以确定各元素磁矩；进行从头算(ab initio)计算以获得晶格参数、磁矩、态密度和能带结构。

Result: 材料呈现铁磁性，TC=146 K，自发磁矩为2.5 μB/f.u.；临界指数为β=0.337(1)、γ=1.121(1)、δ=4.326(1)；NMR确定Mn和Co的磁矩分别为2.2 μB和0.5 μB；从头算给出合理的晶格参数及各组分磁矩；可计算顺磁和铁磁态的态密度与能带结构；晶格压缩导致TC升高。

Conclusion: RhMnFeCoGe₄是一种新型铁磁性B20结构高熵化合物，实验与理论计算相互印证。压力可调控其居里温度（压缩导致TC增大），该材料在磁性材料与自旋电子学领域具有潜在的研究价值。
...
### [141] [Impurities Near the Light Cone](https://arxiv.org/abs/2608.03704)
*Gabriel Cuomo,Simone Giombi,Luigi Tizzano*

Main category: cond-mat.str-el

TL;DR: 本文研究了一般共形线缺陷在洛伦兹号中的类光Wilson线尖点（cusp）类比行为，提出当缺陷存在局域端点算子时，尖点反常维度在强洛伦兹助推极限下趋于由缺陷创建算子标度维度决定的有限值，并分析了cusp的不同解析延拓、物理含义以及洛伦兹cusp反常维度的正性约束，在多种微扰实例（如钉扎场缺陷和自旋杂质）中验证了这些预言。


<details>
  <summary>Details</summary>
Motivation: 类光Wilson线尖点支撑着Sudakov双对数，是规范理论因子化中的核心要素。作者希望理解在更一般的共形线缺陷（非Wilson线）中，什么替代了这种尖点行为，从而深化对一般共形缺陷反常维度的理解。

Method: 通过共形场论中的一般性论证，利用缺陷端点算子的标度维度来确定尖点反常维度在大助推极限下的行为；同时分析cusp的各种解析延拓及其物理诠释，并提出正性约束猜想；最后在钉扎场缺陷和自旋杂质等可微扰计算的例子中进行具体验证。

Result: 证明了当共形线缺陷存在局域端点算子时，尖点反常维度在大助推极限下收敛到由缺陷创建算子标度维度决定的有限值；厘清了cusp函数不同解析延拓的物理意义；提出了洛伦兹尖点反常维度的正性下界；并在多个微扰例子（钉扎场缺陷、自旋杂质）中得到一致验证。

Conclusion: 该工作将类光Wilson线尖点的Sudakov行为推广到了任意共形线缺陷，揭示了尖点反常维度与缺陷创建算子标度维度之间的普适联系，并提出正性约束，为理解规范理论中的相关现象及更一般共形缺陷的动力学提供了新的视角和可检验的预言。
...
### [142] [Surface-Selective Probe of Spin-Triplet Superconductivity in Rhombohedral Graphene](https://arxiv.org/abs/2608.03870)
...
TL;DR: 本文通过在菱方五层石墨烯（R5G）上单侧沉积WS₂，利用位移场调控超导载流子与WS₂界面感应的Ising自旋-轨道耦合的重叠程度，观察到超导性的强烈不对称抑制，从而为自旋三重态超导提供补充证据，并确立单侧TMD近邻效应作为可调的超导自旋结构探针。


<details>
  <summary>Details</summary>
Motivation: 在菱方石墨烯中已观察到多层层数下的超导性，且证据指向自旋三重态配对，但仍然缺乏对超导自旋结构的互补性探测手段，因此需要一种能够直接探测超导载流子自旋性质的实验方法。

Method: 在菱方五层石墨烯上单侧沉积WS₂作为表面选择性自旋-轨道探针。诱导的Ising自旋-轨道耦合对靠近界面的载流子最强，通过调节位移场可控制超导载流子与自旋-轨道微扰的空间重叠。结合栅极追踪特征、量子振荡和自洽能带结构计算，识别相关的层极化和费米面特征，并对比不同位移场符号下的超导口袋（SC1、SC2、SC3）行为。

Result: 观察到强不对称的超导景观：SC1和SC2两个稳健的超导口袋仅在位移场相反符号下分别存活，SC3则明显更弱。当高态密度载流子向WS₂界面极化时，稳健超导态被消除或显著削弱。由于Ising自旋-轨道耦合与自旋单态配对兼容但会通过与自旋三重态配对竞争（倾斜或锁定母体自旋织构）而抑制后者，这种表面选择性抑制为自旋三重态超导提供了额外证据，且涉及空穴型和电子型两种载流子。

Conclusion: 单侧TMD近邻效应可作为位移场可调的超导自旋结构探针，其结果进一步支持菱方石墨烯中的自旋三重态超导电性。
...
### [143] [Thermodynamic phase transition, pairing symmetry and Fermi surface topology in Ruddlesden-Popper nickelate films](https://arxiv.org/abs/2608.03908)
...
TL;DR: 本文通过角分辨光电子能谱研究了新型高温超导Ruddlesden-Popper镍氧化物异质结La₂PrNi₂O₇/NdAlO₃的电子结构，在无赝能隙的情况下直接测量了超导序参量并提取了电子比热，观察到Tc处的超导能隙打开和比热跳变，确立了无节点超导行为和热电动力学证据。


<details>
  <summary>Details</summary>
Motivation: Ruddlesden-Popper镍氧化物为探索高温超导现象和超导机制提供了未开发的新领域，但此前缺乏热力学证据来确认超导相变，且超导序参量的对称性尚未明确。

Method: 采用角分辨光电子能谱(ARPES)研究La₂PrNi₂O₇/NdAlO₃异质结构，结合应变依赖测量，分析费米面拓扑、超导能隙打开过程、相干峰以及电子比热跳变。

Result: 在无赝能隙状态下直接观察到超导能隙在Tc处打开并伴随显著相干峰；电子比热在Tc处出现跳变，证实热力学相变；费米面由α、β、γ三个口袋组成，呈多轨道特征；应变测量显示γ口袋在所有超导和非超导薄膜中均存在。

Conclusion: 该研究提供了镍氧化物中超导相变缺失的热力学证据，直接证明了超导序参量的无节点对称性，并揭示了费米面拓扑与RP镍氧化物薄膜中超导出现之间的关系。
...
### [144] [Computing with traceable tensor networks](https://arxiv.org/abs/2608.02849)
*Sarah Ellwein,Daniele Venturi*

Main category: physics.comp-ph

TL;DR: 本文提出了一种基于SVD的张量分解新方法，适用于任意图拓扑的张量网络，将经典的分层SVD技术扩展到含环和一般连通性的网络，并引入可追踪张量图的加法和取整程序，实现高维PDE的高效图格式时间积分。


<details>
  <summary>Details</summary>
Motivation: 经典的分层SVD张量分解方法（如张量列TT和分层Tucker）主要适用于树状或特定拓扑结构的张量网络，难以处理具有环和更一般连通性的网络结构，限制了其在更复杂高维问题中的应用。

Method: 提出基于SVD的任意图拓扑张量网络分解方法，扩展经典分层SVD技术到含环网络；同时引入可追踪张量图的加法与取整程序，支持直接在图格式下进行逐步取整的时间积分，每步以预定容差控制秩截断。

Result: 在多元函数分解和Fokker-Planck方程数值解上的实验表明，图格式表示在精度上与传统TT和分层Tucker格式相当或更好，同时使用的自由度大幅减少，计算成本更低。

Conclusion: 所提出的任意图拓扑张量分解方法在精度和计算效率上均优于或等同于经典张量格式，为高维偏微分方程的高效数值求解提供了新途径。
...
### [145] [A Unified Graph Neural Network Framework for Non-Equilibrium Carrier and Lattice Dynamics Driven by Electric Fields](https://arxiv.org/abs/2608.03287)
*Jia-Wen Li,Sheng Meng,Xinghua Shi,Jin Zhang,Wei-Hai Fang*

Main category: physics.comp-ph

TL;DR: 本文提出一种电场响应图神经网络（EFR-GNN），统一预测能量、力、Born有效电荷张量、原子分辨电荷和磁矩，支持长时程场驱动分子动力学模拟，并在多个材料体系中验证了其对场驱动原子和局域载流子动力学的描述能力。


<details>
  <summary>Details</summary>
Motivation: 有限温度下电场驱动动力学模拟需要统一描述原子间相互作用、局域电子态和构型依赖的电响应，但第一性原理模拟受尺度限制，传统机器学习势函数缺乏电场效应，而现有机器学习框架通常只涵盖电场响应或原子分辨电子态信息，很少同时包含两者。

Method: 开发了电场响应图神经网络（EFR-GNN），该网络同时预测能量、力、Born有效电荷张量、原子分辨电荷和磁矩，并支持长时程场驱动分子动力学模拟，可原子级追踪局域化电子态。

Result: 在空穴掺杂MgO中，静电场通过最近邻模型量化的前后不对称性整流了热激活的空穴极化子跳跃；在GaAs中，共振太赫兹激发产生与实验一致的相干Γ点横光学声子，且相反螺旋度可反转其旋转方向；在超离子α-AgI中，重现了温度依赖的Ag+迁移率和集体场驱动离子漂移。

Conclusion: EFR-GNN为场驱动原子和局域载流子动力学的有限温度模拟提供了一种统一且可扩展的方法。
...
### [146] [A 3-D Hybrid Numerical Method for Simulating Electromagnetic Fields in Structures with Multiple Inhomogeneous Layered Media](https://arxiv.org/abs/2608.03639)
*Jie Liu,Taihe Li,Ke Chen,Mingwei Zhuang,Qing Huo Liu*

Main category: physics.comp-ph

TL;DR: 提出一种三维混合数值方法（HNM），将三维数值模式匹配（NMM）与基于树-余树（tree-cotree）的混合有限元法（MFEM）相结合，用于处理嵌入多层介质中的任意三维非层状散射区域的电磁散射问题。


<details>
  <summary>Details</summary>
Motivation: 在包含多层非均匀介质与任意三维非层状散射区域的结构中，现有方法（如传统混合模式匹配/有限元法或纯NMM）存在维数约减能力不足或难以处理不规则散射区域的问题，需要一种兼顾二者的高效数值方法。

Method: 将三维NMM与基于tree-cotree的MFEM耦合集成：在NMM中，三维层状介质中的场被分解为二维波导本征模的叠加；MFEM用于离散化三维散射区域。HNM同时继承传统混合MM/FEM与纯NMM的维数约减优势。

Result: 数值实验表明，HNM为嵌入层状介质中的非层状区域散射问题提供了一种高效的计算方案。

Conclusion: HNM结合了NMM与MFEM各自的长处，在保持维数约减优势的同时能够处理任意三维非层状散射区域，是解决此类散射问题的有效替代方法。
...
### [147] [SU3HOB-cgvcs: harmonic-oscillator brackets in the SU(3) basis with isofactors from an external SU(3)$\,\supset\,$SO(3) Clebsch--Gordan library](https://arxiv.org/abs/2608.02640)
*Augustinas Stepšys,Saulius Mickevičius,Darius Germanas*

Main category: physics.comp-ph

TL;DR: 本文介绍SU3HOB-cgvcs，一个Fortran 2008软件包，用于计算任意质量比参数d下的一般Talmi-Moshinsky谐振子变换括号。该括号在SU(3)基中通过Kalinauskas等人的方法构建，变换被简化为对Wigner d函数的单重求和，并利用已有的SU(3) Clebsch-Gordan库（Su3cgvcs）获取所需的iso因子，最终复现现有代码达到机器精度（~10^-13）。


<details>
  <summary>Details</summary>
Motivation: 旨在构建一个自包含的SU(3)方案谐振子括号（HOB）计算代码，证明可以基于通用的SU(3) Clebsch-Gordan库来实现，从而避免重新实现复杂的耦合系数计算，提高代码的可维护性和扩展性。

Method: 采用Kalinauskas等人的方法在SU(3)基中构造Talmi-Moshinsky变换括号，将变换分解为单个求和项（Wigner d-函数乘以SU(3)⊃SO(3) isofactors的乘积）。这些isofactors从外部库Su3cgvcs获取，该库使用矢量相干态（VCS）方法计算。文中对库进行两处修正：重新插入依赖于每个态径向量子数的符号/相位因子，以及在解析上处理平凡的(0,0)表示。

Result: 该软件包能逐括号复现Kamuntavičius等人的一般Talmi-Moshinsky括号代码，精度达到机器精度（约10^-13），验证了方法的正确性。

Conclusion: 证明了可以基于一个通用的SU(3) Clebsch-Gordan库构建自包含的SU(3)方案HOB代码，为量子多体物理中的谐振子变换计算提供了可靠且可复用的工具。
...
### [148] [Integral Hilbert spaces and the dynamics of loop quantum cosmos](https://arxiv.org/abs/2608.02798)
*Janek Kozicki,Tomasz Pawłowski*

Main category: physics.comp-ph

TL;DR: 本文研究了圈量子宇宙学中聚合物量子化产生的不可分希尔伯特空间问题，提出了一种替代方案——通过对所有可用扇区进行积分来构建可分希尔伯特空间，并在平坦各向同性宇宙（含负宇宙学常数和无质量标量场）模型中测试了其动力学行为。数值演化表明，该方案在长期半经典性方面与单一扇区方法无显著差异，同时比较了旧数值方法与更高效的标准工具（eigen library）的性能。


<details>
  <summary>Details</summary>
Motivation: 圈量子引力/宇宙学中的聚合物量子化程序导致不可分希尔伯特空间。通常做法是选取一个可分的超选择扇区来规避此问题，但在更复杂的非各向同性模型中，这种做法往往不再可行。因此需要探索替代方案来构建可分希尔伯特空间。

Method: 作者采用对所有可用扇区进行积分的方式来构建可分希尔伯特空间，以此替代单一扇区方法。在一个具有负宇宙学常数和无质量标量场的平坦各向同性宇宙模型中，对初始半经典态进行数值演化，并与单一扇区方法的半经典性性质进行对比。同时将旧的特定问题数值方法与应用更通用的标准工具（eigen library）进行比较。

Result: 数值实验结果发现，通过积分所有扇区构建的可分希尔伯特空间中的半经典态的长期半经典性性质，与单一扇区方法相比没有显著差异。此外，新的标准数值工具（eigen library）在效率上优于旧的特定问题数值方法。

Conclusion: 该研究验证了通过对所有扇区积分构建可分希尔伯特空间的替代方案在动力学上是可行的，至少在平坦各向同性宇宙这一测试模型中如此；同时展示了采用更通用的高效数值工具在该研究领域的应用优势。
...
### [149] [ScoreField: Neural Inverse Scattering with Score-Based Generative Priors](https://arxiv.org/abs/2608.02937)
*Wenhan Guo,Yuan Gao,Yu Sun*

Main category: physics.comp-ph

TL;DR: ScoreField是一个神经逆散射框架，结合耦合隐式神经表示（INRs）与预训练的基于分数的生成先验。它使用两个INR分别参数化介电常数对比度和感应电流场，并在Lippmann-Schwinger方程约束下联合优化，通过链式法则将分数模型的先验梯度传播到对比度INR，从而有效处理强多次散射问题。


<details>
  <summary>Details</summary>
Motivation: 设计有效的电磁逆散射求解器需要同时满足两点：忠实执行非线性全波物理约束，以及为未知介电常数对比度提供具有表达力的先验。现有方法在强多次散射场景下，非线性波相互作用需要精确的全波物理建模，亟需一种能同时兼顾物理一致性和强先验的方法。

Method: ScoreField的核心方法包括：(1) 使用两个耦合的隐式神经表示（INRs）分别参数化介电常数对比度和感应电流场；(2) 在Lippmann-Schwinger方程下对两个INR进行联合优化；(3) 除了INR架构本身带来的隐式正则化外，利用预训练的基于分数的生成模型提供对比度上的学习先验梯度，并通过链式法则将其传播到对比度INR中。

Result: ScoreField在模拟的弱散射和强散射基准、经典奥地利体模（Austria phantom）以及实验Fresnel测量数据上进行了评估。相比经典全波方法和深度学习基线，ScoreField显著提升了重建保真度并抑制了伪影，在真实Fresnel数据上相比最佳竞争方法平均PSNR提升了1.8 dB。

Conclusion: 通过将隐式神经表示与基于分数的生成先验结合，ScoreField能够有效处理强多次散射场景下的电磁逆散射问题，在重建质量和伪影抑制方面显著优于现有方法，验证了该框架在复杂全波物理条件下的有效性。
...
### [150] [AccelNet: Exact backward-compatible acceleration of polynomial angular descriptors through Cartesian moment factorization](https://arxiv.org/abs/2608.03280)
*Yuki Nagai*

Main category: physics.comp-ph

TL;DR: AccelNet是一种精确、向后兼容的加速方法，用于加速已有的aenet和n2p2神经网络势，无需重新训练。它利用角项中可分离单邻居权重和有限多项式cos θ依赖的隐藏有限秩结构，将显式邻居对循环替换为单邻居笛卡尔矩，从而在保持浮点舍入级精度的同时提升计算速度。


<details>
  <summary>Details</summary>
Motivation: 现有的aenet和n2p2神经网络势在分子动力学模拟中计算成本高，尤其是角项涉及显式的邻居对循环，限制了大规模模拟的效率。需要一种无需重新训练即可加速已有模型的方法。

Method: 识别角项中具有可分离单邻居权重且对cos θ有有限多项式依赖的结构，利用其隐藏的有限秩特性，将显式的邻居对求和分解为单邻居笛卡尔矩的形式，从而消除对级循环。

Result: 在H₂O和TiO₂模型上验证了等价性，生成的描述符、能量和分析力与原始模型在浮点舍入误差范围内一致。在LAMMPS分子动力学模拟中测试了加速后势的性能。

Conclusion: AccelNet提供了一种精确且向后兼容的加速方案，适用于现有aenet和n2p2训练模型，且实现、模型转换工具和LAMMPS接口均已作为开源软件发布。
...
### [151] [A 100x Faster Beam Propagation Method for Nonlinear Optical Wave Propagation Based on Discrete Exterior Calculus](https://arxiv.org/abs/2608.03587)
*Amgad Abdrabou,R. El-Ganainy*

Main category: physics.comp-ph

TL;DR: 本文提出一种基于离散外微积分（DEC）的计算框架，用于求解非线性薛定谔方程，相较传统方法加速超过100倍，同时保持谱级精度，可将大规模多尺度问题的计算时间从数天缩短至数小时。


<details>
  <summary>Details</summary>
Motivation: 复杂光子结构中非线性光传播的高效模拟仍是重大挑战，因为这些系统结合了复杂的横向几何与跨越多个衍射长度的传播距离。现有数值方法要么需要计算密集的均匀离散化，要么难以精确表示复杂的材料边界，限制了多尺度非线性光子器件的实际模拟。

Method: 该方法基于离散外微积分（DEC），允许在非结构化网格上直接进行几何保形离散化，无需传统有限元方法所需的弱形式。与基于傅里叶的谱求解器相比，它避免了全局过采样，消除了材料界面处的Gibbs型振荡，自然地吸纳吸收边界条件，并保持底层微分算子的拓扑结构。关键特征是高阶传播算子可由低阶离散算子算法化生成，为扩展超越标准非线性薛定谔方程的模拟提供了系统性路径。

Result: 在基本孤子、非对称光束和光学涡旋上的基准测试确认了谱级精度，同时展示了超过100倍的计算加速。

Conclusion: 该框架通过几何保形离散化和算子拓扑保持，在保持谱级精度的同时实现了超过两个数量级的加速，为大规模和多尺度非线性光子器件的高效模拟提供了可行的解决方案，并将计算时间从天数缩短到小时级。
...
### [152] [Epitaxial SiGeSn alloys for CMOS-compatible thermoelectric devices](https://arxiv.org/abs/2608.03638)
...
TL;DR: 本文评估了异质外延SiGeSn合金（一种新型CMOS兼容材料体系）的热电性能。基于实验测得的低晶格热导率（~1-2 W/m·K），利用玻尔兹曼输运方程计算品质因数，在300-400K典型芯片工作温度下，p型和n型材料的ZT值均超过1，功率因数达20 μW/cm·K²，表明该材料在热电设备中具有极好的应用前景。


<details>
  <summary>Details</summary>
Motivation: 将热电设备集成到主流微电子技术平台可能是在"绿色IT"领域的重大突破，因此需要评估新型CMOS兼容材料体系SiGeSn合金的热电性能，以探索其在热电设备中的潜在应用。

Method: 利用3-ω方法实验测定SiGeSn/Ge/Si薄膜的晶格热导率（~1-2 W/m·K），并通过玻尔兹曼输运方程计算热电品质因数ZT，计算中考虑了该多谷材料体系特有的谷间散射过程。

Result: 在300-400K的典型芯片工作温度范围内，p型和n型SiGeSn材料的ZT值均超过1，功率因数达约20 μW/cm·K²。

Conclusion: SiGeSn作为新型硅基材料在热电领域具有极佳的实际应用前景，可进一步激发该领域的研究兴趣。
...
### [153] [Analysis of Moment Closures Using $\varphi$-Divergences for Rarefied Dynamics with Binary Collisions and Their Galerkin Discretizations](https://arxiv.org/abs/2608.03640)
*Michael R. A. Abdelmalik,Irene M. Gamba,Torsten Kessler,Sergej Rjasanow*

Main category: physics.comp-ph

TL;DR: 本文提出了一种基于Galerkin方法的确定性框架来近似求解Boltzmann方程，通过φ-散度构造参数化Galerkin闭合族以推导矩方程层级，并设计兼容的近似碰撞算子以恢复熵耗散等物理性质，最终结合熵稳定的间断Galerkin有限元方法实现全隐式时空离散，通过超声速喷管流、通道质量流和平行壁间传热等数值算例验证了方法的鲁棒性与准确性。


<details>
  <summary>Details</summary>
Motivation: 基于φ-散度的Galerkin闭合本身无法保证真实二元碰撞算子的φ-散度熵耗散性质，需要构建兼容的近似碰撞算子来恢复这一关键物理特性，并确保封闭矩系统的对称耗散性及柯西问题的局部适定性，从而为高保真流动模拟提供坚实的数学基础。

Method: 采用族参数化Galerkin闭合求解Boltzmann方程，在速度空间用φ-散度构造矩方程层级；针对每个闭合设计兼容的近似碰撞算子以保持伽利略不变性、质量/动量/能量严格守恒及φ-散度熵严格耗散；空间和时间变量用熵稳定的间断Galerkin有限元离散，形成全隐式熵稳定时空方法。

Result: 封闭矩系统呈对称耗散结构，柯西问题局部适定；全隐式时空方法可超越CFL限制采用大时间步并直接计算稳态；超声速氩气喷管流、通道质量流和平行壁传热等算例与解析基准、实验测量和随机粒子模拟均一致。

Conclusion: 该框架成功地将基于φ-散度的Galerkin闭合与熵稳定DG方法结合，建立了既满足严格物理性质又具备计算效率的Boltzmann方程确定性求解方法，在稀薄气体动力学领域具有广泛的应用前景和验证可信度。
...
### [154] [AFLOW-EMERALD: ElectroMagnetic modes EngineeRing in Advanced LayereD materials](https://arxiv.org/abs/2608.03759)
*Stefano Campanaro,Luca Bursi,Nicholas H. Anderson,Stefano Curtarolo,Arrigo Calzolari*

Main category: physics.comp-ph

TL;DR: 本文介绍了AFLOW-EMERALD，一个开源、模块化、基于Python的计算框架，用于模拟有限和周期性层状（超）材料中的电磁波传播，结合了数值稳定的散射矩阵方法、平面波展开和严格耦合波分析，可计算光学光谱、空间场分布和光子带结构（含复k色散），并通过YAML工作流支持从实验、文献或第一性原理来源导入介电函数数据。


<details>
  <summary>Details</summary>
Motivation: 层状和周期图案化异质结构是先进光学、光子学和等离激元（超）材料的基础，其理性设计需要既数值稳健又与底层材料性质紧密关联的电磁求解器。

Method: 基于统一面向对象架构，结合数值稳定的散射矩阵方法、平面波展开方法，以及适用于光栅等横向图案化结构的严格耦合波分析（RCWA）；提供流线型YAML工作流，支持从实验、文献或第一性原理来源导入介电函数数据集。

Result: 该软件可计算光学光谱、空间场分布和光子带结构（包括有损色散介质中的复k色散），其模块化设计使其易于扩展并适合集成到计算材料设计流程中。

Conclusion: AFLOW-EMERALD为介电光子晶体、等离激元多层结构、双曲超材料以及支持表面和体等离激元-极化激元模式的更复杂架构，提供了材料-几何耦合工程的实际计算平台。
...
### [155] [Reduced-order modeling for electromagnetic inverse problems: a layered medium benchmark](https://arxiv.org/abs/2608.03996)
*Konstantinos Alexopoulos,Josselin Garnier*

Main category: physics.comp-ph

TL;DR: 本文研究了分层介质中逆问题的降阶建模（ROM），从时域测量中恢复阻抗剖面。利用Goupillaud结构将正问题表述为离散动力系统，并引入定义在降阶算子层面的ROM目标函数。在5层介质的多种结构化扰动下的数值实验表明，ROM基反演在干净环境和某些结构化扰动下提供更优的重建，在所有测试情况下均与经典数据失配方法保持竞争力。


<details>
  <summary>Details</summary>
Motivation: 分层介质中的逆问题（从时域测量恢复阻抗剖面）在物探等领域具有重要应用，经典的数据失配目标函数在反演中可能存在局部极值或对扰动敏感等问题，需要探索更稳健的目标函数构造方式。

Method: 采用Goupillaud结构将正问题建模为离散动力系统，通过降阶建模（ROM）构造定义在降阶算子层面的目标函数，替代经典的数据域失配目标。在5层介质上进行数值实验，引入多种结构化扰动进行对比分析。

Result: 在干净环境下，ROM基目标函数能获得比经典数据失配更优的反演重建结果；在某些结构化扰动条件下，ROM方法同样优于经典方法；在其余所有测试场景中，ROM方法始终与经典方法保持同等水平的竞争力。

Conclusion: 基于降阶算子的ROM目标函数是分层介质逆问题中一种有效且稳健的反演策略，在多种情况下优于或至少不劣于经典数据失配方法，为阻抗剖面恢复提供了一种有前景的替代方案。
