...
### [1] [Non-Shattering at and Above the Dynamical Temperature in the Spherical Pure p-Spin Model](https://arxiv.org/abs/2608.14369)
*Taegyun Kim*

Main category: cond-mat.dis-nn

TL;DR: 本文研究球面纯p自旋玻璃中由Ben Arous和Jagannath引入的shattering概念。对于所有p≥3和0<β≤β_sh(p)，作者在两种情况下排除了shattering：q≤2^{-1/2}或q>√((p-2)/(p-1))。对于p=3，这些条件覆盖了所有固定q∈(0,1)，表明在任何T≥T_sh时景观都不会被shatter；对于p≥4，未覆盖的情况局限于特定区间。本文部分解决了原文中的猜想1。


<details>
  <summary>Details</summary>
Motivation: shattering是自旋玻璃理论中描述自由能景观复杂分解的重要概念。Ben Arous和Jagannath此前提出了关于shattering的猜想，本文旨在通过构造性方法在更广的参数范围内排除shattering，从而深化对球面纯p自旋玻璃中玻璃态结构的理解。

Method: 证明结合了三种技术：(1) 对于第一个参数范围(q≤2^{-1/2})，使用确定性的N+1界来排除不相交带中的shattering；(2) 对于第二个参数范围(q>√((p-2)/(p-1)))，提出一般p符号律，证明总标记权重的自由能是次优的；(3) 利用球面码界和Hölder不等式得到额外的依赖于q的障碍，在β≤√log2时排除所有固定重叠参数。

Result: 对于p=3，前两个范围已覆盖所有固定q∈(0,1)，因此在任何T≥T_sh时景观都不会被shatter。对于p≥4，未被准则覆盖的情况限制在2^{-1/2}<q≤√((p-2)/(p-1))且√log2<β≤β_sh(p)的区域内。

Conclusion: 本文部分解决了原论文中的猜想1，并通过引入N+1界、一般p符号律以及球面码界与Hölder不等式等新方法，为在更广参数范围证明非shattering提供了新工具和途径。
...
### [2] [Extinction drives emergent metastability in complex ecosystems](https://arxiv.org/abs/2608.14416)
*Jong Il Park,Tim Rogers,Joseph W. Baron*

Main category: cond-mat.dis-nn

TL;DR: 本文通过将种群统计学随机性引入大尺度复杂生态系统模型，发现该随机性会迅速清除低丰度物种，提升系统稳定性，且存活物种比例呈现异常重尾衰减，揭示出具有稳健亚稳态的残余群落；即使系统在确定性极限下被预测为混沌，考虑人口学波动后生态系统通过降低多样性实现自我稳定。


<details>
  <summary>Details</summary>
Motivation: 传统稳定性判据基于确定性框架，忽视了群体固有的种群波动，而现实中任何物种都可能因随机性而偶然灭绝；作者希望弥补这一空白，考察人口统计学随机性如何影响大尺度生态系统的稳定性与多样性关系。

Method: 构建一个基于规则的模型，将人口统计学随机性纳入大尺度复杂生态系统；采用自下而上的理论框架刻画灭绝动力学的统计特征，分析存活物种比例随时间的衰减行为。

Result: 研究发现人口学波动会快速剪除低丰度物种，从而提高系统整体稳定性；存活物种比例随时间呈现异常重尾衰减，表明残余群落具有稳健的亚稳态；即使在确定性极限下系统被判定为混沌，考虑人口学波动后生态系统仍能通过减少多样性实现自我稳定化。

Conclusion: 人口统计学随机性在生态稳定化中扮演关键角色：生态系统通过自身多样性的降低来换取稳定性，这一机制在确定性理论失效的混沌区域依然有效，为稳定性-多样性辩论提供了新的理论视角。
...
### [3] [A Quantum Optimization Framework for Data-Assimilation-Augmented Parameter Estimation](https://arxiv.org/abs/2608.13614)
*Muhammad Jalil Ahmad,Mohammadhossein Mohammadisiahroudi,Animikh Biswas,Kathleen Hoffman*

Main category: quant-ph

TL;DR: 本文提出一种混合经典-量子计算框架，将非线性动力系统的参数估计问题重构为二次无约束二元优化（QUBO）问题，并映射到Ising哈密顿量上，利用量子优化器搜索低能构型以获取候选参数估计。该方法在SIS/SIR流行病模型、Lorenz-63混沌系统和高维Lorenz-96系统上验证，能仅基于粗网格上的数据同化求解准确恢复参数，且无需量子态层析。


<details>
  <summary>Details</summary>
Motivation: 参数估计是ODE模型标定中的基础性挑战，传统方法需反复数值积分，计算成本高昂。作者旨在探索量子算法能否帮助降低非线性动力系统参数估计的计算负担，并规避量子态层析等实际实现障碍。

Method: 框架将数据同化增强的参数估计问题重构为组合优化任务。模型动力学与数据同化在经典端完成，而参数估计代价泛函被离散化并近似为QUBO代理，再映射为Ising哈密顿量，最后由量子优化器搜索低能构型以获得参数估计。整个过程仅在预设粗网格上进行数据同化解算。

Result: 在SIS/SIR模型、Lorenz-63混沌系统及双层Lorenz-96高维系统上的合成数据实验中，该方法能准确恢复系统参数，覆盖稳态、混沌及高维多尺度动力学场景，且避免了量子态层析的需求。

Conclusion: 该混合量子优化框架为非线性动力系统的数据驱动参数估计提供了一条可行路径，表明量子优化可有效集成到经典计算管线中，同时规避了全量子实现的技术障碍。
...
TL;DR: 本文提出了一种可扩展的边沿通带Purcell滤波器，通过单一传输边沿将读出频带与受保护的量子比特频带分离，消除了读出谐振器的带宽限制。高通滤波器(HPF)和低通滤波器(LPF)分别实现了99.46%和99.49%的平均读出保真度，单比特门保真度达99.94%，同时提供强Purcell保护和有效的量子比特复位通道。


<details>
  <summary>Details</summary>
Motivation: 传统带通Purcell滤波器存在有限通带和较大尺寸的问题，难以扩展到大规模超导量子处理器。需要一种既能提供强Purcell保护实现高保真读出，又具有紧凑可扩展性架构的解决方案。

Method: 提出边沿通带Purcell滤波器架构，通过单一传输边沿分离读出频带与量子比特保护频带。根据传输频带位于截止频率的上方或下方，分别实现为高通滤波器(HPF)或低通滤波器(LPF)。利用滤波器的固有耗散模式作为量子比特复位通道，实现紧凑的集成架构。

Result: HPF在150ns脉冲下达到99.46%(最高99.56%)的平均读出保真度，LPF在130ns脉冲下达到99.49%(最高99.57%)。平均单比特门保真度HPF为99.94%、LPF为99.93%。相比无滤波器的Purcell极限，两种滤波器均显著延长了量子比特寿命，且滤波器阶数越高Purcell保护越强。

Conclusion: 该Edge-pass Purcell滤波器在单一紧凑结构中统一实现了快速高保真读出、Purcell保护和有效复位功能，为大规模容错量子计算提供了可扩展的架构方案。
...
### [5] [Heuristic Lookahead Distillation Protocol Search](https://arxiv.org/abs/2608.13644)
*Matthew Barber,Stefano Pirandola*

Main category: quant-ph

TL;DR: 本文介绍了一种搜索纠缠蒸馏协议的新方法，并利用该方法以比以往协议更高的速率蒸馏量子比特Werner态，同时改进了去极化信道双向辅助量子容量的最佳已知下界。


<details>
  <summary>Details</summary>
Motivation: 纠缠蒸馏是量子中继器的核心操作，对长距离量子通信和分布式量子计算至关重要。然而，寻找高效的蒸馏协议具有挑战性，且去极化信道双向辅助量子容量下界这一量子信息理论长期开放问题需要改进。

Method: 作者引入了一种系统化的方法来搜索纠缠蒸馏协议，利用该方法设计了更高效的蒸馏策略，用于处理量子比特Werner态。

Result: 新方法实现了比此前已知协议更高的Werner态蒸馏速率，并在广泛的信道参数范围内改善了去极化信道的双向辅助量子容量的最佳已知下界。

Conclusion: 该工作展示了新搜索方法的优势，在量子信息理论的一个长期悬而未决的问题上取得了实质性进展，为未来更高效的纠缠蒸馏协议和量子通信提供了新的途径。
...
TL;DR: 该论文提出了一个量子几何理论框架，用于统一刻画周期性驱动量子系统中的Floquet-Bloch态（包括频闪时刻和微运动过程）。该框架基于时间演化的Floquet投影算符，将静态量子几何、微运动算符几何和Floquet拓扑联系起来，并引入了Floquet投影量子度量来表征驱动手性对称可积自旋链中局域态的空间呼吸行为。


<details>
  <summary>Details</summary>
Motivation: 周期性驱动空间周期量子系统能够产生静态晶体中不存在的能带结构，但目前缺乏一个统一的理论框架来同时描述频闪时刻和微运动过程中的Floquet-Bloch态的几何性质，特别是量子几何与Floquet拓扑之间的深层联系尚未被充分揭示。

Method: 论文构建了一个基于时间演化Floquet投影算符的量子几何理论框架，该框架将静态量子几何、微运动算符几何与Floquet拓扑统一起来。具体地，作者引入了Floquet投影量子度量这一新工具，并通过布里渊区积分来刻画局域态在周期性驱动下的空间振荡方差。

Result: 研究发现：Floquet投影量子度量在布里渊区上的积分能够捕捉微运动过程中的振荡方差；在对称性选择的特定时刻，该积分被Floquet拓扑不变量所下界限制；该框架为周期性驱动晶格系统中微运动动力学的系统性研究提供了有效途径。

Conclusion: 该工作建立了一个统一的量子几何框架来刻画Floquet系统的几何与拓扑性质，为研究周期性驱动量子系统中微运动动力学的几何根源提供了新的理论工具，特别是在手性对称可积自旋链中展示了其应用价值。
...
TL;DR: 本文通过数值模拟研究了量子体积（Quantum Volume）协议中退极化噪声、二分负度纠缠度量和重输出概率（HOP）之间的权衡关系，发现由于有限尺寸效应，在纠缠已被破坏的情况下HOP仍可能大于0.5，甚至可能超过2/3的通过阈值，表明NISQ计算机在某些小规模情形下可能通过量子体积基准测试但缺乏全局纠缠。


<details>
  <summary>Details</summary>
Motivation: 量子体积协议被广泛用作NISQ量子计算机的基准测试，但其测量到的重输出概率（HOP）是否真正反映量子计算机的相干性和纠缠能力尚未被充分验证。本文旨在通过数值研究揭示在退极化噪声影响下，HOP与量子纠缠（以二分负度衡量）之间的关系，并检验量子体积基准测试的可靠性。

Method: 作者对量子体积电路进行了数值模拟，研究了三种量之间的权衡关系：退极化噪声参数、以二分负度（bipartite negativity）量化的纠缠程度、以及重输出概率（HOP）。通过有限尺寸系统的数值研究，分析了不同噪声水平下HOP与纠缠状态的变化关系。

Result: 研究发现，在退极化噪声下，由于有限系统尺寸效应，HOP可能大于0.5（对应完全失去相干性的阈值），而二分负度纠缠已经被破坏。进一步表明，对于小规模n，NISQ计算机在缺乏全局纠缠的情况下仍有可能通过量子体积基准测试的2/3阈值，尽管概率较低。

Conclusion: 量子体积基准测试存在局限性：HOP作为指标可能无法可靠地反映底层量子计算的纠缠特性，尤其在有限尺寸系统中。这一发现对量子体积基准作为NISQ设备性能衡量标准的可信度提出了质疑，并强调了需要结合纠缠度量等额外指标来全面评估量子处理器的能力。
...
TL;DR: 本文提出利用开放系统探针（如一个量子比特）耦合到量子多体环境（如横场伊辛模型），通过探针的非马尔可夫速率或谱流来表征环境的量子多体性质，包括识别不动点、推断相关场的标度维度以及推导重整化群流。


<details>
  <summary>Details</summary>
Motivation: 传统上表征量子多体系统需要复杂的直接测量手段。本文旨在探索一种更高效的替代方案：通过将量子多体系统作为工程化环境，将环境的长程空间关联映射到单个简单量子探针的动力学关联上，从而利用探针演化实现高效表征。

Method: 以耦合到横场伊辛模型的单量子比特为探针，研究其开放系统演化。通过分析探针的非马尔可夫速率或谱流，建立其与量子临界系统中稳定/不稳定不动点、相关场标度维度以及重整化群流之间的对应关系。

Result: 证明了非马尔可夫速率或谱流可以成功用于识别系统的稳定与不稳定不动点，推断相关场的标度维度，并推导出围绕任意不动点变形所诱导的重整化群流。

Conclusion: 开放系统探针提供了一种高效表征量子多体系统的手段，通过将环境的空间关联映射为探针的时间演化关联，可以从单探针动力学中提取关键的量子临界信息，为量子多体系统表征开辟了新途径。
...
TL;DR: 本文提出了一种结合经典与量子算法的混合策略来解决哈密顿截断方法中截断希尔伯特空间快速膨胀导致的计算成本瓶颈问题。该策略包含三部分：基于整数划分的高效基组生成方案、利用对称性加速稀疏哈密顿矩阵构建的算法，以及探索量子Krylov对角化求解低能谱。在二维时空下的自由有质量标量场和φ^4理论的基准测试中，显著提升了哈密顿截断的计算效率，并为未来的量子实现开辟了路径。


<details>
  <summary>Details</summary>
Motivation: 哈密顿截断是量子场论的非微扰求解方法，但其精度受限于截断希尔伯特空间的快速膨胀，导致计算成本急剧上升。作者旨在克服这一瓶颈，通过结合经典与量子算法来提升计算效率，并为未来量子计算实现铺路。

Method: 作者提出三管齐下的混合策略：(1) 基于整数划分构建高效的基组生成方案；(2) 利用对称性感知算法加速稀疏哈密顿矩阵的构建；(3) 探索量子Krylov对角化方法求解低能谱。基准测试针对二维时空下的自由有质量标量场和φ^4理论。

Result: 在自由有质量标量场和φ^4理论（二维时空）的基准测试中，该方法在哈密顿截断的计算效率上取得了实质性提升。

Conclusion: 该混合经典-量子策略显著提高了哈密顿截断方法的计算效率，并为未来在量子硬件上实现相关量子场论计算描绘了可行路径。
...
TL;DR: 本文提出一种基于关联张量子扇区长度的判据来认证真正的多体纠缠，该判据可通过部分随机化测量进行评估，避免了随量子比特数呈指数增长的测量需求，并在离子阱量子计算机上成功认证了真正的五体纠缠。


<details>
  <summary>Details</summary>
Motivation: 在量子系统中认证真正的多体纠缠通常需要随系统规模指数增长的测量数量，这对实际量子系统的纠缠验证构成了重大挑战。作者旨在开发一种新的判据，能够在不引入显式指数依赖的情况下高效认证多体纠缠。

Method: 提出一种基于限制在局域测量平面上的关联张量子扇区长度的新判据。该判据利用部分随机化测量进行评估，避免了显式的指数依赖。推导了k-可分离态的相应上界，并使用代表性多体纠缠态族进行说明。

Result: 该判据可高效评估，并在离子阱量子计算平台上成功演示了真正的五体纠缠认证，证明了该方法的实际可行性。

Conclusion: 基于关联张量子扇区长度的判据为多体纠缠认证提供了一种高效且实用的方案，大幅降低了测量复杂度，并在真实量子硬件上得到了验证。
...
TL;DR: 本文概述了分子自旋量子比特的最新化学理论进展，涵盖从从头算电子结构到开放量子系统动力学的完整理论链条，旨在通过理解量子退相干机制来指导长寿命分子量子比特的理性设计。


<details>
  <summary>Details</summary>
Motivation: 分子自旋是量子信息科学中极具潜力的平台，具有化学可调性和可寻址性等优势，但实现这一潜力需要深入理解并抑制量子退相干，此前缺乏从电子结构到量子动力学的系统性理论框架。

Method: 论文采用多层次的综合理论方法：从多参考和相对论电子结构方法出发参数化有效自旋哈密顿量（包括g张量、零场劈裂和超精细相互作用），然后通过自旋-声子和自旋-自旋耦合模型量化T1和T2弛豫，并评估从因子化到矩阵乘积态方法的动态方法层级，最后将理论模型映射到分子架构以建立设计原则。

Result: 该综述建立了从电子结构到相干动力学的完整理论框架，评估了不同动力学方法（从因子化到矩阵乘积态）在计算成本、精度和通用性之间的权衡，并提出了如同位素取代和空间自旋离域化等具体的设计原则来延长相干寿命。

Conclusion: 通过将理论模型与分子架构相关联，可以建立理性设计原则（如同位素取代和空间自旋离域）来理解和延长相干寿命，为长寿命分子量子比特的设计提供了系统化的理论指导。
...
TL;DR: 本文研究了一种“未来引用”量子过程：从正向模拟中提取的信息被反馈到该模拟动力学更早的内部时刻。外部视角这是一个普通因果有序电路，内部却具有未来指向性。通过将过程张量与泄漏仪器和控制器收缩，可诱导消息寄存器上的完全正迹保持映射，作者用五个操作性性质（稳定性、信息量、可反馈性、相干性、量子关联保持）对其不动点进行分类，并用四个命题将常被混淆的概念分离，辅以机器验证的区间算术证书。


<details>
  <summary>Details</summary>
Motivation: 非正式讨论中“来自未来的信息”往往混淆了多种相互独立的概念（如相干性、信息量、可反馈性、纠缠保持等）。作者希望严格区分这些性质，并通过构造性例子说明它们之间的微妙关系，特别是证明“可反馈的未来记录”与“纠缠破坏”并非必然同时成立。

Method: 通过将过程张量（process tensor）与泄漏仪器（leakage instrument）和控制器（controller）收缩，构造消息寄存器上的完全正迹保持（CPTP）映射。引入五个操作性性质对不动点分类。构造两类参数化酉膨胀族与部分交换族：两参数酉膨胀族给出闭式全局吸引相干不动点；四参数部分交换族放开正交性条件。结合外向舍入区间包络与扰动界（追踪信道及稳态漂移），利用机器验证的球算术（ball-arithmetic）证书对参数区域进行严格认证。

Result: 四个主要结果：（1）两参数酉膨胀族存在闭式、全局吸引、相干的不动点，但在未来记录完全可区分时纠缠破坏（Lemma 1）；（2）四参数部分交换族存在非空的非纠缠破坏开区域（Proposition 2）；（3）给出显式的 Choi 部分转置邻域半宽 0.0163π（Proposition 3）；（4）在参数方片半宽 0.0013π 内，反馈信道同时严格收缩（margin ≥ 0.237）、相干（≥ 0.416）、关于目标未来变量信息量 ≥ 0.172 比特、非纠缠破坏（NPT margin ≥ 0.188）（Proposition 4）。直接数值评估显示四种性质在大一个数量级的区域持续成立，认证方片仅是可行性证明而非相界。

Conclusion: 本文通过严格的构造与机器验证，系统地分离了“信息来自未来”讨论中被混淆的多种操作性概念，证明存在一个非空的参数区域，其中反馈信道可以同时具备严格收缩性、相干性、信息性和非纠缠破坏性。该方法论为量子因果序与未来反馈协议提供了精确的可验证定量框架。
...
TL;DR: 该论文研究了1+1维交错费米子哈密顿量中手征反常对纠缠不对称性的影响，发现尽管对易子在热力学极限下趋近于零，纠缠不对称性仍保持非零，并表现出新奇的标度行为。


<details>
  <summary>Details</summary>
Motivation: Shao等人最近证明1+1维交错费米子哈密顿量具有一整族在热力学极限下流向相同轴荷的格点算符代数。在格点上，主轴荷与矢量荷不对易，尽管它们的对易子预期在热力学极限下消失，这提供了手征反常的格点实现。作者希望利用纠缠不对称性研究这一反常对基态的影响，以深入理解反常在格点系统中的表现。

Method: 利用纠缠不对称性这一工具，通过计算基态中轴电荷与矢量电荷之间的对称性破缺程度，来分析手征反常对基态的影响。重点考察纠缠不对称性在热力学极限下的行为及其标度规律。

Result: 出乎意料地发现，尽管对易子在热力学极限下趋于零，纠缠不对称性在热力学极限下仍保持非零，并且展现出一种新颖的标度行为。

Conclusion: 手征反常在格点上对基态纠缠结构有深远影响：即使对易子消失，纠缠不对称性仍不消失，表明异常在热力学极限下依然具有物理可观测效应，且其标度行为与常规情况不同，揭示了反常在格点体系中更丰富的物理内涵。
...
TL;DR: 本文研究了1D费米-哈伯德模型的Néel淬火动力学，证明通过经典张量网络模拟（横向张量网络收缩，键维数32即可）可以比量子硬件更快、更准确地获得7260条可观测轨迹，并将收敛的轨迹外推至比硬件模拟和其他经典模拟更长的时间。


<details>
  <summary>Details</summary>
Motivation: 近年来1D费米-哈伯德模型的Néel淬火动力学已在量子硬件上得到模拟，但量子硬件模拟存在噪声、时间限制等问题。作者希望验证经典张量网络方法能否以更高效率和准确性复现（甚至超越）量子硬件的实验结果，从而为这类问题提供更可靠、可扩展的经典计算方案。

Method: 采用经典张量网络模拟方法，核心是横向张量网络收缩（transverse tensor network contraction）技术，使用键维数（bond dimension）为32的张量网络来复现量子硬件实验中测量的7260条可观测轨迹。

Result: 仅需键维数32的张量网络收缩即可准确复现量子硬件实验的全部7260条可观测轨迹，且计算速度更快、精度更高；同时，该方法能将收敛的可观测轨迹扩展到比硬件模拟和其他近期经典模拟更长的时间范围。

Conclusion: 经典张量网络模拟（特别是横向张量网络收缩）是模拟1D费米-哈伯德模型Néel淬火动力学的强有力工具，不仅能够高效准确地复现量子硬件结果，还具备更优的时间外推能力，表明在该问题上经典方法仍具有显著的竞争力。
...
TL;DR: 本文提出了一种用于估计厄米矩阵一般矩阵函数性质的量子算法，利用经典后处理随机选择的乘积公式电路（数学上对应Richardson外推的近似），仅需单个辅助比特，具有交换子缩放、更低电路深度等优势，并扩展到量子态测量分布估计，数值实验验证了其在某些参数区域显著优于标准乘积公式。


<details>
  <summary>Details</summary>
Motivation: 量子计算中需要高效估计矩阵函数性质（如相位估计、Green's function、含时演化态的测量分布），但现有方法（如乘积公式）电路深度较高，且通常局限于标准可观测量的期望值估计，缺乏对测量分布层面的估计能力。

Method: 核心原语是对随机选择的乘积公式电路进行经典后处理，数学上等价于Richardson外推的近似。该方法仅需单个辅助量子比特，在矩阵参数上展现出与乘积公式类似的交换子缩放，同时在其他参数上具有更低的电路深度。此外还引入了超越标准可观测量估计的量子态测量分布近似协议，并给出了k-局域相互作用、长尾矩阵系综和守恒量等实际系统的紧致门复杂度界。

Result: 数值实验表明，在特定参数区域中该方法能达到比标准乘积公式显著更浅的电路深度，并展示了启发式应用的潜力。同时为k-局域相互作用、长尾矩阵系综和守恒量系统提供了更紧的门复杂度界限。

Conclusion: 该方法提供了一种灵活的框架，在保持交换子缩放的条件下降低电路深度，并扩展了量子算法的适用范围至测量分布估计，具有实际应用潜力。
...
### [16] [de Broglie-Bohm Dynamics with Schrödinger Source Fields: A Framework for Subquantum Theory](https://arxiv.org/abs/2608.13894)
*Said Mikki*

Main category: quant-ph

TL;DR: 本文通过向薛定谔方程引入复源场来扩展de Broglie-Bohm导引波理论，系统地研究了其对量子平衡与非平衡动力学的影响，推导了源修正的Hamilton-Jacobi方程、连续性方程、非平衡比输运方程及精确熵产生公式，并展示了三个应用：驱动指数驰豫到量子平衡、建立Prigogine型双线性熵产生的亚量子热力学、以及精确抵消Bohm量子势实现经典化轨迹。


<details>
  <summary>Details</summary>
Motivation: 在标准de Broglie-Bohm理论中，物理粒子分布P（而非Born密度|ψ|²）承载系综概率，这一自由度允许在不违反总粒子概率守恒的前提下修改导引波。作者旨在利用这一特性引入源场，为研究量子非平衡动力学、熵交换和经典化现象提供一个统一的理论框架。

Method: 方法上，作者将复源项直接加入薛定谔方程，并基于dBB理论中粒子分布P与Born密度分离的核心假设（即允许非平衡态f=P/|ψ|²≠1），推导出源修正的Hamilton-Jacobi方程和连续性方程，进而导出非平衡比的输运方程以及精确的熵产生公式。通过设计特定形式的源函数，分别实现三种物理情境：指数驰豫到量子平衡、Prigogine型双线性熵产生热力学、以及量子势的精确抵消实现经典轨迹。

Result: 研究取得了三项主要结果：(1) 适当设计的源可以使系统指数驰豫至量子平衡；(2) 熵产生率具有Prigogine型双线性形式，支持熵产生与熵提取两种热力学体制；(3) 调谐的源能精确抵消Bohm量子势，使粒子沿经典轨迹运动，同时导引波仍保留非平凡结构且非平衡比保持守恒。

Conclusion: 该框架为源驱动的量子非平衡、熵交换和经典化现象提供了统一的描述平台，并明确指出源场的物理与本体论地位是一个值得深入研究的开放问题。
...
### [17] [Robust Quantum Extremal Numbers](https://arxiv.org/abs/2608.13907)
*Wanchen Zhang,Zicheng Han,Xiande Zhang*

Main category: quant-ph

TL;DR: 本文建立量子极值数在"近似最大混合边缘态"条件下的稳定性理论。通过定义边缘最大混合缺陷D_A，证明了对4m量子比特纯态满足局部稳定性不等式，进而得到8量子比特系统在ε<1/5时稳定性平台Q_ex,ε^D(8,4)=56，并在奇数系统中利用精确禁子超图导出有限误差稳定性半径。


<details>
  <summary>Details</summary>
Motivation: 绝对最大纠缠态要求所有不超过半数粒子的约化态都是最大混合态，这一条件过于严格，且对于量子比特系统往往不可实现。先前工作引入了量子极值数来最大化恰好最大混合的半体边缘态数量，但精确结果难以推广。本文的核心动机是将这一精确极值问题稳健化——即使边缘态不完全最大混合（存在微小缺陷），量子极值数是否仍保持稳定？这需要建立从精确Turán型障碍到定量鲁棒性陈述的桥梁。

Method: 1) 定义边缘最大混合缺陷D_A=2^{|A|}Tr(ρ_A²)-1，并定义Q_ex,ε^D(n,k)为n量子比特纯态中满足D_A≤ε的k体边缘态的最大数目；2) 对4m量子比特纯态证明局部稳定性不等式Σ_{i∈T}D_{T\{i}}≥1 (|T|=2m+1)，从而导出ε<1/(2m+1)时ε-好2m子集的超图是K_{2m+1}^{(2m)}-自由的；3) 组合已知的8量子比特精确构造，得到稳定性平台Q_ex,ε^D(8,4)=56 (0≤ε<1/5)；4) 对2k+1量子比特奇系统，利用精确禁子超图H_k推导显式有限误差稳定性半径，得到Q_ex,ε^D(9,4)≤120 (0≤ε<1/17)。

Result: 主要结果包括：(1) 建立4m量子比特系统的局部稳定性不等式，将边缘缺陷的求和下界与1联系起来；(2) 通过超图K_{2m+1}^{(2m)}-自由条件，证明8量子比特系统在ε∈[0,1/5)区间内量子极值数保持为56，形成稳定性平台；(3) 对9量子比特系统，利用精确禁子超图H_k得到上界120，稳定性半径为ε<1/17。这些结果将精确量子Turán障碍转化为定量鲁棒性结论。

Conclusion: 本文证明了量子极值数在有限误差范围内具有稳定性：当边缘态的缺陷足够小（在特定阈值之内），量子极值数不会因近似而改变。这一发现表明精确量子Turán型构造具有一定的"刚性与稳健性"折中性质，并为近似量子纠缠态的极值问题提供了新的理论工具。
...
### [18] [Maximizing Nonclassicality of Massive Objects via Quantum Zeno Effect](https://arxiv.org/abs/2608.13942)
*Debarshi Das,Pritam Roy,Marko Toroš,Hendrik Ulbricht,Dipankar Home,Sougato Bose*

Main category: quant-ph

TL;DR: 本文提出利用量子Zeno效应（QZE）通过重复测量产生的累积量子扰动，实现对宏观系统中原本微弱且受退相干抑制的非经典信号的放大幅度和可调谐增强，并结合光机械振荡器方案给出了可检验的无漏洞实验设计。


<details>
  <summary>Details</summary>
Motivation: 在宏观领域测试量子力学面临的核心挑战是环境退相干会迅速压制非经典特征，使得可观测的非经典信号极其微弱。作者希望通过引入量子Zeno效应来放大由测量诱导的量子扰动，从而在宏观系统中增强可观测的非经典性，推动宏观量子力学检验的可行性。

Method: 论文提出利用QZE实现非经典性的可调谐放大：通过重复测量产生的累积量子扰动来增强本征非经典信号，放大幅度由测量次数控制。具体方案为：采用大规模机械振子（需进行捕获和基态冷却），通过分束器型相互作用将机械振子与光场耦合，随后进行光子探测。该方案设计为无漏洞检验，并用量子见证（witness）来量化非经典性的放大程度。

Result: 分析表明，该放大方案即使在光机械阻尼的现实参数区间内，且对于足够大的质量，所提出的可检验见证仍然能显著可观测，从而可以在宏观域中演示量子Zeno效应。

Conclusion: 研究证实了QZE可以作为宏观量子力学测试中有效放大非经典信号的工具，其放大效果在现实退相干条件下依然保持鲁棒性，为在宏观尺度验证量子力学提供了可行的实验途径。
...
### [19] [Neural decoders for subsystem many-hypercube codes](https://arxiv.org/abs/2608.13975)
*Ryota Nakai,Hayato Goto*

Main category: quant-ph

TL;DR: 本文为子系统多超立方体（MHC）码开发了基于神经网络的解码器，在电路级噪声模型下利用规范测量信息提升解码性能，并证明循环神经网络解码器优于全连接解码器且能泛化到比训练更长的序列。


<details>
  <summary>Details</summary>
Motivation: 量子纠错码需要高性能解码器才能发挥最大潜力。子系统MHC码兼具高编码率和低权重综合征测量优势，但规范自由度的引入使解码更具挑战性，现有解码方法不足以应对。

Method: 针对子系统MHC码设计神经网络解码器，在电路级噪声模型下工作；通过精心安排综合征测量序列使规范测量信息可用于解码；对比全连接神经网络与循环神经网络解码器的性能差异。

Result: 规范测量信息经合理排列后可改善解码性能；循环神经网络解码器优于全连接解码器，并且能够解码比训练时更长的综合征测量序列，展现出良好的泛化能力。

Conclusion: 神经网络解码器，特别是循环神经网络，适用于子系统MHC码并能充分利用规范自由度信息，为高编码率量子纠错码的实际解码提供了可行方案。
...
### [20] [Limits of independent and identical measurements for quantum illumination with an unknown return phase](https://arxiv.org/abs/2608.13997)
*Ko Shiraiwa,Shingo Kukita*

Main category: quant-ph

TL;DR: 该论文研究未知返回相位条件下的量子照明目标检测问题。作者将目标检测建模为复合假设检验，其中返回相位是未知常数，并将接收机限制为对每个副本的独立同分布测量。他们证明了在低反射率条件下，纠缠并不带来优势：无纠缠的相干光配合零差探测已能达到最坏情况下的错误指数界，相位对相干零差接收机无不利影响。因此，在该设定下纠缠不提供量子优势。


<details>
  <summary>Details</summary>
Motivation: 量子照明利用信号与保留闲置光束之间的纠缠，在已知返回相位时可显著提升目标检测性能（错误指数提升约6 dB）。然而实际应用中，返回相位由目标距离及表面条件决定，难以事先获知。作者希望探究当相位未知时，量子照明的优势是否依然存在，从而填补这一关键理论空白。

Method: 本文将目标检测视为复合假设检验问题，其中返回相位是所有试验共有的未知常数。作者将接收机限制为对每个副本进行独立同分布（iid）测量，并在低反射率条件下推导出每个此类测量及每个输入态（单信号模式加任意维数闲置态）的最坏情况错误指数界。通过数学分析，作者证明无纠缠的相干光配零差探测即可饱和该界。

Result: 在未知返回相位的设定下，纠缠不提供任何优势。相干光配零差探测对每个相位值都能达到最坏情况错误指数界。由于独立同分布测量类包含许多可实现的量子照明接收机（如光参量放大器和相位共轭接收机），结果表明这些接收机均无法在反射率一阶项上实现对相位的量子优势。

Conclusion: 量子照明的纠缠优势依赖已知返回相位这一前提条件。当相位未知时（实际应用中常见），采用独立同分布测量的所有量子照明接收机在最坏相位情况下均无量子优势。相干光配零差探测已达到最优性能，这为实际目标检测系统的设计提供了重要指导。
...
### [21] [Circuit Depth Compression via Spectral Gap Amplification in Quantum Phase Estimation](https://arxiv.org/abs/2608.14002)
*Sk Mujaffar Hossain,Satadeep Bhattacharjee*

Main category: quant-ph

TL;DR: 通过在使用量子相位估计（QPE）前对输入算子施加sigmoid谱滤波器进行预处理，可显著压缩QPE电路的深度。软阶梯变换放大了有效能隙，减少了所需精度量子比特数，实现了高达27倍的深度缩减和21倍的CX门缩减，并在LiH分子中使QPE输出保真度从0.66提升至0.98。


<details>
  <summary>Details</summary>
Motivation: 标准量子相位估计（QPE）对于具有小谱间隙的系统需要大量精度量子比特和指数级电路深度，这对含噪声中等规模量子（NISQ）硬件构成沉重负担。作者试图通过谱域预处理来放大有效能隙，从而在不修改QPE本身的前提下压缩电路深度。

Method: 在QPE施加之前，对输入算子应用参数化的sigmoid软阶梯谱变换f(λ; τ, w)。该变换可放大有效谱间隙（当w<1/4时Δ_f>Δ_λ），从而将所需精度量子比特从m减少到m_f=ceil(log2(1/Δ_f))。作者证明压缩深度上界为log2(1/(4wΔ_λ))+1，并要求阈值参数τ仅需O(w)精度即可避免经典预处理（如协方差对角化或CASSCF）的循环性。作者还给出了净资源优势的条件不等式以及负对照测试条件。

Result: 在LiH和BeH2键拉伸计算、经典协方差数据集和合成近简并案例上验证：深度缩减最高27倍，CX门缩减最高21倍。LiH在1%硬件错误率下QPE输出保真度从0.66提升到0.98。方法将主子空间保持到机器精度，负对照测试确定优势条件为m_raw≥2且Δ_λ>0。

Conclusion: 谱滤波器预处理的QPE压缩方法在理论上被证明是精确的（非简并谱），具有严格的可证明的深度压缩上界，并与现有量子化学工作流兼容。该方法不修改QPE本身，可与读出级和态制备级滤波组合使用，为近端量子硬件上的量子化学模拟提供了实用性改进。
...
### [22] [Non-Abelian geometry and globally inequivalent symmetry reductions of the cylindrical Dirac doublet](https://arxiv.org/abs/2608.14041)
*Zhongze Guo,Bei Xu,Qiang Gu*

Main category: quant-ph

TL;DR: 本文研究圆柱Dirac二重态在动量空间上的全局几何结构，证明不同Dirac构造（横向螺旋度、质量修饰横向积分、螺旋度）虽然是局部等价的不同基选择，但在全局上不等价——它们具有截然不同的拓扑性质（Chern数等），表明单个Dirac二重态的光滑分解在全局层面是唯一的。


<details>
  <summary>Details</summary>
Motivation: 已有工作表明不同的精确圆柱Dirac构造在同一个二维正能量扇区内局部等价，可能仅被视为基的选择差异。作者希望探究这种局部等价性是否能在全局成立，即这些构造是否在拓扑意义上是真正等价的。

Method: 将圆柱Dirac二重态视为动量空间上的秩二向量丛；识别三种不同的对称选择性秩一约化（横向螺旋度、质量修饰横向积分、螺旋度）；利用Pauli代数组织内部二重态的归一化限制；分析正能量Dirac SU(2)联络在不同基下的Abel化性质及其三维曲率；以闭合形式推导方位角Wilson圈谱；通过Chern数等拓扑不变量比较三种约化的全局结构。

Result: 正能量Dirac SU(2)联络在横向螺旋度基下于固定方位角子午线上精确Abel化，但其完整三维曲率对非零质量保持真实的非Abel性。三种约化的全局结构截然不同：横向螺旋度分裂在动量轴上终止；质量修饰分裂光滑延拓且对m>0是Chern平凡；螺旋度则定义具有相反单位Chern数的线丛。

Conclusion: 单个Dirac二重态允许多种局部等价但全局不等价的对称性分解，这揭示了局部等价性不能保证全局等价性，Dirac结构的对称性约化必须考虑其全局拓扑性质。
...
### [23] [Information-Calibrated Quantum Diffusion: Aligning Forward Noise with Reverse Recoverability](https://arxiv.org/abs/2608.14083)
*Qipeng Qian,Yuntao Qian*

Main category: quant-ph

TL;DR: 本文提出将经典-量子信息递减量Δ_t作为标注量子系综的内在扩散坐标，用于替代传统量子扩散模型中基于通道强度的前向加噪参数化。通过均衡Δ_t获得前向路径的唯一极小极大离散化，并利用普适可恢复性赋予其逆向解释。同时证明局部校准对随机生成根本不足，提出结合定理级恢复约束与分布匹配的随机学习器，在四比特TFIM上取得优于官方QuDDPM的性能。


<details>
  <summary>Details</summary>
Motivation: 量子扩散模型通常以原始通道强度参数化前向加噪，但相等的参数增量并不一定抹除相等的信息量或产生可比的反问题。需要一种内在的扩散坐标来更准确地刻画信息擦除过程，并指导更好的离散化与逆向恢复设计。

Method: 引入经典-量子信息递减量Δ_t=I(X:Q_{t-1})-I(X:Q_t)作为内在扩散坐标；沿去极化通道均衡Δ_t得到唯一极小极大离散化；基于普适可恢复性将Δ_t解释为可实现的期望对数保真度预算；提出互补连续性与成对几何逆向下界；构建结合定理级恢复约束与分布匹配的随机学习器，建立有限样本校准与组合迹-Wasserstein控制。

Result: 在四比特TFIM上，受控容量扩展将端点迹-Wasserstein距离从0.622降至0.424（所有十个匹配种子），并以更少的可训练参数获得比官方QuDDPM（0.498）更低的迹-Wasserstein距离。

Conclusion: 信息递减量是比通道强度更本质的扩散坐标；单纯局部校准不足以保证随机生成的全局分布正确性；结合理论恢复界与分布匹配的学习器在量子生成任务上具有实际优越性。
...
### [24] [Entanglement certification via causal-order interferometry in a quantum switch](https://arxiv.org/abs/2608.14110)
*Haojie Wang,Shuheng Liu,Qiongyi He*

Main category: quant-ph

TL;DR: 该论文研究量子开关（indefinite causal order，ICO）是否能在噪声环境下扩大纠缠可认证的参数区域。结果表明，对随机Pauli噪声，后选择ICO输出比任何经典混合的确定因果序能产生更大的纠缠负性，甚至在所有经典混合纠缠度为零时仍保持非零。合适的局域Pauli幺正可大幅扩大这一ICO独有的区域，数值实验将该优势扩展到振幅阻尼噪声及两-qutrit的Weyl噪声，包括对Tiles束缚纠缠态的检测。


<details>
  <summary>Details</summary>
Motivation: 纠缠认证常针对已受噪声影响的态进行，噪声既减少剩余纠缠，也会导致判定准则在纠缠仍存时失效。量子开关作为不确定因果序的范式实现，已在众多量子信息任务中展现优势。作者想探究：因果序的相干控制能否扩大在噪声下仍可被证实纠缠的参数区域，从而增强纠缠认证的鲁棒性。

Method: 将量子开关的两个顺序分支视为因果序干涉仪的两臂，在信道之间插入局域幺正以调节干涉。针对随机Pauli噪声，比较后选择ICO输出与两个确定因果序的经典混合的纠缠负性；研究含噪信道下纠缠认证所允许的参数区域。采用合适的局域Pauli幺正扩大ICO独有的认证区域，并用输入依赖的路径差指示器建立算子非对易性与后选择负性增益之间的联系。数值实验扩展到局域振幅阻尼噪声和两-qutrit Weyl噪声，对3x3 PPT Tiles束缚纠缠态使用非可分解见证者检测后选择ICO输出，并用解析界排除确定序输出及其混合物被整个局域旋转见证者家族检测的可能。

Result: 随机Pauli噪声下，后选择ICO输出可展现出比经典混合更大的纠缠负性；存在ICO输出纠缠度非零而所有经典混合均零的参数区域。局域Pauli幺正可显著扩大该ICO独有区域。非对易性与后选择负性增益间有定性关联。数值上优势延伸至局域振幅阻尼噪声和两-qutrit Weyl噪声；对Tiles束缚纠缠态，非可分解见证者可检测后选择ICO输出，而解析界排除了确定序输出及其混合被整个局域旋转见证者家族检测。

Conclusion: 因果序干涉仪可作为增强纠缠认证的有效策略，其优势跨越多种噪声模型和维度（包括更高维态的束缚纠缠检测），为在强噪声下认证纠缠提供了新的途径。
...
### [25] [A groupoidal approach to quantum reference frames](https://arxiv.org/abs/2608.14133)
*Samuel Fedida,Alberto Ibort,Arnau Mas-Dorca*

Main category: quant-ph

TL;DR: 本文发展了一种基于群胚的关系量子场论（RQFT）框架，适用于弯曲时空。由于全局对称群在一般弯曲洛伦兹时空中通常缺失或过小，作者将量子参考系（QRF）形式推广到连续群胚，构建了群胚关系化映射和关系可观测量，并证明了该形式在适当极限下可退化为标准的群QRF形式。论文还建立了RQFT与Wightman QFT的新联系，论证协变POVM关于拟不变测度的μ-连续性，并指出Poincaré群胚是弯曲时空中Poincaré群的正确替代物，甚至可通过Atiyah群胚进一步推广到内部规范对称性，为关系量子杨-米尔斯场论奠定基础。


<details>
  <summary>Details</summary>
Motivation: 传统的基于群的量子参考系（QRF）形式依赖于全局对称群，但一般弯曲洛伦兹时空缺乏足够大的全局对称群，导致现有QRF框架无法直接适用于弯曲时空中的量子场论。因此需要发展基于群胚的更一般化的形式框架，以建立弯曲时空中的关系量子场论。

Method: 论文的主要方法包括：（1）将QRF概念推广到连续群胚，定义群胚关系化映射和关系可观测量；（2）构造基于群胚的"尖锐"QRF，类比基于L^2(G)的理想群QRF；（3）证明群胚QRF在局部紧群情形下退化为标准操作化QRF形式；（4）利用协变POVM关于拟不变σ-有限正Borel测度的μ-连续性，将关系量子场理解为对点定义核的抹平（smearing）；（5）引入Poincaré群胚替代Poincaré群，并通过Atiyah群胚扩展到内部规范对称性。

Result: 论文证明了群胚QRF框架在局域化极限下能恢复普通的非关系描述；行动群胚QRF仅对特定类别的POVM场才是torsor QRF，且torsor关系化映射仅适用于常数算符场；协变POVM关于拟不变测度是μ-连续的；Poincaré群胚被论证为弯曲时空中Poincaré群的正确替代物，框架可扩展到Atiyah群胚以涵盖内部规范对称性。

Conclusion: 本文建立了弯曲时空中关系量子场论的群胚基础，弥补了基于群的QRF形式在一般弯曲洛伦兹时空中的不足，并建立了RQFT与Wightman QFT之间更深层的联系，为关系规范量子场论（包括量子杨-米尔斯理论）提供了初步的群胚框架基础。
...
### [26] [Photonic Quantum Computing vs. Classical Solvers in Constrained Factor Portfolio Optimization](https://arxiv.org/abs/2608.14134)
*Nirvik Sahoo,Chyng Wen Tee,Paul Robert Griffin*

Main category: quant-ph

TL;DR: 本文对三种优化范式（光子量子退火器Dirac-3、商业混合整数规划求解器Gurobi、深度强化学习SAC）在机构化因子组合构建中的表现进行了严格的实证评估，基于Jensen-Kelly-Pedersen 13因子股票库和164个月测试窗口，通过48种超参数配置全面扫描收益、波动率与偏度之间的权衡。结果表明光子硬件在窄操作范围内可找到更优的风险收益拓扑，而经典混合整数规划在风险约束型任务中仍占优，且强化学习因子分配器存在结构性失效模式。


<details>
  <summary>Details</summary>
Motivation: 随着先进计算硬件（如光子量子退火器）和机器学习方法（如强化学习）在金融优化领域的兴起，业界缺乏对这些新兴优化范式的严格实证对比研究。作者旨在为量化投资组合经理提供基于证据的、针对不同投资约束条件的优化引擎选择指南，填补这一空白。

Method: 在Jensen-Kelly-Pedersen 13因子股票库上，针对164个月测试窗口，对三种优化管线（Dirac-3光子量子退火器、Gurobi混合整数规划求解器、SAC深度强化学习代理）实施全因子惩罚扫描，共48种超参数配置以控制收益、波动率和偏度之间的权衡，并对各方法的风险收益表现、尾部风险控制和跨种子稳定性进行系统比较。

Result: 光子硬件仅在狭窄操作范围内能定位更优的风险收益拓扑；在需要紧致尾部风险控制和跨种子稳定性的风险约束型任务中，经典混合整数规划（Gurobi）表现更优；强化学习因子分配器在无锚定的高阶矩塑造下表现出结构性失效模式。

Conclusion: 不同优化引擎各有适用场景，不应一概而论。作者将实证结果转化为可操作的、基于任务需求的指导意见，为量化组合经理在部署高级优化引擎时提供具体参考。
...
### [27] [Classical Limits of Spectral Filtering in Quantum Generative Models](https://arxiv.org/abs/2608.14169)
*Marco Roth*

Main category: quant-ph

TL;DR: 本文研究量子生成模型中的谱滤波正则化，证明幅度（衰减）滤波器存在二分类现象：在固定可负担阈值下，滤波输出要么是可用经典采样器高效处理的常数大小傅里叶对象，要么通带必然展宽直至无固定频率被衰减。两种情况都无法产生量子-经典分离，任何存留的分离都继承自输入态的谱相位。纯相位滤波器是唯一不受此约束的对角谱操作。


<details>
  <summary>Details</summary>
Motivation: 谱滤波已被提出作为量子生成模型的正则化途径：量子傅里叶变换暴露量子电路Born机的幅度谱，对角滤波器抑制与有限样本噪声相关的高频。其经典对应看似需要操控指数长度的幅度向量。作者想探究这种相干操作是否产生经典后处理无法匹配的效果。

Method: 将滤波器与对称概率核卷积在匹配采样成本下进行比较（考虑衰减的后选择开销），推导两者差距消失的充要条件。通过理论分析幅度（衰减）滤波器在固定可负担阈值下的行为，并在训练的电路Born机上进行数值实验验证分类。

Result: 幅度滤波器呈二分类：要么滤波输出是常数大小傅里叶对象且存在高效经典采样器，要么通带必须展宽直到无固定频率被衰减、滤波器不再平滑。两种情况下滤波器都不产生量子-经典分离。数值实验证实分类，并显示决定性的相位对Born规则训练损失不可见，由初始化设定。

Conclusion: 在对角滤波家族中，纯相位滤波器是唯一不受这些约束的谱操作；任何量子-经典分离都是继承自输入态的谱相位，而非由幅度滤波器本身创造。
...
### [28] [The Organization of Environmental Coupling Shapes What Quantum Reservoirs Remember](https://arxiv.org/abs/2608.14181)
*Markus Baumann,Itamar Fink,Johannes Wittmann,Claudia Linnhoff-Popien,Jonas Stein*

Main category: quant-ph

TL;DR: 本文通过数值模拟证明，开放量子储层计算中环境耦合的模式（而非仅强度）决定了输入历史中哪些部分仍可访问，共享驰豫与独立局域损耗产生不同的任务特征，环境耦合因此是一个设计层而非简单的阻尼参数。


<details>
  <summary>Details</summary>
Motivation: 量子储层计算依赖耗散使旧输入自然衰减，但先前研究仅固定环境过程并调节其强度，忽略了耦合模式（连接独立还是共享环境通道）这一更丰富的设计空间对信息遗忘模式的影响。

Method: 对有限自旋储层进行成对数值仿真，保持哈密顿量、输入、测量和读出固定不变；测试不同的环境耦合模式（独立局域损耗 vs 共享驰豫通道），并考察相对相位、系统尺寸、哈密顿量、输入协议和靶向控制等因素的影响。

Result: 不同耦合模式产生截然不同的任务特征，无普遍最优者；共享驰豫比独立局域损耗保留更近期的输入历史；共享衰减通道中自旋以不同相对相位贡献时，保留的记忆特性会发生改变；该排序在多种系统尺寸、哈密顿量、输入协议和控制条件下均保持一致。

Conclusion: 环境耦合不仅是调控信息衰减快慢的阻尼参数，更是一个设计层，它决定哪些输入历史内容保留下来用于计算，从而为量子储层计算提供新的设计自由度。
...
### [29] [Local geometry for Schmidt number witnesses](https://arxiv.org/abs/2608.14199)
*Young-Hoon Kiem,Seung-Hyeok Kye*

Main category: quant-ph

TL;DR: 本文研究二分量子态凸集人脸F_E附近的Schmidt数见证的存在性。对于特定维度的一般子空间E，存在一个仅依赖于E维数的阈值κ，使得F_{E^\perp}之外存在Schmidt数ℓ的见证当且仅当ℓ≤κ。本文进一步证明，当ℓ>κ时，在F_{E^\perp}中心的投影态附近仍存在Schmidt数ℓ的见证。


<details>
  <summary>Details</summary>
Motivation: Schmidt数见证是量子信息中检测纠缠态（特别是高维纠缠）的重要工具。理解在凸集人脸F_{E^\perp}附近哪些Schmidt数的见证存在，对于刻画纠缠结构的边界和量子态的判定具有重要意义。

Method: 利用文献[Phys. Rev. A 112 (2025), 032426]的结果，对一般子空间E建立阈值κ，并通过对F_{E^\perp}中心的投影态进行局部扰动分析，证明当ℓ>κ时仍可构造Schmidt数ℓ的见证。

Result: 证明了对一般子空间E，F_{E^\perp}之外存在Schmidt数ℓ见证的充要条件是ℓ≤κ（由既有结果）；新结果表明当ℓ>κ时，在F_{E^\perp}中心的投影态周围仍存在Schmidt数ℓ的见证。

Conclusion: 即使超出全局阈值κ，在特定量子态（投影态）附近仍能找到更高Schmidt数的见证，说明Schmidt数见证的存在性在几何上具有局部结构，丰富了关于纠缠见证与凸集几何关系的理解。
...
### [30] [Synthesizing In-Bulk Topological Corner States via Giant Atoms](https://arxiv.org/abs/2608.14206)
*Zhao-Min Gao,Xin Wang*

Main category: quant-ph

TL;DR: 本文提出一种在二维Su-Schrieffer-Heeger（SSH）晶格体中通过巨原子耦合人工合成任意位置拓扑角态的方案。利用L形多点耦合满足空位类缀饰态（VDS）条件，在体相内部形成人工边界，从而在任意目标位置实现零能角态。该方法具有高保真度和高空间局域性，且对无序具有鲁棒性。进一步扩展到多原子网络，实现了基于巨超原子的量子开关，以及两个巨原子间由VDS工程角态介导的相干相互作用。


<details>
  <summary>Details</summary>
Motivation: 传统高阶拓扑绝缘体中的角态严格局限于几何角落，限制了其在可扩展量子信息处理中的灵活性。为了突破这一几何约束，需要在体相内部灵活地合成拓扑角态，以实现可重构的拓扑量子网络。

Method: 1) 将巨原子以L形多点耦合方式与2D SSH晶格连接，通过使光子波函数在耦合位点处消失来满足VDS条件，从而在体相内部形成人工边界。2) 推导出在任意目标位置合成零能角态的条件。3) 推广至多巨原子网络，利用双共振条件实现巨超原子对0D角态和1D边缘态的多通道控制。4) 研究两个巨原子间的相干相互作用及其子格选择定则。

Result: 1) 成功在体相内部任意位置合成零能拓扑角态，具有高保真度和强空间局域性。2) 工程化角态对实际无序扰动保持鲁棒性。3) 通过巨超原子实现了多通道量子开关，可同时控制0D角态和1D边缘态。4) 两个巨原子间的耦合仅在交叉构型中激活，且随距离呈指数衰减。

Conclusion: 该工作建立了高度可重构的平台，将拓扑边界模式嵌入体相内部，为可扩展的拓扑量子网络提供了稳健的路径。VDS工程策略突破了传统角态的几何限制，为量子信息处理中的灵活拓扑态操控开辟了新途径。
...
### [31] [Foundation Neural Effective Hamiltonian for Strongly Correlated Quantum Materials](https://arxiv.org/abs/2608.14208)
*Lixing Zhang,Hongjie Jiang,Di Luo*

Main category: quant-ph

TL;DR: 本文提出FNEH方法，将哈密顿量族投影到由基础神经量子态（FNQS）张成的紧凑子空间上，通过变分组合FNQS来系统改进基态近似，在保持高精度的同时大幅降低多耦合参数扫描的计算成本，并在强关联摩尔材料中成功验证。


<details>
  <summary>Details</summary>
Motivation: 强关联量子材料的模拟通常涉及一族哈密顿量，传统方法需要对每个耦合参数分别进行昂贵的多体计算。基础神经量子态（FNQS）试图将计算成本摊薄到整个哈密顿量族上，但存在两个问题：在相变点附近精度下降，以及目标耦合数量增多时采样成本不可忽略。

Method: FNEH方法将哈密顿量族投影到由在选定耦合处采样的FNQS张成的紧凑子空间上。通过变分组合不同参数空间中的FNQS，系统改进基态近似；一旦所需的算子矩阵元被采样，即可在很小的有效哈密顿量维度下完成对耦合参数、可观测量和相边界的扫描，无需在每个目标耦合处重复神经网络采样。

Result: 在强关联摩尔材料中，FNEH准确分辨了竞争相位，实现了高分辨率的多维相图扫描，并显著降低了探索多个目标哈密顿量的计算成本。同时，该方法能够恢复基础模型误判的相边界。

Conclusion: FNEH为基础模型研究强关联量子材料开辟了新途径，在不依赖重复神经网络采样的前提下实现对哈密顿量族的快速、高精度探索，推动了基础模型在量子材料计算中的实用化。
...
### [32] [Exchange-only qubit stabilized by a single-spin qubit](https://arxiv.org/abs/2608.14214)
*Irina Heinz,Mira Sharma,Joris Kattemölle*

Main category: quant-ph

TL;DR: 本文提出将Loss-DiVincenzo量子比特与纯交换（exchange-only）量子比特相结合的混合方案，利用辅助电子自旋实现错误检测，并展示了单重态纯交换量子比特在三控制轴上的容错π旋转，可将电荷和核噪声转化为擦除错误，从而抑制错误传播并提升量子纠错性能。


<details>
  <summary>Details</summary>
Motivation: 单一量子比特编码方案各有局限，混合不同自旋量子比特编码可发挥互补优势。交换-only量子比特及其扩展提供额外自由度，能增强自旋寿命并借助辅助自旋实现错误检测，但需要找到与现有自旋穿梭架构兼容的实用实现路径。

Method: 将Loss-DiVincenzo量子比特与交换-only量子比特集成，形成兼容自旋穿梭架构的混合方案；利用单重态-only交换-only量子比特进行错误检测（将电荷/核噪声转化为擦除错误）；构造三个控制轴上的容错π旋转门。

Result: 证明了混合方案的实践可行性；实现了基于单重态-only交换-only量子比特的错误检测机制；成功构建了三个控制轴上的容错π旋转门；通过将噪声转化为擦除错误，有效抑制了错误传播，有望提升量子纠错方案的性能。

Conclusion: 该工作为单重态-only交换-only量子比特作为逻辑编码建立了新视角，为半导体量子计算领域打开了容错门构造和自旋定制量子纠错协议的大门。
...
TL;DR: 本文介绍SymQuPS，一个基于SymPy的Python代数学系统，用于Cahill-Glauber形式下的量子力学相空间表示（包括P、Wigner和Q表示），并支持任意Lindblad主方程的相空间表示。


<details>
  <summary>Details</summary>
Motivation: 量子力学相空间表示（如Wigner、P、Q表示）在量子光学和量子信息领域有广泛应用，但现有工具有限，缺乏一个灵活的符号代数平台来系统地处理Cahill-Glauber形式下的s-排序算符、星积以及Lindblad主方程的相空间变换。

Method: 基于SymPy构建的Python包，核心功能包括：s-排序算符（Cahill-Glauber形式）、星积运算以及相空间表示转换；支持将任意Lindblad主方程转换为其对应的相空间运动方程；同时作为正则量子化的代数工具。

Result: 提供了SymQuPS包的核心功能描述，并通过使用示例展示了包在典型场景下的应用，讨论了包在实际使用中的性能表现，验证了其在量子系统相空间方程求解方面的有效性。

Conclusion: SymQuPS是一个实用的符号计算工具，为量子力学的相空间表示和Lindblad主方程的相空间分析提供了便捷的代数平台，可进一步扩展用于量子光学与量子信息的理论计算。
...
TL;DR: 本文提出了Qu-Trefoil，一个基于FPGA平台（Trefoil）的大规模量子电路模拟系统。该系统利用多个FPGA连接八个存储子系统（每个配备32个SATA磁盘），集成了基于HLS的通用量子门（包括Clifford门、T门和酉矩阵计算）以及HDL设计的系统集成模块。实验表明，该系统能够在单个存储子系统上使用一个FPGA模拟超过43个量子比特的电路（需要超过128 TB内存），耗时约3.72至13.06小时，比现有模拟器更具可访问性、灵活性和成本效益。


<details>
  <summary>Details</summary>
Motivation: 基于态矢量的量子电路模拟需要2^(n+4)字节的内存，超过40个量子比特的模拟仅在超级计算机上可行。这限制了大多数研究者的访问能力，因此需要一种更经济、可访问的大规模量子电路模拟方案。

Method: Qu-Trefoil构建在名为Trefoil的多FPGA平台上，该平台连接八个存储子系统，每个配备32个SATA磁盘。系统集成了一套基于HLS的通用量子门库（包括H、Z、S、CNOT等Clifford门、T门和酉矩阵计算），以及用于系统级集成的HDL设计模块。实验评估涵盖量子门性能、块大小、磁盘可扩展性以及不同SATA代际的效率。

Result: 该系统成功模拟了超过43个量子比特的电路（需要超过128 TB内存），在单个配备一个FPGA的存储子系统上仅需约3.72至13.06小时。Qu-Trefoil在可访问性、灵活性和成本效益方面优于现有的大规模量子电路模拟器。

Conclusion: Qu-Trefoil展示了基于FPGA和多存储子系统架构进行大规模量子电路模拟的可行性，为缺乏超级计算机访问权限的研究者提供了一种经济可行的替代方案，是量子计算模拟领域的重要里程碑。
...
TL;DR: 本文研究了绝热动力学中Krylov复杂度与Berry相位之间的联系。在慢变自旋系统的瞬时本征态基下，若初始Krylov基是哈密顿量的瞬时本征态，绝热动力学会使Krylov复杂度消失；但如果初始Krylov基是叠加态而非本征态，Krylov复杂度将非零。其时间演化行为表现为周期性或准周期性，取决于控制参数。特别地，对于恒定参数的单比特系统，Krylov复杂度随时间做简谐振荡，频率与外部场强和几何Berry相位的组合相关。


<details>
  <summary>Details</summary>
Motivation: 探索量子混沌与量子信息领域中的Krylov复杂度这一新兴概念在绝热演化条件下的行为特征，并揭示其与几何Berry相位之间的深层联系，从而深化对量子动力学中复杂度概念的理解。

Method: 在瞬时本征态基下分析慢变自旋系统的绝热动力学，分别考察初始Krylov基为哈密顿量瞬时本征态和叠加态两种情形下Krylov复杂度的演化行为，并通过控制参数分析其周期性与准周期性特征。

Result: 当初始Krylov基为本征态时，绝热动力学迫使Krylov复杂度归零；当初始Krylov基为叠加态时，Krylov复杂度非零并呈现周期或准周期演化。对于恒定参数的单比特系统，Krylov复杂度表现为简谐振荡，振荡频率由外部场强与几何Berry相位的组合决定。

Conclusion: Krylov复杂度在绝热动力学中的行为强烈依赖于初始Krylov基的选择，且其动力学特征与Berry相位存在可观测的联系，为利用Krylov复杂度作为量子动力学诊断工具提供了新的理论依据。
...
TL;DR: 本文证明了混合量子态的平均情况学习困难（AHL）与低效可验证单向状态生成器（IV-OWSGs）的存在性等价，并由此将混合态AHL与EFI对联系起来，同时通过现有结果推论获得了相对于SWAP预言机的IV-OWSGs与OWSGs之间的分离。


<details>
  <summary>Details</summary>
Motivation: 密码学与学习理论的关系是理论计算机科学的核心主题。近年来，量子设置中的类似联系（涉及纯量子态的学习困难与密码学原语如OWSGs）已被探索，但混合量子态情形下的对应关系此前仍是一个悬而未决的问题。

Method: 本文通过构造性的归约证明，建立混合量子态的AHL与IV-OWSGs之间的等价关系。具体而言，证明了一方向混合态AHL可以构造出IV-OWSGs，另一方从IV-OWSGs也能推导出混合态AHL。在此基础上利用这一等价关系推导出与EFI对的联系，并借助已有结果给出相对于SWAP预言机的分离结论。

Result: 1）混合量子态AHL的存在性与IV-OWSGs的存在性完全等价；2）混合态AHL由此与EFI对相关联；3）通过现有结果推论，获得IV-OWSGs与OWSGs在SWAP预言机下的分离。

Conclusion: 该工作填补了量子密码学与量子学习理论之间关于混合态联系的空白，将混合态AHL、IV-OWSGs和EFI对这三个重要概念统一到同一个等价框架下，并揭示了IV-OWSGs与OWSGs之间的结构性区别。
...
TL;DR: 本文推导了腔介导集体自旋相互作用的完整非线性层级闭合解析表达式，证明非线性系数χ_k由切比雪夫多项式控制，满足通用标度χ_k∝η^k，为判断二次近似何时失效及高阶非线性何时起作用提供了系统框架。


<details>
  <summary>Details</summary>
Motivation: 腔介导的集体自旋相互作用通常被简化为二次单轴扭转（one-axis twisting）哈密顿量，但底层的原子-光相互作用天然会生成任意阶的非线性项。此前缺乏对完整非线性层级的一般性描述，导致难以判断高阶非线性何时变得重要以及二次近似的适用范围。

Method: 从底层的原子-光相互作用出发，推导出完整腔介导集体自旋相互作用的闭合解析表达式。通过数学分析揭示非线性系数χ_k（k为非线性阶数）由切比雪夫多项式控制，并由此得到χ_k∝η^k的通用标度律以及其对腔失谐的依赖关系。

Result: 获得了非线性系数的通用标度χ_k∝η^k（η为单原子协同性），并描述了其随腔失谐的变化。识别出实验上可实现的参数区域中，高阶非线性可以显著改变集体自旋动力学，加速量子关联和量子Fisher信息的生成，并证明有限阶展开能准确再现完整的腔介导演化。

Conclusion: 建立了腔量子电动力学中高阶非线性的通用分析框架，阐明了其在集体纠缠和量子增强传感中的作用，为确定二次近似的有效边界以及利用高阶非线性加速量子关联生成提供了理论指导。
...
TL;DR: 本文介绍了一种名为"multiunimon"的超导多模电路，该电路在单个约瑟夫森结嵌入共面波导的结构中编码多个量子比特，并支持原生多量子比特门。通过三比特设备设计和泄漏感知编码方案，模拟实现了十二种受控-受控-非门（CCNOT/Toffoli门），平均保真度达99.5%，并有望通过改进达到99.99%。


<details>
  <summary>Details</summary>
Motivation: 近期量子算法在 noisy 硬件上的高效实现需要原生多量子比特门，而传统的多比特门通常需要分解为大量单比特和两比特门，产生较高的开销和误差累积。作者希望设计一种能够原生执行多量子比特门的硬件，以提高算法在噪声硬件上的执行效率。

Method: 作者设计了multiunimon超导多模电路，通过在共面波导结构中嵌入单个约瑟夫森结来编码多个量子比特。该设计与unimon量子比特密切相关，继承了高非线性、对低频电荷噪声完全防护及对磁通噪声部分防护等特性。采用约瑟夫森能-电感能比大于1的设计，并使用泄漏感知编码方案定义计算态。通过数值模拟，使用简单的正弦平方脉冲在该三比特设备上实现所有十二种Toffoli门。

Result: 在三比特设备上模拟实现了所有十二种不同的CCNOT（Toffoli）门，平均保真度达99.5%，脉冲长度与单比特门相当。性能主要由介电损耗导致的不相干误差限制。模拟进一步表明，通过改进噪声防护、设计和脉冲整形，保真度有望接近99.99%。

Conclusion: multiunimon展示了作为高度连接的多量子比特单元用于构建更大规模超导量子处理器的潜力，为在近阶段噪声硬件上更高效地执行量子算法提供了一条可行的硬件路径。
...
TL;DR: 本文弥合了量子功能编程中两大范式——QSP/QSVT族方法与高阶量子变换（如UHET）之间的理论鸿沟，证明UHET可被解释为Generalised QSP（GQSP）的随机线性化。基于此，作者提出了Universal Hamiltonian Singular Value Transformation (UHSVT)，能在黑盒哈密顿量动力学下，高效地将任意矩阵的奇异值通过任意在原点处消失的充分可微复值函数进行变换，且不依赖之前QSVT方法所需的奇异值下界或X旋转门等附加假设。


<details>
  <summary>Details</summary>
Motivation: 尽管UHET与QSP/QSVT方法在结构上具有明显的相似性，但两者之间的深层理论联系此前一直未被阐明。理解这一关系对于统一量子功能编程的不同范式、推广QSVT的适用范围至更一般设定具有重要的理论意义。

Method: 作者首先建立了UHET与GQSP之间的严格对应关系，证明UHET可视为GQSP的一种随机化线性化形式。随后基于这一洞察，构造了UHSVT算法，该算法将线性化思想应用于哈密顿量编码的QSVT框架中，使得对嵌入哈密顿量中的任意矩阵A的奇异值变换仅需函数f在原点处取零这一条件即可实现。

Result: 提出的UHSVT算法能够对黑盒哈密顿量动力学中的任意矩阵A的奇异值进行任意充分可微复值函数变换，且仅要求f(0)=0，放宽了先前QSVT方法中关于奇异值下界或需要在qubitised子空间上执行X旋转门的限制性假设。

Conclusion: 该工作不仅从理论上统一了UHET与QSP/QSVT两大量子功能编程范式，还通过UHSVT提供了更普适、更具实用性的量子奇异值变换工具，为后续量子算法设计开辟了新的途径。
...
TL;DR: 该论文研究了PT对称系统中非线性与噪声对长期动力学的影响，证明哈密顿Duffing非线性使PT未破缺相仅在有限且非吸引的相空间区域存在，导致涨落驱动的逃逸使线性PT未破缺相成为纯粹瞬态现象；通过引入增益振荡器上的双光子损耗恢复全局随机稳定性，建立了双稳态区域，并指出非厄米实验中长期随机稳定性的真正来源是固有的恢复耗散而非光谱PT对称性本身。


<details>
  <summary>Details</summary>
Motivation: 现实的PT对称物理实现不可避免地引入非线性和噪声，这要求重新严格评估其全局长期动力学行为，因为仅基于线性光谱分析的结论可能不足以描述真实系统的稳定性。

Method: 论文从理论上分析了包含哈密顿Duffing非线性的PT对称耦合谐振子系统，并考虑了不可避免的噪声涨落。通过分析相空间中PT未破缺态的有界性和吸引性，证明了该态的非吸引特性；随后引入双光子损耗作为非线性阻尼机制，分析其如何恢复全局随机稳定性并产生双稳态行为。

Result: 研究表明Duffing非线性限制了PT未破缺相到有限的非吸引区域，涨落导致首次穿越逃逸进入失控轨迹，使PT未破缺相成为瞬态现象。引入双光子损耗后恢复全局随机稳定性，产生低振幅轨道与高振幅极限环共存的双稳态区。

Conclusion: 非厄米实验中所观察到的长期随机稳定性本质上由系统固有的恢复性耗散机制所主导，而非源自光谱PT对称性本身，这为理解非厄米体系稳定性的起源提供了修正性视角。
...
TL;DR: 本文评估了量子跳跃形式在非可加性（non-additive）开放量子系统动力学中的有效性与局限性。研究发现，在完美跳跃检测场景下，对非可加性量子主方程进行可加性分解（additive unravelling）不能生成完全正定动力学；但在非完美跳跃检测场景下，可加性分解可以复现Landauer-Büttiker形式给出的电流及涨落。


<details>
  <summary>Details</summary>
Motivation: 大量量子主方程无法用Lindblad形式描述（即非可加性、非马尔可夫或更一般的动力学），而量子跳跃形式通常基于Lindblad方程构建。因此需要评估量子跳跃形式在非Lindblad动力学中的适用性，以拓展其在电流统计与涨落表征方面的应用范围。

Method: 作者系统评估了量子跳跃形式在非可加性开放量子系统动力学中的有效性。具体考察了两种场景：一是完美跳跃检测情形，检验可加性分解是否保持完全正定的动力学演化；二是非完美跳跃检测情形，探讨可加性分解是否能够复现由Landauer-Büttiker形式给出的电流及电流涨落。

Result: 在完美跳跃检测场景下，非可加性量子主方程的可加性分解无法产生完全正定动力学。然而在非完美跳跃检测场景下，存在一种可加性分解方式，能够准确复现Landauer-Büttiker形式所描述的电流及其涨落特征。

Conclusion: 量子跳跃形式对非Lindblad（非可加性）量子主方程的适用性取决于跳跃检测的完备性：完美检测下不可行，非完美检测下则可以有效复现输运电流及其波动统计。这为非Lindblad动力学下的量子输运表征提供了可行的理论框架。
...
TL;DR: 本文开发了一种机制解析的理论，用于描述量子信息引擎中由测量和反馈共同制备相干性并选择有限时间动力学的过程，推导了条件功统计、量子功准概率中的半量子对干涉特征，以及反馈记忆对功统计和信息热力学成本的影响。


<details>
  <summary>Details</summary>
Motivation: 在量子信息引擎中，测量与反馈不仅驱动体系演化，还引入了相干性和记忆效应。理解这些机制如何影响功统计和涨落修正，对于连接相干功统计、反馈记忆与信息热力学至关重要。作者希望超越传统的只关注种群转移的功理论，建立包含相位敏感的相干跃迁干涉项的完整理论框架。

Method: 作者发展了一种机制解析（mechanism-resolved）的理论框架：将条件功分解为种群转移项和相位敏感的相干跃迁干涉项；利用对称全计数统计（full counting statistics），将干涉映射为功准概率中大小相等、方向相反的半量子对，该对进入奇数阶矩而对偶数阶矩无贡献；进一步引入结果分辨的倾斜核（outcome-resolved tilted kernel），通过相关的测量记录传播这些统计量，从而得到有限周期和固定时间的涨落修正。同时对可逆记录重置成本进行分析，证明其从单符号熵降低为熵率。

Result: 理论结果表明：条件功可分为种群转移与相干跃迁干涉两部分；对称全计数统计将干涉项表现为功准概率中的等大反向半量子对，只贡献于奇数矩；结果分辨倾斜核可传播统计并通过测量记录得到有限循环与固定时间的涨落修正；反馈记忆使可逆记录重置成本由单符号熵降为熵率。

Conclusion: 该工作建立了相干功统计、反馈记忆与信息热力学之间的统一联系，为量子信息引擎中的涨落与信息成本提供了机制层面的定量描述。
...
### [43] [Dual-species alkali and alkaline-earth-like optical tweezer arrays via interferometrically aligned high-NA objectives](https://arxiv.org/abs/2608.13646)
*K. Weber,M. McMaster,M. Anderson,J. Tu,S. E. Eustice,N. A. Schine,I. B. Spielman,J. V. Porto,S. L. Rolston,S. Subhankar*

Main category: quant-ph

TL;DR: 本文介绍了一种双物种光镊阵列装置，结合了87Rb和174Yb原子，利用混合Twyman-Green-Fizeau干涉仪实现两个0.6数值孔径物镜及其高数值孔径光束的精密共准直。该装置实现了两物种在共准直光镊阵列中的同时俘获和单点分辨成像（87Rb在840nm、174Yb在532nm），为混合量子寄存器操作提供了平台基础。


<details>
  <summary>Details</summary>
Motivation: 中性原子阵列量子计算平台通常局限于单一原子物种，严重限制了量子寄存器的功能扩展。实现多物种（如碱金属Rb和碱土Yb）的混合俘获需要解决不同波长光束之间的精密共准直这一关键技术难题，这在高数值孔径系统中尤为困难。

Method: 开发了一种双物种光镊阵列装置，采用永久混合Twyman-Green-Fizeau干涉仪实现两个相对放置的0.6 NA物镜及其高数值孔径光束的精密共准直。利用840nm光镊俘获87Rb，532nm光镊俘获174Yb，并在共准直阵列中同时进行单点分辨成像。

Result: 成功实现了87Rb和174Yb两种原子在共准直光镊阵列中的同时俘获与单点分辨成像，验证了跨广泛分离波长的高数值孔径系统的共准直能力，该技术可推广至其他镊子平台。

Conclusion: 该平台为混合量子寄存器运行（包括电路中途测量、物种内外的非对称相互作用）提供了基础，显著扩展了中性原子阵列在量子计算和量子模拟中的能力。
...
### [44] [Strong-field Herman-Kluk propagator method for high-harmonic generation in molecules](https://arxiv.org/abs/2608.13686)
*Phi-Hung Tran,Hao Quan Truong,R. Esteban Goetz,Anh-Thu Le*

Main category: quant-ph

TL;DR: 本文将半经典强场Herman-Kluk(SFHK)传播子方法扩展到双原子分子中的高次谐波产生(HHG)计算。以H2和N2为例，该方法结合Herman-Kluk传播子与强场近似，在HHG产率和相位上均能与含时薛定谔方程的精确数值解高度吻合。该方法的一个重要优势是可以高效地将取向平均的蒙特卡洛采样与隧道电离后电子波包的初始动量分布积分相结合，且轨迹总数几乎不随取向平均而增加，并支持高效并行化计算。


<details>
  <summary>Details</summary>
Motivation: 分子高次谐波产生(HHG)是研究分子结构和超快动力学的重要工具，但精确求解含时薛定谔方程计算量大、成本高。亟需一种既准确又高效的方法来计算双原子分子在少周期强激光场驱动下的HHG，特别是需要处理分子取向平均这一实验相关且计算上昂贵的步骤。

Method: 将半经典强场Herman-Kluk(SFHK)传播子方法从原子体系扩展到双原子分子HHG计算。该方法结合Herman-Kluk传播子（用于量子传播的半经典近似）和强场近似（描述隧穿电离），其中连续态中的电子波包中心（相干态表示）由独立经典轨迹驱动。取向平均的对准分布积分采用蒙特卡洛采样，并与隧穿出口处电子波包初始动量分布的积分高效合并。

Result: 以H2和N2为例，SFHK方法得到的HHG产率和相位与含时薛定谔方程的精确数值解几乎完全一致。取向平均所需的轨迹总数相比固定取向情况并未显著增加，表明该方法在计算效率上具有显著优势。由于各轨迹相互独立，计算可高效并行化。

Conclusion: SFHK传播子方法为计算双原子分子的HHG提供了一种既准确又高效的半经典途径，其计算结果接近精确量子解。该方法天然支持高效并行，且巧妙地解决了取向平均带来的维度灾难问题，有望成为研究分子HHG及分子超快动力学的有力工具。
...
### [45] [Choi--Jamiołkowski-type isomorphisms for von Neumann algebras](https://arxiv.org/abs/2608.13750)
*Marcin Marciniak,Michał Cholewiak*

Main category: quant-ph

TL;DR: 本文构建了任意von Neumann代数之间正规完全有界映射空间与预对偶之间的典范序同构，推广了Choi–Jamiołkowski同构（CJ同构）到无限维（III型）情形。关键发现是：在自然性要求下，该同构需要N与其反向代数的同构，而这对Connes构造的III型因子不成立，因此此类代数上不存在CJ对应。


<details>
  <summary>Details</summary>
Motivation: Choi–Jamiołkowski同构是有限维量子信息理论的基石，它将完全正映射与双体态对应起来。但在无限维系统中（如III型von Neumann代数），没有迹和密度矩阵，传统同构失效。作者希望为任意von Neumann代数的推广寻找合适的框架，并探究其在何种条件下才能真正成立。

Method: 作者在纯空间张量积框架下，用预对偶（predual）代替迹，将正常完全有界映射空间与M⊗N^op的预对偶建立典范序同构。通过分析同一性条件和自然性要求，证明将N^op替换为N本身需要N与自身的反同构（anti-isomorphism），并证明了该条件在自然性的前提下是必要条件而不仅仅充分。同时考察了算子范数下的正常映射空间为何不适于该目的。

Result: (1) 对任意von Neumann代数M和N，存在从\operatorname{CB}_\sigma(M, N_*)到(M \bar{\otimes} N^{op})_*的典范序同构；(2) 若要求对M自然，则CJ型同构的存在性等价于N存在与其自身的反同构；(3) 对每个与自身反同构的M，平凡的反例可构造，说明某种自然性要求不可避免；(4) Connes构造的III型因子不满足此条件，故不存在CJ对应；(5) 正常映射空间的算子范数结构过大会破坏该同构，且完全正性在此设定下不蕴含完全有界性。

Conclusion: Choi–Jamiołkowski同构可以为一般的von Neumann代数在预对偶框架下重新表述，其有效性本质依赖于代数的自反同构性质。对Connes型III因子这类重要量子场论中的代数结构，该同构从根本上失效，暗示量子信息概念在无穷维Lorentz协变系统中的适用性需要更深层的方法论重新审视。
...
### [46] [Strong photoresponse of edge two-dimensional electrons in a magnetic field](https://arxiv.org/abs/2608.13811)
*Sergey A. Mikhailov,Wladislaw Michailow*

Main category: quant-ph

TL;DR: 本文发展了二维电子气在垂直磁场作用下边缘态光电流的详细量子理论，提出了观测该现象的实验几何构型，并证明在III-V半导体结构中可获得比石墨烯高数个量级的光响应。


<details>
  <summary>Details</summary>
Motivation: 二维电子气在外加垂直磁场下，电子沿边缘以接近费米速度的高速度传播。在微波和太赫兹辐射下，这些电子通过光子吸收可实现灵敏辐射探测。作者希望发展该系统中光电流的量子理论，并设计实验方案以观测和利用这一现象。

Method: 建立了入射电磁波在二维电子气边缘态系统中产生光电流的详细量子理论；设计了合适的辐射约束结构以增强光子吸收；提出了用于观测预测现象的特定实验几何构型。

Result: 通过合理布置辐射约束结构可获得强光电流产生效率；在III-V半导体结构中测得的光响应比石墨烯高数个数量级。

Conclusion: 二维电子气边缘态在太赫兹辐射下是一种高效的光探测机制，III-V半导体结构相较石墨烯在光响应灵敏度上具有显著优势，为太赫兹辐射探测提供了新途径。
...
### [47] [The Capacity Region of the Multiple Access Channel with Non-Signaling Assistance](https://arxiv.org/abs/2608.13860)
*Yuhang Yao,Syed A. Jafar*

Main category: quant-ph

TL;DR: 本文完整刻画了K个发送者的离散无记忆多址接入信道（MAC）在非信号（NS）辅助下的容量区域，证明了该容量区域与经典MAC容量区域形式相同，区别仅在于允许发送者之间的输入分布任意依赖。同时证明NS辅助的容量增益乘数可以任意接近K，确立了K为所有K发送者MAC设置中NS辅助乘法增益的极值。


<details>
  <summary>Details</summary>
Motivation: 此前Fawzi和Fermé仅对K=2发送者的MAC建立了NS辅助容量区域的外界，本文旨在将这一结果推广到任意K个发送者的通用水准，并探索NS辅助在MAC中能带来的最大乘法增益。

Method: 通过将NS辅助的容量区域刻画与经典MAC容量区域的推广形式进行对比，建立K发送者的一般性外边界；通过构造具体示例，展示NS辅助下乘法增益可任意接近K，并结合先前工作的上界确定K为极值。

Result: NS辅助下K发送者MAC的容量区域被完整刻画，形式与经典容量区域一致但允许输入分布任意依赖；给出了乘法增益任意接近K的示例，结合既有上界证明了K是NS辅助乘法增益的极值。

Conclusion: NS辅助可以显著提升K发送者MAC的容量，其最大乘法增益恰好为K，这一结果完善了NS辅助在多址接入场景中的理论边界。
...
### [48] [Hybrid Quantum-inspired Kolmogorov-Arnold Networks for Privacy-Aware Federated Biosignal Learning](https://arxiv.org/abs/2608.13914)
...
TL;DR: 本文提出了一种混合量子启发的Kolmogorov-Arnold网络（HQKAN），用于联邦学习框架下的ECG心律失常分类。在MIT-BIH和INCART数据集上，与MLP基线相比，HQKAN在多数聚合指标和少数类别指标上表现更优，同时分别减少了37.35%和44.81%的可训练参数，以及24.89%和36.41%的通信成本，证明了其作为隐私保护联邦学习紧凑型替代方案的潜力。


<details>
  <summary>Details</summary>
Motivation: 心电图为敏感生物医学数据，医院和可穿戴设备出于隐私限制无法直接共享原始信号进行集中式模型训练。联邦学习虽然能在不共享原始数据的情况下实现协作训练，但面临客户端样本有限、心律失常标签不平衡以及客户端间数据非独立同分布（non-IID）等挑战，亟需同时具备通信高效性和跨客户端分布偏移鲁棒性的分类器。

Method: 研究在联邦平均（FedAvg）框架下，评估混合量子启发的Kolmogorov-Arnold网络（HQKAN）与多层感知机（MLP）在心律失常分类中的表现。实验使用MIT-BIH数据集进行五分类心律失常分类，INCART数据集进行三分类，配置多个客户端场景并比较两者的聚合指标、少数类别指标、参数量及通信成本。

Result: 在多种客户端配置下，HQKAN在MIT-BIH上以少37.35%的可训练参数和少24.89%的通信成本，在多数聚合及少数类别指标上优于MLP；在INCART上实现44.81%的参数减少和36.41%的通信成本减少，同样取得更优指标。

Conclusion: HQKAN是一种紧凑、通信高效且稳健的MLP替代方案，适用于隐私感知的联邦学习框架下的生物信号数据分析，在资源受限和隐私敏感的应用场景中具有实际价值。
...
### [49] [Cold-atom comagnetometry via optical control of spin states](https://arxiv.org/abs/2608.13936)
*J. -L. Zhang,W. -T. Luo,Y. A. Yang,Y. -Q. Wang,T. Xia,Z. -T. Lu*

Main category: quant-ph

TL;DR: 本文展示了一种基于光学晶格中共同囚禁的171Yb（自旋1/2）和173Yb（自旋5/2）核自旋的冷原子共磁力计。通过线性偏振抑制矢量光移、利用薛定谔猫态抑制173Yb的张量光移，实现了60秒的自旋相干时间和超过3×10^4的磁场噪声抑制因子，并将核磁矩比测定精度提升至4 ppm。


<details>
  <summary>Details</summary>
Motivation: 与传统气室共磁力计相比，冷原子系统能提供更短的距离尺度和光学量子控制能力。然而，要实现冷原子的长自旋相干时间，必须采用抗磁性原子并克服光移引起的退相干，这是构建冷原子共磁力计的核心挑战。

Method: 采用光学晶格中共同囚禁的171Yb和173Yb原子，通过确保晶格光束的线性偏振来抑制矢量光移；利用薛定谔猫态抑制173Yb的张量光移。在此基础上对两种同位素同时进行拉姆齐干涉测量。

Result: 实现了60秒的自旋相干时间，磁场噪声抑制因子超过3×10^4，核磁矩比测定精度达到4 ppm。

Conclusion: 该工作建立了一种新型冷原子自旋传感平台，为超越标准模型的量子增强物理搜索开辟了新途径。
...
### [50] [Quantum Multi-Armed Bandits and Linear Bandits: Lower Bounds and Algorithms](https://arxiv.org/abs/2608.14319)
*Maoli Liu,Zhuohua Li,John C. S. Lui*

Main category: quant-ph

TL;DR: 本文针对量子多臂老虎机（QMAB）和量子线性老虎机（QLB）首次证明了极小极大下界：QMAB为Ω(K log(T/K))，有限动作QLB为Ω(d log(T/d))，否定了T无关遗憾的可能性；同时给出一个基于G-最优设计的消除算法，使有限动作QLB的遗憾从d²降至d（匹配下界至多对数因子）。


<details>
  <summary>Details</summary>
Motivation: Wan等人[2023]的量子老虎机模型中，已有算法的遗憾为QMAB的O(K log T)和QLB的O(d² polylog T)。核心开放问题是：K log T的规模是否不可避免？d²的依赖能否改进？以及是否存在T无关的遗憾？本文旨在通过下界和更优算法来解答这些问题。

Method: 下界证明核心：(1)利用多项式方法和Remez型三角多项式不等式，提出高置信度单臂量子检测下界，用于区分固定奖励均值与一个区间内的替代假设；(2)通过bandit-to-testing归约将其提升为QMAB下界；(3)通过线性嵌入获得有限动作QLB下界。算法设计：给出基于设计的消除算法，将低偏置低方差量子均值估计器与小型支撑G-最优设计耦合，按设计权重分配查询；通过方差而非最坏情形绝对误差聚合重构误差，消除剩余的√d因子。

Result: 证明了QMAB的Ω(K log(T/K))和有限动作QLB的Ω(d log(T/d))极小极大下界；提出了基于G-最优设计的消除算法，当动作集大小为poly(d)时遗憾为O(d polylog T)，将维度依赖从d²改善为d；采用量子蒙特卡洛估计时维度依赖从d²降至d^{3/2}，低方差估计器进一步消除√d因子。

Conclusion: 本文解决了Wan等人[2023]提出的开放问题，证明T无关遗憾不可行，给出了匹配的下界，并提供了维度依赖最优（至多对数因子）的算法，在量子老虎机学习的遗憾界上取得了实质性进展。
...
### [51] [Orbital Hall Effect in Weyl Semimetals from quantum geometric band interference](https://arxiv.org/abs/2608.14353)
*Chiara Pacella,Maximilian Ünzelmann,Ahmed Osman,Tim Figgemeier,Friedrich Reinert,Angel Rubio,Domenico Di Sante*

Main category: quant-ph

TL;DR: 该论文通过从头算密度泛函理论和基于绝热微扰理论的最小Weyl模型，在Weyl半金属TaAs家族（TaAs, TaP, NbAs, NbP）中建立了轨道角动量输运、能带几何与拓扑电子结构之间的直接联系。


<details>
  <summary>Details</summary>
Motivation: 轨道角动量（OAM）输运在固体中已显现为可超越自旋基对应物的基本现象（如轨道霍尔效应），但其微观机制仍仅部分被理解，特别是能带几何在轨道输运中的作用尚未解决。

Method: 采用从头算密度泛函理论计算，并结合基于绝热微扰理论构建的最小Weyl模型，从数值和解析两个层面进行分析。

Result: 在TaAs家族四个Weyl半金属中，数值与解析结果一致地建立了OAM输运、能带几何与拓扑电子结构之间的直接联系。

Conclusion: 该工作揭示了轨道角动量输运与能带几何及拓扑结构的内在关联，为理解OAM动力学微观机制提供了明确的物理图景。
...
### [52] [Angular displacement readout of a mechanical oscillator with a guided mode resonance](https://arxiv.org/abs/2608.14447)
*Diego Torres-Barajas,Oscar Angulo,Aman R. Agrawal,Mohamed ElKabbash,Dalziel J. Wilson*

Main category: quant-ph

TL;DR: 本文提出并演示了一种利用集成导模共振（GMR）结构实现相干增强的角度位移读出方案，并将其应用于纳米机械振子的精密测量中。通过在100 nm厚的Si₃N₄薄膜上制备亚波长光栅，并采用直接透射测量记录振动信号，该方法实现了10⁻⁹ rad/√Hz的散粒噪声极限位移精度（仅需纳瓦级光功率），足以以47 dB信噪比分辨Q≈10⁶的扭转模式热运动。控制实验（偏振与波长失谐）证实信号源于GMR介导的转导机制，确立了GMR作为量子光机械传感器中片上角度位移读出的可行途径。


<details>
  <summary>Details</summary>
Motivation: 角度位移测量是机械振子表征中的常见需求，但角光机械耦合的多模特性使得相干信号增强难以实现，亟需一种能够实现片上相干增强的角度读出方案。

Method: 采用集成导模共振（GMR）结构：在100 nm厚的Si₃N₄薄膜上制备亚波长光栅，通过直接透射测量记录机械振子的振动；利用GMR约2.5 mrad的窄线宽实现相干增强，并辅以偏振和波长失谐控制实验验证信号的GMR来源。

Result: 实现了10⁻⁹ rad/√Hz的散粒噪声极限位移精度，仅需纳瓦级光功率；成功以47 dB的信噪比分辨Q≈10⁶扭转模式的热运动。控制实验确认信号源于GMR介导的转导。

Conclusion: 导模共振提供了一种片上、可集成的角度位移读出方法，可作为量子光机械传感器中的可行角度读出方案。
...
### [53] [Universal aspects of bulk density of states in non-Hermitian lattices](https://arxiv.org/abs/2608.14508)
*Mykhailo Pavliuk,Askar Iliasov,Emil J. Bergholtz,Tomáš Bzdušek*

Main category: quant-ph

TL;DR: 本文证明非厄米格点哈密顿量的体态密度（DOS）看似存在边界依赖的歧义，但实际上大部分是假象。所有有限系统热力学极限下的DOS定义共享相同的多极矩，并在有限时间内及远离边界处产生相同的体动力学。作者将Brown测度识别为体DOS的规范代表，并证明点隙拓扑对格林函数施加了额外的普适约束，为任意维度下本征值累积位置提供了系统判据。


<details>
  <summary>Details</summary>
Motivation: 非厄米晶格哈密顿量对边界条件高度敏感，周期和开放边界条件产生不同的复能量平面态密度，导致人们认为扩展非厄米系统缺乏唯一的体态密度。作者挑战这一观点，旨在揭示这些DOS定义之间的深层普适结构。

Method: 对任意有限程紧束缚哈密顿量，作者在数学上证明所有作为有限系统热力学极限的DOS定义共享相同的多极矩；通过热力学格林函数的分析，证明其在足够大的复频率下与边界条件无关；将Brown测度（通过厄米化与预解分析获得）确立为体DOS的规范代表；并利用点隙拓扑证明边界依赖的格林函数在拓扑平凡的点隙中被迫重合。

Result: 确立了非厄米体态密度的普适性结构：不同DOS定义在体性质上等价；Brown测度可作为规范代表；点隙拓扑提供了判别不同边界截断本征值在复平面中累积位置及DOS歧义物理意义的系统判据，适用于任意维度。

Conclusion: 非厄米系统体DOS的看似歧义大部分是假象，Brown测度提供了一个无需边界条件、直接由无穷大体哈密顿量定义的规范选择；DOS歧义仅在特定拓扑区域保留物理意义，且可被精确刻画。
...
### [54] [Learning Spin Hamiltonians from Terahertz Two-Dimensional Coherent Spectroscopy](https://arxiv.org/abs/2608.14460)
*Martin Mootz,Chuankun Huang,Liang Luo,Jigang Wang,Yong-Xin Yao*

Main category: physics.comp-ph

TL;DR: 本文提出了一种监督式机器学习框架，从非线性太赫兹二维相干光谱（THz-2DCS）中推断有效哈密顿量参数，并在稀土正铁氧体（Sm₀.₄Er₀.₆FeO₃）上进行了实验验证。


<details>
  <summary>Details</summary>
Motivation: 有效哈密顿量连接了量子材料中的微观相互作用与宏观集体行为，但直接从实验数据确定其参数是一个具有挑战性的逆问题。

Method: 该方法包括三个关键步骤：(1) 构建校准的正向模型，从候选哈密顿量生成光谱（基于双子晶格Landau-Lifshitz-Gilbert自旋模型，包含交换、Dzyaloshinskii-Moriya相互作用、各向异性和阻尼）；(2) 通过通用的预处理管道将模拟光谱和实验光谱映射到同一表示空间；(3) 训练神经网络学习从光谱指纹到微观参数的非线性逆映射。

Result: 合成基准测试表明非线性光谱能编码超越线性响应的参数，推理精度与物理光谱灵敏度一致，多脉冲间延迟方案可提升抗噪鲁棒性。应用于Sm₀.₄Er₀.₆FeO₃的实验THz-2DCS数据时，推断参数产生合理的正向模拟，残余差异揭示了简化模型的局限性。

Conclusion: 该研究确立了THz-2DCS作为数据丰富的有效哈密顿量推断和模型改进平台，实现了实验驱动的微观相互作用识别，为理解、预测乃至控制量子材料的涌现性质奠定了基础。
...
### [55] [Linear-response time-dependent density-functional theory with local range separation: Core and valence resonances of the neon atom](https://arxiv.org/abs/2608.14052)
*Jari van Gog,Felipe Zapata,Julien Toulouse*

Main category: physics.comp-ph

TL;DR: 本文研究了范围分离杂化（RSH）和局域范围分离杂化（LRSH）方法在线性响应含时密度泛函理论（TDDFT）Sternheimer计算中，对Ne原子光电离光谱的描述。通过改进的两参数范围分离函数，LRSH方法能较好地描述价层和核心共振的能量，但核心共振的寿命（Auger衰变涉及双激发）被高估了数个数量级。


<details>
  <summary>Details</summary>
Motivation: Ne原子同时具有能量尺度差异很大的价层和核心共振，是对交换关联近似处理的严格检验。此前工作使用简单的单参数局域范围分离函数，本文旨在评估更灵活的两参数范围分离函数（改进高密度极限），以提高TDDFT对光电离光谱描述的准确性。

Method: 采用线性响应TDDFT Sternheimer方法，比较RSH和LRSH两种近似方案；使用两参数局域范围分离函数改进高密度极限；计算Ne原子光电离光谱和共振参数（包括价层2s→np共振及核心1s→np共振的能量和寿命）。

Result: LRSH配合两参数范围分离函数能总体较好地描述光电离光谱，包括价层和核心共振的能量。价层共振寿命因其衰变不涉及双激发而能合理再现；但核心共振的寿命因Auger衰变通道涉及双激发（在绝热单行列式TDDFT中缺失）而被高估数个数量级。

Conclusion: 要在线性响应范围分离TDDFT中获得更精确的共振宽度，需要采用多行列式方案和/或添加频率依赖的响应核。
...
### [56] [Accurate and efficient calculation of atomic forces in solids with non-self-consistent hybrid functionals](https://arxiv.org/abs/2608.14147)
*Damian Contant,Maria Hellgren*

Main category: physics.comp-ph

TL;DR: 本文研究非自洽杂化泛函计算的可行性，重点考察原子受力的计算。通过DFPT方法在Quantum ESPRESSO中实现非自洽性引起的解析受力项，证明非自洽杂化受力计算仅需局域/半局域泛函的自洽DFPT计算加上一次Fock交换算符评估，可显著降低计算成本，尤其在需要密集布里渊区取样的固体中；结构参数几乎不受自洽性影响，振动频率误差通常在0.5%以内。


<details>
  <summary>Details</summary>
Motivation: 杂化泛函在广义Kohn-Sham框架中通常自洽使用，但非局域Fock交换算符的评估使计算成本高昂，尤其在平面波基组下。作者旨在探索非自洽杂化泛函计算的优劣，以降低计算复杂度。

Method: 采用密度泛函微扰理论（DFPT）计算由于非自洽性产生的解析受力项，实现于Quantum ESPRESSO软件包中。非自洽杂化受力计算流程为：先进行局域或半局域泛函的自洽DFPT计算，再对Fock交换算符做单次评估。

Result: 非自洽杂化受力计算在总体上降低了计算成本，特别是对于需要密集布里渊区取样的固体；结构参数基本不受自洽性的影响，振动频率通常与自洽结果吻合在0.5%以内。

Conclusion: 非自洽杂化泛函计算是自洽计算的有力替代方案，在保持结构参数和振动频率精度接近的同时显著降低计算成本。
...
### [57] [Electrostatic Phenomenology Benchmarks for Machine-Learned Interatomic Potentials in Electrochemistry: Beyond the Energy-Force Metric](https://arxiv.org/abs/2608.14153)
...
TL;DR: 本文提出EPhEct基准测试套件，用于评估机器学习原子间势（MLIPs）在电化学模拟中长程相互作用的物理准确性，通过聚焦图像电荷吸引、介电屏蔽、电荷转移等定性物理现象来补充传统聚合能量-力误差指标。


<details>
  <summary>Details</summary>
Motivation: 机器学习原子间势（MLIPs）在电化学模拟中需精确处理长程相互作用，但仅依靠聚合能量和力的误差无法捕捉模型的定性物理不一致性，如错误预测图像电荷吸引、介电屏蔽或电荷转移等。因此需要一种能够检测这些定性物理现象的评估方法。

Method: 作者构建了EPhEct（Electrostatic Phenomena for Electrochemistry）基准测试套件，包含多个聚焦的测试用例：金属电极处的图像电荷吸引、纵/横光学声子分裂（作为离子和电子屏蔽探针）、界面水偶极矩、以及离子放电过程中的费米能级钉扎。这些测试旨在作为补充聚合能量-力指标的定性诊断工具。

Result: 该基准测试套件建立了定性的诊断流程，能够检测MLIPs在电化学相关物理现象中的定性错误，与传统聚合能量-力指标形成互补。

Conclusion: EPhEct提供了一套聚焦的定性测试用例，用于系统评估MLIPs在电化学环境下长程静电相互作用的物理准确性，弥补了传统聚合误差指标无法捕捉定性物理不一致性的不足。
...
### [58] [Efficient simulation of second-order phase transitions in quantum anharmonic materials](https://arxiv.org/abs/2608.14292)
*Andrea Baldanza,Lorenzo Monacelli*

Main category: physics.comp-ph

TL;DR: 本文提出一种基于变分自由能原理的新方法，用于预测强非谐系统中的临界长程行为和动力学谱，将随机自洽谐波近似的计算复杂度从O(N^6)降至O(N^2)、内存需求从O(N^4)降至O(N)，并在CsSnI3钙钛矿材料上验证了其高效性。


<details>
  <summary>Details</summary>
Motivation: 二级结构相变（如铁电、Peierls和电荷密度波）中，序参量的发散涨落导致标准声子准粒子图像失效。分子动力学在临界点附近存在严重的临界慢化问题，而谐波近似在鞍点处完全失效，因此模拟这些强非谐区域极具挑战。

Method: 基于变分自由能原理构建新框架，在随机自洽谐波近似（SSCHA）基础之上，通过算法改进将计算复杂度从O(N^6)降至O(N^2)、内存需求从O(N^4)降至O(N)，使量子离子涨落主导的强非谐系统的仿真成为可能。

Result: 在1080原子的CsSnI3超胞上计算了完整自由能Hessian矩阵和临界温度，模拟了相变附近的Raman光谱，明确了准粒子图像失效的特征。该计算在传统方法下需数万年，新方法在消费级硬件上仅需数小时即可完成。

Conclusion: 该方法使大规模强非谐系统的量子效应模拟从不可行变为现实，为研究铁电体、热电材料等类型的相变和光谱性质提供了高效可行的计算工具。
...
### [59] [Coupling-Aware Vanka Smoothing for Multigrid Preconditioning of the Implicit Immersed Boundary Equations](https://arxiv.org/abs/2608.14310)
*Cole Gruninger,Boyce E. Griffith*

Main category: physics.comp-ph

TL;DR: 本文提出一种"耦合感知Vanka"(CAV)平滑策略，用于对隐式浸没边界(IB)方法中约化后的欧拉速度-压力系统进行有效的多重网格预处理。CAV补丁由标准压力中心Vanka补丁的并集构成，其组合方式由欧拉弹性矩阵的图决定。在网格细化下CAV补丁大小有界，每个多重网格循环的计算代价随欧拉自由度线性增长。实验表明CAV预处理的FGMRES在9-15次迭代内将相对残差降低10个数量级，且在网格细化下增长很小，是首个针对时间依赖隐式IB公式的稳健多重网格策略。


<details>
  <summary>Details</summary>
Motivation: 隐式处理浸没边界力可以消除显式时间离散化中由结构刚度决定的时间步长上限，但由此产生的耦合线性系统随结构刚度增加而越来越难求解。代数消去拉格朗日自由度后得到约化的欧拉速度-压力IB系统，然而该系统缺乏有效的求解策略，需要稳健的预处理方法。

Method: 提出耦合感知Vanka(CAV)平滑策略：CAV补丁以标准压力中心Vanka补丁的并集形式构建，利用欧拉弹性矩阵的图结构决定哪些补丁需要组合在一起。CAV补丁大小在网格细化下有界，从而保证每个多重网格循环的计算成本与欧拉自由度数量呈线性增长。

Result: 使用目标点、膜和梁三种力模型测试，CAV预处理的FGMRES在9-15次迭代内将相对残差降低10个数量级，且随网格细化增长很小。在柔性纤维绕流的非线性基准测试中，欧拉网格从32×32细化到256×256时，每次Newton迭代的FGMRES平均迭代次数仅从8.6增加到9.5。

Conclusion: CAV是第一个针对时间依赖隐式IB公式的稳健多重网格策略，在网格细化下保持近最优的收敛性能与线性计算复杂度，为高刚度浸没边界问题的隐式求解提供了有效的手段。
...
### [60] [Quantifying internal variability in large-ensemble climate data with the Wasserstein distance](https://arxiv.org/abs/2608.14455)
*Yuki Yasuda,Shoichiro Kido*

Main category: physics.comp-ph

TL;DR: 本文提出一种基于1-Wasserstein距离（最优传输成本的一种形式）的新指标，用于量化单模型大集合（SMILEs）中内部变率相对于强迫变率的相对大小，该指标仅需排序和分位数差分即可计算，适用于非高斯分布，且在有约40个以上成员时对分布形状和异常值具有稳健性。


<details>
  <summary>Details</summary>
Motivation: 在气候数据中量化强迫变率与内部变率对于检测气候变化信号和评估气候预估的不确定性至关重要，但现有指标可能对分布形状或异常值敏感，缺乏稳健且通用的度量方法。

Method: 提出用两个1-Wasserstein距离分别度量强迫变率和内部变率，所提指标定义为内部变率距离除以两者之和；该计算仅需对样本排序并对分位数求差，无需额外参数；通过高斯、均匀和对数正态合成数据验证，并与两种现有指标对比，然后应用于CESM-LE的2米气温和总降水数据。

Result: 合成数据验证表明，在集合成员数约40个或以上时，所提指标对分布形状和异常值给出稳定估计，优于现有两个指标；在CESM-LE应用中，所有指标都显示强迫增强时内部变率的相对贡献下降，但所提指标对这一响应的反映最为清晰。

Conclusion: 1-Wasserstein距离为分析大集合数据集提供了简单而有效的工具，所提指标能够稳健地量化内部变率的相对大小，适用于非高斯变量，具有广泛的应用前景。
...
### [61] [GPU implementation of mixed quantum-classical Liouville molecular dynamics without momentum jump](https://arxiv.org/abs/2608.14544)
*Koji Ando*

Main category: physics.comp-ph

TL;DR: 本文在GPU上实现了基于无动量跳跃理论的混合量子-经典Liouville分子动力学模拟。通过去除CPU版本中用于采样增强的轨迹衍生(spawning)机制，避免了GPU上的线程分支和动态内存分配开销，实现了相比CPU计算快一个数量级的加速，并获得了关于采样轨迹数量的线性扩展性。


<details>
  <summary>Details</summary>
Motivation: 传统基于CPU的混合量子-经典Liouville分子动力学模拟中，轨迹衍生(spawning)采样增强方法虽然有效，但在GPU并行环境中会引入线程分叉(thread divergence)和动态内存分配的开销，严重限制GPU的计算性能。作者旨在通过去除该机制来充分利用GPU的并行计算能力，实现模拟的大幅加速。

Method: 作者在GPU上实现了基于无动量跳跃(momentum-jump-free)理论的混合量子-经典Liouville分子动力学模拟。核心策略是去除CPU实现中原有的轨迹衍生(spawning)采样增强步骤，因为该步骤会在GPU上导致线程分叉和动态内存分配问题。通过这一简化，使模拟能够高效地映射到GPU的SIMD并行架构上，实现线性扩展的轨迹采样。

Result: 相较于此前的CPU计算（含轨迹衍生），GPU实现取得了约一个数量级（order of magnitude）的加速。同时，随着采样轨迹数量的增加，计算时间呈线性扩展，展现出良好的并行扩展性。

Conclusion: 在GPU上运行混合量子-经典Liouville分子动力学模拟时，去除轨迹衍生机制虽然牺牲了采样增强功能，但显著避免了GPU并行架构中的线程分叉和动态内存分配瓶颈，从而实现了数量级的加速和良好的线性扩展性。这表明针对GPU架构重新设计算法策略（而非简单移植CPU代码）对于充分发挥GPU算力至关重要。
...
### [62] [On first-order thermodynamic equilibrium conditions for fluid-fluid and solid-phase interfaces](https://arxiv.org/abs/2608.13603)
*Nicodemo Di Pasquale,Thomas Hudson*

Main category: cond-mat.stat-mech

TL;DR: 本文从Larché-Cahn约束变分公式出发，发展了一个统一的固体-流体接触系统热力学框架，将界面热力学直接纳入变分原理中，使体相和界面平衡条件均作为单一热力学泛函的平稳条件出现。


<details>
  <summary>Details</summary>
Motivation: 早期理论忽略了对界面贡献的完整处理，无法描述表面效应显著的系统。需要一个将界面热力学直接纳入变分原理的统一框架，为体相、界面和构型热力学提供共同的理论基础。

Method: 从Larché和Cahn的约束变分公式出发，构建统一热力学框架；首先在流体-流体系统中建立基本数学结构，然后扩展至固体-流体界面；通过不同类别的可容许变分自然地推导出经典平衡关系。

Result: 体相和界面平衡条件作为单一热力学泛函的平稳条件出现；经典的非均质系统平衡关系从不同类别的可容许变分中自然涌现。

Conclusion: 该框架为体相、界面和构型热力学提供了统一的理论基础，能够处理表面效应显著的系统，弥补了早期理论的不足。
...
### [63] [Far-from-equilibrium topological phase transition in one dimension](https://arxiv.org/abs/2608.13658)
*Ze-Min Huang,Gustav John,Sebastian Diehl*

Main category: cond-mat.stat-mech

TL;DR: 本文揭示了一种远离平衡态的拓扑相变机制，通过一维紧致相位模型从随机初始条件确定性演化实现。该机制基于拓扑与对称性而非唯象假设：相位紧致性允许涡旋存在，而均匀不动点（由相移对称性隐含）抑制涡旋成核。涡旋引起的失序与向均匀态弛豫的竞争驱动了一个连续非平衡相变，其普适类被确定为定向渗透（DP）。作者通过构建对应的有效场论并在数值上以动力学标度分析确认了DP临界标度。


<details>
  <summary>Details</summary>
Motivation: 探索远离平衡态的拓扑相变是否具有普适的潜在机制，并希望不依赖唯象假设，仅从拓扑与对称性出发来理解这类相变。

Method: 构造一维紧致相位模型（随机初始条件、确定性演化），利用相位紧致性产生涡旋，借助相移对称性推出均匀不动点并抑制涡旋成核，构建有效场论，并通过数值动力学标度分析验证相变临界行为。

Result: 发现涡旋失序与均匀弛豫的竞争导致连续非平衡相变，其普适类为定向渗透（DP），有效场论预测和数值动力学标度分析均支持DP临界标度。

Conclusion: 该工作揭示了远离平衡拓扑相变可以由拓扑与对称性而非唯象假设来刻画，其普适类属于定向渗透，为理解这类非平衡相变提供了统一的机制性框架。
...
### [64] [Far-from-equilibrium scaling of non-abelian Goldstone modes](https://arxiv.org/abs/2608.13666)
*Carl Philipp Zelle,Gustav John,Orla Supple,Romain Daviet,Sebastian Diehl*

Main category: cond-mat.stat-mech

TL;DR: 本文识别了一类由连续对称性破缺与弱非平衡驱动相互作用产生的非热相，将KPZ物理从单一SO(2)时间平移破缺推广到SO(2)×O(N)非阿贝尔对称性，构建了描述非阿贝尔时间晶体的非平衡非线性sigma模型，并在旋转与振荡相中推导了Goldstone理论。单圈重整化群分析揭示了KPZ类维度结构：d=1,2时任意弱扰动都会失稳平衡不动点并产生强耦合非热不动点；d>2时弱扰动无关，有效平衡恢复。核心结果是旋转相中非传统弱动态标度：强耦合Goldstone区具有不同的普适动力学指数。振荡相恢复了漂移聚合物中的弱标度区。拓扑缺陷最终破坏长程有序但留下实验可观测的非热标度窗口。


<details>
  <summary>Details</summary>
Motivation: 将KPZ普适类从单一SO(2)离散时间平移破缺（传统时间晶体）推广到SO(2)×O(N)非阿贝尔对称性破缺的非平衡系统，此类对称性结构出现在驱动量子材料、活性物质和光诱导周期态中。作者旨在理解这类更广泛对称性下非热相的普适行为，特别是不同Goldstone模式间可能存在不同的动态标度指数，以及非平衡驱动的相关性维度依赖。

Method: 作者构造了SO(2)×O(N)对称性的非平衡非线性sigma模型；推导了旋转相和振荡相中的Goldstone有效理论；进行单圈重整化群分析以研究平衡不动点的稳定性及非热不动点的出现；通过解析分析和1+1维度的直接数值模拟来验证非传统弱动态标度行为；并分析了拓扑缺陷（涡旋/瞬子）对长程有序的破坏效应。

Result: 维度d=1,2时任意弱非平衡扰动都会破坏平衡不动点，产生强耦合非热不动点（涌现平衡性破缺）；d>2时弱扰动无关，有效平衡恢复。旋转相中不同强耦合Goldstone扇区表现出不同的普适动力学指数（非传统弱动态标度），并经数值模拟验证。振荡相恢复并扩展了漂移聚合物中的弱标度区。拓扑缺陷虽破坏长程有序，但留下实验可访问的非热标度窗口。

Conclusion: 该工作将KPZ普适类系统性地扩展到非阿贝尔对称性破缺，建立了非阿贝尔时间晶体的动态标度理论框架，揭示了"涌现平衡破缺"与维度相关的非平衡相变结构，并预言了实验中可观测的非热标度行为。
...
### [65] [Small-world structure of quantum computer hardware](https://arxiv.org/abs/2608.13855)
*S. J. da Silva Junior,D. L. Shepelyansky,J. Lages*

Main category: cond-mat.stat-mech

TL;DR: 本文研究表明，量子计算机的量子寄存器态网络在残余二体相互作用下表现出类似人类社会复杂网络的小世界特性（最可能Erdős数约9，对应Milgram的六度分隔）。利用Åberg准则构建有效网络，发现超过临界耦合强度后网络发生渗流相变形成巨分量，该相变与量子混沌和动态热化的起始高度吻合，且计算规模可扩展至30个量子比特。


<details>
  <summary>Details</summary>
Motivation: 先前研究量子混沌和动态热化需要昂贵的精确对角化方法，计算规模受限。作者希望利用网络科学工具，通过构建寄存器态网络来高效探测量子混沌及热化的涌现，并扩展到更大的系统尺寸。

Method: 构建量子寄存器态网络，节点为量子态，边由量子比特间残余二体相互作用连接。使用Åberg准则（仅保留超过局域能级间距的相互作用）构建有效（Åberg）网络，通过网络分析（小世界属性、Erdős数、渗流理论）研究网络结构及临界耦合强度下的渗流转变。

Result: 量子寄存器态网络展现出小世界属性，最可能Erdős数约9，与社会网络中的六度分隔相当。在临界耦合强度以上，Åberg网络发生渗流转变形成跨越几乎整个量子寄存器空间的巨分量。该渗流转变与先前通过精确对角化确立的量子混沌和动态热化起始点高度吻合，且可将可访问系统尺度扩展至30个量子比特。

Conclusion: 网络科学方法（尤其是渗流分析）为研究量子混沌和热化提供了一种高效替代方案，克服了精确对角化的计算瓶颈，揭示量子寄存器态网络结构与量子动力学行为之间的深层联系。
...
### [66] [Generalizing the multidimensional thermodynamic uncertainty relation to combinations of arbitrary counting variables](https://arxiv.org/abs/2608.14276)
*Niklas Buschmann,Udo Seifert,Alexander M. Maier*

Main category: cond-mat.stat-mech

TL;DR: 本文提出了多维热力学不确定关系（MTUR）的统一推广，适用于时间依赖驱动的马尔可夫网络，支持任意初始状态、任意观测时长、不同类型的计数可观测量（净电流、交通量、流量），甚至允许粗粒化观测中多个不可区分跃迁的合并。


<details>
  <summary>Details</summary>
Motivation: 热力学不确定关系（TUR）为部分可观测马尔可夫网络中隐藏量（如平均熵产生率、总动力学活性）提供下界。已有的TUR扩展主要针对单一电流，而多维TUR（MTUR）虽能通过多个观测电流的协方差提供更紧的界，但尚未被推广到时间依赖驱动、任意初始状态和任意测量时长的情况。作者希望将这些不同的推广统一起来。

Method: 作者将多维TUR推广到时间依赖驱动过程，允许任意初始状态和任意观测时长。该界可适用于一组计数可观测量，每个可观测量可为时间反对称（净电流）、时间对称（交通）或时间非对称（流量）。此外还允许粗粒化观测，即每个计数可观测量可由底层系统的多个不可区分跃迁组成。通过统一框架涵盖了MTUR、基于单电流的时变TUR扩展以及基于单一流量或交通的估计器。

Result: 文章给出了统一的广义不确定关系，并通过简单示例加以说明验证。该结果将多种已有的TUR扩展在时间依赖驱动场景下统一为一个更普适的框架。

Conclusion: 作者成功构建了一个统一的广义热力学不确定关系框架，同时覆盖净电流、交通量和流量三类计数可观测量，支持时间依赖驱动、粗粒化观测、任意初始条件和观测时长，为部分可观测马尔可夫网络的隐藏量估计提供了更普适且更紧的下界工具。
...
### [67] [Linear response across interaction regimes in two-dimensional ferromagnets](https://arxiv.org/abs/2608.14477)
*Aaron Müller,Pavel E. Dolgirev,Oleksii Malyshev,Eugene Demler*

Main category: cond-mat.stat-mech

TL;DR: 本文开发了一种基于高斯函数表示分布函数的方法，有效求解二维铁磁体中的线性化量子玻尔兹曼方程(QBE)，从而能够统一描述弹道区和流体动力学区的磁振子输运。将该方法应用于单层CrCl3，成功解释了氮-空位(NV)中心退相干实验中观测到的异常磁噪声（磁振子声）。


<details>
  <summary>Details</summary>
Motivation: 二维铁磁体的交换驱动磁振子-磁振子相互作用具有显著的动量依赖性：低动量磁振子相互作用弱（弹道区），高动量磁振子散射强并表现出集体流体动力学行为。传统上量子玻尔兹曼方程因其多维碰撞积分而难以求解，缺乏能够同时捕捉弹道和流体动力学两种输运机制的微观框架。

Method: 提出一种将分布函数表示为高斯函数之和的高效表示方法，使多维碰撞积分变得可处理。基于此方法精确求解线性化量子玻尔兹曼方程，计算二维铁磁体在广泛温度和磁场范围内动量分辨与频率分辨的线性响应。

Result: 揭示了由弱相互作用低动量磁振子主导的弹道区到由强相互作用高动量模主导的集体流体动力学区的温度驱动交叉行为。将该方法应用于单层CrCl3，与近期氮-空位中心退相干实验报道的与磁振子声一致的异常磁噪声结果吻合良好。

Conclusion: 该工作为在量子玻尔兹曼动力学框架内描述相互作用的二维量子系统，建立了一个通用框架来计算动量分辨与频率分辨的线性响应，具有广泛适用性。
...
### [68] [Lee-Yang paradigm of phase transition in eigenstate thermalized systems](https://arxiv.org/abs/2608.13174)
*Yongjiang Xu,Weixin Sun,Chushun Tian,Huajia Wang*

Main category: cond-mat.stat-mech

TL;DR: 本文展示了在全息共形场论中，即使系统展现出特征态热化，仍可出现符合Lee-Yang范式的热力学型相变。特征态的关联函数期望值可映射为虚拟瞬时子气体的配分函数，在中心荷c→∞时，一对共轭复零点移动至实轴上的禁止奇点处，系统由此经历低到高逸度的相变。


<details>
  <summary>Details</summary>
Motivation: 孤立量子系统中的相变长期以来难以实现和观测，本文旨在证明即使在展现特征态热化的系统中，也能出现符合Lee-Yang范式的类热力学相变，从而说明特征态热化与相变物理之间并不矛盾，且可能蕴含丰富的物理现象。

Method: 通过全息对应，将全息共形场论中特征态期望的自关联函数映射为虚拟相互作用瞬时子气体的配分函数Z_gauge(z)，其中虚时经共形变换z=1-e^{-τ}，中心荷c分别扮演瞬时子逸度和体积的角色。利用Lee-Yang相变理论分析配分函数的零点分布，研究极端大中心荷极限下的相变行为。

Result: 在c→∞极限下，配分函数的一对复共轭零点移动至实轴上的著名禁止奇点处。系统穿越该奇点时发生从低逸度相到高逸度相的转变，伴随自由能标度行为和主导微观构型的剧烈变化，与Lee-Yang范式高度吻合。

Conclusion: 特征态热化系统的相变物理非常丰富，全息CFT中的这个例子表明，即使系统展现特征态热化，仍可容纳Lee-Yang类型的类热力学相变，为孤立量子系统中的相变研究提供了新的视角和范式。
...
### [69] [Phase transition from eigenstate thermalization: forbidden singularity and instanton proliferation via AGT correspondence](https://arxiv.org/abs/2608.13246)
*Yongjiang Xu,Weixin Sun,Chushun Tian,Huajia Wang*

Main category: cond-mat.stat-mech

TL;DR: 本文利用AGT对应将2D CFT中本征态自关联函数的禁区奇点与N=2 SUSY规范理论瞬子气体的相变联系起来，证明禁区奇点恰好对应Lee-Yang型相变的临界逸度。


<details>
  <summary>Details</summary>
Motivation: 在2D CFT中发现本征态热化（ETH）的关键特征——自关联函数中的禁区奇点，并希望通过与不同物理语境（瞬子气体/规范理论）建立联系来揭示其深层结构。

Method: 利用AGT对应将本征态自关联函数映射为N=2超对称规范理论的Nekrasov配分函数（描述瞬子气体）；在重-轻极限的对应情形下分析瞬子气体的两相结构，并用Lee-Yang理论处理相变，推导临界逸度并对应禁区奇点。

Result: 发现瞬子气体存在两相，分别由具有不同结构和热力学性质的弦状Young图构型主导，类似禁闭相与退禁闭相。当瞬子从一侧增殖时发生相变，其临界逸度精确等于禁区奇点。

Conclusion: 禁区奇点在Eigenstate热化的语境下对应于一个真实的相变现象，这一发现建立了ETH现象与规范理论瞬子气体相变之间的深刻联系。
...
### [70] [Nonorthogonal-state erasure as the resource behind apparent second-law violations](https://arxiv.org/abs/2608.13881)
*Xinshu Xia,Hui Hui Qin,Yu-Han Ma,Chang-Pu Sun,Hui Dong*

Main category: cond-mat.stat-mech

TL;DR: 本文证明完美确定性区分非正交量子态并非导致单热库提取功的热力学操作，真正导致熵减的资源是逆操作——非正交态擦除。作者重新分析Peres型Szilard引擎，指出表观提取功0.2766k_BT并非来自态区分，而是来自非正交态擦除。


<details>
  <summary>Details</summary>
Motivation: 长期以来人们假设如果能够完美区分非正交量子态，这将是从单一热库提取功所需的资源。作者意图纠正这一错误认识，指出正确的熵减操作是逆操作（态擦除），而非态区分本身。

Method: 作者从量子力学的线性与幺正结构出发，证明非正交态区分操作实际上会增加系统与探测器的联合熵，而非降低熵。随后定义逆操作——非正交态擦除——作为真正的熵减资源，并重新分析Peres型Szilard引擎进行验证。

Result: 对于由自旋态|↑⟩和|→⟩等比例混合的原子系综，表观提取功为W_ext=0.2766k_BT。该表观第二定律违背并非由非正交态区分提供，而是由非正交量子态擦除所提供。

Conclusion: 非正交态区分不是热力学上有用的操作，它反而增加联合熵；真正能实现单热库功提取的是非正交态擦除这一逆操作。Peres型Szilard引擎的表观功提取应由态擦除来解释。
...
### [71] [Entanglement spectrum ordering and flavor polarization in the two-flavor Schwinger model at vacuum angle $θ= π$](https://arxiv.org/abs/2608.14296)
*Boliang Yu,Ruixin Zhou,Meisen Gao*

Main category: cond-mat.stat-mech

TL;DR: 本文研究了在真空角θ=π处双味Schwinger模型中的纠缠谱，发现联合Schmidt分布p_{Q_A,F_A}揭示了依赖于切割方式的规范扇区层级和质量诱导的味道不对称性。通过定义综合权重W_F^{(q)}和条件极化P_F^{(q)}，作者发现改变切割方式会反转权重的层级顺序，而质量不平衡会驱动条件极化偏离零。


<details>
  <summary>Details</summary>
Motivation: 理解规范理论中纠缠谱如何编码对称性破缺和规范扇区的组织结构。全局纠缠谱中混合了多种信息，需要对称性分辨的联合分布来分离规范扇区排序、扇区权重和味道极化等不同维度的物理内容。

Method: 在双味Schwinger模型(θ=π)中，计算以子系统规范电荷Q_A和味道不平衡F_A标记的联合Schmidt分布p_{Q_A,F_A}。定义综合权重W_F^{(q)}=p_{q,+1}+p_{q,-1}和条件极化P_F^{(q)}=(p_{q,+1}-p_{q,-1})/W_F^{(q)}两个量来刻画规范扇区层级与味道不对称性，并通过改变交错切割方式(old staggered cut vs intra-cell cut)和大规模不平衡来系统研究这些量的响应。

Result: 改变切割方式会重新组织p_{Q_A}分布（切割依赖的规范扇区层级）；质量不平衡破坏条件味道分布p_{F_A|Q_A}的F_A↔-F_A对称性。具体地，在单元胞边界切割处W_F^{(-1)}>W_F^{(+1)}，而在单元胞内切割处权重层级反转W_F^{(+1)}>W_F^{(-1)}。质量不平衡驱动P_F^{(q)}偏离零，且该响应依赖于规范电荷q。

Conclusion: 对称性分辨能够有效分离全局有序纠缠谱中混合的规范扇区排序、扇区权重与味道极化等不同物理成分，为在规范理论中利用纠缠谱研究对称性破缺和组织结构提供了更精细的工具。
...
### [72] [Information Spreading in Diffusion Models from Effective Field Theory](https://arxiv.org/abs/2608.14308)
*Navonil Neogi,Nabil Iqbal*

Main category: cond-mat.stat-mech

TL;DR: 本文研究了卷积架构的得分匹配扩散模型，提出利用物理学中的有效场论框架来描述其去噪动力学，并在玩具模型和MNIST数据集上验证了该理论预测。


<details>
  <summary>Details</summary>
Motivation: 扩散模型的卷积架构具有局部性归纳偏置，作者认为这种局部性使得物理学中的有效场论工具可以适用于描述其去噪动力学，从而提供理论层面的深入理解。

Method: 作者将有效场论形式主义应用于卷积扩散模型，在可解析描述的简单玩具模型上推导理论预测，进而在MNIST数据集上验证，考察两点间互信息随时间的演化规律。

Result: 在玩具模型和MNIST两个案例中，两点之间的互信息增长方式均与简单有效场论所预测的布朗运动行为一致，验证了该理论框架的有效性。

Conclusion: 有效场论为理解卷积扩散模型的去噪动力学提供了有力的理论工具，局部性归纳偏置使得该物理视角在该类模型中具有普适适用性。
...
### [73] [Quantum Snapshots Reveal a Compact Conformal Boundary Mode](https://arxiv.org/abs/2608.14327)
*M. A. Rajabpour*

Main category: cond-mat.stat-mech

TL;DR: 本文研究临界XX自旋链部分占据快照（partial occupation snapshots）中隐含的共形角度。将该环分为两个测量弧和两个未测量弧后，测量得到的占据构型定义了与几何相关的共形权重参数δ_L，其缩放极限服从共形四边形相对Dirichlet相位分布。该角度是精确的微观紧致坐标，其傅里叶矩由精确自由费米子行列式给出，且证明其提升服从方差为2h(ζ)的高斯分布，最终⟨e^{iqδ_L}⟩→e^{-h(ζ)q²}。因此，量子快照在圆上实现了热核，为Born平均中涨落共形边界条件的紧致零模扇区提供了结果层面的微观基础。


<details>
  <summary>Details</summary>
Motivation: 理解多体量子态投影测量产生的经典数据中是否包含普适的、具有精确共形意义的可观测信息，并建立微观量子快照与连续统共形场论中紧致零模（compact zero-mode）之间的联系。

Method: 采用精确自由费米子行列式方法计算δ_L的所有傅里叶矩；将环划分为两个测量弧与两个未测量弧并赋以共形侧权重；利用完整构型提升X_L构造紧致变量δ_L，通过费米子行列式证明其缩放极限的高斯性。

Result: 证明δ_L在任意有限尺寸下仅由被测位点决定；其缩放极限服从相对Dirichlet相位分布，即共形四边形边界坐标共轭于电荷的量；傅里叶矩精确由自由费米子行列式给出；提升X_L在缩放极限下为方差2h(ζ)的高斯分布，从而⟨e^{iqδ_L}⟩→e^{-h(ζ)q²}。

Conclusion: 原始量子快照以结果层面（outcome-level）实现了圆上的热核，为Born平均上涨落共形边界条件的紧致零模扇区提供了微观基础，揭示了随机量子测量数据中蕴含的普适共形几何结构。
...
### [74] [Universal Thermodynamic Interatomic Potentials for Crystalline Materials](https://arxiv.org/abs/2608.14502)
*Juno Nam,Bowen Deng,Xiaochen Du,Luis Barroso-Luque,Benjamin Kurt Miller,Rafael Gómez-Bombarelli*

Main category: cond-mat.stat-mech

TL;DR: 本文提出热力学原子间势（TIP），将传统原子间势从静态能量扩展为热力学一致的吉布斯自由能模型，通过自动微分获得热力学响应，使自由能计算与势能一样便捷，从而将有限温度相稳定性引入高通量材料发现。


<details>
  <summary>Details</summary>
Motivation: 自由能决定固相稳定性，但计算材料发现仍主要依赖基态能量，因为自由能计算需要系综平均，计算成本高、流程复杂，阻碍了有限温度下相稳定性的高通量筛选。

Method: 提出TIP框架，将原子间势从静态能量扩展为热力学一致的吉布斯自由能模型；基于通用势UMA实现TIP[UMA]，在从准谐近似到分子动力学的多层级自由能数据上训练，并标定到更高精度计算或实验；通过温度与压力的自动微分获得热力学响应；单个评估即可得到晶体状态方程并定位相变。

Result: TIP[UMA]可从单次评估返回晶体状态方程、定位竞争分支间的相变（包括动态稳定相）；微调可扩展到合金溶解度极限和混溶间隙预测。

Conclusion: TIP使自由能计算变得与势能计算同样简便，将有限温度相稳定性开放给高通量材料发现，为该领域提供了新的基础工具。
...
### [75] [Learning Unsteady Aneurysm Hemodynamics with Physics-Informed DeepONets](https://arxiv.org/abs/2608.13629)
*Oscar L. Cruz-Gonzalez,Valérie Deplano,Badih Ghattas*

Main category: stat.ML

TL;DR: 本文提出一种改进的多输入多输出物理信息深度算子网络（M3PI-DeepONet），用于预测腹主动脉瘤（AAA）理想化几何中的非定常3D血流动力学。模型采用聚合注入策略和逐层门控机制，融合3D Navier-Stokes物理约束，仅需0.3%标记内部数据即可同时预测速度场和压力场，平均相对L2速度误差低于4%、压力误差约5%，推理速度比CFD快约36倍。


<details>
  <summary>Details</summary>
Motivation: 临床上需要针对患者的血流动力学评估（壁面剪切应力、涡结构、压力分布）来判断AAA的危险演变，但传统CFD计算成本高昂。虽然PI-DeepONets在补充CFD方面有潜力，但现有架构在处理复杂3D流动时仍面临架构性挑战，亟需一种能高效预测非定常3D血流的算子网络架构。

Method: 提出M3PI-DeepONet架构，核心创新包括：（1）聚合注入（Aggregated Injection）策略，将多个输入分支的潜在表示在主干注入前融合，使坐标基适应多种物理约束；（2）首次将逐层门控机制与多分支算子网络拓扑结合，形成输入自适应的主干基；（3）以3D Navier-Stokes方程作为物理约束，结合初边界条件训练，仅使用0.3%标记内部数据及选定的分支条件信号。

Result: M3PI-DeepONet同时预测非定常3D血流速度场和压力场，平均相对L2速度误差低于4%，压力误差约5%，在分支输入条件可用时，相对参考CFD仿真获得约36倍的保守保留周期推理加速。

Conclusion: 该工作推进了深度学习在心血管疾病建模中的应用，朝着实时、无创的临床诊断方向迈出了重要一步，为复杂3D血流动力学的快速预测提供了新的架构方案。
...
### [76] [On the Brittleness of Maximum Likelihood Estimation for Gaussian Process Hyperparameter Optimization](https://arxiv.org/abs/2608.13793)
*Tyler R. Johnson,Kian Ben-Jacob,Christopher P. Muller,Ramin Bostanabad*

Main category: stat.ML

TL;DR: 本文系统评估了最大似然估计（MLE）在训练高斯过程（GP）模型时的脆弱性，提出了基于理论度量的替代训练方案，在概率回归/分类任务及下游贝叶斯优化中展现出优于MLE和表格基础模型的性能。


<details>
  <summary>Details</summary>
Motivation: MLE是训练ML模型最流行的机制，但其前提假设若不满足会导致模型泛化能力差。即使广泛用于工程设计的GP模型也常被错误地认为对过拟合非常鲁棒，其MLE训练方式的脆弱性未被深入评估。

Method: 作者将理论上严谨的度量指标与MLE进行对比，针对GP的概率回归/分类任务系统评估MLE的脆弱性，并提出实用的替代训练方案，通过大量实验在下游设计任务（如贝叶斯优化）中验证。

Result: 实验证明所提出的解决方案在贝叶斯优化等下游任务中非常有效，训练的GP在预测精度、不确定性量化和推理成本方面甚至优于表格基础模型（如TabPFN）。

Conclusion: 本文为从业者提供了构建准确且鲁棒GP模型的蓝图，证明通过替代MLE的理论度量方法可获得更稳健的GP，同时将代码开源供社区使用。
...
### [77] [Deep Vision in Smart Manufacturing: MODERN Framework for Intelligent Quality Monitoring and Diagnosis](https://arxiv.org/abs/2608.13937)
*Yicheng Kang,Yuling Jiao,Xin Geng,Mahesh Nagarajan*

Main category: stat.ML

TL;DR: 本文提出MODERN，一个基于深度学习（初始残差神经网络架构）的工业制造质量监控与故障隔离框架，包括缺陷可能性控制图、利用迁移学习的故障区域估计器，以及适用于小样本场景的迁移监控技术和假设检验方法。理论建立了缺陷似然估计与故障诊断的极小极大最优收敛速率，实证通过模拟和真实数据验证了其优于现有方法。


<details>
  <summary>Details</summary>
Motivation: 智能制造过程安装了大量传感器和成像设备，但如何将这些增强能力有效整合到工业质量控制实践中仍是一个挑战。需要一种深度学习方法来进行质量监控和故障隔离，以充分利用工业大数据。

Method: 1) 采用初始残差神经网络架构构建控制图，监控产品含缺陷的可能性；2) 提出基于迁移学习的故障区域估计器来识别缺陷区域；3) 针对训练数据不足的情形，提出仅需小样本量的迁移监控技术；4) 使用假设检验方法定量评估方法适用性；5) 理论上建立极小极大最优收敛速率。

Result: 建立了缺陷似然估计和故障诊断的极小极大最优收敛速率；通过模拟实验和真实数据验证，方法优于现有最先进方法；得出反直觉的管理启示——制造商不一定总是应升级监控设备。

Conclusion: MODERN框架能够有效整合深度学习能力于工业质量控制，在理论和实证层面均表现出优越性。同时揭示了一个重要的管理洞见：设备升级决策需要考虑成本与收益的平衡，而非一味追求技术更新。
...
### [78] [Extending Occam's inversion with lasso fusion, overcomplete dictionaries, and isotropic total variation regularisation](https://arxiv.org/abs/2608.14225)
*Anandaroop Ray*

Main category: stat.ML

TL;DR: 本文系统梳理了l1正则化的地球物理反演方法，将其统一到Occam反演框架下，阐明了合成与分析、总变差与字典等不同形式之间的数学联系，并对比了坐标下降、IRLS和分裂Bregman三种求解器的性能，为地球物理学家采用l1正则化反演提供了实用指导。


<details>
  <summary>Details</summary>
Motivation: Occam反演传统上使用l2模型粗糙度惩罚，虽然能提供平滑模型，但不利于识别清晰的地质边界。l1正则化虽然能产生视觉上更锐利的模型，但地球物理领域的l1正则化研究已与统计学和成像文献脱节，例如多维总变差存在两种形式（仅一种具有旋转不变性），这一区别常被忽视。作者希望通过统一框架消除这种割裂。

Method: 在Occam框架内，对l2数据范数和l1模型正则化，证明了一维情形下合成（synthesis）和分析（analysis）两种求解方式等价于融合Lasso问题。将框架推广到多维及字典（如小波变换），并解析了三种常用求解器——坐标下降、迭代重加权最小二乘（IRLS）和分裂Bregman——背后的数学原理，随后在1D回归、2D去模糊（线性问题）和1D航空瞬变电磁（非线性问题，含野外数据）上进行了数值验证。

Result: 对一维问题推荐坐标下降法；对二维问题推荐IRLS而非分裂Bregman，因为IRLS所需的二乘求解次数可少一个数量级。所有方法均在Occam反演统一框架下实现，并通过线性和非线性问题测试验证。

Conclusion: 通过将l1正则化反演的各种形式呈现为熟悉的Occam反演变体，有助于地球物理学家更广泛地采用l1正则化，在保持Occam反演稳健性的同时获得更清晰的地下模型。
...
### [79] [Offline Deep Q* Estimation with Diffusion Models](https://arxiv.org/abs/2608.14401)
*Xiaohong Chen,Yuling Jiao,Lican Kang,Jerry Zhijian Yang,Chen Zhong*

Main category: stat.ML

TL;DR: 本文提出了一种解耦算子估计与值函数学习的离线强化学习框架，利用条件扩散模型估计奖励函数和转移核，从而构造最优Bellman算子的数据驱动近似，并据此得到Q*的深度估计器。理论上建立了最优Bellman算子学习的非渐近收敛率、超额Bellman残差风险的oracle率以及Q*估计器的L2收敛率，且不需要深度RL理论中常用的完备性假设。


<details>
  <summary>Details</summary>
Motivation: 离线强化学习中，最优Q*可通过求解最优Bellman方程得到，但奖励函数和转移核未知导致最优Bellman算子无法从数据中直接观测，这是离线RL的基本挑战。

Method: 首先利用条件扩散模型估计奖励定律和转移核，诱导出最优Bellman算子的数据驱动近似；然后将估计器代入Bellman方程，通过在神经网络函数类上最小化经验Bellman残差来获得Q*的深度估计器。

Result: 建立了条件扩散估计在总变差距离下学习最优Bellman算子的锐利非渐近收敛率；获得超额Bellman残差风险的oracle率O(n^{-2β/(d_x+d_a+2β)})；在集中性条件下，得到Q*估计器L2收敛率O(n^{-β/(d_x+d_a+2β)})。

Conclusion: 该框架避免了深度RL理论中常用的完备性假设，数值实验验证了方法的有效性和强经验性能。
...
### [80] [Online Inference in Distributional Temporal-Difference Learning](https://arxiv.org/abs/2608.14408)
*Yang Peng,Liangyu Zhang*

Main category: stat.ML

TL;DR: 本文研究固定策略下回报分布函数的在线统计推断问题。通过非参数分布时序差分学习从单条马尔可夫轨迹中估计回报分布，证明了Polyak-Ruppert平均估计器的根号T误差在Cramér空间中弱收敛到中心化高斯随机元，并验证了自助法推断的可行性，涵盖平滑统计泛函（方差、CVaR、期望损失、期望分位数）与非平滑统计泛函（回报分位数等）。


<details>
  <summary>Details</summary>
Motivation: 强化学习中回报分布的统计推断长期缺乏严格的理论基础。已有工作大多仅关注回报的期望值估计，而对回报分布的估计及其上任意泛函的推断（如风险度量）缺乏可行性保证。本文旨在为回报分布泛函的在线统计推断建立完整的渐近理论基础。

Method: 采用非参数分布时序差分学习（distributional temporal-difference learning）从单条马尔可夫轨迹估计回报分布。对Polyak-Ruppert平均估计器，证明根号T误差在Cramér空间中弱收敛到高斯随机元；通过自助法论证条件收敛性质；对非平滑泛函，建立局部渐近理论用于处理CDF方程刻画的统计量（如分位数）。

Result: 证明了Polyak-Ruppert平均估计器的根号T弱收敛性，以及自助法估计在轨迹条件化下收敛到相同的高斯极限。建立了适用于平滑与非平滑统计泛函（方差、CVaR、期望短损失、期望分位数及回报分位数）的自助推断理论。

Conclusion: 本文为强化学习中回报分布的自助法统计推断提供了严格的理论保证，填补了分布强化学习在线推断理论上的空白，使得对风险敏感统计量的不确定性量化成为可能。
...
### [81] [L-FNO: Lorentzian Fourier Neural Operator for Stochastic Event Dynamics](https://arxiv.org/abs/2608.13562)
*Songhee Kang,Jihoon Kang*

Main category: stat.ML

TL;DR: 本文提出Lorentzian傅里叶神经算子（L-FNO），一种将FNO风格的协变量路径、Lorentzian谱核（用于历史依赖激发）与基于似然的训练目标相结合的随机神经算子，用于对含稀少、突发、自激事件的条件强度进行建模，并在合成与真实数据集上优于回归和基于似然的神经算子基线。


<details>
  <summary>Details</summary>
Motivation: 现代运行系统在常规条件下也面临不确定性，稀有突发且自激的事件由外生协变量和内生事件动态共同产生。标准神经算子通常被训练为回归式函数映射模型而非条件强度估计器，限制了其在稀疏事件场景中的适用性。

Method: L-FNO是一种随机神经算子，包含三个关键组件：(1) FNO风格的协变量路径，用于编码外生协变量信息；(2) Lorentzian谱核，用于建模历史依赖的激发效应（即自激事件动态）；(3) 基于似然的训练目标，用于估计条件强度函数。

Result: L-FNO在八个合成点过程基准和三个真实数据集（涵盖疾病暴发预测和半导体故障/缺陷检测）上进行了评估。结果表明L-FNO在事件似然、校准诊断和罕见事件检测方面均优于回归式和基于似然的神经算子基线。

Conclusion: 结构化谱记忆和基于似然的学习为神经算子模型建模随机事件动态提供了有效的归纳偏置，表明L-FNO在运行系统的高不确定性和稀疏事件场景下具有实际应用价值。
...
### [82] [Robust XGBoosting for Regression](https://arxiv.org/abs/2608.13590)
*Iris Aragón Mladosich,Christophe Croux*

Main category: stat.ML

TL;DR: 本文研究了XGBoost的鲁棒性问题，发现其性能会受到垂直离群点和杠杆点的影响。为解决这一问题，作者探索了基于稳健回归中M估计、S估计和τ估计的替代损失函数，并提出了一种称为MM-XGBoost的两步估计方法，在鲁棒性和预测精度之间取得了最佳平衡。


<details>
  <summary>Details</summary>
Motivation: XGBoost作为一种广泛使用的预测方法，其标准损失函数（二次损失）和Huber损失在面对垂直离群点和杠杆点时性能会受到影响。现有研究缺乏对XGBoost在存在异常数据情况下鲁棒性的系统分析，因此需要探索更稳健的损失函数来增强其对异常值的抵抗力。

Method: 作者系统研究了XGBoost对垂直离群点和杠杆点的敏感性，并从稳健回归领域中引入了基于M估计、S估计和τ估计的替代损失函数。其中特别提出了一种两步骤方法MM-XGBoost，该方法结合了稳健初始化（高崩溃点估计）和后续的高效估计步骤，以兼顾鲁棒性和预测准确性。

Result: 实验结果表明，在各类替代损失函数中，MM-XGBoost方法在鲁棒性与预测精度之间提供了最佳的权衡，能够有效抵御垂直离群点和杠杆点对XGBoost性能的负面影响。

Conclusion: MM-XGBoost是一种有效的稳健提升方法，为在实际数据中可能包含异常值的场景下使用XGBoost提供了更可靠的替代方案。
...
### [83] [Data-driven techniques for translational neuroscience and personalized neuro-health](https://arxiv.org/abs/2608.13749)
...
TL;DR: 本文综述了用于转化神经科学与个性化神经健康的数据驱动技术工具包，围绕四大互补方法论支柱展开，强调这些方法如何汇聚于个性化、机制可解释且临床可操作的个体脑健康模型的共同转化目标，并讨论了主要的开放统计、计算和临床挑战。


<details>
  <summary>Details</summary>
Motivation: 神经退行性疾病（如阿尔茨海默病和帕金森病）只有在大量且往往不可逆的神经元损失发生后才能被最可靠地诊断，因此迫切需要能够从神经影像数据中检测细微、早期且个体特异性脑部变化的定量工具。

Method: 本文是一篇综述性论文，系统调查了广泛且快速演进的数据驱动技术工具包，围绕四个互补的方法论支柱进行组织，涵盖了从神经影像数据中提取个体化信息的多种技术方法。

Result: 综述表明，尽管方法论上具有多样性，这些方法共同汇聚于一个统一的转化目标：构建个性化、机制上有据可依且临床可操作的个体脑健康模型。

Conclusion: 文章指出该领域仍面临统计、计算和临床三方面的主要开放性挑战，为未来研究指明了方向。
...
### [84] [Separating Spatial and Clinical Risk with Node-Splitting SVM Survival Trees](https://arxiv.org/abs/2608.13847)
*Drew Lazar,Aye Aye Maung*

Main category: stat.ML

TL;DR: 本文提出一种非参数两阶段方法，用于从患者临床特征中分离出地理空间对生存风险的影响。第一阶段用临床生存树拟合协变量得到Nelson-Aalen残差，第二阶段用另一棵核偶极分裂生存树对这些残差进行空间拟合，从而得到分片常数、边界锐利且可能弯曲的空间风险图。方法在理论上证明了在乘性脆弱和exogeneity条件下残差能恢复空间信号，并在LeukSurv白血病数据和模拟实验中验证了有效性。


<details>
  <summary>Details</summary>
Motivation: 恢复生存率的地理空间变异需要将空间风险与患者临床特征分离开来，但预后协变量本身也具有空间结构，使得这一分离问题变得复杂。现有方法（如贝叶斯高斯随机场脆弱模型）虽能识别风险区域但会平滑掉锐利的空间边界，而未经调整的空间分析则会将临床变异错误归因于地理位置。

Method: 采用两阶段非参数方法：(1) 第一阶段，用仅含协变量的临床生存树拟合数据，提取叶节点的Nelson-Aalen累积风险残差，将删失生存结构转移到临床调整后的尺度，且不施加临床风险函数形式；(2) 第二阶段，用另一棵以坐标为输入的核偶极分裂生存树拟合这些残差，恢复空间结构。两阶段均使用核偶极分裂生存树，使空间风险图为分片常数并具有锐利、可能弯曲的边界。理论分析基于乘性脆弱和exogeneity条件，通过脆弱Laplace指数的展开量化残差的近似指数性。

Result: 在LeukSurv白血病数据上，该方法与贝叶斯高斯随机场脆弱模型在风险升高区域的位置上一致，但能解析出平滑表面平均掉的锐利相邻区域；未经调整的空间分析则错误地将临床变异归因于位置。模拟实验包含已知区域以及逐步违反exogeneity条件的扫描，确定了两种贡献不再可分离识别的临界点，且平滑基准方法在该点附近性能同步退化。

Conclusion: 两阶段方法能有效从临床特征中分离出空间生存风险，其关键在于第一阶段只要求一致性而非特定模型类，因此可推广到其他生存估计器。该方法在理论和实证上都优于传统平滑空间模型，尤其擅长解析锐利的空间边界，并明确界定了空间与临床贡献可分离识别的条件边界。
...
### [85] [Generalization Error Estimation for Primal--Dual Algorithms in Non-Smooth Regression](https://arxiv.org/abs/2608.13870)
*Kai Tan,Pierre C Bellec*

Main category: stat.ML

TL;DR: 本文研究非光滑回归中原对偶算法沿轨迹的泛化误差估计，提出了一个包含Chambolle--Pock算法的递归框架，通过用过去对偶迭代的加权组合修正样本内拟合值来估计风险，构造了无协方差的数据驱动修正权重，并在高斯设计下给出了有限样本保证。


<details>
  <summary>Details</summary>
Motivation: 非光滑回归（如ℓ1惩罚最小绝对偏差回归和平方根Lasso）中数据拟合损失不可微，现有的基于梯度型优化路径的风险估计器无法直接适用，因此需要针对原对偶算法的轨迹级泛化误差估计方法。

Method: 建立包含Chambolle--Pock算法及相关原对偶分裂方法的通用递归框架；用可观测的拟合信号轨迹的导数收缩构造替代权重，替代理论上依赖Stein导数收缩和设计协方差的理想权重，从而实现无协方差的数据驱动修正；对平方根岭回归进一步建立匹配高斯普适性结果。

Result: 在高维高斯设计和固定有限迭代次数下，证明了两种估计器的有限样本保证；对平方根岭回归建立了超越高斯设计的匹配高斯普适性结果；数值实验表明所提估计器能准确追踪有限优化路径上的样本外风险。

Conclusion: 所提出的轨迹级风险估计方法有效解决了非光滑损失下原对偶路径的泛化误差估计问题，兼具理论保证与实际可行性。
...
### [86] [High-dimensional nonparametric changepoint detection via low-rank degree-two density projection](https://arxiv.org/abs/2608.13922)
*Guoqing Zhang,Zhaixin Chen*

Main category: stat.ML

TL;DR: 提出了一种基于表示（representation-based）的高维分布变化检测方法，通过对特征矩阵进行低秩CUSUM扫描，在无需参数化密度假设的情况下检测高维分布变化。该方法利用密度二阶正交投影的低秩特性，通过矩阵均值估计替代密度估计，实现了非渐近的算子范数分析和多变化点的精确恢复，并在理论上达到Le Cam下界意义下的最优性。


<details>
  <summary>Details</summary>
Motivation: 高维分布变化检测在前后密度均未参数化指定时非常困难，现有的CUSUM方法通常依赖稀疏性假设或需要参数化密度的模型。作者希望构建一种无需密度估计、不依赖坐标稀疏性、能捕捉纯依赖结构变化（均值不变的）的通用方法，同时保证统计和计算上的可行性。

Method: 构建对称特征矩阵H2(X)，使得M(f)=E_f H2(X)是对密度二阶正交投影的等距编码。对矩阵CUSUM进行秩r截断后扫描，利用投影跳跃的低秩性而非坐标稀疏性。提出LRD估计器（具有帐篷形总体目标函数与非渐近算子范数分析）；对多变化点给出seeded narrowest-over-threshold过程，通过归纳法保持未检测变化点的隔离区间；通过交叉拟合的标量精细化方法在训练折学习变化方向、在验证折定位，达到ÕP(κ^{-2})误差；并基于依赖矩阵Bernstein不等式推广到几何β-混合序列。

Result: 主导随机项以sqrt(rd log(nd))缩放；交叉拟合精化达到ÕP(κ^{-2})定位误差，并与Le Cam下界匹配（至多相差对数因子）。在环境维度高达200、d=100的三变化点序列及128特征的人类活动基准实验中，方法保持计算可行性，并能准确检测纯依赖结构变化（这些变化对均值CUSUM不可见）。

Conclusion: 通过矩阵表示和低秩结构利用，该方法在非参数高维分布变化检测中同时实现了理论最优性和实际可行性，特别擅长检测均值变化CUSUM无法捕捉的纯依赖结构变化，并支持多变化点场景与混合依赖数据的推广。
...
### [87] [QUASAR: Lowering the Loss Floor of Quantization-Aware Training with Loss-Aware Reconstruction](https://arxiv.org/abs/2608.13966)
*Vincent Counathe,Ben Athiwaratkun,Christopher De Sa,Tianyi Zhang*

Main category: stat.ML

TL;DR: QUASAR是一种量化感知训练（QAT）方法，通过在训练循环中持续进行轻量级、损失感知的权重重建来降低损失下限并提升低比特模型质量。它利用梯度平方的指数移动平均值作为在线显著性估计，搜索少量裁剪范围，并通过显著性加权最小二乘拟合仿射反量化器。在Qwen3和Llama-3.1上，QUASAR在2/3/4比特下取得了最低的KL散度，2比特时在八项任务上平均准确率提升3.5-4.3个百分点。


<details>
  <summary>Details</summary>
Motivation: 随着大语言模型推理向低精度方向发展，后训练量化（PTQ）变得越来越脆弱，量化感知训练（QAT）对保持模型质量至关重要。然而，传统QAT存在核心矛盾：损失和代理梯度是基于有损重建的全精度权重计算的，而更新却施加在潜在权重上，这种不匹配导致训练轨迹次优、损失下限更高。虽然二阶PTQ方法通过损失感知重建误差最小化缓解了类似问题，但此类方法仅对冻结模型运行一次就可能耗费数小时，在权重不断演化的QAT过程中重复此过程是不切实际的。

Method: QUASAR在每一步训练中执行三项核心操作：1) 使用梯度平方的指数移动平均（EMA）作为在线显著性估计；2) 在一个小的裁剪范围集合中搜索最优范围；3) 通过显著性加权最小二乘法拟合仿射反量化器。理论分析表明，损失感知重建误差是QAT收敛界中唯一与重建相关的项，且控制着最终量化模型的损失，从而确立了QUASAR目标的原理性优化地位。QUASAR仅修改训练过程，支持包括整数量化和NVFP4在内的标准部署格式，推理时无任何改动或开销。

Result: 在Qwen3和Llama-3.1模型上，QUASAR在2、3、4比特下均取得所有竞争性QAT方法中最低的留出集KL散度：3和4比特下KL至少降低10%，2比特下降低29%。在2比特设置中，相比强QAT和PTQ基线，QUASAR在八个任务上平均准确率提升3.5-4.3个百分点。

Conclusion: QUASAR通过将轻量级损失感知重建融入QAT训练循环，有效解决了传统QAT中损失计算与权重更新之间的不匹配问题，降低了损失下限并显著提升了低比特量化模型的性能。该方法具备理论保障、部署无缝兼容以及显著的实证优势，是面向低精度LLM推理的实用高效QAT方案。
...
TL;DR: 本文研究"插入稳定学习器"的概念：当学习器只会因更多正确标注的数据而减少错误区域时，它就能免疫单调对抗者（monotone adversary）的插入攻击。论文证明插入稳定学习器在高概率保证下完全免疫，且闭合类（intersection-closed classes）因Closure算法保持最优清洁率；但经典维度（VC维、Littlestone维）无法预测这种免疫性，两个同样VCdim=Ldim=2的类可分别处于Theta(1/n)和Theta(log(en)/n)的差异。在Mehrotra的困难类上，没有任何有限大小的单调置换不变压缩方案能达到清洁率。


<details>
  <summary>Details</summary>
Motivation: 先前工作（Larsen等人）用单调对抗者建模了"加入正确数据反而伤害学习器"的现象，Mehrotra证明了最坏情况下的代价是Theta((d/n)log(en/d))，比清洁PAC率多一个对数因子。但该最坏情况分析无法回答哪些具体类别实际上承受这种惩罚、哪些学习器免疫，本文旨在填补这一空白。

Method: 定义"插入稳定学习器"：训练样本加入更多正确标注的例子后，学习器的错误区域只能缩小、不能扩大。由此证明这类学习器天然免疫单调对抗者。利用这一框架：(1) 证明Closure算法是插入稳定的，因此所有交集闭合类保持E[Err] <= (21d+34)/n的清洁率；(2) 构造两个VC维和Littlestone维均为2的类别，展示免疫性差异；(3) 对Mehrotra困难类证明其缺乏任何有限大小的单调置换不变压缩方案。

Result: 插入稳定学习器对单调对抗者完全免疫，高概率保证可无变化继承。交集闭合类通过Closure保持清洁率。免疫性不可由VC维、Littlestone维等经典维度刻画——两个维数完全相同的类别可一个为Theta(1/n)、另一个为Theta(log(en)/n)，而区间类虽有无限Littlestone维却仍免疫。Mehrotra困难类上任何有限大小的单调置换不变压缩方案都无法达到清洁率。

Conclusion: 问题的本质不是类别本身难不难，也不是学习器好不好，而是二者是否匹配。拥有干净数据上最优的插入稳定学习器时，正确数据的加入是无代价的；反之，代价归属于类别本身，更换学习器也无法规避。这重新定义了鲁棒学习的理论框架：将对抗鲁棒性问题转化为学习器-类别适配性问题。
...
### [89] [Forecast Collapse in Time-Series Foundation Models](https://arxiv.org/abs/2608.14106)
*Shu Wan,Miles Ma,Hank Zhu,Guangqi Liu,Stephen Wang,Qingsong Wen,Huan Liu*

Main category: stat.ML

TL;DR: 本文发现时间序列预测中的"预测坍缩"现象：预测1000只美国股票的小时收益率时，预测结果近乎平坦且股票排序能力差（横截面相关性低）。该现象与目标可预测性密切相关，源于两个方面：低可预测性限制校准点预测的幅度，以及逐序列目标函数无法识别横截面结构。作者提出CalibRank目标函数，在保持预测幅度接近真实值的同时，将横截面相关性提升近三倍。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列评估只关注逐序列指标，忽略了跨序列结构（如股票之间的横截面排序关系），导致在实际决策（如投资组合构建）中关键信息的缺失。作者旨在揭示这一盲区，并解决预测坍缩这一此前未被识别的现象。

Method: 作者在1000只美国股票的小时收益率预测场景下，系统性地在时间序列基础模型（TSFMs）、12种深度学习预测模型和97个公开基准配置上调查预测坍缩现象。通过对比收益率与交易量预测的差异定位现象与目标可预测性的关联，并识别出两个原因：低可预测性限制校准点预测幅度、逐序列目标缺失横截面结构。进而提出CalibRank目标函数以同时优化校准与排序。

Result: 在Finance1K数据集上，CalibRank将横截面相关性提升近三倍，同时保持预测幅度接近目标值；在所有测试模型上均提升了横截面相关性。此外，研究表明直接优化横截面相关性虽能提升排序但会放大预测幅度超过一个数量级，验证了校准-排序权衡的存在。

Conclusion: 常规时间序列评估存在盲点：逐序列指标可能掩盖下游决策所需的跨序列结构失败。这要求评估体系需要同时考虑校准与排序两方面，CalibRank为此提供了一个简单有效的平衡方案。
...
TL;DR: 本文重新审视了经典深度玻尔兹曼机（DBM）在表格异常检测中的应用，发现其均值场能量与重构式检测器（Autoencoder）组合时能产生互补效果，在两个表格基准上显著提升检测性能（AUROC分别提升+0.014和+0.002），而其他非DBM基线模型与AE组合反而无效或退化。作者主张经典能量基模型（EBM）应在表格异常检测工具箱中占有一席之地。


<details>
  <summary>Details</summary>
Motivation: 当前表格异常检测主要由密度代理方法（Isolation Forest, OCSVM, LOF）、重构式检测器（AE, VAE）和现代非参数评分器（COPOD, ECOD, Deep SVDD）主导，这些方法都只是间接地近似内层（正常）分布，显式的能量基模型（EBM）在该领域基本缺失。受深度学习领域EBM复兴的启发（如Energy-Based Transformers, JEPA），作者希望通过重新审视经典的深度玻尔兹曼机来填补这一空白。

Method: 采用两层隐藏层的DBM用于表格异常检测，将其均值场能量作为异常评分。在UCI Bank Marketing和NSL-KDD两个跨域表格基准上，与八个经典/现代基线进行全面比较（20个随机种子）。进一步通过秩融合（rank fusion）将DBM能量与Autoencoder评分组合，检验其互补性。

Result: DBM均值场能量在Bank Marketing上与最强基线AE持平，在NSL-KDD上统计显著优于AE，同时在这两个数据集上显著优于其余七个基线。通过秩融合与AE结合后，DBM能量在两个数据集上均取得统计显著提升（Bank Marketing: AUROC+0.014, p进入0.01; NSL-KDD: +0.002, p进入0.001）。所有非DBM衍生的基线与AE配对后要么无法提升要么显著退化。

Conclusion: 经典EBM（以DBM为代表）应当被纳入表格异常检测工具箱，作为与当前占主导地位的重构式评分（reconstruction-based scores）互补的非冗余视角，两种方法结合可获得显著的检测性能增益。
...
### [91] [Designing Reinforcement Learning for Diffusion Models: A Unified Path-Space View](https://arxiv.org/abs/2608.14430)
*Yixian Xu,Yuanrui Zhang,Shengjie Luo,Liwei Wang,Di He*

Main category: stat.ML

TL;DR: 本文证明扩散模型强化学习中的反向轨迹方法和前向匹配方法源自同一路径空间原理，并将两类方法之间的经验差距归因于方差缩减效应而非强化学习原则差异，进而提出统一的算法设计空间和一个多样本KDE值梯度估计器。


<details>
  <summary>Details</summary>
Motivation: 当前扩散模型的RL后训练算法分散为两类：反向轨迹方法依赖离散化似然比，前向匹配方法在带奖励标注的加噪样本上训练。两类方法看似不同且缺乏统一的原理框架，阻碍了算法的系统化设计和改进。

Method: 从正则化的扩散RL目标出发，通过采样SDE之间的重要性采样获得轨迹空间上的显式策略梯度估计器。该估计器包含Flow-GRPO类型更新的随机Itô积分；进一步推导出等价且方差缩减的值梯度形式，恢复了AWM和DiffusionNFT的前向匹配结构。基于此构建由值梯度估计、权重函数和采样选择组织的统一设计空间，并提出多样本KDE值梯度估计器与有界缩放权重族。

Result: 在SD3.5-M和Qwen-Image模型上的实验验证了方差缩减解释，所提出的方法优于现有扩散RL基线。

Conclusion: 扩散RL的两类主流方法在原理上统一，经验差异源于方差缩减而非RL原则分歧；统一设计空间与新的KDE估计器能带来性能提升。
...
TL;DR: 本文提出并研究了大型语言模型应用中会话交接（session handover）的方法论与理论特性，将会话交接形式化为任务相关上下文学习（ICL）状态的迁移，确立了精确恢复与目标分布保持之间的区分，并在外生性条件下推导出预测等价性、最粗确定性充分交接及固定长度比特需求，提出了三部分记录机制，并在高斯线性回归与非参数回归中给出了精确的上界与下界。


<details>
  <summary>Details</summary>
Motivation: 现有大语言模型应用在会话交接时缺乏统一的理论框架来回答"哪些信息必须从前一会话传递"这一核心问题。由于上下文长度限制、应用重启或多代理协作等原因，任务需要在不同会话间延续，但如何决定传递哪些信息、记忆需求如何随后续任务变化，此前缺乏严谨的数学刻画。

Method: 本文将交接形式化为任务相关ICL状态的传递，区分精确恢复与目标分布保持两个目标。在外生性条件下利用预测等价性刻画最粗确定性充分交接并导出固定长度比特需求。提出三部分记录结构：(1)精确存储决策与约束；(2)对重复证据使用任务证明合理的统计量；(3)保留未被统计量保留效果的原始观测。针对高斯线性回归给出精确有限维交接与有限比特扰动界，针对非参数回归建立记忆与平方预测误差之间的上下界。

Result: 在高斯线性回归中实现了精确的有限维交接表示，并给出了有限比特扰动界限；在非参数回归中建立了记忆量与平方预测误差之间的理论上下界。分析隔离了记忆约束、写入者与延续过程各自的效应，并量化了在真实下游查询已知之前进行写入所付出的代价。

Conclusion: 该研究为决定会话交接必须保留哪些信息及其记忆需求如何依赖于后续任务提供了完整的理论与方法框架，填补了LLM会话延续在信息论层面的理论空白。
...
### [93] [Generation-Powered Inference for Distribution-Valued Outcomes](https://arxiv.org/abs/2608.14542)
*Yijiao Zhang,Hongzhe Li*

Main category: stat.ML

TL;DR: 本文提出"生成驱动推断"(GPI)框架，利用辅助生成模型的预测结果(如单细胞基因组学中基因扰动的细胞响应)来改进分布值参数的统计推断，同时不依赖生成模型的正确性。该方法通过函数值桥表示将非线性Wasserstein空间中的推断转化为希尔伯特空间中的均值函数估计，实现类似预测驱动推断的增强估计框架。


<details>
  <summary>Details</summary>
Motivation: 现代生成模型越来越多地产生分布值输出(如单细胞基因组学中预测的基因扰动细胞响应)，这些预测虽提供有价值的辅助信息但本质上不完全准确。需要一种统计方法，既能利用这些预测来提升推断效率，又不过度依赖其正确性。

Method: 提出GPI框架，聚焦Wasserstein重心及相关分布泛函。核心创新是函数值桥表示，将非线性Wasserstein空间中的推断转化为希尔伯特空间中的均值函数估计，从而构建类似预测驱动推断的增强估计结构。开发了一族具有最优信息借用能力的GPI估计量，并建立了相合性、渐近正态性和同时置信带等理论基础。

Result: 通过模拟研究证明了GPI相对于仅使用标注数据方法具有效率提升，且在生成模型误设定下表现稳健。在K562细胞的Perturb-seq实际研究中，利用State基础模型生成的合成扰动响应改进了40S核糖体模块扰动相关的通路级共识基因表达分布推断。

Conclusion: GPI是一个通用且有效的框架，能够在生成模型不完美的情况下，利用其分布值预测来增强Wasserstein重心及相关分布泛函的统计推断，在效率上优于仅依靠标注数据的方法，并具有扎实的理论保证和实际应用价值。
...
### [94] [Comment on: Microscopic signatures of an imaginary charge density wave in a kagome metal](https://arxiv.org/abs/2608.13579)
*Ilija K. Nikolov,Adrien Rosuel,Vesna F. Mitrović*

Main category: cond-mat.str-el

TL;DR: 本评述通过精确对角化和定量分析表明，Ref. [arXiv:2605.05101] 中报道的环流（loop current）信号实际上可由晶体镶嵌（mosaicity）和预转变CDW涨落解释，而非时间反演对称破缺（TRSB）的证据。数据显示NQR线宽遵循标准Curie-Weiss定律无异常，Zp-NQR不对称线形源于角度镶嵌展宽，所报告的局域场源于拟合约束和将角度分布误归因于内场。因此原始数据不足以确立TRSB或T*~120K相变。


<details>
  <summary>Details</summary>
Motivation: 对原论文声称在T*~120K观察到由环流导致的TRSB相变提出质疑，检验其数据解释的可靠性，并指出更可能的微观物理机制（晶体镶嵌与CDW涨落）能够自然解释所有观测特征。

Method: 采用精确对角化（exact diagonalization）结合定量分析，对比NQR线宽与Curie-Weiss定律的拟合，分析Zp-NQR不对称线形与角度镶嵌展宽的关系，并检验局域场h_loc数值的来源与拟合约束条件的影响。

Result: 三点结论：(i) NQR线宽遵循标准CW定律，无任何异常；(ii) Zp-NQR不对称线形的偏斜度可由角度镶嵌展宽自然重现，随自旋跃迁变化；(iii) 所报告的表观局域场源于特定的拟合约束，将角度分布误归为内场效应。原始数据不足以支持TRSB或120K相变。

Conclusion: 原论文的信号更可能源于晶体镶嵌性和预转变CDW波动等常规效应，而非环流诱导的TRSB；需要更严格的数据分析和判别性实验才能确立时间反演对称性破缺。
...
### [95] [Systematic extinctions in inelastic neutron scattering from molecular spin clusters](https://arxiv.org/abs/2608.13795)
*Shadan Ghassemi Tabrizi*

Main category: cond-mat.str-el

TL;DR: 本文通过非弹性中子散射实验和对单晶中分子自旋簇的分析，推导出一种仅由点群对称性决定的动量转移Q处的强度消失条件（称之为"消光"）。这些消光点在给定几何下对任何具有该簇对称性的哈密顿量都成立，并通过推导一个方程来系统地寻找它们。


<details>
  <summary>Details</summary>
Motivation: 已有的理论仅在满足特定条件（单一对称物种介导跃迁、局部自旋算子仅含该物种一次）时，才能用点群完全确定磁跃迁强度对动量转移Q的依赖关系。作者希望放宽这些限制，探索在更一般条件下点群对称性能否仍然约束某些物理可观测量——具体而言是有哪些动量转移位置的强度必然为零，从而为实验提供更普适的预测工具。

Method: 作者从分子自旋簇的点群对称性出发，基于两个能级的对称物种、磁位点的位置及散射振幅，推导出一个方程，其每个零点都对应一个消光（强度必然为零的动量转移）。他们进一步给出了两种可解析求解消光的具体情形：(1) 每个对称等价位点轨道对中介物种只贡献一次；(2) 在某一子群下，位点相位重现但相差一个公共因子。

Result: 即使在已有表格不适用的一般条件下，点群仍能确定特定几何配置下强度必然为零的动量转移（消光）。作者推导出通用的消光方程，并在两种情形下给出了闭合形式的解。

Conclusion: 点群对称性对磁跃迁强度的约束比以往认为的更普遍——即使不满足已有理论的严格条件，仍存在由对称性决定的普适消光点。这些结论可用于指导单晶中子散射实验的测量设计，并为解释实验结果提供理论依据。
...
### [96] [Posterior Inference of Hamiltonian Parameters from RIXS Spectroscopy](https://arxiv.org/abs/2608.13848)
...
TL;DR: 本文首次将基于仿真的推断（SBI）应用于共振非弹性X射线散射（RIXS）光谱学，利用截断边际神经比率估计和条件流匹配作为联合密度估计器，在适度的仿真预算下为两种Ni²⁺化合物（NiPS₃和K₂NiF₄）推断完整的后验分布。通过匹配RIXS图谱物理布局的视觉变换器编码器，获得了比通用图像编码器更优的后验覆盖和锐度，并在实验数据上恢复了参数相关性、后验预测分布及多种新的分析能力。


<details>
  <summary>Details</summary>
Motivation: RIXS光谱学中的参数推断长期以来依赖点估计方法，无法揭示参数间相关性及完整的不确定性；同时仿真成本高昂，需要一种能够高效地从模拟数据中学习参数后验分布的方法。

Method: 采用截断边际神经比率估计（truncated marginal neural ratio estimation）高效地约束先验，以条件流匹配（conditional flow matching）作为联合密度估计器；使用视觉变换器（ViT）编码器，并将其tokenization设计与RIXS图谱的物理布局对齐，优于通用图像编码器。

Result: 在NiPS₃（共价体系）和K₂NiF₄（原子性体系）上以适度仿真预算获得完整后验。物理解耦的ViT编码器产生覆盖更好且更锐利的后验。在实验数据上恢复出点估计无法揭示的参数相关性，后验预测分布与观测光谱高度吻合。

Conclusion: 摊销化后验（amortized posterior）为RIXS领域开辟了新分析范式，包括带干扰参数边缘化的不确定性量化、多测量后验融合以及主动实验设计等此前不可用的能力。
...
### [97] [Anomalous Orbital Reconstruction Controlled by Interorbital Correlations and Hund's Coupling](https://arxiv.org/abs/2608.13892)
*Yu Ni,Lvjin Wang,Jinfeng Wang,Yunkun Niu,Zhaoming Fu,Yun Song*

Main category: cond-mat.str-el

TL;DR: 本文通过动力学平均场理论（DMFT）研究二维两轨道Hubbard模型，揭示了低维关联氧化物中反常轨道极化的微观起源：轨道间库仑相互作用引起的电荷竞争是轨道重构（从轨道极化关联金属到轨道极化Mott绝缘体）的关键机制，而Hund耦合作为随填充数变化的调节器，在四分之一填充时抑制、在半填充时完全消除轨道极化态。


<details>
  <summary>Details</summary>
Motivation: 低维关联氧化物中异常轨道极化的微观起源长期悬而未决，因为晶体场效应、电子关联和轨道依赖维度之间存在竞争，现有理论框架（纯晶体场图像）无法解释实验观测到的反常轨道极化行为。

Method: 采用动力学平均场理论（DMFT）研究包含轨道依赖维度的两轨道Hubbard模型，通过分析轨道重构过程，系统考察轨道间库仑相互作用与Hund耦合在不同填充（四分之一填充和半填充）下的作用。

Result: （1）识别出轨道间库仑相互作用诱导的电荷竞争是轨道重分配的关键微观机制，该机制先导致轨道极化关联金属态，进而导致轨道极化Mott绝缘态；（2）Hund耦合在四分之一填充下通过与轨道间电荷涨落竞争削弱关联诱导的轨道重构；（3）在半填充下，Hund耦合通过稳定高自旋轨道平衡构型完全抑制轨道极化态。

Conclusion: 研究提供了关联驱动轨道重构的统一图景，强调轨道间相互作用和Hund耦合对理解低维过渡金属氧化物中轨道现象的关键相关性。
...
### [98] [Periodicity-driven revision of the phase diagram of the generalized Baxter-Wu model with asymmetric complex couplings](https://arxiv.org/abs/2608.14030)
*Yuting Wang,Ye Ling,Haihong Li,Yuhai Liu*

Main category: cond-mat.str-el

TL;DR: 本文揭示了广义Baxter-Wu模型自对偶线的不完整性：除了传统已知的四个自对偶线 sinh(2K)=±cos(2φ)，还存在由配分函数周期性衍生的附加自对偶线 sinh(2K)=±sin(2φ)。通过Metropolis重加权和Wang-Landau模拟表明，配分函数最小值处 φ=(2n+1)π/8 构成临界阈值：当 |K|≥0.32924 时为真实临界边界，较小 |K| 时则不是；该处的负号问题最严重，峰值是有限尺寸伪影而非新相。


<details>
  <summary>Details</summary>
Motivation: 澄清广义Baxter-Wu模型中自对偶线的完整集合，纠正此前仅考虑 sinh(2K)=±cos(2φ) 的不完整认知；明确自对偶线在临界参数区间的真实性质，区分有限尺寸伪影与真实相变，并评估符号问题及其导数在检测相变中的能力与局限。

Method: 理论推导基于配分函数配分的周期性（bundled Boltzmann权重中的余弦因子）推导附加自对偶线；数值方法包括暴力重加权（Metropolis）和Wang-Landau两种蒙特卡洛方法，在完整自对偶候选线上进行有限尺寸标度分析，并系统评估平均符号及其导数在相变检测中的作用。

Result: 发现附加自对偶线 sinh(2K)=±sin(2φ)；在 φ=(2n+1)π/8 处，当 |K|≥1/2·arsinh(cos(π/4))≈0.32924 时为真实临界边界，较小 |K| 时非临界；该处的负号问题最严重、有限尺寸修正最大；温度扫描中临界边界以下的局部峰是有限尺寸伪影。平均符号的负峰（在 φ_Zmin 处）并不对应真实相变。Wang-Landau虽形式上规避符号问题，仍面临指数壁垒。

Conclusion: 完整自对偶线集合应由正弦和余弦两类共同构成；临界性仅在于参数空间中特定区域（|K|≥0.32924）的自对偶线处成立，且须谨慎区分有限尺寸效应与真实相变；平均符号及其导数不能可靠地替代序参量检测相变；Wang-Landau方法并未从根本上解决负号问题的指数困难。
...
### [99] [Observation of in-plane anomalous Nernst effect](https://arxiv.org/abs/2608.14060)
...
TL;DR: 本文报道在对称性裁剪的铁磁氧化物超薄膜中，反常能斯特效应可源于自发面内自旋磁化的本征耦合，突破了传统能斯特效应中磁场（或磁化）必须垂直于温度梯度与感应电压所成平面的正交性限制。系统的球面旋转磁热电测量表明，面外轨道磁化能够产生与面外响应相当幅度的显著能斯特信号，为磁热电材料与器件的柔性设计开辟了新途径。


<details>
  <summary>Details</summary>
Motivation: 传统能斯特效应受正交性条件约束，即外加磁场或自发磁化必须垂直于由温度梯度和感应电压所定义的平面，这从根本上限制了能斯特效应在能量收集和热管理中的应用灵活性与器件设计自由度。

Method: 采用对称性裁剪的典型铁磁氧化物超薄膜，进行系统的磁热电测量，通过在球面旋转（spherical rotations）外加磁场，全面表征能斯特信号对磁场方向的依赖关系，并分析面内自旋磁化与面外轨道磁化对反常能斯特效应的贡献。

Result: 实验发现，即使在磁场（或自发磁化）位于面内的条件下，体系仍表现出显著的反常能斯特信号，其幅度与面外响应相当。该信号被证明与面外轨道磁化的内禀耦合密切相关，说明反常能斯特效应可以在不满足传统正交性条件下稳健产生。

Conclusion: 本研究证明了反常能斯特效应不再受限于正交性条件，打破了传统能斯特效应的几何约束，为未来磁热电材料与器件的柔性化、多维化设计提供了新的物理基础与实验依据。
...
### [100] [Quantum Geometric Kondo Cloud](https://arxiv.org/abs/2608.14072)
*Grant Z. X. Yang,K. T. Law*

Main category: cond-mat.str-el

TL;DR: 本文研究平带中磁性杂质与量子几何的关联，发现平带Kondo问题中缺失的组织原则是量子几何而非费米面动力学。杂质选择的单个活动bath模式是由杂化因子v(k)加权的平带Bloch态相干叠加，形成的flat-band Kondo问题是"量子几何分子"，Kondo尺度由总投影杂化强度决定而非对数重整化尺度。Kondo云的尺寸张量可规范不变地分解为杂化加权的量子度量、dressed Berry联络协方差和正定的杂化梯度项，满足下界ξ²_K≥Σρ(k)Tr g(k)。


<details>
  <summary>Details</summary>
Motivation: 在传统金属中，Kondo屏蔽由费米面准粒子集体形成自旋单重态基态，Kondo云大小由ξ_K~ħv_F/(k_BT_K)决定。然而在平带中v_F=0，色散能壳的层级结构消失，传统运动学图像失效，缺少一个组织原则来理解平带中的Kondo物理。

Method: 作者提出量子几何作为平带Kondo问题的组织原则。具体方法包括：(1) 证明耦合到孤立平带的磁性杂质只选择一个活动bath模式——由杂化因子v(k)加权的平带Bloch态相干叠加，其余正交模保持暗态；(2) 将平带Kondo问题映射为一个量子几何分子模型，Kondo尺度由总投影杂化强度决定；(3) 在实空间中定义量子几何Kondo云，其尺寸张量可分解为三个规范不变部分：杂化加权的量子度量、dressed Berry联络协方差和正定的杂化梯度项；(4) 由此推导出Kondo云尺寸的下界不等式。

Result: 发现平带Kondo屏蔽由量子几何和杂质选择的Bloch波包的干涉结构主导，而非费米面运动学。Kondo尺度呈代数形式（非对数重整化），由总投影杂化强度设定。Kondo云尺寸张量具有规范不变的分解，并满足下界ξ²_K≥Σ_kρ(k)Tr g(k)，其中g(k)为量子度量。

Conclusion: 在平带中，Kondo屏蔽的物理本质从费米面运动学转变量子几何：杂质对波包的干涉选择结构决定了Kondo耦合的强度与空间分布，量子度量给出了Kondo云尺寸的基本下界。这为理解平带系统中的强关联多体物理提供了新的几何视角。
...
### [101] [Topological phases and quantum criticality from $SU(2)$ Chern-Simons-matter theories](https://arxiv.org/abs/2608.14180)
*Yunchao Hao,Yingcheng Li,Kangle Li,Liujun Zou*

Main category: cond-mat.str-el

TL;DR: 本文从解析角度研究了二维系统中SU(2) Chern-Simons-matter理论的拓扑相和量子临界性。具体包括：对具有p4×SO(3)对称性的晶格自旋系统进行SU(2)_k拓扑序分类（k为任意非零整数），依据k的奇偶性和Lieb-Schultz-Mattis (LSM)反常来枚举SET相的数量；同时分析了SU(2)_k Chern-Simons理论与N_f种无质量玻色子或费米子物质场耦合的理论，在联合大N_f和大k极限下计算了双线性算子的标度维度。


<details>
  <summary>Details</summary>
Motivation: 近期的数值研究显示各种SU(2) Chern-Simons-matter理论会在二维系统中涌现，但缺乏系统性的解析理解，作者旨在从解析角度对这些涌现的拓扑相和量子临界点进行完整分类和定量刻画。

Method: 第一部分利用对称性分类方法，结合LSM反常和p4×SO(3)对称性约束，对SU(2)_k拓扑序的所有可能SET相进行分类统计。第二部分采取联合大N_f和大k极限（N_f/k固定），对SU(2)_k Chern-Simons理论与玻色子或费米子物质场耦合的理论计算到1/N_f阶的双线性算子的标度维度。

Result: 分类结果：奇数k时拓扑序可在任意LSM反常系统中涌现且对称性不能置换任意子（非平凡/平凡LSM分别对应1/9个SET相）；偶数k时只能在平凡LSM涌现，其中k∉{6,10,14,...}时有16个SET相，k∈{6,10,14,...}时有4种任意子置换方式和64个SET相。此外，计算了两种类型物质场模型中双线性算子至1/N_f阶的标度维度。

Conclusion: 该工作从解析层面系统地刻画了SU(2) Chern-Simons-matter理论在二维系统中的涌现现象，为理解这些奇异的拓扑相和量子临界性提供了更精确的理论基础，并为后续研究提供了实用指导。
...
### [102] [Tuning crystal-fields by He-irradiation and orbital Widom line in SrVO$_3$ films](https://arxiv.org/abs/2608.14362)
*Matthias Pickem,Karsten Held,Jan M. Tomczak*

Main category: cond-mat.str-el

TL;DR: 通过DFT+DMFT计算，本文揭示了氦离子辐照SrVO3/SrTiO3薄膜的金属-绝缘体转变机制并非此前认为的Mott局域化（由动能减小驱动），而是辐照诱导的四方膨胀所产生的晶体场劈裂所致。相图镜像一维Hubbard模型，其临界端点衍生出轨道Widom线，源于轨道占据（而非电荷占据）的异常压缩率。在半填充有效态下，超交换过程偏好轨道长程有序而非磁有序。计算结果半定量复现了实验报道的能隙和转变阈值，确立了离子注入作为化学膨胀相关材料的途径。


<details>
  <summary>Details</summary>
Motivation: 此前实验发现氦离子辐照SrVO3/SrTiO3薄膜会引发金属-绝缘体转变，但被错误地归因于动能降低导致的Mott局域化。作者希望通过第一性原理计算厘清真正的物理驱动机制，并解释实验中观测到的谱学特征，从而为相关氧化物材料的性质调控提供理论指导。

Method: 采用密度泛函理论结合动力学平均场理论（DFT+DMFT）对辐照后的SrVO3/SrTiO3薄膜进行电子结构计算，系统地分析辐照引起的晶格四方膨胀对晶体场劈裂、相互作用强度与带宽比值以及轨道占据的影响，构建c轴方向与温度的关系相图，并与单带Hubbard模型进行对比。

Result: 研究表明，辐照诱导的四方膨胀导致的晶体场劈裂才是金属-绝缘体转变的主导机制，而非相互作用强度与带宽之比（U/W）。相图与单带Hubbard模型类似，但在临界端点处出现了轨道Widom线，其物理根源是轨道占据而非电荷占据的异常压缩率。在有效四分之一填充下，超交换类过程更为偏好轨道长程有序而非磁长程有序。计算结果半定量地复现了实验观测到的随辐照通量变化的谱学能隙与转变阈值。

Conclusion: 离子注入/辐照可作为化学膨胀相关材料（如SrVO3）的有效调控手段，其金属-绝缘体转变的本质是晶格应变驱动的晶体场效应，而非传统的Mott物理。该工作为理解辐照诱导的相变提供了新的理论框架，并指出了轨道自由度在相关电子体系相变中的核心作用。
...
### [103] [A Fixed Universal Determinant is Variationally Complete for Continuum Fermions](https://arxiv.org/abs/2608.14476)
*Giuseppe Carleo,Riccardo Rossi*

Main category: cond-mat.str-el

TL;DR: 本文证明在变分计算相关的范数下，任何费米子波函数最多只需两个Slater行列式即可精确描述，与粒子数和目标精度无关。一个预先指定的通用Slater行列式乘以光滑玻色子波函数，可在三维及以下空间中逼近任意费米子波函数（一阶Sobolev范数，控制变分能）；要达到二阶Sobolev范数（对Coulomb相互作用即哈密顿量的定义域，控制变分蒙特卡洛局部能量方差），三维情形最多只需额外的单一固定行列式。因此反对称性最多只需两个通用行列式，广义Slater-Jastrow神经量子态是变分完备的。


<details>
  <summary>Details</summary>
Motivation: 精确描述相互作用费米子需要多少Slater行列式是一个基础性问题。精确展开需要组合数量的行列式，而最先进的费米子神经量子态也堆叠越来越多的行列式。作者希望从根本上消除这一计算瓶颈，探索变分意义下是否只需极少数行列式即可实现完备逼近。

Method: 作者在Sobolev范数框架下进行严格数学证明。首先证明一阶Sobolev范数（控制变分能量）下，一个预先指定的通用Slater行列式与光滑玻色子波函数（类似Jastrow因子）之积可逼近任意费米子波函数。随后证明要达到二阶Sobolev范数（Coulomb相互作用下哈密顿量的定义域，控制局部能量方差），在三维情形最多需增加一个固定Slater行列式。该分析将反对称性"外包"给固定行列式，余下部分由光滑函数处理。

Result: 证明得到以下关键结论：(1) 对抗称性成本至多为两个通用行列式；(2) 一阶Sobolev范数（变分能）下单个通用行列式即足够，适用于维度d≤3；(3) 二阶Sobolev范数（变分蒙特卡洛方差）下需一个额外固定行列式，仅限三维情形。这揭示了广义Slater-Jastrow形式的神经量子态具有变分完备性。

Conclusion: 反对称性在变分意义上仅需极少的固定Slater行列式（最多两个）即可完全实现，不影响表达能力。此结果从理论上支持了广义Slater-Jastrow神经量子态作为变分完备的费米子描述框架，为设计更高效、更具扩展性的费米子量子蒙特卡洛方法提供了严格的数学基础。
...
### [104] [Universal Signature of Hundness and Its Quantification](https://arxiv.org/abs/2608.14480)
*Dongwook Kim,Ina Park,Bo Gyu Jang,Ji Hoon Shim*

Main category: cond-mat.str-el

TL;DR: 本文提出了一种定量描述Hund耦合低能有效性的理论框架，通过两个关联因子z_L（低能准粒子重整化）和z_H（高能电荷涨落）来刻画Hund关联特征，并建立了关联因子与自旋/电荷磁化率之间的一一对应关系，从而在多轨道模型和真实材料中提供了衡量"Hundness"的普适量化标准。


<details>
  <summary>Details</summary>
Motivation: Hund耦合J与Hubbard U在物理上诱导了本质不同的关联效应，导致Brinkman-Rice图像失效，并出现具有Janus双面特征的相——其中能带重整化被限制在某个特征能量尺度以下。已有框架无法定量捕捉这种低能有效性，迫切需要一个新的量化框架来刻画Hund关联的本质。

Method: 作者提出了一个定量框架，通过两个关联因子来捕捉低能有效性：(1) z_L用于描述低能准粒子重整化；(2) z_H（新引入）用于描述高能电荷涨落。z_L与z_H的差异反映了Hund关联的特征。进一步建立关联因子与自旋和电荷磁化率之间的一一对应关系，从而识别出自旋自由度有效性是低能有效性的微观起源。

Result: 该框架在多轨道模型和真实材料中均得到验证，能够提供一种普适且定量的Hundness度量。z_L与z_H的分离成功捕捉了Hund耦合导致的Janus-faced相特征，并揭示了其微观机制。

Conclusion: 该研究为理解Hund耦合与Hubbard U的本质上不同关联效应提供了定量工具，所提出的框架作为一种普适的Hundness度量，可用于多轨道关联电子体系和真实材料的系统性研究。
...
### [105] [Chiral spin liquid and chiral antiferromagnetism in half-filled moiré Hubbard model: possible applications to twisted bilayer TMDs](https://arxiv.org/abs/2608.14503)
*Chuyi Tuo,Hong Yao*

Main category: cond-mat.str-el

TL;DR: 本文利用大规模密度矩阵重整化群（DMRG）模拟，研究了三角晶格半填充最小moiré Hubbard模型中位移场与转角耦合的物理效应。研究发现位移场（由自旋依赖的交错通量刻画）会快速破坏手性自旋液体相、在强耦合120°反铁磁相中诱导明显的手性关联，并在弱耦合下稳定非公度自旋密度波相，同时发现了手性自旋液体与手性反铁磁相之间潜在的、由旋子凝聚驱动的连续相变。


<details>
  <summary>Details</summary>
Motivation: 扭曲过渡金属硫化物（TMD）提供了高度可调的moiré平台来研究超越常规凝聚态体系的关联物理，但位移场与转角之间的相互作用尚未被完全理解，需要系统研究。

Method: 采用大规模密度矩阵重整化群（DMRG）模拟，研究三角晶格半填充的moiré Hubbard模型，其中位移场效应通过自旋依赖的交错通量来刻画。

Result: 位移场显著丰富了三角Hubbard相图：(1) 在极窄弱场区间内迅速破坏手性自旋液体相；(2) 在强耦合120°反铁磁相中诱导明显的手性关联；(3) 在弱耦合区稳定非公度自旋密度波相；(4) 在有限位移场下发现手性自旋液体与手性反铁磁相之间可能的连续相变（可能由旋子凝聚驱动）。

Conclusion: 结果表明位移场驱动了丰富的多体物理现象，为moiré超晶格系统的未来实验提供了有价值的理论指导。
...
### [106] [Thermal effect on the anomaly-induced electromechanical response in gapped graphene](https://arxiv.org/abs/2608.13912)
*A. Sedrakyan,K. Ziegler*

Main category: cond-mat.str-el

TL;DR: 本文研究有能隙石墨烯中声子与电子耦合产生的混合电力学Chern-Simons响应在有限温度下的行为。零温结果显示当化学势穿过带边时响应发生突变，而本文证明有限温度将该尖锐特征替换为仅由温度、能隙和化学势之比控制的普适光滑交叉。响应在绝缘区间保持近似量子化，在带边附近被温度量级的栅压窗口圆滑化，并在掺杂区趋近Berry曲率结果并伴随受控的Sommerfeld修正。该结果适用于行波挠曲波（产生横向二次谐波电流）和动态声子与静态波纹混合（产生基频信号），两者具有相同的栅-温线形，可用于分离反常诱导电流与普通电力学背景，并提取石墨烯器件的有效能隙和电子温度。


<details>
  <summary>Details</summary>
Motivation: 零温研究预测化学势穿过带边时混合电力学Chern-Simons响应出现尖锐突变，但实际实验在有限温度下进行。理解这一响应如何随温度演化对于实验探测并由寻常背景中分离反常信号至关重要。

Method: 通过解析计算有限温度下混合电力学Chern-Simons响应的温度依赖，分析其三参数（温度、能隙、化学势）标度行为，并针对两种实验可行的驱动场景（行波挠曲波产生二次谐波、动态声子与静态波纹混合产生基频）推导响应线形。

Result: 有限温度将零温的突变替换为普适光滑交叉，由温度/能隙/化学势的比值唯一控制。绝缘区响应保持近似量子化，带边附近以温度量级的窗口圆滑过渡，掺杂区趋近Berry曲率结果并带Sommerfeld修正。两种实验驱动产生的信号共享同一栅-温线形。

Conclusion: 研究揭示了有限温度下奇偶反常诱导的混合电力学响应的普适标度行为，提供了一种直接分离反常电流与普通电力学背景的方法，并可以从实验线形中提取石墨烯器件的有效能隙和电子温度。
...
### [107] [Microscopic investigation of spin dynamics in the single-chain magnet Sr4Mn2CoO9](https://arxiv.org/abs/2608.14222)
*G. Roy,S. Ghosh,M. Kumar,E. Kushwaha,J. Sannigrahi,V. Caignaert,W. Prellier,D. T. Adroja,D. Voneshen,V. Hardy,T. Basu*

Main category: cond-mat.str-el

TL;DR: 本文通过非弹性中子散射(INS)和理论建模，研究了Sr4Mn2CoO9中一维Mn-Co-Mn自旋链的自旋激发，发现4和7 meV的低能磁激发及更高能的晶场(CEF)激发，并揭示了室温下持续存在的动态磁关联、Ising型Kramers基态双峰、微观交换相互作用、声子谱与自旋-声子耦合，为理解低温维过渡金属氧化物中的自旋动力学提供了统一的微观图像。


<details>
  <summary>Details</summary>
Motivation: 一维单链磁体为研究晶场效应、交换相互作用和晶格动力学之间的相互作用提供了独特平台，理解其低能自旋动力学对于揭示热激活自旋弛豫的微观机制至关重要。

Method: 采用非弹性中子散射(INS)实验测量自旋激发谱；基于Stevens算符形式进行晶场建模以描述Co2+离子的能级结构；使用SpinW软件进行自旋波模拟以提取微观交换相互作用；利用机器学习辅助的晶格动力学计算验证声子谱和自旋-声子耦合；将交换哈密顿量投影到晶场基态双峰上估算交换诱导劈裂。

Result: INS观测量4和7 meV的低能磁激发以及来自两个不等价Co2+位点的更高能CEF激发；这些自旋激发在室温下持续存在，表明无长程有序下存在动态磁关联。晶场建模成功重现CEF谱，确认两个Co2+离子均为强单轴各向异性的Ising型Kramers基态双峰。SpinW模拟重现自旋激发谱并揭示两条非相互作用Mn-Co-Mn自旋链中的微观交换相互作用。机器学习晶格动力学计算确认了声子谱和自旋-声子耦合。交换诱导劈裂与观测激发一致。

Conclusion: 该工作建立了Sr4Mn2CoO9中晶场效应、磁交换和晶格动力学相互作用的统一微观理解，阐明了晶场各向异性与交换相互作用协同产生的低能自旋动力学，为低温维过渡金属氧化物中的自旋动力学研究提供了深入见解。
...
### [108] [Dispersive-phonon-driven room-temperature Ni1+-Ni2+ polaron hopping in spin-charge coupled rutile niobate](https://arxiv.org/abs/2608.14234)
*Gourab Roy,Mohit Kumar,Sayan Ghosh,Ekta Kushwaha,Manh Duc Le,Devashibhai T. Adroja,Tathamay Basu*

Main category: cond-mat.str-el

TL;DR: 本研究通过非弹性中子散射（INS）和机器学习声子计算，在室温金红石铌酸盐中揭示了色散声子激发，阐明了Ni1+-Ni2+极化子跳跃机制，并证实了即使在磁短程有序态下也存在罕见的自旋-电荷-声子耦合，为耦合电荷-自旋-声子机制介导极化子跳跃提供了微观证据。


<details>
  <summary>Details</summary>
Motivation: 理解晶格动力学如何介导极化子跳跃对设计多功能关联氧化物至关重要，但实验上关于色散声子驱动的极化子跳跃证据十分匮乏。

Method: 采用非弹性中子散射（INS）实验测量室温色散声子激发，并结合基于机器学习的声子计算方法进行互补验证，系统分析声子、磁激发与极化子跳跃之间的耦合关系。

Result: INS测量显示21、33和47 meV处存在显著的色散声子激发，表明集体晶格动力学促进离域极化子传播；4和8 meV处的低能磁激发指示局域短程磁关联或自旋轨道耦合导致的各向异性。机器学习声子计算成功复现实验声子激发并展示晶格不稳定性，与极化子产生的动态局域畸变相吻合。

Conclusion: 本研究为金红石氧化物中耦合电荷-自旋-声子机制介导极化子跳跃提供了微观证据，深化了对室温多功能关联氧化物中极化子输运机制的理解。
...
### [109] [Skyrmion Fractional Chern Insulator: An Intrinsically Multiband Route to Fractionalization in Rhombohedral Graphene](https://arxiv.org/abs/2608.14535)
*Julian May-Mann,Tixuan Tan,Patrick J. Ledwith,Zhengyan Darius Shi,Trithep Devakul*

Main category: cond-mat.str-el

TL;DR: 本文提出了一种全新的分数量子反常霍尔（FQAH）效应微观起源：skyrmion分数化。在菱面体石墨烯莫尔超晶格中，ν<1填充态被视为skyrmion空穴（电荷+e）的金属态，这些空穴由ν=1的skyrmion晶格陈绝缘体中移除层赝自旋skyrmion形成。这些空穴本质上是多带自由度，能自身分数化从而诱导电荷分数化。作者聚焦ν=2/3，构建了skyrmion分数陈绝缘体的变分试探波函数，并提供数值证据和一般性论证表明该过程在能量上是有利的。


<details>
  <summary>Details</summary>
Motivation: 传统认为FQAH效应需要部分填充的陈带，而本文旨在探索一种不依赖部分填充陈带的替代微观机制。在菱面体石墨烯莫尔超晶格中，ν=1的skyrmion晶格陈绝缘体态为研究提供了新的起点，作者希望解释ν<1时观察到的FQAH态，并将其归因于集体赝自旋纹理的分数化而非单粒子陈带的分数化。

Method: 作者首先基于最近提出的理想极限，建立了一个有效场论框架来描述skyrmion空穴的分数化。随后聚焦于ν=2/3填充因子，构造了skyrmion分数陈绝缘体的显式变分试探波函数，并结合数值计算与一般性论证来验证该分数化过程在能量上的可行性。

Result: 数值证据和一般性论证表明，skyrmion空穴的分数化过程是能量上有利的，从而在ν=2/3处可以实现skyrmion分数陈绝缘体态，这为FQAH效应提供了一条现实的实现路径。

Conclusion: 该研究确立了通往FQAH态的一条现实路线，该路线不依赖于部分填充的陈带，而是源于集体赝自旋纹理的分数化，为理解菱面体石墨烯莫尔超晶格中的FQAH效应提供了新的物理图像。
