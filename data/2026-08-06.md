...
TL;DR: 本文通过复制与折叠幺正动力学构造了一个统计力学框架，引入了一个循环红外扇区中的涌现时钟场r_x∈Z_t，其关联函数在MBL、临界和热化相分别呈现指数衰减、无标度衰减和长程锁定三种不同行为，从而为区分多体局域化（MBL）与本征态热化（ETH）提供了统一的序参量。


<details>
  <summary>Details</summary>
Motivation: 传统上区分MBL与ETH依赖一系列各自的诊断量而非单一序参量，缺乏统一的理论框架；本文旨在通过复制与折叠孤立幺正动力学，构造一个能够统一描述MBL、临界点与热化相的单一序参量。

Method: 通过对孤立幺正动力学进行复制与折叠操作，生成前向-后向历史配对的复杂统计力学结构；在闭合循环红外扇区引入涌现时钟场r_x∈Z_t；利用受控l比特约化使时钟作用量近局域化；通过逆关联长度ξ_pair^{-1}=κ_el=ln|ρ_0/ρ_q|统一历史相干性、缺陷线张力和转移谱。

Result: 时钟关联函数在MBL相呈现指数衰减，在临界点呈现无标度衰减，在热化相呈现长程锁定；逆关联长度与缺陷线张力及转移谱统一；复制序提供了一个离散的第二坐标（隐藏动力学不变量首次可见的阶数），能够区分具有相同空间关联长度的局域动力学。

Conclusion: 该工作为MBL与ETH的区分提供了基于复制与折叠相变框架的统一序参量表征，并通过l比特约化保证了可计算性，同时复制序为分辨不同局域化动力学提供了额外自由度。
...
### [2] [Entanglement entropy of fermions in a strange metal](https://arxiv.org/abs/2608.04098)
*Santanu Singh,Surajit Bera,Chenyuan Li,Subir Sachdev,Sumilan Banerjee*

Main category: cond-mat.dis-nn

TL;DR: 本文研究了可解大N一维晶格模型（类似Yukawa-SYK模型）中费米子的纠缠熵。该模型通过空间随机Yukawa相互作用将两个费米点与标量玻色子耦合，在量子临界点提供奇异金属的可解模型。作者精确计算了费米子在空间子区域中的二阶Rényi熵，发现子区域内部费米子-玻色子纠缠与子区域间纠缠共同起关键作用，并提出一个统一标度假设，将不同子区域尺寸和温度下的二阶Rényi熵塌缩为一条普适曲线。该普适标度曲线可用标准CFT公式描述，但有效中心荷显著大于无相互作用值。


<details>
  <summary>Details</summary>
Motivation: 一维无间隙系统（由CFT描述）和具有费米面的自由费米子系统中的基态纠缠熵及其随温度向热熵的交叉已被充分理解，但缺乏准粒子的无间隙费米子系统（如奇异金属）的纠缠熵知之甚少，本文旨在填补这一空白。

Method: 利用可解的大N一维晶格模型（Yukawa-SYK类模型），其中两个费米点通过空间随机Yukawa相互作用与标量玻色子耦合；精确计算费米子在空间子区域中的二阶Rényi熵。

Result: 揭示了子区域内部费米子-玻色子纠缠与子区域间纠缠在理解强耦合费米子-玻色子系统基态中的关键作用；证明了从热熵到纠缠熵的交叉可由单一标度假设描述，将不同子区域尺寸和温度下的二阶Rényi熵塌缩为一条普适曲线；该普适标度曲线可由标准CFT公式很好地描述，但有效中心荷显著大于无相互作用值。

Conclusion: 临界奇异金属的纠缠熵标度行为遵循标准CFT公式，但有效中心荷大幅增大，反映了强耦合费米子-玻色子相互作用的效应；该工作为理解无准粒子费米系统的纠缠结构提供了新的可解框架。
...
### [3] [Experimental Kuramoto Platform](https://arxiv.org/abs/2608.04119)
*Leandro Freitas,Victor H. S. Bittencourt,Débora D. Superbi,Wanderley C. Silva Junior,Arthur N. Montanari,Luis A. Aguirre*

Main category: cond-mat.dis-nn

TL;DR: 本文提出了一种基于电子正交振荡器的实验平台，其相位动力学在弱耦合和强耦合条件下均与Kuramoto模型等价，可支持任意加权有向网络拓扑，并可靠复现全局和簇同步的相变行为，为复杂网络同步的理论与数值结果提供了低成本实验验证工具。


<details>
  <summary>Details</summary>
Motivation: Kuramoto框架在分析振荡器同步方面具有强大的理论价值，但将基于弱耦合假设的理论结果与实验数据连接起来仍是一个开放挑战，因为许多物理平台不满足标准相位约简的弱耦合条件。作者希望构建一个实验平台，使其实验结果能直接与Kuramoto模型的理论预测对应。

Method: 采用电子正交振荡器作为实验平台，通过非扩散耦合电路实现相位动力学在弱和强耦合条件下均等价于Kuramoto模型。该设计支持任意加权和有向网络拓扑，同时保持可扩展性和振荡规律性。实验中针对不同耦合方案，验证了全局同步和簇同步的相变行为。

Result: 实验平台在不同耦合方案下均能可靠复现Kuramoto模型预测的相变行为，包括全局同步和簇同步，且该等价性在弱耦合和强耦合条件下同时成立。

Conclusion: 该低成本实验平台为复杂网络同步的分析和数值结果提供了系统性的实验验证途径，填补了Kuramoto理论框架与实验数据之间的空白。
...
### [4] [Variational Bounds for Perceptron Learning from Structured Data](https://arxiv.org/abs/2608.04882)
*Francesco Camilli,Pierluigi Contucci,Federica Gerace,Emanuele Mingione*

Main category: cond-mat.dis-nn

TL;DR: 本文提出了一种针对有限温度连续自旋感知机（在混合高斯数据上训练）的变分方法，通过结合插值方法与对数凹性和浓度估计，推导出极限淬火压力的上下变分极小极大界。两个界仅在两个变分参数的优化顺序上不同，当优化可交换时两者一致，从而确定模型解。


<details>
  <summary>Details</summary>
Motivation: 为有限温度下训练于高斯混合数据的连续自旋感知机建立统一的变分框架，扩展已知的精确解结果，并统一计算基态能量、训练损失和泛化误差。

Method: 结合插值方法（interpolation method）与对数凹性（log-concavity）及浓度估计（concentration estimates），分别推导极限淬火压力的下界和上界极小极大变分界。两个界的差异仅在于两个变分参数的优化顺序，其余极值由变分势的凹-凸结构控制。

Result: 当两个变分参数的优化顺序可交换时，上下界匹配，从而得到模型的精确解。同一变分势给出定点方程作为驻点条件，为计算基态能量、训练损失和泛化误差提供统一路径。

Conclusion: 该变分框架适用于广泛的凹效用函数和对数凹可分离先验测度，为有限温度连续自旋感知机提供了通用且统一的分析工具。
...
### [5] [Statistical learning theory and Occam's razor: Regularization](https://arxiv.org/abs/2608.04049)
*Tom F. Sterkenburg*

Main category: stat.ML

TL;DR: 本文基于统计学习理论为奥卡姆剃刀原则（正则化/简单性偏好）提供了一个方法论层面的合理性证明：为了获得理论可靠性以及"所见即所得"的泛化保证，必须优先选择简单性而非拟合度。


<details>
  <summary>Details</summary>
Motivation: 奥卡姆剃刀原则（偏好简单性）在科学哲学和机器学习中都被广泛讨论，但始终缺乏一个严格的理论辩护。已有的解释要么退化为"为简单而简单"的纯粹实用主义原则，要么依赖"真理是简单的"这一本体论假设，均不令人满意。

Method: 基于先前提出的"核心论证"，从统计学习理论的视角出发，构建"手段-目的"（means-ends）论证：说明为了获得理论可靠性以及"所见即所得"（what-you-see-is-what-you-get）的保障，必须实现某种对简单性优于拟合度的偏好。该论证不依赖于本体论假设或纯实用主义理由。

Result: 论文成功地从统计学习理论中推导出对正则化（即在拟合度与简单性之间进行权衡）的合理性证明。该证明表明简单性偏好是实现可靠泛化和保证性的必要手段，而非目的本身。

Conclusion: 这构成了一种真正的方法论层面的辩护：它既没有退化为"为简单而简单"的纯实用主义原则，也没有依赖"真理本身是简单的"这一本体论预设，从而填补了奥卡姆剃刀原则在归纳推理中缺乏严格理论基础的空缺。
...
### [6] [Automatic Statistical Test for Rationally Expressible Algorithms by Selective Inference, with Applications to Feature Selection](https://arxiv.org/abs/2608.04667)
*Teruyuki Katsuoka,Tomohiro Shiraishi,Shuichi Nishino,Ichiro Takeuchi*

Main category: stat.ML

TL;DR: AutoSI 是一个自动化选择性推断（Selective Inference）框架，能从算法的单个操作中自动构造选择事件，并将可处理的算法类别从线性/二次不等式扩展到任意有理函数。论文证明其p值在有限样本下精确有效，并通过三种特征选择方法（包括无法被现有SI框架处理的交叉验证lasso）验证了错误控制率和检验功效。


<details>
  <summary>Details</summary>
Motivation: 传统选择性推断需要专家针对每个新算法人工推导并实现选择事件（即假设被选中的条件），这一高度专业化的重复工作导致精确SI仅适用于极窄的算法类别。作者旨在消除这一障碍，使SI能够自动推广到更广泛的算法。

Method: AutoSI 通过两方面实现自动化的SI：（1）从算法的单个操作自动构造选择事件，用户只需以普通NumPy风格的代码编写算法而无需手工推导；（2）将SI可处理的选择事件类别从线性或二次不等式扩展到任何可用数据的有理函数（多项式之比）表达的算法。框架自动完成选择事件的构建，并计算有限样本下精确有效的p值。

Result: 论文证明AutoSI计算的p值在有限样本下精确有效（控制type I error）。在三种特征选择方法上做了演示，每种只需几十行代码。特别地，交叉验证R²选参的lasso是现有精确SI框架无法处理的，AutoSI使其成为可能。在合成和真实数据上的实验表明，p值能在名义水平上控制type I error并保持较高的统计功效。

Conclusion: AutoSI通过自动化选择事件的构造，大幅降低了选择性推断的使用门槛，并显著扩展了可处理算法的范围，使得更多实际应用中的算法能够获得统计上有效的推断结果。
...
### [7] [Intrinsic-Hybrid Latent Diffusion Models for Generative Modeling on Unknown Manifolds](https://arxiv.org/abs/2608.04827)
*Yizhu Wang,Mu Niu,Xiaochen Yang*

Main category: stat.ML

TL;DR: 本文提出内在混合潜扩散模型（ILDM），将概率降维与未知流形上的几何感知扩散相结合。ILDM将潜空间视为未知黎曼流形的坐标图，通过概率解码器量化几何与不确定性；前向过程为混合扩散，根据局部不确定性在黎曼与欧几里得动力学之间切换；采用近似的去噪分数匹配方法训练混合扩散生成动力学。在COIL-100、MNIST和心脏MRI数据集上，ILDM相较标准扩散模型和潜扩散模型取得了更低的FID和LPIPS分数。


<details>
  <summary>Details</summary>
Motivation: 标准扩散模型（DMs）在高维数据合成中表现出色，但依赖大量训练数据且忽略内在几何结构；潜扩散模型（LDMs）虽然通过学习潜空间解决高维问题，但通常强加欧几里得结构，无法捕捉底层流形的几何特性，尤其在数据稀疏场景下问题更为突出。

Method: ILDM将潜空间解释为未知黎曼流形的一个坐标图，通过概率解码器量化几何与不确定性。前向过程是一种混合扩散，根据局部不确定性在黎曼和欧几里得动力学之间切换，其中黎曼分量由从解码器导出的概率度量张量控制。训练阶段采用为混合扩散设置量身定制的近似去噪分数匹配方法，使反向过程由混合朗之万动力学定义。

Result: 在COIL-100、MNIST和心脏MRI数据集上的实验表明，ILDM相比标准扩散模型和潜扩散模型显著提升了生成质量，取得了更低的FID和LPIPS分数。

Conclusion: ILDM通过将几何感知的概率降维与流形上的混合扩散动力学相结合，有效解决了数据稀疏场景下潜扩散模型忽视底层流形几何的问题，验证了混合黎曼-欧几里得扩散范式在低数据和高维生成任务中的优越性。
...
### [8] [Stable Density Ridges: Consistency and Convergence of Subspace Constrained Mean Shift](https://arxiv.org/abs/2608.05112)
*Wanli Qiao*

Main category: stat.ML

TL;DR: 本文推翻了文献中广泛认为的SCMS算法收敛于"静态脊"（static ridge）的假设，指出该假设忽略了尾部特征空间沿向量场连续旋转的影响。作者提出"稳定脊"（stable ridge）这一全新几何结构，并通过动力系统视角证明了它才是SCMS算法的真实理论目标。在此基础上，作者开发了广义SCMS框架，采用恒定步长，建立了均匀R-线性收敛性和拓扑满射性，推导了稳定脊估计的Hausdorff距离收敛速率，并揭示了原始SCMS算法因步长与带宽隐式耦合而具有多项式时间复杂度的缺陷。


<details>
  <summary>Details</summary>
Motivation: 现有文献普遍认为SCMS轨迹收敛于通过密度梯度及Hessian特征值/特征向量定义的"静态脊"，但这一假设在一般情况下不成立，因为静态定义未能考虑算法底层向量场中尾部特征空间的旋转效应。作者旨在纠正这一理论错误，建立SCMS算法真正的收敛目标。

Method: 通过动力系统理论视角，引入投影密度梯度的Jacobian，定义"稳定脊"作为新的几何结构；证明SCMS算法收敛于该稳定脊；构建使用恒定步长的广义SCMS框架，并利用Hausdorff距离分析其估计收敛速率。

Result: 证明了稳定脊是SCMS算法的真正理论目标；广义SCMS框架具有均匀R-线性收敛性和拓扑满射性；推导了稳定脊估计的Hausdorff距离收敛速率；揭示了原始SCMS算法因步长与平滑带宽通过Mean Shift算子隐式耦合而导致多项式时间内不可收敛的计算复杂度问题，而广义框架提供了统计一致且更高效的解决方案。

Conclusion: SCMS算法收敛目标的传统认知存在根本性错误，动力系统视角下的"稳定脊"才是其真正收敛目标。广义SCMS框架通过解耦步长与带宽，不仅修正了理论偏差，还显著提升了计算效率并保证了统计一致性。
...
### [9] [Informational Frustration in Neural Manifolds: Shannon Bottlenecks and the Limits of Learnability](https://arxiv.org/abs/2606.30512)
*Srinivasa Rao P.,Vangmayi P Reddy*

Main category: stat.ML

TL;DR: 本文提出一个统一框架，将信息论、拓扑学和统计力学联系起来，解释深度网络为何能有效泛化。核心是"熵可学习性视界"（ELH）定律，以及香农-拓扑瓶颈定理，将泛化失败归因于"信息挫折"玻璃态相变，并以此解释grokking现象，最后提出熵梯度下降（EGD）算法。


<details>
  <summary>Details</summary>
Motivation: 经典学习理论框架（VC维、Rademacher复杂度）无法解释现代过参数化深度网络的良好泛化能力，理论与实际之间存在巨大鸿沟，迫切需要新的理论视角来弥合这一差距。

Method: 提出统一框架，将信息论（香农熵）、拓扑学（决策边界的拓扑熵）与统计力学（冯·诺依曼熵、相变理论）相结合。引入"熵可学习性视界"作为学习能力的基本定律，并据此证明香农-拓扑瓶颈定理，将学习失败建模为熵驱动的相变现象。基于该理论提出熵梯度下降（EGD）优化算法。

Result: 证明了当决策边界几何复杂度超过信息可学习视界时，系统发生熵相变，进入"信息挫折"状态——即玻璃态的刚性记忆阶段，此时泛化在热力学意义上不可能。grokking现象被解释为"熵释放"过程，即权重突然重组解锁瓶颈。EGD算法通过动态管理权重熵来维持学习进程。

Conclusion: 该工作将熵重新定位为决定机器能否学习的基本物理量，超越了仅作为不确定性跟踪工具的既有视角，为深度学习理论提供了统一框架和可落地的优化算法。
...
### [10] [Statistical Mechanics of Learning on Product Wasserstein Manifolds](https://arxiv.org/abs/2608.01434)
*Srinivasa Rao P Vangmayi P Reddy*

Main category: stat.ML

TL;DR: 本文提出将分布约束从"限制模型容量"重新诠释为定义学习的固有几何结构。作者将深度神经网络和变分量子电路分别建模为经典Wasserstein流形和量子Wasserstein流形上的梯度流，并提出两种实用算法（Hierarchical DisCo-SGD和Quantum DisCo）来沿流形上的近似测地线进行优化。实验表明，尊重这些分布几何结构相比无约束和纯范数基线能改善泛化、稳定训练并减少贫瘠高原问题。


<details>
  <summary>Details</summary>
Motivation: 传统统计力学观点认为权重分布约束会缩小解空间、降低模型容量。作者希望提出相反观点：与其将预设的权重分布视为限制，不如将其定义为学习自然展开的内在几何结构，从而将结构约束重新框架化为几何先验。

Method: 1) 将深度神经网络和变分量子电路分别表述为Wasserstein流形乘积上的梯度流——每层对应一个经典Wasserstein空间，电路参数对应一个量子Wasserstein空间（量子Wasserstein距离阶数为1）。2) 为深度网络开发层次化平均场描述。3) 提出两种实用算法：Hierarchical DisCo-SGD（经典）和Quantum DisCo（量子），沿流形乘积上的近似测地线进行优化。

Result: 在teacher-student问题、标准图像分类任务和小型变分量子分类器上的实验表明，遵循分布几何结构相比无约束和纯基于范数的基线，能改善泛化性能、稳定训练过程，并减轻量子训练中贫瘠高原（barren plateaus）的严重程度。

Conclusion: 该方法首次将结构约束重新框架化为几何先验，为将生物、谱或硬件衍生的分布信息纳入经典和量子学习系统开辟了新途径，同时也统一了深度学习与量子机器学习中分布约束的几何视角。
...
### [11] [Multimodal Alignment Through Joint Kernel Entropic Gromov--Wasserstein Optimal Transport](https://arxiv.org/abs/2608.04234)
*Yixuan Florence Wu,Yilun Zhu,Naichen Shi*

Main category: stat.ML

TL;DR: 本文提出JK-EGW框架，利用联合核熵Gromov-Wasserstein最优传输将多个模态映射到共享表示空间，在预训练单模态编码器可用但跨模态配对数据稀缺的场景下实现了结构保持的对齐，并建立了n^{-1/2}的样本复杂度下界、推导出可扩展的交替求解算法，在少样本多模态检索任务上优于现有对齐基线。


<details>
  <summary>Details</summary>
Motivation: 跨模态数据对齐是机器学习中的核心问题，但实际应用中跨模态配对数据往往稀缺昂贵。现有对齐方法通常依赖大量配对样本或仅利用原始特征空间距离，忽视了模态内部及模态间的细粒度相似性结构，导致在数据稀缺场景下性能退化。如何利用预训练单模态编码器的强大先验，仅在少量配对数据下实现高质量的多模态表示对齐，是本文的核心动机。

Method: JK-EGW框架通过最小化二次最优传输目标实现多模态到公共潜空间的对齐。其关键创新在于：1）利用模态内和模态间的细粒度相似关系构建全局亲和核，而非直接使用原始特征空间距离；2）框架允许显式控制潜嵌入的几何特性与分布；3）在算法上通过低秩核近似和变分提升（variational lifting）将二次目标转化为可利用现有熵最优传输（EOT）求解器的交替过程，缓解二次目标的计算负担。

Result: 理论上建立了n^{-1/2}的参数样本复杂度率，与标准最优传输、熵最优传输和Gromov-Wasserstein最优传输的相应速率匹配。在实验上，聚焦于数据稀缺场景下预训练编码器嵌入的事后对齐（post-hoc alignment），结果表明所提方法在多模态检索性能上优于现有对齐基线。

Conclusion: JK-EGW为跨模态对齐提供了一种兼具理论保证和实用可扩展性的统一框架，特别适合预训练编码器可用但配对数据稀缺的现实场景。通过结构保持的亲和核设计与EOT求解器的有效复用，实现了样本效率与计算效率的双重提升，为多模态表示学习了提供了新的思路。
...
### [12] [When Is a Conformal Guarantee Fair? Auditing Silent Subgroup Under-Coverage in Alzheimer's Disease Longitudinal Prediction](https://arxiv.org/abs/2608.04254)
*Lujia Zhong,Xinkai Wang,Shuo Huang,Yonggang Shi*

Main category: stat.ML

TL;DR: 本文提出一种基于机制驱动的框架来审计和修复共形预测在阿尔茨海默病生物标志物纵向预测中对高风险亚组覆盖不足的问题。研究发现，尽管总体边际覆盖达到名义水平，但群体级预测带在68个审计组合中有57个对高风险亚组覆盖不足，主要归因于"稀有性"和"重尾性"两种机制，并提出了相应的共形校正方法来恢复亚组覆盖保证。


<details>
  <summary>Details</summary>
Motivation: 共形预测在阿尔茨海默病生物标志物纵向预测中提供带有限样本覆盖保证的预测区间，但标准共形预测仅保证边际覆盖，可能掩盖临床上重要亚组内的严重覆盖不足问题。由于高风险亚组（如高遗传风险、高疾病严重程度）的决策影响更关键，需要系统性审计并修复这类亚组层面的覆盖缺陷。

Method: 本文提出一个通用的机制驱动框架：(1) 审计阶段，评估群体级共形预测带在多个临床相关亚组上的条件覆盖表现；(2) 机理归因阶段，将覆盖失败追溯到两种机制——稀有性（亚组样本量n限制条件覆盖上限为k/(n+1)）和重尾性（群体级预测带对重尾亚组过窄，增加数据无法弥补）；(3) 修复阶段，针对不同机制采用相应校正：交叉共形池化用于稀有性、逐亚组校准用于重尾性、两者兼有时采用覆盖安全边际下限。实验在两个队列（ADNI, OASIS-3）、两个基预测器和九个属性（涵盖遗传风险、人口统计学和临床严重程度）上进行验证。

Result: 在68个审计组合中，57个存在高风险亚组覆盖不足；高遗传风险和疾病严重程度的患者平均覆盖缺口达6.1个百分点（95% CI [3.3, 8.9]），而人口统计学亚组平均覆盖处于目标水平（0.0 pp）。两种机制（稀有性和重尾性）均被证实为覆盖失败的驱动因素。所提出的三种校正方法在几乎所有高风险亚组上恢复了目标覆盖水平，跨两个队列和两种基预测器均有效。

Conclusion: 群体级共形预测的边际覆盖保证不足以保护高风险临床亚组的预测可靠性。通过识别覆盖不足的具体机制并针对性地应用共形校正，可以在不牺牲整体边际覆盖的前提下恢复亚组级别的覆盖保证，从而提升阿尔茨海默病生物标志物预测在临床决策中的可信度。
...
### [13] [Sample Complexity of Multicalibration for Multilevel Properties](https://arxiv.org/abs/2608.04288)
*Jiuyao Lu,Krishnakumar Balasubramanian,Aleksandr Podkopaev,Shiva Prasad Kasiviswanathan*

Main category: stat.ML

TL;DR: 本文研究了序列化多性质多校准（sequential multicalibration）的样本复杂度问题，其中每个性质在前序性质固定后可识别。对于任意k≥2，作者建立了匹配的上界和下界（至多差对数因子），证明了实现ε多校准误差需要Ω̃(ε^{-(k+2)})样本，同时给出O(ε^{-(k+2)}+ε^{-2}log|G|)样本的随机学习器。


<details>
  <summary>Details</summary>
Motivation: 多校准（multicalibration）要求预测器在自身上预测条件下无偏，并在多组上同时满足。然而许多预测任务需要预测同一条件结果分布的几个相关特征（如方差相对均值、偏度相对均值和方差、条件风险价值相对分位数），这些特征构成一个序列，每个特征需要在前序特征固定的基础上定义。现有理论主要针对单一性质，缺少对这类序列化多性质校准问题的样本复杂度分析。

Method: 论文提出一个统一的框架，研究k个性质的序列化多校准。其方法包括：(1) 建立匹配的样本复杂度下界，即使只有极多对数个二值组，也需要Ω̃(ε^{-(k+2)})样本；(2) 构造随机化学习器，对任意有限组族G使用O(ε^{-(k+2)}+ε^{-2}log|G|)样本；(3) 将理论应用于三个经典示例（包括方差、偏度、条件风险价值等序列化性质）。

Result: 对于每个固定的k≥2，在正则条件下建立了上下界至多差对数因子的样本复杂度。具体地：下界表明以ε多校准误差需要Ω̃(ε^{-(k+2)})样本；上界给出一个随机学习器仅需O(ε^{-(k+2)}+ε^{-2}log|G|)样本。对于多项式大小的组族，样本复杂度为Θ̃(ε^{-(k+2)})。论文并将理论应用于三个经典示例进行了实例化。

Conclusion: 该研究将多校准理论从单一性质推广到序列化多性质场景，建立了紧的（至多差对数因子）样本复杂度界。框架不仅涵盖贝叶斯对（Bayes pairs），且不要求性质来自单一损失函数，具有更广泛的适用性。结果表明对于任意固定的k≥2，样本复杂度本质上是ε^{-(k+2)}量级。
...
### [14] [ArborEnum: Decision Tree Rashomon Sets over Continuous Features](https://arxiv.org/abs/2608.04310)
*Zakk Heile,Hayden McTavish,Margo Seltzer,Cynthia Rudin*

Main category: stat.ML

TL;DR: 本文提出了第一个能够精确枚举决策树Rashomon集的算法，该算法利用连续特征的有序结构，而无需先对数据进行二值化处理，并进一步开发了近似枚举的松弛方法以及逐步精化候选阈值的任意时间算法。


<details>
  <summary>Details</summary>
Motivation: Rashomon效应描述了多个模型在同一学习任务上达到近似同等性能的现象，这对鲁棒性、特征重要性和可定制性有重要影响。此前决策树Rashomon集的完全枚举依赖于数据的二值化（限制分裂或增加组合复杂度），这可能导致遗漏大量模型和重要特征。因此需要一种能够直接利用连续特征有序结构的精确枚举方法。

Method: 提出首个精确枚举决策树Rashomon集的算法，直接利用连续特征的排序结构；开发了用于近似枚举的松弛方法；设计了一种任意时间算法，能逐步精化候选阈值集合，产生越来越详细的近似结果，并收敛到连续特征Rashomon集。

Result: 实验表明，粗略的二值化可能遗漏大量决策树、重要特征和预测多重性；所提算法相比现有枚举方法实现了数量级的加速，近似方法在保持接近完美召回率的同时提供进一步加速。

Conclusion: 通过避免二值化带来的信息损失和复杂度增加，所提出的算法能够更完整、更高效地枚举决策树Rashomon集，为后续的鲁棒性分析、特征重要性评估和模型定制化提供更可靠的支撑。
...
### [15] [Achieving First-Order Statistical Improvements in Data-Driven Optimization: From No-Free-Lunch to Amplified Decision Perturbation](https://arxiv.org/abs/2608.04312)
*Henry Lam,Tianyu Wang*

Main category: stat.ML

TL;DR: 本文系统剖析了数据驱动优化方法（统称为"EO+"框架），证明在没有正确指定的侧信息时，任何EO+方法最多只能带来二阶改进（"没有免费午餐"）；而当利用几何有效的侧信息时，通过选择比文献建议更大的超参数，可以实现一阶改进，并提出了基于超额风险估计（通过系统知识或bootstrap重采样）的方法来最大化一阶增益。


<details>
  <summary>Details</summary>
Motivation: 尽管大量数据驱动优化方法（如正则化、分布鲁棒优化、迁移学习和上下文优化）被提出，但它们的统计收益往往不清晰，且仅基于个案分析。作者希望通过统一的"方向扰动经验优化"（EO+）框架对这些方法进行系统性剖析，揭示其统计性能的边界和条件。

Method: 作者提出了"方向扰动经验优化"（EO+）的统一框架，涵盖正则化、分布鲁棒优化、迁移学习和上下文优化等方法。理论分析分为两面：一方面证明没有侧信息时EO+方法最多只有二阶改进；另一方面构建基于超额风险估计（利用系统知识或bootstrap重采样）的方法论，通过选择更大的超参数实现一阶改进，并将该增益与控制变量原理（Monte Carlo方差缩减技术）联系起来。

Result: 证明了在没有正确指定的额外侧信息下，EO+方法至多只能实现二阶改进（负面结论）；当利用几何有效的侧信息时，通过选择显著大于文献通常建议的超参数值，可以实现一阶改进；提出了基于超额风险估计的方法论用于最大化一阶增益，并通过控制变量原理解释了为什么几何有效的侧信息是必要的。

Conclusion: 本文为数据驱动优化方法提供了统一的统计理论框架，明确了其统计能力的上界。核心结论是：EO+方法本身不会带来"免费午餐"，但结合几何有效的侧信息并选择适当的超参数，可以获得实质性的统计改进。这一发现对实践中的超参数选择和数据使用策略具有重要指导意义。
...
### [16] [Equitable System-Prompt Selection via Constrained Mixed-Strategy GroupDRO](https://arxiv.org/abs/2608.04339)
*Mengyu Xu,Qiaoxin Yang,Zhihan Liu,Ruiyao Xu,Zachary Liu,Kezhen Chen,Chongyang Gao*

Main category: stat.ML

TL;DR: 本文提出了一种约束混合策略GroupDRO框架，用于从现有系统提示池中选择系统提示，以最小化最差情况下的信息质量损失，而非仅优化平均质量。实验表明该方法在五个LLM、两个双语医疗和消费金融基准上，相比无缓解方案平均降低Overall Mean、Worst 25% Mean和Worst指标13.1%、13.2%和13.7%，同时保持整体质量与平均值选择方法接近。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在信息检索中，语义等价但措辞不同的问题可能获得质量差异显著的答案。系统提示广泛用于引导响应行为，但通常针对平均情况优化，导致某些问题措辞仍可能获得不完整或低质量的答案，缺乏对最坏情况质量的保障。

Method: 构建约束混合策略GroupDRO框架：不直接优化系统提示文本，而是对现有系统提示池分配权重，在约束平均损失接近平均选择方法的同时，最小化跨评估指标和组的最坏情况信息质量损失。池生成与选择解耦，适用于任何系统提示池，并支持使用互补系统提示的集成而非单一提示。

Result: 在五个LLM上的两个双语（医疗和消费金融）基准中，该约束方法相比无缓解方案，在Overall Mean、Worst 25% Mean和Worst指标上平均分别降低13.1%、13.2%和13.7%，同时保持整体质量接近Average选择方法。多提示权重揭示了指标-组对之间的互补性。

Conclusion: 约束混合策略GroupDRO有效缓解了系统提示选择中的公平性问题，在不牺牲整体平均质量的前提下大幅改善最坏情况下的信息质量，且方法具有通用性，适用于任何系统提示池。
...
TL;DR: 本文提出GEDS（图增强描述符排序）算法，基于列置换问题原理对表格特征进行结构化排序，并集成到顺序感知高效Transformer框架（iStructTab）中，通过专有损失函数使记忆token遵循特征排序，从而减少特征分散、提升多模态学习的预测性能和鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 图像与表格数据的多模态学习常受无效特征表示困扰，导致特征冗余、分散和泛化问题，需要更有效的特征结构化和排序方法来改善多模态学习效果。

Method: 提出GEDS算法，基于列置换问题的原理，通过相似性图计算优化特征的统计描述符，系统性地确定有效的特征排序序列；将GEDS嵌入到顺序感知的高效Transformer框架中，利用顺序感知的记忆token并通过专有损失函数显式约束其遵循推导出的特征排序。

Result: 在多模态基准测试上的实验表明，iStructTab有效最小化特征分散，改善了预测性能和鲁棒性，验证了结构化特征排序在多模态学习中的重要性。

Conclusion: 结构化特征排序能显著提升多模态学习效果，GEDS算法和iStructTab框架为该方向提供了有效解决方案。
...
### [18] [Non-asymptotic implicit bias of logistic regression at early-stage gradient descent dynamics](https://arxiv.org/abs/2608.04382)
*Han Bao*

Main category: stat.ML

TL;DR: 本文研究梯度下降中参数向量与最大间隔方向之间的早期弱对齐现象，证明了在O(exp(exp(-δ)))次迭代内即可实现弱对齐（δ为允许的对齐误差），且该迭代界是紧的。


<details>
  <summary>Details</summary>
Motivation: 梯度下降在现代机器学习中不仅用于优化，其隐式偏置（如最大间隔偏置）对泛化至关重要。虽然已知参数向量渐近收敛到最大间隔方向，但渐近收敛速度远慢于纯凸优化。然而经验观察表明，在远少于渐近收敛所需迭代次数内，参数向量就已与最大间隔方向呈正相关。本文旨在理解这一早期对齐现象的机制。

Method: 通过追踪径向流（radial flow）和切向流（tangential flow），直接在数据集几何上操作对齐动力学，避免了渐近展开分析。这种方法使得建立更快的弱对齐结果成为可能。

Result: 理论证明参数向量在O(exp(exp(-δ)))次迭代内与最大间隔方向达到弱对齐，其中δ>0为允许的对齐误差，并证明了该迭代上界是紧的（tight）。

Conclusion: 首次从理论上解释了梯度下降早期阶段参数向量与最大间隔方向的正向对齐现象（尽管此时并未达到渐近最大间隔收敛），揭示了隐式偏置在早期迭代中就已发挥作用的机制，为"train longer, generalize better"这一经验现象提供了理论支撑。
...
### [19] [Incremental Aggregation on the Grassmannian for Asynchronous Eigenspace Computation](https://arxiv.org/abs/2608.04406)
*Xiaolu Wang,Jiang Hu,Hoi-To Wai*

Main category: stat.ML

TL;DR: 本文提出一种Grassmann流形上用于异质分布式系统中有限和特征空间计算（如PCA）的增量聚合异步优化方法，通过极坐标外推更新保持子空间几何，并建立了基于角度依赖的梯度支配性与盆地不变性的两阶段线性收敛理论。


<details>
  <summary>Details</summary>
Motivation: 异步特征空间计算的理论基础稀缺，现有方法对Grassmann流形上陈旧信息动态的覆盖有限，缺乏在异构分布式环境下高效、理论上可保证的异步方法。

Method: 提出Grassmannian增量聚合方法：仅刷新到达的组件并复用缓存梯度，避免全局同步，降低每次更新开销；采用外部极坐标更新（polar update）保持内蕴子空间几何，无需对陈旧切向量进行平行移动。理论上建立了目标函数的紧致角度依赖梯度支配性刻画，以及陈旧聚合更新的盆地不变性，进而得到两阶段线性收敛分析（宽盆地区域+局部锐利区域），收敛常数由组件谱展控制。

Result: 在串行和分布式PCA实验中，该方法在样本效率和墙钟收敛速度上均优于代表性基线方法；理论上证明了两个阶段的线性收敛速率。

Conclusion: 该工作为异步子空间/特征空间计算提供了坚实的理论支撑，证明了在先宽后锐的两阶段框架下异步增量聚合方法的收敛性与实用性，为异构分布式环境下的大规模PCA和特征空间计算提供了新方案。
...
### [20] [An adaptive split-combine Gaussian mixture filter for nonlinear and multimodal state estimation](https://arxiv.org/abs/2608.04430)
*San Kim,Won Chang,Daniel B. Forger,Dae Wook Kim*

Main category: stat.ML

TL;DR: 本文提出了一种自适应分裂-合并高斯混合滤波器（AMF），通过自适应地分裂和合并高斯粒子来估计非线性和混沌系统中非对称、多模态的概率密度函数演化，无需辅助在线数值优化，并在多个基准测试中优于各种基线滤波器，同时支持并行实现以降低计算成本。


<details>
  <summary>Details</summary>
Motivation: 非线性系统（特别是振荡或混沌动力学）中的状态PDF常呈现高度非对称甚至多模态特征，这违反了卡尔曼型滤波器基于单一高斯的假设。虽然高斯混合滤波已被提出，但精确传播混合分量以及自适应调整其数量和权重仍是未解决的开放挑战。

Method: AMF通过自适应地分裂和合并高斯粒子来估计不对称和多模态PDF的时间演化，无需辅助在线数值优化。该方法的分裂机制保证沿目标水平集点方向的方差减少，从而实现粒子的准确高效传播。此外还提出了并行实现方案。

Result: 在多个基准测试中（包括单个和耦合的快慢Van der Pol振荡器以及Lorenz吸引子），AMF一致优于各种基线滤波器。并行实现可在实际计算成本下实现高保真PDF估计。

Conclusion: AMF通过自适应分裂-合并策略有效解决了高斯混合滤波中分量传播和数量/权重自适应调整的挑战，为非线性系统的非高斯PDF估计提供了一种准确且高效的新方法。
...
### [21] [Discretization and Statistical Consistency of Functional Flow Matching](https://arxiv.org/abs/2608.04531)
*Lennon J. Shikhman*

Main category: stat.ML

TL;DR: 本文研究函数分布上的函数流匹配（functional flow matching）方法，证明在散乱或自适应细化条件下，有限条件速度目标即使在σ-代数不嵌套的情况下仍可强L²收敛到传感器极限，并给出端到端Wasserstein界及显式收敛率。


<details>
  <summary>Details</summary>
Motivation: 函数流匹配方法在定义时面向函数分布，但实际实现仅基于有限系数或点值。在散乱或自适应细化下，条件σ-代数不必嵌套，因此不能依赖鞅收敛来证明传感器极限。作者旨在建立无需嵌套条件的强收敛理论，并处理实际学习中端到端的误差界。

Method: 通过强一致有限秩重构序列证明有限条件速度目标的强L²收敛；对正交投影给出定量界，并通过正则性空间扩展点传感器；将学习流直接耦合到总体叠加路径，在不假设总体有限维ODE唯一性的情况下获得端到端Wasserstein界；使用空间正则性-立方体（cubature）证书封闭算子实现项，用Bernstein论证得到过采样风险项，通过可精确实现的截断高斯缩放特例得到显式端到端速率。

Result: 证明了强有限秩重构序列下有限条件速度目标强L²收敛；对归一化求积神经算子验证了与传感器无关的常数（含全局Lipschitz激活的显式幅度递推）；给出了非交换迹类高斯例子：投影约束下边界乘子为0，精确条件下为0.72；获得固定模型维数的过采样风险项及截断高斯特例的显式端到端收敛率。

Conclusion: 该工作建立了函数流匹配在非嵌套细化下的收敛理论，克服了鞅收敛限制，提供了定量误差界、与传感器无关的常数保证和显式的端到端速率，为函数分布上的流匹配方法提供了坚实的理论基础。
...
### [22] [An entropic explanation of insistence on sameness in autism](https://arxiv.org/abs/2608.04616)
*Przemysław Śliwiński*

Main category: stat.ML

TL;DR: 本文提出一个基于信息论的框架，将自闭症中的"坚持同一性"解释为个体试图减少惊讶和不确定性的普遍行为模式，并通过熵度量D_H(R,M)=H(R|M)+H(M|R)的形式化分析，得出坚持同一性是限制环境刺激以匹配已知记忆的结论。该框架可量化多种自闭症相关概念，指导学习疗法和护理方案，并可通过类图灵测试方法验证。


<details>
  <summary>Details</summary>
Motivation: 自闭症中的"坚持同一性"（insistence on sameness）缺乏统一的理论解释框架。作者试图用信息论提供一个正式、可量化且可验证的认知模型，以理解这种行为模式背后的认知机制，并为治疗和日常护理提供设计基础。

Method: 提出一个信息论框架，使用条件熵定义惊讶（surprise）和不确定性（uncertainty），构建熵度量D_H(R,M)=H(R|M)+H(M|R)来描述外部随机刺激序列R与内部记忆M之间的关系。通过分析最小化该度量的两种途径（学习R或限制R于已知的M），推导出坚持同一性源于后者的结论，并提出类图灵测试验证方案。

Result: 该框架能够：(1)量化惊讶、不确定性、感觉过载和剥夺、焦虑、舒适区、失望、迷失方向、迂腐、僵化、恪守或异常精确等概念；(2)生成学习疗法和日常护理的指南，可形式化为优化算法并实现为机器人护理程序；(3)通过无需真人参与实验的图灵测试式方法进行验证。

Conclusion: 该框架若获积极验证，将为改善自闭症患者基本日常生活自理能力的疗法提供正式基础和设计指南。它将自闭症重新定义为认知功能局限于对环境有形属性的辨别、记忆和预测的能力障碍，并为坚持同一性提供了信息论层面的合理解释。
...
### [23] [Personalized Federated Sparse Adaptation of Time-Series Foundation Models](https://arxiv.org/abs/2608.04695)
*Priyanka Nihalchandani,Naman Srivastava,Varun Ojha,Pandarasamy Arjunan*

Main category: stat.ML

TL;DR: 本文提出一个个性化联邦稀疏自适应框架，在预训练时间序列基础模型（TSFM）后放置异构时间混合专家（MoE）适配器，通过序列级路由器将每个168小时上下文窗口映射到具有周期性、长程交互、局部变化、趋势残差和多分辨率行为特征的top-k专家子集。在50栋建筑和三个TSFM骨干上，个性化方法始终优于全局FL和局部MoE变体，且最佳稀疏自适应策略因骨干和指标而异。


<details>
  <summary>Details</summary>
Motivation: 建筑能耗预测中，仪表数据是私有、分布式且高度非IID的，联邦适配TSFM具有很强的吸引力。然而统一的参数共享策略无法适配所有预训练TSFM或建筑客户端：完全共享适配器会抑制建筑特定的时间行为，而完全局部适配则丢弃了跨建筑的迁移信息。因此需要一种既能保持个性化又能利用跨建筑知识的方法。

Method: 提出个性化联邦稀疏自适应框架，核心是在预训练TSFM表示之后放置一个异构时间混合专家（MoE）适配器。使用序列级路由器将每个168小时上下文窗口映射到top-k个专家子集，这些专家分别专门处理周期性、长程交互、局部变化、趋势残差结构和多分辨率行为。比较了全局FL、局部训练和个性化FL变体，并探索了全局共享或客户端私有的专家库两种配置。

Result: 在50栋建筑和三个TSFM骨干上的实验表明：个性化方法在性能上一致优于Global FL-MoE和Local MoE；最佳稀疏自适应策略因骨干和评估指标而异；路由行为分析揭示了客户端级别的专家专业化、专家集中度以及跨骨干的近均匀路由模式。

Conclusion: 联邦TSFM适配应当同时考虑客户端感知和骨干感知。单一的策略无法解决所有场景，个性化的稀疏适配结合异构MoE设计是建筑能耗预测中联邦TSFM适配的有效方案。
...
### [24] [Nonparametric Goodness-of-fit Testing under Covariate Shift](https://arxiv.org/abs/2608.04860)
*Zhen Hou,Dong Xia*

Main category: stat.ML

TL;DR: 本文提出了协变量偏移下非参数拟合优度检验的程序，结合截断重要性加权核岭回归与乘子自助法构建回归函数的置信集，并在有界矩或次指数尾部条件下证明其非渐近有效性和锐度。


<details>
  <summary>Details</summary>
Motivation: 在实际应用中，训练数据（源总体）与评估数据（目标总体）的分布往往不一致（即协变量偏移）。传统的拟合优度检验在协变量偏移下失效，因为标签数据来自源总体而检验需针对目标总体进行。现有的重要性加权方法在密度比具有重尾时不稳定，需要发展稳健的检验程序。

Method: 该方法将截断的重要性加权核岭回归与乘子自助法相结合。通过对目标-源密度比施加有界矩条件或次指数尾部条件来量化分布不匹配；截断步骤用于稳定重要性加权核岭回归及其自助校准，使方法在密度比重尾时依然适用。理论分析在算子相容性条件下证明置信集的非渐近有效性与锐度。

Result: 证明了所得置信集的非渐近有效性和锐度，并在目标-源密度比的具体条件及核积分算子谱衰减条件下建立了覆盖概率的显式误差率。数值实验验证了理论发现。

Conclusion: 该工作为协变量偏移下的非参数拟合优度检验提供了理论上严格且计算上可行的工具，尤其适用于密度比重尾的场景，扩展了传统核岭回归方法在分布偏移下的适用范围。
...
### [25] [Stochastic Emulation using Generalized Stratified Sampling for Performance-Based Risk Optimization of Structures](https://arxiv.org/abs/2608.05006)
*Isabela D. Rodrigues,Seymour M. J. Spence,Henrique M. Kroetz,André T. Beck*

Main category: stat.ML

TL;DR: 本文提出了一种将广义分层采样（GSS）与随机多项式混沌展开（SPCE）相结合的框架（GSS-SPCE），用于基于性能的风险优化（PBRO）中的结构可靠性分析。该框架通过按灾害强度对输入空间分层，并在每一层内独立训练SPCE元模型，从而改善了结构响应分布尾部的极端响应表示，减少了非线性模型评估次数。


<details>
  <summary>Details</summary>
Motivation: SPCE虽然不需要在固定输入条件下对非线性分析进行重复采样，但在准确表示结构响应分布尾部（即极端响应）方面存在局限。为解决这一不足，需要一种改进策略来提升对尾部概率（如超越概率）的估计精度，同时保持PBRO中的计算效率。

Method: 将广义分层采样（GSS）与SPCE结合：GSS方案根据灾害强度将输入空间划分为多个层（strata），在每个层内独立训练SPCE元模型以更好地捕捉极端响应；各层的条件超越概率通过全概率定理重组，以评估概率约束。将GSS-SPCE应用于两层钢结构建筑中屈曲约束支撑（BRB）横截面积的优化设计。

Result: 提出的GSS-SPCE框架能够准确估计结构响应分布（包括其尾部区域），同时显著减少了PBRO所需的非线性模型评估次数。

Conclusion: GSS-SPCE框架有效克服了SPCE在尾部响应表征方面的局限性，在保证精度的情况下大幅降低了计算成本，为PBRO提供了一种高效且准确的可靠性分析与优化方法。
...
### [26] [Algorithm-Driven SVARs: Navigating the Wilderness of Big Data](https://arxiv.org/abs/2608.05017)
*Yucheng Yang,Tao Zha*

Main category: stat.ML

TL;DR: 本文提出一种贝叶斯方法，系统性地在SVAR（结构向量自回归）框架中筛选信息集（变量选择），并在给定识别约束下自动保留能被样本外准则接受的最大系统。实证发现：递归识别下，产出对住房生产而非单纯家庭信贷更敏感；货币政策方面，无锚定联合贝叶斯代理SVAR配合多工具强化了信用利差渠道；核心系统扩展企业利差后，预期违约风险成为重要的传导渠道。


<details>
  <summary>Details</summary>
Motivation: 现有SVAR文献对识别冲击所需的约束条件（第一个选择）已有大量研究，但第二个选择——变量集合（信息集）——往往由研究者手工决定，缺乏系统化的统计准则来指导。这导致结果可能依赖主观变量选择，缺乏严谨性和可复现性。

Method: 开发一种贝叶斯方法论：(1) 构造候选的信息集；(2) 使用样本外（out-of-sample）准则作为评价标准；(3) 自动保留满足准则的最大系统（即允许尽可能多的变量进入模型）。同时提出一种无锚定（anchor-free）的联合贝叶斯代理SVAR方法，可整合多个外部工具变量。

Result: 在递归识别下，发现产出增长与住房生产（housing production）的关联强于仅与家庭信贷的关联；在货币政策分析中，采用多工具的联合贝叶斯代理SVAR强化了信用利差渠道的估计证据；当核心系统中加入筛选出的企业信用利差变量时，预期违约风险被识别为强有力的货币政策传导渠道。

Conclusion: SVAR的变量选择不应依赖手工设定，而应通过系统的贝叶斯样本外准则来自动化确定。该框架同时提升了递归识别与代理SVAR两类识别策略下结论的可靠性，并揭示出住房部门、信用利差与违约风险在宏观经济传导中的重要作用。
...
### [27] [Canonical Joint Energy-Based Model on CIFAR-10: failure modes and practical indistinguishability of Predictor-Corrector and SGLD samplers](https://arxiv.org/abs/2608.05025)
*Dmytro Knopov*

Main category: stat.ML

TL;DR: 本文系统复现了联合能量模型（JEM）在WideResNet-28-10上的训练，并测试了Predictor-Corrector（PC）采样器替代SGLD是否保留理论优势。结果显示PC在所有协议（生成FID、OOD检测AUROC、训练动态）上与SGLD无方法层面的显著差异，且两种方法均存在晚期训练发散和run依赖的OOD判别动态两种失败模式。理论上这种不可区分性是预期的：在固定噪声下PC的预测步退化合其保证无法迁移到规范化JEM。


<details>
  <summary>Details</summary>
Motivation: JEM统一了分类与生成并支持OOD检测，其标准训练依赖SGLD；而理论上更有依据的PC采样器此前未在规范化模型上进行系统复现测试。作者旨在检验PC在真实规范化JEM设置中是否保持理论优势，并复现规范JEM的可靠性。

Method: 采用规范JEM在WideResNet-28-10（无归一化层）上独立运行两次，设计三类协议测试PC：1）PC替换SGLD贯穿约130个训练epoch；2）冷启动生成（FID评估）；3）精炼式多OOD检测（AUROC评估）。使用层级种子级bootstrap和等价检验进行统计分析。

Result: 重建达到92.88%测试准确率和buffer-FID 44.46（规范值：92.9%和38.40）。PC与SGLD在所有协议上无方法级优势：推理时AUROC绝对差低于0.007（十个checkpoint-OOD对），FID差低于0.5；训练协议上AUROC差的95%置信区间包含零。数据同时与等价性和小的方向效应一致。两种方法均出现相同的两种失败模式。

Conclusion: PC在固定噪声下的规范化JEM中理论上预测步退化，其理论保证不迁移到规范JEM，故PC与SGLD的实际不可区分是预期的理论结果。此外，规范JEM的训练可靠性存在隐患（晚期发散），需要关注。
...
### [28] [Representational separation between unitary and channel quantum generative models via shared classical randomness at shallow depth](https://arxiv.org/abs/2608.05110)
*Arunava Majumder,Marius Krumm,Hendrik Poulsen Nautrup,Hans J. Briegel*

Main category: stat.ML

TL;DR: 本文证明在浅层量子生成模型中,共享经典随机性这一相对较弱的资源足以在固定浅层深度下实现与纯酉Born模型的严格可扩展表示分离,即基于测量驱动的量子计算(MBQC)可以利用共享经典随机比特生成纯浅层酉模型无法复现的长程关联分布。


<details>
  <summary>Details</summary>
Motivation: 近期量子硬件限制了电路深度和几何局域连接性,使得纯浅层酉Born模型只能访问受限的输出分布。虽然已有研究表明在随机性引入到酉Born模型中可以提升经验生成性能,并在小规模架构上证明了严格表示分离,但能否在固定浅层深度下对任意大规模系统提供可证明的分离仍然是一个悬而未决的问题。

Method: 作者将有界连通性的浅层酉电路(后接计算基测量)与空间分离的局域Pauli操作相结合,这些操作的联合应用由单一经典采样的随机比特控制。由此得到的浅层通道模型能够生成经典输出分布中的长程关联,而任何纯酉浅层模型在有界连通性下无法复现。同时利用MBQC通过随机测量结果的适应性调整来自然实现所需的共享经典随机性。

Result: 对于一维最近邻架构,纯酉模型在最坏情况下需要深度Ω(N)才能复现此类分布,这证明了共享经典随机性在固定浅层深度下可实现与纯酉Born模型的严格可扩展表示分离。基于MBQC的生成模型数值实验支持了分析结果。

Conclusion: 共享经典随机性尽管在纠缠理论中是较弱的资源,却足以在任意大规模系统中实现严格的浅层表示分离,且MBQC为此类随机性提供了自然的实现途径,这对近端量子硬件上的生成式量子建模设计具有重要的理论和实践意义。
...
### [29] [SSTQ:Privacy-Preserving Vector Quantization via Subsampled Stochastic TurboQuant](https://arxiv.org/abs/2608.05127)
*Adel Javanmard,David P. Woodruff,Vahab Mirrokni*

Main category: stat.ML

TL;DR: 本文提出SSTQ框架，用于在分布式优化中实现局部差分隐私的同时保持低通信成本，通过过完备等范数紧框架、坐标子采样和隐私感知一维量化的结合，在仅需⌈log₂N⌉+b位每客户端的条件下达到最优均方误差缩放，并通过代理隐私感知码本目标将MSE缩放从O(4^b)降至O(2^b)。


<details>
  <summary>Details</summary>
Motivation: 在分布式优化中同时实现局部差分隐私和低通信成本仍然具有挑战性。现有向量量化方法（如vqSGD）依赖高维几何构造，但会产生不利的维度相关方差，需要一种能兼顾隐私、通信效率和量化精度的方法。

Method: SSTQ框架包含两种变体：Flat Randomized Response版本和Metric-Aware Laplace版本。核心思路结合了过完备等范数紧框架、坐标子采样和隐私感知的一维量化；此外推导了一个代理隐私感知码本目标函数，将码本相关的MSE缩放从O(4^b)降至O(2^b)。

Result: SSTQ在仅使用每客户端⌈log₂N⌉+b位的情况下达到最优均方误差缩放（N=Θ(d)为帧大小）；在CIFAR-10和Fashion-MNIST上的联邦学习任务中，与既有基线相比表现出更优的效用和通信效率。

Conclusion: SSTQ通过简单的1D量化框架有效解决了分布式优化中局部差分隐私与通信效率之间的矛盾，同时显著降低了码本相关的MSE缩放复杂度，为高维场景下的隐私保护联邦学习提供了高效解决方案。
...
### [30] [Breakdown of Monotonic Impurity Entropy Flow in $\mathscr{PT}$-Symmetric Multichannel Kondo Systems](https://arxiv.org/abs/2608.04083)
*Pradip Kattel,Abay Zhakenov,Natan Andrei*

Main category: cond-mat.str-el

TL;DR: 本文研究了一类具有PT对称性的非厄米多通道近藤模型，通过精确Bethe Ansatz求解，揭示了四种杂质相（过屏蔽近藤、零模、Yu-Shiba-Rusinov、局域磁矩），证明RG不可逆性（广义Affleck-Ludwig g定理）在近藤相中成立，但在零模和局域磁矩相中，实谱和缺陷熵并不保证RG不可逆性。


<details>
  <summary>Details</summary>
Motivation: 探索非厄米系统与强关联多体物理的交叉领域，研究PT对称非厄米多通道近藤模型的精确性质，特别是RG不可逆性与缺陷CFT的关系是否在非厄米框架下仍然成立。

Method: 构建由一对自旋1/2杂质通过复共轭近藤耦合连接n个导电子通道的PT对称非厄米模型，利用精确Bethe Ansatz求解，引入非厄米参数α，并采用广义热力学Bethe Ansatz计算杂质自由能和Affleck-Ludwig g函数。

Result: 发现四种杂质相及其PT对称性性质；Kondo相中杂质熵从2ln2流向2ln[2cos(π/(n+2))]，与缺陷CFT一致；零模相中零能杂质弦重构谱为多个激发塔；局域磁矩相中RG流变成循环的；并揭示了零模和局域磁矩相中杂质熵的非单调性，表明实谱不一定意味着RG不可逆性。

Conclusion: 通过精确求解证明了广义Affleck-Ludwig g定理在Kondo相成立，但实谱与缺陷CFT一致的缺陷熵并不能保证RG不可逆性——这一发现深化了对非厄米强关联系统中RG流结构的基本理解。
...
### [31] [Magnetic-Field-Driven Dimensional Reduction in a Quantum Antiferromagnet](https://arxiv.org/abs/2608.04096)
*Subhankar Khatua,Marcin Raczkowski,Jeroen van den Brink,Fakher F. Assaad*

Main category: cond-mat.str-el

TL;DR: 该论文提出了一种新的维度降低机制：在由交替铁磁（FM）和反铁磁（AFM）自旋-1/2链耦合构成的二维系统中，施加磁场可使FM链极化变硬，抑制横向AFM涨落在链间的传播，从而在低能下有效实现一维行为。通过量子蒙特卡洛动力学模拟，在描述botallackite（Cu₂(OH)₃Br）的模型中发现，超过临界磁场后，低能谱退化为具有场依赖不相称双自旋子涨落的一维AFM海森堡自旋-1/2链，为非弹性中子散射提供了清晰的特征信号。


<details>
  <summary>Details</summary>
Motivation: 探索低维系统中量子涨落增强所引发的新奇量子物质态，特别是寻找除强各向异性耦合之外的新机制来实现维度降低，从而在二维材料中观察到一维量子行为。

Method: 以包含交替铁磁和反铁磁自旋-1/2链的二维模型为基础，通过施加外磁场驱动维度降低；采用量子蒙特卡洛（QMC）动力学方法对botallackite（Cu₂(OH)₃Br）模型进行数值模拟。

Result: 超过临界磁场后，FM链完全极化变硬，导致横向AFM涨落无法在链间传播，低能谱退化为等效一维AFM海森堡自旋-1/2链的谱，并表现出随磁场变化的不相称双自旋子（two-spinon）涨落特征，可被非弹性中子散射实验观测。

Conclusion: 磁场可以作为一种有效手段，在二维弱耦合体系中诱导维度降低，产生有效的一维量子磁性行为，为实验上探索这类新奇量子态提供了可验证的理论预测。
...
TL;DR: 本文研究一形式对称性在可缩对称扇区上的吉布斯态投影，在三维簇模型插值中发现对称保护拓扑相与投影顺磁相均与热无序截然不同，并用通量扭转膜不变量区分三类相。


<details>
  <summary>Details</summary>
Motivation: 理解一形式对称性的约束投影如何影响有限温度下的量子态，并区分对称保护拓扑相、投影顺磁相与热无序这三类热平衡态。

Method: 通过在三维簇模型中引入吉布斯态到对称扇区的投影，构造通量扭转膜不变量作为序参量，利用量子上限蒙特卡洛模拟进行张力诊断与有限尺寸标度分析。

Result: 通量扭转膜不变量能区分三类相，取值分别为-1、+1、0；在端点、自对偶、零温和无限温线上取精确值，其余区域通过正的空间片张力、缠绕线张力和界面张力保证量子化，量子蒙特卡洛支持该量子化结论。

Conclusion: 一形式对称性投影下的吉布斯态能稳定具有非平凡膜不变量的对称保护拓扑相和投影顺磁相，二者均与热无序有本质区别，提供了一套新的拓扑序诊断工具。
...
TL;DR: 本文提出利用分数量子霍尔系统中基态零点的位移比来量化和可视化真实态与模型波函数之间的距离，该位移比分布可作为从Laughlin相到拓扑平凡相转变的序参量，并发现ν=1/5基态在长程库仑相互作用下可被复合费米子流体描述所精确解释。


<details>
  <summary>Details</summary>
Motivation: 分数量子霍尔系统中基态零点的聚类模式是其拓扑性质的定义性特征，但如何将真实物理态与理想模型波函数进行定量比较一直存在困难，尤其是在短距离下液体序与晶体序的竞争难以调和的长期问题。

Method: 分析基态中零点围绕单个电子的几何涨落，定义零点的位移比作为量化指标，通过统计分析该位移比的分布来度量真实态到模型波函数的距离，并将此方法应用于不同Jain序列的量子霍尔态的统计比较。

Result: 零点位移比分布表现出类似序参量的行为，可清晰区分Laughlin相与拓扑平凡相；对于ν=1/5基态，复合费米子流体描述在长程库仑相互作用下与数值结果几乎完美吻合，仅需3-5个电子即可实现。

Conclusion: 零点位移比是一种有效的诊断工具，可用于可视化量子霍尔态的拓扑性质及量化其与理想波函数的偏离，并成功解决了短距离下液体与晶体序竞争这一长期难题。
...
TL;DR: 本文通过含时密度矩阵重整化群方法研究受挫锯齿形自旋链中自旋向列液体的自旋输运性质，发现饱和时动量k=π的磁振子对波包保持局域化，原因是反铁磁四极准长程有序导致的能隙零色散结构具有平坦特征，即传播速度为零。


<details>
  <summary>Details</summary>
Motivation: 研究自旋向列液体在磁场的自旋输运特性，特别是利用周期性边界条件避免开放边界条件中边缘诱导的磁化结构，探索饱和状态下磁振子对波包的传播行为。

Method: 采用含时密度矩阵重整化群方法，从波包时间演化的视角，在周期性边界条件下研究受挫锯齿形自旋链中自旋向列液体的自旋输运性质。

Result: 发现在饱和时，动量k=π的磁振子对波包保持局域化，因为反铁磁四极准长程有序产生的无间隙色散具有相当平坦的结构，导致传播速度为零。

Conclusion: 自旋向列液体的饱和状态下，特定动量的磁振子对波包传播速度为零，揭示了该系统独特的自旋输运机制。
...
TL;DR: 本文通过μSR实验研究了候选三维量子自旋液体材料PbCuTe₂O₆在[110]轴单轴压缩下的磁响应。发现仅需约10.8 MPa的临界应力即可显著增强弛豫率并加宽内部场分布，同时伴随局部晶体对称性破缺，但未出现传统静态长程磁有序，表明压缩驱动系统进入一个具有增强准静态关联和持续缓慢自旋动力学的强关联态。


<details>
  <summary>Details</summary>
Motivation: 量子自旋液体(QSL)是凝聚态物理的重要研究方向，而三维QSL候选材料尤为稀少。如何在保持无磁序的前提下调节磁性交换作用、探索隐秘的磁不稳定性，是理解QSL物理的关键挑战。该工作旨在探索通过单轴压缩这一对称选择性扰动来调控三维QSL候选材料交换作用景观的可能性。

Method: 采用原位[110]方向单轴压缩(最高至37.7 MPa)结合μSR(μ子自旋弛豫/旋转)测量技术，研究S=1/2的3D QSL候选材料PbCuTe₂O₆在压缩下的局部磁响应、内场分布及自旋动力学变化。

Result: 发现微小方向性晶格扰动即可显著改变局部磁响应；超过临界应力σ_cr~10.8 MPa后，弛豫率大幅增强且内场分布显著展宽，伴随局部晶体对称性破缺。未观测到传统静态长程磁有序，但压缩驱动系统进入增强准静态相关与持续缓慢自旋动力学共存的强关联态。

Conclusion: 该工作展示了一种干净且对称选择性的途径来调控受挫交换作用景观、触及三维QSL候选材料中隐藏的磁不稳定性，为其他磁-晶格耦合相关的关联体系提供了可推广的调控思路。
...
TL;DR: 本文提出了一种推广的ghost-RISB方法，将局域声子模耦合到任意电子自由度，实现电子-电子和电子-声子相互作用的非微扰变分处理，并在Hubbard-Holstein模型上与DMFT取得了定量一致的结果，且计算成本大幅降低。


<details>
  <summary>Details</summary>
Motivation: 现有的静态slave-boson方法无法捕捉声子引起的动力学自能效应和重整化；而DMFT虽然精确但计算成本高昂，难以快速探索强耦合电子-声子相图。因此需要一种既高效又能非微扰处理电子-声子相互作用的变分方法。

Method: 将ghost-orbital构造推广到包含局域声子模的情形，扩展ghost-RISB框架，使其能够捕捉动态自能效应和声子诱导的重整化。方法基于静态观测量而非动力学观测量进行自洽，从而大幅降低计算成本。

Result: 在Hubbard-Holstein模型上，该方法与DMFT结果在电子准粒子权重和声子性质上定量一致，涵盖广泛的耦合强度范围，并准确捕捉电子-电子与电子-声子相互作用的竞争。发现ghost轨道对于准确定义低频强动力学声子区域至关重要。在强耦合绝热声子区域观察到超导序参量的抑制，将其解释为双极化子区域中空位与双占据位点声子波函数之间Franck-Condon型重叠的减少。

Conclusion: 扩展的ghost-RISB方法为研究关联电子-声子系统提供了一种高效且准确的理论工具，以远低于DMFT的计算成本实现相当精度，特别适合快速探索强耦合和绝热声子等极端参数区域的相图。
...
TL;DR: 本文对高度各向异性的非化学计量比CeCuBi2单晶进行了结构、角度依赖的磁性和磁输运性质系统研究，发现其为各向异性Kondo反铁磁体，具有复杂场致磁性行为和异常磁输运特性，包括14K的反铁磁有序、Kondo重费米子行为、多个场致变磁相、自旋玻璃样行为以及高达22%的室温磁电阻和10.9%的各向异性磁电阻(AMR)。


<details>
  <summary>Details</summary>
Motivation: 探索Kondo关联、磁各向异性和场可调自旋构型之间的强相互作用，寻找研究关联及各向异性量子现象的理想平台。

Method: 对CeCuBi2单晶进行结构表征、磁化率与比热测量、电输运测量、磁化强度测量、交流磁化率测量以及角度依赖的磁输运测量。

Result: 发现CeCuBi2在TN=14K处发生强各向异性的反铁磁有序并表现出弱重费米子行为；电阻率在47K附近出现宽峰；磁场诱导多个变磁相，中间态表现出自旋玻璃样慢自旋动力学；室温300K、9T下磁电阻约22%，AMR值最高约10.9%。

Conclusion: CeCuBi2展现出Kondo关联、磁各向异性与场可调自旋构型间的强耦合，是探索关联及各向异性量子现象的可能平台。
...
TL;DR: 该研究通过高分辨率角分辨光电子能谱直接追踪TiSe2中电荷密度波（CDW）转变过程中能带边的温度演化，发现从高温正常相到160 K，基本带隙大小保持不变，推翻了此前关于CDW转变伴随带隙开启或增强的普遍预期。结果表明TiSe2并未经历温度驱动的电子带隙开启，其转变是由晶格对称性破缺重构所主导，该重构折叠而非打开了一个预存带绝缘体的电子结构。


<details>
  <summary>Details</summary>
Motivation: TiSe2中电荷密度波（CDW）转变的驱动机理数十年来存在争议，主要观点涵盖激子绝缘体和晶格驱动不稳定性。核心问题在于该转变是否伴随体带隙的开启或增强，此前缺乏直接追踪能带边温度演化的实验证据。

Method: 采用高分辨率角分辨光电子能谱（ARPES）技术，直接测量TiSe2体带边从高温正常相到160 K（低于CDW转变温度TCDW=200 K）的完整温度演化过程。

Result: 研究发现基本带隙的大小在从高温正常相到160 K的温度范围内保持恒定不变。CDW转变虽然引发清晰的能带折叠特征和谱权重重新分布，但底层带极值（band extrema）并未受到扰动。

Conclusion: TiSe2不发生温度驱动的电子带隙开启，其CDW转变并非由电子间隙开启机制（如激子绝缘体）主导。该转变由晶格对称性破缺重构所驱动，这种重构将预存带绝缘体的电子结构折叠而非打开间隙，支持晶格驱动不稳定性机制。
...
TL;DR: 本文展示在可编程量子硬件平台上，非阿贝尔分数量子霍尔（FQH）态竟比阿贝尔态更容易制备：簇聚非阿贝尔态只需与系统尺寸无关的恒定深度（两比特深度为3）的并行量子电路，而阿贝尔Laughlin态需要线性深度的顺序电路。作者提出了一个系统化框架来分类和制备FQH态，在IBM Heron处理器上成功制备了18个家族的FQH态（最多达156量子比特），并通过测量验证了分数量子洞电荷和非阿贝尔e/4准粒子的编织数据。


<details>
  <summary>Details</summary>
Motivation: 非阿贝尔任意子作为分数量子霍尔（FQH）物质中的奇特激发，在传统平台上极难实现。作者希望利用可编程量子硬件来制备和研究这些FQH态，特别是非阿贝尔FQH态，以超越传统平台的局限，建立一条可扩展的研究FQH物理的新途径。

Method: 作者开发了一个系统化框架（cataloging framework）来分类可能的FQH态，并设计对应的量子电路制备方案。关键发现是簇聚非阿贝尔FQH态（如parafermionic Read-Rezayi态）具有与系统尺寸无关的并行制备电路（两比特深度为3），而阿贝尔Laughlin态需要线性深度的顺序电路链。他们在IBM Heron处理器上实现了这些电路，覆盖8到154+量子比特的规模，并通过测量分数量子洞电荷和使用干涉测量扩展来验证编织数据。

Result: 在IBM Heron处理器上成功制备了18个家族的FQH态，扩展到全部156个量子比特。parafermionic Read-Rezayi Z3态的深度仅为3（从8到118量子比特），完整根采样扩展到154量子比特的104电子Read-Rezayi Z4态。测量恢复预期的分数量子洞电荷，簇聚态的电荷估计器在每个对称选择的射击中精确，并通过干涉测量扩展测量了非阿贝尔e/4准粒子的编织数据。

Conclusion: 该工作建立了在量子处理器上可扩展研究FQH物理的路线，开辟了制备和探测非阿贝尔拓扑物质的新途径，其规模远超传统平台所能达到的水平。这一反直觉的结果——更奇异的非阿贝尔FQH激发反而更易制备——为量子模拟和拓扑量子计算研究提供了重要的新方向。
...
### [40] [Valley polarization of moiré interlayer exciton complexes driven by many-body interactions](https://arxiv.org/abs/2608.04151)
*Adrian O. Paulus,Mikołaj J. Metelski,Alain Dijkstra,Kenji Watanabe,Takashi Taniguchi,Nathan P. Wilson,Jonathan J. Finley*

Main category: cond-mat.str-el

TL;DR: 本文研究了R型WSe2/WS2异质双层中局域层间激子的光致发光寿命和谷极化，发现电荷不可压缩相中的谷极化降低源于激子寿命的剧增导致极化被稀释，而非增强的谷间散射；同时在高激发密度下观察到双占据位点（IXX）反常的强谷极化。


<details>
  <summary>Details</summary>
Motivation: 理解moiré过渡金属硫族化合物异质结构中层间激子与多体态的相互作用，特别是激子寿命和谷极化如何受到电荷晶体与激子Mott绝缘体等强关联多体物理所调控。

Method: 在R型WSe2/WS2双层体系中，通过系统改变层间激子密度和电荷密度，测量不同填充下的层间激子光致发光寿命与时间分辨谷极化，并对比时间积分测量结果以区分极化衰减的来源。

Result: 在电荷不可压缩相中，层间激子寿命显著增强，导致时间积分测量的谷极化表观下降，但实际谷间散射并未增强；在高激发密度下，IXX退激过程中出现违反预期的强谷极化。

Conclusion: 层间激子的激子与谷动力学被moiré异质双层中丰富的多体物理强烈修饰，激子寿命增强引起的极化稀释是电荷晶体相中谷极化降低的真正机制，而IXX动力学中涌现的反常谷极化揭示了新的多体谷自由度行为。
...
TL;DR: 本文系统研究了2-群全局对称性中，携带1-形式荷的线缺陷如何显式破坏0-形式对称性，并通过"族反常"（family anomaly）这一拓扑障碍物，揭示了对称缺陷无法存在的普适约束。作者利用Wess-Zumino一致性条件和广义反常流入框架，在连续非阿贝尔、连续阿贝尔和离散三种情形下分别给出了族反常的具体刻画，并通过大量实例计算了倾斜算符（tilt operator）及其高阶推广。


<details>
  <summary>Details</summary>
Motivation: 2-群全局对称性以一种非平凡的方式交织了0-形式和1-形式对称性。理解线缺陷在2-群对称性下的行为对于量子场论和凝聚态物理中的对称性分类至关重要，但目前缺乏对携带1-形式荷的线缺陷如何影响0-形式对称性的系统性普适约束分析。

Method: 作者采用Wess-Zumino一致性条件和广义反常流入形式主义（适用于缺陷和边界）作为主要工具。对于非阿贝尔连续2-群，族反常源于缺陷模空间上的高阶Berry联络；对于连续阿贝尔情形，使用微分上同调处理2-群背景场，推广了Wess-Zumino条件；对于离散情形，则通过对称缺陷的结合性来重新表述。

Result: 证明了携带1-形式荷的线缺陷会普遍破坏0-形式对称性，产生由破缺对称生成元参数化的线缺陷族，该族的存在性在重整化群流下不变。通过大量实例计算了倾斜算符及其高阶类比（用于探测缺陷对破缺对称性的线性响应），并澄清了简单线与非简单线的区别以及Postnikov类的消解等微妙问题。

Conclusion: 2-群对称性对线缺陷施加了普适的对称性破缺约束，由族反常这一拓扑障碍物决定。该框架统一了连续与离散2-群下的对称性约束，揭示了带电线缺陷导致的对称性破缺与体态对称性破缺层级之间的普适联系。
...
### [42] [Critical and multicritical Kasner scaling in holographic phase transitions](https://arxiv.org/abs/2608.04522)
*Ling-Long Gao,Yan Liu,Hong-Da Lyu*

Main category: cond-mat.str-el

TL;DR: 本文研究全息相变如何将其标度律印刻在爱因斯坦-标量黑洞内部的局部Kasner几何上。在固定双迹耦合下，推导了第一时期Kasner指数偏离Schwarzschild值的近临界标度，发现其与边界凝聚态的平方成正比，标度指数由临界性阶数决定。超指数标量势产生一系列由标量场反冲分隔的Kasner时期，后期可连续追踪的时期继承相同的温度指数但系数不同。数值解验证了普通临界和三相临界情形；解析证明了红外不动点的主导非相关指数控制低温Kasner标度。


<details>
  <summary>Details</summary>
Motivation: 全息相变是AdS/CFT对偶中的重要现象，理解其如何影响黑洞内部几何（尤其是Kasner奇点结构）对于揭示强耦合量子场论的量子临界行为与引力内部结构之间的深层联系至关重要。

Method: 采用解析推导与数值验证相结合的方法：(1) 固定双迹耦合条件下推导近临界标度关系；(2) 利用超指数标量势构造多Kasner时期结构；(3) 通过数值求解Einstein-标量方程验证普通临界与三相临界情形的预言；(4) 利用红外不动点重整化群分析推导低温Kasner标度的解析结果。

Result: 第一时期Kasner指数满足p_t^(1)+1/3 ∝ O² ∝ (T_c-T)^(1/r)的标度律，其中r=1,2,≥3分别对应普通临界、三相临界和更高阶多临界。超指数势产生多Kasner时期且后期继承相同的温度指数。数值解确认了普通和三相临界情形；解析证明了红外不动点的主导非相关指数控制低温Kasner标度。

Conclusion: 全息相变的临界标度行为系统地贯穿到黑洞内部Kasner几何中，形成了从边界临界到内部奇点结构的完整映射，为量子临界现象与引力内部动力学之间的普遍联系提供了新的理论框架。
...
### [43] [Generalized comodule tube algebras for boundary and domain wall defects of (2+1)D topological order](https://arxiv.org/abs/2608.05071)
*Zhian Jia,Sheng Tan*

Main category: cond-mat.str-el

TL;DR: 本文将管道代数框架推广至(2+1)维Turaev–Viro–Barrett–Westbury TQFT（特别是Levin–Wen弦网模型）中的余维2缺陷，提出了"缺陷管道代数"概念，其自然地携带着关于拓扑激发对应的弱Hopf管道代数的余模代数结构，并进一步阐明了边界、畴壁及一般k-缺陷的统一代数描述。


<details>
  <summary>Details</summary>
Motivation: 管道代数是刻画拓扑相中拓扑激发的基本工具，携带C*弱Hopf代数结构。然而，原有的管道代数框架尚未系统推广到余维2缺陷（如边界缺陷、畴壁缺陷）的情形。本文旨在填补这一空白，为更一般缺陷场景下的拓扑激发与缺陷表征提供统一的代数框架。

Method: 本文在TVBW TQFT（Turaev–Viro–Barrett–Westbury）框架下，以Levin–Wen弦网模型为主要研究对象，构造了"缺陷管道代数"（defect tube algebra）。作者证明了该代数自然具有关于弱Hopf管道代数的（余）模代数结构，并利用其表示论来刻画边界缺陷与畴壁缺陷。对于连接N个体相的畴壁，相应的畴壁缺陷管道代数被视为N元组代数；该视角进一步推广至一般的k-缺陷（连接k个余维1缺陷），所得的广义管道代数具有多余模代数结构。

Result: 建立了缺陷管道代数的完整框架：边界与畴壁缺陷由相应缺陷管道代数的表示刻画；连接两个边界缺陷的畴壁缺陷管道代数可视为两个相邻管道代数的广义Drinfeld double；连接N个体相的畴壁给出N元组代数；一般k-缺陷则给出具有多余模代数结构的广义管道代数。

Conclusion: 本文建立了余维2缺陷（包括边界缺陷、畴壁缺陷及更一般的k-缺陷）的统一代数描述框架，将原有管道代数理论自然地推广到更丰富的缺陷场景中，为拓扑相中缺陷的代数表征提供了新的视角和工具。
...
### [44] [From populations to absolute binding affinities in molecular simulations: exact volumetric terms and practical estimators](https://arxiv.org/abs/2608.04834)
*Davide Mandelli,Emiliano Ippoliti,Charles Plate*

Main category: physics.comp-ph

TL;DR: 本文提出一个统计力学框架，用于在稀溶液中从第一性原理精确计算平衡结合常数K。该框架明确揭示了未结合态体积项的来源，允许精确追踪体积约束的传播效应，并在同质未结合态的物理温和假设下，使K可通过反应坐标直方图直接计算。在两种实际体系（葫芦脲/金刚烷醇主客体复合物和半乳糖酸-DgoT配体-蛋白质复合物）中，作者发现常用的单箱估计器与理论正确值偏差约1 kcal/mol，该偏差源于结合态的定义方式。通过将结合态定义锚定于实验可分辨的状态，该理论将隐藏假设变为可控输入，为从分子模拟获得绝对结合亲和力提供了原则性路径。


<details>
  <summary>Details</summary>
Motivation: 分子模拟计算绝对结合自由能长期以来面临一个核心问题：结合态与未结合态的定义不明确，导致结合常数的计算结果对模拟参数（如体积约束）敏感且不可控。现有的标准估计器隐含了特定的结合态定义，但其影响未被系统评估。作者旨在提供一个从第一性原理出发的普适框架，明确揭示各种隐含假设的数学来源，从而使结合常数的计算变得理论严谨且可复现。

Method: 作者从统计力学第一性原理出发，推导出结合常数K与结合/未结合态相对布居数的一般关系式。该表达式将未结合态的体积项显式化，并精确追踪体积约束在表达式中的传播。在假设未结合态为同质（homogeneous）的物理温和条件下，体积贡献可以被精确校正，从而使得受限模拟（restrained simulations）直接计算出K。估计器可从任意适当定义的反应坐标的直方图计算，并明确热力学状态边界的定义方式。作者在葫芦脲[7]uril/1-金刚烷醇主客体体系和半乳糖酸-DgoT蛋白质配体体系上验证了该框架。

Result: 在葫芦脲[7]uril/1-金刚烷醇和半乳糖酸-DgoT两个体系上，常用的单箱（single-bin）估计器与理论正确的估计器偏差约1 kcal/mol。该偏差的来源被精确定位为结合态的定义：当结合态定义与实验可分辨的状态（state-of-the-art experiments）对接后，原本隐藏的假设转化为受控的输入参数，从而获得理论正确的结合常数。

Conclusion: 该工作为绝对结合亲和力的分子模拟计算奠定了严谨的统计力学基础。核心洞见是：结合态的定义不应是武断的模拟参数，而应锚定于实验可探测的物理状态。这使得理论上正确的结合常数与实验值之间具有一一对应的可解释性，为计算预测绝对结合自由能建立了原理上可靠的路线。
...
### [45] [Leveraging Machine Learning to Gain Insights on Quantum Thermodynamic Entropy](https://arxiv.org/abs/2305.06177)
*Srinivasa Rao. P*

Main category: physics.comp-ph

TL;DR: 本文对基于单个量子粒子的量子引擎进行了热力学分析，该引擎灵感来源于Szilard经典单粒子引擎和混沌Szilard映射。研究重点是比较量子与经典极限下观察和控制粒子所产生的热力学成本，探讨信息处理诱导热力学耗散（Landauer原理）的权衡。通过机器学习方法验证了在Szilard引擎第二定律约束下量子引擎的能量分析和仿真，并发现量子引擎的工作机制与经典版本有显著差异，其中插入隔板（partition）的成本在量子实现中起关键作用。


<details>
  <summary>Details</summary>
Motivation: 理解量子信息处理中的热力学成本，特别是观察和控制单粒子（测量与重置）所引入的能量消耗，并比较这些成本在量子与经典极限下的区别，从而深化对Landauer原理在量子信息热力学中应用的理解。

Method: 基于类混沌Szilard映射设计量子引擎模型，构建包含测量、热能提取和记忆重置的热力学循环；在量子与经典极限下分别分析观察和控制粒子的热力学成本，并利用机器学习方法对引擎进行能量分析和仿真，验证其满足Szilard引擎的广义第二定律。

Result: 机器学习方法成功实现了量子引擎的能量分析与仿真，确认引擎可在满足Szilard引擎第二定律的工作条件下运行。研究发现量子引擎与经典引擎的运行机制存在本质差异，隔板插入成本在量子实现中至关重要，这与经典情形不同。

Conclusion: 量子Szilard引擎在热力学行为上与经典对应物有根本性区别，插入隔板的操作成本和量子效应的耦合决定了量子引擎的热力学效率与耗散特征。该工作为量子信息热力学中的Landauer原理及信息处理诱导耗散提供了新的理论见解。
...
### [46] [Computational Methods and GPU Acceleration in Plasma Physics: A Empirical Analysis of arXiv Publications and Research Trends](https://arxiv.org/abs/2608.04171)
*Jeremy J. Williams,Anders Brostrom,Stefano Markidis*

Main category: physics.comp-ph

TL;DR: 该研究通过文本挖掘分析2010-2025年间arXiv上5,522篇计算等离子体物理论文，发现MHD研究的摘要比PIC和回旋动力学论文更长，摘要长度随团队规模略有增加，而PIC和回旋动力学方法在近十年中相对占比显著增长且与GPU采用强相关，揭示了方法学冗长程度与方法流行度之间的脱钩现象。


<details>
  <summary>Details</summary>
Motivation: 计算等离子体物理越来越依赖高性能计算方法（PIC、回旋动力学、MHD），但领域内缺乏关于方法选择、团队规模和GPU采用如何影响研究成果的系统性证据。作者旨在通过大规模文献计量分析填补这一空白。

Method: 对arXiv上2010-2025年间5,522篇计算等离子体物理论文进行大规模文本挖掘，以摘要长度作为方法论和算法复杂性的代理指标，采用普通最小二乘（OLS）、Tobit和Logistic回归模型，分析摘要长度和GPU提及次数如何随计算方法、作者数量和发表年份变化，并控制团队规模和时间趋势。

Result: MHD研究的摘要比PIC和回旋动力学论文更长；摘要长度随团队规模略微增加；时间效应显示摘要简洁性存在渐进变化；PIC和回旋动力学方法在近十年相对占比显著增长，且与GPU采用强相关，反映其更高的计算强度和对加速器架构的适配性。

Conclusion: 研究揭示了方法论阐述的冗长程度与方法流行度之间的脱钩：MHD摘要更长但不流行，PIC和回旋动力学更流行但摘要更简洁。这为HPC驱动的等离子体物理研究演化提供了新的文献计量视角。
...
### [47] [A commutant gate for spectral fitting through symmetry forced degeneracy](https://arxiv.org/abs/2608.04903)
*Stelios Savva*

Main category: physics.comp-ph

TL;DR: 本文提出了一种"门控估计器"来解决学习谱模型在对称强制简并扇区中的失效问题。该方法通过读取算子族的线性交换子（即同时块对角化的矩阵零空间）直接获取对称结构，用交换子的中心而非特征值聚类来区分强制简并与偶然简并，并在门控（强制块）与非门控（逐能级）目标之间切换。实验表明该方法在算子估计噪声下分类正确率达0.3（能量聚类在0.02就已失效），在对称及对称破缺机制下均能达到机器精度，验证了其结构而非度量的判定优势。


<details>
  <summary>Details</summary>
Motivation: 学习谱模型在对称强制简并扇区中因两个原因失效：一是对称强制能级完全重合时，逐能级可观测量定义不良（共享空间内任意单位向量都是特征向量）；二是对称保护的能级交叉附近，特征向量可观测量的梯度带有1/(λi-λj)因子，在能隙闭合时真正奇异。常规的正则化或阈值化手段都有代价：固定能隙无法同时保护强制多重态和区分真正不同的能级。因此需要一种基于对称结构而非度量的新方法。

Method: 方法核心有三步：(1) 从观测算子族中直接读取对称结构——计算该算子族的线性交换子（线性交换子代数），通过Maehara和Murota的同步块对角化方法，使用一次奇异值分解零空间来获得块的划分；(2) 块身份（block identity）从交换子的中心读出，而非通过特征值聚类，这使得"强制"与"偶然"简并的区分是结构性的而非度量性的；(3) 目标函数在强制块上使用投影迹（projector trace），在非强制区域使用逐能级目标，通过门控切换。在合成算子族上使用已知对称答案键（answer key）进行验证，并测试了估计噪声下的鲁棒性以及正则表示之外的验证场景。

Result: 在算子估计噪声下，门控分类器正确到ε约0.3，而能量聚类在0.02时就已失效，表明门控方法在噪声鲁棒性上有数量级优势。门控拟合在对称和对称破缺两种机制下均达到机器精度，在后一种情形收敛到无门控目标的奇点，可观测偏差处于噪声底。在正则表示之外的验证（多重性与维度分离的场景）中，该方法排除了两个所有正则表示测试都通过的缺陷估计器，说明正则表示交叉验证具有盲区。

Conclusion: 作者成功展示了门控估计器（gated estimator）的概念，它利用对称结构信息来克服学习谱模型在简并区域的固有困难。该方法将强制简并与偶然简并的区分从度量问题转为结构问题，大幅提高了噪声鲁棒性。下一步工作是完整的参数化矩阵模型（矩阵本身也被学习），目前展示的对象是门控估计器而非完整的端到端学习框架。
...
### [48] [Resolving coupled transport in space and time from molecular fluctuations in confined fluids](https://arxiv.org/abs/2608.04920)
*Thê Hoang Ngoc Minh,Ian C. Bourg*

Main category: physics.comp-ph

TL;DR: 本文提出将纳米受限流体的耦合输运描述为时空分辨的Onsager响应矩阵，并从平衡态分子动力学模拟中提取该矩阵，从而解析粒子、溶质、热和电荷输运之间的非局域且瞬态的耦合路径。


<details>
  <summary>Details</summary>
Motivation: 传统输运描述依赖连续介质定律和体相系数（如粘度、扩散率、滑移长度、界面阻力）来表征流体输运，但在纳米尺度下，空间异质性、分子结构和有限弛豫时间与输运过程不可分离，此连续描述不再完备。

Method: 将纳米受限流体的耦合输运公式化为时空分辨的Onsager响应矩阵，并从平衡态分子动力学模拟中提取该矩阵。以受限带电流体为例进行验证和应用。

Result: 框架成功解析了耦合粒子、溶质、热和电荷输运的非局域和瞬态路径。发现动量输运表现为长寿命、非局域的流体动力学模式，而电荷输运通过局域离子摩擦快速弛豫。非对角响应揭示了不同的投影动力学过程。

Conclusion: 该工作为纳米尺度下非局域、历史依赖的输运定律提供了微观基础，弥补了传统连续介质描述在纳米尺度的不足。
...
### [49] [Gibbs variational principles and Boltzmann irreversible theorem](https://arxiv.org/abs/2608.04129)
*Mário J. de Oliveira,Silvio R. Salinas*

Main category: cond-mat.stat-mech

TL;DR: 本文分析了孤立系统和恒温系统概率分布所对应的Gibbs变分原理，利用Gibbs不等式获得Ising模型（含竞争相互作用）的自由能并分析其相图，同时通过Kolmogorov方程将Boltzmann不可逆定理与Gibbs变分原理联系起来。


<details>
  <summary>Details</summary>
Motivation: 旨在统一两个看似独立的热力学变分框架——孤立系统的熵最大化原理和恒温系统的自由能最小化原理，并揭示它们与Boltzmann不可逆定理之间的内在联系，从而为统计力学的基本原理提供统一的视角。

Method: 首先分别推导孤立系统与恒温系统对应的Gibbs变分原理；然后利用Gibbs不等式求解含竞争相互作用的Ising模型的自由能并分析其相图；最后通过Kolmogorov方程描述概率分布的演化，证明孤立系统的熵单调递增以及恒温系统自由能单调递减。

Result: 成功建立了Gibbs变分原理与Boltzmann不可逆定理之间的桥梁：Kolmogorov方程预测孤立系统熵的单调增加和恒温系统自由能的减少，并通过Ising模型实例验证了该变分方法的实用性。

Conclusion: Gibbs变分原理为统计力学的两个基本场景（孤立系统和热浴系统）提供了统一的变分框架，并且与不可逆热力学过程紧密关联，可作为研究平衡与非平衡统计力学问题的有力工具。
...
### [50] [The two-particle-irreducible vertex of the two-dimensional lattice $φ^4$ model across the Ising transition](https://arxiv.org/abs/2608.04497)
*Lode Pollet*

Main category: cond-mat.stat-mech

TL;DR: 本文通过蒙特卡洛测量重构了二维单分量φ⁴格点场论中的2PI顶点Γ(k,p;q)，并追踪其跨越Ising相变的行为。研究发现不稳定性由A₁（铁磁）通道在零转移动量处驱动，但B₁和B₂通道也显著配合，表明软模是多维的。在有序相中q=0本征值塌缩，因为铁磁权重凝聚到序参量中。剥离交叉通道梯图后得到的完全不可约顶点近似为局部接触项，将其代入parquet和Schwinger-Dyson方程可高精度复现蒙特卡洛自能（误差<0.1%），为动力学局部顶点近似（DΓA）提供了第一性原理基准。


<details>
  <summary>Details</summary>
Motivation: 二维φ⁴格点场论是研究相变和临界现象的理想模型，但传统的量子多体方法（如parquet方程、DΓA等）在处理这种强关联系统时缺乏精确的基准。作者希望通过精确的蒙特卡洛方法重构双粒子顶点，从而验证近似方法的可靠性，并深入理解Ising相变驱动机制的具体物理图像。

Method: 从蒙特卡洛模拟中测量连通双粒子关联函数，通过Bethe-Salpeter方程重构2PI顶点Γ(k,p;q)。将顶点按C₄v点群不可约表示分解，分析各通道（A₁铁磁、B₁向列、B₂对角向列）的本征值演化。通过剥离交叉通道梯图获得完全不可约顶点，并将其代入parquet和Schwinger-Dyson方程与蒙特卡洛自能对比。

Result: (1) A₁通道零转移动量处对称化Bethe-Salpeter核的本征值趋近于1，是该相变的驱动通道；(2) B₁和B₂通道贡献显著，软模是多维的；(3) 实空间中顶点在临界点处发展出幂律尾巴，远离临界时为短程；(4) 有序相中q=0本征值塌缩源于铁磁权重凝聚到序参量中；(5) 完全不可约顶点近似为局部接触项，代入parquet/Schwinger-Dyson方程后与蒙特卡洛自能误差小于0.1%；(6) 临界区中parquet方程的物理解表现为排斥型不动点，初始由单一序参量模式驱动。

Conclusion: 该工作首次从第一性原理的蒙特卡洛数据出发，验证了DΓA中局部不可约顶点假设的高精度有效性，并揭示了Ising相变中软模的多维性质。这为发展更精确的多体近似方法提供了关键基准，同时也阐明了parquet方程在临界区的不动点行为。
...
### [51] [Analytical Floquet Quantum Statistics from Nonequilibrium Green's Functions](https://arxiv.org/abs/2608.04558)
*Yuhua Ren,Gaomin Tang,Hui Pan,Jian-Sheng Wang*

Main category: cond-mat.stat-mech

TL;DR: 本文利用非平衡格林函数(NEGF)形式体系，推导了周期驱动开放量子系统稳态量子统计的解析表达式，通过将Floquet理论嵌入NEGF得到Floquet格林函数的闭式解，提出了Floquet费米分布、扩展了其适用范围，并建立了Floquet版本的Landauer公式用于描述直流输运。


<details>
  <summary>Details</summary>
Motivation: 周期驱动量子系统与热浴耦合时的稳态统计行为是开放量子系统研究中的核心问题，此前的工作仅局限于对角对易哈密顿量的特殊情况，缺乏对一般非对易情形下稳态占据分布的系统性理论描述，需要一个更普适的理论框架来刻画Floquet边带占据及其输运性质。

Method: 将Floquet理论嵌入非平衡格林函数(NEGF)形式体系，在Floquet表象下推导推迟、超前和小于格林函数的闭式表达式；利用微运动算符的傅里叶分量定义权重因子，构造Floquet费米分布；并通过放宽热浴谱函数条件、引入非对角项推广到一般非对易哈密顿量情形；最后导出Floquet版本的Landauer电流公式。

Result: 获得了Floquet表象下三种格林函数的解析闭式解；建立了Floquet费米分布，其中稳态占据由按驱动频率整数倍平移的费米函数的加权和表示，权重完全由微运动算符的傅里叶分量决定；证明该分布对广泛类型的弱耦合热浴谱函数均成立，超越了理想无特征热浴近似；并推导了将平衡费米函数替换为Floquet修正版本的Landauer公式。

Conclusion: 该工作为周期驱动开放量子系统的稳态量子统计和输运提供了统一而连贯的理论描述，Floquet费米分布和Floquet-Landauer公式为理解和预测Floquet边带占据及驱动系统的直流输运行为提供了实用且透明的理论基础。
...
### [52] [An improved car-oriented mean-field theory for stochastic traffic flow models](https://arxiv.org/abs/2608.04731)
*Yasar Efe Dai,Andreas Schadschneider,Michael Schreckenberg*

Main category: cond-mat.stat-mech

TL;DR: 本文提出了一种改进的均值场分析方法，用于单车道车辆交通元胞自动机模型，通过结合Car-Oriented-Mean-Field (COMF)理论和2-site团簇方法，同时捕捉短程和长程相关性，特别适用于具有非均匀稳态和相分离特性的模型（如慢启动规则的VDR模型），并在v_max=1的VDR模型中验证了其改进的精度和新的物理洞察。


<details>
  <summary>Details</summary>
Motivation: 经典均值场理论在处理具有非均匀稳态和相分离特性的交通元胞自动机模型时存在精度不足的问题，无法准确捕捉这类模型中的关键物理现象。作者希望开发一种改进的均值场方法，能够在保持均值场框架简单性的同时，更准确地描述短程和长程关联效应。

Method: 将Car-Oriented-Mean-Field (COMF)理论与2-site团簇方法相结合，构建一种改进的均值场分析方法。该方法能够同时捕捉短程（通过2-site关联）和长程（通过COMF特性）相关性，从而适用于具有非均匀稳态的模型，并能捕获相分离的本质属性。

Result: 改进的方法在v_max=1的VDR模型（含慢启动规则）上的应用中展示了更高的精度，成功捕获了经典均值场理论难以描述的相分离关键特性，并提供了新的物理洞察。

Conclusion: 所提出的改进均值场方法是经典均值场理论的重要拓展，特别适合分析具有非均匀稳态和相分离现象的交通流模型，为理解慢启动规则下的交通动力学提供了更有效的分析工具。
...
### [53] [Residual Saturation under Pressure-Controlled Drainage](https://arxiv.org/abs/2608.04748)
*Fernando Alonso-Marroquin,Hans J. Herrmann*

Main category: cond-mat.stat-mech

TL;DR: 本文将压控排水过程建模为孔隙网络图上的渗流（含陷阱），建立了逾渗理论与压力-饱和度关系的直接联系。研究发现2D和3D下残余饱和度的有限尺寸标度行为不同，扩展了标准侵入逾渗理论。


<details>
  <summary>Details</summary>
Motivation: 标准侵入逾渗理论仅描述突破态（breakthrough state），此时侵入簇是分形的，被侵入相饱和度在无限尺寸极限下趋于零。然而实际多孔介质中的压控排水过程关注的是残余饱和度这一宏观性质，其在热力学极限下非零，需要更完整的理论框架来桥接逾渗理论与压力-饱和度关系。

Method: 将压控排水过程形式化为孔隙网络图上的键逾渗（bond percolation）并包含捕陷（trapping）效应，通过有限尺寸标度分析研究残余饱和度在不同维度下的偏离行为，检验其与晶格微观细节的无关性以及配位数的影响。

Result: 二维中残余饱和度偏离其非零热力学极限的有限尺寸标度指数δ≈0.25，与晶格微观细节无关；三维中有限尺寸修正衰减更快（δ≈0.75），渐近残余饱和度仍有限且依赖于配位数。

Conclusion: 该工作将逾渗理论从突破态推广到突破态之后的完整排水过程，揭示残余饱和度在有限尺寸下的普适标度行为（2D普适指数0.25，3D指数0.75），其中维度依赖性反映了捕陷机制和空间维度的耦合。
...
### [54] [Is Szilard Engine Really Broken?](https://arxiv.org/abs/2111.12300)
*Srinivasa Rao. P*

Main category: cond-mat.stat-mech

TL;DR: 本文基于范德华表面能原理，研究了麦克斯韦妖信息引擎的可工作性。作者指出，近期有文献基于"无内部结构活塞"的假设否定了此类引擎的可行性，但通过引入更基本的范德华表面能原理，在特定条件下证实此类引擎可以利用类似的传统活塞正常工作。


<details>
  <summary>Details</summary>
Motivation: 麦克斯韦妖与信息引擎是连接热力学与信息论的核心概念，Landauer原理确立了信息的物理本质。然而近期有研究基于活塞无内部结构的简化假设，对这类信息引擎的可行性提出了质疑，需要对这一矛盾进行更根本层面的论证。

Method: 采用范德华表面能原理这一更基本的物理框架，替代仅考虑无内部结构活塞的简化模型，重新分析Szilard式单粒子信息引擎的工作机制，考察在既定条件下引擎能否借助传统活塞实现做功。

Result: 研究发现，在考虑了范德华表面能效应的条件下，该信息引擎可以在规定条件下利用类似传统活塞正常工作，从而支持了麦克斯韦妖信息引擎的可实现性，反驳了基于无内部结构活塞假设的否定性结论。

Conclusion: 通过更基本的范德华表面能原理分析，证明了信息引擎在合理的物理条件下是可行的，调和了近期文献中的矛盾结论，进一步巩固了信息与热力学之间的物理关联。
...
### [55] [The rank and layer distributions in random recursive trees](https://arxiv.org/abs/2608.04303)
*Huck Stepanyants,P. L. Krapivsky,Harrison Hartle,Dmitri Krioukov*

Main category: cond-mat.stat-mech

TL;DR: 本文研究了随机递归树中节点深度分布的两种度量——rank（节点到最近叶子的距离）和layer（剥离所有叶子直到节点成为叶子的次数），推导出二者的精确递推表达式，并证明rank分布呈阶乘衰减、layer分布呈几何衰减，且固定rank或layer的节点数量具有自平均性。该方法不依赖标签或根节点，比以往研究更具通用性。


<details>
  <summary>Details</summary>
Motivation: 网络中的节点深度是分析网络结构的关键指标。rank和layer这两种度量量化了节点在网络中的内部深度，但目前对它们在随机递归树中的分布特性缺乏严格的数学刻画。此外，以往研究依赖于标签或根节点，限制了框架的通用性。

Method: 作者推导出随机递归树中rank和layer分布的精确递推表达式，并利用这些表达式分析其渐近行为。通过数学分析证明rank分布呈阶乘衰减、layer分布呈几何衰减，同时证明固定rank或layer的节点数量满足自平均性。该方法的创新之处在于不依赖标签或根节点，提供了更通用的分析框架。

Result: 获得了rank和layer分布的精确递推表达式；证明了rank分布呈阶乘衰减，layer分布呈几何衰减；证明固定rank或layer的节点数具有自平均性。

Conclusion: 该研究为随机递归树中节点深度分布提供了不依赖标签或根节点的通用分析框架，丰富了复杂网络中rank与layer分布的理论基础。
...
### [56] [Equilibrium Thermodynamics of Non-Hermitian Dirac Fermions: Caloric and Magnetic Responses](https://arxiv.org/abs/2608.04369)
*Francisco J. Peña,Bastian Castorene,Juan Pablo Esparza,Vladimir Juričić,Patricio Vargas*

Main category: cond-mat.stat-mech

TL;DR: 本文建立了实谱非厄米狄拉克费米子在磁场中的平衡热力学标度关系。通过相似变换将系统映射为具有约化速度的厄米狄拉克模型，并推导出化学势、熵、热容和轨道磁响应的标度规律。


<details>
  <summary>Details</summary>
Motivation: 非厄米量子系统在磁场下的热力学行为尚缺乏系统框架，尤其是实谱非厄米狄拉克费米子的平衡热力学性质未被充分探索。

Method: 假设系统相对于准厄米哈密顿量热化，利用相似变换将非厄米系统映射为约化速度的厄米Dirac模型；在固定投影填充因子（PFF）和固定化学势两种系综下分别分析热力学函数；对固定PFF下的准静态非厄米绝热形变得出了绝热温度标度关系。

Result: 在固定投影填充因子下，自洽化学势遵循压缩的朗道能级阶梯，正则热力学函数表现为重新标度的厄米响应；在固定化学势下，朗道能级交叉产生由同一谱压缩支配的振荡量热和磁响应。准静态非厄米形变在固定PFF下产生绝热温度标度。

Conclusion: 该工作为实谱非厄米量子物质建立了热力学框架，并为在该形式体系内纳入相互作用和无序效应提供了起点。
...
### [57] [Single-eigenstate test of eigenstate thermalization hypothesis via perturbed eigenstate quench](https://arxiv.org/abs/2608.04696)
*Zhouhao Guo,Jiaju Zhang*

Main category: cond-mat.stat-mech

TL;DR: 本文提出并数值验证了一种基于扰动本征态淬火协议的单本征态遍历态假设(ETH)诊断方法。通过给能量本征态引入弱随机扰动打破其稳定性，刻画子系统平均演化速度与子系统/总系统尺寸比的关系。满足ETH的本征态呈现具有拐点的S形曲线，而违反ETH的本征态呈现凸J形轮廓。该方法在混沌、可积、多体局域化和量子多体疤痕四种一维自旋链模型中均与既有热化现象学一致，无需显式构造热系综，可在单本征态层面实现实验可及的ETH探测。


<details>
  <summary>Details</summary>
Motivation: 传统的ETH诊断通常需要构造热系综或进行全系统长时间演化，难以在单本征态层面直接评估热化性质。作者旨在开发一种简洁、高效且无需热系综构造的单本征态ETH判据，以更直接地刻画不同本征态是否满足ETH，并适用于实验场景。

Method: 采用扰动本征态淬火协议：对能量本征态施加弱随机扰动以打破其驻定性，然后监测子系统约化态的时均演化速度，并将其作为子系统尺寸与总系统尺寸之比的函数。基于该函数曲线的形状（S形含拐点 vs 凸J形）来区分ETH满足与违反的本征态。在混沌、可积、多体局域化和量子多体疤痕等有代表性的1D自旋链模型中进行基准检验。

Result: 该方法在所有测试的模型体系中均获得了与既有热化现象学完全一致的判断结果：满足ETH的本征态呈现出带拐点的S形曲线，而违反ETH的本征态呈现凸J形轮廓。验证了该判据跨模型普适性及鲁棒性。

Conclusion: 该诊断方案提供了一种无需显式构造热系综的单本征态ETH探针，具有强鲁棒性和实验可行性，为在单本征态层面研究量子热化提供了新的有效工具。
...
### [58] [Laminar gaps mirror turbulent puffs in pipe flow](https://arxiv.org/abs/2608.04823)
*Shai Kapon,Tobias Grafke,Anna Frishman*

Main category: cond-mat.stat-mech

TL;DR: 本文通过直接数值模拟研究管道流在中等雷诺数（Re=2400-2550）下湍流与层流共存的间歇性现象，揭示了层流间隙作为一种类似于"puffs"的独立动力学状态，其寿命呈指数分布，并在Re约2900时失稳消失，标志着湍流进入空间和时间上的均匀状态。


<details>
  <summary>Details</summary>
Motivation: 管道流在中间雷诺数范围内存在层流与湍流共存的间歇性状态，此前的理解主要集中于低雷诺数范围（1750-2300）的"puffs"结构，但对更高雷诺数范围（2300-3000）中扩展湍流态内层流间隙的性质缺乏深入研究。

Method: 对管道流在Re=2400、2450、2500、2550下进行直接数值模拟（DNS），分析层流间隙的平均空间剖面，统计其寿命分布，并推断其在高雷诺数下的失稳行为。

Result: 研究发现层流间隙是与puffs相对应的独立动力学状态，即在湍流包围中传播的层流口袋，由剪切依赖的自调谐机制稳定。间隙寿命呈指数分布，表明其关闭对应从混沌鞍的逃逸。该状态在Re约2900时失稳消失。

Conclusion: 层流间隙构成了一个与puffs对称的动力学状态，其失稳点Re~2900可被解释为空间和时间上均匀湍流的起始点，为理解管道流转捩的完整图景提供了新的视角。
...
### [59] [Structural Chirality from Short-Range Order in Heteroanionic Materials](https://arxiv.org/abs/2608.04841)
*Benjamin J. Morgan*

Main category: cond-mat.stat-mech

TL;DR: 本文提出了一种通过阴离子有序化而非原子位移来形成无机晶体结构手性的新路线。以ReO₃型氧氟化物（如NbO₂F）为例，通过直接枚举证明2:1阴离子化学计量比下顺式八面体配位与周期三阴离子链两种有序模式的组合必定产生手性结构，属于Sohncke空间群。DFT和Wang-Landau蒙特卡洛模拟预测NbO₂F基态为最大对称手性构型，手性相可在高达494 K的一级相变前稳定存在。


<details>
  <summary>Details</summary>
Motivation: 传统无机晶体中结构手性的形成依赖于对称性降低的原子位移，但作者认为可以通过两种阴离子物种在非手性母体晶格位点上的有序排列来强制产生手性，无需依赖原子位移，从而开辟了异阴离子材料作为手性功能材料的新途径。

Method: 采用直接枚举方法证明在最小公度胞上所有同时满足顺式八面体配位和周期三阴离子链两种有序模式的构型均为手性且属于Sohncke空间群；对NbO₂F使用密度泛函理论（DFT）计算预测基态构型，并训练簇展开（cluster-expansion）模型结合Wang-Landau蒙特卡洛模拟研究有限温度下的手性有序-无序相变。

Result: DFT预测NbO₂F的基态是最大对称手性构型；Wang-Landau模拟表明手性相在494 K的一级相变之前保持热力学稳定，即在高达494 K的温度下仍存在平衡手性有序相。

Conclusion: 构型有序化（configurational ordering）是一种化学上可实现的产生结构手性的新途径，将具有类似2:1阴离子有序化学的异阴离子材料确立为新型手性功能材料的候选体系。
...
### [60] [Two-dimensional Toda--Arnoldi correspondence: Holomorphic Krylov geometry and counterdiabatic transport](https://arxiv.org/abs/2608.04850)
*Urei Miura*

Main category: cond-mat.stat-mech

TL;DR: 本文证明非Hermitian哈密顿量的Arnoldi约化产生的上Hessenberg矩阵中，其对角与次对角系数构成一个闭合的二维Toda晶格扇区。Krylov Gram行列式是Toda格子的τ函数，Flaschka变量与Arnoldi系数精确对应；次对角系数的平方决定Fubini-Study度量和Berry曲率，且Arnoldi-frame联络可提供精确等谱输运的Hermitian三对角生成元，实现反绝热驱动。


<details>
  <summary>Details</summary>
Motivation: 研究非Hermitian哈密顿量的Krylov空间结构，探索Arnoldi约化中的可积结构与几何意义，并为反绝热驱动（counterdiabatic driving）提供新的构造框架。

Method: 利用Arnoldi约化将非Hermitian哈密顿量化为上Hessenberg矩阵，分析对角与次对角系数；引入Krylov Gram行列式识别其为二维Toda晶格τ函数；通过Flaschka变量建立与Arnoldi系数的精确对应；利用次对角系数联系Fubini-Study度量与Berry曲率；沿光滑实路径构造Arnoldi-frame联络生成等谱输运和反绝热驱动。

Result: 证明了闭合Toda扇区存在于Arnoldi系数的对角与次对角部分；Krylov Gram行列式为有限二维Toda格子的τ函数；次对角平方编码FS度量和Berry曲率；Arnoldi-frame联络给出Hermitian三对角生成元，在对角化且谱非简并条件下可实现反绝热驱动。

Conclusion: Arnoldi约化在一股非Hermitian情形下仍保留可积的Toda结构，且这一结构具有明确的几何（FS度量、Berry曲率）与动力学（等谱输运、反绝热驱动）意义，为量子态的Krylov空间分析提供了统一框架。
...
TL;DR: 本文通过微扰动力学重整化群方法研究非互易Cahn-Hilliard（NRCH）模型在临界点附近的标度行为，发现结构相关长度与动力学相关长度由不同标度律控制，并揭示了新的临界指数。


<details>
  <summary>Details</summary>
Motivation: 非互易性是微观尺度非平衡活性的普遍表现形式，广泛存在于化学活性酶混合物、胶体、液滴及工程化光控活性胶体等活性物质系统。NRCH模型作为描述具有非互易相互作用的二元保守组分混合物动力学的最小模型，其非互易耦合是内禀非平衡活性源，打破了宇称和时间反演对称性，可能导致时空结构的形成。理解其临界行为对于揭示活性系统如何调控相分离和时空斑图具有重要意义。

Method: 采用微扰动力学重整化群（perturbative dynamical renormalization group）技术，对近临界点的NRCH模型进行系统研究，分析结构相关函数和动力学相关函数的标度行为，并推导相关临界指数的重整化群流方程。

Result: 发现结构相关和动力学相关由不同的相关长度控制，两者在临界点均发散但遵循不同标度律。结构相关始终由温度控制，遵循经典Wilson-Fisher临界指数；而动力学相关长度呈现多重标度区间，温度或非互易耦合均可作为主导调谐参数，以新的临界指数表征其发散行为。临界点对应具有奇（odd）迁移率的守恒平衡类动力学，即奇Cahn-Hilliard（OCH）模型。

Conclusion: 该发现揭示了NRCH模型临界点处结构与动力学性质的解耦标度行为，暗示活性系统可以通过催化反应速率和代谢等作为控制参数来调控相分离及时空斑图形成，对理解生命系统中的非平衡自组织有重要启示。
...
### [62] [Constructing Non-Hermitian Theories with Tunable Exceptional Points and Controlled State Purification](https://arxiv.org/abs/2608.05052)
*Soumya Kanti Pal,Rupak Majumder,Shamik Gupta*

Main category: cond-mat.stat-mech

TL;DR: 本文提出动量空间形变作为设计二次多体哈密顿量中奇异点(EP)的统一设计原理，揭示了单个动量扇区奇异点导致多体本征矢量的指数级并合，并将奇异点确立为纯化任意混合量子态的普适机制，同时提供系统性的逆向工程协议来生成短程/长程、互易/非互易的非厄米量子物质。


<details>
  <summary>Details</summary>
Motivation: 奇异点是非厄米量子系统的标志性特征，但其在多体系统中的创造与控制缺乏统一的设计原则。作者希望建立一种普适框架，将动量空间形变作为统一途径，系统性地工程化奇异点并设计多体非厄米量子系统。

Method: 通过动量空间形变这一通用设计原理，研究二次多体哈密顿量中奇异点的产生与控制。作者识别了动量扇区容纳奇异点的普适判据及相应临界形变强度，揭示了单动量扇区奇异点引发的多体本征矢量指数并合现象，并建立了奇异点作为混合量子态纯化普适机制的框架，同时提出逆向工程协议以生成多种非厄米量子物质及显式Lindblad嵌入。

Result: 发现单一动量扇区奇异点可导致多体本征矢量产生指数级并合；明确了不同的纯化区域以及热力学极限下奇偶系统尺寸的根本差异；建立了从目标非厄米行为到动量空间形变参数的逆向工程协议。

Conclusion: 动量空间形变是一个统一且通用的路线，可用于奇异点工程和多体非厄米量子系统的受控设计。
...
### [63] [Unifying quantum measurement constructions via a relative-entropy minimum change principle](https://arxiv.org/abs/2608.04055)
*Nana Liu,Mark M. Wilde*

Main category: quant-ph

TL;DR: 本文基于最小变化原则（minimum change principle），利用量子相对熵研究了量子统计推断中的测量优化问题。作者给出了该原则下最优测量的闭式刻画，通过对偶形式只需优化单一无约束厄米变量即可找到最优测量。该框架统一了pretty good测量和Fermi-Dirac热测量，并发掘了一个新的测量族——softmin热测量。进一步证明softmin热测量是熵正则化半定规划问题的最优解，类似于统计力学中热态的角色，最后证明了相对熵最小变化原则的可加性并评估了Fermi-Dirac热测量在量子假设检验中的性能。


<details>
  <summary>Details</summary>
Motivation: 经典概率论中，最小变化原则为Bayes反演信道提供了信息论刻画，最近被提议作为将Bayes规则推广到量子信息论的框架。本文旨在将该原则应用于量子统计推断场景，特别是在经典到量子的制备信道（前向过程）和量子到经典的测量信道（逆向过程）的设定下，研究最优测量的表征与性质。

Method: 使用量子相对熵作为度量工具，形式化量子统计推断中的最小变化原则。通过建立对偶公式，将最优测量的求解转化为单变量无约束厄米矩阵的优化问题，从而获得闭式解。同时利用熵正则化半定规划的框架进一步刻画softmin热测量，并利用可加性证明和假设检验性能分析对相关测量族进行理论分析。

Result: 建立了最小变化原则下最优测量的闭式刻画；证明了该最优测量可通过单无约束厄米变量的对偶形式求得；恢复了pretty good测量和Fermi-Dirac热测量作为该框架的特例，并发现了新的softmin热测量族；证明了softmin热测量是熵正则化半定优化问题的解；证明了相对熵最小变化原则的可加性；分析了Fermi-Dirac热测量在量子假设检验中的表现。

Conclusion: 该工作将最小变化原则系统地推广到量子统计推断领域，不仅统一了多个已知的重要测量方案，还发现了新的测量族（softmin热测量），并揭示了测量理论与统计力学之间的联系——softmin热测量在测量领域的作用类似于热态在统计力学中的作用。这为量子测量设计和量子统计推断提供了新的理论工具。
...
### [64] [Graph-Aware Exact Branch-and-Bound with Device Profiles for Static Qubit Allocation](https://arxiv.org/abs/2608.04058)
*Kamer Kaya*

Main category: quant-ph

TL;DR: 本文针对静态量子比特分配问题（矩形二次分配问题），提出了一种结合图感知精确归约与工程化搜索的轻量级分支定界方法，在单线程和共享内存多线程场景下均显著提升了求解速度。


<details>
  <summary>Details</summary>
Motivation: 静态量子比特分配将电路的逻辑量子比特映射到稀疏物理设备上，需最小化交互加权的物理距离成本，本质上是矩形二次分配问题。现有方法依赖强下界与分布式分支定界，但计算开销大、可扩展性受限，亟需更高效的精确求解方案。

Method: 提出了四条关键优化路径：(1) 不可避免的已分配成本过滤；(2) 增量维护的根轨道与前缀稳定子对称性剪枝；(3) 条件化的父级线性指派问题（LAP）筛选；(4) 与电路无关的物理设备配置文件。整体形成一个轻量级的分配-边界路径，并集成到单线程及共享内存多线程框架中。

Result: 在21个Melbourne实例和6个Boeblingen实例上（GLB基线可完成的），最终单线程配置分别获得几何平均2.98倍和13.27倍的加速。在60线程共享内存服务器上，最终Boeblingen-Cairo实验的所有实例均在半小时内被认证为最优（不含一次性设备工件构建时间）。

Conclusion: 图感知的节点处理与搜索过程的工程化优化能够大幅降低精确量子比特分配所需的计算资源，验证了该方法的有效性与可扩展性。
...
### [65] [Guiding Compiler Optimizations for Neutral Atom Quantum Computers Through Visualizations](https://arxiv.org/abs/2608.04081)
*Yannick Stade,Robert Wille*

Main category: quant-ph

TL;DR: NAViz是首个公开可用的中性原子（NA）量子计算实时可视化工具，作为慕尼黑量子工具包（MQT）的一部分发布。通过案例研究验证，该工具成功帮助识别并解决了现有编译策略中的低效问题，促成了更优编译策略的诞生。


<details>
  <summary>Details</summary>
Motivation: 中性原子量子计算机的规模增长需要自动化编译工具，而设计启发式编译方法需要深入理解复杂的硬件权衡，可视化可以提供关键洞察。目前缺少用于NA设备量子计算可视化的公开工具。

Method: 开发了NAViz——首个公开可用的NA量子设备实时可视化应用，并作为MQT工具包的一部分开源发布在GitHub上。

Result: 通过案例研究展示了NAViz在识别和解决现有编译策略低效问题方面的能力，并据此开发了新的、性能更优的编译策略。

Conclusion: NAViz通过实时可视化有效辅助了NA量子编译策略的设计与优化，证明了可视化工具在量子编译开发中的实用价值。
...
### [66] [Quantum Many-Body Metrology of Rotation Sensing with Strong Interactions](https://arxiv.org/abs/2608.04082)
*Yong-Jun Baak,Uwe R. Fischer*

Main category: quant-ph

TL;DR: 本文研究了准一维环形势阱中带两个弱链接的少量强相互作用玻色子的旋转感测极限，发现必须采用自洽多体求解才能正确预测强关联多体陀螺仪的极限灵敏度。在小旋转速度和少粒子数情况下，多体量子Fisher信息在大相互作用耦合时达到最大，表明强相互作用的微型多体传感器有望以高空间分辨率精确估计慢速旋转。


<details>
  <summary>Details</summary>
Motivation: 探索强关联多体系统在旋转感测中的量子极限，特别是少粒子强相互作用玻色子在环形阱中的量子Fisher信息行为，以评估微型化多体传感器进行精密旋转测量的潜力。

Method: 采用自洽多体求解方法，研究在具有两个弱链接的准一维环形陷阱中少量强相互作用玻色子的旋转感测问题；计算多体量子Fisher信息作为旋转速度和粒子数的函数，并分析不同相互作用耦合强度下的灵敏度变化。

Result: 结果表明：对于小旋转速度和少粒子数情形，多体量子Fisher信息在强相互作用耦合时达到最大值，显示出强相互作用能够增强旋转感测的灵敏度。

Conclusion: 强相互作用的少粒子多体系统可以作为高效、高空间分辨率的微型陀螺仪，为慢速旋转的精密测量提供了新途径；但正确的灵敏度预测必须依赖自洽的多体理论框架，而非单粒子近似。
...
### [67] [Revealing Noise in Axial Motion Through Quantum Noise Spectroscopy on a Trapped Ion Processor](https://arxiv.org/abs/2608.04089)
...
TL;DR: 本文利用抗退相干量子噪声谱学，在单个可寻址的捕获离子处理器中识别并表征了由轴向运动引起的控制噪声。研究发现该噪声主要受局部光束曲率支配，表现为控制噪声谱中的低频成分，并通过改变离子在光束内的位置分离了曲率依赖的轴向运动噪声与曲率无关的固有控制噪声，同时该方法可并行应用于四离子寄存器。


<details>
  <summary>Details</summary>
Motivation: 量子处理器中的本征噪声过程往往难以隔离，因为多种错误机制会对同一测量到的相干性损失做出贡献，需要一种能够区分不同噪声来源的方法。

Method: 采用抗退相干的量子噪声谱学方法，通过改变离子在寻址光束中的位置，分离曲率依赖的轴向运动噪声与曲率无关的固有控制噪声，并提取难以通过其他手段获得的行运动参数；同时将协议并行应用于四离子寄存器以展示其可扩展性。

Result: 轴向运动噪声主要受局部光束曲率支配，表现为控制噪声谱中的低频贡献；通过改变离子位置可分离两类噪声并提取运动参数；该协议可并行应用于多离子寄存器，实现跨多个量子比特的位置相关控制噪声同步表征。

Conclusion: 研究确定了光束拐点作为抑制轴向运动诱导噪声的工作区域（以降低拉比频率为代价），为捕获离子量子处理器的噪声优化提供了实用指导。
...
### [68] [An optical-fibre-integrated buffer for packet-switched quantum networks](https://arxiv.org/abs/2608.04093)
...
TL;DR: 本文展示了一种基于光纤集成架构的量子数据包缓冲器，利用循环回路和光纤存储线，通过读取附加数据包头来确定极化编码量子比特的存储时间。核心组件是超低损耗的极化光纤相位调制器，可在光纤中实现快速、极化不敏感切换。实验实现了最长47微秒的存储和检索，平均量子误码率为1.8%，并可稳定运行数小时。


<details>
  <summary>Details</summary>
Motivation: 分组交换量子网络需要能够延迟量子比特载荷的缓冲区，以便实时读取路由信息。然而，此前的方法未能在完全光纤集成、兼容电信基础设施的架构中实现这一功能。

Method: 采用基于循环回路和光纤存储线的缓冲方案，核心器件是超低损耗极化光纤相位调制器，它能够在光纤中直接进行快速且极化不敏感的切换，使得数据包头和量子比特载荷能在缓冲区内部被处理。

Result: 成功实现极化编码量子比特载荷的存储与检索，存储时间最长47微秒，平均量子误码率1.8%，可稳定运行数小时。

Conclusion: 该工作确立了实用的基于光纤的分组级量子网络缓冲架构，可便捷集成到现有电信基础设施中，为量子互联网的部署开辟了新路径。
...
### [69] [Contextuality in Sequential State Discrimination](https://arxiv.org/abs/2608.04099)
*Nyan Raess,Farid Shahandeh*

Main category: quant-ph

TL;DR: 该论文研究了广义语境性在量子态辨识的顺序任务中的作用。对于非顺序情形（n=1），使用COPE形式证明了不仅最优测量，连特定非最优测量也必然存在语境性；对于顺序情形（n>1），语境性的出现取决于制备的量子态及所选择的协议（无歧义辨识或最小错误辨识）。


<details>
  <summary>Details</summary>
Motivation: 广义语境性已知是量子态辨识最优策略中必备的资源，但以往研究主要聚焦在非顺序的单一场景。顺序辨识任务中，多个玩家依次尝试判别量子比特的制备态，该场景下语境性所扮演的角色尚不清楚，本文旨在系统探究这一问题。

Method: 采用COPE（Contextuality of Preparation and Measurement）形式主义来分析量子态辨识中的语境性。分别对非顺序（n=1）和顺序（n>1）情况下的无歧义辨识与最小错误辨识两种协议进行考察，评估语境性是否存在及其条件依赖。

Result: 在非顺序n=1情况下，语境性的存在不仅限于最优测量，对特定非最优测量也同样被保证。在顺序n>1情况下，语境性的出现与否取决于具体的量子态制备以及所采用的协议类型（无歧义辨识或最小错误辨识）。

Conclusion: 语境性在量子态辨识中的作用是协议依赖和状态依赖的。顺序辨识任务中，语境性并非在所有情形下都必然出现，其存在条件与所制备的量子态及所选协议密切相关，这深化了对语境性作为量子资源在不同辨识场景中角色的理解。
...
### [70] [Reconstructing non-Abelian braiding and fusion without anyon transport](https://arxiv.org/abs/2608.04103)
*Lucy Byles,Matthew D. Horner,Benjamin T. H. Varcoe,Jiannis K. Pachos*

Main category: quant-ph

TL;DR: 本文提出了并实验实现了基于时间有序ribbon操作的测量唯一协议，用于在量子硬件上重建非阿贝尔任意子D(S₃)量子双模型的编织和融合原语。在Quantinuum H2阱离子处理器上用双qutrit降低版本实现，平均归一化输出态保真度分别达到0.9988（编织）和0.9987（融合），并展示了将其组合产生非Clifford编织和非稳定子资源态。


<details>
  <summary>Details</summary>
Motivation: 非阿贝尔任意子有望实现容错和通用量子计算，但在量子硬件上实现其定义性的编织和融合操作受到巨大资源开销的限制。实验上获取任意子的编织与融合数据仍然非常困难，因此需要一种资源效率更高的协议来在量子硬件上重建这些关键原语。

Method: 提出一种基于时间有序ribbon操作的测量唯一协议，在不进行物理任意子传输的情况下重建D(S₃)量子双模型的非阿贝尔编织和融合原语。在Quantinuum H2阱离子处理器上实现降低版双qutrit协议，采用辅助量子比特辅助的ribbon操作和任意子电荷投影的qubit编码方式。使用改进的Hadamard测试重建编织相位的平方，通过后选择测量重建融合振幅。

Result: 编织和融合变换在理想作用下的平均归一化输出态保真度分别为F_R=0.9988和F_F=0.9987。将这些原语组合后成功产生了非Clifford编织和非稳定子资源态，验证了测量唯一任意子编码作为更大拓扑编码量子处理器构建模块的可行性。

Conclusion: 测量唯一协议能够在当前量子硬件上高效重建非阿贝尔任意子的编织和融合原语，无需物理任意子传输和高昂资源开销。高保真度的结果以及非Clifford编织和非稳定子资源态的生成，表明该方案可作为构建大规模拓扑编码量子处理器的基础模块。
...
### [71] [Poled-fibre phase modulator for efficient high-dimensional quantum measurements](https://arxiv.org/abs/2608.04112)
...
TL;DR: 本文首次将极化光纤相位调制器（PFPM）用于全光纤集成平台中的主动量子态测量，实现四维量子密钥分发（QKD）。仅需单个PFPM即可完成四维量子比特的基选择，大幅简化接收端架构，并获得超越此前所有已报道QKD演示的有限密钥速率。


<details>
  <summary>Details</summary>
Motivation: 高维编码可提升量子信息协议的性能（增强噪声鲁棒性和信息容量），但其实际实现面临挑战，主要瓶颈在于高维度下的基选择通常需要主动调制器，导致显著光学损耗和偏振敏感操作。亟需一种低损耗、偏振无关且与标准通信光纤兼容的调制方案。

Method: 利用极化光纤相位调制器（PFPM）这一光纤原生电光技术，其具备亚dB插入损耗、固有偏振无关性及与标准电信光纤直接兼容的特性。在接收端仅使用单个PFPM实现四维量子比特的基选择，构建全光纤集成的测量系统，并执行四维QKD基准测试。

Result: 成功完成四维QKD会话，获得的有限密钥速率（每秒每脉冲）超越了所有此前报道的QKD演示。PFPM方案显著简化了接收器架构，同时保持高效率检测性能。

Conclusion: 极化光纤电光调制被确立为光纤集成量子信息处理中高效检测的广泛适用平台，为高维量子协议的实际部署提供了可行的技术路线，兼具低损耗、偏振无关和大带宽等实用优势。
...
### [72] [Quantum Contextuality and Entanglement-Free Grover Search in a Trapped-Ion Optical Qudit](https://arxiv.org/abs/2608.04128)
*Tarun Dutta,Jasper Phua Sing Cheng,Alex Jin,Sergi Ramos-Calderer,José Ignacio Latorre,Manas Mukherjee*

Main category: quant-ph

TL;DR: 本文在单个囚禁138Ba+离子中实现了一个可编程的四维光学量子比特（qudit），展示了无纠缠的Grover量子搜索算法（目标识别概率高达94.5±2.0%）和基于CHSH型非语境不等式的状态依赖量子语境性（最大违背S=2.816±0.082，接近Tsirelson界），为研究相干干涉与语境性之间的关系提供了可扩展平台。


<details>
  <summary>Details</summary>
Motivation: 量子计算优势通常归因于相干干涉和其他非经典资源，但在天然存在多体纠缠的实验平台中，这些资源各自的作用难以区分。高维量子系统为此提供了有吸引力的研究途径，同时能降低量子信息处理的硬件开销。

Method: 在单个囚禁138Ba+离子中编码可编程四维光学qudit，通过相位可编程的光学旋转实现通用相干控制。利用该平台分别实现无纠缠的Grover量子搜索算法和CHSH型非语境不等式的量子语境性测量。

Result: Grover搜索算法实现了最高94.5±2.0%的目标态识别概率；CHSH型非语境不等式获得最大违背S=2.816±0.082，与Tsirelson界高度一致。

Conclusion: 该工作将可编程量子计算与语境性测量集成于单一多能级囚禁离子平台，建立了研究相干干涉与语境性之间关系的多功能架构，为高维量子技术的发展提供了可扩展路线。
...
### [73] [Photon localization: a comparative study](https://arxiv.org/abs/2608.04134)
*Laura M. Ferguson,Ricardo Rojas Castellano,Kayla Oltman,Joshua G. Fenwick,Rainer Dick*

Main category: quant-ph

TL;DR: 本文比较了光子局域化的不同度量方法在二维高斯波包中的表现，发现经过足够长时间演化后各种度量趋于一致；但Landau-Peierls波函数给出最大且正的偏移量&lang;r&rang;-ct&gt;0，而标量傅里叶变换给出最小偏移且最快收敛到0。作者还通过离子阱模型探测器讨论了光子的局域检测，发现局域检测概率可由三种局域化度量（Landau-Peierls波函数、能量波函数和Hawton密度）表达，且这三种度量在单光子波包时间演化中始终保持接近。


<details>
  <summary>Details</summary>
Motivation: 光子的局域化是量子光学中的一个基本问题，不同研究者提出了不同的光子局域化度量方法，但它们在物理含义和具体数值上存在差异。作者希望通过二维高斯波包的框架系统比较这些度量，并探讨它们与光子局域检测的关联，以澄清何种度量最能反映光子的实际可探测局域位置。

Method: 以二维高斯波包作为研究载体，系统比较多种光子局域化度量方法；通过解析和数值分析考察各度量在时间演化中的行为差异。同时构建由离子阱中离子构成的模型探测器，从散射矩阵推导位置相关的探测概率，并将其与各种局域化度量进行对比。

Result: 所有局域化度量在波包演化时间超过若干倍动量空间宽度倒数之后开始趋同；Landau-Peierls波函数产生最大的正偏移&lang;r&rang;-ct&gt;0，而标量傅里叶变换产生最小偏移且最快收敛至0。光子局域检测概率可以由Landau-Peierls波函数、能量波函数和Hawton密度三种度量表达，这三种度量在单光子波包时间演化中保持接近。

Conclusion: 光子局域化度量在长时间演化下趋于一致，但在瞬态行为上存在差异；Landau-Peierls波函数、能量波函数和Hawton密度三种度量与离子阱模型探测器的局部探测结果密切相关，表明它们可能是描述光子局域检测最合适的度量候选。
...
### [74] [Applications of Spin-Dependent Generalized Squeezing in Hybrid Spin-Oscillator Quantum Processors](https://arxiv.org/abs/2608.04197)
*Kai Shinbrough,Donovan J. Webb,Iver R. Øvergaard,Oana Băzăvan,Sebastian Saner,Christopher J. Ballance,Raghavendra Srinivas*

Main category: quant-ph

TL;DR: 本文提出并研究了基于广义压缩相互作用的四种混合自旋-振子量子处理器应用：自旋相关的几何相位门、N体自旋相互作用、振子测温以及通过中电路测量制备高保真振子量子态。这些应用共同依赖于广义压缩诱导的几何相位，该相位对振子的Fock占据数和广义压缩相互作用阶数呈非线性关系。


<details>
  <summary>Details</summary>
Motivation: 广义压缩相互作用是量子光学的基础，最近已在混合自旋-振子量子处理器中实现实验控制。作者旨在利用这一新近可控的相互作用，开发离散变量和连续变量量子信息处理的新应用，为混合平台上的量子计算和传感奠定基础。

Method: 作者提出了四种具体应用方案：(1) 利用作用于两个自旋和公共振子的自旋相关广义压缩实现几何相位门；(2) 通过独立寻址的自旋相关广义压缩实现真正的N体自旋相互作用；(3) 通过自旋读出实现振子测温；(4) 利用自旋相关广义压缩与中电路测量制备高保真振子量子态。这些方案统一依赖于广义压缩相互作用的几何相位特性。

Result: 研究表明广义压缩相互作用诱导的几何相位对振子的Fock占据数和广义压缩的相互作用阶数呈非线性依赖，这一特征是实现各应用方案的关键机制。该工作为在混合自旋-振子平台上实现快速、高保真的离散变量和连续变量量子计算与传感奠定了理论基础。

Conclusion: 广义压缩相互作用为混合自旋-振子量子处理器提供了新的应用范式，其非线性几何相位特性可支持量子门、多体相互作用模拟、精密测温以及高保真量子态制备等多种功能，具有广泛的量子信息处理应用前景。
...
### [75] [Dynamical Lie Algebras Cannot Describe Shallow QAOA: Cragged Terrains, Barren Plateaus, and Empirical Hardness Models](https://arxiv.org/abs/2608.04252)
*Harrison Copp,Charlton Li,Anžej Margeta-Cacace,Amy Qiao*

Main category: quant-ph

TL;DR: 本文通过大规模数值实验（约23,000个问题实例）发现，QAOA在最大独立集问题上的浅层电路（特别是常数深度）中，DLA理论预测的贫瘠高原（barren plateaus）实际上非常罕见，而损失景观方差随系统尺寸多项式增长的"崎岖地形"（cragged terrains）在各类图族中普遍存在。DLA基于酉设计中心主义的渐近预测在浅层QAOA中失效，作者提出经验硬度模型作为权宜方案。


<details>
  <summary>Details</summary>
Motivation: 基于动力李代数（DLA）理论预测变分量子算法的损失景观会出现指数级消失的梯度/方差，但这一理论是否适用于浅层电路场景缺乏验证。作者希望在QAOA-MIS的浅层电路设置下检验DLA预测的适用性，并探索更可靠的损失景观分析方法。

Method: 作者在约23,000个问题实例上开展大规模数值研究，涵盖多种图族（包括一般低对称性随机图和高度对称的顶点传递图），统计QAOA-MIS损失景观的方差缩放规律。同时训练经验硬度模型（empirical hardness models）来预测实例级硬度指标，并评估其泛化能力及对景观缩放类别的分类精度。

Result: （1）DLA理论预测的贫瘠高原在浅层QAOA-MIS中很少出现；（2）损失景观方差随系统尺寸多项式增长（崎岖地形）在各类图族中普遍存在；（3）经验硬度模型虽整体泛化能力差，但能以高保真度正确区分贫瘠高原与崎岖地形两类景观缩放类别。

Conclusion: 浅层QAOA-MIS是一个典型的反例，表明基于渐近酉设计中心主义的DLA预测可能根本不足以描述更广泛的浅层变分量子算法。研究强调了发展更多基于经验的VQA损失景观模型的必要性。
...
### [76] [Poisson-Compiled Quantum Singular Value Transformation for Power-Exponential Dissipation](https://arxiv.org/abs/2608.04263)
*Chao Wang,Xi-Ning Zhuang,Menghan Dou,Zhao-Yun Chen,Guo-Ping Guo*

Main category: quant-ph

TL;DR: 本文提出一种利用泊松求和公式实现量子算子收缩 exp(-T H^α)（H为半正定厄米算子，α>0）的新方法。通过将傅里叶采样编译成单一切比雪夫多项式，采用多项式特征值变换替代频率线性组合酉算子。针对普通QSVT的奇偶性限制，提出精确二次提升使移位信号支持所有正整数幂，并将非整数幂的固定尺度近似误差从Θ(d^{-α})改善至Θ(d^{-2α})。


<details>
  <summary>Details</summary>
Motivation: 量子模拟中需要高效实现算子收缩exp(-T H^α)，特别是非整数幂的情形。现有基于QSVT的方法受奇偶性约束限制，对于偶数幂需要使用全纯延拓且仅对偶正整数有效；同时非整数幂的近似误差收敛较慢，需要更好的构造方法。

Method: 1) 利用泊松求和得到精确的目标-尾部分解，将傅里叶样本离线编译为单一切比雪夫多项式；2) 对比对H/||H||和对移位信号2H/||H||-I的块编码；3) 对移位信号引入精确二次提升，使所有正整数幂变为整函数；4) 建立非交换Weyl-Poisson恒等式，兼容LCHS求积；5) 用同一多项式构造实现振幅-相位分离中的受控耗散族。

Result: 在相同访问和奇偶性类别下，非整数幂的固定尺度近似误差从Θ(d^{-α})提升至Θ(d^{-2α})，其中d为多项式次数。推导了大尺度固定误差和固定尺度高精度两种极限下的匹配阶界，包括输出归一化开销u_r。最近邻拉普拉斯算子给出单位归一化的移位信号，验证了方法的有效性。

Conclusion: 该方法通过切比雪夫多项式特征值变换回避了频率线性组合的局限性，二次提升技术突破QSVT奇偶性限制并显著改善近似精度，为量子算子收缩模拟提供了更高效且普适的实现方案，同时可推广至受控耗散量子动力学模拟。
...
### [77] [Measurement-based simulation of lattice gauge theory dynamics with adaptive quantum circuits on a trapped-ion processor](https://arxiv.org/abs/2608.04290)
*Hiroki Sukeno,Enrico Rinaldi,Takuya Okuda*

Main category: quant-ph

TL;DR: 本文报告了在Quantinuum H2离子阱处理器上首次实验实现基于测量的量子模拟(MBQS)用于(2+1)维Z₂规范理论的实时动力学模拟，通过消耗虚拟三维团簇态资源态并以自适应测量驱动演化，在2×2和3×3空间格点上观测到规范不变的相干演化，并通过一形式对称性征候进行后选择以抑制高斯定律违背。


<details>
  <summary>Details</summary>
Motivation: 格点规范理论的量子模拟是量子计算的标志性目标之一，但传统基于门电路的方法在硬件上受限于门深度和错误累积。MBQS作为一种新提出的架构，通过消耗模型特异的纠缠资源态来模拟哈密顿量动力学，理论上可更自然地将格点规范理论的局部约束映射为资源态的高阶形式对称性，但此前缺乏实验验证。

Method: 利用Quantinuum System Model H2离子阱处理器实现MBQS架构：在56量子比特寄存器中通过测量、重置和重新纠缠的方式，瞬时生成48和54量子比特的块，进而消耗虚拟三维团簇态（分别包含200和288个资源态量子比特），在2×2和3×3空间格点上模拟(2+1)维Z₂规范理论的实时动力学。驱动演化的测量记录同时提供一形式对称性征候，用于无额外成本的后选择，以抑制高斯定律违背并改善与理想Trotter动力学的吻合度。

Result: 在2×2和3×3空间格点上观测到了规范不变可观测量的一致（coherent）演化；由测量记录提取的一形式对称性征候支持后选择，显著抑制了观测到的高斯定律违背，并提高了与理想Trotter化动力学的整体一致性。

Conclusion: 实验证明MBQS是一种在当前硬件上可行的、对称性感知的格点场论模拟架构，为格点规范理论的高效量子模拟提供了一条有前景的路径。
...
### [78] [The Born Representation Theorem and the Unistochastic Theorem](https://arxiv.org/abs/2608.04354)
*Jacob A. Barandes*

Main category: quant-ph

TL;DR: 本文给出了两个关于随机矩阵的新定理的自包含构造性证明，证明了任何随机矩阵的每个元素都可以用广义Born规则（POVM与PVM的迹）表示，并且通过有限维扩张可以将任意随机矩阵边缘化为一个更大的单随机矩阵，从而建立了单随机矩阵相对于随机矩阵的某种优先性。


<details>
  <summary>Details</summary>
Motivation: 探索随机矩阵与量子理论之间的深层联系，特别是将经典概率论中的随机矩阵与量子力学的Born规则统一起来，并论证单随机矩阵（unistochastic）在数学结构上的基础性地位。

Method: 采用自包含的构造性证明方法。首先证明Born表示定理：每个随机矩阵的每个元素都可表示为POVM元素与PVM元素成对乘积的迹；其次证明单随机定理：通过将底层的向量空间扩张有限数量的额外维度，使得每个随机矩阵元素可以表示为两个PVM元素乘积的迹，从而通过边缘化从更大的单随机矩阵导出。

Result: 证明了两个新定理：Born表示定理（任何随机矩阵条目可用广义Born规则表示）及其推论（POVM为PVM时随机矩阵是单随机的）；单随机定理（通过有界维数扩张，任何随机矩阵都可从更大的单随机矩阵通过边缘化获得）。

Conclusion: 单随机矩阵在数学上先于（或至少不劣于）一般随机矩阵，这暗示了与量子理论中酉时间演化的密切联系，并提出了对离散时间确定性过程和马尔可夫链的潜在应用。
...
### [79] [Information locality of a quantum locally recoverable code](https://arxiv.org/abs/2608.04403)
*Ryutaroh Matsumoto*

Main category: quant-ph

TL;DR: 本文提出量子稳定子码的信息局部性概念，基于Hermitian正交构造，并提供一种线性代数过程来计算更小的修复组并最小化解码中测量的可观测量数量。作者通过显式示例证明先前提出的量子符号局部性(r,δ)会高估纠删所需的码字符号数，同时展示通过Euclidean正交构造的量子码中，经典信息局部性的自然翻译会低估所需符号数。


<details>
  <summary>Details</summary>
Motivation: 经典线性码的符号局部性定义会高估多重纠删所需的码字符号数量，因此信息局部性被提出以更精确地定义r。最近量子纠错码的局部性(r,δ)研究直接沿用了经典符号局部性的原始定义，继承了这一缺陷。本文旨在为量子稳定子码建立信息局部性概念，以弥补这一不足。

Method: 基于Hermitian正交性构造的量子稳定子码，提出了量子信息局部性的对应概念；设计了线性代数过程，在计算更小修复组的同时，将解码所需测量的可观测量数量降至最小；通过构造反例验证提出的方法。

Result: 通过显式量子稳定子码示例，证明了先前提出的量子局部性(r,δ)定义确实会高估纠删所需的码字符号数量；同时给出了另一个基于Euclidean正交性和两个不同线性码构造的量子码示例，说明经典信息局部性的直接翻译在量子场景下会低估所需符号数量。

Conclusion: 先前基于符号局部性定义的量子局部性(r,δ)存在高估问题，而经典信息局部性向量子设置的直接翻译也不总是正确。本文提出的基于Hermitian正交性的量子信息局部性及其线性代数计算方法，为量子码的局部性刻画提供了更精确的框架。
...
### [80] [Analytic correspondence between multipartite entanglement and quantum phase transitions](https://arxiv.org/abs/2608.04467)
*Huynh Le Dan Linh,Vu Tuan Hai,Le Bin Ho*

Main category: quant-ph

TL;DR: 本文推导出多体可浓缩纠缠（CE）与一维量子自旋系统中量子相变之间的解析对应关系，证明CE与广义序参量共享同一解析结构、识别相同的量子相，并可通过常数深度SWAP测试电路直接测量，从而无需模型特定序参量即可实验探测量子相变。


<details>
  <summary>Details</summary>
Motivation: 传统的量子相变探测依赖于模型特定的序参量，缺乏普适性且难以直接实验测量。作者希望建立一种通用的、模型无关的量子相变探针。

Method: 作者从解析上证明CE与广义序参量具有相同的解析性结构，能识别相同的量子相；对于高斯基态，CE完全由单粒子关联矩阵决定。在横向场Ising模型和广义cluster-Ising模型上进行数值验证，涵盖对称破缺和对称保护拓扑两种量子相变类型。CE可通过常数深度并行SWAP测试电路直接测量。

Result: 数值验证确认了所有解析预测，CE在对称破缺和对称保护拓扑量子相变中均能正确识别相变点。由于CE可以直接测量且无需模型特定序参量，它被确立为一种实验可及、模型无关的量子相变探针。

Conclusion: 这项工作建立了可浓缩纠缠作为通用量子相变探针的理论基础，将其与量子相变的解析结构与实验可测性联系起来，提供了一种不依赖特定模型序参量的普适量子相变探测方案。
...
### [81] [Heralded Non-Gaussian Squeezed-State Inputs for Parity-Detection SU(1,1) Interferometry](https://arxiv.org/abs/2608.04476)
*Lifen Guo,Qingqian Kang,Teng Zhao,Cunjin Liu,Xin Su,Liyun Hu*

Main category: quant-ph

TL;DR: 本文系统比较了光子减法、光子加法和光子催化三种非高斯操作在平衡SU(1,1)干涉仪中的计量学优势。在固定制备参数下，单光子减法和加法在高透射率区域优于高斯参考，多光子催化在低透射率区域有优势；但当在固定条件探针能量和固定增益下独立优化资源分配时，三种非高斯操作的成功加权Fisher信息均低于优化的高斯基准。


<details>
  <summary>Details</summary>
Motivation: 非高斯操作有潜力重塑连续变量探针的光子统计特性以获得计量学优势，但此前的优势比较往往未统一考虑宣告概率和光子数资源。本文旨在严谨地解决这一问题，明确非高斯增强在实际可获得的精度上是否真正优于高斯方案。

Method: 采用统一的有限透射率映射，导出任意操作阶数下的闭式条件矩和量子Fisher信息；将内部损耗吸收为一个有效奇偶可观测量的单一参数；基于该框架在平衡SU(1,1)干涉仪中对光子减法、光子加法和光子催化进行统一比较。

Result: 固定制备参数下，单光子减法和加法的条件相位信息在高透射率区域大部分优于高斯参考，多光子催化在低透射率区域有用。但固定条件探针能量和固定增益下独立优化后，三种非高斯操作的成功加权Fisher信息均低于优化的高斯基准。光子催化产生的高局部量子Fisher信息分支被暗点奇偶测量提取不足，属于测量不匹配而非态制备失败。

Conclusion: 在明确陈述的资源约束下，条件非高斯增强与实际可获得的精度之间存在尖锐边界。结论受限于测试条件：单光子操作、相干加压缩真空高斯族、固定增益和奇偶读出的架构。
...
### [82] [Impact of molecular orbital localization on quantum computational resources for Hamiltonian simulation: A benchmark study of hydrogen chain systems](https://arxiv.org/abs/2608.04481)
*Kenji Sugisaki,Yuhei Tachi,Masayoshi Terabe,Hiroyuki Tezuka*

Main category: quant-ph

TL;DR: 本文系统研究了一维氢链体系中轨道基函数（正则分子轨道CMO vs 局域分子轨道LMO）及哈密顿量截断策略（算符系数截断 vs 局部性截断）对Trotter分解哈密顿量模拟计算成本的影响，结果表明LMO+局部性截断方案可实现量子门数量呈多对数增长，较CMO+系数截断方案的类多项式增长具有指数级优势。


<details>
  <summary>Details</summary>
Motivation: 量子化学哈密顿量模拟是量子计算的重要应用场景，但Trotter分解方法的计算成本高度依赖轨道表示和哈密顿量截断策略。目前缺乏对不同轨道基函数（CMO与LMO）和不同截断方案（系数截断与局部性截断）系统性的比较分析，因此需要厘清何种组合能在保证精度的前提下最小化量子门资源开销，为大规模量子化学模拟提供方法论指导。

Method: 作者在短氢链上分别采用Hartree-Fock正则分子轨道（CMO）和Pipek-Mezey局域分子轨道（LMO）进行波函数展开，并分别施加基于算子系数的截断和基于算子局部性的截断，评估基态能量与保真度。通过经验公式拟合截断阈值与基态能量及保真度的关系，估算了实现高保真度（F≥0.99）所需的阈值，并在此基础上对长至H100的氢链进行量子门资源估算。

Result: 研究发现：CMO波函数展开下，算符系数截断在减少门成本方面更有效；而LMO波函数展开下，算符局部性截断更为有效。量子门资源估算表明，CMO+系数截断方案中门数量呈多项式增长，而LMO+局部性截断方案中门数量呈多对数（polylogarithmic）增长，后者实现了指数级优势。

Conclusion: LMO波函数展开与哈密顿量局部性截断的组合为大规模量子化学Trotter模拟提供了最优的资源效率方案，该研究为在不同量子化学模拟场景中选择轨道表示与哈密顿量截断策略提供了实用指南。
...
### [83] [Quantum-information fingerprints of partial dynamical symmetry in the interacting boson model](https://arxiv.org/abs/2608.04486)
*Dhritimalya Roy*

Main category: quant-ph

TL;DR: 本文提出部分动力学对称性(PDS)的一种纯结构特征——标量方差Var[C[G]]，即对称性Casimir的方差。该量在PDS的可解子集上严格为零，在混合态上为N²量级，并且在Leviatan的一阶和二阶临界点处呈现两种不同形式。与双粒子纠缠不同，该诊断量能清晰区分可解态与混合态，不随多体纠缠或magic相关。作者以$^{168}$Er为例，并展示了在量子寄存器上变分制备可解与混合本征态的步骤。


<details>
  <summary>Details</summary>
Motivation: PDS是一种介于精确对称与完全破缺之间的代数结构：一部分本征态保持好量子数且可解，其余谱混叠。目前PDS通过谱学数据、带首能量、能级系统学和B(E2)比值识别，作者追问其是否在纯结构层面（本征态内）也有可辨识的签名，而不仅仅是幅值上的纠缠。

Method: 引入对称性Casimir的方差Var[C[G]]作为诊断量，证明它等价于块相干熵(block-coherence entropy)和块杂质(block impurity)，三者在态携带单一不可约表示标签时同时为零。作者在稳定对称点和Leviatan的一阶、二阶临界点处逐态解析该量，区分其N²量级行为在两阶相变中的不同形式。同时对比双粒子纠缠度量的局限性，并在$^{168}$Er核上锚定分析，给出与块纯度/相干性的联系，最后在量子比特寄存器上通过变分方法制备可解与混合本征态。

Result: 发现PDS确实存在纯结构签名——标签方差Var[C[G]]在可解本征态上严格为零、在混合态上为N²量级，且其形式由相变阶数（一阶或二阶）决定；该量不随多体纠缠或magic变化。双粒子纠缠幅值无法分离可解态与混合态，甚至会在标签精确时漂移。以$^{168}$Er验证了分析，并完成了量子寄存器上的变分制备作为后续量子设备验证步骤。

Conclusion: 标签方差（等价于块相干熵/块杂质）为PDS提供了一种全新的、纯结构层面的逐态诊断工具，优于纠缠幅值。它在量子信息语言（相干/杂质）与核结构PDS之间建立了桥梁，并为在量子设备上评估该诊断量铺平了道路。
...
### [84] [Engineering Nanodiamonds for Quantum Sensing: Material Constraints at the Nanoscale](https://arxiv.org/abs/2608.04489)
...
TL;DR: 本文系统分析了纳米金刚石（NDs）中氮-空位（NV）色心作为可移动量子传感器的物理限制机制，重点阐述了小尺寸带来的晶格应变、表面噪声和电荷态不稳定等问题如何缩短NV自旋弛豫时间并导致颗粒间差异性，同时提出了相应的缓解策略。


<details>
  <summary>Details</summary>
Motivation: 块体金刚石中的NV色心已具备优异的量子控制与灵敏度，但将其嵌入纳米金刚石以实现复杂生物和纳米环境中的移动探测时，尺寸缩减引入了多种物理限制（晶格应变、表面噪声、电荷态不稳定、颗粒间差异），这些效应影响传感性能和多模态信号的定量解读，亟需系统性梳理其机制与对策。

Method: 本文采用结构化综述视角，系统梳理材料特性约束纳米金刚石中NV色心行为的物理机制，并归纳相应的缓解策略，以支撑其在生物传感和纳米科学中的稳健应用。

Result: 文章识别并阐明了小尺寸效应对NV色心的主要约束机制，包括增强的晶格应变、表面诱导噪声源对T1/T2弛豫时间的缩短影响、NV电荷态的不稳定性，以及自上而下制备方法引起的显著颗粒间变异性，并提出了一系列缓解策略以提升传感稳健性。

Conclusion: 理解并缓解纳米金刚石中材料特性对NV色心的物理限制，是充分发挥其作为移动量子传感器在生物传感和纳米科学中应用潜力的关键；文章为这一领域的系统化研究和工程化应用提供了结构化框架。
...
### [85] [Dynamically suppressing cavity dephasing induced by frequency fluctuations of a coupled nonlinear mode](https://arxiv.org/abs/2608.04494)
*Yunwei Lu,Xinyuan You,Ziwen Huang,Sébastien Léger,Jens Koch,Yao Lu*

Main category: quant-ph

TL;DR: 本文提出Stark-Assisted Flux-noise Evasion (SAFE)协议，通过向非线性元件施加弱失谐微波驱动，保护超导腔免受由约瑟夫森元件频率涨落引起的退相干，可将腔退相干时间延长一个数量级以上。


<details>
  <summary>Details</summary>
Motivation: 高相干超导腔是量子信息的有前景平台，具备长相干时间和可忽略的内禀退相干。但腔控制依赖于非线性约瑟夫森元件，其频率涨落会被腔继承为退相干，限制控制保真度并侵蚀纠错协议中的噪声偏置优势。

Method: 提出SAFE协议：仅通过对非线性元件施加弱失谐微波驱动来保护腔免受继承性退相干影响。以与磁通可调transmon (FTT)色散耦合的3D超导腔为具体系统，在1/f磁通噪声下进行解析分析，并通过蒙特卡罗模拟验证。

Result: 解析预测与具有现实参数的蒙特卡罗模拟一致，表明SAFE可将腔退相干时间延长一个数量级以上，同时保持残余驱动引起的退相干为次主导效应。

Conclusion: SAFE是一种硬件高效的协议，仅需弱微波驱动即可有效抑制非线性元件频率涨落对腔的退相干传播，显著提升腔的相干性能。
...
### [86] [Coupling Does Not Reduce the Auxiliary-Mode Count for $1/|ω|$ Spectra in Passive Lindblad Networks](https://arxiv.org/abs/2608.04539)
*Qing-Ao Xiang,Yan Liu,Xin-Yuan Yang,Ya-Ju Song*

Main category: quant-ph

TL;DR: 本文证明在固定模式预算下，用于近似非马尔可夫开放量子系统中1/|ω|谱的相干模间耦合并无优势：一般耦合类与无耦合对角子类共享相同的最优误差，即2N阶Zolotarev符号近似误差，且该最优值可由N个独立阻尼辅助模式在零失谐下实现。


<details>
  <summary>Details</summary>
Motivation: 非马尔可夫开放量子系统中常用有限个马尔可夫辅助模式来近似连续环境，但一个关键问题悬而未决：在固定模式数量预算下，相干模间耦合能否降低谱近似误差？这关系到辅助网络设计的效率与最优性。

Method: 考虑被动、数守恒的高斯Lindblad辅助网络，对有限双侧频带上的1/|ω|谱进行近似。理论上将一般耦合类与无耦合对角子类进行对比分析，证明两者达到相同的最优误差，并指出该误差精确等于2N阶Zolotarev符号近似误差。

Result: 证明相干模间耦合不提供任何近似优势。尽管一般耦合网络具有O(N²)个实参数，相干模间耦合、集体耗散和非正规结构均不能减少达到指定容差所需的辅助模式数。该精确关系同时给出了指定正频率动态范围和容差下的最小模式数，以及指定模式预算和容差下可达到的最大动态范围。

Conclusion: 对于近似1/|ω|谱的问题，N个独立的零失谐阻尼辅助模式已是最优选择；一般耦合类不会带来超越对角子类的收益。该结果适用于辅助网络处于稳态真空态、系统通过单一厄米浴算符耦合且无白噪声直通项的条件。
...
### [87] [Quantum annealers as programmable thermal machines](https://arxiv.org/abs/2608.04564)
*Jakub Pawłowski,Tomasz Śmierzchalski,Fengping Jin,Bartłomiej Gardas,Sebastian Deffner,Zakaria Mzaouali*

Main category: quant-ph

TL;DR: 本文提出将D-Wave量子退火机表征为一个闭合热力学循环，通过初始和最终Ising能量及拟合的有效温度，获得熵产生、环境能量交换、功和功率的下界，并据此将不同运行条件划分为加热器、加速器、制冷器和引擎四种热力学模式，从而为量子优化和采样提供超越单纯解质量和运行时间的能量感知评估框架。


<details>
  <summary>Details</summary>
Motivation: 量子退火机的性能通常仅以优化质量和运行时间报告，忽略了计算过程中交换的能量。这种信息缺失使得不同运行模式（如正向退火与逆向退火）之间的能耗差异无法量化，也无法从能量角度理解设备行为对编程目标函数的影响。

Method: 将D-Wave量子退火机建模为闭合热力学循环，从输出分布拟合有效温度，结合初始与最终Ising能量推导熵产生、环境能量交换、功和功率的下界；通过改变初始分布的制备和逆向退火转折点，在一维链及高连通实例上系统映射热力学模式，并在Advantage和Advantage2硬件上进行验证。

Result: 识别出加热器、加速器、制冷器和引擎四种兼容的热力学模式；处理器能量变化能反映编码优化问题的平均目标改进或恶化；拟合温度提供了概率集中于低能态的强度的操作性度量；热力学模式补充了解质量或运行时间无法提供的信息，可区分受驱动细化、净加热和热泵效应。

Conclusion: 该框架将量子优化、概率计算、统计物理模拟、硬件诊断和能量感知评估联系起来，但强调热力学标签本身不能单独决定计算性能，需与计算指标结合使用。
...
### [88] [Resource Estimation for Fault-Tolerant Quantum Programs](https://arxiv.org/abs/2608.04573)
*Bonan Su,Yuan Feng,Li Zhou,Mingsheng Ying*

Main category: quant-ph

TL;DR: 本文提出了一种面向容错量子计算的量子编程语言及其资源估算框架，通过程序员可见的纠错方案抽象和跨层程序-硬件分析，在保持可编程性的同时实现高效资源利用与细粒度资源估算。


<details>
  <summary>Details</summary>
Motivation: 容错量子计算中纠错开销巨大，资源估算是核心问题。现有量子编程语言要么要求程序员处理底层硬件细节导致实现繁琐，要么过度抽象纠错方案导致资源利用和估算效率低下，缺乏兼顾可编程性与资源优化能力的方案。

Method: 设计一种新的量子编程语言，提供程序员可见的纠错方案抽象（programmer-visible abstractions of error-correction schemes），支持跨层程序-硬件分析（cross-layer program-hardware analysis），从而系统化探索资源权衡。

Result: 在包括现有框架通常视为黑箱组件在内的大型量子算法的详细容错实现上进行评估，结果表明该框架能够实现显著的资源节省，并为容错量子程序提供详细、细粒度且准确的资源估算。

Conclusion: 所提出的框架在保持可编程性的同时，实现了高效资源利用和全面资源分析，弥补了现有量子编程语言在容错资源管理上的不足。
...
### [89] [Measurement-induced generation of Schrödinger cat states in cavity QED](https://arxiv.org/abs/2608.04578)
*Tong Wang,Peng-Fei Wei,Hai-Jun Xing,Zhihai Wang*

Main category: quant-ph

TL;DR: 该论文提出了一种基于测量的简单协议，通过相干驱动、色散原子-腔相互作用和原子后选择，在腔QED系统中生成薛定谔猫态。数值模拟表明该方案对中等腔耗散具有鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 现有的薛定谔猫态生成协议通常依赖强非线性相互作用、复杂控制技术或工程化耗散，实验实现难度大。作者旨在提出一种更简单、实验可及的方案，无需强光学非线性或工程化耗散即可制备非经典腔态。

Method: 该方案通过三个方面实现猫态生成：(1) 相干驱动激发腔场；(2) 色散原子-腔相互作用建立原子与光子自由度之间的相干关联；(3) 原子后选择将腔场投影到具有明显Wigner负性的非高斯叠加态上。采用Lindblad主方程进行数值模拟验证方案的有效性和鲁棒性。

Result: 数值模拟显示，生成的薛定谔猫态具有显著的Wigner负性，且对中等强度的腔耗散保持鲁棒性，验证了该测量方案的可行性。

Conclusion: 条件性原子测量为制备非经典腔态提供了一种有效且实验可及的方法，无需依赖强光学非线性或工程化耗散，为连续变量量子信息处理提供了实用的猫态生成途径。
...
### [90] [Solving Differential Equations Using Continuous-Variable Quantum Annealing](https://arxiv.org/abs/2608.04601)
*Kazuki Miyanishi,Soshun Naito,Asuka Koura,Kazue Kudo,Toshiki Yamaji,Yuichiro Matsuzaki*

Main category: quant-ph

TL;DR: 该论文提出了一种基于连续变量（玻色子）量子退火的公式化方法，用于求解线性微分方程，避免了基于量子比特架构中变量离散化带来的大量量子比特开销。数值模拟表明，在考虑的条件下，该方法能够复现二阶线性微分方程的解析解。


<details>
  <summary>Details</summary>
Motivation: 现有的量子退火方法主要针对量子比特架构设计，将其应用于连续变量优化问题时需要对变量进行离散化，这会引入巨大的量子比特开销。近来提出的基于玻色子系统的连续变量量子退火提供了一个替代框架，每个优化变量可直接编码在玻色子模式（如腔模）中。本文旨在将该框架扩展到线性微分方程求解领域，从而避免离散化开销。

Method: 将微分方程求解问题重新表述为一个连续变量优化问题，并将微分方程映射为与玻色子量子退火兼容的目标函数。通过这种方式，每个优化变量直接编码在玻色子模式中，无需进行离散化。对二阶线性微分方程进行了数值模拟验证。

Result: 数值模拟结果表明，在所考虑的条件下，所提出的连续变量量子退火公式能够复现二阶线性微分方程的解析解。

Conclusion: 该工作为在不引入量子比特离散化开销的情况下求解微分方程开辟了潜在途径，展示了连续变量量子退火在科学计算中的应用前景。
...
### [91] [Efficient Depth--Ancilla Tradeoffs for Hamming Weight Computation and Symmetric Boolean Functions](https://arxiv.org/abs/2608.04627)
*Wei Zi,Pei Yuan,Junhong Nie,Shengyu Zhang*

Main category: quant-ph

TL;DR: 本文研究了量子计算中汉明权重计算（含对称布尔函数）的深度-辅助量子比特权衡问题，分析了全连接和二维最近邻网格两种量子比特连接模型下的标准电路与动态电路性能，给出了多种复杂度最优或近最优的电路构造。


<details>
  <summary>Details</summary>
Motivation: 汉明权重计算是量子计算中的基本子程序，也是对称布尔函数的核心构建模块，而对称布尔函数是量子计算中最常见的原语之一。高效的电路构造直接影响众多量子算法的效率，因此研究其在不同连接模型下的深度与辅助量子比特资源权衡具有重要意义。

Method: 针对四种场景分别设计电路结构：（1）标准全连接模型下，利用亚线性数量的辅助量子比特实现O(log n)深度；（2）标准2D网格模型下，用O(log²n)辅助量子比特实现O(√n)深度，并证明深度下界Θ(√n)为最优；（3）两种动态电路模型中，对任意固定ε>0，用O(n^{1+ε}polylog n)辅助量子比特实现常数深度。所有构造均提供平滑的深度-辅助量子比特权衡，并可推广至任意对称布尔函数。

Result: 标准全连接模型达到O(log n)深度；标准2D模型达到O(√n)深度且有匹配的下界Θ(√n)；动态电路模型实现常数深度，辅助量子比特开销为O(n^{1+ε}polylog n)。所有结果均支持平滑权衡并扩展到对称布尔函数。

Conclusion: 本文系统地刻画了汉明权重计算在不同量子比特连接模型和电路模型下的深度-辅助量子比特权衡关系，提供了最优或近最优的电路构造，为量子计算中对称布尔函数的高效实现奠定了理论基础。
...
### [92] [Effect of Cross-Spectral Correlations on Qubit Dynamics: Coherence Revival and Relaxation Modulation](https://arxiv.org/abs/2608.04672)
*Siddhartha Dutta,Sujay Mondal,Abhijit Bandyopadhyay*

Main category: quant-ph

TL;DR: 本文研究了一个量子比特同时受到纵向量子噪声（退相干）和横向噪声（弛豫）且二者通过共享玻色子库相互关联时的约化动力学。在二阶时间卷积（TCL）框架下推导了布洛赫矢量分量的闭合时域方程，数值验证了其正确性。关键发现是：跨谱项将原本独立的退相干与弛豫扇区耦合，产生无法通过独立噪声叠加复现的动力学，包括非单调的粒子数弛豫和退相干后的相干性瞬态恢复，且这些效应可由交叉谱的强度、带宽、延迟和相位控制。


<details>
  <summary>Details</summary>
Motivation: 实际量子系统中噪声通常并非单一轴方向，而是多轴噪声同时存在且可能共享同一环境来源，从而在频域上产生相关性。以往研究多将退相干和弛豫视为独立过程，忽略了二者之间可能存在的关联，导致对弱耦合体系中动力学行为的预测不完整。理解这种关联噪声如何塑造量子比特动力学，对量子信息处理和量子纠错具有重要意义。

Method: 采用正半定矩阵值谱密度描述环境涨落，其中复值非对角元编码退相干与弛豫通道在频域的关联。在二阶时间卷积（time-convolutionless, TCL）框架下推导量子比特布洛赫矢量分量的闭合时域局部方程，并通过精确纯退相干解和横向耦合自旋玻色子模型的已知行为对数值实现进行验证。

Result: 交叉谱项将退相干与弛豫扇区耦合，产生无法由独立噪声叠加复现的动力学。具体表现为：非单调的粒子数弛豫，以及初始衰减后的相干性瞬态恢复。交叉谱的强度、带宽、延迟和相位可作为控制参数，调节这些效应的幅度和时间结构，从而在弱耦合区域内提供有限时长的增强相干性窗口或抑制弛豫窗口。

Conclusion: 关联的多轴噪声可以在时间上重新分配相干性损失和能量弛豫，从而在弱耦合框架内实现暂时增强的相干性或抑制的弛豫。这为利用环境噪声关联作为量子动力学调控手段提供了新思路。
...
### [93] [Spectral evolution of two-photon emission in microresonators](https://arxiv.org/abs/2608.04684)
...
TL;DR: 该论文研究了高Q值硅氮化物微谐振器中通过四波混频产生光子对的谱相干性，追踪从自发量子态到光参量振荡的转变过程，通过结合时间相关测量和相位敏感测量，连续监测发射线宽从腔寿命受限到泵浦线宽尺度的演变。


<details>
  <summary>Details</summary>
Motivation: 硅氮化物微谐振器是可扩展量子网络中有前景的集成光子源，但其光谱相干性特性尚未被充分理解。理解发射线宽从量子自发辐射到参量振荡状态的演变过程，对于优化集成光子源在可扩展量子网络中的应用至关重要。

Method: 采用高Q值硅氮化物微谐振器作为四波混频光子对源，结合时间关联测量（表征量子相关性）和相位敏感测量（表征相干性），连续监测发射线宽的变化，追踪从自发量子过程到光参量振荡的转变。

Result: 成功追踪了发射线宽从腔寿命限制宽度向泵浦线宽尺度的连续演变过程，揭示了从自发量子辐射到受激参量振荡的谱相干性转变机制。

Conclusion: 该表征方法为优化集成光子源用于可扩展量子网络提供了重要依据，明确了不同工作状态下发射线宽的受限机制。
...
### [94] [Quantum SWITCH-induced non-Markovianity is not entirely quantum](https://arxiv.org/abs/2608.04685)
*Rajeev Gangwar,Ananda G. Maity*

Main category: quant-ph

TL;DR: 本文研究了量子SWITCH产生的记忆效应（非马尔可夫信息回流的来源），通过分析离散时间演化和开放量子系统动力学映射两种典型场景，证明这些记忆效应并非真正的量子非马尔可夫性，而是源于经典起源，从而引发对不定因果序框架下量子优势来源的重新审视。


<details>
  <summary>Details</summary>
Motivation: 量子SWITCH作为不定因果序的经典实现，已在通信、计算、计量学等领域展现出操作优势，但其物理资源来源一直不清楚。近期研究发现量子SWITCH能产生记忆效应（非马尔可夫信息回流），本文旨在厘清这些记忆效应是否反映真正的量子非马尔可夫性，还是仅源于经典效应，以帮助理解量子优势的本质来源。

Method: 作者分析了两个代表性场景：(1) 基于离散时间演化的量子SWITCH；(2) 通过开放量子系统动力学映射表述的量子SWITCH。通过对比这两种框架下的记忆效应特征，判断其是否构成真正的量子非马尔可夫性，抑或可以追溯到经典起源。

Result: 研究表明，量子SWITCH产生的记忆效应并非真正的量子非马尔可夫性，而是具有经典来源。

Conclusion: 该发现促使研究者重新审视不定因果序框架中量子优势的来源，暗示量子SWITCH的操作优势可能并不来源于其产生的"量子"记忆效应，需要重新评估其在量子信息处理中的根本物理机制。
...
### [95] [From normal Lindbladians to non-normal quantum trajectories](https://arxiv.org/abs/2608.04775)
*Shakib Daryanoosh*

Main category: quant-ph

TL;DR: 本文针对正常Lindblad生成元（其正交本征算子分解排除瞬态放大）研究了量子轨迹框架，通过将Lindbladian分解为确定性光滑与随机跳跃两部分，导出了精确的稳态平衡关系，并证明正常Lindblad算子排除异常点，且整体平均后随机耦合消除，恢复独立正交弛豫模。


<details>
  <summary>Details</summary>
Motivation: 马尔可夫开放量子系统的密度矩阵描述随系统尺寸呈指数增长，高效模拟仍是核心挑战。量子轨迹方法通过随机纯态实现替代混合态演化，但正常Lindblad生成元在轨迹层面的行为机制尚不清晰，需要深入理解其随机量子动力学如何实现Lindblad算子的全局性质。

Method: 将Lindbladian分解为确定性光滑过程和随机跳跃过程两个组成部分，利用正常Lindblad生成元的正交本征算子分解性质，推导精确的稳态平衡关系，并通过在量子轨迹层面分析Liouvillian本征模式间的随机耦合及其系综平均行为来揭示Lourinvillian正常性的本质。

Result: 推导出精确的稳态平衡关系，表明确定性光滑与随机跳跃过程的相互作用是Liouvillian正常性的机制；证明正常Lindblad算子排除异常点；尽管单个量子轨迹中Liouvillian本征模式间存在随机耦合，但系综平均后这些耦合恰好抵消，恢复独立正交弛豫模式。

Conclusion: 研究为Liouvillian正常性提供了轨迹层面的物理解释，阐明了Lindblad生成元的全局属性如何通过随机量子动力学实现，增进了对开放量子系统复杂性的理解。
...
### [96] [Constructive realization of self-referential prediction limits in quantum control: Resource bounds and Gödel-safe architectures](https://arxiv.org/abs/2608.04779)
*Salman Sajad Wani,Álvaro Perales-Eceiza,Saif Al-Kuwari,Mir Faizal*

Main category: quant-ph

TL;DR: 本文将由Wolpert提出的通用自预测不可能性定理转化为一个可以通过有限资源在实验室中明确实现的物理障碍，利用Kleene递归定理构造出可逆协议，使预测器被嵌入其自身分析对象的实验中，并强制产生与预测相矛盾的指针记录。文章同时引入“Gödel安全架构”以阻断自引用的因果路径，并讨论了其对实时量子纠错的表达能力权衡。


<details>
  <summary>Details</summary>
Motivation: 随着可编程量子控制系统越来越依赖预测模块（用于认证、实时反馈和自主决策），一个根本性问题浮现：自分析量子平台能否普遍预测自身的实验输出？Wolpert已形式化了通用自预测的不可可能性，但该限制在物理实验和工程实践中的具体含义尚不明确。作者旨在将该数学抽象转化为可实验验证的、面向量子硬件的具体约束。

Method: 作者将Wolpert的不可可能性转化为显式的实验室阻碍：在可编程量子控制设置中，允许预测器作为其分析对象的子程序嵌入。利用Kleene递归定理构造对角化协议，将任意确定性有界时间预测器编译为可逆协议（编码其自身规格），该协议在逆转后调用预测器，并确定性产生一个与预测矛盾了的经典指针记录。同时，作者定义了Gödel安全架构，阻断从协议描述到能够影响同一轮运行中指针的致动器的因果路径。

Result: 对于高效预测器，编译过程具有多项式开销，并可实现为容错量子电路和最小Mach-Zehnder干涉仪等具体物理实现。作者分析了Gödel安全架构对实时量子纠错的影响，包括由此产生的表达能力权衡。

Conclusion: 随着量子控制回路计算表达能力的增长，自引用的极限从纯数学抽象转变为自主量子技术可靠运行的明确工程约束。Gödel安全架构提供了一种在阻断自引用因果路径的框架，但牺牲了部分表达能力，这为量子系统设计提供了新的指导原则。
...
### [97] [Demonstrating advantages of dynamic quantum circuits on a hybrid superconducting qubit-cavity processor](https://arxiv.org/abs/2608.04780)
...
TL;DR: 本文在单个混合超导量子比特-腔处理器上展示了动态量子电路（DQC）的优势，实现了一个10比特Bernstein-Vazirani算法（82%成功率）、一个8比特量子相位估计协议（误差低于10^-3）以及首个在超导平台上动态电路实现的Shor算法（分解15，在所有互质基上的平方统计重叠值超过99.8%），为未来DQC实现提供了具体基准。


<details>
  <summary>Details</summary>
Motivation: 静态量子电路在物理量子比特开销和电路拓扑方面存在局限，而动态量子电路（DQC）通过中间测量、量子比特重置与重用以及经典前馈控制，有望提供更高效的量子计算路径。然而，DQC的优势需要在具体的硬件平台上进行系统性验证和基准测试。

Method: 采用混合超导量子比特-腔处理器架构：高维腔量子比特（qudit）作为计算寄存器，色散耦合的超导transmon辅助量子比特（ancilla）被反复测量、重置和重用以实现动态控制。在该架构上实现了三个复杂度递增的算法层级：10比特Bernstein-Vazirani算法、8比特量子相位估计协议以及动态电路版本的Shor算法。

Result: 10比特Bernstein-Vazirani算法平均成功率达82%，在规模和性能上超越现有最先进的动态和静态实现；8比特量子相位估计协议估计误差低于10^-3；首次在超导平台上实现动态电路版本的Shor算法，成功分解15，在所有互质基上的平方统计重叠值超过99.8%。

Conclusion: 研究结果为未来DQC实现提供了具体基准，凸显了混合量子比特-量子比特架构下DQC的多方面优势，确立了其作为迈向可扩展、可编程量子计算的有前景路线的地位。
...
### [98] [High-cooperativity coupling and spin-resolved extinction of tin-vacancy centers in a diamond-like microcavity](https://arxiv.org/abs/2608.04797)
...
TL;DR: 本文展示了金刚石中锡空位（SnV）色心与全可调法布里-珀罗微腔在1K温度和原位磁场控制下的腔增强光-物质耦合。通过集成低粗糙度金刚石薄膜实现类金刚石混合腔模，有效Purcell因子达到C₀=4.1(1)，较类空气模提升两倍以上，并实现了96%的消光对比度、相干协同性C=4.0(14)以及自旋分辨腔消光（对比度0.91）。


<details>
  <summary>Details</summary>
Motivation: SnV色心是量子网络中有前景的自旋-光子界面，具有良好光学性质和1K以上的自旋相干性，但需要腔增强来提升光子-发射体耦合效率，充分发挥其潜力。

Method: 采用全可调的法布里-珀罗微腔，工作在低至1K温度并具备原位磁场控制；通过集成低粗糙度金刚石薄膜实现混合腔模的类金刚石区域，使场集中在金刚石内部以最大化Purcell增强；进行共振探测和磁场下的自旋分辨腔消光实验。

Result: 类金刚石模的等效Purcell因子达到C₀=4.1(1)，是类空气模（C₀=1.85(5)）的两倍以上，同时放宽了机械稳定性要求；共振探测显示96%的消光对比度和C=4.0(14)的相干协同性；磁场下实现自旋分辨腔消光，自旋选择光学跃迁对比度达0.91。

Conclusion: 结果表明，金刚石中SnV色心与开放式法布里-珀罗微腔的耦合是高效自旋-光子界面的有前景平台，具备腔增强、自旋分辨和低温操作的综合优势。
...
### [99] [Motional refocusing for trap-off Rydberg gates](https://arxiv.org/abs/2608.04812)
*Yoav Sagi,Ofer Firstenberg,Nir Davidson,Guy Raz*

Main category: quant-ph

TL;DR: 本文提出一种基于光镊强度调制的运动重聚焦（motional refocusing）协议，在谐振近似下精确消除中性原子Rydberg纠缠门中释放-再捕获过程引入的加热效应，并推广到各向异性三维陷阱及弱非谐陷阱的复合序列，将残余加热标度从U₀⁻²改善到U₀⁻⁴。


<details>
  <summary>Details</summary>
Motivation: 在光镊阵列中执行Rydberg纠缠门通常需要关闭陷阱光，每次门操作都包含释放-再捕获循环，这会加热原子运动并限制电路深度。现有的方法无法在不增加复杂装置的情况下消除这一加热来源，因此亟需一种仅利用陷阱光强度调制的方案来精确补偿该加热效应。

Method: 作者开发了一种运动重聚焦协议，在谐振近似下通过对陷阱光进行可编程强度切换来闭合每个匹配谐振模式的释放-再捕获循环，使任意运动布居和相干恢复到静态陷阱下的普通演化。他们以闭式形式导出了任意捕获深度的恢复序列，并在有界陷阱强度下证明了该序列是唯一时域最优解。随后将谐振理论拓展到两个方向：(1)构造精确的公共强度恢复序列以同时重聚焦多个非简并谐振模式（包括径向-轴向和完全各向异性三维陷阱）；(2)推导复合序列以消除四次方非谐性诱导的一阶运动跃迁，将残余加热律从U₀⁻²改善到U₀⁻⁴。最后通过真实高斯镊子中的波包模拟验证了理论。

Result: 波包模拟验证了解析理论，并量化了非谐性、有限开关斜坡、陷阱椭圆率和控制误差的残余效应。应用于代表性铯Rydberg门时，该协议将主导再捕获加热抑制到非谐性下限，并防止与运动有关的Doppler贡献随电路深度增加而增大。

Conclusion: 该框架提供了一条实用的路径，仅通过陷阱强度调制即可实现无加热的陷阱关闭中性原子门，有望显著延长量子电路的深度和保真度。
...
### [100] [Quantum States Protection under Environmental Noise](https://arxiv.org/abs/2608.04822)
*Kai Wang,Zhen-Yang Peng*

Main category: quant-ph

TL;DR: 本文提出并系统分析了基于量子滤波器电路结构的量子态保护方案，用于抑制振幅阻尼（AD）噪声。该方案无需完整的量子纠错码，即可显著提升量子态保真度，且对具有固定量子汉明权重的多体量子比特态可实现完美保护。


<details>
  <summary>Details</summary>
Motivation: 所有现实的量子系统都不可避免地与环境接触，环境噪声（尤其振幅阻尼噪声）对量子态的影响是当前量子技术中的关键挑战。传统的量子纠错方案资源开销大，因此需要一种资源高效、无需完整纠错的量子态保护方案。

Method: 基于电路结构"量子滤波器"，系统分析了使用单控制比特和多控制比特的滤波电路。通过滤波电路对振幅阻尼噪声进行过滤，研究其对量子态保护保真度的提升能力以及成功概率的保持效果。同时对多体量子比特态，分析了固定量子汉明权重态的保护特性以及最大汉明权重差对保真度下界的设定。

Result: 单控制和多控制量子比特的滤波电路均能增强量子态保护保真度并保持较高的成功概率。对于多体量子比特态，固定量子汉明权重的态可被完美保护免受AD噪声影响，而汉明权重差最大的态则确定了可实现保护保真度的下限。

Conclusion: 该工作提供了一条资源高效的量子态保护途径，无需完整的量子纠错码即可实现对振幅阻尼噪声的有效抑制，为实际量子技术中的噪声抑制提供了实用的方案。
...
### [101] [Quantum walker trapped by self-similarity of the Sierpiński carpet](https://arxiv.org/abs/2608.04844)
*Tomasz Sowiński*

Main category: quant-ph

TL;DR: 本文研究了量子粒子在具有Sierpiński地毯分形结构的有限大小方格子上的动力学行为，并与相同大小的均匀格子进行比较。研究发现，在均匀格子上粒子以弹道方式到达对角，传播时间与格子尺寸成正比；而在Sierpiński格子上，粒子越来越被束缚在初始位置附近，这种束缚以自相似方式在整个角区域层级中建立。


<details>
  <summary>Details</summary>
Motivation: 理解量子粒子在分形结构上的输运与传播动力学，特别是与均匀晶格相比，分形结构如何影响量子粒子的扩散和局域化行为。Sierpiński地毯作为一种经典的分形结构，其自相似特性对量子动力学的影响值得深入探索。

Method: 在有限大小的方形格子上构建Sierpiński地毯分形结构，将粒子初始局域在格子的一个角落，使用区域积分概率（zone-integrated probabilities）监测粒子出现在初始角落和相对角落附近的概率，从而在不同分形阶数之间进行一致比较。

Result: 均匀格子上粒子以弹道方式到达相对角落，传播时间与格子尺寸成正比；而在Sierpiński格子上，粒子越来越被限制在初始位置附近，且这种束缚效应以自相似方式在格子的整个角区域层级中逐步建立起来。

Conclusion: Sierpiński地毯分形结构显著改变了量子粒子的传播动力学，导致粒子从弹道输运变为近乎局域化，且这种局域化具有自相似的层级结构特征。
...
### [102] [Disentanglement in the macroscopic limit](https://arxiv.org/abs/2608.04858)
*Eyal Buks*

Main category: quant-ph

TL;DR: 本文探讨了自发解纠缠假说在宏观极限下的表现，通过具有已知精确解的多体模型发现非局部纠缠在宏观极限下不稳定，而局部纠缠的稳定性未被排除，从而验证了该假说能够弥合微观量子世界与宏观经典世界之间的鸿沟。


<details>
  <summary>Details</summary>
Motivation: 量子力学基础中存在一些悬而未决的问题，尤其是量子测量问题，这促使研究者提出自发解纠缠假说（基于修改的薛定谔方程，加入非线性项）。本研究旨在探索该假说在宏观极限下的行为，以检验其能否解释从微观量子性到宏观经典性的过渡。

Method: 采用具有已知精确解的多体模型来研究自发解纠缠在宏观极限下的表现，具体分析非局部纠缠和局部纠缠在宏观极限下的稳定性。

Result: 在所研究的模型中，非局部纠缠在宏观极限下变得不稳定；而局部纠缠在宏观极限下的稳定性并未被排除。

Conclusion: 这些发现表明，自发解纠缠假说能够作为连接微观量子世界与宏观经典世界的桥梁，为理解量子测量问题提供了新的视角。
...
### [103] [Universal Scaling of the Minimum Error Probability in Qualification of Quantum States](https://arxiv.org/abs/2608.04870)
*Zhaoyu Fei,Yaotian Li,Weicheng Huang,Xiaoguang Wang,Y. M. Du*

Main category: quant-ph

TL;DR: 本文研究了量子态的"资格判定"问题（即判定未知量子态属于两个参数区域中的哪一个），将其形式化为复合量子假设检验，揭示了最小错误概率的普适标度律：不相交区域下按N^{-3/2}exp(-Nξ)标度，相邻区域下按(NF)^{-1/2}标度，并以最小错误概率作为序参量揭示了这两种标度行为之间的转变构成N→∞时的二级相变。


<details>
  <summary>Details</summary>
Motivation: 量子态资格判定在量子信息处理中具有重要应用（如偏振方向判定、纯度判定等），但传统量子态层析在大型量子系综中代价过高。作者希望找到一种无需完整态层析即可判定量子态所属参数区域的方法，并揭示其误差率随拷贝数N的普适标度规律，为有限样本下大规模系综的量子态资格判定奠定理论基础。

Method: 将量子态资格判定表述为复合量子假设检验问题，利用N拷贝置换对称性和参数区域的几何对称性来确定最优测量和"最差配对态"（worst pairwise states）。通过量子Chernoff散度和量子Fisher信息这两个信息论量来刻画最小错误概率的指数衰减和幂律衰减行为。

Result: 对于不相交的参数区域，最小错误概率按N^{-3/2}exp(-Nξ)标度衰减，其中ξ为最差配对态的量子Chernoff散度；对于相邻区域，错误性质按(NF)^{-1/2}标度衰减，其中F为最差配对态的量子Fisher信息。以最小错误概率为序参量，两种标度行为之间的转变在N→∞时表现为二级相变。

Conclusion: 该方法使得无需完整态层析即可判定量子态是否属于给定集合，且可在有限样本条件下对大型量子系综进行资格判定，为量子态验证与量子计量学提供了新的理论工具。
...
### [104] [Quantum-Limited Distance Estimation in Three-Dimensional Optical Superresolution](https://arxiv.org/abs/2608.04876)
*Junyan Li,Shengshi Pang*

Main category: quant-ph

TL;DR: 本文推导了三维空间不变成像系统中两个非相干点源完整距离估计的量子极限精度，发现亚瑞利区域距离信息仍然有限，由点扩散函数的二阶位移响应张量决定。


<details>
  <summary>Details</summary>
Motivation: 传统成像在瑞利极限以下对分离度的敏感性消失，但这并不代表光学场中信息的基本丢失。然而，三维成像系统中估计两个非相干点源物理距离的量子极限以及其与点扩散函数空间结构的依赖关系仍然 largely unknown，因此需要从量子信息理论角度揭示这一极限。

Method: 推导了具有任意强度不平衡的两个非相干点源在三维空间不变成像系统中的全距离估计量子极限精度。引入点扩散函数的二阶位移响应张量，通过其本征系统确定两源间最优相对取向，并利用点扩散函数的反射对称性简化最优取向的识别。对一般三维高斯点扩散函数，响应张量正比于空间协方差的逆矩阵。

Result: 证明了在亚瑞利区域距离信息仍然有限，由点扩散函数的二阶位移响应张量所决定；响应张量的本征系统决定了两个源间的最优相对取向；坐标不变的几何结构提供了通过物理旋转各向异性成像系统对准最优主响应方向来提升分辨率的直接策略；高斯点扩散函数下建立了量子极限距离精度与高斯几何之间的直接联系。

Conclusion: 量子超分辨在三维成像中可以突破瑞利极限的限制，距离估计的量子极限由点扩散函数的几何结构（位移响应张量）决定，且这一几何结构具有坐标不变性，为实际设计高分辨率三维成像系统提供了理论指导。
...
### [105] [Complementary Quantum Correlations Are Universal for Qubits](https://arxiv.org/abs/2608.04916)
*Jinbo Wang,Qihang Wang,Kun Chen*

Main category: quant-ph

TL;DR: 本文证明对于任意两量子比特（qubit）态，在两个互补局域基下观测到的互信息之和被量子互信息（总关联）所约束，从而避免了维度大于2时互补测量关联计数中"重复计算"经典关联的问题。该结果表明，通过两组互补测量表可以无需量子态层析即可得到总关联的下界，且当该分数超过1比特时，可定量认证单向纠缠蒸馏速率；应用于量子信道的Choi态时，还可下界其量子容量。


<details>
  <summary>Details</summary>
Motivation: 从量子系统提取总关联通常需要对量子态进行完整的层析重构，但许多实验只能访问有限的测量设置。一种可能的捷径是累加从互补测量中获得的互信息，但在维度大于2时此方法会重复计算部分经典关联。因此，亟需确定在何种维度下、何种条件下这种互补测量关联记账方式是有效且无重复的。

Method: 作者将保护机制追溯到Bloch球上的二元熵曲率，并利用量子比特信息排除权衡（information-exclusion tradeoff）结合局部退相干下的数据处理不等式完成了证明。具体地，对于任意两qubit态，证明其在两个互补局域基中观测到的关联总和不会超过测量前的量子互信息。

Result: 论文确立了两qubit系统不受互补测量关联重复计数问题的保护性质。两个互补的相关性表可提供无需层析的总关联下界；当关联分数超过1比特时，可定量地认证单向纠缠蒸馏速率；将该方法应用于qubit信道的Choi态时，相同的数据可为其量子容量提供下界。

Conclusion: 该定理同时揭示了互补性的一种操作性用途，并确定了在二维系统中其关联记账是可信的适用场景，为无需全态层析的关联度量与量子容量估计提供了理论支撑。
...
### [106] [From Promise to Practice: Closing the Application Gap in Quantum Computing](https://arxiv.org/abs/2608.04936)
*Nicole Holzmann*

Main category: quant-ph

TL;DR: 本文从Perspective角度论证量子计算的发展需要"自底向上"（从物理、硬件、算法出发）与"自顶向下"（从工业需求与约束出发）两条路径同等重要且持续耦合，并提出通过建立团队技术接口与中间翻译层来实现这种双向协同设计。


<details>
  <summary>Details</summary>
Motivation: 量子计算作为深度技术，仅靠数学层面的算法加速（自底向上）不足以推动工业落地；同时计算不可任意求解，工业介入必须锚定算法可解性。两者的割裂源于历史叙事与专业知识的结构性分离，需要论证建立双向耦合的紧迫性与可行性。

Method: 以Perspective形式展开分析：首先论证双向视角（bottom-up从物理/硬件/算法出发，top-down从工业决策环境出发）的同等原初性与持续耦合必要性；随后剖析历史叙事与专业性结构分离如何阻碍这种耦合；最后提出建设路径——建立技术团队与领域情境的显式接口，以及将量子输出转化为决策相关可观测量（decision-relevant observables）的中间翻译层。

Result: 指出量子计算的机会最清晰地出现在"深层物理建模"与"高价值决策"的交汇处（如药物发现中的候选物筛选、飞机气动外形设计），但前提是领域两端必须从一开始就协同设计（co-design），而非事后拼接。

Conclusion: 量子计算的可持续进步要求双向发展的同等优先与持续耦合；建立技术-领域接口和中间翻译层是弥合断层的关键机制，最终协同设计是兑现量子计算工业价值的必要条件。
...
### [107] [Sharp Continuity of Petz and Sandwiched Rényi Conditional Entropies](https://arxiv.org/abs/2608.04947)
*Hao-Chung Cheng,Po-Chieh Liu*

Main category: quant-ph

TL;DR: 本文确定了优化Petz与sandwiched Rényi条件熵在迹距离下的sharp模量连续性界，对所有阶数α∈[1/2,1)成立，并证明了该界可由各向同性态对达到。证明通过线性化凹Rényi泛函、Schmidt秩支配、迹距离对偶和非交换校准估计完成。


<details>
  <summary>Details</summary>
Motivation: 量子条件熵及其Rényi推广在量子信息论中具有核心地位，但其在迹距离下的最优连续性界此前未知。该工作旨在填补这一空白，并为α↑1时恢复Berta等人的sharp量子条件熵连续性界提供统一框架。

Method: (1) 在由各向同性等式族确定的比较点处线性化相关的凹Rényi泛函；(2) 利用Schmidt秩支配将等式几何推广到任意锚态；(3) 通过迹距离对偶性以及一个非交换校准估计来控制扰动项和锚项，且不削弱sharp常数；(4) 校准估计需要矩阵分析工具。

Result: 对任意两个迹距离为δ的二分态，两种Rényi条件熵之差至多为(1/(1-α))·log[(1-ε)^α+(D-1)^{1-α}ε^α]，其中ε=min{δ,1-1/D}，D为有效维度（第一子系统维度乘以最大Schmidt秩）。对每个距离约束δ∈[0,1]，界由具有最大纠缠锚态的等向性态对达到；α↑1时恢复Berta等人的结果。

Conclusion: 本文给出了优化Petz与sandwiched Rényi条件熵在α∈[1/2,1)上的sharp模量连续性界，界可由各向同性态对饱和达到，并统一推广了量子条件熵的已知连续性结果。证明了在给定距离约束下的最优性。
...
### [108] [Diffusive Speed Limits for U(1)-Covariant Quantum Error Correction](https://arxiv.org/abs/2608.04953)
*Jianqi Sheng*

Main category: quant-ph

TL;DR: 本文研究量子纠错编码器在几何局域性和U(1)电荷守恒约束下的快速制备问题，通过互补信道几何、电荷扇区Haar分析和门分辨连通矩展开，证明电荷Haar码可达到相邻电荷下界，并推导出局部数守恒砖墙电路中编码时间与扩散现象的严格下界。


<details>
  <summary>Details</summary>
Motivation: 可扩展量子存储需要快速制备量子纠错码，但几何局域性和U(1)电荷守恒会带来不可避免的输运约束，亟需从理论上理解这些约束对编码时间的根本限制。

Method: 将互补信道几何、电荷扇区Haar分析和门分辨连通矩展开三者相结合，研究一维协变编码器在标记擦除下的表现。针对局部数守恒砖墙电路，分析逻辑电荷扩散对编码时间的影响，并利用经典分量混合界与源限制低支撑算子扩散问题来约束完整信道。

Result: 电荷Haar码达到通用相邻电荷下界（误差指数小），得到精确的n^{-1/2}广泛擦除律和锐利半擦除相变。证明局部数守恒砖墙电路中存在Ω(n^2)编码时间下界，以及经典分量的O(n^3)混合上界，将完整信道上界归约为源限制低支撑算子扩散问题。

Conclusion: 这些结果将扩散确立为对称约束下量子编码的操作性极限，并建立了精确编码形成时间的推导路径，对量子纠错码的快速制备具有重要理论指导意义。
...
### [109] [Quantum Bayes Correlated Equilibrium and the Comparison of Quantum Information Structures in Games](https://arxiv.org/abs/2608.04973)
*Furkan Sezer*

Main category: quant-ph

TL;DR: 本文将Bergemann和Morris(2016)的信息结构比较理论推广到量子领域。信息结构被建模为以支付状态为索引的密度算子族，中介者观测之。作者证明服从性等价于某个玩家子系统上的Loewner序支配关系，均衡集是非空紧致谱面体，可通过半定规划计算，经典结构精确嵌入，并在量子个体充分性条件下，更多信息在每个博弈中都缩小均衡集。


<details>
  <summary>Details</summary>
Motivation: Bergemann和Morris(2016)建立了经典信息结构比较的完备理论，证明一个信息结构比另一个更具信息量当且仅当它在每个博弈中诱导出更小的Bayes相关均衡(Bayes Correlated Equilibrium, BCE)结果集。本文的动机是将这一深刻的比较静态框架推广到量子信息结构，探索量子博弈中信息量比较的类似刻画。

Method: 作者将经典信息结构替换为量子信息结构——由支付状态索引的密度算子族。关键创新在于：(1) 将服从性(obedience)条件等价转化为某个玩家子系统上的Loewner序矩阵不等式，从而将均衡问题归结为半定规划；(2) 建立量子BCE均衡集为非空紧致谱面体(spectrahedron)；(3) 证明经典信息结构作为量子结构的特例精确嵌入；(4) 引入量子个体充分性(quantum individual sufficiency)概念，建立量子意义上的信息量增加=>均衡集缩小的单调性定理。

Result: 文章证明了三项主要结果：(1) 量子BCE均衡集等价于满足Loewner序约束的谱面体，可通过半定规划有效计算；(2) 经典信息结构在量子框架中被精确保持，实现了经典理论的兼容性；(3) 在量子个体充分性条件下，信息结构的精化(refinement)导致每个博弈中均衡集单调收缩，将Bergemann-Morris的比较静态定理推广到量子情形。

Conclusion: 本文建立了量子Bayes相关均衡和信息结构比较的完整理论框架，证明经典信息序理论可以在量子机制设计中自然推广，且均衡集的收敛性与可计算性均得到保留。这为量子信息和博弈论的交叉研究提供了严格的理论基础。
...
### [110] [Universal linear manipulation via routing and projective measurements](https://arxiv.org/abs/2608.05003)
*Alessio Baldazzi,Sonia Mazzucchi,Lorenzo Pavesi*

Main category: quant-ph

TL;DR: 本文提出了一种基于投影测量的新型路由方案，用于在经典相干光或单光子上实现通用酉变换，采用线性或树状几何结构，仅需最少组件和单个探测器即可完成N端口多端口干涉仪的复用。该方案被推广到散射玻色子采样实验中的多光子态，并验证了其对损耗和相位噪声的鲁棒性。


<details>
  <summary>Details</summary>
Motivation: 传统的Reck和Clements方案虽然可以实现完全可重构的通用多端口干涉仪，但需要大量组件和复杂的几何布局。作者希望通过引入基于投影测量的路由方案，以最少的组件数量实现通用酉变换，从而简化实验装置并降低实现成本。

Method: 作者提出了利用单探测器上的多次投影测量来实现通用酉变换的路由方案，支持线性和树状两种几何结构。该方案首先针对经典相干光和单光子场景设计，然后通过多路由方案推广到任意多光子态，用于散射玻色子采样实验。最后进行了与通用方案在损耗和相位噪声下的鲁棒性对比测试。

Result: 成功展示了以最少组件数在单个探测器上通过多次投影测量实现通用酉变换的可行性，并验证了该路由方案在多光子散射玻色子采样场景中的适用性。鲁棒性测试表明该方案在损耗和相位噪声条件下的表现优于或相当于传统通用方案。

Conclusion: 路由方案为多端口干涉仪的通用酉变换实现提供了一种更简洁、组件更少的替代途径，在资源效率和鲁棒性方面具有优势，有望推动量子光学和光子学实验的简化和扩展。
...
### [111] [Neutral Atom Quantum Computing: Principles, Routes, Progress, and Challenges](https://arxiv.org/abs/2608.05010)
*Junchao Wang,Zeyuan Wang,Lei Li,Feng Wang,Shibo Liang,Keduo Yan*

Main category: quant-ph

TL;DR: 本文系统综述了中性原子量子计算的发展全景，从2000年的理论基础到2026年的最新成果，涵盖工作机理、技术路线、关键突破（如6100原子量子比特阵列、3000量子比特连续运行、容错架构等）以及核心瓶颈分析，为学术界和产业界提供系统性参考。


<details>
  <summary>Details</summary>
Motivation: 中性原子量子计算凭借激光捕获中性原子作为量子比特并通过里德堡态相互作用实现量子逻辑门操作，已成为量子计算硬件领域最具活力的方向之一。作者旨在为该领域提供一份系统性综述，梳理从理论奠基到最新突破的发展脉络，并深入剖析阻碍产业化的核心瓶颈，以指导后续学术研究和技术开发方向。

Method: 本文采用系统性文献综述方法，从以下维度展开：（1）工作原理层面，涵盖量子比特编码、原子俘获与操控、里德堡态及其相互作用、里德堡阻塞量子门机制、可重构架构下的原子重排；（2）技术路线层面，调研光镊阵列+里德堡相互作用、光学晶格、偶极阱阵列等主流方案；（3）发展历程层面，回顾2000年至2026年国内外研究进展；（4）瓶颈分析层面，聚焦可扩展性-保真度权衡、量子纠错工程实现、原子损耗与中途中止补充、激光系统工业化、控制电子学可扩展性以及长距离量子互联等关键挑战。

Result: 综述揭示了中性原子量子计算领域近年来的里程碑式进展：实现了6100原子量子比特阵列构建、3000量子比特系统连续运行、Kitaev蜂窝模型的量子模拟、环面码纠错演示、编码率超过1/2的量子纠错以及容错架构的建立，千量子比特规模的逻辑量子比特系统已初步实现。

Conclusion: 中性原子量子计算虽已取得千量子比特规模的重大突破，但仍面临可扩展性-保真度权衡、量子纠错工程实现、激光系统工业化、控制电子学扩展性及长距离量子互联等核心瓶颈。本文通过全景式梳理为该领域提供了系统性参考框架，指明了从实验室原型走向实用化量子计算的挑战与机遇。
...
### [112] [Symbol-Oriented Quantum Communication via Temporal-Mode Multiplexing](https://arxiv.org/abs/2608.05038)
*Ali Vahedi,Parsa Mahdavifar*

Main category: quant-ph

TL;DR: 本文提出并分析了一种基于LM05操作构建的量子辅助经典通信协议，将有限字母表的符号编码到时间模式上，通过比特翻转门独立作用于对应时间槽，形成并行信道上的张量积结构。该协议引入了标准LM05安全性证明未覆盖的集体攻击场景。理论分析表明，在完美损耗识别假设下，随机猜测完全恢复的成功概率存在一个上界；对于53符号字母表，乐观的1%成功界在零QBER下约为0.8公里，QBER=0.01时降至约0.6公里，实际性能受限严重。协议非独立QSDC方案，经典排序信息需加密。


<details>
  <summary>Details</summary>
Motivation: 标准LM05协议的安全性证明未能覆盖张量积结构所引入的集体攻击场景，本文旨在填补这一安全分析空白，并量化协议在实际条件下的性能边界。

Method: 利用LM05操作作为构建模块，对时间槽独立施加比特翻转门以实现符号编码，形成并行信道上LM05操作的张量积。基于该结构推导随机猜测下完全恢复成功概率的理论上界（假设完美损耗识别），并推导符号集模式的渐近集体攻击界。

Result: 推导了随机猜测恢复的成功概率上界，考虑了接收者对丢失光子的50%猜测能力；给出了53符号字母表下1%成功界距离：零QBER约0.8公里，QBER=0.01时约0.6公里，说明实际约束严重限制协议性能。

Conclusion: 该协议不是独立的量子安全直接通信方案，因经典排序信息需要加密。虽然完成了集体攻击界和性能边界的理论刻画，但可组合安全性仍存在开放挑战，且实际通信距离十分有限。
...
### [113] [Observing the Quantum Compiler through Automatic Experiment Tracking for Qiskit](https://arxiv.org/abs/2608.05041)
*Vlad Stirbu,Arianne Meijer van de Griend*

Main category: quant-ph

TL;DR: 本文提出一个受MLflow启发的Qiskit自动日志框架，自动捕获编译器溯源信息，包括编译阶段、pass级执行数据、后端特征、编译器配置和执行结果，扩展QProv溯源模型并将数据存储到MLflow Tracking Server中，以提升编译器可观测性并支持可复现的量子编译工作流评估。


<details>
  <summary>Details</summary>
Motivation: 理解量子编译技术的有效性需要看到整个transpilation过程的可见性，而不仅仅是最终电路指标。手动收集编译器溯源信息繁琐且容易遗漏，缺乏自动化的方法来捕获编译过程的完整信息。

Method: 提出了一个MLflow启发的自动日志（autologging）框架：1）扩展QProv溯源模型，加入编译器特定信息；2）自动捕获编译阶段、pass级执行数据、后端特征、编译器配置及执行结果；3）将收集的数据存储在MLflow Tracking Server中进行分析和可视化。

Result: 通过消除手动仪器化，该框架提升了编译器的可观测性，并支持量子编译工作流的可复现评估。

Conclusion: 该框架通过自动化的编译器溯源捕获，实现了对量子编译过程的完整可见性和可复现性，有助于更深入地理解和评估量子编译技术。
...
### [114] [Preparation geometry and slow-sector routing in driven Kerr resonators: an operational spectral theory of Liouvillians](https://arxiv.org/abs/2608.05046)
*Kilian Seibold*

Main category: quant-ph

TL;DR: 本文提出一种基于左右匹配本征算符的操作性谱理论，用于超越Liouvillian本征值，刻画系统模式的激发、传播与探测机制。


<details>
  <summary>Details</summary>
Motivation: Liouvillian本征值虽能决定衰减速率和振荡频率，但无法解释在特定协议中模式如何被激发、传播和探测，缺乏一套操作性的谱理论框架。

Method: 构建匹配的左右本征算符：左本征算符决定输入/源对模式的激发，右本征算符决定密度形变与读出重叠；二者乘积构成规范不变的模态权重。对玻色子系统，相干制备将左本征算符转化为相空间激发映射（零点指示模式选择性抑制），右本征算符给出对应的Wigner形变。解析慢子空间定义操作性坐标，在正性与Markov可允许性成立时构造投影路由生成元。

Result: 在驱动Kerr谐振子系统中，框架识别了抑制切换模式的制备方案，分离了对称性分辨的弛豫通道，并揭示投影多通道路由中的偏置诱导交叉，同时相干制备分区持续形变。

Conclusion: 制备几何与慢扇区传播提供了超越Liouvillian本征值的互补操作性信息，丰富了开放量子系统动力学分析的视角。
...
### [115] [Perfect Games in Dimension-Bounded Communication](https://arxiv.org/abs/2608.05092)
*E. Zambrini Cruzeiro*

Main category: quant-ph

TL;DR: 本文研究了具有禁止输出支撑约束的"完美准备-测量游戏"（prepare-and-measure games），建立了两类结构定理：二元输出支撑游戏约化为冲突图（经典完美实现对应d染色，量子完美实现对应d维正交表示）；任意有限输出字母表下，每个完美qubit策略都可被完美经典比特策略实现。并以13射线qutrit图为例构造了压缩游戏，展示了该机制在各维度下的适用性。


<details>
  <summary>Details</summary>
Motivation: 准备-测量游戏中的完美实现问题是连接量子信息、图论与零误差信息论的核心课题。理解完美量子优势何时出现、其结构性条件是什么，对于刻画经典-量子信道能力差距及量子通信协议的最优性具有重要意义。此前缺乏对禁止输出支撑约束下完美实现的系统结构刻画。

Method: 通过将二元输出支撑游戏精确约化为冲突图，利用图的染色理论与正交表示理论分别刻画经典与量子完美实现；利用图的边双团覆盖数（edge biclique-cover number）刻画实现给定冲突图所需的最小Bob输入数。对于任意输出字母表，通过构造经典比特仿真协议证明完美qubit策略的经典比特可实现性。

Result: 证明了二元输出支撑游戏与冲突图之间的精确等价关系；证明了任意有限输出下完美qubit策略可被经典比特完美仿真。利用13射线qutrit图构造了(X,Y,B)=(13,8,2)的压缩游戏，满足C_3=39<Q_3=S=40，且8个Bob输入对该图的所有二元输出实现是最小的。图扩展将机制推广到任意维度，Torpedo与反可区分性（antidistinguishability）游戏展示了非二元输出情形下的独特行为。

Conclusion: 本研究建立了准备-测量游戏、图染色、上下文性（contextuality）、状态排除（state exclusion）与零误差信息论之间的深刻联系，为理解完美量子通信优势提供了统一的图论框架，并为构造最小规模量子优势游戏提供了系统方法。
...
### [116] [Imaginarity as a necessary resource for trainability in QAOA](https://arxiv.org/abs/2608.05093)
*Syed Muhammad Ali Hassan,Kostas Blekos,Stefan Kühn,Nikos Kollas,Karl Jansen*

Main category: quant-ph

TL;DR: 本文证明量子近似优化算法（QAOA）中用于调节电路最终参数的梯度受"虚部量"（imaginarity）所约束，该量衡量候选解之间由电路连接强度和问题评分差异加权的相位关系。虚部量是梯度非零的必要但非充分条件，并将该界限推广到三种常见噪声模型，在Max-Cut模拟中与梯度进行了数值比较。


<details>
  <summary>Details</summary>
Motivation: QAOA通过经典循环中调谐量子电路来解决组合优化问题，常依赖梯度引导。理解梯度的根本约束条件对于算法优化和理论分析至关重要，但目前缺乏对梯度界的深刻理论刻画。

Method: 作者推导了QAOA电路最终参数梯度的上界，证明该梯度受imaginarity（虚部量）约束。该量度加权了候选解之间的相位关系，权重由电路连接强度与问题评分差异共同决定。研究还将此界限扩展到三种常见噪声模型，并通过Max-Cut数值模拟验证。

Result: 证明imaginarity是梯度非零的必要条件但非充分条件；将梯度界扩展到三种噪声模型下，并在Max-Cut模拟中数值比较了虚部量界限与实际梯度。

Conclusion: 虚部量提供了QAOA梯度可调性的理论界标，对理解QAOA的优化动力学、梯度消失现象及噪声影响具有理论意义，但还需进一步探究充分性条件及更复杂问题的推广。
...
### [117] [Fast Quantum Interconnects via Neutral Atom Ensembles](https://arxiv.org/abs/2608.05147)
*Sina Zeytinoglu,Wenchao Xu,Thomas Pohl*

Main category: quant-ph

TL;DR: 该论文提出了一种基于中性原子里德伯态强偶极-偶极相互作用的可扩展量子互连方案，无需光学腔即可在静态量子比特与传播光子之间产生远程纠缠，在现实实验参数下纠缠产生速率可达约3×10^5 s⁻¹。


<details>
  <summary>Details</summary>
Motivation: 在远距离量子比特之间分发纠缠是可扩展量子计算的关键要素，现有方案通常依赖光学腔，限制了速率与可扩展性，需要更高效的量子互连方案。

Method: 利用原子里德伯态之间的强偶极-偶极相互作用，在静态量子比特和传播光子之间产生纠缠，无需光学腔；并对最优实验条件进行了详尽描述，基于镱原子的里德伯态进行了现实参数下的速率评估。

Result: 在现实实验参数下，纠缠产生速率可达约3×10^5 s⁻¹，接近当前中性原子量子处理器双量子比特门的工作速率。

Conclusion: 该量子互连方案具有内在的可扩展性和设计灵活性，为基于中性原子的分布式量子网络提供了一条有前景的技术路径。
...
### [118] [A Reproducible Software Workflow for Unanchored Approximate MUB Optimization: A Case Study in Dimension Six](https://arxiv.org/abs/2607.10615)
*Abdul Fatah,Ian McLoughlin,Saim Ghafoor*

Main category: quant-ph

TL;DR: 本文提出了一种基于李代数酉参数化的可复现工作流，用于在任意维度下优化近似互不偏置基（AMUB）配置，支持多后端（CPU/MPS/CUDA/HPC）执行，并在维度六场景下进行了系统评估，同时在156比特Heron量子处理器上对优化后的单位进行硬件验证。


<details>
  <summary>Details</summary>
Motivation: 互不偏置基（MUB）在量子信息理论中具有基础重要性，但在非素数幂维度中完整MUB的存在性不明确，需要通过数值优化近似MUB配置。现有方法缺乏一个可复现、参数驱动、跨硬件可移植的通用工作流，同时缺少对优化后的AMUB单位在真实量子硬件上可执行性的评估。

Method: 使用李代数酉参数化构建可优化的AMUB配置空间，采用Taylor级数矩阵指数层作为加速器兼容路径，支持CPU、Apple MPS、CUDA GPU和HPC后端。在维度六下，对基数为n=3,4,5,6的未锚定配置进行100个随机种子的优化，使用complex128和complex64两种精度。并将d=6,n=4的过渡单位嵌入到三量子比特8x8单位中，在156比特Heron处理器ibm-marrakesh上通过子空间后选择执行验证。

Result: 工作流能精确恢复三基配置，识别出四基部分精确的枢纽-三角结构，但对于n=5和n=6未在主容差下发现近精确对。硬件实验显示QPU成对损失被约0.02-0.08的硬件和编译噪声底（对应平均37个原生CZ门的编译电路）主导，掩盖了经典近精确对与缺陷对之间的区分。

Conclusion: 该工作提供了一个可复现的计算框架用于探索AMUB地貌，并初步评估了在当前量子硬件上执行维度六优化单位的挑战。结果表明当前量子硬件噪声水平尚不足以区分近精确与有缺陷的AMUB配置。
...
### [119] [General purpose graphical rendering on quantum devices with composable function systems](https://arxiv.org/abs/2608.04022)
*James Schloss,Ayaka Usui*

Main category: quant-ph

TL;DR: 本文提出一种量子兼容的通用渲染方法，通过将经典可组合函数系统（CFSs）扩展到量子架构，实现了量子硬件上的物体渲染，并成功生成了首个在量子架构上渲染的视频。


<details>
  <summary>Details</summary>
Motivation: 量子态创建极具挑战性，历史上无法用其作为复杂场景的渲染目标，尤其在NISQ时代噪声大、可用量子比特少。因此需要探索一种能在当前量子硬件上实现通用渲染的新方法。

Method: 将经典的可组合函数系统（CFSs）扩展适配到量子架构：首先分析经典CFS实现的局限性，然后设计将经典方法适配到量子的物理映射，展示物体基元的生成以及物体变换（包括复制和涂抹，其中部分变换在拓扑上非平凡）。

Result: 结果表明该方法在量子硬件上可能实现加速，并且该方法已被用于创建第一个在量子架构上渲染的视频。

Conclusion: 量子CFS方法为NISQ时代的通用渲染提供了一种可行的新途径，展示了量子渲染的实际可能性，并为未来量子可视化应用奠定了基础。
...
### [120] [From Understanding to Resonance: A Case Study of Quantum Music and Embodied Science Communication](https://arxiv.org/abs/2608.04044)
*Kenji Noda,Akira Koga,Stefan Heusler,Koji Hashimoto*

Main category: quant-ph

TL;DR: 本文通过分析日本Quantum Fest和德国Quantum100两个量子科学传播项目（共吸引4000多名参与者），探讨以沉浸式艺术体验（音乐、视觉艺术、身体参与）作为抽象科学领域传播的替代性入口，并基于调查和访谈识别出共鸣的三个维度，提出以共鸣驱动的方式作为以解释为中心的科普的补充。


<details>
  <summary>Details</summary>
Motivation: 科学传播面临的一大挑战是如何让受众参与高度抽象的、常被视为遥远、技术性和难以接近的科学领域。量子物理正是这类领域的典型代表，需要寻找新的传播路径来突破传统的以解释和概念理解为导向的模式。

Method: 以国际量子科学与技术年期间的日本Quantum Fest和德国Quantum100两个项目为案例，采用事后问卷调查、参与者评论和利益相关者访谈等混合方法收集数据，并对数据进行主题分析以识别共鸣的核心维度。

Result: 识别出共鸣的三个维度：（1）感官沉浸与心理障碍的降低；（2）集体身体化的意义建构；（3）新兴的转化性参与。这些维度表明共鸣驱动的策略能够有效地让受众在概念理解之前、之中或之外建立与科学的有意义联系。

Conclusion: 共鸣驱动的科学传播方法可以补充以解释为中心的传播范式，通过让受众在认知理解之前或超越认知层面与科学建立情感和具身层面的联结，从而为抽象科学领域提供更广泛的公众参与路径。
...
### [121] [Hidden Supersymmetry in Wigner-Yang Quantum Mechanics](https://arxiv.org/abs/2608.04503)
*Georg Junker*

Main category: quant-ph

TL;DR: 本文研究了实数轴上由Wigner于1950年提出的变形海森堡代数所描述的量子系统，其坐标表示由Yang于1951年给出，本质上与Dunkl的差-微分算子相同。作者通过仅考虑相空间中两个运动方程之一来推广Yang的表示，发现相应的无相互作用系统对应Witten的SUSY量子力学模型；在同时施加第二个运动方程后，将Wigner-Yang的原始结果推广到实数轴上的一般对称势，并以谐振子和吸引型类库仑势为例进行讨论，还建立了这两类系统之间的Hooke-Newton对偶性。


<details>
  <summary>Details</summary>
Motivation: Wigner提出的变形海森堡代数及其Yang坐标表示在形式上与Dunkl算子相同，且在一定条件下展现出超对称结构。作者希望推广Yang的表示，探索其与Witten SUSY量子力学模型的联系，并将Wigner-Yang的原始讨论从特定情况推广到一般对称势，同时揭示不同物理系统之间的对偶关系。

Method: 作者通过两个步骤推广Yang的表示：(1) 仅施加相空间两个运动方程之一，得到无相互作用系统，并证明其对应Witten的SUSY量子力学模型；(2) 同时施加两个运动方程，将Wigner-Yang的讨论推广到实数轴上的一般对称势。以谐振子和吸引型类库仑势V(x)=-γ/|x|作为具体例子，并建立Hooke-Newton对偶性。

Result: 作者证明了仅施加一个运动方程时的系统精确对应Witten的SUSY量子力学模型；在完整施加两个运动方程后，成功将Wigner-Yang的结果推广到一般对称势。通过谐振子和类库仑势两个具体例子验证了理论，并确立了类库仑系统与Wigner-Yang谐振子系统之间的Hooke-Newton对偶关系。

Conclusion: 这项工作揭示了Wigner-Yang变形海森堡代数、Dunkl算子与SUSY量子力学之间的深刻联系，并通过Hooke-Newton对偶性将不同类型势场下的量子系统统一起来，为理解这类变形量子系统的超对称结构提供了新的视角。
...
### [122] [Methods for traceable scanning magnetometry using single nitrogen vacancy centers in diamond: determining orientation, distance and localization](https://arxiv.org/abs/2608.04632)
...
TL;DR: 本文报道了一种利用扫描NV显微镜对微图案化垂直磁化条带和圆盘进行成像的方法，无需外部矢量磁体控制即可直接推断NV中心到样品的距离（d_NV = 31.5 nm）及其方位角取向（精度3°），并利用商用硅针成像金刚石纳米结构尖端形貌以检测表面污染，同时通过监测NV荧光随针尖位置的变化来估计NV在金刚石纳米结构内的横向位置。


<details>
  <summary>Details</summary>
Motivation: NV中心在单晶金刚石纳米结构中可实现纳米级磁性杂散场定量成像，但NV中心与样品之间的距离以及NV高对称轴的取向等关键参数通常无法精确已知，只能作为自由拟合参数进入数据评估，这限制了定量测量的精度和可靠性。

Method: 采用扫描NV成像技术对微图案化、垂直磁化的条带和圆盘样品进行测量，从测量数据中直接推断NV-样品距离d_NV和NV的方位角取向，无需外部矢量磁体控制。同时利用商用硅针成像金刚石纳米结构的尖端形貌以检测表面污染，并通过监测NV荧光随针尖位置的变化估计NV在纳米结构内的横向位置。

Result: 确定了NV-样品距离为d_NV = 31.5 nm，推断方位角取向的精度达到3°。此外，成功利用商用硅针成像金刚石纳米结构尖端形貌检测表面污染，并通过NV荧光监测估计了NV在金刚石纳米结构内的横向位置。

Conclusion: 该方法提供了一种无需外部矢量磁体控制即可精确标定NV中心关键几何参数的手段，提升了扫描NV成像的定量测量精度与可操作性，对纳米级磁性成像应用具有重要意义。
